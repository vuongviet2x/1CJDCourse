...
Function ConnectionCheckUp(ExchangePlanName, ExchangePlanNodeCode, ErrorMessage)
	...
	If ExchangePlans[ExchangePlanName].FindByCode(ExchangePlanNodeCode).IsEmpty() Then
		...
		ErrorMessage = NStr("ru = 'Заданный узел плана обмена не найден. Обратитесь к администратору приложения.';
								|en = 'The specified exchange plan node is not found. Please contact the application administrator.';", Common.DefaultLanguageCode());
		Return False;
...
Function ImportDataToInfobase(ExchangePlanName, ExchangePlanNodeCode, FileID, BackgroundJobIdentifier, ErrorMessage)
	...
	ExecutionParameters.BackgroundJobDescription = NStr("ru = 'Загрузка данных в информационную базу через web-сервис ""Enterprise Data Exchange""';
															|en = 'Import data in the infobase using the ""Enterprise Data Exchange"" web service';");
...
Function PrepareDataToImport(ExchangePlanName, ExchangePlanNodeCode, FilePartSize, BackgroundJobIdentifier, ErrorMessage)
	...
	ExecutionParameters = TimeConsumingOperations.BackgroundExecutionParameters(New UUID);
	ExecutionParameters.BackgroundJobDescription = NStr("ru = 'Подготовка к выгрузке данных из информационной базы через web-сервис ""Enterprise Data Exchange""';
															|en = 'Prepare for data export from infobase via ""Enterprise Data Exchange"" web service';");
	ExecutionParameters.BackgroundJobKey = String(New UUID);
