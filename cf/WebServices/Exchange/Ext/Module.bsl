...
Function CreateDataExchange(ExchangePlanName, ParametersString1, FilterSettingAsString, DefaultValuesAsString)
	...
	If Cancel Then
		Message = NStr("ru = 'При создании настройки обмена во второй информационной базе возникли ошибки: %1';
						|en = 'Errors occurred in the peer infobase during the data exchange setup: %1';");
...
Procedure RegisterDataForInitialExport(Val ExchangePlanName, Val NodeCode, TimeConsumingOperation, OperationID, CatalogsOnly)
	...
	Else
		...
		If CatalogsOnly Then
			...
			JobDescription = NStr("ru = 'Регистрация изменений справочников для начальной выгрузки.';
										|en = 'Register catalog changes for initial export.';");
		Else
			MethodName = "DataExchangeServer.RegisterAllDataExceptCatalogsForInitialBackgroundExport";
			JobDescription = NStr("ru = 'Регистрация изменений всех данных кроме справочников для начальной выгрузки.';
										|en = 'Register all data changes except for catalogs for initial export.';");
		EndIf;
		...
		Else
			...
			Raise StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Ошибка при выполнении фонового задания: %1';
					|en = 'Background job error: %1.';"),
				JobDescription);
		EndIf;
