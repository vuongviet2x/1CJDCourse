...
Procedure ImportRules(Command)
	...
	DialogParameters.Insert("Title", NStr("ru = 'Укажите, из какого файла загрузить правила';
												|en = 'Select a file to import rules from';"));
	DialogParameters.Insert("Filter",
		  NStr("ru = 'Файлы правил регистрации (*.xml)';
				|en = 'Registration rule files (*.xml)';") + "|*.xml|"
		+ NStr("ru = 'Архивы ZIP (*.zip)';
...
Procedure UnloadRules(Command)
	...
	If Not Record.RulesAreImported Then
		...
		MessageText = NStr("ru = 'Правила регистрации незагружены';
								|en = 'Registration rules are not imported';");
	...
	NameFilter = NStr("ru = 'Файлы правил (*.xml)';
						|en = 'Rule files (*.xml)';") + "|*.xml";
	...
	If IsBlankString(NameParts.BaseName) Then
		FullFileName = NStr("ru = 'Правила регистрации';
...
Procedure SaveRegistrationRulesFromTemplate(Command)
	...
	If List.Count() = 0 Then
		...
		Template = NStr("ru = 'Для плана обмена ""%1"" не обнаружено ни одного макета правил регистрации';
						|en = 'Cannot find a registration rule template for the ""%1"" exchange plan';", 
	...
	Else
		...
		List.ShowChooseItem(Notification, NStr("ru = 'Выберите макет правил регистрации';
														|en = 'Select a registration rule template';"));
...
Procedure SaveRegistrationRulesFromTemplateCompletion(TemplateName)
	...
	SavingParameters.Dialog.Title = NStr("ru = 'Укажите в какой файл выгрузить правила';
												|en = 'Select a file to export rules to';");
	SavingParameters.Dialog.Filter = NStr("ru = 'Файлы правил (*.xml)';
											|en = 'Rule files (*.xml)';") + "|*.xml";
...
Procedure UpdateRuleTemplateChoiceList()
	...
	If IsBlankString(Record.ExchangePlanName) Then
		...
		Items.MainGroup2.Title = NStr("ru = 'Правила конвертации';
												|en = 'Conversion rules';");
...
Procedure UpdateRuleInfo()
	...
	If Record.RulesSource = Enums.DataExchangeRulesSources.File Then
		...
		StringPattern = NStr("ru = 'Использование правил, загруженных из файла,
	...
	ElsIf Record.RulesSource = Enums.DataExchangeRulesSources.CustomManager Then
		...
		StringPattern = NStr("ru = 'Использование правил, из пользовательского модуля,
                             |может привести к ошибкам при переходе на новую версию программы.
                             |
                             |%1';
							|en = 'Using rules imported from a user module
							|might cause errors when migrating to a new version of the application.
							|
							|%1';");	
	Else
...
Procedure UpdateInformationAboutUnwrittenRules()
	...
	ElsIf RulesSource = Enums.DataExchangeRulesSources.ConfigurationTemplate Then
		...
		RulesInformation = NStr("ru = 'Информация о правилах будет доступна после записи';
									|en = 'Save changes to view rule details';");
		
	ElsIf RulesSource = Enums.DataExchangeRulesSources.File Then
		
		RulesInformation = NStr("ru = 'Информация о правилах будет доступна после загрузки из файла';
									|en = 'Finish importing to view rule details';");
	...
	Else
		...
		Except
			...
			MessageText = NStr("ru = 'При получении информации о правилах регистрации произошла ошибка:
                                   |%1';
									|en = 'Error getting registration rules details:
									|%1';");
...
Procedure ImportRulesCompletion(Val PutFilesResult, Val AdditionalParameters) Export
	...
	If IsBlankString(ErrorText) And IsBlankString(PutFileAddress) Then
		ErrorText = NStr("ru = 'Ошибка передачи файла на сервер';
							|en = 'An error occurred when transferring the file to the server';");
	EndIf;
