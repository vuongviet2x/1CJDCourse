...
Procedure OnOpen(Cancel)
	...
	TooltipTemplate = NStr("ru = 'Комплект правил можно скачать с %1
		|или найти в %2';
		|en = 'You can download a rule set from %1 
		|or find in %2';");
	
	UpdateDirectoryPattern = NStr("ru = 'каталоге поставки программы ""%1""';
									|en = 'the %1 application directory';");
	UpdateDirectoryPattern = StringFunctionsClientServer.SubstituteParametersToString(UpdateDirectoryPattern, ApplicationName);
	
	UserSitePattern = NStr("ru = 'сайта поддержки пользователей системы ""1С:Предприятие 8""';
										|en = '1C:Enterprise support website';");
	...
	If Not IsBlankString(RuleSetLocation.PathToRulesSetFileInTemplateDirectory)
		...
		SuggestionText = NStr("ru = 'Для открытия каталога необходимо необходимо установить расширение для работы с 1С:Предприятием.';
								|en = 'To open the directory, install 1C:Enterprise Extension.';");
...
#If WebClient Then
	...
	Raise NStr("ru = 'Операция в веб-клиенте не предусмотрена';
							|en = 'The operation is not available in web client';");
	...
	If DebugMode Then
		...
		If ExportDebugMode Then
			...
			If Not ValueIsFilled(FileName) Then
				...
				MessageString = NStr("ru = 'Не задано имя файла внешней обработки.';
		...
		If DataExchangeLoggingMode Then
			...
			If Not ValueIsFilled(FileName) Then
				...
				MessageString = NStr("ru = 'Не задано имя файла протокола обмена.';
...
Procedure ExternalDataProcessorForExportDebugStartChoice(Item, ChoiceData, StandardProcessing)
	...
	DialogSettings.Insert("Filter", NStr("ru = 'Внешняя обработка (*.epf)';
...
Procedure ExchangeProtocolFileStartChoice(Item, ChoiceData, StandardProcessing)
	...
	DebuggingSettingsChanged = True;
	DialogSettings = New Structure;
	DialogSettings.Insert("Filter", NStr("ru = 'Текстовый документ (*.txt)';
...
Procedure ImportRules(Command)
	...
	DialogParameters.Insert("Title", NStr("ru = 'Укажите архив с правилами обмена';
												|en = 'Select an exchange rule archive';"));
	DialogParameters.Insert("Filter", NStr("ru = 'Архивы ZIP (*.zip)';
...
Procedure UnloadRules(Command)
	...
	If IsBlankString(NameParts.BaseName) Then
		FullFileName = NStr("ru = 'Правила конвертации';
								|en = 'Conversion rules';");
	Else
...
Procedure WriteAndClose(Command)
	...
	Else
		If ConversionRuleSource = PredefinedValue("Enum.DataExchangeRulesSources.ConfigurationTemplate") Then
			...
			|en = 'Rules are not imported. If you close the form, the default conversion rules will apply.
			|Apply the default rules?';");
			
			Notification = New NotifyDescription("CloseRuleImportForm", ThisObject);
			
			Buttons = New ValueList;
			Buttons.Add("Use", NStr("ru = 'Использовать';
												|en = 'Use';"));
...
Procedure ImportRulesCompletion(Val PutFilesResult, Val AdditionalParameters) Export
	...
	If IsBlankString(ErrorText) And IsBlankString(PutFileAddress) Then
		ErrorText = NStr("ru = 'Ошибка передачи файла настроек синхронизации данных на сервер';
	...
	If Lower(NameParts.Extension) <> ".zip" Then
		CommonClient.MessageToUser(NStr("ru = 'Некорректный формат файла комплекта правил. Ожидается zip архив, содержащий три файла:
			...
			|en = 'Invalid rules file. Expected a ZIP archive with three files:
...
Procedure PerformRuleImport(Val PutFileAddress, Val FileName, ErrorDescription = Undefined)
	...
	If TypeOf(ErrorDescription) <> Type("Boolean") And ErrorDescription <> Undefined Then
		...
		If ErrorDescription.ErrorKind = "InvalidConfiguration" Then
			Buttons.Add("Cancel", NStr("ru = 'Закрыть';
											|en = 'Close';"));
		Else
		...
		FormParameters.PromptDontAskAgain = False;
		If ErrorDescription.ErrorKind = "InvalidConfiguration" Then
			FormParameters.Title = NStr("ru = 'Правила не могут быть загружены';
											|en = 'Cannot import rules';");
		Else
			FormParameters.Title = NStr("ru = 'Синхронизация данных может работать некорректно';
											|en = 'Data synchronization might be performed incorrectly';");
		EndIf;
	...
	Else
		ShowUserNotification(,, NStr("ru = 'Правила успешно загружены в информационную базу.';
...
Function GetRuleArchiveTempStorageAddressAtServer()
	...
	If Result.IsEmpty() Then
		...
		NString = NStr("ru = 'Не удалось получить правила обмена.';
...
Procedure UpdateRuleInfo()
	...
	If RulesSource = "RuelsImportedFromFile" Then
		UsageInformation = NStr("ru = 'Используются правила загруженные из файла.';
										|en = 'Exchange rules imported from a file are applied.';");
	Else
		UsageInformation = NStr("ru = 'Используются типовые правила из состава конфигурации.';
										|en = 'Default configuration exchange rules are used.';");
	EndIf;
