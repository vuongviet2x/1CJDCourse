...
Procedure ShowAccessToObject(Command)
	...
	If Not ValueIsFilled(AccessUpdateObject) Then
		ShowMessageBox(, NStr("ru = 'Выберите объект.';
...
Procedure ScheduleListsAccessUpdate(Command)
	...
	If Not UpdateDataItemsAccessKeys
	   ...
	   And Not DeleteObsoleteInternalData Then
		...
		ShowMessageBox(, NStr("ru = 'Выберите хотя бы один вид обновления.';
										|en = 'Please select at least one update kind.';"));
		Return;
	EndIf;
	
	If Not ScheduleListsAccessUpdateAtServer() Then
		ShowMessageBox(, NStr("ru = 'Выберите хотя бы один список.';
	...
	ShowUserNotification(NStr("ru = 'Обновление запланировано.';
...
Function UpdateAccessToObjectAtServer()
	...
	If Not ValueIsFilled(AccessUpdateObject) Then
		Return NStr("ru = 'Выберите объект.';
					|en = 'Please select an object.';");
	EndIf;
...
Procedure UpdateAccessToObjectForUsersKind(ObjectReference, FullName, TransactionID, ForExternalUsers, Text)
	...
	If ForExternalUsers Then
		If RestrictionParameters.AccessDenied Then
			Text = Text + NStr("ru = 'Для внешних пользователей (доступ запрещен):';
								|en = 'For external users (access denied):';");
			
		ElsIf RestrictionParameters.RestrictionDisabled Then
			Text = Text + NStr("ru = 'Для внешних пользователей (ограничение отключено):';
								|en = 'For external users (restriction disabled):';");
		Else
			Text = Text + NStr("ru = 'Для внешних пользователей:';
								|en = 'For external users:';");
		EndIf;
	Else
		If RestrictionParameters.AccessDenied Then
			Text = Text + NStr("ru = 'Для пользователей (доступ запрещен):';
								|en = 'For users (access denied):';");
			
		ElsIf RestrictionParameters.RestrictionDisabled Then
			Text = Text + NStr("ru = 'Для пользователей (ограничение отключено):';
								|en = 'For users (restriction disabled):';");
		Else
			Text = Text + NStr("ru = 'Для пользователей:';
								|en = 'For users:';");
		EndIf;
	EndIf;
	...
	If RestrictionParameters.DoNotWriteAccessKeys Then
		If RestrictionParameters.AccessDenied
		 Or RestrictionParameters.RestrictionDisabled Then
			Text = Text + Chars.LF + NStr("ru = 'Обновление не требуется. Ключи доступа к объектам этого типа не нужны.';
												|en = 'Update not required. Objects of this type don''t require access keys.';");
		Else
			Text = Text + Chars.LF + NStr("ru = 'Обновление не требуется. Объекты этого типа используют ключи доступа к их владельцам.';
	...
	If SourceAccessKeyObsolete Then
		Text = Text + Chars.LF + NStr("ru = '1. Обновлен ключ доступа к объекту.';
											|en = 'The object access key is updated.';");
	Else
		Text = Text + Chars.LF + NStr("ru = '1. Обновление не требуется. Ключ доступа к объекту не устарел.';
	...
	If HasRightsChanges Then
		If RestrictionParameters.HasUsersRestriction Then
			If ForExternalUsers Then
				Text = Text + Chars.LF + NStr("ru = '2. Обновлены права на ключ доступа у внешних пользователей или групп доступа.';
													|en = '2. Rights to the access key are updated for external users and access groups.';");
			Else
				Text = Text + Chars.LF + NStr("ru = '2. Обновлены права на ключ доступа у пользователей или групп доступа.';
													|en = '2. Rights to the access key are updated for users and access groups.';");
			EndIf;
		Else
			Text = Text + Chars.LF + NStr("ru = '2. Обновлены права на ключ доступа у групп доступа.';
												|en = '2. Rights to the access key are updated for access groups.';");
		EndIf;
	Else
		If RestrictionParameters.HasUsersRestriction Then
			If ForExternalUsers Then
				Text = Text + Chars.LF + NStr("ru = '2. Обновление не требуется. Права на ключ доступа у внешних пользователей и групп доступа не устарели.';
													|en = '2. Update is not required. Rights to the access key for external users and access groups are current.';");
			Else
				Text = Text + Chars.LF + NStr("ru = '2. Обновление не требуется. Права на ключ доступа у пользователей и групп доступа не устарели.';
													|en = '2. Update is not required. Rights to the access key for users and access groups are current.';");
			EndIf;
		Else
			Text = Text + Chars.LF + NStr("ru = '2. Обновление не требуется. Права на ключ доступа у групп доступа не устарели.';
