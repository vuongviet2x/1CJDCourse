...
Procedure FullPathStartChoice(Item, ChoiceData, StandardProcessing)
	...
	Title = NStr("ru = 'Выбор папки для хранения файлов обмена';
					|en = 'Select a folder to store exchange files';");
...
Procedure Save(Command)
	...
	If FilesForDownloading.Count() <> 0 Then
		Title = NStr("ru = 'Выберите каталог для сохранения файлов';
...
Procedure SetConditionalAppearance()
	...
	// Font color for files larger than 100 MB
...
Procedure CheckAvailabilityOfServerDir()
	...
	If HelpDir.Exists() Then
		Common.MessageToUser(NStr("ru = 'Каталог доступен';
													|en = 'Directory is available';"));
	Else
		Common.MessageToUser(NStr("ru = 'Каталог не доступен';
													|en = 'Directory is not available';"));	
	EndIf;
...
Function PrepareFilesAtServer(InfobaseNode, TimeIntervals_)
	...
	While Selection.Next() Do
		...
		If BinaryData = Undefined Then
			...
			Template = NStr("ru = 'Не обнаружено сообщение №%1 от %2';
							|en = 'Message #%1 dated %2 is not found';");
