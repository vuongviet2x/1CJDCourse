...
Procedure AssignPerformers(Command)
	...
	If Purpose = Undefined Then
		ShowMessageBox(,NStr("ru = 'Выберите роль в списке.';
									|en = 'Select a role in the list.';"));
		Return;
	EndIf;
...
Procedure RefreshItemsData()
	...
	For Each ListLine In ListObject Do
		If ListLine.Assignees = 0 Then
			ListLine.PerformersString = ?(ListLine.ExternalRole, NStr("ru = 'заданы в другом приложении';
																				|en = 'specified in another app';"), NStr("ru = 'не заданы';
																														|en = 'not specified';"));
		...
		ElsIf ListLine.Assignees = 1 Then
			...
			ListLine.Picture = -1;
		Else
			ListLine.PerformersString = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = '%1 чел';
																											|en = '%1 user(s)';"), String(ListLine.Assignees) );
			ListLine.Picture = -1;
		EndIf;
	EndDo;
