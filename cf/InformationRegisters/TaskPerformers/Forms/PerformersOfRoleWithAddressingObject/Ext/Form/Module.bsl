...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	Title = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Исполнители роли ""%1""';
																			|en = 'Users assigned to role ""%1""';"), Role);
...
Procedure BeforeWrite(Cancel, WriteParameters)
	For Each LinePerformer In RecordSet Do
		If Not ValueIsFilled(LinePerformer.Performer) Then
			ShowMessageBox(, NStr("ru = 'Укажите исполнителей.';
											|en = 'Specify assignees.';"));
			Cancel = True;
		EndIf;
	EndDo;
EndProcedure
...
Procedure Pick(Command)
	...
	If RoleIsAvailableToExternalUsers And RoleIsAvailableToUsers Then
		Case = New ValueList;
		Case.Add("ExternalUser", NStr("ru = 'Внешний пользователь';
													|en = 'External user';"));
		Case.Add("User", NStr("ru = 'Пользователь';
											|en = 'User';"));
		NotifyDescription = New NotifyDescription("AfterUserTypeChoice", ThisObject);
		Case.ShowChooseItem(NotifyDescription, NStr("ru = 'Выберите тип пользователя';
															|en = 'Select user type';"));
