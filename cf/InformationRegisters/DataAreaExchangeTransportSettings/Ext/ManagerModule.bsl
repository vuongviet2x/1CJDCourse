...
Function TransportSettings(Val Peer) Export
	...
	If Not Common.SubsystemExists("CloudTechnology") Then
		Raise NStr("ru = 'Не определен менеджер настроек транспорта обмена сообщениями.';
								|en = 'The manager of exchange message transport settings is not defined.';");
	EndIf;
...
Function TransportSettingsWS(Val Peer) Export
	...
	If Not ValueIsFilled(SettingsStructure_.WSWebServiceURL) Then
		Raise StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Не заданы настройки подключения веб-сервиса для корреспондента ""%1"".';
				|en = 'Peer infobase ""%1"" connection is not set up.';"),
			String(Peer));
	EndIf;
