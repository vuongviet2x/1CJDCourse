...
Procedure MetadataObjectsTreeBeforeDeleteRow(Item, Cancel)
	...
	If SettingToDelete <> Undefined Then
		...
		If SettingToDeleteParent <> Undefined And SettingToDeleteParent.DetailedInfoAvailable Then
			...
			QueryText = NStr("ru = 'Удаление настройки приведет к прекращению очистки файлов
	...
	MessageText = NStr("ru = 'Расширенная настройка очистки файлов не предусмотрена для этого объекта.';
							|en = 'Advanced file cleanup settings are unavailable for this object.';");
...
Function CanClearVersions()
	If Items.MetadataObjectsTree.SelectedRows.Count() = 1 Then
		...
		If Not CurrentData.IsFile
			...
			ShowMessageBox(, NStr("ru = 'Для данного объекта версии файлов не хранятся.';
											|en = 'File versions are not stored for this object.';"));
			Return False;
		EndIf;
	EndIf;
...
Procedure Clear(Command)
	...
	ShowQueryBox(Notification, NStr("ru = 'Очистить ненужные файлы?
		|
...
Procedure CalculateDeletableFilesInfo()
	...
	ExecutionParameters.RefinementErrors =
		NStr("ru = 'Не удалось подсчитать объем удаляемых файлов по причине:';
			|en = 'Cannot calculate the total size of files to delete:';");
...
Procedure DisplayTotals(TheAmountOfFilesBeingDeleted, IrrelevantFilesVolume)
	Items.DeletedFilesVolume.Title = StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = 'Удаляемые файлы: %1 Мб';
			|en = 'Files being deleted: %1 MB';"),
	...
	Items.IrrelevantFilesVolume.Title = StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = 'Ненужные файлы: %1 Мб';
			|en = 'Unused files: %1 MB';"),
...
Procedure AddSettingsByOwner(ValueSelected)
	...
	For Each Setting In ValueSelected Do
		...
		DetalizedSetting.FilterRule = NStr("ru = 'Изменить правило';
														|en = 'Change rule';");
	EndDo;
...
Procedure ConfigureFilePurgeModes()
	...
	Items.AutomaticallyCleanUpUnusedFiles.Title = ?(UseVolumes, 
		NStr("ru = 'Автоматически очищать:';
			|en = 'Clean up automatically:';"), NStr("ru = 'Автоматически очищать ненужные файлы:';
													|en = 'Clean up unused files automatically:';"));
