...
Procedure ProcessDataForMigrationToNewVersion(Parameters) Export
		...
		Try
			...
			If BusinessProcessState = Undefined Then
				BadData[String.Owner] = StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'В регистре Бизнес-процессы имеются сведения о несуществующем бизнес-процессе ""%1"".';
						|en = 'The ""Business processes"" information register contains records of a non-existent business process: ""%1"".';"),
					String.Owner);
	...
	If ObjectsProcessed = 0 And ObjectsWithIssuesCount <> 0 Then
		MessageText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Не удалось обработать некоторые сведения о бизнес-процессах (пропущены): %1';
				|en = 'Couldn''t process (skipped) some information records about business processes: %1';"), 
	...
	Else
		WriteLogEvent(InfobaseUpdate.EventLogEvent(), EventLogLevel.Information,
			...
			StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Обработана очередная порция сведений о бизнес-процессах: %1';
					|en = 'Another batch of information records about business processes is processed: %1';"),
				ObjectsProcessed));
	EndIf;
