...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Parameters.DataImportRestrictionDates Then
		...
		If Not HasRightToViewDataImportRestrictionDates Then
			Raise(NStr("ru = 'Недостаточно прав для просмотра дат запрета загрузки данных прошлых периодов из других приложений.';
									|en = 'Insufficient rights to view data import restriction dates of previous periods from other applications.';"),
				ErrorCategory.AccessViolation);
		EndIf;
	ElsIf Not HasRightToViewPeriodEndClosingDates Then
		Raise(NStr("ru = 'Недостаточно прав для просмотра дат запрета ввода и редактирования данных прошлых периодов.';
								|en = 'Insufficient rights to view period-end closing dates of previous periods.';"),
			ErrorCategory.AccessViolation);
	EndIf;
	...
	If Parameters.DataImportRestrictionDates Then
		Items.ClosingDatesUsageDisabledLabel.Title =
			NStr("ru = 'Даты запрета загрузки данных прошлых периодов из других приложений отключены в настройках.';
				|en = 'Data import restriction dates of previous periods from other apps are disabled in the settings.';");
		
		Title = NStr("ru = 'Даты запрета загрузки данных';
						|en = 'Data import restriction dates';");
		Items.SetPeriodEndClosingDates.ChoiceList.FindByValue("ForAllUsers").Presentation =
			NStr("ru = 'Для всех информационных баз';
				|en = 'For all infobases';");
		Items.SetPeriodEndClosingDates.ChoiceList.FindByValue("ForSpecifiedUsers").Presentation =
			NStr("ru = 'По информационным базам';
				|en = 'By infobases';");
		
		Items.UsersFullPresentation.Title =
			NStr("ru = 'Приложение: информационная база';
				|en = 'App: infobase';");
		
		Items.UsersComment.ToolTip =
			NStr("ru = 'Описывает причину отдельного запрета для информационной базы или приложения';
				|en = 'Describes a reason for a particular restriction for an infobase or an app';");
		...
		If GetFunctionalOption("UsePeriodClosingDates") Then
			If HasRightToEditPeriodEndClosingDates Then
				GoToOtherClosingDates = StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Также можно настроить <a href=""%1"">даты запрета ввода и редактирования данных</a> прошлых периодов.';
						|en = 'You can also set up <a href=""%1"">period-end closing dates</a> of previous periods.';"),
			...
			ElsIf HasRightToViewPeriodEndClosingDates Then
				GoToOtherClosingDates = StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Также можно посмотреть <a href=""%1"">даты запрета ввода и редактирования данных</a> прошлых периодов.';
						|en = 'You can also view <a href=""%1"">period-end closing dates</a> of previous periods.';"),
					"e1cib/command/InformationRegister.PeriodClosingDates.Command.PeriodEndClosingDates");
			EndIf;
		EndIf;
	Else
		Items.ClosingDatesUsageDisabledLabel.Title =
			NStr("ru = 'Даты запрета ввода и редактирования данных прошлых периодов отключены в настройках приложения.';
				|en = 'Dates of restriction of entering and editing previous period data are disabled in the settings.';");
		Items.UsersFullPresentation.Title = 
			?(GetFunctionalOption("UseUserGroups"),
			NStr("ru = 'Пользователь, группа пользователей';
				|en = 'User, user group';"), NStr("ru = 'Пользователь';
																	|en = 'User';"));
		Items.UsersComment.ToolTip =
			NStr("ru = '1. Определяет порядок групп пользователей при расчете дат запрета
			           |2. Описывает причину отдельного запрета для пользователя или группы пользователей';
						|en = '1. Determines a user group order when calculating period-end closing dates
		...
		UserTypesList.Add(
			Type("CatalogRef.Users"),        NStr("ru = 'Пользователь';
																|en = 'User';"));
		UserTypesList.Add(
			Type("CatalogRef.ExternalUsers"), NStr("ru = 'Внешний пользователь';
																|en = 'External user';"));
		...
		If GetFunctionalOption("UseImportForbidDates") Then
			If HasRightToEditDataImportRestrictionDates Then
				GoToOtherClosingDates = StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Также можно настроить <a href=""%1"">даты запрета загрузки данных</a> из других приложений.';
						|en = 'You can also set up <a href=""%1"">restriction dates</a> for importing data from apps.';"),
			...
			ElsIf HasRightToViewDataImportRestrictionDates Then
				GoToOtherClosingDates = StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Также можно посмотреть <a href=""%1"">даты запрета загрузки данных</a> из других приложений.';
						|en = 'You can also set up <a href=""%1"">restriction dates</a> for importing data from apps.';"),
					"e1cib/command/InformationRegister.PeriodClosingDates.Command.DataImportRestrictionDates");
			EndIf;
		EndIf;
...
Procedure BeforeClose(Cancel, Exit, WarningText, StandardProcessing)
	...
	QueryText = NStr("ru = 'Заданные настройки дат запрета будут автоматически скорректированы.';
						|en = 'The period-end closing date settings will be adjusted automatically.';") 
		+ Chars.LF + Chars.LF + QueryText + Chars.LF + Chars.LF + NStr("ru = 'Закрыть?';
																					|en = 'Close?';");
...
Procedure SetPeriodEndClosingDatesOnChange(Item)
	...
	If CurrentSettingOfPeriodEndClosingDate = "ForSpecifiedUsers" And ValueSelected = "ForAllUsers" Then
		...
		If HasInvalidObjectsByUsers Then
			...
			Raise(NStr("ru = 'Недостаточно прав для изменения дат запрета.';
									|en = 'You are not authorized to edit period-end closing dates.';"), ErrorCategory.AccessViolation);
		EndIf;
			
		QueryText = NStr("ru = 'Отключить все даты запрета, кроме установленных для всех пользователей?
		|
		|Внимание: отключенные настройки будут безвозвратно удалены.';
		|en = 'Do you want to turn off all period-end closing dates except the dates applied for all users?
		|
...
Procedure UsersBeforeDeleteRow(Item, Cancel)
	...
	If ValueIsFilled(CurrentData.Presentation) And Not CurrentData.NoPeriodEndClosingDate Then
		...
		If AdditionalParameters.ClosingDatesForAllUsers Then
			QueryText = NStr("ru = 'Отключить даты запрета для всех пользователей?';
								|en = 'Do you want to turn off period-end closing dates for all users?';");
		Else
			If TypeOf(CurrentData.User) = Type("CatalogRef.Users") Then
				QueryText = StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Отключить даты запрета для ""%1""?';
						|en = 'Do you want to turn off period-end closing dates for ""%1""?';"), CurrentData.User);
				
			ElsIf TypeOf(CurrentData.User) = Type("CatalogRef.UserGroups") Then
				QueryText = StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Отключить даты запрета для группы пользователей ""%1""?';
						|en = 'Do you want to turn off period-end closing dates for ""%1"" user group?';"), CurrentData.User);
				
			ElsIf TypeOf(CurrentData.User) = Type("CatalogRef.ExternalUsers") Then
				QueryText = StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Отключить даты запрета для внешнего пользователя ""%1""?';
						|en = 'Do you want to turn off period-end closing dates for external user ""%1""?';"), CurrentData.User);
				
			ElsIf TypeOf(CurrentData.User) = Type("CatalogRef.ExternalUsersGroups") Then
				QueryText = StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Отключить даты запрета для группы внешних пользователей ""%1""?';
						|en = 'Do you want to turn off period-end closing dates for external user group ""%1""?';"), CurrentData.User);
			Else
				QueryText = NStr("ru = 'Отключить даты запрета?';
									|en = 'Do you want to turn off period-end closing dates?';");
			EndIf;
		EndIf;
		
		QueryText = QueryText + Chars.LF + Chars.LF 
			+ NStr("ru = 'Внимание: отключенные настройки будут безвозвратно удалены.';
					|en = 'Warning: disabled settings will be permanently deleted.';");
...
Procedure ClosingDatesBeforeDeleteRow(Item, Cancel)
	...
	If SeveralSectionsSelected Then
		QueryText = NStr("ru = 'Отключить даты запрета для выбранных разделов?';
							|en = 'Do you want to turn off period-end closing dates for the selected sections?';");
	ElsIf CurrentData.IsSection Then
		If ValueIsFilled(CurrentData.Section) Then
			If CurrentData.GetItems().Count() > 0 Then
				QueryText = StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Отключить все настроенные даты запрета для раздела ""%1"" и его объектов?';
						|en = 'Do you want to turn off all period-end closing dates for section ""%1"" and its objects?';"), CurrentData.Section);
			Else
				QueryText = StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Отключить дату запрета для раздела ""%1""?';
						|en = 'Do you want to turn off the period-end closing date for section ""%1""?';"), CurrentData.Section);
			EndIf;
		Else
			QueryText = NStr("ru = 'Отключить общую дату запрета для всех разделов?';
								|en = 'Do you want to turn off the common-date restriction setting for all sections?';");
		EndIf;
	Else
		QueryText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Отключить дату запрета для объекта ""%1""?';
				|en = 'Do you want to turn off the period-end closing date for object ""%1""?';"), CurrentData.Object);
	EndIf;
	...
	If CurrentData.IsSection Then
		...
		If PeriodEndClosingDateSet(CurrentData, CurrentUser) Or SectionItems.Count() > 0 Then
			...
			ShowQueryBox(New NotifyDescription("ClosingDatesBeforeDeleteSection", ThisObject, CurrentData),
				QueryText, QuestionDialogMode.YesNo);
		Else
			MessageText = NStr("ru = 'Для отключения даты запрета по конкретному объекту выберите интересующий объект в одном из разделов.';
									|en = 'To disable a period-end closing date for an object, select the object in one of the sections.';");
	...
	If Not ValueFound2 Then
		ShowMessageBox(, StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = '""%1"" уже есть в списке объектов';
				|en = '""%1"" is already in the object list';"), ValueSelected));
		Return;
	EndIf;
...
Procedure SetConditionalAppearance()
	...
	Item = ConditionalAppearance.Items.Add();
	Item.Appearance.SetParameterValue("Text", NStr("ru = 'Во всех остальных случаях, когда ниже нет уточняющих настроек.';
																|en = 'Default settings. Effective when there are no overriding settings.';"));
	Item.Appearance.SetParameterValue("TextColor", StyleColors.InaccessibleCellTextColor);
...
Procedure IndicationMethodOfClosingDateChoiceProcessingIdleHandler()
	...
	If CurrentMethod = "BySectionsAndObjects" And ValueSelected = "SingleDate" Then
		QueryText = NStr("ru = 'Отключить даты запрета, установленные для разделов и объектов?';
							|en = 'Do you want to turn off period-end closing dates for sections and objects?';");
	...
	ElsIf CurrentMethod = "BySectionsAndObjects" And ValueSelected = "BySections"
		...
		QueryText = NStr("ru = 'Отключить даты запрета, установленные для объектов?';
							|en = 'Do you want to turn period-end closing dates for objects?';");
	...
	ElsIf CurrentMethod = "BySectionsAndObjects" And ValueSelected = "ByObjects"
		...
		QueryText = NStr("ru = 'Отключить даты запрета, установленные для разделов?';
							|en = 'Do you want to turn off period-end closing dates for sections?';");
	...
	If ValueIsFilled(QueryText) Then
		...
		If HasUnavailableObjects Then
			Raise(NStr("ru = 'Недостаточно прав для изменения дат запрета.';
									|en = 'You are not authorized to change period-end closing dates.';"), ErrorCategory.AccessViolation);
		EndIf;
...
Procedure UsersFullPresentationChoiceProcessingIdleHandler()
	...
	Else
		ShowMessageBox(, StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = '""%1"" уже есть в списке пользователей.';
				|en = '""%1"" is already in the user list.';"), ValueSelected));
	EndIf;
		...
		If CurrentIndicationMethod <> IndicationMethodValueInList.Value 
			...
			ShowQueryBox(
				...
				MessageTextExcessSetting(
					...
					CurrentUser,
					ThisObject) + Chars.LF + Chars.LF + NStr("ru = 'Продолжить?';
																|en = 'Continue?';"),
				QuestionDialogMode.YesNo);
			Return;
		EndIf;
...
Procedure ReadUserData(Form, ErrorText, CurrentIndicationMethod = Undefined, Data = Undefined)
		...
		If FoundRows.Count() > 0 Then
			Form.Items.CurrentUserPresentation.Title =
				StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Настройка для ""%1"":';
						|en = 'Setting for ""%1"":';"), FoundRows[0].Presentation);
		EndIf;
	EndIf;
	...
	If Form.ShowCurrentUserSections Then
		If Form.AllSectionsWithoutObjects Then
			...
			Form.Items.ClosingDatesFullPresentation.Title = NStr("ru = 'Раздел';
																			|en = 'Section';");
			Form.Items.ClosingDates.Representation = TableRepresentation.List;
			
		Else
			Form.Items.ClosingDatesFullPresentation.Title = NStr("ru = 'Раздел, объект';
																			|en = 'Section, object';");
	...
	If ExternalUsersSelectionAndPickup Then
		If Not ExternalUsersCatalogAvailable Then
			Raise(NStr("ru = 'Недостаточно прав для выбора внешних пользователей.';
									|en = 'Insufficient rights to select external users.';"), ErrorCategory.AccessViolation);
		EndIf;
...
Procedure SetVisibility1()
	...
	If Parameters.DataImportRestrictionDates Then
		If SetPeriodEndClosingDates = "ForAllUsers" Then
			ExtendedTooltip = NStr("ru = 'Даты запрета загрузки данных из других приложений действуют одинаково для всех пользователей.';
										|en = 'Data import restriction dates are applied for all users.';");
		Else
			ExtendedTooltip = NStr("ru = 'Персональная настройка дат запрета загрузки данных прошлых периодов из других приложений для выбранных пользователей.';
										|en = 'Customization of data import restriction dates of previous periods for selected users.';");
		EndIf;
	Else
		If SetPeriodEndClosingDates = "ForAllUsers" Then
			ExtendedTooltip = NStr("ru = 'Даты запрета ввода и редактирования данных прошлых периодов действуют одинаково для всех пользователей.';
										|en = 'Dates of restriction of entering and editing previous period data are applied the same way for all users.';");
		Else
			ExtendedTooltip = NStr("ru = 'Персональная настройка дат запрета ввода и редактирования данных прошлых периодов для выбранных пользователей.';
										|en = 'Custom setup of period-end closing dates of previous periods for the selected users.';");
		EndIf;
	EndIf;
...
Procedure EditPeriodEndClosingDateInForm()
	...
	If SelectedRows.Count() = 0 Then
		ShowMessageBox(, NStr("ru = 'Выделенные строки не заполнены.';
										|en = 'The selected lines are not filled in.';"));
		Return;
	EndIf;
	...
	If UpdateSelection Then
		...
		ShowMessageBox(
			...
			NStr("ru = 'Снято выделение с незаполненных строк.';
				|en = 'Unfilled lines are unchecked.';"));
	Else
...
Procedure EditPeriodEndClosingDateInFormCompletion(SelectedRows) Export
	...
	Else
		FormParameters.Object = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Выбранные строки (%1)';
				|en = 'Selected lines (%1)';"), SelectedRows.Count());
	EndIf;
...
Function NoClosingDatePresentation(Data)
	...
	If Data.IsSection And Not Data.Section.IsEmpty() Then
		Return NStr("ru = 'Общая дата для всех разделов';
					|en = 'Common date for all sections';");
	ElsIf Not Data.IsSection Then	
		...
		ElsIf Not ValueIsFilled(SectionData.PeriodEndClosingDate) Then
			Return StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Общая дата для раздела ""%1""';
					|en = 'Common date for section %1';"), SectionData.Presentation);
		Else	
	...
	If Form.Parameters.DataImportRestrictionDates Then
		For Each ListValue In Form.UserTypesList Do
			If TypeOf(ListValue.Value) = TypeOf(User) Then
				...
				Else
					Return ListValue.Presentation + ": " + NStr("ru = '<Все информационные базы>';
																		|en = '<All infobases>';");
				EndIf;
			EndIf;
		EndDo;
...
Function MessageTextInSelectedSectionClosingDatesForObjectsNotSet(Section)
	...
	Return ?(Section <> SectionEmptyRef, 
		StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'В разделе ""%1"" не предусмотрена установка дат запрета для отдельных объектов.';
				|en = 'Period-end closing date setting is not available for separate objects in the ""%1"" section.';"), Section),
		NStr("ru = 'Для установки дат запрета по отдельным объектам выберите один из разделов ниже и нажмите ""Подобрать"".';
			|en = 'To set a period-end closing date for particular objects, select one of the sections below, and then click Pick.';"));
...
Function HeaderTextDataTypeSelection()
	...
	Return NStr("ru = 'Выбор типа данных';
				|en = 'Select data type';");
