	...
	For Each CurRow In SpreadsheetDocumentChangeTable Do
		...
		For IndexOf = 0 To UBound Do
			...
			If CurRow[ColumnName] = "And" Then
				...
				TextTemplate1 = NStr("ru = 'сравнить версию №%1 с версией №%2';
									|en = 'compare version #%1 with version #%2';");
			...
			ElsIf CurRow[ColumnName] = "U" Then
				...
				Area.Text = NStr("ru = 'Сохранение изменений табличного документа отключено';
									|en = 'Saving changes is disabled for spreadsheet documents';");
				
			ElsIf CurRow[ColumnName] = "D" Then
				Area = ReportTS.Join(TemplateCellAdded);
				Area.Text = NStr("ru = 'открыть';
									|en = 'open';");
...
Procedure OutputHeader(ReportTS, VersionsList, VersionsCount, CommonTemplate, ObjectReference)
	...
	SectionHeader.Parameters.ReportDescription1 = NStr("ru = 'Отчет по изменениям версий объекта';
													|en = 'Object version delta report';");
...
Function GetVersionDetails(ObjectReference, VersionNumber)
	...
	LongDesc = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = '№ %1 / (%2) / %3';
																			|en = '#%1 / (%2) / %3';"), VersionNumber.Presentation, 
...
Procedure AddRowNumbersToTabularSections(TabularSections)
	...
	For Each Map In TabularSections Do
		...
		If Table.Columns.Find("LineNumber") <> Undefined Then
			Continue;
		EndIf;
		Table.Columns.Insert(0, "LineNumber",,NStr("ru = '№ строки';
														|en = '#';"));
...
Procedure ProgressDeletingSyncAlerts(Val CurrentStep, Maximum, SampleIterator = 0)
	...
	If SampleIterator = 0 Then
		...
		Template = NStr("ru = 'Выполнено итераций %1 из %2';
						|en = '%1 out of %2 iterations completed';",  Common.DefaultLanguageCode());
		ProgressText = StringFunctionsClientServer.SubstituteParametersToString(Template, CurrentStep, Maximum);
		
	Else
		
		Template = NStr("ru = 'Выполнено итераций %1 из %2 (%3)';
						|en = '%1 out of %2 iterations completed (%3)';",  Common.DefaultLanguageCode());
...
Procedure RemovePreviousWarningExchanges(ObjectVersionInfo) Export
	...
	// The following approaches are accepted:
	// 1. If the warning type relates to synchronization, clean up by type.
	// 2. If synchronization comments are found, clear all similar comments.
	//
	// In both approaches, delete records where the conditions match and the user is their author
	// since it is impossible to determine which exchange the warning belongs to.
