...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	Items.NoVersions.Title = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Предыдущие версии отсутствуют: ""%1"".';
																								|en = 'Earlier versions are not available: %1.';"), String(Ref));
	RefreshVersionList();
	...
	Attributes = NStr("ru = 'Все';
					|en = 'All';");
	Title = NStr("ru = 'История изменений:';
					|en = 'Change history:';") + " " + Ref;
...
#Region FormTableItemsEventHandlersVersionsList
...
Procedure GenerateReportOnChanges(Command)
	...
	If VersionsToCompare.Count() < 2 Then
		ShowMessageBox(, NStr("ru = 'Для формирования отчета по изменениям выберите хотя бы две версии.';
										|en = 'To generate a delta report, select at least two versions.';"));
		Return;
	EndIf;
...
Procedure GoToSelectedVersion(CancelPosting = False)
	...
	ElsIf Result = "PostingError" Then
		...
		Buttons.Add("GoTo", NStr("ru = 'Перейти';
										|en = 'Yes';"));
	...
	Else //Result = "RestoringComplete"
		...
		ShowUserNotification(
			StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Восстановлена версия № %1.';
																		|en = 'Version #%1 is restored.';"), VersionNumberPresentation),
			GetURL(Ref),
			String(Ref),
