...
Procedure UpdateTemplatesCheckSum() Export
	...
	For Each TemplateDetails In TemplatesToProcess Do
		...
		Except
			...
			ErrorText = NStr("ru = 'Не удалось записать сведения о макете';
								|en = 'Failed to save template info';") + Chars.LF
				+ Template.FullName() + Chars.LF
				+ ErrorProcessing.DetailErrorDescription(ErrorInfo);
			
			WriteLogEvent(NStr("ru = 'Контроль изменения поставляемых макетов';
											|en = 'Build-in template edit monitor';", Common.DefaultLanguageCode()),
	...
	If ValueIsFilled(ErrorList) Then
		ErrorList.Insert(0, NStr("ru = 'Не удалось записать сведения о макетах печатных форм конфигурации:';
...
Procedure Print(ObjectsArray, PrintParameters, PrintFormsCollection, PrintObjects, OutputParameters) Export
	...
	If PrintForm <> Undefined Then
		PrintForm.TemplateSynonym = NStr("ru = 'Как сделать факсимильную подпись и печать';
		...
		If PrintParameters.ScanAvailable And Common.SubsystemExists("StandardSubsystems.FilesOperations") Then
			RefArea.Text = NStr("ru = 'Сканировать';
										|en = 'Scan';");
