...
Procedure MetadataObjectsTreeSynchronizeOnChange(Item)
	...
	If Not CurrentData.Synchronize And CurrentData.PreviousSynchronization Then
		...
		MessageText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Завершить и отключить синхронизацию файлов для ""%1""?';
				|en = 'Do you want to finish and disable file synchronization for ""%1""?';"), 
...
Procedure MetadataObjectsTreeBeforeDeleteRow(Item, Cancel)
	...
	QueryText = NStr("ru = 'Удаление настройки приведет к прекращению синхронизации 
	...
	ShowQueryBox(NotifyDescription, QueryText, QuestionDialogMode.YesNo, , DialogReturnCode.No, NStr("ru = 'Предупреждение';
																												|en = 'Warning';"));
...
Procedure ItemSynchronization(Command)
	...
	If Not TreeRow.DetailedInfoAvailable Then
		MessageText = NStr("ru = 'Добавление настройки возможно только для иерархических справочников';
								|en = 'The setting is only available for hierarchical catalogs.';");
	...
	ChoiceFormParameters.Insert("PickFormHeader", NStr("ru = 'Подбор элементов настроек';
																|en = 'Select settings items';"));
	...
	For Each OwnerInfo1 In FilesOwnersTable Do
		...
		If AllCatalogs.ContainsType(OwnerInfo1.FileOwner) Then
			If CatalogsNode = Undefined Then
				...
				CatalogsNode.ObjectDescriptionSynonym = NStr("ru = 'Справочники';
																	|en = 'Catalogs';");
			EndIf;
		...
		ElsIf AllDocuments.ContainsType(OwnerInfo1.FileOwner) Then
			If DocumentsNode = Undefined Then
				...
				DocumentsNode.ObjectDescriptionSynonym = NStr("ru = 'Документы';
																|en = 'Documents';");
			EndIf;
		...
		ElsIf BusinessProcesses.AllRefsType().ContainsType(OwnerInfo1.FileOwner) Then
			If BusinessProcessesNode = Undefined Then
				...
				BusinessProcessesNode.ObjectDescriptionSynonym = NStr("ru = 'Бизнес-процессы';
		...
		If DetailedSettings.Count() > 0 Then
			For Each Setting In DetailedSettings Do
				...
				DetalizedSetting.FilterRule    =
					?(HasFilterRules, NStr("ru = 'Выбранные файлы';
												|en = 'Selected files';"), NStr("ru = 'Все файлы';
																				|en = 'All files';"));
		...
		If FoundSettings.Count() > 0 Then
			...
			If FilterRule <> Undefined And FilterRule.Filter.Items.Count() > 0 Then
				NewTableRow.FilterRule = ?(IsBlankString(FoundSettings[0].Description), 
					NStr("ru = 'Выбранные файлы';
						|en = 'Selected files';"), FoundSettings[0].Description);
			Else
				NewTableRow.FilterRule = NStr("ru = 'Все файлы';
														|en = 'All files';");
			EndIf;
...
Procedure SetFilterSettings(ValueSelected, AdditionalParameters) Export
	...
	If ValueSelected.HasFilterRules Then
		RowToRefresh.FilterRule =
			?( ValueIsFilled(ValueSelected.Description), ValueSelected.Description, NStr("ru = 'Выбранные файлы';
																										|en = 'Selected files';"));
	Else
		RowToRefresh.FilterRule = NStr("ru = 'Все файлы';
												|en = 'All files';");
	EndIf;
...
Function ClearSettingData()
	...
	BackgroundExecutionParameters.BackgroundJobDescription = NStr("ru = 'Подсистема ""Работа с файлами"": Отключение синхронизации файлов с облачным сервисом';
																|en = 'Subsystem ""File management"": Disable file synchronization with cloud service';");
...
Procedure RunScheduledJob()
	...
	Else
		...
		JobParameters.BackgroundJobDescription = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Запуск вручную: %1';
																													|en = 'Manual start: %1';"), ScheduledJobMetadata1.Synonym);
