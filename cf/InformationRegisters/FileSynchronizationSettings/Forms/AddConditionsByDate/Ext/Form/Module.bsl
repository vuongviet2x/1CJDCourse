...
Procedure OK(Command)
	...
	If IntervalException = 0 Then
		CommonClient.MessageToUser(NStr("ru = 'Количество дней не может быть равно 0.';
														|en = 'Please specify a nonzero number of days.';"),,, "IntervalException");
		Return;
	EndIf;
	
	If Not ValueIsFilled(DateTypeAttribute) Then
		CommonClient.MessageToUser(NStr("ru = 'Заполните условия очистки файлов.';
														|en = 'Fill in file cleanup conditions.';"),,, "DateTypeAttribute");
		Return;
	EndIf;
