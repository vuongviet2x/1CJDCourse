...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	Title = NStr("ru = 'Перезапуск отложенного обновления';
					|en = 'Restart deferred update';");
EndProcedure
...
Procedure AfterSelectHandlers(Result, AdditionalParameters) Export
	...
	If SelectedHandlers.Count() = 0 Then
		Items.HyperlinkSelectionHandlers.Title = NStr("ru = 'Выбрать обработчики';
																|en = 'Select handlers';");
	Else
		Items.HyperlinkSelectionHandlers.Title = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Выбрано обработчиков: %1';
				|en = 'Handlers selected: %1';"),
