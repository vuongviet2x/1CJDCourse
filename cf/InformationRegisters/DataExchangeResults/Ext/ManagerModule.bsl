...
Procedure ClearSynchronizationWarnings(TimeConsumingOperationParameters, StorageAddress = Undefined) Export
	...
	ProgressText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Удалено %1 из %1';
				|en = 'Deleted %1 of %1';", Common.DefaultLanguageCode()),
...
Function TitleOfWarningsByNumber(NumberOfWarnings)
	...
	If NumberOfWarnings > 100 Then
		...
		WarningTitle = NStr("ru = 'предупреждений более 100';
										|en = 'more than 100 warnings';", Common.DefaultLanguageCode());
	...
	Else
		...
		WarningTitle = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'предупреждений %1';
				|en = '%1 warnings';", Common.DefaultLanguageCode()),
			NumberOfWarnings);
...
Procedure ProgressDeletingSyncAlerts(Val CurrentStep, Maximum, SampleIterator = 0)
	...
	If SampleIterator = 0 Then
		...
		ProgressText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Выполнено итераций %1 из %2';
	...
	Else
		...
		ProgressText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Выполнено итераций %1 из %2 (%3)';
...
Procedure ProcessDataForMigrationToNewVersion(Parameters) Export
	...
	While Selection.Next() Do
		...
		Except
			...
			MessageText = StringFunctionsClientServer.SubstituteParametersToString(
				...
				|%2';
				|en = 'Couldn''t process the register record set ""%1"" due to:
	...
	If Processed = 0 And RecordsWithIssuesCount <> 0 Then
		MessageText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Процедуре РегистрыСведений.ResultыОбменаДанными.ОбработатьДанныеДляПереходаНаНовуюВерсию не удалось обработать некоторые записи. Пропущены: %1';
				|en = 'Procedure InformationRegisters.DataExchangeResults.ProcessDataForMigrationToNewVersion failed to process (skipped) some records: %1';"), 
	...
	Else
		WriteLogEvent(InfobaseUpdate.EventLogEvent(), EventLogLevel.Information,
			...
			NStr("ru = 'Процедура РегистрыСведений.ResultыОбменаДанными.ОбработатьДанныеДляПереходаНаНовуюВерсию обработала очередную порцию записей: %1';
				|en = 'The InformationRegisters.DataExchangeResults.ProcessDataForMigrationToNewVersion procedure processed records: %1';"),
			Processed));
	EndIf;
