...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Common.DataSeparationEnabled() And VersioningUsed Then
		Items.ConflictsOtherVersionAuthor.Title = NStr("ru = 'Версия получена из приложения';
															|en = 'The version is received from the application';");
	EndIf;
...
Procedure AcceptVersionDeclined(Command)
	...
	ShowQueryBox(NotifyDescription, NStr("ru = 'Принять версию, несмотря на запрет загрузки?';
											|en = 'Do you want to accept the version even though import is restricted?';"), QuestionDialogMode.YesNo, , DialogReturnCode.No);
...
Procedure ChangeConflictResult(Command)
	...
	If Items.Conflicts.CurrentData <> Undefined Then
		...
		If Items.Conflicts.CurrentData.OtherVersionAccepted Then
			...
			QueryText = NStr("ru = 'Заменить версию, полученную из другой программы, на версию из этой программы?';
								|en = 'Do you want to replace the version from another application with the version from this application?';");
			
		Else
			
			QueryText = NStr("ru = 'Заменить версию этой программы на версию, полученную из другой программы?';
								|en = 'Do you want to replace the version from this application with the version from another application?';");
...
Procedure PostDocuments(Val SelectedRows, ErrorMessage = "")
		...
		Except
			...
			If Not LockSet Then
				Common.MessageToUser(StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Не удалось провести документ ""%1"" по причине:
					|%2.';
					|en = 'Cannot post the document ""%1"" due to:
					|%2.';"),
	...
	If UnpostedDocumentsCounter > 0 Then
		ErrorMessage = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Не проведено документов: %1 из %2.';
				|en = 'Not posted documents: %1 from %2.';"),
...
Procedure FillNodeList()
	...
	If NoneExchangeByRules And NoneXDTOExchange And NoneStandardExchange Then
		...
		Title = NStr("ru = 'Конфликты при синхронизации данных';
						|en = 'Data synchronization conflicts';");
...
Procedure ShowDifferences(Item)
	...
	If VersionsToCompare.Count() <> 2 Then
		...
		CommonClient.MessageToUser(NStr("ru = 'Нет версии для сравнения.';
														|en = 'No object version to compare.';"));
		Return;
...
Procedure UpdatePageTitles()
	...
	SetPageTitle(Items.UnpostedDocumentsPage, NStr("ru = 'Непроведенные документы';
																				|en = 'Unposted documents';"), UnpostedDocumentCount());
	SetPageTitle(Items.BlankAttributesPage, NStr("ru = 'Незаполненные реквизиты';
																				|en = 'Blank attributes';"), BlankAttributeCount());
	SetPageTitle(Items.XDTOErrorPage,             NStr("ru = 'Ошибки проверки конвертируемых объектов';
																				|en = 'An error occurred when checking the objects being converted';"), XDTOErrorsCount());	
	
	If VersioningUsed Then
		SetPageTitle(Items.ConflictPage, NStr("ru = 'Конфликты';
																	|en = 'Conflicts';"), ConflictCount());
		SetPageTitle(Items.RejectedByRestrictionDatePage, NStr("ru = 'Непринятые по дате запрета';
																					|en = 'Items rejected due to restriction date';"), NotAcceptedCount());
	EndIf;
...
Procedure AcceptRejectVersionAtServer(Val SelectedRows, TagName, ErrorMessage = "")
	...
	For Each SelectedRow In SelectedRows Do
		...
		Else
			...
			Except
				...
				Common.MessageToUser(StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Не удалось принять версию объекта ""%1"" по причине:
					|%2.';
					|en = 'Cannot accept the object version ""%1"" due to:
					|%2.';"),
					ObjectPresentation,
					ErrorProcessing.BriefErrorDescription(ErrorInfo())));
				RejectedDocumentsCounter = RejectedDocumentsCounter + 1;
	...
	If RejectedDocumentsCounter > 0 Then
		ErrorMessage = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Не принято версий объектов: %1 из %2.';
				|en = 'Not accepted object versions: %1 from %2.';"),
...
Procedure SetConditionalAppearance()
	...
	Item.Appearance.SetParameterValue("Text", NStr("ru = 'Удалена';
																|en = 'Deleted';"));
	...
	Item.Appearance.SetParameterValue("Text", NStr("ru = 'Отсутствуют';
																|en = 'Missing';"));
