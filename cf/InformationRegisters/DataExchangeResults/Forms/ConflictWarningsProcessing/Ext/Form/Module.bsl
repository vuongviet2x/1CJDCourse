...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Not Parameters.Property("DataForSelectedRows") Then
		...
		Raise NStr("ru = 'Самостоятельное использование формы не предусмотрено.';
								|en = 'The form cannot be used independently.';", Common.DefaultLanguageCode());
...
Procedure ReviseResult(Command)
	...
	QueryText = NStr("ru = 'Пересмотреть результаты коллизий по отмеченным строкам?';
						|en = 'Revise the conflict results for the marked lines?';");
...
Procedure ReviewTheResultsOfCollisions(Response, AdditionalParameters) Export
	...
	If RevisionParameters.NumberOfRevised <> RevisionParameters.TotalCount1 Then
		...
		MessageTemplate = NStr("ru = 'Отмечено строк: %1. Изменено объектов: %2.';
								|en = 'Lines selected: %1. Objects changed: %2.';", CommonClient.DefaultLanguageCode());
...
Procedure ShowDifferencesBetweenObjectVersions()
	...
	If VersionsToCompare.Count() <> 2 Then
		...
		MessageText = NStr("ru = 'Нет версии для сравнения.';
								|en = 'No object version to compare.';", CommonClient.DefaultLanguageCode());
...
Procedure ReviewTheResultsOnTheServer(RevisionParameters)
	...
	ErrorTextTemplate = NStr("ru = 'Не удалось изменить версию объекта по причине:%1%2';
								|en = 'Cannot change the object version due to:%1%2';", Common.DefaultLanguageCode());
