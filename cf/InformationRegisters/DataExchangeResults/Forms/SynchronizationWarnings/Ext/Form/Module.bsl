...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	List.Parameters.SetParameterValue("UnpostedDocument", NStr("ru = 'Непроведенный документ';
																				|en = 'Unposted document';", Common.DefaultLanguageCode()));
	List.Parameters.SetParameterValue("EmptyAttributes", NStr("ru = 'Пустые реквизиты';
																		|en = 'Empty attributes';", Common.DefaultLanguageCode()));
	List.Parameters.SetParameterValue("CheckBeforeSend", NStr("ru = 'Проверка перед отправкой';
																				|en = 'Check before sending';", Common.DefaultLanguageCode()));
	List.Parameters.SetParameterValue("TechnicalErrorSend", NStr("ru = 'Тех. ошибка (отправка)';
																					|en = 'Technical error (sending)';", Common.DefaultLanguageCode()));
	List.Parameters.SetParameterValue("TechnicalErrorGet", NStr("ru = 'Тех. ошибка (получение)';
																					|en = 'Technical error (receipt)';", Common.DefaultLanguageCode()));
	
	If TheVersioningSubsystemExists Then
		
		List.Parameters.SetParameterValue("UnrecognizedExisting", NStr("ru = 'Непринятый, существующий';
																					|en = 'Rejected, existing';", Common.DefaultLanguageCode()));
		List.Parameters.SetParameterValue("RejectedNew", NStr("ru = 'Непринятый, новый';
																			|en = 'Rejected, new';", Common.DefaultLanguageCode()));
		List.Parameters.SetParameterValue("AcceptedCollisionData", NStr("ru = 'Приняты данные коллизии';
																					|en = 'Received conflict data';", Common.DefaultLanguageCode()));
		List.Parameters.SetParameterValue("RejectedCollisionData", NStr("ru = 'Неприняты данные коллизии';
																						|en = 'Rejected conflict data';", Common.DefaultLanguageCode()));
...
Procedure EventLog(Command)
	...
	WarningsLevels.Add(NStr("ru = 'Ошибка';
										|en = 'Error';", CommonClient.DefaultLanguageCode()));
	WarningsLevels.Add(NStr("ru = 'Предупреждение';
										|en = 'Warning';", CommonClient.DefaultLanguageCode()));
...
Procedure EventLogAtSpecifiedTime(Command)
	...
	If Not ValueIsFilled(CurrentRowData.OccurrenceDate) Then
		...
		WarningText = NStr("ru = 'В текущей строке не указано время и дата возникновения предупреждения';
									|en = 'The current line does not contain the time and warning date specified';", CommonClient.DefaultLanguageCode());
...
Function CurrentWarningListData()
	...
	If CurrentRowData = Undefined
		...
		ExceptionText = NStr("ru = 'Для текущей строки выполнение команды не предусмотрено';
								|en = 'No command execution is provided for the current line';", CommonClient.DefaultLanguageCode());
		Raise ExceptionText;
...
Procedure OpenTheObjectOfTheCurrentLine(CurrentRowData)
	...
	If Not ValueIsFilled(CurrentRowData.ObjectWithIssue) Then
		...
		WarningText = NStr("ru = 'Предупреждение в текущей строке не привязано к объекту информационной базы';
									|en = 'Warning in the current line is not linked to an infobase object';", CommonClient.DefaultLanguageCode());
		ShowMessageBox(Undefined, WarningText);
		Return;
...
Procedure FilterByPeriodPresentation()
	...
	If Not ValueIsFilled(ValuesCache.SelectionByDateOfOccurrence) Then
		...
		FilterByPeriodPresentation = NStr("ru = 'Отбор не установлен';
											|en = 'Filter not set';", CommonClient.DefaultLanguageCode());
...
Procedure RepresentationOfTheSelectionOfExchangeNodes()
	...
	If ValuesCache.SelectionOfExchangePlanNodes = Undefined
		...
		SynchronizationsFilterPresentation = NStr("ru = 'Отбор не установлен';
												|en = 'Filter not set';", CommonClient.DefaultLanguageCode());
	...
	Else
		...
		TextTemplate1 = NStr("ru = '%1%2 (и еще %3 шт.)';
							|en = '%1%2 (and %3 more pcs)';", CommonClient.DefaultLanguageCode());
...
Procedure RepresentationOfTheSelectionByTypesOfWarnings()
	...
	If ValuesCache.SelectingTypesOfWarnings = Undefined
		...
		WarningsTypesFilterPresentation = NStr("ru = 'Отбор не установлен';
														|en = 'Filter not set';", CommonClient.DefaultLanguageCode());
	...
	ElsIf ValuesCache.SelectingTypesOfWarnings.Count() > 8 Then
		...
		WarningsTypesFilterPresentation = NStr("ru = 'Все типы предупреждений';
														|en = 'All warning types';", CommonClient.DefaultLanguageCode());
...
Procedure SelectionByDateOfOccurrence()
	...
	RepresentationOfTheSelectionGroup = NStr("ru = 'Отбор по дате возникновения';
									|en = 'Filter by issue date';", Common.DefaultLanguageCode());
	...
	ParametersOfTheSelectionElement.Insert("FilterPresentation", NStr("ru = 'Отбор даты с';
																|en = 'Filter date from';", Common.DefaultLanguageCode()));
	...
	ParametersOfTheSelectionElement.FilterPresentation = NStr("ru = 'Отбор даты по';
														|en = 'Filter date to';", Common.DefaultLanguageCode());
...
Procedure SelectionByNodesOfTheInformationBase()
	...
	ParametersOfTheSelectionElement.Insert("FilterPresentation", NStr("ru = 'Отбор синхронизаций';
																|en = 'Synchronization filter';", Common.DefaultLanguageCode()));
	ParametersOfTheSelectionElement.Insert("Use", Use);
...
Procedure SelectionByTypesOfWarnings()
	...
	ParametersOfTheSelectionElement.Insert("FilterPresentation", NStr("ru = 'Отбор по типу предупреждения';
																|en = 'Filter by warning type';", Common.DefaultLanguageCode()));
	ParametersOfTheSelectionElement.Insert("Use", Use);
...
Procedure SelectionOfHiddenWarnings()
	...
	ParametersOfTheSelectionElement.Insert("FilterPresentation", NStr("ru = 'Отбор скрытых предупреждений';
																|en = 'Filter hidden warnings';", Common.DefaultLanguageCode()));
	ParametersOfTheSelectionElement.Insert("Use", Use);
...
Procedure NavigateDocumentsThroughTheSelectedLines(SelectedRows)
	...
	If DataForSelectedRows.Count() = 0 Then
		...
		ErrorMessage = NStr("ru = 'В выделенных строках нет предупреждений с типом <Непроведенный документ>';
								|en = 'There are no warnings with the <Unposted document> type in the selected lines';", CommonClient.DefaultLanguageCode());
...
Procedure CollisionsOnSelectedLines(SelectedRows)
	...
	If DataForSelectedRows.Count() = 0 Then
		...
		ErrorMessage = NStr("ru = 'В выделенных строках нет предупреждений с типами <Неприняты данные коллизии>, <Приняты данные коллизии>';
								|en = 'There are no warnings with the <Rejected conflict data> and <Received conflict data> types in the selected lines';", CommonClient.DefaultLanguageCode());
		ShowMessageBox(Undefined, ErrorMessage);
		Return
...
Procedure ForbiddingLoadingBySelectedLines(SelectedRows)
	...
	If DataForSelectedRows.Count() = 0 Then
		...
		ErrorMessage = NStr("ru = 'В выделенных строках нет предупреждений с типами <Непринятый, существующий>, <Непринятый, новый>,
			|которые возникли из-за попытки загрузить данных в закрытый период.';
			|en = 'The selected lines do not contain warnings with the <Rejected, existing> and <Rejected, new> types
...
Procedure GroupChangeOfDetailsForSelectedLines(SelectedRows)
	...
	If DataForSelectedRows.Count() = 0 Then
		...
		ErrorMessage = NStr("ru = 'В выделенных строках нет предупреждений с типами <Пустые реквизиты>.';
								|en = 'There are no warnings with the <Empty attributes> types in the selected lines.';", CommonClient.DefaultLanguageCode());
		ShowMessageBox(Undefined, ErrorMessage);
		Return
