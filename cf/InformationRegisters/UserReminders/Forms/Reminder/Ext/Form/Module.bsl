...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If UserRemindersClientServer.IsMessageURL(Object.Id) Then
		...
		If BegOfDay(Message.Date) = BegOfDay(CurrentSessionDate()) Then
			DataAsText = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'сегодня %1';
																						|en = 'today %1';"), Format(Message.Date, NStr("ru = 'ДФ=HH:mm';
																																		|en = 'DF=HH:mm';")));
		Else
			DataAsText = Format(Message.Date, NStr("ru = 'ДФ=dd.MM.yyyy HH:mm';
														|en = 'DF=MM/dd/yyyy HH:mm';"));
		EndIf;
		
		MessageDetails = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Сообщение от %1, полученное %2';
				|en = 'Message from %1 received on %2';"),
...
Procedure OnOpen(Cancel)
	...
	If Object.ReminderTimeSettingMethod = PredefinedValue("Enum.ReminderTimeSettingMethods.RelativeToCurrentTime") Then
		ReminderTimeSettingMethod = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'через %1';
				|en = 'in %1';"), UserRemindersClient.TimePresentation(Object.ReminderInterval));
	Else
...
Procedure ReminderTimeSettingMethodOnChange(Item)
	...
	If TimeInterval > 0 Then
		...
		ReminderTimeSettingMethod = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'через %1';
				|en = 'in %1';"), TimeIntervalString);
	Else
		If Items.ReminderTimeSettingMethod.ChoiceList.FindByValue(ReminderTimeSettingMethod) = Undefined Then
			CommonClient.MessageToUser(NStr("ru = 'Интервал времени не определен.';
															|en = 'Please specify the time interval.';"), , "ReminderTimeSettingMethod");
		EndIf;
	EndIf;
...
Procedure OnChangeTimeInterval(Item)
	...
	Else
		CommonClient.MessageToUser(NStr("ru = 'Интервал времени не определен';
														|en = 'Please specify the time interval.';"), , "TimeIntervalString");
	EndIf;
EndProcedure
...
Procedure Delete(Command)
	...
	DialogButtons.Add(DialogReturnCode.Yes, NStr("ru = 'Прекратить';
														|en = 'Dismiss';"));
	DialogButtons.Add(DialogReturnCode.Cancel, NStr("ru = 'Не прекращать';
															|en = 'Not now';"));
	
	NotifyDescription = New NotifyDescription("DeleteReminder", ThisObject);
	ShowQueryBox(NotifyDescription, NStr("ru = 'Прекратить напоминание?';
											|en = 'Dismiss the reminder?';"), DialogButtons);
...
Function GetPredefinedNotificationMethods()
	Result = New Map;
	Result.Insert(NStr("ru = 'относительно предмета';
							|en = 'based on subject';"), 
		PredefinedValue("Enum.ReminderTimeSettingMethods.RelativeToSubjectTime"));
	Result.Insert(NStr("ru = 'в указанное время';
							|en = 'at specified time';"), 
		PredefinedValue("Enum.ReminderTimeSettingMethods.AtSpecifiedTime"));
	Result.Insert(NStr("ru = 'периодически';
							|en = 'periodically';"), 
...
Procedure FillPeriodicityOptions()
	...
	Items.FrequencyOption.ChoiceList.Add("", NStr("ru = 'по заданному расписанию...';
																|en = 'custom schedule…';"));	
EndProcedure
...
Procedure OpenScheduleSettingDialogCompletion(SelectedSchedule, AdditionalParameters) Export
	...
	If Not ScheduleMeetsRequirements(Schedule) Then 
		ShowMessageBox(, NStr("ru = 'Периодичность в течение дня не поддерживается, соответствующие настройки очищены.';
										|en = 'Repetition during one day is not supported. The settings are cleared.';"));
	EndIf;
...
Function EstimatedTimeAsString()
	...
	TimeAsString = Format(EstimatedReminderTime, NStr("ru = 'ДФ=H:mm';
															|en = 'DF=H:mm';"));
