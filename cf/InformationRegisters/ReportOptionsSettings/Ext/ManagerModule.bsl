...
Procedure NotifyReportOptionUsers(Records) Export 
	...
	Text = StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = 'Настроен отчет %1';
			|en = 'Report %1 is configured';"),
	...
	Except
		...
		WriteLogEvent(
			NStr("ru = 'Варианты отчетов';
				|en = 'Report options';", DefaultLanguageCode),
			EventLogLevel.Error,,
...
Procedure ProcessDataForMigrationToNewVersion(Parameters) Export 
	...
	While Variant.Next() Do
		...
		Except
			...
			CommentTemplate = NStr("ru = 'Не удалось перенести настройки доступности варианта отчета ""%1"" в регистр ""%2""
				|по причине: %3';
				|en = 'Cannot move the availability settings of the ""%1"" report option to the ""%2"" register.
				|Reason: %3';");
	...
	If Processed = 0 And Declined <> 0 Then
		MessageTemplate = NStr("ru = 'Не удалось обработать некоторые настройки вариантов отчетов: %1';
								|en = 'Couldn''t process (skipped) some report option settings: %1';");
		MessageText = StringFunctionsClientServer.SubstituteParametersToString(MessageTemplate, Declined);
		Raise MessageText;
	Else
		CommentTemplate = NStr("ru = 'Обработан очередной пакет настроек вариантов отчетов: %1';
								|en = 'Yet another batch of report option settings is processed: %1';");
