...
Procedure WriteSettingsOnClient()
	...
	If PostponedProcessStart <> DeferredProcessStartOnOpen Then 
		...
		NotificationText1 = ?(PostponedProcessStart, NStr("ru = 'Отложенный старт:';
															|en = 'Deferred start:';"), NStr("ru = 'Отложенный старт отменен:';
																							|en = 'Deferred start canceled:';"));
...
Procedure UpdateTimeSelectionList()
	...
	For Indus = 1 To 48 Do
		Items.DeferredStartDateTime.ChoiceList.Add(DateEmpty, Format(DateEmpty, NStr("ru = 'ДФ=ЧЧ:мм';
																										|en = 'DF=HH:mm';")));
...
Function IntevalText(StartDate, EndDate)
	...
	If StartDate > EndDate Then
		Return NStr("ru = 'Дата запуска задания находится в прошлом.';
					|en = 'Duty start date is in the past.';");
	EndIf;	
	...
	If UseDateAndTimeInTaskDeadlines Then
		If NumberOfDays > 0 And NumberOfHours > 0 Then
			DateDiff = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Задание будет запущено через %1 дн. и %2 ч.';
					|en = 'The duty will be started in %1 days and %2 hours.';"),
				String(NumberOfDays),
				String(NumberOfHours));
		ElsIf NumberOfDays > 0 Then
			DateDiff = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Задание будет запущено через %1 дн.';
					|en = 'The duty will be started in %1 days.';"), String(NumberOfDays));
		ElsIf NumberOfHours > 0 Then
			DateDiff = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Задание будет запущено через %1 ч.';
					|en = 'The duty will be started in %1 hours.';"), String(NumberOfHours));
		Else
			DateDiff = NStr("ru = 'Задание будет запущено менее чем через час.';
								|en = 'The duty will be started in less than an hour.';");
		EndIf;
	Else
		If NumberOfDays > 0 Then
			DateDiff = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Задание будет запущено через %1 дн.';
					|en = 'The duty will be started in %1 days.';"), String(NumberOfDays));
		Else
			DateDiff = NStr("ru = 'Задание будет запущено менее чем через день.';
								|en = 'The duty will be started in less than a day.';");
		EndIf;
	EndIf;
...
Function FormIsFilledInCorrectly()
	...
	If PostponedProcessStart And DeferredStartDate < CurrentServerDate Then
		CommonClient.MessageToUser(NStr("ru = 'Дата и время отложенного старта должны быть больше текущей даты.';
														|en = 'Date and time of the deferred start must be greater than the current date.';"),,
			"DeferredStartDate");
		FilledInCorrectly = False;
	EndIf;
		
	If PostponedProcessStart And DeferredStartDate > TaskDueDate Then
		CommonClient.MessageToUser(NStr("ru = 'Дата и время отложенного старта должны быть меньше срока исполнения задания.';
														|en = 'Date and time of the deferred start must be less than the duty due date.';"),,
			"DeferredStartDate");
		FilledInCorrectly = False;
	EndIf;
