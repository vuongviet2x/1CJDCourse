...
Procedure ProcessUserTemplates(Parameters) Export
		...
		If Metadata.FindByFullName(TemplateMetadataObjectName) = Undefined Then
			...
			ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Обнаружен пользовательский макет, отсутствующий в метаданных конфигурации:
					|""%1"".';
					|en = 'Custom template not registered in configuration metadata found:
	...
	If ObjectsProcessed = 0 And ObjectsWithIssuesCount <> 0 Then
		MessageText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Не удалось обработать некоторые пользовательские макеты (пропущены): %1';
				|en = 'Couldn''t process (skipped) some custom templates: %1';"),
		...
		WriteLogEvent(InfobaseUpdate.EventLogEvent(),
			EventLogLevel.Information, Metadata.InformationRegisters.UserPrintTemplates,,
				StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Обработана очередная порция пользовательских макетов: %1';
						|en = 'Another batch of custom templates is processed: %1';"),
			ObjectsProcessed));
	EndIf;
