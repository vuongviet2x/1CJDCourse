...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Not HasUpdateRight Then
		...
		MessageText = NStr("ru = 'Недостаточно прав для изменения макетов.';
								|en = 'Insufficient rights to edit templates.';");
	...
	If Common.SubsystemExists("StandardSubsystems.NationalLanguageSupport") Then
		...
		If Common.SubsystemExists("StandardSubsystems.NationalLanguageSupport.Print") Then
			...
			Items.FilterByLanguage.ChoiceList.Add("", NStr("ru = 'Все';
																|en = 'All';"));
...
Procedure AddTemplate(Command)
	...
	UniqueDescr = AssignUniqueDescription(NStr("ru = 'Новая печатная форма';
																|en = 'New print form';"), False);
	ShowInputString(NotifyDescription, UniqueDescr, NStr("ru = 'Введите наименование макета';
...
Procedure OpenPrintFormTemplateForEdit()
	...
	If CurrentData.Changed And Not CurrentData.ChangedTemplateUsed Then
		...
		QueryText = NStr("ru = 'Имеется измененный макет, но он не используется.
		|Можно переключиться на него и продолжить редактирование,
		|либо начать редактирование стандартного макета.';
		|en = 'An edited version of this template is available.
		|You can switch to the edited template or continue with the standard one.
		|';");
		
		Buttons = New ValueList();
		Buttons.Add(True, NStr("ru = 'Измененный ранее';
									|en = 'Edited template';"));
		Buttons.Add(False, NStr("ru = 'Стандартный (текущий)';
									|en = 'Standard template';"));
		Buttons.Add(Undefined, NStr("ru = 'Отмена';
											|en = 'Cancel';"));
		
		ShowQueryBox(NotifyDescription, QueryText, Buttons, , , NStr("ru = 'Какой макет редактировать?';
																			|en = 'Which template do you want to proceed with?';"));
		Return;
	EndIf;
...
Procedure Copy(Command)
	...
	ShowInputString(NotifyDescription, CopyDescr, NStr("ru = 'Введите наименование макета';
																	|en = 'Enter a template description';"), 100, False)
...
Procedure GoToList()
	...
	For Each ClientApplicationWindow In GetWindows() Do
		If ClientApplicationWindow.GetURL() = ListURL Then
			...
			Buttons = New ValueList;
			Buttons.Add("Reopen", NStr("ru = 'Переоткрыть';
												|en = 'Reopen';"));
			Buttons.Add("Cancel", NStr("ru = 'Не переоткрывать';
											|en = 'Do not reopen';"));
			QueryText = 
...
Procedure FillPrintFormsTemplatesTable(Branch1)
	...
	GroupOther.Presentation = NStr("ru = 'Прочие';
										|en = 'Other';");
