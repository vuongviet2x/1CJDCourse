...
Procedure BeforeWrite(Cancel, Replacing)
	...
	If Count() > 0
		...
		Cancel = True;
		ExceptionText = NStr("ru = 'Запись в %1 возможна только при отметке выполнения отложенного обработчика обновления информационной базы, который выполняется только в корневом узле.';
								|en = 'You can save data to %1 only when the deferred infobase update handler running in the root node is marked as completed.';");
