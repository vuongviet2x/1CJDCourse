...
Procedure ProcessDataForMigrationToNewVersion(Parameters) Export
	...
	FilterPresentation   = NStr("ru = 'УзелИнформационнойБазы = ""%1""';
								|en = 'InfobaseNode = %1';");
	...
	While Selection.Next() Do
		...
		Except
			...
			MessageText = StringFunctionsClientServer.SubstituteParametersToString(
				...
				|%3';
				|en = 'Couldn''t process a set of ""%1"" register records with filter %2 due to:
	...
	If Processed = 0 And RecordsWithIssuesCount <> 0 Then
		MessageText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Процедуре РегистрыСведений.ОбщиеНастройкиУзловИнформационныхБаз.ОбработатьДанныеДляПереходаНаНовуюВерсию не удалось обработать некоторые записи узлов обмена (пропущены): %1';
				|en = 'Procedure InformationRegisters.CommonInfobasesNodesSettings.ProcessDataForMigrationToNewVersion failed to process (skipped) some exchange node records: %1';"), 
	...
	Else
		WriteLogEvent(InfobaseUpdate.EventLogEvent(), EventLogLevel.Information,
			...
			NStr("ru = 'Процедура РегистрыСведений.ОбщиеНастройкиУзловИнформационныхБаз.ОбработатьДанныеДляПереходаНаНовуюВерсию обработала очередную порцию записей: %1';
				|en = 'The InformationRegisters.CommonInfobasesNodesSettings.ProcessDataForMigrationToNewVersion procedure processed records: %1';"),
			Processed));
	EndIf;
