...
Procedure ProcessDataForMigrationToNewVersion(Parameters) Export
	...
	FilterPresentation   = NStr("ru = 'Календарь = ""%1""';
								|en = 'Calendar = ""%1""';");
	...
	While Selection.Next() Do
		...
		Except
			...
			MessageText = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Ошибка при обработке записей регистра с отбором %1 по причине:
					|%2';
					|en = 'Error processing register records with the filter ""%1"" applied due to:
					|%2';"),
	...
	If Processed = 0 And RecordsWithIssuesCount <> 0 Then
		MessageText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Процедуре %1 не удалось обработать некоторые записи (пропущены): %2';
				|en = 'Procedure %1 failed to process and skipped %2 records.';"), 
			ProcedureName,
	...
	WriteLogEvent(
		...
		StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Процедура %1 обработала очередную порцию записей: %2';
