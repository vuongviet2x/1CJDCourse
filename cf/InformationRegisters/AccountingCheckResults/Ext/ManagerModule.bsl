...
Procedure ProcessDataForMigrationToNewVersion(Parameters) Export
	...
	While Selection.Next() Do
		...
		Except
			...
			MessageText = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Не удалось обработать результаты проверки ведения учета с отбором %1 по причине:
				|%2';
				|en = 'Couldn''t process data integrity check results with filter %1. Reason:
	...
	If ObjectsProcessed = 0 And ObjectsWithIssuesCount <> 0 Then
		MessageText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Не удалось обработать некоторые результаты проверки ведения учета (пропущены): %1';
				|en = 'Couldn''t process (skipped) some data integrity check results: %1';"), 
	...
	Else
		WriteLogEvent(InfobaseUpdate.EventLogEvent(), 
			...
			StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Обработана очередная порция результатов проверки ведения учета: %1';
					|en = 'Yet another batch of data integrity check results is processed: %1';"),
				ObjectsProcessed));
	EndIf;
