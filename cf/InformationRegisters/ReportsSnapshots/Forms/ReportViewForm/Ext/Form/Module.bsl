...
Procedure ReadReportSnapshot()
	...
	Else
		Common.MessageToUser(NStr("ru = 'Ошибка при чтении снимка отчета - отчет был удален';
													|en = 'An error occurred when reading the report snapshot: the report is deleted.';"));
	EndIf;
	
	Title = NStr("ru = 'Актуальность';
					|en = 'Last updated';") + ": " + UpdateDate;
