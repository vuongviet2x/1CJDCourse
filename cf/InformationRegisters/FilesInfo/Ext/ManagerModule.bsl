...
Procedure ProcessDataForMigrationToNewVersion(Parameters) Export
	...
	For Each String In SelectedData Do
		...
		Try
			...
			If FileStorageType = Undefined Then
				BadData[String.Owner] = StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'В регистре Сведения о файлах содержится ссылка на несуществующий файл ""%1"".';
						|en = 'The ""File info"" information register references a non-existent file: ""%1"".';"),
					String.File);
		...
		Except
			...
			MessageText = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Ошибка при обработке сведений о файле %1 по причине:
				|%2';
				|en = 'Couldn''t process information records on file %1. Reason:
				|%2';"), 
	...
	If ObjectsProcessed = 0 And ObjectsWithIssuesCount <> 0 Then
		MessageText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Не удалось обработать некоторые сведения о файлах (пропущены): %1';
				|en = 'Couldn''t process (skipped) some of the file info: %1';"), 
	...
	Else
		WriteLogEvent(InfobaseUpdate.EventLogEvent(), EventLogLevel.Information,
			...
			StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Обработана очередная порция сведений о файлах: %1';
					|en = 'Yet another batch of file info is processed: %1';"),
				ObjectsProcessed));
	EndIf;
