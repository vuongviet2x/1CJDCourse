...
Procedure FillAvailableTransportKinds()
	...
	If Common.DataSeparationEnabled() Then
		...
		Else
			...
			ExceptionText = NStr("ru = 'Настройка транспорта сообщений в режиме сервиса актуальна только для синхронизаций использующих ВЕБ-сервисы';
									|en = 'Message transport setup in SaaS mode is only relevant for synchronizations that use web services';");
			Raise ExceptionText;
...
Procedure NotifyUserAboutConnectionResult(Val AttachmentError)
	...
	WarningText = ?(AttachmentError, NStr("ru = 'Не удалось установить подключение.';
													|en = 'Cannot establish connection.';"),
											   NStr("ru = 'Подключение успешно установлено.';
