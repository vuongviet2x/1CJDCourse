...
Function JobStatusMessage(Form)
	...
	If Form.Object.Completed Then
		...
		TextString = ?(Form.Object.Completed2, 
			NStr("ru = 'Задание выполнено %1.';
				|en = 'The duty is completed on %1.';"), NStr("ru = 'Задание отменено %1.';
														|en = 'The duty is canceled on %1.';"));
	...
	ElsIf Form.Object.Started Then
		StateText = ?(Form.ChangeJobsBackdated, 
			NStr("ru = 'Изменения формулировки, важности, автора, а также перенос сроков исполнения и проверки задания вступят в силу немедленно для ранее выданной задачи.';
				|en = 'Changes to the wording, priority, author, deadlines, and revision will take into effect immediately for the previous task.';"),
			NStr("ru = 'Изменения формулировки, важности, автора, а также перенос сроков исполнения и проверки задания не будут отражены в ранее выданной задаче.';
				|en = 'Changes to the wording, priority, author, deadlines, and revision will not apply to the previous task.';"));
	ElsIf Form.Defer Then
		...
		StateText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Задание будет запущено <a href=""%1"">%2</a>';
				|en = 'The duty will be started on <a href=""%1"">%2</a>';"), "OpenDeferredStartSetup",
...
Procedure CheckDeferredProcessEndDate(ObjectToCheck, Cancel)
	...
	If ObjectToCheck.TaskDueDate < DeferredStartDate Then
		Common.MessageToUser(
			NStr("ru = 'Срок исполнения задания не может быть меньше даты отложенного старта.';
				|en = 'The duty deadline must be later than the start date.';"),, 
...
Function FormKeyAttributesAreFilledIn()
	...
	If Not ValueIsFilled(Object.Performer) Then
		CommonClient.MessageToUser(NStr("ru = 'Поле ""Исполнитель"" не заполнено.';
	...
	If Not ValueIsFilled(Object.Description) Then
		CommonClient.MessageToUser(NStr("ru = 'Поле ""Задание"" не заполнено.';
														|en = 'Duty is required.';"),, 
			"Performer", "Object.Description");
		FormAttributesAreFilledIn = False;
	EndIf;
	If Not ValueIsFilled(Object.TaskDueDate) Then
		CommonClient.MessageToUser(NStr("ru = 'Поле ""Срок"" исполнения не заполнено.';
