...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Common.IsMobileClient() Then
		Raise NStr("ru = 'Редактирование офисного документа недоступно в мобильном клиенте.
		|Используйте тонкий клиент или веб-клиент.';
		|en = 'Cannot edit an office document in the mobile client.
...
Procedure BeforeClose(Cancel, Exit, WarningText, StandardProcessing)
	...
	If Exit Then
		If Modified And Not CancelSavingChanges Then
			WarningText = NStr("ru = 'Макет печатной формы отредактирован в другом приложении. 
				|Нажмите Записать, чтобы не потерять изменения.';
				|en = 'The print form''s template has been edited in another application.
...
Procedure LoadFromFile(Command)
	...
	ImportParameters.Dialog.Title = NStr("ru = 'Выберите офисный документ';
												|en = 'Select an office document';");
	ImportParameters.Dialog.Filter = NStr("ru = 'Офисный документ';
											|en = 'Office document';") + " (*.docx)|*.docx";
...
Procedure PutToClipboard(Command)
	...
	If Button = Undefined Then
		ShowMessageBox(,NStr("ru = 'Выберите поле объекта, оператор или функцию.';
									|en = 'Select an object field, operator, or function.';"));
	Else
		Button.onclick();
	EndIf;
...
Procedure ViewPrintFormFollowUp(Result, Context) Export
	...
	If SampleAddress <> Undefined Then
		...
		TemplateOpenParameters.NameOfFileToOpen = NStr("ru = 'Образец';
															|en = 'Preview';")+" "+ StrReplace(CurrentDate, ":", "_")+".docx";
		OpenTemplate(TemplateOpenParameters);
	EndIf;
EndProcedure
...
Procedure SaveToFileFollowUp(Result, AdditionalParameters) Export
	...
	SavingParameters.Dialog.Title = NStr("ru = 'Сохранение макета в файл';
												|en = 'Save template to file';");
	SavingParameters.Dialog.Filter = NStr("ru = 'Офисный документ';
											|en = 'Office document';") + " (*.docx)|*.docx";
...
Procedure BeforeCloseEnd(Result, AdditionalParameters) Export
	...
	QueryText = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Сохранить изменения в %1%2?';
																				|en = 'Do you want to save the changes to %1%2?';"), 
...
Procedure SetInHTMLField(Value)
	...
	HTMLField = StringFunctionsClientServer.SubstituteParametersToString(HTMLText,
		StrReplace(Value, """", "&quot;"),
		NStr("ru = 'Перенос текста';
			|en = 'Wrap the text';"),
		NStr("ru = 'Перетащите<br> в редактор';
			|en = 'Drag<br> to the editor';"),
		NStr("ru = 'Скопировать';
			|en = 'Copy';"),
		NStr("ru = 'Выберите поле';
			|en = 'Select a field';"));
...
Procedure SwitchLangAfterImportFileFollowUp(Result, AdditionalParameters) Export
	...
	If Modified Then
		LanguageDetails = ?(ValueIsFilled(CurrentLanguage), " (" + CurrentLanguage + ")", "");
		QueryText = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Сохранить изменения в %1%2?';
																					|en = 'Do you want to save the changes to %1%2?';"), DocumentName, LanguageDetails);
...
Procedure WarnAboutLockAndOpenApp()
	...
	ShowMessageBox(NotifyDescription, NStr("ru = 'Завершите работу с файлом в другом приложении.';
													|en = 'Complete the operation with the file in another application.';"), , NStr("ru = 'Файл открыт в другом приложении';
																													|en = 'The file is opened in another application';"));
EndProcedure
...
Function ListOfOperators()
	...
	If Common.SubsystemExists("StandardSubsystems.Print") Then
		...
		DetailsOfAdditionalFields.Presentation = NStr("ru = 'Начало условия';
														|en = 'Condition start';");
		DetailsOfAdditionalFields.Type = New TypeDescription("Boolean");
		AdditionalFields.Insert("AreaStart", DetailsOfAdditionalFields);
		
		DetailsOfAdditionalFields = New Structure("Presentation, Type");
		DetailsOfAdditionalFields.Presentation = NStr("ru = 'Окончание условия';
														|en = 'Condition end';");
		DetailsOfAdditionalFields.Type = New TypeDescription("Boolean");
		...
		AdditionalFieldsGroupDetails.Presentation = NStr("ru = 'Условный вывод';
