...
Procedure OnOpen(Cancel)
	...
	If UsersClient.IsExternalUserSession() Then 
		Cancel = True;
		Raise(NStr("ru = 'Недостаточно прав для выполнения поиска.';
								|en = 'Insufficient rights to search.';"), ErrorCategory.AccessViolation);
	EndIf;
...
Procedure OnExecuteSearch(Val Var_SearchDirection)
	...
	If IsBlankString(SearchString) Then
		ShowMessageBox(, NStr("ru = 'Введите, что нужно найти.';
										|en = 'Please enter text to search for.';"));
		Return;
	EndIf;
...
Procedure AfterOpenURL(ApplicationStarted, Context) Export
	...
	If Not ApplicationStarted Then 
		ShowMessageBox(, NStr("ru = 'Открытие объектов данного типа не предусмотрено';
...
Procedure UpdateForm(SearchResults)
	...
	ElsIf SearchState = "IndexUpdateInProgress"
		...
		SearchStatePresentation = NStr("ru = 'Результаты поиска могут быть неточными, повторите поиск позднее.';
											|en = 'Search results might be inaccurate. Try the search later.';");
	ElsIf SearchState = "SearchSettingsError" Then 
		
		// For non-administrators.
		SearchStatePresentation = NStr("ru = 'Полнотекстовый поиск не настроен, обратитесь к администратору.';
											|en = 'Full-text search is not set up. Contact your administrator.';");
		
	ElsIf SearchState = "SearchProhibited" Then 
		SearchStatePresentation = NStr("ru = 'Полнотекстовый поиск отключен.';
...
Procedure UpdateSearchAreaPresentation()
	...
	If Not SearchInSections Or Not SearchAreasSpecified Then
		SearchAreasPresentation = NStr("ru = 'Везде';
											|en = 'Everywhere';");
		Return;
	EndIf;
	...
	Else	
		SearchAreasPresentation = NStr("ru = 'В выбранных разделах';
...
Function NewHTMLErrorPage()
	...
	If ErrorCode = "FoundNothing" Then 
		...
		QueryTextRecommendationHTML =
			NStr("ru = '<li><b>Упростите запрос, исключив из него какое-либо слово.</b></li>';
