...
Procedure UpdateHierarchicalTree()
	...
	If Not MainDocumentAvailable() Then
		...
		MessageText = NStr("ru = 'Документ, для которого сформирован отчет о структуре подчиненности, стал недоступен.';
								|en = 'The source document is no longer available.';");
...
Function DeletionMarkEditScenario(SelectedItems, StatisticsBySelectedItems)
	...
	If SelectedItemsCount = 1 Then 
		...
		If ObjectsMarkedForDeletionCount = 0 Then 
			...
			Scenario.Check = True;
			Scenario.Notification = NStr("ru = 'Пометка удаления установлена';
			...
			Scenario.DoQueryBox = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Пометить ""%1"" на удаление?';
					|en = 'Do you want to mark %1 for deletion?';"), ItemPresentation);
			
		Else
			
			ItemPresentation = StatisticsBySelectedItems.WithDeletionMark[Item];
			
			Scenario.Notification = NStr("ru = 'Пометка удаления снята';
										|en = 'Deletion mark cleared';");
			Scenario.Explanation = ItemPresentation;
			Scenario.DoQueryBox = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Снять с ""%1"" пометку на удаление?';
					|en = 'Do you want to clear the deletion mark from ""%1""?';"), ItemPresentation);
	...
	Else
		...
		If ObjectsMarkedForDeletionCount = 0 Then 
			...
			Scenario.Check = True;
			Scenario.DoQueryBox = NStr("ru = 'Пометить выделенные элементы на удаление?';
									|en = 'Do you want to mark the selected items for deletion?';");
			Scenario.Notification = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Пометка удаления установлена (%1)';
					|en = 'Deletion mark set (%1)';"), SelectedItemsCount);
		
		ElsIf ObjectsMarkedForDeletionCount = SelectedItemsCount Then 
			
			Scenario.DoQueryBox = NStr("ru = 'Снять с выделенных элементов пометку на удаление?';
									|en = 'Clear marks for deletion of the selected items?';");
			Scenario.Notification = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Пометка удаления снята (%1)';
					|en = 'Deletion mark cleared (%1)';"), SelectedItemsCount);
...
Function ChangeItemsDeletionMark(SelectedItems, Check)
	...
	For Each Item In SelectedItems Do 
		...
		Except
			...
			RollbackTransaction();
			WriteLogEvent(NStr("ru = 'Связанные документы.Изменение пометки удаления';
											|en = 'Related documents.Change deletion mark';", Common.DefaultLanguageCode()),
...
Procedure ChangeDocumentsPosting(Mode)
	...
	If ProcessedDocumentsCount = 1 Then 
		...
		Notification = NStr("ru = 'Изменение';
							|en = 'Change';");
		Ref = GetURL(Document);
	...
	Else
		...
		Notification = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Изменение (%1)';
				|en = 'Change (%1)';"), ProcessedDocumentsCount);
	EndIf;
...
Function ProcessedDocuments(SelectedDocuments, Mode, Errors)
	...
	While IndexOf >= 0 Do 
		...
		Except
			...
			RollbackTransaction();
			WriteLogEvent(NStr("ru = 'Связанные документы.Проведение документа';
											|en = 'Related documents.Post documents';", Common.DefaultLanguageCode()),
...
Procedure WarnAboutAnErrorWhenChangingElements(Errors, Scenario)
	...
	Else
		WarningTemplate = ?(Scenario = "DeletionMark", 
			NStr("ru = 'Не удалось изменить пометку удаления документов:
				|%1';
				|en = 'Cannot change the following document deletion mark:
				|%1';"),
			NStr("ru = 'Не удалось провести документы:
				|%1';
				|en = 'Cannot post the documents:
				|%1';"));
