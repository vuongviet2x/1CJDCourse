	...
	Else
		If TimeoutButtonName <> "" Then
			NewTitle = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = '%1 (осталось %2 сек.)';
					|en = '%1 (%2 seconds remaining)';"),
				TimeoutButtonTitle,
...
Function StandardSet(Buttons)
	...
	If Buttons = "QuestionDialogMode.YesNo" Then
		Result.Add("DialogReturnCode.Yes",  NStr("ru = 'Да';
															|en = 'Yes';"));
		Result.Add("DialogReturnCode.None", NStr("ru = 'Нет';
															|en = 'No';"));
	ElsIf Buttons = "QuestionDialogMode.YesNoCancel" Then
		...
		Result.Add("DialogReturnCode.None",    NStr("ru = 'Нет';
															|en = 'No';"));
		Result.Add("DialogReturnCode.Cancel", NStr("ru = 'Отмена';
															|en = 'Cancel';"));
	ElsIf Buttons = "QuestionDialogMode.OK" Then
		Result.Add("DialogReturnCode.OK", NStr("ru = 'ОК';
														|en = 'OK';"));
	...
	ElsIf Buttons = "QuestionDialogMode.RetryCancel" Then
		Result.Add("DialogReturnCode.Retry", NStr("ru = 'Повторить';
																|en = 'Retry';"));
		Result.Add("DialogReturnCode.Cancel",    NStr("ru = 'Отмена';
																|en = 'Cancel';"));
	ElsIf Buttons = "QuestionDialogMode.AbortRetryIgnore" Then
		Result.Add("DialogReturnCode.Abort",   NStr("ru = 'Прервать';
																|en = 'Abort';"));
		Result.Add("DialogReturnCode.Retry",  NStr("ru = 'Повторить';
																|en = 'Retry';"));
		Result.Add("DialogReturnCode.Ignore", NStr("ru = 'Пропустить';
																|en = 'Ignore';"));
	EndIf;
