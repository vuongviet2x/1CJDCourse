...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If ValueIsFilled(Parameters.ExtensionMode) Then
		...
		If Parameters.ExtensionMode = "RequireImprovementSignatures" Then 
			...
			NStr("ru = 'Подписи будут усовершенствованы до типа
			|%1';
		...
		ElsIf Parameters.ExtensionMode = "RequiredAddArchiveTags" Then
			...
			Items.DecorationAddingTimestamps.Title = NStr("ru = 'В подписи будут добавлены архивные метки времени';
	...
	If Items.SignatureType.Visible Then
		...
		If Items.AddArchiveTimestamp.Visible Then
			Items.SignatureType.ChoiceList.Add(Enums.CryptographySignatureTypes.EmptyRef(), NStr("ru = 'Не усовершенствовать';
																												|en = 'Do not enhance';"));
		EndIf;
...
Procedure ExecuteActions(Command)
	...
	If Not ValueIsFilled(Signature) Then
		ShowMessageBox(, NStr("ru = 'Не заполнены данные подписей';
...
Procedure UpdateDataView(CalculateQuantity = False)
	...
	If Parameters.ExtensionMode = "RequireImprovementSignatures" Then
		DataPresentation = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Подписи для усовершенствования (%1)';
				|en = 'Signatures pending enhancement (%1)';"), SignaturesCount);
	ElsIf Parameters.ExtensionMode = "RequiredAddArchiveTags" Then
		DataPresentation = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Подписи для добавления архивных меток (%1)';
				|en = 'Signatures to add archive timestamps (%1)';"), SignaturesCount);
	ElsIf Parameters.ExtensionMode = "rawsignatures" Then
		DataPresentation = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Ранее добавленные подписи с незаполненным типом (%1)';
				|en = 'Previously added signatures with a blank type (%1)';"), SignaturesCount);
	ElsIf Parameters.ExtensionMode = "ErrorsOnAutoRenewal" Then
		DataPresentation = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Ошибки при автоматическом продлении подписей (%1)';
				|en = 'Automatic signature renewal errors (%1)';"), SignaturesCount);
	EndIf;
