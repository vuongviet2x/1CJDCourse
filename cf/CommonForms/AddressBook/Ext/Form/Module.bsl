...
Procedure FindContactsExecute()
	...
	If IsBlankString(SearchString) Then
		CommonClient.MessageToUser(NStr("ru = 'Не задано, что нужно найти.';
														|en = 'Please enter text to search for.';"),, "SearchString");
		Return;
	EndIf;
...
Procedure SetContactAddressExecute()
	...
	CurrentData = Items.EmailRecipients.CurrentData;
	If CurrentData = Undefined Then
		ShowMessageBox(, NStr("ru = 'Выберите адрес получателя в списке справа.';
	...
	If Result.Count() = 0 Then
		ShowMessageBox(, 
			StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'У контакта ""%1"" нет адресов электронной почты.';
