...
Procedure EnableEdit(Command)
	...
	If Not ValueIsFilled(Result) Then
		ShowMessageBox(, NStr("ru = 'Выберите хотя бы один реквизит';
										|en = 'Please select at least one attribute.';"));
		Return;
	EndIf;
...
Procedure AddBankingDetailsToForm(ItemsToAdd1)
	...
	For Each AttributeToLock In AttributesToLock Do
		...
		If Not IsCommonLabelSetUp Then
			...
			Else
				LabelTitle =
					NStr("ru = 'Перед изменением реквизитов рекомендуется проверить использование объекта.
					           |Если объект используется, то необходимо оценить последствия изменений.';
								|en = 'Before you change the attributes, we recommend that you check whether the object is used.
...
Function AreObjectsUsed()
	...
	ExecutionParameters.BackgroundJobDescription = NStr("ru = 'Разблокирование реквизитов: Проверка использования ссылок на объекты';
															|en = 'Unlock attributes: Check the object reference usage';");
...
Procedure ValidateCompletion(Result, AdditionalParameters) Export
	...
	If TypeOf(AreObjectsUsed) <> Type("Boolean") Then
		ShowMessageBox(, NStr("ru = 'Не удалось получить результат проверки, попробуйте еще раз';
										|en = 'Cannot receive the check result. Please try again';"));
		Return;
	EndIf;
	...
	If AreObjectsUsed Then
		If RefsCount = 1 Then
			MessageText =
				NStr("ru = 'Объект уже используется в других местах в приложении.
				           |Не рекомендуется разрешать редактирование из-за риска рассогласования данных.';
							|en = 'The object is used elsewhere in the app.
							|Editing this object might lead to data inconsistency.';");
		Else
			MessageText = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Выбранные объекты (%1) уже используются в других местах в приложении.
				           |Не рекомендуется разрешать редактирование из-за риска рассогласования данных.';
							|en = '%1 selected objects are used elsewhere in the app.
	...
	Else
		If RefsCount = 1 Then
			MessageText =
				NStr("ru = 'Объект еще не используется в других местах в приложении.
				           |Можно разрешать редактирование без риска рассогласования данных.';
							|en = 'The object is not used in other places in the app.
							|You can allow editing it without the risk of data inconsistency.';");
		Else
			MessageText = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Выбранные объекты (%1) уже используются в других местах в приложении.
				           |Можно разрешать редактирование без риска рассогласования данных.';
							|en = 'The selected objects (%1) are used in other places in the app.
