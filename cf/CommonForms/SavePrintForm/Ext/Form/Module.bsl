...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Parameters.PrintObjects.Count() > 1 And AttachableItem <> Undefined Then
		AttachableItem.Presentation = NStr("ru = 'Присоединить к документам';
													|en = 'Attach to documents';")
...
Procedure Save(Command)
	...
	If Items.FolderToSaveFiles.Visible Then
		If SavingOption = "SaveToFolder" And IsBlankString(SelectedFolder) Then
			CommonClient.MessageToUser(NStr("ru = 'Укажите папку.';
															|en = 'Select a folder.';"),,"SelectedFolder");
			Return;
		EndIf;
	EndIf;
	...
	If SaveFormats.Count() = 0 Then
		ShowMessageBox(,NStr("ru = 'Укажите как минимум один из предложенных форматов.';
	...
	If SavingOption = "Join" Then
		...
		If ErrorString <> "" Then
			...
			QueryText = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Прикрепление не будет выполнено для объектов:
			| %1';
			|en = 'Cannot attach the objects:
			| %1';"), ErrorString);
			
			Buttons = New ValueList;
			Buttons.Add("Cancel", NStr("ru = 'Отменить';
											|en = 'Cancel';"));
			Buttons.Add("Continue", NStr("ru = 'Продолжить';
												|en = 'Continue';"));
			
			QuestionParameters = StandardSubsystemsClient.QuestionToUserParameters();
			QuestionParameters.Title = NStr("ru = 'Недостаточно прав для прикрепления';
												|en = 'Insufficient rights to attach';");
