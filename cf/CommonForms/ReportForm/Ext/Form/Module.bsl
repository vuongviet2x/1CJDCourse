...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	ReportsClientServer.DisplayReportState(
		ThisObject, NStr("ru = 'Отчет не сформирован. Нажмите ""Сформировать"" для получения отчета.';
...
#If WebClient Then
	If ValueIsFilled(PathToExternalReportFileAtClient) Then
		ErrorText = NStr("ru = 'Для этого действия требуется запустить клиентское приложение';
							|en = 'For this action, start the client application';");
		Raise ErrorText;
	EndIf;
#EndIf
...
Procedure OnLoadVariantAtServer(NewDCSettings)
	...
	If IsMobileClient Then
		...
		If ResourcePlacement.Use And ResourcePlacement.Value = DataCompositionResourcesPlacement.Vertically Then
			Items.DecorationEditResourcePlacement.Title = NStr("ru = 'Горизонтально';
																			|en = 'Horizontal';");
		EndIf;
	EndIf;
...
Procedure FillCheckProcessingAtServer(Cancel, CheckedAttributes)
	...
	For Each SettingItem In SettingsItems Do
		...
		If StartDate.AutoMarkIncomplete = True And Not ValueIsFilled(Value.StartDate)
			...
			ErrorText = NStr("ru = 'Не указан период';
								|en = 'The period is not specified.';");
			DataPath = StartDate.DataPath;
		ElsIf Value.StartDate > Value.EndDate Then
			ErrorText = NStr("ru = 'Конец периода должен быть больше начала';
								|en = 'Period end must be later than period start.';");
...
Procedure FactorExtendedTooltipURLProcessing(Item, FormattedStringURL,
	...
	Else
		ShowMessageBox(, NStr("ru = 'Расширенные возможности настройки фильтров недоступны';
										|en = 'Advanced filter settings are not available';"));
	EndIf;
...
Procedure SendByEmail(Command)
	...
	If StatePresentation.Visible = True 
		...
		QueryText = NStr("ru = 'Отчет не сформирован. Сформировать?';
							|en = 'Report not generated. Do you want to generate the report?';");
...
Procedure ImportSchema(Command)
	...
	ImportParameters.Dialog.Filter = NStr("ru = 'Файлы XML (*.xml) |*.xml';
											|en = 'XML files (*.xml) |*.xml';");
...
#Else
	ShowMessageBox(, NStr("ru = 'Для того чтобы редактировать схему компоновки, запустите приложение в режиме толстого клиента.';
									|en = 'To edit composition schema, run the application in thick client mode.';"));
#EndIf
EndProcedure
...
Procedure SaveReport(Command)
	...
	Dialog.Title = NStr("ru = 'Сохранение результата отчета';
...
Procedure SaveReportSnapshot(Command)
	...
	If ReportSettings.ResultProperties.SettingsComposer = Undefined Then
		WarningText = NStr("ru = 'Перед сохранением снимка необходимо сформировать отчет.';
									|en = 'Generate the report before saving the snapshot.';");
...
Procedure Attachable_ImportReportOption(Command)
	...
	If FoundItems.Count() = 0 Then
		ShowMessageBox(, NStr("ru = 'Вариант отчета не существует.';
...
Procedure AfterGenerate(Result, GenerationParameters) Export
	...
	If Result = Undefined Then
		...
		ShowGenerationErrors(NStr("ru = 'Формирование отчета прервано администратором';
										|en = 'Report generation is canceled by the administrator';"));
		ShowUserNotification(NStr("ru = 'Отчет не сформирован';
	...
	ElsIf Result.Status = "Completed2" Then
		...
		ShowUserNotification(NStr("ru = 'Отчет сформирован';
											|en = 'Report is generated';"),
			?(Window <> Undefined, Window.GetURL(), Undefined), Title);
...
Function ReportGenerationResult(Val GeneratingOnOpen, Directly, AfterWarning)
	...
	ExecutionParameters.BackgroundJobDescription = StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = 'Выполнение отчета: %1';
			|en = 'Running report: %1';"), ReportName);
	...
	Else
		ReportsClientServer.DisplayReportState(
			ThisObject, NStr("ru = 'Отчет формируется...';
							|en = 'Generating report…';"), PictureLib.TimeConsumingOperation48);
	EndIf;
	Return Result;
...
Procedure ImportReportGenerationResult()
	If Not IsTempStorageURL(BackgroundJobStorageAddress) Then
		ShowGenerationErrors(NStr("ru = 'Не удалось сформировать отчет';
	...
	If Result = Undefined Then
		ShowGenerationErrors(NStr("ru = 'Не удалось сформировать отчет (пустой результат)';
										|en = 'Cannot generate the report (empty result)';"));
		Return;
	EndIf;
...
Procedure ShowSendByEmailDialog()
	...
	Attachment.Insert("AddressInTempStorage", PutToTempStorage(ReportResult,
		UUID));
...
Procedure ApplyPassedSettings(SettingsDescription)
	If VariantModified Then
		ShowMessageBox(, NStr("ru = 'Вариант отчета был изменен.
...
Function SuitableUniversalSearchValues(Query)
	...
	If SuitableValues.Count() = 0 Then
		...
		If StrLen(SearchParameters.SearchString) > 2 Then
			Message = NStr("ru = 'Совпадений не найдено';
							|en = 'No matches found';");
		Else
			Message = NStr("ru = 'Продолжите ввод...';
							|en = 'Continue typing…';");
		EndIf;
...
Procedure FindSuitableValuesBooleanUniversalSearch(SuitableValues, SearchFieldsByType, SearchParameters)
	...
	If StrCompare(SearchParameters.SearchString, NStr("ru = 'Да';
														|en = 'Yes';")) = 0 
		Or StrCompare(SearchParameters.SearchString, NStr("ru = 'Истина';
															|en = 'True';")) = 0 
		Or StrCompare(SearchParameters.SearchString, NStr("ru = 'Включено';
															|en = 'Enabled';")) = 0 Then
		For Each FieldForSearch In SearchFields Do
			SearchProperties = UniversalSearchProperties(FieldForSearch.Key, True, SearchParameters.ControlCharacters);
			SearchOptions = New Array;
			...
			ValuesSuitableForBoolean.Add(SearchOptions, ViewSearchResultClient(
				FieldForSearch.Value, SearchProperties.ComparisonType, SearchProperties.RightValue));
		EndDo;
	ElsIf StrCompare(SearchParameters.SearchString, NStr("ru = 'Нет';
															|en = 'No';")) = 0 
		Or StrCompare(SearchParameters.SearchString, NStr("ru = ' False';
															|en = 'False';")) = 0 
		Or StrCompare(SearchParameters.SearchString, NStr("ru = 'Выключено';
															|en = 'Disabled';")) = 0 Then
		For Each FieldForSearch In SearchFields Do
			SearchProperties = UniversalSearchProperties(FieldForSearch.Key, False, SearchParameters.ControlCharacters);
			SearchOptions = New Array;
...
Procedure FindSuitableValuesStringUniversalSearch(SuitableValues, SearchFieldsByType, SearchParameters)
	...
	If StrCompare(SearchParameters.SearchString, NStr("ru = 'Да';
		...
		Or StrCompare(SearchParameters.SearchString, NStr("ru = 'Включено';
															|en = 'Enabled';")) = 0
		Or StrCompare(SearchParameters.SearchString, NStr("ru = 'Нет';
															|en = 'No';")) = 0 
		Or StrCompare(SearchParameters.SearchString, NStr("ru = ' False';
															|en = 'False';")) = 0 
		Or StrCompare(SearchParameters.SearchString, NStr("ru = 'Выключено';
...
Function RepresentationOfTheUniversalLinkSearch(Data, FieldForSearch, Condition, SearchString)
	...
	If ValueIsFilled(Data.AdditionalData) Then
		ValuePresentation = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = '%1 (%2)';
				|en = '%1 (%2)';"), String(Data.Ref), Data.AdditionalData);
	EndIf;
...
Function ConditionFragmentTemplate(Condition)
	...
	Return StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = '%1<span style=""color: %2"">%3</span>%4';
			|en = '%1<span style=""color: %2"">%3</span>%4';"), " ", "HiddenReportOptionColor", 
...
Function SearchStringFragmentTemplate(SearchString)
	...
	Return StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = '<span style=""color: %1; font: %2"">%3</span>';
			|en = '<span style=""color: %1; font: %2"">%3</span>';"), "MyReportsOptionsColor", "ImportantLabelFont",
		SearchString);
...
Function ReportGroupingsBasedOnAFieldWithAHierarchicalGrouping(IndexOfTheReportStructure, FieldsWithHierarchicalGrouping,
	...
	GroupingViewTemplate = NStr("ru = '%1 - уровень %2';
											|en = '%1 - level %2';");
...
Procedure SaveReportAfterFilenameSelected(Result, Context) Export
	...
	If Not ValueIsFilled(FullNameOfTheReportFile) Then
		...
		FormatsList.ShowChooseItem(OnCloseNotifyDescription, NStr("ru = 'Выберите формат сохранения';
...
Procedure SaveReportAfterSavedReportResults(Result, FullNameOfTheReportFile) Export
	...
	ShowUserNotification(NStr("ru = 'Отчет сохранен в файл';
										|en = 'The report is saved to the file';"), Handler, FullNameOfTheReportFile);
...
Procedure SetVisibilityAvailability()
	...
	If DetailsMode Then
		Title = Title + " (" + NStr("ru = 'Расшифровка';
											|en = 'Details';") + ")";
	EndIf;
EndProcedure
...
Procedure LoadVariant(VariantKey, ClearStackSettings = True)
	...
	ReportsClientServer.DisplayReportState(	ThisObject, 
		NStr("ru = 'Выбран другой вариант отчета. Нажмите ""Сформировать"" для получения отчета.';
			|en = 'Another report option is selected. To run the report, click ""Generate"".';"),
...
Procedure DefineBehaviorInMobileClient()
	...
	Items.CommandsAndIndicators.Title = NStr("ru = 'показатели';
												|en = 'indicators';");
...
Function RefinementsEventSettings()
	...
	Prefixes = New Map;
	Prefixes.Insert("HighlightInYellow", NStr("ru = 'Оформить желтым';
											|en = 'Highlight in yellow';"));
	Prefixes.Insert("HighlightInRed", NStr("ru = 'Оформить красным';
												|en = 'Highlight in red';"));
	Prefixes.Insert("HighlightInGreen", NStr("ru = 'Оформить зеленым';
												|en = 'Highlight in green';"));
	Prefixes.Insert("ApplyAppearanceMore", NStr("ru = 'Оформить (еще)';
											|en = 'Format (more)';"));
...
Procedure ShowGenerationErrors(ErrorInfo)
	If TypeOf(ErrorInfo) = Type("ErrorInfo") Then
		...
		DetailErrorDescription = NStr("ru = 'Отчет не сформирован по причине:';
											|en = 'The report is not generated due to:';") + Chars.LF
...
Procedure SaveAsNewOrOverwriteExistingReportOptionAndContinue(Command)
	...
	If Not Items.SaveOptionAs.Visible Then
		ShowMessageBox(, NStr("ru = 'Невозможно выполнить действие, так как недоступно сохранение вариантов отчетов.';
										|en = 'Cannot perform the action as saving report options is unavailable.';"));
		Return;
	EndIf;

	If Items.SaveVariant.Visible Then
		Buttons = New ValueList;
		Buttons.Add("SaveAndResume", NStr("ru = 'Сохранить и продолжить';
		...
		StandardSubsystemsClient.ShowQuestionToUser(
			...
			NStr("ru = 'Сохранить вариант отчета?';
				|en = 'Do you want to save the report option?';"), Buttons, QuestionParameters);
		Return;
	Else
		FormParameters = New Structure;
...
Procedure SaveReportOption(AfterAddedNew = False)
	...
	ShowUserNotification(NStr("ru = 'Вариант отчета сохранен';
										|en = 'The report option is saved';"), 
		?(Window <> Undefined, Window.GetURL(), Undefined), Title);
