...
Function OnlyBuildNumberOfMainConfigurationChanged()
	...
	If ReturnCode <> 0 Then
		...
		ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Не удалось выгрузить конфигурацию в файлы по причине:
			           |%1';
						|en = 'Cannot dump the configuration to files due to:
						|%1';"),
	...
	ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = 'Не найдена версия в файле выгрузки %1';
			|en = 'A version is not found in the %1 dump file';"),
		"Configuration.xml");
		...
		If TypeOf(NextNode) = Type("DOMElement")
		   And Upper(NextNode.TagName) = Upper("Version") Then
			...
			If StrSplit(Version, ".", False).Count() < 4 Then
				ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Некорректная версия ""%1"" в файле выгрузки %2';
						|en = 'Incorrect version ""%1"" in the %2 dump file';"),
...
Procedure WriteError(ErrorText)
	...
	ErrorTitle = NStr("ru = 'Не удалось получить версию новой конфигурации по причине:';
							|en = 'Cannot get the new configuration version due to:';") + Chars.LF;
