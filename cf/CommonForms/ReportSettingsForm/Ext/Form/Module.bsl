...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	Else
		...
		Title = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Настройки отчета ""%1""';
				|en = '%1 report settings';"), DescriptionOption);
	EndIf;
	...
	If Common.SubsystemExists("StandardSubsystems.MonitoringCenter") Then 
		...
		Comment = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = '%1 режим';
																					|en = '%1 mode';"), CurMode.Presentation);
	...
	If CommonClientServer.CompareVersions(SystemInfo.AppVersion, "8.3.23.1688") >= 0 Then
		Items.FiltersValue.MultipleValuesExtendedEdit = True;
		Items.FiltersRightValue.MultipleValuesExtendedEdit = True;
	EndIf;
EndProcedure
...
Procedure BeforeClose(Cancel, Exit, WarningText, StandardProcessing)
	...
	If Not SelectionResultGenerated Then
		...
		CommonClient.ShowFormClosingConfirmation(Notification, Cancel, Exit,
			NStr("ru = 'Настройки изменены. Сохранить изменения?';
				|en = 'The settings were changed. Save the changes?';"));
	EndIf;
...
Procedure Attachable_List_ChoiceProcessing(Item, SelectionResult, StandardProcessing)
	...
	If AddOn.Total > 0 Then
		If AddOn.Total = 1 Then
			NotificationTitle = NStr("ru = 'Элемент добавлен в список';
										|en = 'The item added to the list.';");
		Else
			NotificationTitle = NStr("ru = 'Элементы добавлены в список';
...
Procedure SelectedFields_Ungroup(Command)
	...
	If RowsIDs.Count() <> 1 Then 
		ShowMessageBox(, NStr("ru = 'Выберите одну группу.';
										|en = 'Select a group.';"));
		Return;
	EndIf;
	...
	If TypeOf(SourceRowParent.Id) <> Type("DataCompositionID") Then 
		ShowMessageBox(, NStr("ru = 'Выберите группу.';
										|en = 'Select a group.';"));
		Return;
	EndIf;
...
Procedure OptionStructure_SaveToFile(Command)
	...
	FileName = NStr("ru = 'Настройки.xml';
					|en = 'Settings.xml';");
	
	SavingParameters = FileSystemClient.FileSavingParameters();
	SavingParameters.Dialog.Title = NStr("ru = 'Выберите файл для сохранения настроек отчета';
												|en = 'Select a file to save report settings';");
	SavingParameters.Dialog.Filter    = NStr("ru = 'Настройки отчета (*.xml)|*.xml';
												|en = 'Report settings (*.xml)|*.xml';");
...
Procedure AppearanceSelection(Item, RowID, Field, StandardProcessing)
	...
	If String.IsOutputParameter Then 
		If String(String.Id) = "TITLE"
			...
			ShowInputString(Handler, String.Value, NStr("ru = 'Заголовок для печати';
																|en = 'Printing header';"),, True);
		EndIf;
...
Procedure UpdateGroupFields()
	...
	For Each SettingItem In StructureItemProperty.Items Do 
		...
		If TypeOf(SettingItem) = Type("DataCompositionAutoGroupField") Then 
			String.Title  = NStr("ru = 'Авто (по всем полям)';
									|en = 'Auto (all fields)';");
...
Procedure UpdateDataParameters()
	...
	Section.Title = NStr("ru = 'Параметры';
							|en = 'Parameters';");
...
Function FiltersGroupingParameters()
	...
	If Rows.Count() = 0 Then 
		ShowMessageBox(, NStr("ru = 'Выберите элементы.';
										|en = 'Select items.';"));
		Return Undefined;
	EndIf;
	...
	If Parents.Count() > 1 Then 
		ShowMessageBox(, NStr("ru = 'Выбранные элементы не могут быть сгруппированы, поскольку они принадлежат разным родителям.';
										|en = 'Cannot group selected items as they have different parents.';"));
		Return Undefined;
	EndIf;
...
Function FiltersShiftParameters()
	...
	If Parents.Count() > 1 Then 
		ShowMessageBox(, NStr("ru = 'Выбранные элементы не могут быть перемещены, поскольку они принадлежат разным родителям.';
										|en = 'Cannot move selected items as they have different parents.';"));
		Return Undefined;
	EndIf;
...
Procedure UpdateSelectedFields(Rows = Undefined, SettingsItems = Undefined)
	...
	For Each SettingItem In SettingsItems Do 
		...
		If TypeOf(SettingItem) = Type("DataCompositionAutoSelectedField") Then 
			String.Title = NStr("ru = 'Авто (поля родителя)';
									|en = 'Auto (parent fields)';");
			String.Picture = 18;
			Continue;
		EndIf;
...
Procedure UpdateSorting()
	...
	Section.IsSection = True;
	Section.Title = NStr("ru = 'Сортировки';
							|en = 'Sorts';");
	Section.Picture = ReportsOptionsInternalClientServer.IndexOfTheFieldImage(Undefined);
	Rows = Section.GetItems();
	...
	For Each SettingItem In SettingsItems Do 
		...
		If TypeOf(SettingItem) = Type("DataCompositionAutoOrderItem") Then 
			String.Title = NStr("ru = 'Авто (сортировки родителя)';
									|en = 'Auto (parent orders)';");
...
Procedure ChangeRowsOrderType(OrderType)
	CollectionItems = Sort.GetItems()[0].GetItems();
	...
	For Each SourceRow In Rows Do 
		...
		ElsIf TypeOf(SettingItemSource) = Type("DataCompositionAutoSelectedField") Then
			...
			If Not Found Then
				...
				DestinationRow.Title = NStr("ru = 'Авто (сортировки родителя)';
												|en = 'Auto (parent orders)';");
	...
	While IndexOf >= 0 Do 
		...
		If TypeOf(SettingItemSource) = Type("DataCompositionAutoOrderItem") Then
			...
			If Not Found Then
				...
				DestinationRow.Picture = 18;
				DestinationRow.Title = NStr("ru = 'Авто (поля родителя)';
												|en = 'Auto (parent fields)';");
			EndIf;
...
Procedure ReadPredefinedAppearanceParameters()
	...
	String.Title = StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = 'Выводить заголовок для печати: %1';
			|en = 'Show printing header: %1';"),
		?(ValueIsFilled(Object.Value), Object.Value, NStr("ru = '<Отсутствует>';
																	|en = '<None>';")));
	String.Presentation = NStr("ru = 'Выводить заголовок для печати';
								|en = 'Show printing header';");
	...
	String.Title = NStr("ru = 'Выводить фильтры';
							|en = 'Display filters';");
	String.Presentation = NStr("ru = 'Выводить фильтры';
								|en = 'Display filters';");
...
Procedure AppearanceChangeItem(RowID = Undefined, String = Undefined)
	...
	FormParameters.Insert("Title", StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = 'Элемент условного оформления отчета ""%1""';
			|en = 'Conditional appearance item of report ""%1""';"), DescriptionOption));
...
Procedure ChangePredefinedOutputParametersUsage(Use = True)
	...
	AppearanceItems = Appearance.GetItems();
	StructureItemProperty = Report.SettingsComposer.Settings.OutputParameters;
	
	For Each AppearanceItem In AppearanceItems Do 
		If TypeOf(AppearanceItem.Id) <> Type("DataCompositionID") Then 
			Continue;
		EndIf;
		
		AppearanceItem.Use = Use;
		SettingItem = SettingItem(StructureItemProperty, AppearanceItem);
		SettingItem.Use = Use;
	EndDo;
EndProcedure
...
Procedure UpdateStructure()
	...
	Section.Presentation = NStr("ru = 'Отчет';
								|en = 'Report';");
	Section.IsSection = True;
	Section.Picture = -1;
...
Function StructureCollectionItemProperties(Item)
	...
	ElsIf ElementType = Type("DataCompositionNestedObjectSettings") Then 
		...
		If Not ValueIsFilled(ItemProperties.Presentation) Then
			ItemProperties.Presentation = NStr("ru = 'Вложенная группировка';
													|en = 'Nested grouping';");
		EndIf;
		
	ElsIf ElementType = Type("DataCompositionTable") Then 
		
		ItemProperties.Presentation = NStr("ru = 'Таблица';
												|en = 'Table';");
		If ValueIsFilled(Item.UserSettingPresentation) Then 
			ItemProperties.Presentation = Item.UserSettingPresentation;
		EndIf;
	...
	ElsIf ElementType = Type("DataCompositionChart") Then 
		...
		ItemProperties.Presentation = NStr("ru = 'Диаграмма';
												|en = 'Chart';");
		If ValueIsFilled(Item.UserSettingPresentation) Then 
			ItemProperties.Presentation = Item.UserSettingPresentation;
		EndIf;
...
Procedure AddSettingsStructureItem(ElementType)
	...
	If String.Type = "DataCompositionChart" Then
		...
		String.Presentation = NStr("ru = 'Диаграмма';
									|en = 'Chart';");
		...
		SubordinateRow.Presentation = NStr("ru = 'Точки';
												|en = 'Dots';");
		...
		SubordinateRow.Id = StructureItemProperty.GetIDByObject(SubordinateSettingItem);
		SubordinateRow.Picture = -1;
		SubordinateRow.Presentation = NStr("ru = 'Серии';
												|en = 'Series';");
	ElsIf String.Type = "DataCompositionTable" Then
		String.Presentation = NStr("ru = 'Таблица';
									|en = 'Table';");
		...
		SubordinateRow.Id = StructureItemProperty.GetIDByObject(SubordinateSettingItem);
		SubordinateRow.Picture = -1;
		SubordinateRow.Presentation = NStr("ru = 'Строки';
												|en = 'Rows';");
		...
		SubordinateRow.Id = StructureItemProperty.GetIDByObject(SubordinateSettingItem);
		SubordinateRow.Picture = -1;
		SubordinateRow.Presentation = NStr("ru = 'Колонки';
												|en = 'Columns';");
	EndIf;
...
Procedure OptionStructureAfterSelectField(SettingDetails, ExecutionParameters) Export
	...
	If SettingDetails = "<>" Then
		...
		Presentation = NStr("ru = '<Детальные записи>';
							|en = '<Detailed records>';");
	Else
...
Procedure ChangeStructureItem(String, PageName = Undefined, UseOptionForm = Undefined)
	...
	TitleTemplate1 = NStr("ru = 'Настройка %1 отчета ""%2""';
							|en = '%1 settings of report %2';");
	If String.Type = "DataCompositionChart" Then
		ItemPresentation = NStr("ru = 'диаграммы';
									|en = 'Chart';");
	Else
		ItemPresentation = NStr("ru = 'группировки';
									|en = 'Grouping';");
	EndIf;
...
Function ListFillingParameters(Var_CloseOnChoice = False, MultipleChoice = True, AddRow1 = True)
	...
	// Standard parameters of the choice form. See "Client application form extension for dynamic lists" in Syntax Assistant.
...
&AtClient
Procedure ChangeUsage(CollectionName, Use = True, CollectionItems = Undefined)
	If CollectionItems = Undefined Then 
		RootRow = DefaultRootRow(CollectionName);
		If RootRow = Undefined Then 
			Return;
		EndIf;
		
		CollectionItems = RootRow.GetItems();
	EndIf;
...
Procedure DefineSelectedRows(Context)
	...
	If Context.TreeRows.Count() = 0 Then
		Context.CancelReason = NStr("ru = 'Выберите элементы.';
										|en = 'Select items.';");
		Return;
	EndIf;
	If Context.CurrentRow = Undefined Then
		If Context.Action = "ChangeGroup" Then
			Context.CancelReason = NStr("ru = 'Выберите группу.';
											|en = 'Select group.';");
			Return;
		EndIf;
	EndIf;
...
Function RowAdded(Rows, TreeRow, Specifics)
	...
	If (Specifics.CanBeParameters And TreeRow.IsParameter)
		...
		If Rows.Action = "MoveTo" Then
			Rows.CancelReason = NStr("ru = 'Параметры не могут быть перемещены.';
										|en = 'Parameters cannot be moved.';");
		ElsIf Rows.Action = "Group" Then
			Rows.CancelReason = NStr("ru = 'Параметры не могут быть участниками групп.';
										|en = 'Parameters cannot be group participants.';");
		ElsIf Rows.Action = "Delete" Then
			Rows.CancelReason = NStr("ru = 'Параметры не могут быть удалены.';
										|en = 'Parameters cannot be deleted.';");
		EndIf;
		Return False;
	EndIf;
	If Specifics.RequireOneParent Then
		...
		ElsIf Rows.CurrentParent <> Parent Then
			If Rows.Action = "MoveTo" Then
				Rows.CancelReason = NStr("ru = 'Выбранные элементы не могут быть перемещены, поскольку они принадлежат разным родителям.';
											|en = 'Cannot move selected items as they have different parents.';");
			ElsIf Rows.Action = "Group" Then
				Rows.CancelReason = NStr("ru = 'Выбранные элементы не могут быть сгруппированы, поскольку они принадлежат разным родителям.';
											|en = 'Cannot group selected items as they have different parents.';");
			EndIf;
			Return False; 
		EndIf;
...
Function AvailableDisplayModes(ShowCheckBoxesModes)
	...
	AvailableDisplayModes.Add("ShowInReportHeader", NStr("ru = 'В шапке отчета';
																		|en = 'In report header';"), , PictureLib.QuickAccess);
	
	If ShowCheckBoxesModes Then
		AvailableDisplayModes.Add("ShowOnlyCheckBoxInReportHeader", NStr("ru = 'Только флажок в шапке отчета';
																						|en = 'Only check box in report header';"), , PictureLib.QuickAccessWithFlag);
	EndIf;
	
	AvailableDisplayModes.Add("ShowInReportSettings", NStr("ru = 'В настройках отчета';
																			|en = 'In report settings';"), , PictureLib.Attribute);
	
	If ShowCheckBoxesModes Then
		AvailableDisplayModes.Add("ShowOnlyCheckBoxInReportSettings", NStr("ru = 'Только флажок в настройках отчета';
																							|en = 'Only check box in report settings';"), , PictureLib.NormalAccessWithCheckBox);
	EndIf;
	
	AvailableDisplayModes.Add("NotShow", NStr("ru = 'Не показывать';
															|en = 'Hide';"), , PictureLib.GrayCross);
...
#Region FiltersPageItemsProperties
	...
	If IsExtendedMode Then
		Items.FiltersPage.Title = NStr("ru = 'Фильтры';
												|en = 'Filters';");
	Else
		Items.FiltersPage.Title = NStr("ru = 'Основное';
												|en = 'Main';");
...
#Region FieldsAndSortingPageItemsProperties
	...
	If DisplaySelectedFields And DisplaySorting Then
		Items.SelectedFieldsAndSortingsPage.Title = NStr("ru = 'Поля и сортировки';
																	|en = 'Fields and sorts';");
	ElsIf DisplaySelectedFields Then
		Items.SelectedFieldsAndSortingsPage.Title = NStr("ru = 'Поля';
																	|en = 'Fields';");
	ElsIf DisplaySorting Then
		Items.SelectedFieldsAndSortingsPage.Title = NStr("ru = 'Сортировки';
																	|en = 'Sorts';");
	EndIf;
...
#Region CommonItemsProperties
	...
	If SettingsStructureItemChangeMode Then
		Items.GenerateAndClose.Title = NStr("ru = 'Завершить редактирование';
														|en = 'Finish editing';");
		Items.Close.Title = NStr("ru = 'Отмена';
											|en = 'Cancel';");
	Else
		Items.GenerateAndClose.Title = NStr("ru = 'Закрыть и сформировать';
														|en = 'Close and generate';");
...
Function RepresentationOfACollectionOfAStructureElement(CollectionName)
	...
	PresentationOfCollections.Insert("Structure", NStr("ru = 'Структура';
														|en = 'Structure';"));
	PresentationOfCollections.Insert("Rows", NStr("ru = 'Строки';
													|en = 'Rows';"));
	PresentationOfCollections.Insert("Columns", NStr("ru = 'Колонки';
													|en = 'Columns';"));
	PresentationOfCollections.Insert("Points", NStr("ru = 'Точки';
													|en = 'Dots';"));
	PresentationOfCollections.Insert("Series", NStr("ru = 'Серии';
													|en = 'Series';"));
