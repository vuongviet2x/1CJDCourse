...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Common.IsMobileClient() Then
		Raise NStr("ru = 'Редактирование табличного документа недоступно в мобильном клиенте.
		|Используйте тонкий клиент или веб-клиент.';
		|en = 'Cannot edit a spreadsheet document in mobile client.
	...
	If IsPrintForm And Common.SubsystemExists("StandardSubsystems.FormulasConstructor") Then
		...
		AddingOptions.HintForEnteringTheSearchString = NStr("ru = 'Найти оператор или функцию...';
																|en = 'Find operator or function…';");
...
Procedure BeforeClose(Cancel, Exit, WarningText, StandardProcessing)
	...
	QueryText = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Сохранить изменения в %1?';
																				|en = 'Do you want to save the changes to %1?';"), DocumentName);
...
Procedure TemplateOwnersClick(Item)
	...
	PickingParameters.Title = NStr("ru = 'Назначение макета';
										|en = 'Template assignment';");
...
Procedure SaveToFile(Command)
	...
	FileDialog.Filter = NStr("ru = 'Табличный документ';
									|en = 'Spreadsheet document';") + " (*.mxl)|*.mxl";
...
Procedure Translate(Command)
	...
	QueryText = StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = 'Выполнить автоматический перевод на %1 язык?';
			|en = 'Do you want to automatically translate into the %1 language?';"), Items.Language.Title);
	Buttons = New ValueList;
	Buttons.Add(DialogReturnCode.Yes, NStr("ru = 'Выполнить перевод';
												|en = 'Translate';"));
	Buttons.Add(DialogReturnCode.No, NStr("ru = 'Не выполнять';
												|en = 'Do not translate';"));
...
Procedure Rename(Command)
	...
	ShowInputString(NotifyDescription, DocumentName, NStr("ru = 'Введите наименование макета';
																|en = 'Enter a template description';"), 100, False);
...
Procedure WriteSpreadsheetDocumentFileNameSelected(Val CompletionHandler, UnlockFile)
	...
	If IsBlankString(Parameters.PathToFile) Then
		...
		Else
			...
			ShowQueryBox(NotifyDescription, NStr("ru = 'Макет заполнен некорректно. Продолжить запись?';
													|en = 'Some entries are invalid. Save the template anyway?';"), QuestionDialogMode.YesNo, , DialogReturnCode.No);
		EndIf;
	Else
...
Procedure StartFileSavingDialog(Val CompletionHandler, UnlockFile)
	...
	SaveFileDialog.Filter = NStr("ru = 'Табличный документ';
										|en = 'Spreadsheet documents';") + " (*.mxl)|*.mxl";
...
Procedure SetHeader()
	...
	If IsNew() Then
		Title = Title + " (" + NStr("ru = 'создание';
											|en = 'Create';") + ")";
	ElsIf EditingDenied Then
		Title = Title + " (" + NStr("ru = 'только просмотр';
											|en = 'Read-only';") + ")";
	EndIf;
...
Procedure Attachable_SwitchLanguage(Command)
	...
	If CommonClient.SubsystemExists("StandardSubsystems.Print") Then
		...
		If CurrentLanguage = CommonClient.DefaultLanguageCode() Then
			Items.DeleteLayoutLanguage.Title = NStr("ru = 'Удалить все изменения макета';
														|en = 'Delete all template changes';");
		Else
			Items.DeleteLayoutLanguage.Title = NStr("ru = 'Удалить макет на текущем языке';
														|en = 'Delete template in current language';");
		EndIf;
	EndIf;
...
Procedure Attachable_WhenSwitchingTheLanguage(LanguageCode, AdditionalParameters) Export
	...
	If TranslationRequired And AutomaticTranslationAvailable Then
		QueryText = StringFunctionsClientServer.SubstituteParametersToString(
			...
			|Выполнить автоматический перевод?';
			|en = 'Template has not been translated into the %1 language yet.
...
Procedure UpdateInputFieldCurrentCellValue()
	...
	ViewArea = NStr("ru = 'Текст выбранной ячейки';
								|en = 'Text of the selected cell';");
	If TypeOf(CurrentArea) = Type("SpreadsheetDocumentRange") Then
		If CurrentArea.AreaType = SpreadsheetDocumentCellAreaType.Rows
			...
			If CurrentArea.Top <> CurrentArea.Bottom Then
				Span = StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = '%1-%2';
						|en = '%1-%2';"), CurrentArea.Top, CurrentArea.Bottom);
				ViewArea = StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Условия вывода строк %1';
						|en = 'Output conditions for rows %1';"), Span);
			Else
				ViewArea = StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Условия вывода строки %1';
						|en = 'Output conditions for row %1';"), Span);
			EndIf;
		...
		If ValueIsFilled(CurrentArea.Left) And Not ValueIsFilled(CurrentArea.Top) Then
			...
			If CurrentArea.Left <> CurrentArea.Right Then
				Span = StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = '%1-%2';
						|en = '%1-%2';"), CurrentArea.Left, CurrentArea.Right);
				ViewArea = StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Условия вывода колонок %1';
						|en = 'Output conditions for columns %1';"), Span);
			Else
				ViewArea = StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Условия вывода колонки %1';
						|en = 'Output conditions for column %1';"), Span);
			EndIf;
		EndIf;
		If ValueIsFilled(CurrentArea.Left) And ValueIsFilled(CurrentArea.Top) Then
			If CurrentArea.Left <> CurrentArea.Right Or CurrentArea.Top <> CurrentArea.Bottom Then
				ViewArea = NStr("ru = 'Текст в выбранной области';
											|en = 'Text in the selected area';");
			Else
				ViewArea = NStr("ru = 'Текст выбранной ячейки';
											|en = 'Text of the selected cell';");
			EndIf;
...
Procedure CustomizeHeadersFooters(Command)
	...
	StateText = ?(Items.ShowHeadersAndFooters.Check, NStr("ru = 'Редактирование колонтитулов';
																	|en = 'Edit headers and footers';"), "");
...
Function DefaultFormat(TypeDescription)
	...
	ElsIf Type = Type("Date") Then
		If TypeDescription.DateQualifiers.DateFractions = DateFractions.Date Then
			Format = NStr("ru = 'ДЛФ=D';
							|en = 'DLF=D';");
		Else
			Format = NStr("ru = 'ДЛФ=DT';
							|en = 'DLF=DT';");
		EndIf;
...
Function PrepareLayoutForRecording(SetLanguageCode = True, Cancel = False)
	...
	For LineNumber = 1 To Template.TableHeight Do
		For ColumnNumber = 1 To Template.TableWidth Do
			...
			If ValueIsFilled(Area.Text) Then
				FieldPresentation = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'стр. %1 кол. %2';
																								|en = 'string %1 column %2';"), LineNumber, ColumnNumber);
	...
	ReplaceViewParameters(Template.Header.LeftText, "TopLeftText", Cancel, NStr("ru = 'верхний колонтитул слева';
																										|en = 'header on the left';"));
	ReplaceViewParameters(Template.Header.CenterText, "TopMiddleText", Cancel, NStr("ru = 'верхний колонтитул в центре';
																											|en = 'header in the center';"));
	ReplaceViewParameters(Template.Header.RightText, "TopRightText", Cancel, NStr("ru = 'верхний колонтитул справа';
																											|en = 'header on the right';"));
	ReplaceViewParameters(Template.Footer.LeftText, "BottomLeftText", Cancel, NStr("ru = 'нижний колонтитул слева';
																										|en = 'footer on the left';"));
	ReplaceViewParameters(Template.Footer.CenterText, "BottomCenterText", Cancel, NStr("ru = 'нижний колонтитул в центре';
																											|en = 'footer in the center';"));
	ReplaceViewParameters(Template.Footer.RightText, "BottomRightText", Cancel, NStr("ru = 'нижний колонтитул справа';
																										|en = 'footer on the right';"));
...
Procedure ExpandFieldList()
	...
	ColumnPresentation.Title = NStr("ru = 'Поле';
											|en = 'Field';");
	...
	ColumnPattern.Title = NStr("ru = 'Образец';
									|en = 'Preview';");
	...
	ButtonSettingsFormat.Title = NStr("ru = 'Настройка формата';
											|en = 'Configure format';");
	...
	Else
		If Not ValueIsFilled(Pattern) Then
			...
			CommonClient.MessageToUser(
				NStr("ru = 'Выберите образец, по данным которого будет формироваться печатная форма';
					|en = 'Select a template whose data will be used to generate a print form';"), , NameOfTheFieldList() + "[0].Pattern");
			Return;
		EndIf;
		GeneratePrintForm();
...
Procedure SetUpFieldSample()
	...
	For Each FieldDetails In FieldsCollection Do
		If FieldDetails.DataPath = "Ref" Then
			FieldDetails.Title = NStr("ru = 'Образец';
											|en = 'Preview';");
			If Offset <> 0 Then
