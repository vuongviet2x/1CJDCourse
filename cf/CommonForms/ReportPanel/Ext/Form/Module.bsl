...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	PresentationTruth = NStr("ru = 'Все варианты отчетов';
								|en = 'All report options';");
	PresentationLies = NStr("ru = 'Для компьютеров и планшетов';
							|en = 'Computers and tablets';");
...
Function SearchStringIsTooShort(Text)
	...
	If StrLen(Text) < 2 Then
		ShowMessageBox(, NStr("ru = 'Введена слишком короткая строка поиска.';
										|en = 'Search text is too short.';"));
		Return True;
	EndIf;
	...
	If Not HasNormalWord Then
		ShowMessageBox(, NStr("ru = 'Введены слишком короткие слова для поиска.';
										|en = 'Search words are too short.';"));
		Return True;
	EndIf;
...
Procedure ResetSettings(Command)
	QueryText = NStr("ru = 'Сбросить настройки расположения отчетов?';
						|en = 'Do you want to reset report assignment settings?';");
...
Function UpdateReportPanelAtServer(Val Event = "")
	...
	// Title.
	SetupModeSuffix = " (" + NStr("ru = 'настройка';
										|en = 'setting';") + ")";
...
Procedure DefineBehaviorInMobileClient()
	...
	SearchSubstring = NStr("ru = 'нажмите на отчете правой кнопкой мыши и';
							|en = 'right-click the report and';");
	ReplaceSubstring = NStr("ru = 'в контекстном меню';
							|en = 'in the context menu';");
	...
	Items.Move(Items.DisplayAllReportOptions, Items.TopBarMobileClient,
		Items.ShowTooltips);
	Items.DisplayAllReportOptions.Title = NStr("ru = 'Отображать для компьютеров и планшетов';
															|en = 'Show reports for computers and tablets';");
...
Procedure DefineSubsystemsAndTitle(Var_Parameters)
	...
	PanelTitle = ?(TitleIsSet, Var_Parameters.Title, NStr("ru = 'Отчеты';
																	|en = 'Reports';"));
	...
	TableRow.Presentation  = NStr("ru = 'Не включенные в разделы';
	...
	If Var_Parameters.SubsystemPath = "NonIncludedToSections" Then
		...
		PanelTitle = NStr("ru = 'Отчеты, не включенные в разделы';
								|en = 'Reports not included in sections';");
	EndIf;
	
	If CurrentSectionRef = Undefined Then
		Raise StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Для панели отчетов указан несуществующий раздел ""%1"" (см. %2).';
				|en = 'Non-existent section ""%1"" specified in report panel. See %2.';"),
...
Function FillReportPanelInBackground()
	...
	ClearInformationAboutChangesInUserSettings();
...
&AtServer
Procedure ClearInformationAboutChangesInUserSettings()
	AddedOptions.Clear();
EndProcedure
...
Procedure FillReportPanel(ResultAddress)
	...
	Else
		...
		If FillParameters.OtherSections.Count() = 0 Then
			...
			Label.Title = NStr("ru = 'Отчеты в других разделах не найдены.';
									|en = 'Reports not found in other sections.';") + Chars.LF;
			Label.Height = 2;
		EndIf;
		...
		If FillParameters.NotDisplayed > 0 Then // Show the note.
			LabelTitle = NStr("ru = 'Выведены первые %1 отчетов из других разделов, уточните поисковый запрос.';
									|en = 'Limited to first %1 reports. Please narrow your search.';");
...
Procedure OutputSectionOptions(FillParameters, SectionReference)
	...
	If FillParameters.OptionsNumber = 0 Then
		...
		If FillParameters.CurrentSectionOptionsDisplayed Then
			...
			If ValueIsFilled(SearchString) Then
				If FillParameters.CurrentSectionOnly Then
					Label.Title = NStr("ru = 'Отчеты не найдены.';
											|en = 'Reports not found.';");
				Else
					Label.Title = NStr("ru = 'Отчеты в текущем разделе не найдены.';
											|en = 'Reports not found in current section.';");
					Label.Height = 2;
				EndIf;
			Else
				Label.Title = NStr("ru = 'В этом разделе отчеты еще не размещены.';
										|en = 'No reports in this section.';");
			EndIf;
...
Function AddSubsystemsGroup(FillParameters, OutputOrderRow, ToGroup)
	...
	Else
		If OutputOrderRow.IsFollowUp Then
			SubsystemPresentation = SubsystemPresentation + " " + NStr("ru = '(продолжение)';
																			|en = '(continued)';");
		EndIf;
...
Function AddReportOptionItems(FillParameters, Variant, ToGroup, NestingLevel = 0)
	...
	If ValueIsFilled(Variant.Author) Then
		Label.ToolTip = TrimL(Label.ToolTip + Chars.LF) + NStr("ru = 'Автор:';
																		|en = 'Author:';") + " " + TrimAll(String(Variant.Author));
	EndIf;
	...
	If FillParameters.UseHighlighting Then
		...
		If HighlightParameters <> Undefined Then
			...
			If HighlightParameters.AuthorPresentation1.FoundWordsCount > 0 Then
				...
				EndIf;
				TooltipContent.Add(NStr("ru = 'Автор:';
												|en = 'Author:';") + " ");
			...
			If HighlightParameters.UserSettingsDescriptions.FoundWordsCount > 0 Then
				If TooltipContent.Count() > 0 Then
					TooltipContent.Add(Chars.LF);
				EndIf;
				TooltipContent.Add(NStr("ru = 'Сохраненные настройки:';
												|en = 'Saved setting:';") + " ");
			...
			If HighlightParameters.FieldDescriptions.FoundWordsCount > 0 Then
				If TooltipContent.Count() > 0 Then
					TooltipContent.Add(Chars.LF);
				EndIf;
				TooltipContent.Add(NStr("ru = 'Поля:';
												|en = 'Fields:';") + " ");
			...
			If HighlightParameters.FilterParameterDescriptions.FoundWordsCount > 0 Then
				If TooltipContent.Count() > 0 Then
					TooltipContent.Add(Chars.LF);
				EndIf;
				TooltipContent.Add(NStr("ru = 'Настройки:';
												|en = 'Settings:';") + " ");
			...
			If HighlightParameters.Keywords.FoundWordsCount > 0 Then
				If TooltipContent.Count() > 0 Then
					TooltipContent.Add(Chars.LF);
				EndIf;
				TooltipContent.Add(NStr("ru = 'Ключевые слова:';
												|en = 'Keywords:';") + " ");
