...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Not ValueIsFilled(ObjectReference) Then
		Raise NStr("ru = 'Не указан владелец настроек прав.';
								|en = 'The owner of access rights is required.';");
	EndIf;
	...
	If Not AccessRight("View", Metadata.FindByType(ObjectRefType)) Then
		Raise StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Нет права Просмотр на объекты типа ""%1"".';
				|en = 'Insufficient rights to read objects of the ""%1"" type.';"), String(ObjectRefType));
	EndIf;
	
	Title = StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = 'Права доступа: %1 (%2)';
			|en = 'Access rights: %1 (%2)';"), String(ObjectReference), String(ObjectRefType));
...
Procedure Reread(Command)
	...
	Else
		ShowQueryBox(
			...
			NStr("ru = 'Данные изменены. Прочитать без сохранения?';
				|en = 'The data was changed. Do you want to read the data without saving it?';"),
...
Procedure WriteBeginning(Close = False)
	...
	If ConfirmRightsManagementCancellation = True Then
		...
		Buttons.Add("WriteAndClose", NStr("ru = 'Записать и закрыть';
												|en = 'Save and close';"));
		...
		ShowQueryBox(
			...
			NStr("ru = 'После записи настройка прав станет недоступной.';
				|en = 'Once you save the access rights, you will not be able to change them.';"),
			Buttons,, "Cancel");
	Else
		If Close Then
			Close();
	...
	If NewAttributes = Undefined And Parameters.ObjectReference.Metadata().Hierarchical Then
		...
		Item.ToolTip = NStr("ru = 'Права не только для текущей папки,
		                               |но и для ее нижестоящих папок';
										|en = 'Apply the folder access rights
										|to its subfolders.';");
		...
		Item.Title   = NStr("ru = 'Наследуется от';
									|en = 'Inherit from';");
		Item.ToolTip   = NStr("ru = 'Папка, от которой наследуются настройка прав';
									|en = 'The folder that is the source of access rights.';");
	...
	While Not Cancel And LineNumber >= 0 Do
		...
		If NoFilledRight Then
			CommonClient.MessageToUser(
				NStr("ru = 'Не заполнено ни одно право доступа.';
					|en = 'No access right specified.';"),
				,
		...
		If Not ValueIsFilled(CurrentRow["User"]) Then
			CommonClient.MessageToUser(
				NStr("ru = 'Не заполнен пользователь или группа.';
					|en = 'A user or a group is required.';"),
				,
		...
		If RightsGroups.FindRows(Filter).Count() > 1 Then
			If TypeOf(Filter.User) = Type("CatalogRef.Users") Then
				MessageText = NStr("ru = 'Настройка для пользователя ""%1"" уже есть.';
										|en = 'Access rights for user ""%1"" already exist.';");
			Else
				MessageText = NStr("ru = 'Настройка для группы пользователей ""%1"" уже есть.';
										|en = 'Access rights for user group ""%1"" already exist.';");
			EndIf;
			CommonClient.MessageToUser(
	   ...
	   And CurrentSettingOwner <> Parameters.ObjectReference Then
		...
		MessageText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Эти права унаследованы, их можно изменить в форме настройки прав
			           |вышестоящей папки ""%1"".';
						|en = 'These access rights are inherited. Edit access rights
		...
		If DeletionCheck Then
			MessageText = MessageText + Chars.LF + Chars.LF + StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Для удаления всех унаследованных прав
				           |снимите флажок ""%1"".';
							|en = 'To delete all inherited access rights,
...
Procedure ValidatePermissionToManageRights()
	...
	Raise NStr("ru = 'Настройка прав недоступна.';
							|en = 'You cannot change access rights.';");
