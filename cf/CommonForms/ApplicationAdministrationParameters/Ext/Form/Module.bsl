...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Common.FileInfobase() And Parameters.PromptForClusterAdministrationParameters Then
		Raise NStr("ru = 'Настройка параметров кластера серверов доступна только в клиент-серверном режиме работы.';
								|en = 'Specifying cluster server parameters is only available in client/server mode.';");
	EndIf;
...
Procedure OnOpen(Cancel)
	...
	If Parameters.PromptForClusterAdministrationParameters
		...
		Cancel = True;
		MessageText = NStr("ru = 'Подключение к кластеру серверов недоступно в клиенте под управлением ОС X.';
								|en = 'Cannot connect to the server cluster on the client running on OS X.';");
