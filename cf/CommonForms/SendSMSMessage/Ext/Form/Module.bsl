...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	OutboundMessageStatus = NStr("ru = 'Сообщение отправляется...';
							|en = 'Sending message…';");
	...
	TitleTemplate1 = NStr("ru = 'Отправка SMS на телефон %1';
							|en = 'Text message to: %1';");
	If PhoneNumbers.Count() > 1 Then
		TitleTemplate1 = NStr("ru = 'Отправка SMS на телефоны: %1';
								|en = 'Text message to: %1';");
	EndIf;
...
Procedure Send(Command)
	...
	If StrLen(MessageText) = 0 Then
		ShowMessageBox(, NStr("ru = 'Введите текст сообщения';
										|en = 'Enter a message text';"));
		Return;
	EndIf;
...
Function DeliveryStatus(MessageID)
	...
	DeliveryStatuses.Insert("Error", NStr("ru = 'нет подключения к провайдеру SMS';
											|en = 'no connection to the text message service owner';"));
	DeliveryStatuses.Insert("Pending", NStr("ru = 'сообщение не отправлялось провайдером';
													|en = 'Provider queued message for delivery';"));
	DeliveryStatuses.Insert("Sending2", NStr("ru = 'выполняется отправка провайдером';
													|en = 'Provider is delivering message';"));
	DeliveryStatuses.Insert("Sent", NStr("ru = 'отправлено провайдером';
												|en = 'Provider sent the message';"));
	DeliveryStatuses.Insert("NotSent", NStr("ru = 'сообщение не отправлено провайдером';
													|en = 'Provider did not send message';"));
	DeliveryStatuses.Insert("Delivered", NStr("ru = 'сообщение доставлено';
												|en = 'Message is delivered';"));
	DeliveryStatuses.Insert("NotDelivered", NStr("ru = 'сообщение не доставлено';
													|en = 'Message is not delivered';"));
