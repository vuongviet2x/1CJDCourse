...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Not StandaloneModeInternal.IsStandaloneWorkplace() Then
		Raise NStr("ru = 'Эта информационная база не является автономным рабочим местом.';
								|en = 'The infobase is not a standalone workstation.';");
	EndIf;
...
Procedure RefreshVisibilityAtServer()
	...
	If RoleAvailableDataSynchronizationSetup Then
		Items.UpdateReceivedCommentLabel.Title = NStr("ru = 'Получено обновление программы из Интернета.
			|Установите полученное обновление, после чего синхронизация будет продолжена.';
			|en = 'The application update is received from the Internet.
...
Procedure OnChangeDataSynchronizationSchedule()
	...
	Items.DataSynchronizationScheduleOption.ChoiceList.Add(1, NStr("ru = 'Каждые 15 минут';
																				|en = 'Every 15 minutes';"));
	Items.DataSynchronizationScheduleOption.ChoiceList.Add(2, NStr("ru = 'Каждый час';
																				|en = 'Every hour';"));
	Items.DataSynchronizationScheduleOption.ChoiceList.Add(3, NStr("ru = 'Каждый день в 10:00, кроме сб. и вс.';
																				|en = 'Every day at 10:00 AM except for Sa and Su.';"));
