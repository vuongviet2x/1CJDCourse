...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	Title = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Что нового в конфигурации %1';
																			|en = 'What''s new in %1';"), Metadata.Synonym);
	...
	If DocumentUpdatesDetails.TableHeight = 0 Then
		Text = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Конфигурация успешно обновлена на версию %1';
																			|en = 'The application is updated to version %1.';"), Metadata.Version);
	...
	If Common.FileInfobase() Then
		MessageTitle = NStr("ru = 'Необходимо выполнить дополнительные процедуры обработки данных';
									|en = 'Additional data processing required';");
		Items.DeferredDataUpdate.Title = MessageTitle;
	EndIf;
	
	If Not Users.IsFullUser(, True) Then
		Items.DeferredDataUpdate.Title =
			NStr("ru = 'Не выполнены дополнительные процедуры обработки данных';
				|en = 'Additional data processing skipped';");
	EndIf;
...
Procedure ScheduledJobsDisabledURLProcessing(Item, FormattedStringURL, StandardProcessing)
	...
	QueryText = NStr("ru = 'Перезапустить приложение?';
						|en = 'Restart the app?';");
