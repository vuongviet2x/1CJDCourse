...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Not ValueIsFilled(StandaloneWorkstation) Then
		Raise NStr("ru = 'Не задано автономное рабочее место.';
								|en = 'Standalone workstation is not specified.';");
	EndIf;
...
Procedure TimeConsumingOperationIdleHandler()
	...
	Except
		...
		TimeConsumingOperation = False;
		GoBack();
		ShowMessageBox(,NStr("ru = 'В процессе работы возникли ошибки.';
									|en = 'Errors occurred when processing.';"));
...
Procedure DeleteStandaloneWorkstation1(Cancel, ErrorMessage = "")
	...
	ExecutionParameters.BackgroundJobDescription = NStr("ru = 'Удаление автономного рабочего места';
															|en = 'Delete standalone workstation';");
...
Function Attachable_WaitTimeConsumingOperationProcessing(Cancel, GoToNext)
	...
	If Cancel Then
		...
		ShowMessageBox(, StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'При удалении автономного рабочего места возникли ошибки: %1';
				|en = 'Errors occurred when deleting the standalone workstation: %1';"), ErrorMessage));
