...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Not ValueIsFilled(Parameters.Sender) Then
		...
		If AvailableEmailAccounts.Count() = 0 Then
			MessageText = NStr("ru = 'Не обнаружены доступные учетные записи электронной почты, обратитесь к администратору системы.';
									|en = 'There are no email accounts available. Please contact your system administrator.';");
	...
	ElsIf TypeOf(Parameters.Sender) = Type("ValueList") Then
		...
		If EmailAccountList.Count() = 0 Then
			MessageText = NStr("ru = 'Не указаны учетные записи для отправки сообщения, обратитесь к администратору системы.';
									|en = 'No accounts for sending mail are specified. Please contact your system administrator.';");
			Common.MessageToUser(MessageText,,,, Cancel);
			Return;
		EndIf;
...
Procedure RecipientPostalAddressesBeforeEditEnd(Item, NewRow, CancelEdit, Cancel)
	...
	If Not CommonClientServer.EmailAddressMeetsRequirements(Address, True) Then
		ShowMessageBox(, NStr("ru = 'Введите корректный адрес электронной почты.';
										|en = 'Please specify a correct email address.';"));
		Cancel = True;
		Return;
	EndIf;
	...
	For Each EmailRecipient In RecipientsMailAddresses Do
		...
		Else
			ShowMessageBox(, NStr("ru = 'Такой адрес электронной почты уже есть в списке.';
...
Procedure SendMail()
	...
	Except
		...
		ErrorTitle = NStr("ru = 'Письмо не отправлено';
								|en = 'The message is not sent';");
	...
	If MessageSent Then
		...
		ShowUserNotification(NStr("ru = 'Сообщение отправлено:';
											|en = 'Message sent:';"), ,
			?(IsBlankString(EmailSubject), NStr("ru = '<Без темы>';
											|en = '<No subject>';"), EmailSubject), PictureLib.DialogInformation);
		
		If HasWrongRecipients Then
			ShowMessageBox(, NStr("ru = 'Письмо отправлено не всем получателям.';
											|en = 'The message is not sent to some recipients.';"));
		Else
			Close();
		EndIf;
	EndIf;
...
Function FieldsFilledCorrectly()
	...
	If RecipientsMailAddresses.Count() = 0 Then
		CommonClient.MessageToUser(
			NStr("ru = 'Заполните получателя письма';
				|en = 'Please specify at least one recipient.';"), , "RecipientsMailAddresses");
		Result = False;
	EndIf;
	For Each EmailRecipient1 In RecipientsMailAddresses Do
		...
		If IsBlankString(Address) Then
			CommonClient.MessageToUser(
				NStr("ru = 'Заполните получателя письма';
					|en = 'Please specify at least one recipient.';"),, "RecipientsMailAddresses[" + Format(RecipientsMailAddresses.IndexOf(EmailRecipient1), "NG=0") + "].Presentation");
			Result = False;
		ElsIf Not CommonClientServer.EmailAddressMeetsRequirements(Address, False) Then
			CommonClient.MessageToUser(
				NStr("ru = 'Неверный адрес электронной почты';
					|en = 'Invalid email address.';"),, "RecipientsMailAddresses[" + Format(RecipientsMailAddresses.IndexOf(EmailRecipient1), "NG=0") + "].Presentation");
			Result = False;
		EndIf;
	EndDo;
...
Function GetSpreadsheetDocumentByBinaryData(Val BinaryData)
	...
	Except
		WriteLogEvent(NStr("ru = 'Получение табличного документа';
										|en = 'Get spreadsheet document';", Common.DefaultLanguageCode()), EventLogLevel.Error, , , 
...
Function GetNormalizedEmailInFormat(Text)
	...
	If Addresses.Count() > 1 Then
		CommonClient.MessageToUser(
			NStr("ru = 'Можно указывать только один адрес для ответа.';
				|en = 'Please specify a single reply-to address.';"), , "ReplyToAddress");
		Return Text;
	EndIf;
...
Procedure ShowQueryBoxBeforeCloseForm()
	QueryText = NStr("ru = 'Сообщение еще не отправлено. Закрыть форму?';
						|en = 'The message is not yet sent. Do you want to close the window?';");
	...
	ShowQueryBox(NotifyDescription, QueryText, Buttons,,
		DialogReturnCode.Cancel, NStr("ru = 'Отправка сообщения';
										|en = 'Send message';"));
EndProcedure
