...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Attribute = Undefined Then
		ErrorTemplate = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'При открытии формы %1 в параметре %2 указан не существующий реквизит %1';
				|en = 'When opening form %1, parameter %2 contains attribute %1 that does not exist';"), "InputInMultipleLanguages", "AttributeName");
	...
	Else
		Title = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = '%1 на разных языках';
				|en = '%1 in different languages';"), Attribute.Presentation());
