...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If TypeOf(Parameters.User) = Type("CatalogRef.ExternalUsers") Then
		Items.Profiles.Title = NStr("ru = 'Профили внешнего пользователя';
											|en = 'External user profiles';");
	Else
		Items.Profiles.Title = NStr("ru = 'Профили пользователя';
											|en = 'User profiles';");
	EndIf;
...
Procedure OnOpen(Cancel)
	...
	If TypeOf(FormOwner) <> Type("ClientApplicationForm")
	 Or FormOwner.Window <> Window Then
		...
		AutoTitle = False;
		Title = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Права доступа (%1)';
																				|en = 'Access rights (%1)';"), String(Parameters.User));
	EndIf;
