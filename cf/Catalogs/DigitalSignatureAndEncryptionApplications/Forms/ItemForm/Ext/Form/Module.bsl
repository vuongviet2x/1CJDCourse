...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	Items.Description.ChoiceList.Add("", NStr("ru = '<Другое приложение>';
														|en = '<Another app>';"));
...
Procedure FillCheckProcessingAtServer(Cancel, CheckedAttributes)
	...
	If Not Query.Execute().IsEmpty() Then
		...
		Common.MessageToUser(
			NStr("ru = 'Программа с указанным именем и типом уже добавлена в список.';
				|en = 'Application with the specified name and type has already been added to the list.';"),
			,
...
Procedure UsageModeChoiceProcessing(Item, ValueSelected, StandardProcessing)
	...
	If ValueSelected = PredefinedValue(
		...
		If ApplicationToSupply = Undefined Then
			StandardProcessing = False;
			ShowMessageBox(, NStr("ru = 'Приложение с указанными именем и типом не может быть определено автоматически.';
											|en = 'An app with the specified name and type cannot be detected automatically.';"));
			Return;
		EndIf;
...
Procedure SetDeletionMark(Command)
	...
	ShowQueryBox(
		...
		NStr("ru = 'Для установки отметки удаления запишите сделанные изменения.
		           |Записать данные?';
...
Procedure SetTitleAutoSettings(SettingToSupply)
	...
	If SettingToSupply = Undefined Then
		Items.DecorationLabelAutoSettings.Title = NStr("ru = 'Это приложение не может быть определено автоматически';
																			|en = 'This app cannot be detected automatically';");
		Return;
	EndIf;
	
	Items.DecorationLabelAutoSettings.Title = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'При работе с этим приложением используются алгоритмы:
			|Для подписи: %1
			|Для хеширования: %2
			|Для шифрования: %3';
			|en = 'Algorithms used in this app:
