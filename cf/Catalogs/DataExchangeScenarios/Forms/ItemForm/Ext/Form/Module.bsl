...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If IsNew
		...
		Description = NStr("ru = 'Сценарий синхронизации для %1';
	...
	If DataSeparationEnabled Then
		...
		ToolTipText = NStr("ru = 'Минимальный интервал не должен быть меньше 15 минут (900 секунд).
...
Procedure EditScheduledJobScheduleCompletion(Schedule, AdditionalParameters) Export
	...
	If Schedule <> Undefined Then
		...
		If DataSeparationEnabled 
			...
			WarningText = NStr("ru = 'Минимальный интервал не должен быть меньше 15 минут (900 секунд).';
										|en = 'Minimum interval must be over 15 minutes (900 seconds).';");
...
Procedure RefreshSchedulePresentation()
	...
	If SchedulePresentation = String(New JobSchedule) Then
		...
		SchedulePresentation = NStr("ru = 'Расписание не задано';
										|en = 'No schedule';");
...
Procedure ExecuteDataExchangeAtClient()
	...
	If NumberOfRowToProcess > RowsCount Then // Exit from the recursion.
		...
		Status(NStr("ru = 'Данные синхронизированы.';
						|en = 'Data is synchronized.';"), ?(OutputState, 100, Undefined));
	...
	If CurrentData.CurrentAction = PredefinedValue("Enum.ActionsOnExchange.DataImport") Then
		MessageString = NStr("ru = 'Выполняется получение данных из ""%1"".';
								|en = 'Receiving data from %1.';");
	ElsIf CurrentData.CurrentAction = PredefinedValue("Enum.ActionsOnExchange.DataExport") Then
		MessageString = NStr("ru = 'Выполняется отправка данных в ""%1"".';
								|en = 'Sending data to %1.';");
	EndIf;
		...
		If Not Result.IsEmpty() Then
			...
			While Selection.Next() Do
				...
				If Selection.MissingAction = 1 Then
					MessageTemplate = NStr("ru = 'Для информационной базы ""%1"" не хватает действия по отправке данных';
											|en = 'An action to send data is missing for infobase ""%1""';");
				Else
					MessageTemplate = NStr("ru = 'Для информационной базы ""%1"" не хватает действия по получению данных';
