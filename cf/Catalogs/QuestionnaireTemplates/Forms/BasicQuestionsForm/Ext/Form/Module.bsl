...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If TreeRowType = "Section" Then
		...
		Items.Wording.Title             = NStr("ru = 'Имя раздела';
															|en = 'Section name';");
		Title                                   = NStr("ru = 'Раздел шаблона анкеты';
															|en = 'Questionnaire template section';");
...
Procedure MoveToTemplate(Command)
	...
	If Not ValueIsFilled(Wording) Then
		Cancel = True;
		CommonClient.MessageToUser(NStr("ru = 'Не заполнена формулировка';
														|en = 'Wording not filled in';"),,"Wording");
	EndIf;
	
	If TreeRowType = "DoQueryBox" And (Not ValueIsFilled(ElementaryQuestion)) Then
		Cancel = True;
		CommonClient.MessageToUser(NStr("ru = 'Не указан элементарный вопрос';
														|en = 'General question is not specified';"),,"ElementaryQuestion");
	EndIf; 
		
	If ShouldUseRefusalToAnswer And Not ValueIsFilled(RefusalToAnswerText) Then
		Cancel = True;
		CommonClient.MessageToUser(NStr("ru = 'Не заполнена формулировка';
														|en = 'Wording not filled in';"),,"RefusalToAnswerText");
	EndIf;
