...
Procedure BeforeWriteAtServer(Cancel, CurrentObject, WriteParameters)
	...
	If QuestionnaireTemplateTree.Rows[0].Rows.Find("","Wording",True) <> Undefined Then
		Common.MessageToUser(NStr("ru = 'Не все формулировки или имена разделов заполнены.';
													|en = 'Not all wordings or section names are filled in.';"),,"QuestionnaireTree");
		Cancel = True;
	EndIf;
	...
	If FoundRows.Count() <> 0 Then
		For Each FoundRow In FoundRows Do
			If FoundRow.QuestionType <> Enums.QuestionnaireTemplateQuestionTypes.Tabular
				...
				Common.MessageToUser(NStr("ru = 'Не все вопросы заполнены.';
															|en = 'Not all questions are filled in.';"),,"QuestionnaireTree");
				Cancel = True;
				Break;
...
Procedure QuestionnaireTreeFormBeforeAddRow(Item, Cancel, Copy, Parent, Var_Group)
	...
	ChoiceList.Add("Section", NStr("ru = 'Раздел';
										|en = 'Section';"));
	ChoiceList.Add("Basic", NStr("ru = 'Простой вопрос';
											|en = 'Basic question';"));
	ChoiceList.Add("Complex", NStr("ru = 'Комплексный вопрос';
												|en = 'Interview question';"));
	ChoiceList.Add("Conditional", NStr("ru = 'Условный вопрос';
											|en = 'Conditional question';"));
	ChoiceList.Add("Tabular", NStr("ru = 'Табличный вопрос';
											|en = 'Question chart';"));
	
	OnCloseNotifyHandler = New NotifyDescription("SelectAddedItemTypeOnCompletion", ThisObject);
	ChoiceList.ShowChooseItem(OnCloseNotifyHandler, NStr("ru = 'Выберите тип добавляемого элемента.';
																			|en = 'Select a type of the item being added.';"), ChoiceList[0]);
...
Procedure QuestionnaireTreeNotesStartChoice(Item, ChoiceData, StandardProcessing)
	...
	CommonClient.ShowMultilineTextEditingForm(ClosingNotification1, Item.EditText, NStr("ru = 'Заметки';
																																	|en = 'Notes';"));
...
Procedure SetConditionalAppearance()
	...
	ItemFilter.RightValue = NStr("ru = 'Вопрос';
										|en = 'Question';");
	...
	ItemFilter.LeftValue = New DataCompositionField("QuestionnaireTree.RowType");
	ItemFilter.ComparisonType = DataCompositionComparisonType.NotEqual;
	ItemFilter.RightValue = NStr("ru = 'Корень';
										|en = 'Root';");
...
Procedure AddQuestion(CurrentData, QuestionType)
	...
	If Parent.RowType = "Root" Then
		ShowMessageBox(, NStr("ru = 'Вопросы нельзя добавлять в корень анкеты.';
										|en = 'Cannot add questions to the questionnaire root.';"));
		Return;
	EndIf;
