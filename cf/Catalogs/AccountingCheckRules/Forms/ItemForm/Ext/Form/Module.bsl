...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Not ValueIsFilled(Object.Ref) Then
		Raise NStr("ru = 'Интерактивное создание запрещено.';
								|en = 'Interactive creation is prohibited.';");
	EndIf;
...
Procedure ExecuteCheck(Command)
	...
	If Not Object.Use Then
		...
		ShowQueryBox(CallbackOnCompletion, NStr("ru = 'Проверка отключена. Все равно выполнить?';
													|en = 'Checkup is disabled. Check anyway?';"), QuestionDialogMode.YesNo);
		Return;
	EndIf;	
...
Procedure CustomizeStandardSettings(Command)
	...
	QueryText = NStr("ru = 'Установить стандартные настройки?';
						|en = 'Set standard settings?';");
...
Procedure ClearCheckResults(Command)
	...
	Message = NStr("ru = 'Результаты предыдущих проверок успешно очищены.';
					|en = 'The results of the previous checks are successfully cleared.';");
...
Function RunCheckAtServer()
	...
	ExecutionParameters.BackgroundJobDescription = StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = 'Выполнение проверки ведения учета ""%1""';
			|en = 'Run data integrity check ""%1""';"), Object.Description);
	...
	If Not Common.DataSeparationEnabled() Then
		...
		If SeparateScheduledJob = Undefined Then
			Items.IndividualSchedulePresentation.Title = 
				New FormattedString(NStr("ru = 'Настроить расписание';
												|en = 'Set schedule';"), , , , PutToTempStorage("AddJob", UUID));
		Else
			Items.IndividualSchedulePresentation.Title = 
...
Procedure SetPathToHandlerProcedure(Form, CurrentCheckMetadata)
	Form.HandlerProcedurePath = ?(CurrentCheckMetadata = Undefined, NStr("ru = 'Не задан обработчик';
																						|en = 'Handler is not defined';"), CurrentCheckMetadata.HandlerChecks);
EndProcedure
...
Procedure SetStandardSettingsAtServer()
	...
	If CurrentCheckMetadata = Undefined Then
		Raise StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Проверка ведения учета с идентификатором ""%1"" не существует.';
				|en = 'Data integrity check with ID %1 does not exist.';"), Object.Id);
	EndIf;
