...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Not UsersInternal.ExternalUsersEmbedded() Then
		Raise NStr("ru = 'Внешние пользователи не предусмотрены в данной версии приложения.';
								|en = 'This application version does not support external users.';");
	EndIf;
		...
		For Each Item In TreeItems Do
			...
			If Item.IsNonExistingRole Then
				CommonClientServer.AddUserError(Errors,
					...
					StringFunctionsClientServer.SubstituteParametersToString(
						NStr("ru = 'Несуществующая роль ""%1"".';
							|en = 'Role ""%1"" does not exist.';"), 
						Item.Synonym),
					"Roles",
					TreeItems.IndexOf(Item),
					StringFunctionsClientServer.SubstituteParametersToString(
						NStr("ru = 'Несуществующая роль ""%1"" в строке %2.';
							|en = 'Non-existent role ""%1"" in line %2.';"), 
						Item.Synonym, "%1"));
			EndIf;
			If Item.IsUnavailableRole Then
				CommonClientServer.AddUserError(Errors,
					"Roles[%1].RolesSynonym",
					StringFunctionsClientServer.SubstituteParametersToString(
						NStr("ru = 'Роль ""%1"" недоступна для внешних пользователей.';
							|en = 'Role ""%1"" is unavailable to external users.';"), 
						Item.Synonym),
					"Roles",
					TreeItems.IndexOf(Item),
					StringFunctionsClientServer.SubstituteParametersToString(
						NStr("ru = 'Роль ""%1"" в строке %2 недоступна для внешних пользователей.';
							|en = 'Role ""%1"" in line %2 is unavailable to external users.';"), 
						Item.Synonym, "%1"));
			EndIf;
		EndDo;
	EndIf;
...
#Region FormTableItemsEventHandlersContent
	...
	If UserMessage <> Undefined Then
		ShowUserNotification(
			NStr("ru = 'Перемещение пользователей';
				|en = 'Move users';"), , UserMessage, PictureLib.DialogInformation);
	EndIf;
...
Procedure SelectPurpose(Command)
	...
	UsersInternalClient.SelectPurpose(ThisObject, NStr("ru = 'Выбор вида пользователей';
																	|en = 'Select users type';"), False, False, NotifyDescription);
...
Function ExtendedPickFormParameters()
	...
	PickingParameters.PickFormHeader = NStr("ru = 'Подбор участников группы внешних пользователей';
													|en = 'Pick external user group members';");
