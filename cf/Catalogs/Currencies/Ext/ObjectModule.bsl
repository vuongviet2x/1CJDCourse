...
Procedure FillCheckProcessing(Cancel, CheckedAttributes)
	...
	If Cancel Then
		Common.MessageToUser(
			NStr("ru = 'Курс валюты можно связать только с курсом независимой валюты.';
				|en = 'An exchange rate can only depend on an independent exchange rate.';"));
	EndIf;
	...
	If Not IsNew() And RateSource = Enums.RateSources.MarkupForOtherCurrencyRate
		...
		Common.MessageToUser(
			NStr("ru = 'Валюта не может быть подчиненной, так как она является основной для других валют.';
				|en = 'The currency cannot be subordinate because it is used as the base currency for other currencies.';"));
		Cancel = True;
	EndIf;
	...
	If ValueIsFilled(RateCalculationFormula) Then
		...
		Except
			...
			Refinement = CommonClientServer.ExceptionClarification(ErrorInfo,
				NStr("ru = 'Не удалось вычислить курс валюты по формуле:';
					|en = 'Failed to calculate the exchange rate using the formula:';"));
	...
	If RecordingOfCoursesIsRequired And IsBackgroundCurrencyExchangeRatesRecalculationRunning Then
		Raise NStr("ru = 'Не удалось записать валюту, так как еще не завершился фоновый пересчет курсов.
							   |Попробуйте записать валюту позже.';
								|en = 'Couldn''t save the currency because the background calculation of exchange rates is running.
...
Procedure StartBackgroundCurrencyExchangeRatesUpdate()
	...
	If Result.Status = "Error" Then
		Refinement = CommonClientServer.ExceptionClarification(Result.ErrorInfo,
			NStr("ru = 'Не удалось запустить обновление курсов валюты по причине:';
				|en = 'Couldn''t update exchange rates due to:';"));
...
Function CurrenciesUsedInCalculatingTheExchangeRate()
	...
	If QueryResult.IsEmpty() Then
		ErrorText = NStr("ru = 'В формуле должна быть использована хотя бы одна основная валюта.';
							|en = 'The formula must include at least one base currency.';");
