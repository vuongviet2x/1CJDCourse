...
Procedure ProcessDataForMigrationToNewVersion(Parameters) Export
	...
	If ObjectsProcessed = 0 And ObjectsWithIssuesCount <> 0 Then
		MessageText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Не удалось обработать некоторые макеты печати (пропущены): %1';
				|en = 'Couldn''t process (skipped) some print templates: %1';"),
		...
		WriteLogEvent(InfobaseUpdate.EventLogEvent(),
			EventLogLevel.Information, Metadata.InformationRegisters.UserPrintTemplates,,
				StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Обработана очередная порция макетов печати: %1';
						|en = 'Another batch of print templates is processed: %1';"),
			ObjectsProcessed));
	EndIf;
...
Procedure OnAddUpdateHandlers(Handlers) Export
	...
	Handler.Comment = NStr("ru = 'Заполняет сведения об источниках данных печати для пользовательских печатных форм. До завершения обработки некоторые печатные формы могут быть недоступны.';
									|en = 'Fills information about print data sources for custom print forms. Some print forms might be unavailable until processing is completed.';");
