...
Procedure BeforeWriteAtServer(Cancel, CurrentObject, WriteParameters)
	...
	If Not CheckInformation.Success Then
		Common.MessageToUser(NStr("ru = 'Шаблон сообщения не может быть записан.';
													|en = 'Couldn''t save template.';")
...
Function ProcessTemplateText()
	...
	If InvalidParameters.Count() > 0 Then
		ErrorText = ?(InvalidParameters.Count() = 1,
			NStr("ru = 'Некорректный реквизит в шаблоне сообщения:';
				|en = 'Invalid placeholder:';"),
			NStr("ru = 'Некорректные реквизиты в шаблоне сообщения:';
				|en = 'Invalid placeholders:';"));
...
Procedure CheckTemplateFilling(Command)
	...
	If CheckInformation.Success Then
		ShowMessageBox(, NStr("ru = 'Шаблон заполнен корректно';
										|en = 'Template is valid.';"));
	Else
...
Procedure CopyAttachment(Command)
	...
	If CurrentData.Ref = PredefinedValue("Catalog.MessageTemplatesAttachedFiles.EmptyRef") Then
		...
		QueryText = NStr("ru = 'Файл возможно скопировать только после записи шаблона сообщения. Записать?';
							|en = 'To copy the file, you need to save the template. Do you want to save it?';");
		ShowQueryBox(OnCloseNotifyHandler, QueryText, QuestionDialogMode.YesNo);
	Else
...
Procedure ShowFormItems(EmailFormat = "")
	...
	If Object.ForSMSMessages Then
		TitleSuffix = NStr("ru = 'Шаблон сообщения SMS';
								|en = 'Text message template';");
	...
	ElsIf Object.ForEmails Then
		TitleSuffix = NStr("ru = 'Шаблон сообщения электронного письма';
								|en = 'Email message template';");
	...
	Else
		TitleSuffix = NStr("ru = 'Шаблон сообщения';
								|en = 'Message template';");
		Items.FormEmailTextKind.Visible = False;
	...
	Else
		Title = TitleSuffix + " (" + NStr("ru = 'создание';
													|en = 'Create';")+ ")";
	EndIf;
...
Procedure SetEmailPlainText(TextWrappingRequired = False)
	Items.FormEmailTextKind.Title = NStr("ru = 'Обычный текст';
																|en = 'Plain Text';");
...
Function CopyAttachmentsFromSource()
	...
	ListOfFiles = New Array; // Array of DefinedType.AttachedFile
	ErrorList = Undefined;
	ErrorDescription = NStr("ru = 'Не удалось скопировать вложение по причине: %1';
							|en = 'Cannot copy attachment due to: %1';");
		...
		For Each Attachment In ListOfFiles Do
			If IsBlankString(Attachment.EmailFileID) Then
				...
				Except
					...
					WriteErrorToEventLog(EventNameMessageTemplates(), ErrorInfo, NStr("ru = 'Не удалось извлечь и записать присоединенный файл для копирования';
																											|en = 'Failed to extract and save the file';"));
...
Function EventNameMessageTemplates()
	...
	Return NStr("ru = 'Шаблоны сообщений';
				|en = 'Message templates';", Common.DefaultLanguageCode());
...
Procedure FillTemplateByExternalDataProcessor()
	...
	If Common.SubsystemExists("StandardSubsystems.AdditionalReportsAndDataProcessors") Then
		...
		Else
			...
			ErrorDescription = NStr("ru = 'Предмет %1 определенный в внешней обработке не найдено. Внешняя обработка не может быть подключена.';
									|en = 'Subject ""%1"" specified in the external data processor is not found. Cannot attach the external data processor.';");
