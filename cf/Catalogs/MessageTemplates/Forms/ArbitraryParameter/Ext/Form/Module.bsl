...
Procedure OK(Command)
	...
	For Each ParameterFromForm In ParametersList Do
		If StrCompare(ParameterFromForm.Value, ParameterName) = 0 Then
			ShowMessageBox(, NStr("ru = 'Некорректное имя параметра. Параметр с таким именем уже был добавлен ранее.';
		...
		If StrCompare(ParameterFromForm.Presentation, ParameterPresentation) = 0 Then
			ShowMessageBox(, NStr("ru = 'Некорректное представление параметра. Параметр с таким представлением уже был добавлен ранее.';
	...
	If InvalidParameterName(ParameterName) Or IsBlankString(ParameterName) Then
		ShowMessageBox(, NStr("ru = 'Некорректное имя параметра. Нельзя использовать пробелы, знаки пунктуации и другие спец. символы.';
										|en = 'Invalid placeholder name. Special characters and whitespace are not allowed.';"));
		Return;
	EndIf;
	
	If IsBlankString(ParameterPresentation) Then
		ShowMessageBox(, NStr("ru = 'Некорректное представление параметра.';
										|en = 'Invalid placeholder presentation.';"));
		Return;
	EndIf;
	
	If IsBlankString(TypeAsString) Then
		ShowMessageBox(, NStr("ru = 'Некорректный тип параметра.';
...
Procedure FillChoiceListInputOnBasis(ParameterType)
	...
	If ParameterType = Type("String") Then
		TypePresentation = NStr("ru = 'Строка';
								|en = 'String';");
	ElsIf ParameterType = Type("Date") Then
		TypePresentation = NStr("ru = 'Дата';
								|en = 'Date';");
	EndIf;
	
	Items.TypeAsString.ChoiceList.Insert(0, "Date", NStr("ru = 'Дата';
																|en = 'Date';"));
	Items.TypeAsString.ChoiceList.Insert(0, "String", NStr("ru = 'Строка';
																|en = 'String';"));
