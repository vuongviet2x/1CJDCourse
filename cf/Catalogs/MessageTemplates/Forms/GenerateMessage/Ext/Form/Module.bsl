...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If MessageKind = "SMSMessage" Then
		ForSMSMessages = True;
		Title = NStr("ru = 'Шаблоны сообщений SMS';
						|en = 'Text templates';");
	ElsIf MessageKind = "MailMessage" Then
		ForEmails = True;
	Else
		Title = NStr("ru = 'Шаблоны сообщений';
	...
	If ChoiceMode Or MessageKind = "Arbitrary" Then
		...
		Items.FormGenerate.Title = NStr("ru = 'Выбрать';
													|en = 'Select';");
...
Procedure AfterGenerateAndSendMessage(Result, SendOptions)
	...
	Else
		...
		ErrorDescription = Result.ErrorDescription + Chars.LF + NStr("ru = 'Открыть форму отправки сообщения?';
...
Procedure FillAvailableTemplatesList()
	...
	If Not ChoiceMode And Not PrepareTemplate Then
		...
		FirstRow.Presentation = NStr("ru = '<Без шаблона>';
											|en = '<No template>';");
	EndIf;
