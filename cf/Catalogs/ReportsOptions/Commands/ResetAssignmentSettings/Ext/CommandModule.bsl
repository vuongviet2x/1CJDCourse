...
Procedure CommandProcessing(Variants, CommandExecuteParameters)
	If TypeOf(Variants) <> Type("Array") Or Variants.Count() = 0 Then
		ShowMessageBox(, NStr("ru = 'Выберите варианты отчетов, для которых необходимо сбросить настройки размещения.';
										|en = 'Select report options to reset location settings.';"));
		Return;
	EndIf;
