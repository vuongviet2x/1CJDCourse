...
Procedure FormGetProcessing(FormType, Parameters, SelectedForm, AdditionalInformation, StandardProcessing)
	...
	If FormType = "ObjectForm" Then
		...
		If Not ValueIsFilled(OptionRef1) Then
			Raise NStr("ru = 'Новый вариант отчета можно создать только из формы отчета';
									|en = 'You can create a report option only from a report form.';");
		EndIf;
		...
		ElsIf OpeningParameters.ReportType = "Additional" Then
			...
			If Not OpeningParameters.Connected Then
				Raise NStr("ru = 'Вариант внешнего отчета можно открыть только из формы отчета.';
										|en = 'You can open an external report option only from a report form.';");
			EndIf;
		Else
			Raise NStr("ru = 'Вариант внешнего отчета можно открыть только из формы отчета.';
									|en = 'You can open an external report option only from a report form.';");
...
Procedure ProcessDataForMigrationToNewVersion(Parameters) Export
		...
		Except
			...
			If OptionDataSourceSetup Then
				CommentTemplate = NStr("ru = 'Не удалось установить источник данных варианта отчета %1.
										...
										|Details: %2.';");
			Else
				CommentTemplate = NStr("ru = 'Не удалось установить назначение варианта отчета %1.
										 |Возможно он поврежден и не подлежит восстановлению.
										 |
										 |Техническая информация о проблеме: %2';
										|en = 'Cannot identify the type of device for the %1 report option.
										|It might be corrupted and cannot be recovered.
										|
										|Technical details: %2';");
			EndIf;
	...
	If Processed = 0 And Declined <> 0 Then
		MessageTemplate = NStr("ru = 'Не удалось обработать некоторые варианты отчетов: %1';
								|en = 'Couldn''t process (skipped) some report options: %1';");
		MessageText = StringFunctionsClientServer.SubstituteParametersToString(MessageTemplate, Declined);

		Raise MessageText;
	Else
		CommentTemplate = NStr("ru = 'Обработан очередной пакет вариантов отчетов: %1';
								|en = 'Yet another batch of report options is processed: %1';");
