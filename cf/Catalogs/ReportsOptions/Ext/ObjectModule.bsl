	...
	If Description <> "" And ReportsOptions.DescriptionIsUsed(Report, Ref, Description) Then
		...
		Common.MessageToUser(
			StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = '""%1"" занято, укажите другое наименование.';
					|en = '""%1"" is taken. Enter another description.';"), 
				Description),, 
			"Description");
	EndIf;
...
Procedure BeforeWrite(Cancel)
	...
	If Not Custom And UserChangedDeletionMark Then
		If DeletionMark Then
			ErrorText = NStr("ru = 'Пометка на удаление предопределенного варианта отчета запрещена.';
								|en = 'Predefined report options cannot be marked for deletion.';");
		Else
			ErrorText = NStr("ru = 'Снятие пометки удаления предопределенного варианта отчета запрещено.';
								|en = 'Predefined report options cannot be unmarked for deletion.';");
		EndIf;
		Raise ErrorText;
	EndIf;
	...
	If Not DeletionMark And UserChangedDeletionMark Then
		...
		If DescriptionIsUsed Or OptionKeyIsUsed Then
			ErrorText = NStr("ru = 'Не удалось снять пометку удаления варианта отчета:';
								|en = 'Cannot clear the deletion mark from the report option:';");
			If DescriptionIsUsed Then
				ErrorText = ErrorText + StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Наименование ""%1"" уже занято другим вариантом этого отчета.';
						|en = 'Name ""%1"" is taken by another option of this report.';"), 
					Description);
			Else
				ErrorText = ErrorText + StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Ключ варианта ""%1"" уже занят другим вариантом этого отчета.';
						|en = 'Key ""%1"" is assigned to another option of this report.';"), 
					VariantKey);
			EndIf;
			ErrorText = ErrorText + NStr("ru = 'Перед снятием пометки удаления варианта отчета
											...
											|en = 'Before you clear the deletion mark for the report option 
...
Procedure FillFieldsForSearch()
	...
	Except
		ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Не удалось проиндексировать схему варианта ""%1"" отчета ""%2"":';
				|en = 'Cannot index the scheme of option ""%1"" of report ""%2"":';"), 
...
Procedure CheckPredefinedReportOptionFilling(Cancel)
	...
	ElsIf ReportType <> ReportsOptions.ReportType(Report) Then
		ErrorText = NStr("ru = 'Противоречивые значения полей ""%1"" и ""%2""';
							|en = 'Fields ""%1"" and ""%2"" contains inconsistent values.';");
...
Function FieldIsRequired(FieldName)
	...
	Return StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Не заполнено поле ""%1""';
																		|en = 'Field %1 is required.';"), FieldName);
