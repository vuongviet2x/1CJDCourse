...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	TreeRow.Presentation  = NStr("ru = 'Не включенные в разделы';
	...
	ChoiceList.Add(1, NStr("ru = 'Все, кроме внешних';
									|en = 'All but external reports';"));
	ChoiceList.Add(Enums.ReportsTypes.BuiltIn, NStr("ru = 'Внутренние';
																	|en = 'Integrated reports';"));
	ChoiceList.Add(Enums.ReportsTypes.Extension, NStr("ru = 'Расширения';
																	|en = 'Extensions';"));
	ChoiceList.Add(Enums.ReportsTypes.Additional, NStr("ru = 'Дополнительные';
																		|en = 'Additional reports';"));
	ChoiceList.Add(Enums.ReportsTypes.External, NStr("ru = 'Внешние';
																|en = 'External reports';"));
...
Procedure SubsystemsTreeDrag(Item, DragParameters, StandardProcessing, String, Field)
	...
	If PlacementParameters.Variants.Total = 1 Then
		If PlacementParameters.Action = "Copy" Then
			QuestionTemplate = NStr("ru = 'Разместить ""%1"" в ""%4""?';
								|en = 'Do you want to add ""%1"" to %4?';");
		Else
			QuestionTemplate = NStr("ru = 'Переместить ""%1"" из ""%3"" в ""%4""?';
								|en = 'Do you want to move %1 from %3 to %4?';");
		EndIf;
	Else
		If PlacementParameters.Action = "Copy" Then
			QuestionTemplate = NStr("ru = 'Разместить варианты отчетов ""%1"" (%2 шт.) в ""%4""?';
								|en = 'Do you want to add %2 report options %1 to %4?';");
		Else
			QuestionTemplate = NStr("ru = 'Переместить варианты отчетов ""%1"" (%2 шт.) из ""%3"" в ""%4""?';
								|en = 'Do you want to move %2 report options %1 from %3 to %4?';");
		EndIf;
	EndIf;
...
Function PlacementParameters(DragParameters, String)
	...
	If Source.Ref = Receiver.Ref Then
		ShowMessageBox(, NStr("ru = 'Выбранные варианты отчетов уже в данном разделе.';
										|en = 'Selected report options already assigned to this section.';"));
		Return Undefined;
	EndIf;
	...
	If PlacementParameters.Variants.Total = ExecutionResult.Placed Then
		If PlacementParameters.Variants.Total = 1 Then
			If PlacementParameters.Action = "Move" Then
				Template = NStr("ru = 'Успешно перемещен в ""%1"".';
								|en = 'Report successfully moved to %1.';");
			Else
				Template = NStr("ru = 'Успешно размещен в ""%1"".';
								|en = 'Report successfully added to %1.';");
			EndIf;
			Text = PlacementParameters.Variants.Presentation;
			Ref = GetURL(PlacementParameters.Variants.Array[0]);
		Else
			If PlacementParameters.Action = "Move" Then
				Template = NStr("ru = 'Успешно перемещены в ""%1"".';
								|en = 'Reports successfully moved to %1.';");
			Else
				Template = NStr("ru = 'Успешно размещены в ""%1"".';
								|en = 'Reports successfully added to %1.';");
			EndIf;
			Text = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Варианты отчетов (%1).';
																				|en = '%1 report options.';"),
	...
	Else
		...
		If Not IsBlankString(ExecutionResult.CannotBePlaced) Then
			ErrorsText = ?(ErrorsText = "", "", ErrorsText + Chars.LF + Chars.LF) 
				+ NStr("ru = 'Не могут размещаться в командном интерфейсе:';
						|en = 'Cannot add to the command interface:';") + Chars.LF
				+ ExecutionResult.CannotBePlaced;
		EndIf;
		If Not IsBlankString(ExecutionResult.AlreadyAssigned) Then
			ErrorsText = ?(ErrorsText = "", "", ErrorsText + Chars.LF + Chars.LF) 
				+ NStr("ru = 'Уже размещены в этом разделе:';
						|en = 'Already added to this section:';") + Chars.LF + ExecutionResult.AlreadyAssigned;
		EndIf;
		...
		Else
			Template = NStr("ru = 'Размещено вариантов отчетов: %1 из %2.
						  |Подробности:
						  |%3';
							|en = '%1 out of %2 report options have been added.
							|Details:
							|%3';");
		EndIf;
...
Procedure SetSubsystemsTreeTitle()
	Items.SectionsGroup.Title = ?(Items.SubsystemsTree.CurrentData = Undefined, 
		NStr("ru = 'Все разделы';
			|en = 'All sections';", CommonClient.DefaultLanguageCode()),
		...
		For Each OptionRef In PlacementParameters.Variants.Array Do
			If OptionRef.ReportType = Enums.ReportsTypes.External Then
				CannotBePlaced = ?(CannotBePlaced = "", "", CannotBePlaced + Chars.LF) + "  " 
					+ String(OptionRef) + " (" + NStr("ru = 'внешний';
															|en = 'external';") + ")";
				Continue;
			ElsIf OptionRef.DeletionMark Then
				CannotBePlaced = ?(CannotBePlaced = "", "", CannotBePlaced + Chars.LF) + "  " 
					+ String(OptionRef) + " (" + NStr("ru = 'помечен на удаление';
