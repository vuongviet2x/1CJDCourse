...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	MixedImportance = NStr("ru = 'Различная';
							|en = 'Mixed';");
...
Procedure Place(Command)
	WriteAtServer();
	NotificationText1 = NStr("ru = 'Изменены настройки вариантов отчетов (%1 шт.).';
							|en = 'Settings changed for %1 report options.';");
...
Procedure FillSections()
	...
	If ErrorsCount > 0 Then
		...
		For Each TableRow In FilteredOptions Do
			If CurrentReason <> TableRow.Cause Then
				...
				If CurrentReason = 1 Then
					ErrorsMessages.More = ErrorsMessages.More + NStr("ru = 'Помеченные на удаление:';
																					|en = 'Marked for deletion:';");
				ElsIf CurrentReason = 2 Then
					ErrorsMessages.More = ErrorsMessages.More + NStr("ru = 'Недостаточно прав для изменения:';
																					|en = 'Insufficient rights to modify:';");
				ElsIf CurrentReason = 3 Then
					ErrorsMessages.More = ErrorsMessages.More + NStr("ru = 'Отчет отключен или недоступен по правам:';
																					|en = 'The report is disabled or cannot be accessed with the rights:';");
				ElsIf CurrentReason = 4 Then
					ErrorsMessages.More = ErrorsMessages.More + NStr("ru = 'Вариант отчета отключен по функциональной опции:';
																					|en = 'Report option is disabled using the functional option:';");
				EndIf;
			EndIf;
		...
		If OptionsCount = 0 Then
			ErrorsMessages.Text = NStr("ru = 'Недостаточно прав для размещения в разделах выбранных вариантов отчетов.';
											|en = 'Insufficient rights to add selected report options to sections.';");
		Else
			ErrorsMessages.Text = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Недостаточно прав для размещения в разделах некоторых вариантов отчетов (%1).';
					|en = 'Insufficient rights to add some report options (%1) to sections.';"),
