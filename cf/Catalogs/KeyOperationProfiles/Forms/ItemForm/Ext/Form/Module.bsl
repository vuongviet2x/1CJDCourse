...
Procedure LoadKeyOperationsProfile(Command)
	...
	If CoreAvailable Then
		...
		If TypeOf(ModuleFileSystemClient) = Type("CommonModule") Then
			...
			ImportParameters.Dialog.Title = NStr("ru = 'Выберите файл профиля ключевых операций';
														|en = 'Select file of key operation profile';");
	...
	Else          		
		...
		FileDialogMode.Open, 
		NStr("ru = 'Выберите файл профиля ключевых операций';
			|en = 'Select file of key operation profile';"),
...
Procedure SaveKeyOperationsProfile(Command)
	...
	If CoreAvailable Then
		...
		If TypeOf(ModuleFileSystemClient) = Type("CommonModule") Then
			...
			SavingParameters.Dialog.Title = NStr("ru = 'Сохранить профиль ключевых операций в файл';
														|en = 'Save key operation profile to file';");
	...
	Else              		
		AdditionalParameters = New Structure("Mode, Title, ClosingNotification1",  
			FileDialogMode.Save, 
			NStr("ru = 'Сохранить профиль ключевых операций в файл';
				|en = 'Save key operation profile to file';"),
...
Procedure SaveFileDialogCompletion(SelectedFiles, AdditionalParameters) Export
	...
	Status(NStr("ru = 'Сохранение файлов завершено.';
