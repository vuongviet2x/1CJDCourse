	...
	If Common.DataSeparationEnabled() Then
		...
		If Not CanChangeUsers Then
			If Object.Ref.IsEmpty() Then
				Raise NStr("ru = 'В демонстрационном режиме создание новых учетных записей пользователей недоступно.';
										|en = 'The demo does not support creating new user accounts.';");
			EndIf;
			ReadOnly = True;
		EndIf;
...
Procedure BeforeWrite(Cancel, WriteParameters)
	...
	QuestionTitle1 = NStr("ru = 'Запись пользователя информационной базы';
	   ...
	   And (Not WriteParameters.Property("NotCopyUserRights")
		...
		ShowQueryBox(
			...
			StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Скопировать также права пользователя ""%1""?';
					|en = 'Do you want to copy the rights of the user ""%1""?';"), String(CopyingValue)),
	...
	If Not WriteParameters.Property("WithEmptyRoleList")
	   ...
	   And IBUserRoles.Count() = 0 Then
		...
		ShowQueryBox(
			...
			NStr("ru = 'Пользователю информационной базы не установлено ни одной роли. Продолжить?';
	...
	If IBUserWritingRequired(ThisObject) Then
					...
					If ValueIsFilled(AttributeWithEmailForPasswordRecoveryName)
						...
						If Not ChangeEmailWithoutPasswordConfirmation Then
							...
							If Not ThePasswordIsTheSameAsTheSavedOne Then
								PasswordToConfirmEmailChange = Undefined;
								Raise NStr("ru = 'Пароль указан неверно';
		...
		If WriteParameters.Property("WithFirstAdministratorAdding") Then
			CurrentObject.AdditionalProperties.Insert("CreateAdministrator",
				NStr("ru = 'Первый пользователь информационной базы назначается администратором.';
					|en = 'The first infobase user is granted administrator rights.';"));
		EndIf;
	EndIf;
	...
	If CanSignIn
	   ...
	   And ValidityPeriod <= BegOfDay(CurrentSessionDate()) Then
		...
		Common.MessageToUser(
			NStr("ru = 'Ограничение должно быть до завтра или более.';
				|en = 'The expiration date must be tomorrow or later.';"),, "CanSignIn",, Cancel);
	EndIf;
		...
		For Each Item In TreeItems Do
			...
			If Item.IsNonExistingRole Then
				CommonClientServer.AddUserError(Errors,
					...
					StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Несуществующая роль ""%1"".';
																				|en = 'Role ""%1"" does not exist.';"), Item.Synonym),
					"Roles",
					TreeItems.IndexOf(Item),
					StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Несуществующая роль ""%1"" в строке %2.';
																				|en = 'Non-existent role ""%1"" in line %2.';"), Item.Synonym, "%1"));
			EndIf;
			If Item.IsUnavailableRole Then
				CommonClientServer.AddUserError(Errors,
					"Roles[%1].RolesSynonym",
					StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Роль ""%1"" недоступна для пользователей.';
																				|en = 'Role ""%1"" is unavailable to users.';"), Item.Synonym),
					"Roles",
					TreeItems.IndexOf(Item),
					StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Роль ""%1"" в строке %2 недоступна для пользователей.';
																				|en = 'Role ""%1"" in line %2 is unavailable to users.';"), Item.Synonym, "%1"));
			EndIf;
		EndDo;
...
Procedure CanSignInOnChange(Item)
	...
	If Object.DeletionMark And CanSignIn Then
		CanSignIn = False;
		ShowMessageBox(,
			NStr("ru = 'Чтобы разрешить вход в приложение, снимите пометку на удаление с этого пользователя.';
				|en = 'To allow logging in to the application, clear the deletion mark from the user.';"));
		Return;
	EndIf;
	...
	If Not AccessLevel.ChangeAuthorizationPermission
	   And Not CanSignIn Then
		...
		ShowMessageBox(,
			NStr("ru = 'После записи вход в приложение сможет разрешить только администратор.';
				|en = 'Once you save the changes, only the administrator can allow login to the application.';"));
	EndIf;
...
Procedure PhotoClick(Item, StandardProcessing)
	...
	ImportParameters.Dialog.Filter = NStr("ru = 'Изображения';
...
#If Not WebClient Then
		...
		If Picture.Format() = PictureFormat.UnknownFormat Then
			ShowMessageBox(, NStr("ru = 'Необходимо выбрать файл с изображением.';
											|en = 'Select a file with a picture.';"));
			Return;
		EndIf;
		
		If Picture.FileSize() > 2 * 1024 * 1024 Then
			ShowMessageBox(, NStr("ru = 'Необходимо выбрать файл с изображением меньше 2 Мб.';
...
Procedure CheckFirstAdministrator() 
	...
	QuestionTitle = NStr("ru = 'Запись пользователя информационной базы';
							|en = 'Save infobase user';");
	ShowQueryBox(
...
Procedure FindUserAndIBUserDifferences(WriteParameters = Undefined)
	...
	ElsIf AccessLevel.ListManagement Then
		...
		If IBUserFullName <> Object.Description Then
			...
			PropertiesToResolve.Insert(0, StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Полное имя ""%1""';
																										|en = 'Full name: ""%1""';"),
				IBUserFullName));
		EndIf;
		...
		If CanSignInOnRead And Object.Invalid Then
			...
			PropertiesToResolve.Insert(0, NStr("ru = 'Вход в приложение разрешен';
												|en = 'Login allowed';"));
		EndIf;
		...
		If AccessLevel.ChangeCurrent Then
			If Common.SubsystemExists("StandardSubsystems.ContactInformation") Then
				...
				If DetailsString = Undefined Then
					...
					PropertiesToResolve.Insert(0, StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Почта для восстановления пароля ""%1""';
		 ...
		 Or AreSavedInfobaseUserPropertiesMismatch Then
			...
			If ShowCommentToDiffResolve Then
				Recommendation = Chars.LF
					+ NStr("ru = 'Нажмите ""Записать"", чтобы устранить различия и не выводить это предупреждение.';
							|en = 'To resolve the differences and not to show this message again, click ""Save"".';");
			
			ElsIf Not Users.IsFullUser() Then
				Recommendation = Chars.LF
					+ NStr("ru = 'Обратитесь к администратору, чтобы устранить различия.';
			...
			If ValueIsFilled(PropertiesToResolveString) Then
				MismatchClarification = StringFunctionsClientServer.SubstituteParametersToString(
					...
					|" + NStr("ru = 'Картинка в списке пользователей может отображать устаревшее состояние,
	...
	If ActionsOnForm.ItemProperties = "Edit" Then
		Recommendation = Chars.LF
			+ NStr("ru = 'Нажмите ""Записать"", чтобы устранить проблему и не выводить это предупреждение.';
	...
	Items.MappingMismatchNote.Title =
		NStr("ru = 'Несуществующий пользователь информационной базы.';
			|en = 'The infobase user does not exist.';") + Recommendation;
