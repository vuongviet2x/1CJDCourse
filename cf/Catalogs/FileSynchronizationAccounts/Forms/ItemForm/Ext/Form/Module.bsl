...
Procedure ServiceChoiceProcessing(Item, ValueSelected, StandardProcessing)
	...
	Else
		Object.Description = NStr("ru = 'Облачный сервис файлов';
									|en = 'Cloud file service';");	
	EndIf;
...
Procedure CheckSettings(Command)
	...
	If Object.Ref.IsEmpty() Or Modified Then
		...
		QueryText = NStr("ru = 'Для проверки настроек необходимо записать данные учетной записи. Продолжить?';
...
Procedure CheckCanSyncWithCloudService()
	...
	If ResultStructure1.Cancel Then
		...
		ErrorMessage = NStr("ru = 'Проверка параметров для синхронизации файлов завершилась неудачно.
				|
				|Рекомендуем:
				|%3
				|
				...
				|%5%4';
				|en = 'Failed to check parameters for file synchronization.
				|
		...
		Recommendations.Add(StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Повторить попытку позднее (возможно временные неполадки на сервисе). Обратиться в поддержку сервиса %1.';
				|en = 'Try again later (there might be temporary issues in the service). Contact the technical service %1.';"),
			Object.Service));
		Recommendations.Add(NStr("ru = 'Выбрать другой сервис для синхронизации файлов.';
									|en = 'Select another service for file synchronization.';"));
		...
		ElsIf ResultStructure1.ErrorCode = 404 Then
			Recommendations.Insert(0, NStr("ru = 'Проверить, что указанная корневая папка существует в облачном сервисе.';
											|en = 'Check whether the specified root folder exists in the cloud service.';"));
		ElsIf ResultStructure1.ErrorCode = 401 Then
			Recommendations.Insert(0, NStr("ru = 'Проверить правильность введенных логина и пароля.';
											|en = 'Check whether the username and password are valid.';"));
		...
		Else
			Recommendations.Insert(0, NStr("ru = 'Проверьте правильность введенных данных.';
											|en = 'Check the validity of the data you entered.';"));
		EndIf;
		
		QuestionParameters = StandardSubsystemsClient.QuestionToUserParameters();
		QuestionParameters.PromptDontAskAgain = False;
		QuestionParameters.Picture = PictureLib.DialogStop;
		QuestionParameters.Title = NStr("ru = 'Проверка настройки';
	...
	Else
		...
		StandardSubsystemsClient.ShowQuestionToUser(
			Undefined,
			StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Проверка параметров для синхронизации файлов завершилась успешно. 
						   |%1';
...
Function CheckConnection(Val Service, Val ProtocolText)
	...
	Else
		...
		Return New Structure("ErrorDescription, DiagnosticsLog",
			NStr("ru = 'Проверьте соединение с сетью Интернет.';
				|en = 'Please check the Internet connection.';"), ProtocolText);
