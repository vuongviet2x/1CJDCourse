	   ...
	   And IBUserRoles.Count() = 0 Then
		...
		If Not WriteParameters.Property("WithEmptyRoleList") Then
			...
			ShowQueryBox(
				...
				,
				,
				NStr("ru = 'Запись пользователя информационной базы';
					|en = 'Save infobase user';"));
			Return;
		EndIf;
	EndIf;
	...
	If Not Items.Roles.ReadOnly Then
		...
		For Each Item In TreeItems Do
			...
			If Item.IsUnavailableRole Then
				CommonClientServer.AddUserError(Errors,
					"Roles[%1].RolesSynonym",
					StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Роль ""%1"" недоступна для внешних пользователей.';
																				|en = 'Role ""%1"" is unavailable to external users.';"), Item.Synonym),
					"Roles",
					TreeItems.IndexOf(Item),
					StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Роль ""%1"" в строке %2 недоступна для внешних пользователей.';
																				|en = 'Role ""%1"" in line %2 is unavailable to external users.';"), Item.Synonym, "%1"));
			EndIf;
		EndDo;
...
Procedure CanSignInOnChange(Item)
	...
	If Object.DeletionMark And CanSignIn Then
		CanSignIn = False;
		ShowMessageBox(,
			NStr("ru = 'Чтобы разрешить вход в приложение, снимите
			           |пометку на удаление с этого внешнего пользователя.';
						|en = 'To allow logging in to the application, clear
...
Procedure FindUserAndIBUserDifferences(WriteParameters = Undefined)
	...
	ElsIf AccessLevel.ListManagement Then
		...
		If IBUserOSAuthentication <> False Then
			PropertiesToResolve.Add(NStr("ru = 'Аутентификация ОС (включена)';
											|en = 'OS authentication (enabled)';"));
		EndIf;
		...
		If IBUserOSUser <> "" Then
			PropertiesToResolve.Add(NStr("ru = 'Пользователь ОС (указан)';
											|en = 'OS user (specified)';"));
		EndIf;
		
		If IBUserShowInList Then
			HasDifferencesResolvableWithoutAdministrator = True;
			PropertiesToResolve.Add(NStr("ru = 'Показывать в списке выбора (включено)';
											|en = 'Show in choice list (enabled)';"));
		EndIf;
		
		If IBUserRunMode <> "Auto" Then
			HasDifferencesResolvableWithoutAdministrator = True;
			PropertiesToResolve.Add(NStr("ru = 'Режим запуска (не Авто)';
											|en = 'Run mode (not Auto)';"));
		EndIf;
		 ...
		 Or AreSavedInfobaseUserPropertiesMismatch Then
			...
			If ValueIsFilled(PropertiesToResolveString) Then
				MismatchClarification = StringFunctionsClientServer.SubstituteParametersToString(
					...
					|" + NStr("ru = 'Картинка в списке внешних пользователей и объектов авторизации может отображать устаревшее состояние,
					                |так как отличаются и некоторые другие сохраненные свойства.';
									|en = 'The picture in the list of external users and authorization objects might display an outdated state
