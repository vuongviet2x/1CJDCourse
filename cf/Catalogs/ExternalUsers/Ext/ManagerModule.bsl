	...
	If FormType = "ObjectForm" And Parameters.Property("AuthorizationObject") Then
		...
		Else
			ErrorAsWarningDetails =
				NStr("ru = 'Разрешение на вход в приложение не предоставлялось.';
					|en = 'The right to log in is not granted.';");
...
Procedure ProcessDataForMigrationToNewVersion(Parameters) Export
	...
	If ObjectsProcessed = 0 And ObjectsWithIssuesCount <> 0 Then
		MessageText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Не удалось обработать некоторых внешних пользователей (пропущены): %1
				|%2';
				|en = 'Couldn''t process (skipped) some external users: %1
	...
	Else
		WriteLogEvent(InfobaseUpdate.EventLogEvent(), EventLogLevel.Information,
			...
			StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Обработана очередная порция внешних пользователей: %1';
					|en = 'Yet another batch of external users is processed: %1';"),
				ObjectsProcessed));
	EndIf;
