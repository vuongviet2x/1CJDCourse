...
Procedure Print(ObjectsArray, PrintParameters, PrintFormsCollection, PrintObjects, OutputParameters) Export
	...
	If Common.SubsystemExists("StandardSubsystems.Print") Then
		Title = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Производственный календарь ""%1"" на %2 год';
				|en = 'Business calendar ""%1"" for %2';"), 
...
Function DefaultBusinessCalendars() Export
	...
	Except
		EventName = NStr("ru = 'Календарные графики.Получение календарей из классификатора';
							|en = 'Calendar schedules.Get calendars from classifier';", Common.DefaultLanguageCode());
		MessageText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Не удалось получить список производственных календарей из классификатора.
                  ...
                  |%1';
					|en = 'Cannot get business calendars from the classifier.
...
Function BusinessCalendarPrintForm(PrintFormPreparationParameters)
	...
	While SelectionByYear.Next() Do
		...
		While SelectionByQuarter.Next() Do
			...
			QuarterNumber.Parameters.QuarterNumber = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = '%1 квартал';
																												|en = 'quarter %1';"), SelectionByQuarter.CalendarQuarter);
			...
			If SelectionByQuarter.CalendarQuarter = 2 
				...
				MonthColumn.Parameters.MonthName = StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = '%1 полугодие';
						|en = 'half year %1';"), SelectionByQuarter.CalendarQuarter / 2);
				SpreadsheetDocument.Join(MonthColumn);
			EndIf;
		EndDo;
		MonthColumn = Template.GetArea("MonthColumn");
		...
		MonthColumn.Parameters.MonthName = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = '%1 год';
				|en = 'year %1';"), Format(SelectionByYear.CalendarYear, "NG="));
		SpreadsheetDocument.Join(MonthColumn);
	EndDo;
	
	MonthColumn = Template.GetArea("MonthAverage");
	FillAreaParameters(MonthColumn.Parameters, ForYear);
	MonthColumn.Parameters.MonthName = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = '%1 год';
																									|en = 'year %1';"), Format(YearNumber, "NG="));
	...
	MonthColumn.Parameters.MonthName = NStr("ru = 'Среднемесячное количество';
