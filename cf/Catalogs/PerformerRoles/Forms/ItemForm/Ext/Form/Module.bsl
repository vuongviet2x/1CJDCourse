		...
		For Each TableRow In Object.Purpose Do
			If TypeOf(TableRow.UsersType) <> TypeOf(Catalogs.Users.EmptyRef()) Then
				...
				Common.MessageToUser( 
					StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Роль не может использоваться с обязательным уточнением для назначения: %1.';
																				|en = 'Cannot use the Role with the refinement: %1.';"), PurposeDescription ),,,
...
Procedure SelectPurpose(Command)
	...
	UsersInternalClient.SelectPurpose(ThisObject, NStr("ru = 'Выбор назначения роли';
																	|en = 'Select role assignment';"),,, NotifyDescription);
EndProcedure
