...
Procedure ProcessVersionStoragePath(Parameters) Export
	...
	If ObjectsProcessed = 0 And ObjectsWithIssuesCount <> 0 Then
		MessageText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Не удалось обработать некоторые версии файлов (пропущены): %1
				 |%2';
				|en = 'Couldn''t process (skipped) some file versions: %1
	...
	Else
		WriteLogEvent(InfobaseUpdate.EventLogEvent(),
			...
			StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Обработана очередная порция версий файлов: %1';
					|en = 'Another batch of file versions is processed: %1';"), 
				ObjectsProcessed));
	EndIf;
EndProcedure
...
Function RemoveExtraSeparator(VersionRef)
	...
	Except
		...
		MessageText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Не удалось обработать версию файла %1 по причине: %2';
				|en = 'Cannot process file version %1. Reason: %2';"), VersionRef,
