...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	EndIf;
	If Parameters.Representation <> Undefined Then
	...
	ChoiceList.Add(1, NStr("ru = 'Только отчеты';
									|en = 'Reports only';"));
	ChoiceList.Add(2, NStr("ru = 'Только обработки';
									|en = 'Data processors only';"));
...
Procedure EditPublication(PublicationOption)
	...
	If RowsCount = 0 Then
		ShowMessageBox(, NStr("ru = 'Не выбран ни один дополнительный отчет (обработка)';
										|en = 'No additional report or data processor is selected.';"));
		Return;
	EndIf;
	...
	If RowsCount = 1 Then
		MessageText = NStr("ru = 'Изменена публикация дополнительного отчета (обработки) ""%1""';
								|en = 'Availability for the additional report or data processor has been changed: %1.';");
		MessageText = StringFunctionsClientServer.SubstituteParametersToString(MessageText, String(SelectedRows[0]));
	Else
		MessageText = NStr("ru = 'Изменена публикация у дополнительных отчетов (обработок): %1';
								|en = 'Availability for the additional reports or data processors have been changed: %1.';");
		MessageText = StringFunctionsClientServer.SubstituteParametersToString(MessageText, RowsCount);
	EndIf;
	
	ShowUserNotification(NStr("ru = 'Изменена публикация';
										|en = 'Availability changed';"),, MessageText);
