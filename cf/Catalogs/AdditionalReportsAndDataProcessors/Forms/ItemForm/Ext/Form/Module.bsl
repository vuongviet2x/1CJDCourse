...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If ValueIsFilled(Parameters.CopyingValue) Then
		Raise NStr("ru = 'Создание нового элемента копированием запрещено.';
								|en = 'Create by copying is prohibited.';");
	EndIf;
	...
	If Object.Kind = Enums.AdditionalReportsAndDataProcessorsKinds.PrintForm
		...
		Cancel = True;
		Common.MessageToUser(NStr("ru = 'Работа с печатными формами не поддерживается.';
													|en = 'Print forms are not supported.';"));
		Return;
	EndIf;
	...
	If Not InsertRight1 Then
		If IsNew Then
			Raise(NStr("ru = 'Недостаточно прав доступа для добавления дополнительных отчетов или обработок.';
									|en = 'Insufficient rights to add additional reports and data processors.';"),
	...
	If ShowImportFromFileDialogOnOpen And Not Items.LoadFromFile.Visible Then
		Raise(NStr("ru = 'Недостаточно прав для загрузки дополнительных отчетов и обработок.';
								|en = 'Insufficient rights to import additional reports and data processors.';"),
			ErrorCategory.AccessViolation);
	EndIf;
...
Procedure AdditionalReportOptionsBeforeDeleteRow(Item, Cancel)
	...
	If Not Variant.Custom Then
		ShowMessageBox(, NStr("ru = 'Пометка на удаление предопределенного варианта отчета запрещена.';
										|en = 'Predefined report option cannot be marked for deletion.';"));
		Return;
	EndIf;
	
	QueryText = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Пометить ""%1"" на удаление?';
																				|en = 'Do you want to mark %1 for deletion?';"), Variant.Description);
...
Procedure AdditionalReportOptionsOpen(Command)
	Variant = Items.AdditionalReportOptions.CurrentData;
	If Variant = Undefined Then
		ShowMessageBox(, NStr("ru = 'Выберите вариант отчета.';
										|en = 'Choose a report option.';"));
		Return;
	EndIf;
...
Procedure SetPrintCommandVisibility(Command)
	If Modified Then
		...
		QueryText = NStr("ru = 'Для настройки видимости команд печати запишите обработку. Продолжить?';
							|en = 'To configure the visibility of print commands, save the data processor. Continue?';");
...
Procedure ExecuteCommand(Command)
	...
	If Object.Ref.IsEmpty() Or Modified Then
		QueryText = NStr("ru = 'Для выполнения команды запишите данные.';
							|en = 'Save the data before running the command.';");
		Buttons = New ValueList;
		Buttons.Add("WriteAndContinue", NStr("ru = 'Записать и продолжить';
...
Procedure ContinueWriteAtClient(Result, CloseAfterWrite)  Export
	...
	If WriteParameters.DataProcessorRegistration Then
		RefreshReusableValues();
		NotificationText1 = NStr("ru = 'Для применения изменений в открытых окнах закройте их и откройте заново.';
								|en = 'To apply the changes in open windows, close and reopen them.';");
...
Procedure UpdateFromFileAfterConfirm(Response, RegistrationParameters) Export
	...
	ElsIf Object.Kind = KindAdditionalReport Or Object.Kind = KindOfReport Then
		...
		ImportParameters.Dialog.Title = NStr("ru = 'Выберите файл внешнего отчета';
													|en = 'Select an file with external report';");
	Else
		ImportParameters.Dialog.FilterIndex = 2;
		ImportParameters.Dialog.Title = NStr("ru = 'Выберите файл внешней обработки';
													|en = 'Select a file with external data processor';");
	EndIf;
...
Procedure UpdateFromFileAfterFileChoice(FileDetails, RegistrationParameters) Export
	...
	Else
		...
		WarningText = NStr("ru = 'Расширение файла не соответствует расширению внешнего отчета (ERF) или обработки (EPF).';
									|en = 'The file extension does not match external report extension (ERF) or external data processor extension (EPF).';");
...
Procedure UpdateFromFileAndMessage(RegistrationParameters)
	...
	If RegistrationParameters.Success Then
		NotificationTitle1 = ?(RegistrationParameters.IsReport, NStr("ru = 'Файл внешнего отчета загружен';
																	|en = 'External report file is imported';"), NStr("ru = 'Файл внешней обработки загружен';
																													|en = 'External data processor file is imported';"));
...
Procedure ShowConflicts(RegistrationParameters)
	...
	If RegistrationParameters.ConflictsCount > 1 Then
		If RegistrationParameters.IsReport Then
			QuestionTitle = NStr("ru = 'Конфликты при загрузке внешнего отчета';
									|en = 'External report import conflict';");
			QueryText = NStr("ru = 'Внутреннее имя отчета ""[Name]""
		...
		Else
			QuestionTitle = NStr("ru = 'Конфликты при загрузке внешней обработки';
									|en = 'Conflicts occurred during import of external data processor';");
			QueryText = NStr("ru = 'Внутреннее имя обработки ""[Name]""
		...
		EndIf;
		DisableButtonPresentation = NStr("ru = 'Отключить конфликтующие';
											|en = 'Disable conflicting objects';");
		OpenButtonPresentation = NStr("ru = 'Отменить и показать список';
											|en = 'Cancel and show list';");
	Else
		If RegistrationParameters.IsReport Then
			QuestionTitle = NStr("ru = 'Конфликт при загрузке внешнего отчета';
									|en = 'External report import conflict';");
			QueryText = NStr("ru = 'Внутреннее имя отчета ""[Name]""
			...
			DisableButtonPresentation = NStr("ru = 'Отключить другой отчет';
												|en = 'Disable another report';");
		Else
			QuestionTitle = NStr("ru = 'Конфликт при загрузке внешней обработки';
									|en = 'External data processor import conflict';");
			QueryText = NStr("ru = 'Внутреннее имя обработки ""[Name]""
			...
			DisableButtonPresentation = NStr("ru = 'Отключить другую обработку';
												|en = 'Disable another data processor';");
		EndIf;
		OpenButtonPresentation = NStr("ru = 'Отменить и открыть';
											|en = 'Cancel and open';");
	EndIf;
	ContinueButtonPresentation = NStr("ru = 'В режиме отладки';
										|en = 'Debug mode';");
...
Procedure UpdateFromFileConflictDecision(Response, RegistrationParameters) Export
	...
	ElsIf Response = "CancelAndOpen" Then
		...
		If ShowList Then // List form with a filter by conflicting items.
			...
			FormTitle = NStr("ru = 'Дополнительные отчеты и обработки с внутреннем именем ""%1""';
									|en = 'Additional reports and data processors with name ""%1""';");
...
Procedure OpenOption()
	...
	If Not ValueIsFilled(Variant.Ref) Then
		ErrorText = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Вариант отчета ""%1"" не зарегистрирован.';
																					|en = '""%1"" report option is not registered.';"), Variant.Description);
...
Procedure AfterScheduleEditComplete(Schedule, Context) Export
	...
	Else
		...
		If DataSeparationEnabled
			...
			MessageText = NStr("ru = 'Не допускается установка интервала повтора регламентного задания менее 60 секунд.';
									|en = 'Setting the retry interval of the scheduled job less than 60 seconds is not allowed.';");
		...
		Else
			ItemCommand.ScheduledJobPresentation = NStr("ru = 'Не заполнено';
																	|en = 'Not filled';");
			If ItemCommand.ScheduledJobUsage Then
				ItemCommand.ScheduledJobUsage = False;
				ShowUserNotification(
					NStr("ru = 'Запуск по расписанию отключен';
						|en = 'Scheduling is disabled';"),
					,
					NStr("ru = 'Расписание не заполнено';
						|en = 'Schedule is not filled';"));
			EndIf;
		EndIf;
	EndIf;
EndProcedure
...
Function UsersQuickAccessPresentation(UsersCount)
	...
	QuickAccessPresentation = StringFunctionsClientServer.StringWithNumberForAnyLanguage(
		NStr("ru = ';%1 пользователь;;%1 пользователя;%1 пользователей;%1 пользователя';
			|en = ';%1 user;;;;%1 users';"), UsersCount);
...
Function StartExecuteServerCommandInBackground(CommandToExecute, UUID)
	...
	StartSettings1.BackgroundJobDescription =
		NStr("ru = 'Дополнительные отчеты и обработки: Выполнение серверного метода обработки';
			|en = 'Additional reports and data processors: executing data processor server method.';");
	StartSettings1.RefinementErrors =
		NStr("ru = 'Не удалось выполнить команду по причине:';
			|en = 'Cannot execute the command. Reason:';");
...
Function PrepareMetadataObjectsSelectionFormParameters()
	...
	FormParameters.Title = NStr("ru = 'Назначение дополнительной обработки';
									|en = 'Additional data processor assignment';");
...
Procedure SetVisibilityAvailability(Registration = False)
	...
	If Object.Kind = KindAdditionalReport And Object.UseOptionStorage Then
		...
		If Registration Or OptionsCount = 0 Then
			...
			Items.OptionsPage.Title = NStr("ru = 'Варианты отчета';
														|en = 'Report options';");
		Else
	...
	Else
		...
		If UseSecurityProfiles And SafeMode = True Then
			Items.SafeModeWithPermissionsPages.CurrentPage = Items.SafeModeWithPermissionsPage;
		ElsIf TypeOf(SafeMode) = Type("Boolean") Then
		...
		ElsIf TypeOf(SafeMode) = Type("String") Then
			...
			Items.DecorationPersonalSecurityProfileLabel.Title = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Дополнительный отчет или обработка будет подключаться к приложению с использованием ""персонального""
	...
	If PurposePresentation = "" Then
		PurposePresentation = NStr("ru = 'Не определено';
										|en = 'Undefined';");
	EndIf;
	...
	If IsNew Then
		Title = ?(IsReport, NStr("ru = 'Дополнительный отчет (создание)';
									|en = 'Additional report (Create)';"), NStr("ru = 'Дополнительная обработка (создание)';
																					|en = 'Additional data processor (Create)';"));
	Else
		Title = Object.Description + " " + ?(IsReport, NStr("ru = '(Дополнительный отчет)';
																|en = '(Additional report)';"), NStr("ru = '(Дополнительная обработка)';
																										|en = '(Additional data processor)';"));
	EndIf;
	...
	For Each ItemCommand In Object.Commands Do
			...
			If Not IsBlankString(ItemCommand.CommandsToReplace) And ObjectPrintCommands <> Undefined Then
				...
				If CommandsToReplaceCount > 0 Then
					If CommandsToReplaceCount = 1 Then
						CommentTemplate = NStr("ru = 'Заменяет стандартную команду печати %1';
												|en = 'Replace standard print command %1';");
					Else
						CommentTemplate = NStr("ru = 'Заменяет стандартные команды печати: %1';
												|en = 'Replace standard print commands: %1';");
					EndIf;
		...
		Else
			ItemCommand.ScheduledJobPresentation = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Неприменимо для команд с вариантом запуска ""%1""';
					|en = 'Not applicable for commands with the ""%1"" startup option';"),
...
Procedure AdditionalReportOptionsFill()
	...
	Except
		ErrorText = NStr("ru = 'Не удалось получить список вариантов отчета из-за ошибки, возникшей при подключении этого отчета:';
							|en = 'Cannot get the list of report options due to report attachment error:';");
...
Function CommandsPageName()
	If Object.Kind = Enums.AdditionalReportsAndDataProcessorsKinds.PrintForm Then
		Return NStr("ru = 'Команды печати';
					|en = 'Form''s print commands.';");
	Else
		Return NStr("ru = 'Команды';
					|en = 'Commands';");
	EndIf;
EndFunction
