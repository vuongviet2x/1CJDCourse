	...
	If Not AdditionalReportsAndDataProcessors.CheckGlobalDataProcessor(Kind) Then
		If Not UseForObjectForm And Not UseForListForm 
			...
			Common.MessageToUser(
				NStr("ru = 'Отключите публикацию или выберите для использования как минимум одну из форм.';
					|en = 'Disable publication or select at least one of the forms to use.';"),,, 
		...
		If Conflicting.Count() > 0 Then
			...
			If ItemCheck Then
				ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
					...
					|en = 'The %1 report or data processor name is not unique.
					|
			...
			Else
				ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Имя ""%1"", используемое отчетом (обработкой) ""%2"", уже занято другим опубликованным дополнительным отчетом (обработкой).';
						|en = 'The name %1 of the %2 report or data processor is not unique. The same name is assigned to another report or data processor.';"),
					ObjectName,
					String(Ref));
			EndIf;
...
Procedure BeforeWrite(Cancel)
	...
	If IsNew() And Not AdditionalReportsAndDataProcessors.InsertRight1(ThisObject) Then
		Raise(NStr("ru = 'Недостаточно прав для добавления дополнительных отчетов или обработок.';
								|en = 'Insufficient rights to add additional reports or data processors.';"),
	...
	If Not IsNew() And Kind <> Common.ObjectAttributeValue(Ref, "Kind") Then
		Common.MessageToUser(
			NStr("ru = 'Невозможно сменить вид существующего дополнительного отчета или обработки.';
				|en = 'Cannot change the type of additional report or data processor.';"),,,,
			Cancel);
		Return;
	EndIf;
...
Procedure OnWriteReport(Cancel)
	...
	If Common.SubsystemExists("StandardSubsystems.ReportsOptions") Then
		...
		Try
			...
			EndIf;
		Except
			ErrorText = NStr("ru = 'Ошибка подключения:';
								|en = 'Attachment error:';") + Chars.LF + ErrorProcessing.DetailErrorDescription(ErrorInfo());
