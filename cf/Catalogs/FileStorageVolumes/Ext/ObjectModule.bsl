...
Procedure FillCheckProcessing(Cancel, CheckedAttributes)
	...
	If Not AdditionalProperties.Property("SkipBasicFillingCheck") Then
		...
		If Not SequenceNumberUnique(FillOrder, Ref) Then
			ErrorText = NStr("ru = 'Порядок заполнения не уникален - в системе уже есть том с таким порядком';
		...
		If MaximumSize <> 0 Then
			...
			If MaximumSize < ActualSize Then
				ErrorText = NStr("ru = 'Максимальный размер тома меньше, чем текущий размер';
		...
		If IsBlankString(FullPathWindows) And IsBlankString(FullPathLinux) Then
			ErrorText = NStr("ru = 'Не заполнен полный путь';
		...
		If Not UseSecurityProfiles
		   ...
		   And (    Left(FullPathWindows, 2) <> "\\"
			...
			ErrorText = NStr("ru = 'Путь к тому должен быть в формате UNC (\\servername\resource).';
								|en = 'The volume path must be in the UNC format (\\servername\resource).';");
			Common.MessageToUser(ErrorText, , "FullPathWindows", "Object", Cancel);
			Return;
		EndIf;
	EndIf;
	...
	If Not AdditionalProperties.Property("SkipDirectoryAccessCheck") Then
		...
		Except
			...
			If UseSecurityProfiles Then
				ErrorTemplate =
					NStr("ru = 'Некорректный путь к тому или сервер с томом в данный момент недоступен.
...
Procedure CheckTheUniquenessOfThePathToTheVolumes(Cancel)
	...
	ErrorTemplate = NStr("ru = 'Путь к тому должен быть уникален. Каталог указан в томе %1.';
