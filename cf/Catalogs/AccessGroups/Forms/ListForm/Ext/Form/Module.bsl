...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Parameters.ChoiceMode Then
		...
		If Parameters.CloseOnChoice = False Then
			...
			Title = NStr("ru = 'Подбор групп доступа';
							|en = 'Pick access groups';");
		Else
			Items.List.MultipleChoice = False;
			Title = NStr("ru = 'Выбор группы доступа';
							|en = 'Select access group';");
		EndIf;
	EndIf;
...
Procedure ListValueChoice(Item, Value, StandardProcessing)
	...
	If Value = PersonalAccessGroupsParent Then
		...
		ShowMessageBox(, NStr("ru = 'Эта группа только для персональных групп доступа.';
										|en = 'The group can contain only personal access groups.';"));
	EndIf;
...
Procedure ListBeforeAddRow(Item, Cancel, Copy, Parent, Var_Group)
	...
	If Parent = PersonalAccessGroupsParent Then
		...
		If Var_Group Then
			ShowMessageBox(, NStr("ru = 'В этой группе не используются подгруппы.';
											|en = 'The group cannot contain subgroups.';"));
		...
		Else
			ShowMessageBox(, NStr("ru = 'Персональные группы доступа не используются.';
											|en = 'Personal access groups are disabled.';"));
		EndIf;
		...
		ShowMessageBox(,
			NStr("ru = 'Используются только персональные группы доступа,
			           |которые создаются только в форме ""Права доступа"".';
						|en = 'Personal access groups can be created
...
Procedure ListDrag(Item, DragParameters, StandardProcessing, String, Field)
	...
	If String = PersonalAccessGroupsParent Then
		StandardProcessing = False;
		ShowMessageBox(, NStr("ru = 'Эта папка только для персональных групп доступа.';
										|en = 'This folder is for personal access groups only.';"));
		
	ElsIf DragParameters.Value = PersonalAccessGroupsParent Then
		StandardProcessing = False;
		ShowMessageBox(, NStr("ru = 'Папка персональных групп доступа не переносится.';
										|en = 'Cannot move a personal access groups folder.';"));
	EndIf;
