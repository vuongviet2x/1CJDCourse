		...
		For Each String In Rows Do
			...
			If TypeOf(CertificateData) <> Type("BinaryData") Then
				Raise StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Данные сертификата ""%1"" не существуют в справочнике';
						|en = 'The ""%1"" certificate data does not exist in the catalog.';"), Selection.Presentation);
			EndIf;
			Try
			...
			Except
				ErrorInfo = ErrorInfo();
				Raise StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Данные сертификата ""%1"" в справочнике некорректны по причине:
					           |%2';
								|en = 'The ""%1"" certificate data in the catalog is incorrect due to:
								|%2';"),
	...
	If ValueIsFilled(Certificate) Then
		If Not ValueIsFilled(CertificateApp)
			...
			Context.ErrorAtClient.Insert("ErrorDescription",
				NStr("ru = 'У выбранного личного сертификата не указано приложение для закрытого ключа и его не удалось определить автоматически.
				           |Выберите другой сертификат.';
							|en = 'For the selected personal certificate, no private key management app is specified or it cannot be found automatically.
...
Function CertificatesProperties(Val References, Val FormIdentifier)
	...
	While Selection.Next() Do
		...
		CertificateData = Selection.CertificateData.Get();
		If TypeOf(CertificateData) <> Type("BinaryData") Then
			Raise StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Данные сертификата ""%1"" не существует в справочнике';
																							|en = 'The ""%1"" certificate data does not exist in the catalog.';"),
...
Procedure WriteEncryptionCertificatesAtServer(ObjectsDetails, CertificatesAddress, FormIdentifier, Error)
	...
	Except
		RollbackTransaction();
		ErrorInfo = ErrorInfo();
		Error.Insert("ErrorDescription", NStr("ru = 'Не удалось записать сертификаты шифрования по причине:';
												|en = 'Cannot save the encryption certificates due to:';")
...
Procedure HandleError(Notification, ErrorAtClient, ErrorAtServer)
	...
	If DataDetails.Property("StopExecution") Then
		...
		DataDetails.ErrorDescription = DigitalSignatureInternalClientServer.GeneralDescriptionOfTheError(
			ErrorAtClient, ErrorAtServer, NStr("ru = 'Не удалось зашифровать данные по причине:';
	...
	Else
		...
		DigitalSignatureInternalClient.ShowApplicationCallError(
			NStr("ru = 'Не удалось зашифровать данные';
