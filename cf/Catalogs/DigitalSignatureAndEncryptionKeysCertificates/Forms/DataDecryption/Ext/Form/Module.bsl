...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If IsAuthentication Then
		Items.FormDecrypt.Title = NStr("ru = 'ОК';
													|en = 'OK';");
		Items.AdvancedPasswordNote.Title = NStr("ru = 'Нажмите ОК, чтобы перейти к вводу пароля.';
															|en = 'Click OK to enter the password.';");
	EndIf;
...
Function WriteEncryptionCertificatesAtServer(Val ObjectsDetails, Val FormIdentifier, Val Error)
	...
	Except
		RollbackTransaction();
		ErrorInfo = ErrorInfo();
		Error.Insert("ErrorDescription", NStr("ru = 'Не удалось очистить сертификаты шифрования по причине:';
												|en = 'Cannot clear the encryption certificates due to:';")
...
Procedure HandleError(Notification, ErrorAtClient, ErrorAtServer, AdditionalData = Undefined)
	...
	If DataDetails.Property("StopExecution") Then
		...
		DataDetails.ErrorDescription = DigitalSignatureInternalClientServer.GeneralDescriptionOfTheError(
			ErrorAtClient, ErrorAtServer, NStr("ru = 'Не удалось расшифровать данные по причине:';
	...
	Else
		...
		DigitalSignatureInternalClient.ShowApplicationCallError(
			NStr("ru = 'Не удалось расшифровать данные';
