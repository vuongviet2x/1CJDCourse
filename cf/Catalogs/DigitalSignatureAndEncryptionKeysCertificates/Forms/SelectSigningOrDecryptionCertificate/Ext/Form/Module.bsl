...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Parameters.CanAddToList Then
		CanAddToList = True;
		Items.Select.Title = NStr("ru = 'Добавить';
											|en = 'Add';");
		
		Items.AdvancedPasswordNote.Title =
			NStr("ru = 'Нажмите Добавить, чтобы перейти к вводу пароля.';
				|en = 'Click Add to enter the password.';");
		
		PersonalListOnAdd = Parameters.PersonalListOnAdd;
		Items.ShowAll.ToolTip =
			NStr("ru = 'Показать все сертификаты без отбора (например, включая добавленные и просроченные)';
				|en = 'Show all certificates without filter (for example, including added and overdue)';");
	EndIf;
	...
	If ToEncryptAndDecrypt = True Then
		If Parameters.CanAddToList Then
			Title = NStr("ru = 'Добавление сертификата для шифрования и расшифровки данных';
							|en = 'Add a certificate to encrypt and decrypt data';");
		Else
			Title = NStr("ru = 'Выбор сертификата для шифрования и расшифровки данных';
							|en = 'Select certificate to encrypt and decrypt data';");
		EndIf;
	ElsIf ToEncryptAndDecrypt = False Then
		If Parameters.CanAddToList Then
			Title = NStr("ru = 'Добавление сертификата для подписания данных';
							|en = 'Add a certificate to sign data';");
		EndIf;
	ElsIf DigitalSignature.UseEncryption() Then
		Title = NStr("ru = 'Добавление сертификата для подписания и шифрования данных';
						|en = 'Add a certificate to sign and encrypt data';");
	Else
		Title = NStr("ru = 'Добавление сертификата для подписания данных';
						|en = 'Add a certificate to sign data';");
	EndIf;
	 ...
	 Or HasCloudSignature Then
		...
		If ExecuteAtServer = True Then
			Items.CertificatesGroup.Title =
				NStr("ru = 'Личные сертификаты на сервере';
					|en = 'Personal certificates on the server';");
		Else
			Items.CertificatesGroup.Title =
				NStr("ru = 'Личные сертификаты на компьютере и сервере';
...
Procedure FillCheckProcessingAtServer(Cancel, CheckedAttributes)
	...
	If Items.CertificateCompany.Visible
	   ...
	   And Not ValueIsFilled(CertificateCompany) Then
		...
		MessageText = NStr("ru = 'Поле Организация не заполнено.';
								|en = 'Company is not populated.';");
...
Procedure CertificatesUnavailableAtClientLabelClick(Item)
	...
	DigitalSignatureInternalClient.ShowApplicationCallError(
		NStr("ru = 'Сертификаты недоступны на компьютере';
			|en = 'Certificates not available on computer';"),
		"",
...
Procedure CertificatesUnavailableAtServerLabelClick(Item)
	...
	DigitalSignatureInternalClient.ShowApplicationCallError(
		NStr("ru = 'Сертификаты недоступны на сервере';
			|en = 'Certificates not available on server';"),
		"",
		New Structure,
...
Procedure SelectAfterVerifyingTheCloudSignatureCertificate(Result, Context) Export
	...
	If ValueIsFilled(ErrorDescription) Then
		If ToEncryptAndDecrypt = True Then
			FormCaption = NStr("ru = 'Проверка шифрования и расшифровки';
									|en = 'Encryption and decryption check';");
		Else
			FormCaption = NStr("ru = 'Проверка установки электронной подписи';
...
Procedure PickIndividual(Command)
	...
	FilterParameters.Insert("Property", NStr("ru = 'Кому выдан:';
												|en = 'Owner:';"));
	...
	If CertificateDataDetailsStrings.Count() = 0 Then
		CommonClient.MessageToUser(NStr("ru = 'Подходящее физическое лицо не существует.';
														|en = 'An appropriate individual does not exist.';"));
		Return;
	EndIf;
...
Procedure GoToCurrentCertificateChoice(Notification)
	...
	If Items.Certificates.CurrentData = Undefined Then
		Result.ErrorDescription = NStr("ru = 'Выделите сертификат, который будет использоваться.';
										|en = 'Select a certificate to be used.';");
		...
		Result.ErrorDescription =
			NStr("ru = 'Для этого сертификата заявление на выпуск еще не исполнено.
			           |Откройте заявление на выпуск сертификата и выполните требуемые шаги.';
						|en = 'The application for this certificate has not yet been fulfilled.
	...
	If Not HaveRightToAddInDirectory And Not CurrentData.Isinthedirectory Then
		Result.UpdateCertificatesList = True;
		Result.ErrorDescription =
			NStr("ru = 'Недостаточно прав на использование сертификата, отсутствующего в справочнике.';
	...
	If CertificateAtServer Then
		...
		Else
			Result.ErrorDescription = NStr("ru = 'Сертификат отсутствует на сервере (возможно удален).';
											|en = 'Certificate does not exist on the server (it might have been deleted).';");
			Result.UpdateCertificatesList = True;
			ExecuteNotifyProcessing(Notification, Result);
		EndIf;
		Return;
...
Procedure GoToCurrentCertificateChoiceAfterCertificateSearch(SearchResult, Context) Export
	...
	If TypeOf(SearchResult) <> Type("CryptoCertificate") Then
		If SearchResult.Property("CertificateNotFound") Then
			Context.Result.ErrorDescription = NStr("ru = 'Сертификат не установлен на компьютере (возможно удален).';
													|en = 'The certificate is not installed or was deleted from your computer.';");
		Else
...
Procedure GoToCurrentCertificateChoiceAfterCertificateSearchInCloudService(SearchResult, Context) Export
	...
	If Not ValueIsFilled(SearchResult.Certificate) Then
		Context.Result.ErrorDescription = NStr("ru = 'Сертификат отсутствует в сервисе (возможно удален).';
...
Procedure GoToTheCurrentCertificateSelectionAfterSearchingForTheCertificateInTheCloudSignature(SearchResult, Context) Export
	...
	If Not ValueIsFilled(SearchResult.CertificateData) Then
		Context.Result.ErrorDescription = NStr("ru = 'Сертификат отсутствует на сервере DSS (возможно удален).';
...
Procedure GoToCurrentCertificateChoiceAfterFillCertificateProperties(Context)
	...
	If CanAddToList Then
		String = ?(ValueIsFilled(Certificate), NStr("ru = 'Обновить';
														|en = 'Update';"), NStr("ru = 'Добавить';
																				|en = 'Add';"));
