...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If ValueIsFilled(AdditionalDataChecks) And TypeOf(AdditionalDataChecks) = Type("Structure") Then
		...
		Items.DecorationCertificate.Title = ?(ValueIsFilled(AdditionalDataChecks.ErrorText), 
			AdditionalDataChecks.ErrorText,
			AdditionalDataChecks.AdditionalInfo);
	...
	Else
		...
		If ValueIsFilled(Certificate) Then
			...
			Items.DecorationCertificate.Title = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = '%1 закончится срок действия сертификата';
					|en = 'The certificate expires on %1';"), ValidBefore);
			If Not YesReissued Then
				If CertificateIssueRequestAvailable Then
					Decision = StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Подайте <a href = ""%1"">заявление</a> на новый сертификат.';
						|en = 'Submit <a href = ""%1"">application</a> for a new certificate.';"),
						"ApplyforCertificate");
				Else
					Decision = Decision();
				EndIf;
			...
			EndIf;
		Else
			Items.DecorationCertificate.Title = NStr("ru = 'Не заполнен параметр Сертификат при открытии формы.';
															|en = 'The Certificate parameter is not filled in when opening the form.';");
		EndIf;
...
Function Decision()
	...
	If DigitalSignature.CommonSettings().AvailableCheckAccordingtoCAList Then
		...
		Return StringFunctions.FormattedString(
				NStr("ru = 'Получите новый сертификат в <a href = ""%1"">соответствующем удостоверяющем центре</a>.';
					|en = 'Request a new certificate from the <a href = ""%1"">appropriate CA</a>.';"),
					ModuleDigitalSignatureClientServerLocalization.LinktothearticleonCAs());
	Else
		Return NStr("ru = 'Получите новый сертификат.';
					|en = 'Request a new certificate.';");
	EndIf;
