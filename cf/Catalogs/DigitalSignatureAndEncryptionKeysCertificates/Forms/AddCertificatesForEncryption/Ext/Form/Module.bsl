...
Async Procedure AddCertificatesToCatalog(Command)
	...
	If Success Then
		Response = Await DoQueryBoxAsync(NStr("ru = 'Сертификаты добавлены успешно. Закрыть форму?';
										|en = 'Certificates are added. Do you want to close the form?';"), QuestionDialogMode.YesNo);
	...
	For Each CurrentRow In Certificates Do
		...
		Except
			...
			ErrorMessage = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Не удалось записать сертификат %1: %2';
					|en = 'Cannot save the %1 certificate: %2';"), CurrentRow.Presentation,
