...
Procedure PresentationGetProcessing(Data, Presentation, StandardProcessing)
	...
	If ValueIsFilled(Data.Email) And StrFind(Data.Description, Data.Email) = 0 Then
		Presentation = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = '%1 (%2)';
																					|en = '%1 (%2)';"), 
...
Procedure ProcessDataForMigrationToNewVersion(Parameters) Export
	...
	Except
		RollbackTransaction();
		WriteLogEvent(NStr("ru = 'Обновление учетных записей';
										|en = 'Email account update';", Common.DefaultLanguageCode()),
...
Procedure OnInitialItemsFilling(LanguagesCodes, Items, TabularSections) Export
	...
	Item.Description                     = NStr("ru = 'Системная учетная запись';
													|en = 'System account';", 
		Common.DefaultLanguageCode());
	Item.UserName                  = NStr("ru = '1С:Предприятие';
													|en = '1C:Enterprise';", Common.DefaultLanguageCode());
...
Function TestConnectionToIncomingMailServer(Profile, Protocol)
	...
	If Protocol = InternetMailProtocol.POP3 Then
		TextForLog = StringFunctionsClientServer.SubstituteParametersToString("%1:%2%3 (%4)" + Chars.LF + "%5",
			...
			Profile.User,
			?(IsBlankString(ErrorText), NStr("ru = 'OK';
												|en = 'OK';"), ErrorText));
	Else
		TextForLog = StringFunctionsClientServer.SubstituteParametersToString("%1:%2%3 (%4)" + Chars.LF + "%5",
...
Function TestConnectionToOutgoingMailServer(Profile, Email)
	...
	Subject = NStr("ru = 'Тестовое сообщение 1С:Предприятие';
				|en = 'Test message from 1C:Enterprise';");
	Body = NStr("ru = 'Это сообщение отправлено подсистемой электронной почты 1С:Предприятие';
				|en = 'This message is sent by 1C:Enterprise.';");
	EmailSenderName = NStr("ru = '1С:Предприятие';
								|en = '1C:Enterprise';");
...
Function CheckProfilesSettings(OutgoingMailProfile, IncomingMailProfile, Val Email) Export
	...
	If OutgoingMailProfile <> Undefined Then
		...
		If ValueIsFilled(ErrorText) Then
			...
			If AuthenticationError(ErrorText) Then
				MessageText.Add(NStr("ru = 'Отправка тестового сообщения не выполнена: не удалось авторизоваться.';
											|en = 'Cannot send test mail: authorization failed.';"));
			Else
				MessageText.Add(NStr("ru = 'Отправка тестового сообщения не выполнена.';
											|en = 'Sending the test mail failed.';"));
			...
			EndIf;
		Else
			ExecutedChecks.Add("- " + NStr("ru = 'Выполнена отправка тестового сообщения.';
													|en = 'Sending the test mail succeeded.';"));
		EndIf;
	...
	If IncomingMailProfile <> Undefined Then
		...
		If ValueIsFilled(ErrorText) Then
			...
			AuthenticationError = AuthenticationError(ErrorText);
			If AuthenticationError(ErrorText) Then
				MessageText.Add(NStr("ru = 'Подключение к серверу входящей почты не выполнено: не удалось авторизоваться.';
											|en = 'Cannot connect to the incoming mail server: authorization failed.';"));
			Else
				MessageText.Add(NStr("ru = 'Подключение к серверу входящей почты не выполнено.';
											|en = 'Connection to the incoming mail server failed.';"));
			...
			EndIf;
		Else
			ExecutedChecks.Add("- " + NStr("ru = 'Выполнено подключение к серверу входящей почты.';
													|en = 'Connection to the incoming mail server succeeded.';"));
		EndIf;
		
	EndIf;
	
	TechnicalDetails.Add(StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = 'Адрес почты: %1';
			|en = 'Email address: %1';"), Email));
	...
	If ValueIsFilled(MessageText) Then
		...
		If AuthenticationError Then
			Recommendations.Add(NStr("ru = 'Проверьте правильность ввода логина и пароля, а также выбранный способ авторизации.';
										|en = 'Ensure that the username, password, and authentication method are correct.';"));
		EndIf;
		If OutgoingMailSettingsCheckRequired Then
			Recommendations.Add(NStr("ru = 'Проверьте настройки подключения к серверу исходящей почты.';
										|en = 'Ensure that the outgoing mail server settings are correct.';"));
		EndIf;
		If IncomingMailSettingsCheckRequired Then
			Recommendations.Add(NStr("ru = 'Проверьте настройки подключения к серверу входящей почты.';
										|en = 'Ensure that the incoming mail server settings are correct.';"));
		EndIf;
		If OutgoingMailSettingsCheckRequired Or IncomingMailSettingsCheckRequired Then
			Recommendations.Add(NStr("ru = 'Обратитесь к администратору локальной сети.';
										|en = 'Contact the network administrator.';"));
		EndIf;
		
		Recommendations = StrConcat(Recommendations, Chars.LF);
		
		ConnectionErrors = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = '%1
			|
			|%2
			|
	...
	If ValueIsFilled(ConnectionErrors) Then
		WriteLogEvent(NStr("ru = 'Проверка настроек почты';
										|en = 'Validating email account settings';", Common.DefaultLanguageCode()),
	...
	If ValueIsFilled(Profile.SMTPServerAddress) Then
		...
		For Each PropertyName In StrSplit(SMTPPropertyList, ",", False) Do
			Settings.Add(StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = '%1=""%2""';
					|en = '%1=""%2""';"), PropertyName, Profile[PropertyName]));
		EndDo;
...
Function ApplicationInfo()
	...
	Result = New Array;
	Result.Add(StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Операционная система: %1';
																					|en = 'Operating system: %1';"), SystemInfo.OSVersion));
	Result.Add(StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Платформа: %1 (%2)';
																					|en = '1C:Enterprise platform: %1 (%2)';"),
		SystemInfo.AppVersion, SystemInfo.PlatformType));
	Result.Add(StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Конфигурация: %1 (%2)';
																					|en = '1C:Enterprise application: %1 (%2)';"), 
		?(ValueIsFilled(Metadata.Synonym), Metadata.Synonym, Metadata.Name), Metadata.Version));
		
	Result.Add(Chars.LF + NStr("ru = 'Расширения:';
										|en = 'Extensions:';"));
	
	SetPrivilegedMode(True);
	For Each Extension In ConfigurationExtensions.Get() Do
		If Extension.Active Then
			Result.Add(StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = '%1 (%2)';
																							|en = '%1 (%2)';"),
...
Function MailServerConnectionTestEvent()
	Return NStr("ru = 'Проверка подключения к почтовому серверу';
				|en = 'Mail server connection test';", Common.DefaultLanguageCode());
EndFunction
