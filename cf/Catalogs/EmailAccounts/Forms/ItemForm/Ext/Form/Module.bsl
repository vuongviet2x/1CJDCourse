...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Not CanReceiveEmails Then
		Items.ForSending.Title = NStr("ru = 'Использовать для отправки писем';
												|en = 'Use this account to send mail';");
	EndIf;
	...
	If Common.IsMobileClient() Then
		...
		Items.AuthenticationPassword.ChoiceList.Add("OAuth", NStr("ru = 'Авторизация на почтовом сервисе';
																				|en = 'Authorize in the email service';"));
		Items.AuthenticationPassword.ChoiceList.Add("Password", NStr("ru = 'Использовать пароль';
																				|en = 'Use password';"));
...
Procedure FillCheckProcessingAtServer(Cancel, CheckedAttributes)
	If UserAccountKind = "Personal1" And Not ValueIsFilled(Object.AccountOwner) Then 
		Cancel = True;
		MessageText = NStr("ru = 'Не выбран владелец почты.';
								|en = 'Select the account owner.';");
...
Procedure ShowCorrectionMethod(WayFix, AdditionalParameters = Undefined) Export
	...
	If WayFix = "EnableUseAuthorizationSMTP" Then
		CommonClient.MessageToUser(NStr("ru = 'Включите авторизацию на сервере исходящей почты.';
														|en = 'Enable authorization on the outgoing mail server.';"),
			Object.Ref, , "Object.AuthorizationRequiredOnSendEmails");
	ElsIf WayFix = "Readjust" Then
		CommonClient.MessageToUser(NStr("ru = 'Для перенастройки почты нажмите на кнопку ""Перенастроить"".';
														|en = 'To reconfigure your account, click ""Reconfigure"".';"),
			Object.Ref);
	ElsIf WayFix = "UseSTARTTLSForIncomingMail" Then
		CommonClient.MessageToUser(NStr("ru = 'Переключите шифрование на STARTTLS (для входящей почты).';
														|en = 'Switch encryption to STARTTLS (for incoming emails).';"),
			Object.Ref, "EncryptOnReceiveMail");
	ElsIf WayFix = "RefillLoginPassword" Then
		CommonClient.MessageToUser(NStr("ru = 'Попробуйте очистить и ввести заново логин.';
														|en = 'Try clearing and entering a username again.';"),
			Object.Ref, , "Object.User");
	ElsIf WayFix = "RefillPassword" Then
		CommonClient.MessageToUser(NStr("ru = 'Введите пароль.';
														|en = 'Enter your password';"),
			Object.Ref, "Password");
	ElsIf WayFix = "FillinMailAddress" Then
		CommonClient.MessageToUser(NStr("ru = 'Проверьте адрес почты.';
...
Procedure ProtocolOnChange(Item)
	...
	Items.IncomingMailServer.Title = StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = 'Сервер %1';
			|en = '%1 server';"), Object.ProtocolForIncomingMail);
...
Procedure SetGroupTypeAuthorizationRequired(Form, POPIsUsed)
	...
	If POPIsUsed Then
		Form.Items.AuthorizationRequiredOnSendMail.Title = NStr("ru = 'При отправке писем требуется авторизация';
																			|en = 'Outgoing server (POP) requires authentication';");
	Else
		Form.Items.AuthorizationRequiredOnSendMail.Title = NStr("ru = 'При отправке писем требуется авторизация на сервере исходящей почты (SMTP)';
																			|en = 'Outgoing server (SMTP) requires authentication';");
	EndIf;
...
#Else
			Items.Password.Enabled = Not Object.EmailServiceAuthorization;
#EndIf
			WarningText = NStr("ru = 'Не удалось авторизоваться на почтовом сервисе.';
										|en = 'Authorization in the email service failed.';");
