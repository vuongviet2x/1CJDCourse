...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	Items.AccountSettingsTitle.Title = ?(ContextMode,
		NStr("ru = 'Для отправки писем настройте почту';
			|en = 'To send messages, set up the email account.';"),
		NStr("ru = 'Введите настройки почты';
			|en = 'Enter email settings';"));
		
	Items.AccountSettingsTitle.Visible = ContextMode;
	Title = NStr("ru = 'Настройка почты';
					|en = 'Account setup';");
...
Procedure ShowQueryBoxBeforeCloseForm()
	QueryText = NStr("ru = 'Введенные данные не записаны. Закрыть форму?';
	...
	Buttons.Add("Close", NStr("ru = 'Закрыть';
									|en = 'Close';"));
	Buttons.Add(DialogReturnCode.Cancel, NStr("ru = 'Не закрывать';
													|en = 'Do not close';"));
	ShowQueryBox(NotifyDescription, QueryText, Buttons, , DialogReturnCode.Cancel, NStr("ru = 'Настройка почты';
																								|en = 'Account setup';"));
EndProcedure
	...
	If CurrentPage = Items.UserAccountSetup Or CurrentPage = Items.TechnicalDetailsOfError Then
		...
		If Not Cancel And AuthenticationOption = "OAuth" Then
			...
			If Not ValueIsFilled(RedirectAddress) 
				...
				ErrorsMessages = NStr("ru = 'Не найдены настройки авторизации почтового сервиса. Используйте авторизацию по паролю.';
											|en = 'Email service authorization settings are not found. Use password authorization.';");
...
Procedure CheckFillingOnAccountSettingsPage(Cancel)
	...
	If IsBlankString(Email) Then
		CommonClient.MessageToUser(NStr("ru = 'Введите адрес электронной почты';
														|en = 'Email address required';"), , "Email", , Cancel);
	ElsIf Not CommonClientServer.EmailAddressMeetsRequirements(Email, True) Then
		CommonClient.MessageToUser(NStr("ru = 'Адрес электронной почты введен неверно';
														|en = 'Invalid email address';"), , "Email", , Cancel);
	EndIf;
...
Procedure SetCurrentPageItems()
	...
	If CurrentPage = Items.AccountConfigured Then
		If ContextMode Then
			ButtonNextTitle = NStr("ru = 'Продолжить';
										|en = 'Continue';");
		Else
			ButtonNextTitle = NStr("ru = 'Закрыть';
										|en = 'Close';");
		EndIf;
	Else
		If CurrentPage = Items.UserAccountSetup
			And ValidationCompletedWithErrors Or CurrentPage = Items.TechnicalDetailsOfError Then
				ButtonNextTitle = NStr("ru = 'Повторить';
											|en = 'Retry';");
		ElsIf CurrentPage = Items.UserAccountSetup
			And SetupMethod = "Automatically" Then
			If ContextMode Or Reconfigure Then
				ButtonNextTitle = NStr("ru = 'Настроить';
											|en = 'Setup';");
			Else
				ButtonNextTitle = NStr("ru = 'Создать';
											|en = 'Create';");
			EndIf;
		ElsIf CurrentPage = Items.ValidatingAccountSettings Then
			ButtonNextTitle = NStr("ru = 'Пропустить проверку';
										|en = 'Skip test';");
		Else
			ButtonNextTitle = NStr("ru = 'Далее >';
										|en = 'Next >';");
		EndIf;
	EndIf;
	...
	If Not ContextMode And CurrentPage = Items.AccountConfigured Then
		Items.GoToSettingsButton.Title = NStr("ru = 'Перейти к настройкам';
															|en = 'Settings';");
	Else
		Items.GoToSettingsButton.Title = NStr("ru = 'Настроить вручную';
															|en = 'Manual setup';");
	EndIf;
...
Procedure NewAccount1()
	...
	Except
		RollbackTransaction();
		WriteLogEvent(NStr("ru = 'Работа с почтовыми сообщениями';
										|en = 'Email management';", Common.DefaultLanguageCode()),
...
Procedure SetUpConnectionParametersAutomatically()
	...
	ErrorsMessages = NStr("ru = 'Не удалось определить настройки подключения. 
	|Настройте параметры подключения вручную.';
	|en = 'Couldn''t configure email server settings.
...
Function StartSearchAccountSettings()
	...
	ExecutionParameters.BackgroundJobDescription = NStr("ru = 'Поиск настроек почтового сервера';
															|en = 'Look up mail server settings';");
...
Function ParametersAuthorizationRequest()
	...
	If IsWebClient() Then
		If Not ValueIsFilled(AuthorizationSettings.DeviceRegistrationAddress) Then
			QueryOptions.Insert("device_name", NStr("ru = '1С:Предприятие';
															|en = '1C:Enterprise';"));
		EndIf;
...
Procedure OnReceiveMailServerResponse(ParametersString1, KeyReceiptAddress)
	...
	ElsIf QueryID <> Response["state"] Then
		ErrorsMessages = NStr("ru = 'Не удалось авторизоваться на почтовом сервере. Неверный идентификатор ответа.';
									|en = 'Cannot authorize on the mail server. Incorrect response ID.';");
		ValidationCompletedWithErrors = True;
...
Procedure AdjustCurrentPageElementsOnOpening()
	...
	If OnlyAuthorization Then
		If Not ValueIsFilled(AuthorizationSettings) Then
			Close(NStr("ru = 'Не найдены настройки авторизации для указанного адреса электронной почты.
			|Используйте авторизацию по логину и паролю.';
			|en = 'Cannot find authorization settings for the specified email address.
