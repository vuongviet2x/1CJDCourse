...
Function CanReplaceItems(Val ReplacementPairs, Val ReplacementParameters = Undefined) Export
	...
	For Each KeyValue In ReplacementPairs Do
		...
		If Not ReplacementAllowed Then
			Error = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Элемент ""%1"" относится к ""%2"", а ""%3"" - к ""%4""';
...
Procedure DuplicatesSearchParameters(SearchParameters, AdditionalParameters = Undefined) Export
	...
	Restriction.Insert("Presentation",      NStr("ru = 'Относятся к одной группе и одного типа (адрес, телефон и пр.).';
...
Procedure OnInitialItemsFilling(LanguagesCodes, Items, TabularSections) Export
	...
	Else
		Item.Description = NStr("ru = 'Контактная информация справочника ""Пользователи""';
	...
	Else
		Item.Description = NStr("ru = 'Электронная почта';
									|en = 'Email';", Common.DefaultLanguageCode());
	EndIf;
	...
	Else
		Item.Description = NStr("ru = 'Телефон';
									|en = 'Phone';", Common.DefaultLanguageCode());
	EndIf;
...
Procedure CheckIDUniqueness(IDForFormulas, Ref, Parent, Cancel) Export
	...
	If ValueIsFilled(IDForFormulas) Then
		...
		If Not Upper(VerificationID) = Upper(IDForFormulas) Then
			...
			ErrorText = NStr("ru = 'Идентификатор ""%1"" не соответствует правилам именования переменных.
			...
			EventName = NStr("ru = 'Запись дополнительного реквизита (сведения)';
								|en = 'Save additional attribute or information record';", LanguageCode);
			ErrorText = NStr("ru = 'Идентификатор ""%1"" не соответствует правилам именования переменных.
									|Идентификатор не должен содержать пробелов и специальных символов.';
									|en = 'ID ""%1"" does not comply with variable naming rules.
		...
		If IDByRules Then
			...
			If Not IDForFormulasUnique(IDForFormulas, Ref, TopLevelParent) Then
				...
				ErrorText = NStr("ru = 'В базе данных уже содержится вид контактной информации с идентификатором ""%1"" внутри группы ""%2"". Идентификатор должен быть уникальным';
									|en = 'The database already contains a contact information kind with ID ""%1"" within group ""%2"". The ID must be unique';");
				ErrorText = StringFunctionsClientServer.SubstituteParametersToString(ErrorText,
				...
				LanguageCode = Common.DefaultLanguageCode();
				ErrorText = NStr("ru = 'В базе данных уже содержится вид контактной информации с идентификатором ""%1"" внутри группы ""%2"". Идентификатор должен быть уникальным';
									|en = 'The database already contains a contact information kind with ID ""%1"" within group ""%2"". The ID must be unique';", LanguageCode);
				ErrorText = StringFunctionsClientServer.SubstituteParametersToString(ErrorText,
					IDForFormulas, TopLevelParent);
				EventName = NStr("ru = 'Запись дополнительного реквизита (сведения)';
									|en = 'Save additional attribute or information record';", LanguageCode);
	...
	Else
		...
		ErrorText = NStr("ru = 'Идентификатор для формул не заполнен';
		...
		EventName = NStr("ru = 'Запись дополнительного реквизита (сведения)';
							|en = 'Save additional attribute or information record';", LanguageCode);
		ErrorText = NStr("ru = 'Идентификатор для формул не заполнен';
							|en = 'ID for formulas is required';", LanguageCode);
		WriteLogEvent(EventName,
			EventLogLevel.Error,
			Ref.Metadata(),
			Ref,
...
Procedure ProcessDataForMigrationToNewVersion(Parameters) Export
	...
	If ObjectsProcessed = 0 And ObjectsWithIssuesCount <> 0 Then
		MessageText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Не удалось обработать некоторые виды контактной информации (пропущены): %1';
				|en = 'Couldn''t process (skipped) some contact information kinds: %1';"), 
		...
		WriteLogEvent(InfobaseUpdate.EventLogEvent(), EventLogLevel.Information,
			Metadata.Catalogs.ContactInformationKinds,,
				StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Обработана очередная порция видов контактной информации: %1';
