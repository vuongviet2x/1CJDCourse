...
Procedure OnFillToDoList(ToDoList) Export
	...
	For Each Section In Sections Do
		...
		ToDoItem.Presentation = NStr("ru = 'Не совместимы с текущей версией';
									|en = 'Profiles incompatible with the current version';");
		...
		ToDoItem.Presentation = NStr("ru = 'Профили групп доступа';
									|en = 'Access group profiles';");
		ToDoItem.Count    = IncompatibleAccessGroupsProfilesCount;
	...
	Try
		...
		Else
			ProfileByName = ProfileByName(
				NStr("ru = 'Администратор';
					|en = 'Administrator';", Common.DefaultLanguageCode()));
		...
		ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Профиль c идентификатором ""%1""
			           |не поставляется в приложении.';
						|en = 'Cannot find profile with ID:
						|%1.';"),
			String(Id));
		Raise ErrorText;
	EndIf;
		...
		ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Поставляемый профиль с идентификатором ""%1""
			           |не существует в приложении.';
						|en = 'Built-in profile with this id does not exist:
						|%1';"),
			String(Id));
		Raise ErrorText;
	EndIf;
...
Function SuppliedProfileChanged(Profile) Export
	...
	For Each Role In ProfileRolesDetails Do
		...
		If RoleMetadata = Undefined Then
			ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'В поставляемом профиле ""%1""
				           |указана несуществующая роль ""%2"".';
							|en = 'Role ""%2"" specified in built-in profile
							|""%1"" does not exist.';"),
...
Procedure OnInitialItemsFilling(LanguagesCodes, Items, TabularSections) Export
	...
	Item.Description = NStr("ru = 'Администратор';
								|en = 'Administrator';", Common.DefaultLanguageCode());
	Item.SuppliedDataID =
...
Procedure FillAdministratorProfile(AdministratorProfileDetails, ExcludeDetails = False)
	...
	AdministratorProfileDetails.Description  = NStr("ru = 'Администратор';
														|en = 'Administrator';", Common.DefaultLanguageCode());
...
Procedure FillInTheProfilesFolderAdditionalProfiles(FolderDescription_)
	...
	FolderDescription_.Description  = NStr("ru = 'Дополнительные профили';
										|en = 'Additional profiles';", Common.DefaultLanguageCode());
...
Function VerifiedSuppliedSessionProfiles(AccessKindsProperties = Undefined, HashSum = "") Export
	...
	ErrorTitle = StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = 'Заданы недопустимые значения в процедуре %1
		           |общего модуля %2.';
					|en = 'In common module ""%2"",
	...
	If ParametersOfUpdate.DenyProfilesChange
	   And Not ParametersOfUpdate.UpdateModifiedProfiles Then
		...
		ErrorText = ErrorTitle +  StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Когда в параметре %1 свойство
			           |%2 установлено  False,
			           |тогда свойство %3 тоже
			           |должно быть установлено  False.';
						|en = 'In parameter ""%1"", 
	...
	For Each ProfileDetails In ProfilesDetails Do
		...
		If Not ValueIsFilled(ProfileDetails.Id) Then
			ErrorTemplate = ?(IsFolder,
				NStr("ru = 'В описании папки профилей ""%1"" не заполнено свойство %2.';
					|en = 'Property ""%2"" is not specified for profile folder ""%1"".';"),
				NStr("ru = 'В описании профиля ""%1"" не заполнено свойство %2.';
					|en = 'Property ""%2"" is not specified for profile ""%1"".';"));
		...
		ElsIf Not StringFunctionsClientServer.IsUUID(ProfileDetails.Id) Then
			ErrorTemplate = ?(IsFolder,
				NStr("ru = 'В описании папки профилей ""%1"" указан некорректный идентификатор: ""%2"".';
					|en = 'Profile folder ""%1"" contains invalid ID: ""%2"".';"),
				NStr("ru = 'В описании профиля ""%1"" указан некорректный идентификатор: ""%2"".';
					|en = 'Profile ""%1"" contains invalid ID: ""%2"".';"));
			ErrorText = ErrorTitle + StringFunctionsClientServer.SubstituteParametersToString(ErrorTemplate,
				?(ValueIsFilled(ProfileDetails.Name), ProfileDetails.Name, ProfileDetails.Description),
				ProfileDetails.Id);
			Raise ErrorText;
		...
		If AllIDs.Get(Upper(ProfileProperties.Id)) <> Undefined Then
			ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Значение ""%1"" свойства %2 уже используется в описании другого профиля или папки профилей.';
					|en = 'In another profile or profile folder, property ""%2"" already contains value ""%1"".';"),
		...
		ElsIf ValueIsFilled(ProfileProperties.Name) Then
			If TrimAll(ProfileProperties.Name) <> ProfileProperties.Name Then
				ErrorText = ErrorTitle + StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Значение ""%1"" свойства %2 профиля или папки профилей содержит непечатаемые символы.';
						|en = 'In a profile or profile folder, property ""%2"" has value ""%1"" with non-printable characters.';"),
	...
	For Each ProfileDetails In ProfilesDetails Do
		...
		ErrorTemplate = ?(ProfileDetails.Is_Directory,
			NStr("ru = 'В описании папки профилей ""%1"" в свойстве %2
			           |указано несуществующее имя папки профилей ""%3"".';
						|en = 'In the details of profile folder ""%1"",
						|property ""%2"" contains a non-existent name ""%3"".';"),
			NStr("ru = 'В описании профиля ""%1"" в свойстве %2
			           |указано несуществующее имя папки профилей ""%3"".';
						|en = 'In the details of profile ""%1"",
						|property ""%2"" contains a non-existent name ""%3"".';"));
		ErrorText = ErrorTitle + StringFunctionsClientServer.SubstituteParametersToString(ErrorTemplate,
			?(ValueIsFilled(ProfileDetails.Name), ProfileDetails.Name, ProfileDetails.Description),
			...
			ErrorText = ErrorTitle + StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'В описании папки профилей ""%1"" в свойстве %2
				           ...
				           |создающее циклическую зависимость: ""%4"".';
							|en = 'In the details of profile folder ""%1"", property ""%2""
			...
			ErrorText = ErrorTitle + StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'В описании профиля ""%1""
				           |указан несуществующий вид доступа ""%2"".';
							|en = 'Access kind ""%2"" specified in profile
							|""%1"" does not exist.';"),
				?(ValueIsFilled(ProfileDetails.Name),
				  ProfileDetails.Name,
				  ProfileDetails.Id),
...
Function ProfileAssignmentPresentation(ProfileAssignment)
	...
	If ProfileAssignment = "BothForUsersAndExternalUsers" Then
		Return NStr("ru = 'Совместно для пользователей и внешних пользователей';
					|en = 'For users and external users';");
		
	ElsIf ProfileAssignment = "ForExternalUsers" Then
		Return NStr("ru = 'Для внешних пользователей';
					|en = 'For external users';");
	EndIf;
	
	Return NStr("ru = 'Для пользователей';
				|en = 'For users';");
					...
					ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
						NStr("ru = 'При обновлении поставляемого профиля ""%1""
						           |обнаружена несуществующая роль ""%2"".';
									|en = 'When updating built-in profile ""%1"",
