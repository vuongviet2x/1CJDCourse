...
Procedure CheckSuppliedDataUniqueness(Var_FillChecking = False, Cancel = False)
	...
	If Selection.Count() > 1 Then
		...
		BriefErrorDescription = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Поставляемый профиль ""%1"" уже существует:';
				|en = '1C-supplied profile ""%1"" already exists:';"),
			Description);
		
		DetailErrorDescription = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Идентификатор поставляемых данных ""%1"" уже используется в профиле ""%2"":';
				|en = 'The ""%1"" default master data ID is already used in the ""%2"" profile:';"),
		...
		Else
			WriteLogEvent(
				NStr("ru = 'Управление доступом.Нарушение однозначности поставляемого профиля';
					|en = 'Access management.Duplicate built-in profile';",
