		...
		For Each String In TreeItems Do
			...
			If String.IsNonExistingRole Then
				CommonClientServer.AddUserError(Errors,
					...
					StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Роль ""%1"" не найдена в метаданных.';
																				|en = 'Role ""%1"" is not found in the metadata.';"), String.Synonym),
					"Roles",
					TreeItems.IndexOf(String),
					StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Роль ""%1"" в строке %2 не найдена в метаданных.';
																				|en = 'Role ""%1"" in line #%2 is not found in the metadata.';"), String.Synonym, "%1"));
			EndIf;
			If String.IsUnavailableRole Then
				CommonClientServer.AddUserError(Errors,
					"Roles[%1].RolesSynonym",
					StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Роль ""%1"" недоступна для назначения профиля.';
																				|en = 'Role ""%1"" is not available for profile assignment.';"), String.Synonym),
					"Roles",
					TreeItems.IndexOf(String),
					StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Роль ""%1"" в строке %2 недоступна для назначения профиля.';
																				|en = 'Role ""%1"" in line #%2 is not available for profile assignment.';"), String.Synonym, "%1"));
			EndIf;
		EndDo;
	EndIf;
...
Procedure RolesCheckOnChange(Item)
	...
	ElsIf StandardExtensionRoles.Get(TableRow.Name) <> Undefined Then
		...
		ShowMessageBox(, NStr("ru = 'Стандартные роли расширений добавляются и удаляются автоматически.';
										|en = '1C:Enterprise automatically deletes extensions'' standard roles.';"));
	Else
...
Procedure RestoreByInitialFilling(Command)
	...
	ShowQueryBox(
		...
		NStr("ru = 'Восстановить профиль по содержимому начального заполнения?';
			|en = 'Do you want to restore the profile to the initial settings?';"),
...
Procedure SelectPurpose(Command)
	...
	UsersInternalClient.SelectPurpose(ThisObject, NStr("ru = 'Выбор назначения профиля групп доступа';
																	|en = 'Select access group profile assignment';"),,, NotifyDescription);
EndProcedure
...
Procedure RestoreByInitialFillingCompletion(Response, Context) Export
		...
		Text =
			NStr("ru = 'Профиль ""%1"" восстановлен по содержимому начального заполнения,
			           |группы доступа профиля обновлены.';
						|en = 'Profile ""%1"" has been restored.
	...
	ShowUserNotification(NStr("ru = 'Профиль восстановлен';
										|en = 'Profile restored';"),
		GetURL(Object.Ref),
	...
	Return
		NStr("ru = 'Обновить группы доступа, использующие этот профиль?
		           ...
		           |значениями доступа и добавлены недостающие виды доступа.';
					|en = 'Do you want to update the access groups that use the profile?
					|
