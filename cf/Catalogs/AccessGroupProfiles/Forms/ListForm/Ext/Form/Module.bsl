...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Parameters.ChoiceMode Then
		...
		If Parameters.CloseOnChoice = False Then
			...
			Title = NStr("ru = 'Подбор профилей групп доступа';
							|en = 'Pick access group profiles';");
		Else
			Title = NStr("ru = 'Выбор профиля групп доступа';
							|en = 'Select access group profile';");
		EndIf;
	Else
...
Procedure UsersKindStartChoice(Item, ChoiceData, StandardProcessing)
	...
	UsersInternalClient.SelectPurpose(ThisObject,
		NStr("ru = 'Выбор назначения профилей';
			|en = 'Select profile assignment';"), True, True, NotifyDescription);
