...
Procedure Release(Command)
	...
	If Modified Then
		ShowQueryBox(ReleaseCompletion,
			NStr("ru = 'При отмене редактирования внесенные изменения будут утеряны. Продолжить?';
				|en = 'If editing is canceled, changes made will be lost. Continue?';"),
...
Procedure RefreshTitle()
	...
	If TypeOf(Object.FileOwner) = Type("CatalogRef.FilesFolders") Then
		FileType = NStr("ru = 'Файл';
						|en = 'File';");
	Else
		FileType = NStr("ru = 'Присоединенный файл';
						|en = 'Attachment';");
	EndIf;
	...
	If ValueIsFilled(Object.Ref) Then
		Title = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = '%1 (%2)';
				|en = '%1 (%2)';"), String(Object.Ref), FileType);
	Else
		Title = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = '%1 (Создание)';
				|en = '%1 (Create)';"), FileType);
	EndIf;
