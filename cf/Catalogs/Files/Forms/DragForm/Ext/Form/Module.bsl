...
#If WebClient Then
	WarningText =
		NStr("ru = 'В веб-клиенте импорт файлов недоступен. Используйте команду ""Добавить"" в списке файлов.';
			|en = 'File upload is not available in the web client. Please use the ""Add"" command in the file list.';");
...
Procedure ImportFiles()
	...
	If SelectedFiles.Count() = 0 Then
		CommonClient.MessageToUser(
			NStr("ru = 'Нет файлов для добавления.';
				|en = 'No files to add.';"), , "SelectedFiles");
		FieldsNotFilled = True;
	EndIf;
	
	If FolderForAdding.IsEmpty() Then
		CommonClient.MessageToUser(
			NStr("ru = 'Укажите папку.';
				|en = 'Please select a folder.';"), , "FolderForAdding");
		FieldsNotFilled = True;
	EndIf;
