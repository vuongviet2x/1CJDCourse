...
Procedure UpdateFromThePortal(Command)
	...
	If Modified Then
		...
		ShowQueryBox(Notification, 
			NStr("ru = 'Для проверки обновления запишите изменения. Записать?';
				|en = 'Before checking for updates, save the changes. Save the changes?';"),
...
Procedure SaveAs(Command)
	...
	If IsTempStorageURL(ComponentBinaryDataAddress) Then
		ShowMessageBox(, NStr("ru = 'Перед сохранением компоненты в файл запишите элемент справочника.';
										|en = 'Save the catalog item before saving the add-in.';"));
	Else 
		ClearMessages();
...
Procedure ImportAddInAfterSecurityWarning(Response, Context) Export
	...
	ImportParameters.Dialog.Filter    = NStr("ru = 'Внешняя компонента (*.zip)|*.zip';
												|en = 'Add-in (*.zip)|*.zip';")+"|"
			+ StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Все файлы (%1)|%1';
																			|en = 'All files (%1)|%1';"), GetAllFilesMask());
	ImportParameters.Dialog.Title = NStr("ru = 'Выберите файл внешней компоненты';
...
Procedure ImportAddInAfterPutFile(FileThatWasPut, Context) Export
	...
	If FileThatWasPut = Undefined Then
		ImportAddInOnErrorDisplay(NStr("ru = 'Не удалось загрузить файл компоненты.';
													|en = 'Cannot import an add-in file.';"));
		Return;
	EndIf;
...
Function ImportAddInFromFileOnServer(Val ImportParameters)
	...
	If Not Users.IsFullUser(,, False) Then
		Raise(NStr("ru = 'Недостаточно прав для загрузки компоненты.';
								|en = 'Insufficient rights to import an add-in.';"), ErrorCategory.AccessViolation);
	EndIf;
	...
	If ValueIsFilled(ObjectOfCatalog.Id)
		...
		If ObjectOfCatalog.Id <> Information.Attributes.Id Then 
			Result.ErrorDescription = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Невозможно обновить компоненту, т.к. различаются идентификаторы.
					|Ожидался %1, а идентификатор загружаемой внешней компоненты: %2.';
					|en = 'Cannot update the add-in as IDs are different.
	...
	ObjectOfCatalog.ErrorDescription = StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = 'Загружена из файла %1. %2.';
			|en = 'Imported from file %1. %2.';"),
