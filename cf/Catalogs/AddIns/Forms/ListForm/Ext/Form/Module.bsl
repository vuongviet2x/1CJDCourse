...
Procedure AddFromDirectory(Command)
	...
	SuggestionText =  NStr("ru = 'Для загрузки внешних компонент из каталога требуется расширение для работы с 1С:Предприятием.';
							|en = 'To import add-ins from the directory, install 1C:Enterprise Extension.';");
...
Procedure AddFromFiles(Command)
	...
	ImportParameters.Dialog.Title = NStr("ru = 'Выберите файл(ы) компонент';
												|en = 'Select add-in files';");
	ImportParameters.Dialog.Filter = NStr("ru = 'Архив (*.zip)|*.zip';
											|en = 'Archive (*.zip)|*.zip';") + "|"
...
Procedure AddAddInsFromDirectoryAfterExtensionsAttached(Result, CreationParameters) Export
	...
	Items.Pages_Group.CurrentPage = Items.WaitPage;
	FileSystemClient.SelectDirectory(Notification, NStr("ru = 'Выберите каталог с файлами компонент';
															|en = 'Select a directory with add-in files';"));
...
Async Procedure AddAddInsAfterDirectorySelected(SelectedDirectory, AdditionalParameters) Export
	...
	If DetailsOfFilesToTransfer.Count() = 0 Then
		Raise NStr("ru = 'Указанный каталог не содержит файлов компонент.';
								|en = 'The specified directory does not contain any add-in files.';");
	EndIf;
...
Procedure DownloadAddInsAfterSafetyWarning(Response, PlacedFiles) Export
	...
	If Result.Count() > 0 Then
		...
		TextDocument.InsertLine(0, NStr(
			...
			+ Chars.LF);
		TextDocument.Show(NStr("ru = 'Не удалось загрузить компоненты';
										|en = 'Cannot import the add-ins';"));
	EndIf;
