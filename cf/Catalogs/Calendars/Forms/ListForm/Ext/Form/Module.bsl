	...
	While Selection.Next() Do
		...
		If ValueIsFilled(Selection.BusinessCalendar) Then
			If Not ValueIsFilled(Selection.BusinessCalendarFillDate) Then
				PossibleReason = NStr("ru = 'Производственный календарь не заполнен';
										|en = 'The business calendar is blank.';");
			Else
				If Selection.BusinessCalendarFillDate < RequiredFillingDate Then
					PossibleReason = NStr("ru = 'Производственный календарь не заполнен на очередной календарный год';
											|en = 'The business calendar for the next calendar year is blank.';");
				EndIf;
			EndIf;
		Else
			If Not ValueIsFilled(Selection.EndDate) Then
				PossibleReason = NStr("ru = 'График не был заполнен на очередной календарный год';
										|en = 'The schedule for the next calendar year was not filled in.';");
			Else
				If Selection.EndDate < RequiredFillingDate Then
					PossibleReason = NStr("ru = 'Период заполнения графика ограничен (см. поле ""Дата окончания"").';
