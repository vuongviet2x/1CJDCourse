...
Procedure FillCheckProcessingAtServer(Cancel, CheckedAttributes)
	...
	If Object.FillingTemplate.FindRows(New Structure("DayAddedToSchedule", True)).Count() = 0 Then
		Common.MessageToUser(
			NStr("ru = 'Не отмечены дни, включаемые в график работы';
				|en = 'Please select the days to include in the work schedule.';"), , "Object.FillingTemplate", , Cancel);
	EndIf;
...
Procedure GenerateFillingTemplate(FillingMethod, FillingTemplate, Val PeriodLength, Val StartingDate = Undefined)
	...
	If FillingMethod = PredefinedValue("Enum.WorkScheduleFillingMethods.ByWeeks") Then
		FillingTemplate[0].DayPresentation = NStr("ru = 'Понедельник';
													|en = 'Monday';");
		FillingTemplate[1].DayPresentation = NStr("ru = 'Вторник';
													|en = 'Tuesday';");
		FillingTemplate[2].DayPresentation = NStr("ru = 'Среда';
													|en = 'Wednesday';");
		FillingTemplate[3].DayPresentation = NStr("ru = 'Четверг';
													|en = 'Thursday';");
		FillingTemplate[4].DayPresentation = NStr("ru = 'Пятница';
													|en = 'Friday';");
		FillingTemplate[5].DayPresentation = NStr("ru = 'Суббота';
													|en = 'Saturday';");
		FillingTemplate[6].DayPresentation = NStr("ru = 'Воскресенье';
													|en = 'Sunday';");
	Else
...
Function DaySchedulePresentation(Form, DayNumber)
	...
	Return StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = '%1 ч. (%2)';
																		|en = '%1 h (%2)';"), Hours, IntervalsPresentation);
...
Function DayPartPresentation(Val DayPart, TwelveHourTimeFormat)
	...
	TimeFormat = ?(TwelveHourTimeFormat,
		NStr("ru = 'ДФ=''чч:мм вв''';
			|en = 'DF=''hh:mm tt''';"), NStr("ru = 'ДФ=ЧЧ:мм; ДП=';
											|en = 'DF=HH:mm; DE=';"));
...
Function BlankSchedulePresentation()
	...
	Return NStr("ru = 'Заполнить расписание';
				|en = 'Fill in the schedule';");
...
Procedure AdjustScheduleFilled(Form)
	...
	If Not ValueIsFilled(Form.FillDate) Then
		Form.IsFilledInformationText = NStr("ru = 'График работы не заполнен';
														|en = 'The work schedule is blank.';");
		Form.RequiresFilling = True;
	Else	
		If Not ValueIsFilled(Form.Object.PlanningHorizon) Then
			InformationText = NStr("ru = 'График работы заполнен до %1';
										|en = 'The work schedule is filled in till %1.';");
...
#EndIf
			...
			InformationText = NStr("ru = 'График работы заполнен до %1, с учетом горизонта планирования график должен быть заполнен до %2';
										|en = 'The work schedule is filled till %1. The provided scheduling interval requires that you fill it till %2.';");
	...
	Else
		If Form.ResultFilledByTemplate Then
			If ValueIsFilled(Form.Object.BusinessCalendar) Then
				InformationText = NStr("ru = 'График работы автоматически обновляется при изменении производственного календаря за текущий год.';
											|en = 'The work schedule is updated automatically upon changing the business calendar for the current year.';");
...
Procedure FillInformationTextManualEditing(Form)
	...
	If Form.ManualEditing Then
		InformativeImage = PictureLib.Warning;
		InformationText = NStr("ru = 'График работы на текущий год изменен вручную. Изменения выделены в результатах заполнения.';
									|en = 'The work schedule for the current year is changed manually. The changes are highlighted.';");
	EndIf;
