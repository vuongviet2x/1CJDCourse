...
Procedure SetHeader()
	...
	If Not IsBlankString(PropertyName) Then
		If ValueIsFilled(Object.Ref) Then
			Title = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = '%1 (%2)';
																					|en = '%1 (%2)';"),
				Object.Description,
				PropertyName);
		Else
			Title = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = '%1 (Создание)';
																					|en = '%1 (Create)';"), PropertyName);
		EndIf;
	Else
		PropertyName = String(AttributesValues.Title);
		
		If ValueIsFilled(Object.Ref) Then
			Title = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = '%1 (Группа значений свойства %2)';
																					|en = '%1 (""%2"" property values group)';"),
				Object.Description,
				PropertyName);
		Else
			Title = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Группа значений свойства %1 (Создание)';
																					|en = '""%1"" property values group (Create)';"), PropertyName);
		EndIf;
	EndIf;
