...
Procedure ApplyRules(ExportingParameters, StorageAddress) Export
	...
	If Result[2].IsEmpty() Then
		MessageText = NStr("ru = 'В выбранной папке нет писем.';
								|en = 'The selected folder is empty.';");
	...
	While SelectedRule.Next() Do
		...
		Except
			...
			ErrorMessageTemplate = NStr("ru = 'Не удалось применить правило обработки писем ""%1"" для электронной почты ""%2"" по причине: 
			                                |%3
	...
	If MailFolders.Count() > 0 Then
		MessageText = NStr("ru = 'Перенос писем в папки выполнен.';
								|en = 'All messages are moved to the folders.';");
	Else
		MessageText =  NStr("ru = 'Ни одно письмо не было перенесено.';
								|en = 'No messages were moved.';");
	EndIf;
