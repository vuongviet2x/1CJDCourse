...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Common.IsMobileClient() Then
		Items.SpecifyContacts.Title                  = NStr("ru = 'Сохранить адреса в контакты';
																	|en = 'Save emails to Address Book';");
...
Procedure BeforeClose(Cancel, Exit, WarningText, StandardProcessing)
	...
	If Not Exit
		...
		If HasFilesToEdit Then
			...
			QueryText = NStr("ru = 'Данные были изменены, Записать?';
...
Procedure RecipientsListBeforeEditEnd(Item, NewRow, CancelEdit, Cancel)
	...
	If FoundRows.Count() > 1 Then
		ErrorTextTemplate = NStr("ru = 'Адрес %1 уже есть в списке.';
									|en = 'You already added %1.';");
...
Procedure SendForwardExecute(Command)
	...
	If RecipientsList.Count() = 0 Then
		...
		CommonClient.MessageToUser(
			NStr("ru = 'Укажите хотя бы одного получателя письма.';
				|en = 'Specify at least one email recipient.';"),, "RecipientsList");
		Return;
	...
	ElsIf (RecipientsList.Count() = 1 And IsBlankString(RecipientsList[0].Address)) Then
		...
		CommonClient.MessageToUser(
			NStr("ru = 'Укажите хотя бы одного получателя письма.';
				|en = 'Specify at least one email recipient.';"),, "RecipientsList[0].Presentation");
		Return;
...
Procedure ImportanceHigh(Command)
	...
	Items.DecorationImportance.ToolTip = NStr("ru = 'Высокая важность';
												|en = 'High importance';");
	Modified = True;
...
Procedure ImportanceNormal(Command)
	...
	Items.DecorationImportance.ToolTip = NStr("ru = 'Обычная важность';
												|en = 'Normal importance';");
	Modified = True;
...
Procedure ImportanceLow(Command)
	...
	Items.DecorationImportance.ToolTip = NStr("ru = 'Низкая важность';
												|en = 'Low importance';");
	Modified = True;
...
Procedure DefineItemsVisibilityAvailabilityDependingOnEmailStatus()
	...
	If Object.EmailStatus <> Enums.OutgoingEmailStatuses.Draft Then
		...
		If Object.TextType = Enums.EmailTextTypes.HTML Then
			...
			EmailText = Interactions.ProcessHTMLText(Object.Ref, False, False);
...
Procedure AttachIncomingBaseEmailAsAttachmentIfNecessary(CurrentObject)
	...
	If CurrentObject.EmailStatus = Enums.OutgoingEmailStatuses.Outgoing 
		...
		FileParameters.BaseName = NStr("ru = 'Пересылаемое сообщение';
...
Procedure AttachmentProperties(Command)
	...
	If CurrentData.Ref = Undefined Then
		...
		QueryText = NStr("ru = 'Свойства файла доступны только после его записи. Записать?';
							|en = 'You can access the file''s properties after you save the file. Save it now?';");
...
Function GenerateOutgoingMessagePlainText(SelectionIncomingEmailData, CurrentObject)
	...
	StringHeader1 = NStr("ru = '-----Пересылаемое сообщение-----';
						|en = '-----Forwarded message-----';");
	
	StringHeader1 = StringHeader1 + Chars.LF+ NStr("ru = 'От';
												|en = 'From';") + ": "+ SelectionIncomingEmailData.SenderPresentation
		          + ?(SelectionIncomingEmailData.MetadataObjectName = "IncomingEmail",
		          "[" + SelectionIncomingEmailData.SenderAddress +"]",
		          "");
		
	StringHeader1 = StringHeader1 + Chars.LF+ NStr("ru = 'Отправлено';
												|en = 'Sent on';") + ": " 
	              + Format(SelectionIncomingEmailData.Date,"DLF=DT");
	
	StringHeader1 = StringHeader1 + Chars.LF+ NStr("ru = 'Кому';
												|en = 'To';") + ": " 
	...
	If CCRecipientsTable.Count() > 0 Then
		StringHeader1 = StringHeader1 + Chars.LF+ NStr("ru = 'Копии';
													|en = 'Cc';") + ": "
		+ Interactions.GetIncomingEmailRecipientsPresentations(CCRecipientsTable);
	EndIf;
	
	StringHeader1 = StringHeader1 + Chars.LF+ NStr("ru = 'Тема';
												|en = 'Subject';") + ": " + SelectionIncomingEmailData.Subject;
...
Procedure SendExecute()
	...
	If FoundRows.Count() = 0 Then
		CommonClient.MessageToUser(
			NStr("ru = 'Выбранная почта недоступна для отправки писем.';
				|en = 'The selected account cannot be used to send mail.';"),, "SenderPresentation", "Object");
		Return;
	EndIf;
	...
	If FoundRows[0].DeleteAfterSend Then
		...
		ButtonsList.Add(DialogReturnCode.Yes, NStr("ru = 'Отправить';
															|en = 'Send';"));
		ButtonsList.Add(DialogReturnCode.No, NStr("ru = 'Отправить и сохранить';
															|en = 'Send and save';"));
		ButtonsList.Add(DialogReturnCode.Cancel, NStr("ru = 'Отмена';
																|en = 'Cancel';"));
		
		QueryText = NStr("ru = 'В настройках почты не предусмотрено сохранение отправленных писем в приложении.
		                    |Продолжить?';
							|en = 'This email account doesn''t store sent messages in the app.
							|Do you want to continue?';");
		CloseNotificationHandler = New NotifyDescription("PromptForNotSavingSentEmail", ThisObject);
		ShowQueryBox(CloseNotificationHandler,QueryText, ButtonsList,, DialogReturnCode.Yes, NStr("ru = 'Отправка письма';
																											|en = 'Send message';"));
	Else
		SendMailClient();
	EndIf;
...
Procedure SetButtonTitleByDefault()
	...
	If Object.EmailStatus = Enums.OutgoingEmailStatuses.Sent Then
		Items.Send.Title = NStr("ru = 'Переслать';
											|en = 'Forward';");
	ElsIf Object.EmailStatus = Enums.OutgoingEmailStatuses.Outgoing Then
		Items.Send.Title = NStr("ru = 'Отправить сейчас';
											|en = 'Send now';");
	ElsIf Object.EmailStatus = Enums.OutgoingEmailStatuses.Draft Then
		...
		Items.Send.Title = NStr("ru = 'Отправить';
											|en = 'Send';");
...
Procedure ShowEmailAddressRequiredMessage()
	ShowMessageBox(, NStr("ru = 'Введите адрес электронной почты';
...
Procedure SendMailClient()
	...
	If Result.AttachmentError Then
		EmailOperationsClient.ReportConnectionError(Object.Account, 
			NStr("ru = 'Письмо не отправлено';
				|en = 'The message is not sent';"), Result.MessageText);
	Else
