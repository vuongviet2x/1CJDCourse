...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Common.IsMobileClient() Then
		...
		Items.EndDate.Title                          = NStr("ru = 'Конец';
																		|en = 'End';");
...
Procedure EndTimeOnChange(Item)
	...
	If BegOfDay(Object.EndDate) + ExtractTime(EndTime) < Object.StartDate Then
		ClearMessages();
		CommonClient.MessageToUser(NStr("ru = 'Время окончания не может быть меньше времени начала.';
														|en = 'End time cannot be earlier than Start time.';"),,"EndTime");
...
Procedure EndDateOnChange(Item)
	...
	If BegOfDay(EndDate) + ExtractTime(EndTime) < Object.StartDate Then
		...
		If BegOfDay(EndDate) < BegOfDay(StartDate) Then
			CommonClient.MessageToUser(NStr("ru = 'Дата окончания не может быть меньше даты начала.';
															|en = 'End date cannot be earlier than Start date.';"),,"EndDate");
		Else
			CommonClient.MessageToUser(NStr("ru = 'Время окончания не может быть меньше времени начала.';
															|en = 'End time cannot be earlier than Start time.';"),,"EndTime");
		EndIf;
