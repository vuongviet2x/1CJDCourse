...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	Surveys.SetQuestionnaireSectionsTreeItemIntroductionConclusion(SectionsTree, 
		NStr("ru = 'Заключение';
			|en = 'Closing statement';"), "ClosingStatement");
	...
	TitleSectionsCommand = NStr("ru = 'Показать разделы';
									|en = 'Show sections';");
...
Procedure AfterWrite(WriteParameters)
	...
	ShowUserNotification(NStr("ru = 'Изменение';
										|en = 'Edited';"),, String(Object.Ref), PictureLib.DialogInformation);
...
Procedure FillingFormEndAndClose(Command)
	...
	ShowQueryBox(OnCloseNotifyHandler, 
		NStr("ru = 'Ваша анкета будет принята. 
			...
			|Продолжить?';
			|en = 'You will not be able to edit your responses
...
Function CheckQuestionnaireFilling()
	...
	While Selection.Next() Do
		Common.MessageToUser(NStr("ru = 'Не указан ответ на вопрос';
													|en = 'No response was given to question';") + "- " 
...
Procedure SetAttributesValuesBySurvey(AttributesSurvey)
	...
	ClosingStatement = ?(IsBlankString(AttributesSurvey.ClosingStatement), 
		NStr("ru = 'Спасибо за то, что заполнили анкету.';
...
Procedure ChangeSectionsTreeVisibility()
	...
	TitleSectionsCommand = ?(Items.SectionsTreeGroup.Visible, NStr("ru = 'Скрыть разделы';
																				|en = 'Hide sections';"), 
		NStr("ru = 'Показать разделы';
			|en = 'Show sections';"));
	Items.HideShowSectionsTree.Title = TitleSectionsCommand;
