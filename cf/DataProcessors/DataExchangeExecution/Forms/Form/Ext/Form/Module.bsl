...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Not ValueIsFilled(InfobaseNode) Then
		...
		Else
			DataExchangeServer.ReportError(NStr("ru = 'Не заданы параметры формы. Форма не может быть открыта.';
													|en = 'Cannot open the form. The form parameters are not specified.';"), Cancel);
			Return;
		EndIf;
	...
	Title = StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = 'Синхронизация данных с ""%1""';
			|en = 'Synchronize data with %1';"), PeerInfobaseName);
...
Function Attachable_UserPasswordRequestOnGoNext(Cancel)
	...
	If Not SynchronizationPasswordSpecified
		...
		CommonClient.MessageToUser(NStr("ru = 'Не указан пароль.';
														|en = 'Please enter the password.';"), , "WSPassword", , Cancel);
		Return Undefined;
	EndIf;
...
Function BackgroundJobStartAtServer(JobParameters, VersionDifferenceErrorOnGetData, CheckVersionDifference)
	...
	If JobParameters.ExecuteImport1 Then
		...
		DescriptionTemplate = NStr("ru = 'Выполняется загрузка данных из %1';
									|en = 'Importing data from %1';");
		
	Else
		DescriptionTemplate = NStr("ru = 'Выполняется выгрузка данных в %1';
...
Procedure BackgroundJobGetResultAtServer()
	...
	StandardErrorPresentation = StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = 'Не удалось выполнить %1. Подробности см. в журнале регистрации';
			|en = 'Cannot %1. See the event log for details.';"),
		?(BackgroundJobCurrentAction = 1, NStr("ru = 'получение данных';
														|en = 'receive data';"), NStr("ru = 'отправку данных';
																						|en = 'send data';")));
	...
	If BackgroundJobCompleteResult.Status = "Error" Then
		...
		ErrorText = NStr("ru = 'Обнаружено дублирование настроек синхронизации данных';
							|en = 'Duplicate data synchronization settings are detected';", Common.DefaultLanguageCode()); 
...
Procedure DecorationErrorIDAssignmentForNodeURLProcessing(Item, FormattedStringURL, StandardProcessing)
	...
	OpenForm("CommonForm.AdditionalDetails", New Structure("Title,TemplateName",
		NStr("ru = 'Установка кода для предопределенного узла';
			|en = 'Set predefined node code';"), "InstructionToSetCodeForPredefinedNode_en"));
