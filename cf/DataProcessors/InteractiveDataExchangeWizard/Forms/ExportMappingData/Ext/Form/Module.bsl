...
Procedure BeforeClose(Cancel, Exit, WarningText, StandardProcessing)
	...
	If InitialExport Then
		WarningText = NStr("ru = 'Отменить начальную выгрузку данных?';
									|en = 'Do you want to cancel the initial data export?';");
	Else
		WarningText = NStr("ru = 'Отменить выгрузку данных?';
									|en = 'Do you want to cancel the data export?';");
	EndIf;
...
Procedure CheckCanUseForm(Cancel = False)
	...
	If DataExchangeCached.IsDistributedInfobaseNode(Parameters.ExchangeNode) Then
		MessageText = NStr("ru = 'Начальная выгрузка не поддерживается для узлов распределенных информационных баз.';
								|en = 'Initial export is not supported for distributed infobase nodes.';");
		Common.MessageToUser(MessageText, , , , Cancel);
		Return;
	EndIf;
