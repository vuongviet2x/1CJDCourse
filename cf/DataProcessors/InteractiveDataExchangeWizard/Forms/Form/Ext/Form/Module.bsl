...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	ElsIf Parameters.Property("InfobaseNodeCode") Then
		...
		If Not ValueIsFilled(Object.InfobaseNode) Then
			Raise NStr("ru = 'Настройка обмена данными не найдена.';
	...
	Else
		...
		Raise NStr("ru = 'Непосредственное открытие помощника не предусмотрено.';
								|en = 'Cannot open this wizard directly.';");
	...
	If Not DataExchangeCached.IsUniversalDataExchangeNode(Object.InfobaseNode) Then
		Raise NStr("ru = 'Для выбранного узла выполнение обмена данными с настройкой не предусмотрено.';
								|en = 'The selected node does not support settings-based data exchange.';");
	EndIf;
	...
	If AllNodes.Find(Object.InfobaseNode) = Undefined Then
		Raise NStr("ru = 'Для выбранного узла сопоставление данных не предусмотрено.';
								|en = 'The selected node does not provide data mapping.';");
	EndIf;
	...
	If Not GetData And Not SendData And Not MessageReceivedForDataMapping Then
		Raise NStr("ru = 'Заданный сценарий синхронизации данных не поддерживается.';
								|en = 'The data synchronization scenario is not supported.';");
	EndIf;
	...
	PeerInfobaseName = String(Object.InfobaseNode);
	
	SetFormHeader();
	
	Items.DeleteMessageForMappingDecoration.Title = StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = 'При загрузке сообщения для сопоставления возникла ошибка.
		...
		|en = 'An error occurred while importing a message for mapping.
...
Procedure BeforeClose(Cancel, Exit, WarningText, StandardProcessing)
	...
	CommonClient.ShowArbitraryFormClosingConfirmation(ThisObject, Cancel, Exit,
		NStr("ru = 'Выйти из помощника?';
			|en = 'Do you want to exit the wizard?';"), "ForceCloseForm");
...
Procedure NotificationProcessing(EventName, Parameter, Source)
	...
	If EventName = "ObjectMappingFormClosing" Then
		...
		If Cancel Then
			ShowMessageBox(, NStr("ru = 'При получении информации статистики возникли ошибки.';
											|en = 'Error gathering statistic data.';"));
		Else
			
			ExpandStatisticsTree(Parameter.UniqueKey);
			
			ShowUserNotification(NStr("ru = 'Сбор информации завершен';
												|en = 'Information collection is complete';"));
		EndIf;
...
Procedure RunDataImportForRow(Command)
	...
	If SelectedRows.Count() = 0 Then
		NString = NStr("ru = 'Выберите имя таблицы в поле статистической информации.';
						|en = 'Select a table name in the statistics field.';");
...
Procedure OpenMappingForm(Command)
	...
	If Not CurrentData.UsePreview Then
		ShowMessageBox(, NStr("ru = 'Для типа данных нельзя выполнить сопоставление объектов.';
										|en = 'Object mapping cannot be performed for the data type.';"));
		Return;
	EndIf;
...
Procedure ExportAdditionClearGeneralFilter(Command)
	...
	QueryText   = NStr("ru = 'Очистить общий отбор?';
							|en = 'Do you want to clear the common filter?';");
...
Procedure ExportAdditionClearDetailedFilter(Command)
	TitleText = NStr("ru = 'Подтверждение';
							|en = 'Confirm operation';");
	QueryText   = NStr("ru = 'Очистить детальный отбор?';
							|en = 'Do you want to clear the detailed filter?';");
...
Procedure ExportAdditionFilterHistoryMenuSelection(Val SelectedElement, Val AdditionalParameters) Export
	...
	If TypeOf(SettingPresentation)=Type("String") Then
		...
		NStr("ru = 'Восстановить настройки ""%1""?';
			|en = 'Do you want to restore ""%1"" settings?';"), SettingPresentation);
...
Procedure ExecuteDataImportForRowContinued(Val SelectedRows) 
	...
	ShowUserNotification(NStr("ru = 'Загрузка данных завершена.';
...
Procedure OpenNodeDataExchangeDirectory()
	...
	If IsBlankString(DirectoryName) Then
		ShowMessageBox(, NStr("ru = 'Не задан каталог синхронизации данных.';
										|en = 'Data synchronization directory is not specified.';"));
		Return;
	EndIf;
...
Procedure UpdateTooltipTitleOfStatisticsInformationMove()
		...
		If EndDataMapping Then
			If LoadMessageAfterMapping Then
				Items.StatisticsDataNavigationTooltipDecoration.Title =
					NStr("ru = 'Нажмите кнопку ""Далее"" для завершения сопоставления данных и загрузки сообщения обмена.';
						|en = 'Click Next to confirm the mapping and import the exchange message.';");
			Else
				Items.StatisticsDataNavigationTooltipDecoration.Title =
	...
	Else
		Items.StatisticsDataNavigationTooltipDecoration.Title =
			NStr("ru = 'Нажмите кнопку ""Далее"" для синхронизации данных.';
				|en = 'Click Next to synchronize data.';");
	EndIf;
...
Procedure OnChangeExchangeMessagesTransportKind(Initialize = False, ConfiguredTransportKinds = Undefined)
	...
	If Initialize Then
		...
		If TransportChoiceList.Count() = 0 Then
			TransportChoiceList.Add(Undefined, NStr("ru = 'подключение не настроено';
																|en = 'no connections are configured';"));
...
Procedure SetFormHeader()
	...
	TitleTemplate1 = NStr("ru = 'Обмен данными с ""%1""';
							|en = 'Exchange data with %1';");
	
	If MessageReceivedForDataMapping Then
		TitleTemplate1 = NStr("ru = 'Сопоставление данных ""%1""';
								|en = 'Map %1 data';");
	ElsIf GetData And SendData Then
		TitleTemplate1 = NStr("ru = 'Синхронизация данных с ""%1""';
								|en = 'Synchronize data with %1';");
	ElsIf SendData Then
		TitleTemplate1 = NStr("ru = 'Отправка данных для ""%1""';
								|en = 'Send data to %1';");
	ElsIf GetData Then
		TitleTemplate1 = NStr("ru = 'Получение данных от ""%1""';
								|en = 'Receive data from %1';");
	EndIf;
		
	Title = StringFunctionsClientServer.SubstituteParametersToString(
		TitleTemplate1, PeerInfobaseName);
...
Procedure SetAdditionDetailDescription()
	...
	NoFilter2 = IsBlankString(Text);
	If NoFilter2 Then
		Text = NStr("ru = 'Дополнительные данные не выбраны';
...
Procedure RefreshDataExchangeStatusItemPresentation()
	...
	If Items.DataImportStatusPages.CurrentPage=Items.ImportStatusUndefined Then
		Items.GoToDataImportEventLog.Title = NStr("ru = 'Загрузка данных не произведена';
	...
	If Items.DataExportStatusPages.CurrentPage=Items.ExportStatusUndefined Then
		Items.GoToDataExportEventLog.Title = NStr("ru = 'Выгрузка данных не произведена';
...
Procedure ProcessBackgroundJobExecutionStatus(CurrentAction = Undefined)
	...
	ElsIf BackgroundJobExecutionResult.Status = "Canceled" Then
		ErrorMessage = NStr("ru = 'Действие отменено пользователем.';
								|en = 'The operation was canceled by user.';");
		CommitCompletion = True;
	EndIf;
...
Function Attachable_BeginningPageOnGoNext(Cancel)
	...
	If Object.InfobaseNode.IsEmpty() Then
		...
		NString = NStr("ru = 'Укажите узел информационной базы';
						|en = 'Please specify the infobase node.';");
	...
	ElsIf Object.ExchangeMessagesTransportKind.IsEmpty()
		...
		NString = NStr("ru = 'Укажите вариант подключения';
						|en = 'Please specify the connection option.';");
	...
	ElsIf ExchangeOverWebService And IsBlankString(WSPassword) Then
		...
		NString = NStr("ru = 'Не указан пароль.';
...
Procedure TestConnectionAndSaveSettings(Cancel)
	...
	If Not HasConnection Then
		ErrorMessage = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Не удалось подключиться к приложению в Интернете, по причине ""%1"".
			|Убедитесь, что:
	...
	ElsIf Not SettingCompleted Then
		ErrorMessage = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Для продолжения перейдите в программу ""%1"" и завершите в ней настройку синхронизации. Выполнение обмена данными отменено.';
				|en = 'To continue, set up synchronization in ""%1"". The data exchange is canceled.';"),
			PeerInfobaseName);
		Cancel = True;
	ElsIf DataReceivedForMapping Then
		ErrorMessage = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Для продолжения перейдите в программу ""%1"" и выполните загрузку сообщения для сопоставления данных. Выполнение обмена данными отменено.';
				|en = 'To continue, open %1 and import the data mapping message. The data exchange is canceled.';"),
			PeerInfobaseName);
		Cancel = True;
	EndIf;
...
Function Attachable_PageDataExchangeJobCheck_OnOpen(Cancel, SkipPage, IsMoveNext)
	...
	If HasNodeScheduledExchange Then
		...
		If ValueIsFilled(ScenarioUsingInternalPublication) Then
			...
			Template = NStr("ru = 'Синхронизация для узла ""%1"" уже запланирована 
                           |в сценарии ""%2"".
                           |
                           |Нажмите Далее, что бы отменить выполнение сценария
                           |и запустить синхронизацию по узлу.';
							|en = 'Synchronization is already scheduled for node ""%1"" 
							|in scenario ""%2"".
							|
		...
		Else
			...
			Template = NStr("ru = 'Для узла ""%1"" уже выполняется синхронизация.
                           |
                           |Нажмите Далее, что-бы прервать текущую синхронизации
                           |и запустить ее снова';
							|en = 'Synchronization is already in progress for node ""%1"".
							|
...
Function Attachable_DataAnalysisWaitingPageTimeConsumingOperationProcessing(Cancel, GoToNext)
	...
	JobParameters.JobDescription  = NStr("ru = 'Получение сообщения обмена во временный каталог';
												|en = 'Get exchange message to temporary directory';");
...
Function Attachable_DataAnalysisWaitingPageTimeConsumingOperationCompletionTimeConsumingOperationProcessing(Cancel, GoToNext)
	...
	If TimeConsumingOperationCompleted
		...
		If Not ValueIsFilled(Object.ExchangeMessageFileName) Then
			...
			JobParameters.MethodParameters      = MethodParameters;
			JobParameters.JobDescription  = NStr("ru = 'Получение файла с данными сообщения обмена во временный каталог';
														|en = 'Get exchange message file to temporary directory';");
...
Procedure GetDataToTemporaryDirectoryAtServerCompletion()
	...
	ErrorMessageTemplate = NStr("ru = 'Не удалось выполнить загрузку данных. Подробности см. в журнале регистрации';
									|en = 'Cannot import data. See the event log for details.';");
...
Function Attachable_DataAnalysisTimeConsumingOperationProcessing(Cancel, GoToNext)
	...
	JobParameters.MethodParameters      = MethodParameters;
	JobParameters.JobDescription  = NStr("ru = 'Анализ данных сообщения обмена';
												|en = 'Analyze exchange message data';");
...
Function Attachable_StatisticsInformationPageOnGoNext(Cancel)
	...
	Buttons.Add(DialogReturnCode.Yes,  NStr("ru = 'Продолжить';
												|en = 'Continue';"));
	Buttons.Add(DialogReturnCode.No, NStr("ru = 'Отмена';
												|en = 'Cancel';"));
...
Function Attachable_DataImportTimeConsumingOperationProcessing(Cancel, GoToNext)
	...
	JobParameters.MethodParameters      = MethodParameters;
	JobParameters.JobDescription  = NStr("ru = 'Загрузка данных из сообщения обмена';
												|en = 'Import data from exchange message';");
	...
	Else
		...
		Else
			CommonClient.MessageToUser(
				NStr("ru = 'Не удалось зарегистрировать данные для выгрузки.';
					|en = 'Cannot register data to export.';"));
		EndIf;
	EndIf;
...
Procedure OnStartExportData(Cancel)
	...
	Else
		...
		JobParameters.MethodParameters      = MethodParameters;
		JobParameters.JobDescription  = NStr("ru = 'Выгрузка данных в сообщение обмена';
													|en = 'Export data to exchange message';");
...
Procedure OnCompleteDataExport()
	...
	Else
		...
		Else
			CommonClient.MessageToUser(
				NStr("ru = 'Не удалось выполнить обмен данными.';
					|en = 'Cannot perform data exchange.';"));
		EndIf;
	EndIf;
...
Procedure DataExportCompletionAtServer()
	...
	If MethodExecutionResult.Cancel
		...
		ErrorMessage = NStr("ru = 'Не удалось выполнить отправку данных. Подробности см. в журнале регистрации';
								|en = 'Cannot send data. See the event log for details.';");
	EndIf;
