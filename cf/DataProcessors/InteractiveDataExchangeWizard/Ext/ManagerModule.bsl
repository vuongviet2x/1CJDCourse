	...
	If Parameters.MessageReceivedForDataMapping Then
		...
		If IsBlankString(StructureOfData.ExchangeMessageFileName) Then
			...
			ErrorMessage = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Сообщение для сопоставления с идентификатором %1 не найдено по пути %2.';
					|en = 'A message to be mapped with ID %1 was not found by path %2.';"),
...
Procedure OnStartRecordData(RegistrationSettings, HandlerParameters, ContinueWait = True) Export
	...
	BackgroundJobKey = StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = 'Регистрация данных для выгрузки (%1)';
			|en = 'Register data for export (%1)';"),
...
Procedure OnStartExportDataForMapping(ExportSettings1, HandlerParameters, ContinueWait = True) Export
	...
	BackgroundJobKey = StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = 'Выгрузка данных для сопоставления (%1)';
			|en = 'Export mapping data (%1)';"),
	...
	If HasActiveBackgroundJobs(BackgroundJobKey) Then
		Raise StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Выгрузка данных для сопоставления для ""%1"" уже выполняется.';
				|en = 'Data to map for ""%1"" is already being exported.';"),
			ExportSettings1.ExchangeNode);
	EndIf;
...
Procedure RegisterDataforExport(Parameters, ResultAddress) Export
	...
	Except
		...
		Result.ErrorMessage = NStr("ru = 'Возникла проблема при добавлении данных к выгрузке:';
...
Procedure ExportDataForMapping(Parameters, ResultAddress) Export
	...
	If Not Result.DataExported1
		...
		Result.ErrorMessage = NStr("ru = 'При выполнении выгрузки данных для сопоставления возникли ошибки (см. Журнал регистрации).';
											|en = 'Errors occurred while exporting mapping data. See the event log.';");
	EndIf;
...
Function AutomaticDataMappingResult(Val Peer,
	...
	If Result.Cancel Then
		Raise StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Не удалось загрузить данные из ""%1"" (этап автоматического сопоставления данных).';
				|en = 'Cannot import data from ""%1"" (automatic data mapping step).';"),
			String(Peer));
	EndIf;
...
Procedure SupplementStatisticTable(StatisticsInformation, Cancel, ErrorMessage = "")
	...
	For Each TableRow In StatisticsInformation Do
		...
		Except
			Cancel = True;
			ErrorMessage = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Ошибка: тип ""%1"" не определен.';
					|en = 'Error: the %1 type is not defined.';"), TableRow.ObjectTypeString);
			Break;
		EndTry;
