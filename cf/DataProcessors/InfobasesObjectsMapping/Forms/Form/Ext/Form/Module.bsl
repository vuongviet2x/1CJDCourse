...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	NewListItem = ChoiceList.Add("AllObjects", NStr("ru = 'Все данные';
																	|en = 'All data';"));
	MappingStatusFilterOptions.Insert(NewListItem.Value, New FixedStructure);
	
	NewListItem = ChoiceList.Add("UnapprovedMappedObjects", NStr("ru = 'Изменения';
																							|en = 'Changes';"));
	MappingStatusFilterOptions.Insert(NewListItem.Value, 
						New FixedStructure("MappingStatus",  3));
	
	NewListItem = ChoiceList.Add("MappedObjects", NStr("ru = 'Сопоставленные данные';
																			|en = 'Mapped data';"));
	MappingStatusFilterOptions.Insert(NewListItem.Value, 
						New FixedStructure("MappingStatusAdditional", 0));
	
	NewListItem = ChoiceList.Add("UnmappedObjects", NStr("ru = 'Несопоставленные данные';
																				|en = 'Unmapped data';"));
	MappingStatusFilterOptions.Insert(NewListItem.Value, 
						New FixedStructure("MappingStatusAdditional", 1));
	
	NewListItem = ChoiceList.Add("UnmappedDestinationObjects", NStr("ru = 'Несопоставленные данные этой базы';
																						|en = 'Unmapped data (this infobase)';"));
	MappingStatusFilterOptions.Insert(NewListItem.Value, 
						New FixedStructure("MappingStatus",  1));
	
	NewListItem = ChoiceList.Add("UnmappedSourceObjects", NStr("ru = 'Несопоставленные данные второй базы';
																						|en = 'Unmapped data in the peer infobase';"));
	MappingStatusFilterOptions.Insert(NewListItem.Value, 
						New FixedStructure("MappingStatus", -1));
	...
	Title = NStr("ru = 'Сопоставление данных ""[DataPresentation]""';
					|en = 'Data mapping ""[DataPresentation]""';");
	...
	CurrentApplicationDescription = ?(IsBlankString(CurrentApplicationDescription), NStr("ru = 'Эта программа';
																				|en = 'This application';"), CurrentApplicationDescription);
	
	SecondApplicationDescription = String(Object.InfobaseNode);
	SecondApplicationDescription = ?(IsBlankString(SecondApplicationDescription), NStr("ru = 'Вторая программа';
																					|en = 'Other application';"), SecondApplicationDescription);
...
Procedure RunDataImport(Command)
	NString = NStr("ru = 'Получить данные в информационную базу?';
					|en = 'Do you want to import data into the infobase?';");
...
Procedure CancelMapping(Command)
	...
	If HasMappingByRef Then
		...
		Title = NStr("ru = 'Отменить сопоставление';
						|en = 'Clear mapping';", CommonClient.DefaultLanguageCode());
...
Procedure SetConditionalAppearance()
	...
	Item.Appearance.SetParameterValue("Text", NStr("ru = 'Нет соответствия, объект будет скопирован';
																|en = 'No mapping. The object will be copied';"));
	...
	Item.Appearance.SetParameterValue("TextColor", StyleColors.InaccessibleCellTextColor);
	Item.Appearance.SetParameterValue("Text", NStr("ru = 'Нет соответствия';
																|en = 'No mapping';"));
...
Procedure RunDataImportAfterPromptToConfirmDataImport(Val QuestionResult, Val AdditionalParameters) Export
	...
	If Object.DataImportedSuccessfully Then
		NString = NStr("ru = 'Данные уже были получены. Выполнить получение данных повторно?';
						|en = 'Data is already received. Do you want to receive data again?';");
...
Procedure ExecuteDataImportAfterConfirmGettingData()
	...
	If Cancel Then
		NString = NStr("ru = 'При получении данных возникли ошибки.
							...
							|en = 'Errors occurred while receiving data.
...
&AtClient
Procedure FillInSearchFields()

	If Object.UsedFieldsList.Count() = 0 Then
		
		// For catalogs.
		AddSearchField("Description");
		AddSearchField("Code");
		AddSearchField("Owner");
		AddSearchField("Parent");
		
		// For documents and business processes
		AddSearchField("Date");
		AddSearchField("Number");
		
		// Popular search fields.
		AddSearchField("Organization");
		AddSearchField("TIN");
		AddSearchField("CRTR");
		
		If Object.UsedFieldsList.Count() = 0 
			And Object.TableFieldsList.Count() > 0 Then
			
			Value = Object.TableFieldsList[0].Value;
...
&AtClient
Procedure AddSearchField(Value)
	
	Item = Object.TableFieldsList.FindByValue(Value);
	
	If Item <> Undefined Then
		
		Item.Check = True;
		Object.UsedFieldsList.Add(Value, Value, True);
...
Procedure CheckUserFieldsFilled(Cancel, UserFields)
	...
	If UserFields.Count() = 0 Then
		...
		NString = NStr("ru = 'Укажите хотя бы одно поле для отображения';
						|en = 'Specify at least one field to display';");
	...
	ElsIf UserFields.Count() > MaxUserFields() Then
		...
		MessageString = NStr("ru = 'Уменьшите количество полей (можно выбирать не более %1 полей)';
								|en = 'Reduce the number of fields. You can select no more than %1 fields.';");
...
Procedure SetMappingInteractively()
	...
	If Not (CurrentData.MappingStatus=-1 Or CurrentData.MappingStatus=+1) Then
		...
		ShowMessageBox(, NStr("ru = 'Объекты уже сопоставлены';
										|en = 'Objects are already mapped.';"), 2);
	...
	Buttons.Add(DialogReturnCode.Yes,     NStr("ru = 'Установить';
													|en = 'Apply';"));
	...
	QueryText = NStr("ru = 'Установить соответствие между выбранными объектами?';
						|en = 'Do you want to map the selected objects?';");
...
Procedure BackgroundJobCompletion(Result, AdditionalParameters) Export
	...
	If Result = Undefined Then
		RecordError(NStr("ru = 'Фоновое задание отменено администратором.';
...
Function BackgroundJobStartAtServer(Cancel)
	...
	ExecutionParameters.BackgroundJobDescription = NStr("ru = 'Сопоставление объектов';
															|en = 'Object mapping';");
...
Procedure RecordError(DetailErrorDescription)
	...
	WriteLogEvent(
		NStr("ru = 'Помощник сопоставления объектов.Анализ данных';
			|en = 'Object mapping wizard.Data analysis';", Common.DefaultLanguageCode()),
		EventLogLevel.Error,
		,
