...
Function AllCatalogsFilterGroupTitle() Export
	Return NStr("ru = 'Все справочники';
				|en = 'All catalogs';");
EndFunction
...
Function InitializeComposer(MetadataNamesList = Undefined, LimitUsageWithFilter = False, SchemaSavingAddress = Undefined)
	...
	AutomaticallyParameter.Value = NStr("ru = 'По общим правилам';
											|en = 'By common rules';");
	
	AdditionallyParameter = Parameters.Find("RegistrationReasonAdvanced");
	AdditionallyParameter.Value = NStr("ru = 'Дополнительно';
											|en = 'Additional registration';");
	
	ParameterByRef = Parameters.Find("RegistrationReasonByRef");
	ParameterByRef.Value = NStr("ru = 'По ссылке';
									|en = 'By reference';");
...
Procedure AddTabularSectionCompositionAdditionalSets(SourceItems, AddingOptions)
	...
	If ObjectMetadata = Undefined Then
		Raise StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Некорректное имя метаданных ""%1"" для регистрации на узле ""%2""';
																						|en = 'Invalid metadata name ""%1"" for registration at node ""%2"".';"),
...
Function AdditionalParameterText()
	...
	If ExportOption = 0 Then
		// All automatic data.
		Return NStr("ru = 'Без дополнительных данных.';
					|en = 'No additional data.';");
