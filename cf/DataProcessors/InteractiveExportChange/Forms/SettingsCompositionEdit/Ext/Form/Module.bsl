...
Procedure SettingVariantsBeforeDeleteRow(Item, Cancel)
	...
	QueryText   = NStr("ru = 'Удалить настройку ""%1""?';
							|en = 'Do you want to delete setting ""%1""?';");
...
Procedure SaveSetting(Command)
	...
	If IsBlankString(CurrentSettingsItemPresentation) Then
		CommonClient.MessageToUser(
			NStr("ru = 'Не заполнено имя для текущей настройки.';
				|en = 'Please enter the setting name.';"), , "CurrentSettingsItemPresentation");
		Return;
	EndIf;
	...
	If SettingVariants.FindByValue(CurrentSettingsItemPresentation)<>Undefined Then
		TitleText = NStr("ru = 'Подтверждение';
								|en = 'Confirm operation';");
		QueryText   = NStr("ru = 'Перезаписать существующую настройку ""%1""?';
								|en = 'Do you want to overwrite setting ""%1""?';");
