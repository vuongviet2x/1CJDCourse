...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If TypeOf(Parameters.ContactInformationList) = Type("Array") Then
		...
		EndDo;
		Title = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'История изменений (%1)';
																				|en = 'Change history (%1)';"), ContactInformationPresentation);
...
Procedure HistoryBeforeDeleteRow(Item, Cancel)
	...
	If Not Cancel Then
		...
		ShowQueryBox(Notification, NStr("ru = 'Удалить адрес действующий с';
										|en = 'Do you want to remove address registered on';") + " " + Format(ValidFrom, "DLF=DD")+ "?", QuestionDialogMode.YesNo);
	EndIf;
	Cancel = True;
EndProcedure
...
Procedure GenerateData(EnterNewAddress = False)
	...
	For IndexOf = 0 To History.Count() - 1 Do
		...
		Else
			CommonClient.MessageToUser(NStr("ru = 'Не допускается ввод адресов с одинаковыми датами.';
															|en = 'You cannot enter addresses with the same date.';"),, 
	...
	If Not EnterNewAddress And NoInitialDate Then
		ShowMessageBox(, NStr("ru = 'Отсутствует адрес, действующий с даты начала ведения учета.';
										|en = 'An address valid on the accounting start date is required.';"));
		Return;
	EndIf;
...
Procedure SetConditionalAppearance()
	...
	Item.Appearance.SetParameterValue("Text", NStr("ru = 'Начальное значение';
																|en = 'Default value';"));
