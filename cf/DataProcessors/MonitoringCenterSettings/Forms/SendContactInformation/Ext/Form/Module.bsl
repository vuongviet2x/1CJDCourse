...
Function FilledCorrectly1()
	...
	If OnRequest Then
		If IsBlankString(Contacts)Then
			CommonClient.MessageToUser(NStr("ru = 'Не указана контактная информация.';
															|en = 'Contact information is not specified.';"),,"Contacts");
			CheckResult = False;
		EndIf; 
	Else 
		If IsBlankString(Contacts)Then
		...
		If IsBlankString(Comment)Then
			CommonClient.MessageToUser(NStr("ru = 'Не заполнен комментарий.';
															|en = 'Comment is not filled in.';"),,"Comment");
			CheckResult = False;
		EndIf; 
	EndIf;                	
	Return CheckResult;		
