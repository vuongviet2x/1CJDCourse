...
#If WebClient Or MobileClient Then
	ShowMessageBox(, NStr("ru = 'Действие недоступно в веб-клиенте или в мобильном клиенте.
...
#EndIf
	...
	If Not ValueIsFilled(BusinessProcess) Then
		CommonClient.MessageToUser(
			NStr("ru = 'Укажите бизнес-процесс.';
				|en = 'Specify a business process.';"),,
			"BusinessProcess");
		Return;
	EndIf;
	...
	If CurItem = Undefined 
		...
		CommonClient.MessageToUser(
			NStr("ru = 'Для просмотра списка задач выберите точку действия или вложенный бизнес-процесс карты маршрута.';
				|en = 'To view the task list, select an action point or a nested business process of the flowchart.';"),,
			"Flowchart");
		Return;
	EndIf;

	FormCaption = NStr("ru = 'Задачи по точке маршрута бизнес-процесса';
							|en = 'Business process route point tasks';");
