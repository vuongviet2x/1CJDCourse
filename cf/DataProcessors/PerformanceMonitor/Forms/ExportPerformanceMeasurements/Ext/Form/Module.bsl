...
Procedure SelectExportDirectorySuggested(FileSystemExtensionAttached1, AdditionalParameters) Export
	...
	If FileSystemExtensionAttached1 Then
		...
		SelectingFile.Title = NStr("ru = 'Выбор каталога экспорта';
...
Procedure ExportPerformanceTestFile(Command)
    ...
    If Not ValueIsFilled(ExportDirectory) Then
        ...
        UserMessage.Text = NStr("ru = 'Значение параметра ""Каталог экспорта"" не заполнено.
			|Экспорт невозможен.';
			|en = 'Export directory is required.
			|Cannot export.';");
        UserMessage.Message();
    ...
    Else
        UserMessage = New UserMessage;
        UserMessage.Text = NStr("ru = 'За указанный период нет замеров. Файл архива не сформирован.';
											|en = 'There are no samples for the period. The archive was not generated.';") + Chars.LF;
        UserMessage.Message();
    EndIf;
