...
Procedure EvaluateResponseTimeThreshold(Command)                             
	...
	If CheckResult Then
		...
		APDEXScoreChart.PlotArea.ValuesScale.TitleText = NStr("ru = 'Количество замеров, шт';
...
Function FillChecking()
	...
	If KeyOperation.IsEmpty() Then
		Message = New UserMessage;
		Message.Text = NStr("ru = 'Не указана ключевая операция.';
								|en = 'A key operation is required.';");
	...
	If Not ValueIsFilled(CurrentAPDEX) Then
		Message = New UserMessage;
		Message.Text = NStr("ru = 'Не указан текущий APDEX.';
								|en = 'The current Apdex score is required.';");
	...
	If Not ValueIsFilled(Period.StartDate) Then
		Message = New UserMessage;
		Message.Text = NStr("ru = 'Не указана дата начала периода.';
	...
	If Not ValueIsFilled(Period.EndDate) Then
		Message = New UserMessage;
		Message.Text = NStr("ru = 'Не указана дата окончания периода.';
...
Function CalculateResponseTimeThresholdAtServer(PickingParameters)
	...
	Else
		CalculationResult1.Insert("ErrorDescription", NStr("ru = 'Не удалось получить данные о замерах, попробуйте изменить настройки.';
														|en = 'No metrics have been received. Try to change Apdex settings.';"));
	...
	If CalculationResult1.MeasurementsCount = 0 Then
		CalculationResult1.Insert("ErrorDescription",
			PerformanceMonitorClientServer.SubstituteParametersToString(
				NStr("ru = 'Получено 0 замеров по ключевой операции %1. Измените период или выберите другую ключевую операцию.';
					|en = 'No samples have been received for the %1 key operation. Change the period or choose another key operation.';"),
