...
Procedure GetConnectionConfirmation(Command)
	...
	If Not DataExchangeWithExternalSystem
		...
		ShowMessageBox(, NStr("ru = 'Подключение подтверждено.';
										|en = 'The connection is confirmed.';"));
		Return;
	EndIf;
...
Function CorrespondentSetupIsComplete(ExchangeNode, TransportKind)
	...
	If TransportKind = Enums.ExchangeMessagesTransportTypes.WS Then
		...
		If Not HasConnection Then
			...
			MessageTemplate = NStr("ru = 'Не удалось подключиться к приложению %1, по причине ""%2"".
				|Убедитесь, что:
				...
				|en = 'Cannot connect to the %1 application. Reason: ""%2"".
				|Ensure that:
		...
		ElsIf Not CheckData.SettingCompleted Then
			...
			MessageTemplate = NStr("ru = 'Для продолжения перейдите в программу ""%1"" и завершите в ней настройку синхронизации. 
	...
	Else
		...
		If ExternalConnectionExchangePlanNode = Undefined Then
			...
			MessageTemplate = NStr("ru = 'Ошибка. В программе ""%1"" не найдена настройка синхронизации для этой программы.
					|Выполнение обмена данными отменено.';
					|en = 'Error. In the ""%1"" application, the synchronization setting for this application is not found.
					|The data exchange is canceled.';", Common.DefaultLanguageCode());
			CheckData.ErrorMessage = StrTemplate(MessageTemplate, PeerInfobaseName);
...
Procedure SetDataSendingAndReceivingRulesFollowUp(ClosingResult, AdditionalParameters) Export
	...
	If SetupExecuted Then
		ShowMessageBox(, NStr("ru = 'Настройка правил отправки и получения данных выполнена.';
										|en = 'The rules for sending and receiving data are configured.';"));
		...
		Else
			Title = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Настройка синхронизации данных с ""%1""';
					|en = 'Configure data synchronization with %1';"),
...
Procedure SetInitialFormItemsView()
	...
	If Backup Then
		Items.BackupLabelDecoration.Title = StringFunctions.FormattedString(
			NStr("ru = 'Перед началом настройки новой синхронизации данных рекомендуется <a href=""%1"">создать резервную копию данных</a>.';
				|en = 'It is recommend that you <a href=""%1"">back up your data</a> before you start setting up a new data sync.';"),
			BackupDataProcessorURL);
	EndIf;
