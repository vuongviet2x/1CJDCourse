...
Procedure BeforeClose(Cancel, Exit, WarningText, StandardProcessing)
	...
	If Items.PanelMain.CurrentPage = Items.PageWait Then
		...
		WarningText = NStr("ru = 'Отменить перенумерацию?';
									|en = 'Undo renumbering?';");
...
Function SetExclusiveModeAtServer()
	...
	Else
		...
		Items.ErrorDecoration.Title = NStr("ru = 'Невозможно изменить префикс, т.к. с программой работают другие пользователи:';
													|en = 'Couldn''t change the prefix. There are active user sessions:';");
		Items.ActiveUsers.Title = StringFunctions.FormattedString(NStr("ru = 'Активные пользователи (%1)';
																								|en = 'Active users (%1)';"), ActiveSessionCount);
		Items.DecorationExplanationOnError.Title = NStr("ru = 'Для продолжения необходимо завершить их работу.';
...
Function StartIBPrefixChangeInBackgroundJob()
	...
	ExecutionParameters.BackgroundJobDescription = NStr("ru = 'Изменение префикса';
															|en = 'Change prefix';");
