...
Procedure BeforeClose(Cancel, Exit, WarningText, StandardProcessing)
	...
	WarningText = NStr("ru = 'Прервать настройку параметров подключения для синхронизации данных?';
								|en = 'Do you want to discard the connection parameters for data synchronization?';");
...
Procedure ConnectionSettingsFileNameToImportStartChoice(Item, ChoiceData, StandardProcessing)
	...
	DialogSettings.Insert("Title", NStr("ru = 'Выберите файл с настройками подключения';
												|en = 'Select connection settings file';"));
	DialogSettings.Insert("Filter",    NStr("ru = 'Файл настроек подключения (*.xml)';
												|en = 'Connection settings file (*.xml)';") + "|*.xml");
...
Procedure XDTOCorrespondentSettingsFileNameStartChoice(Item, ChoiceData, StandardProcessing)
	...
	DialogSettings = New Structure;
	DialogSettings.Insert("Title", NStr("ru = 'Выберите файл с настройками корреспондента';
												|en = 'Select peer settings file';"));
	DialogSettings.Insert("Filter",    NStr("ru = 'Файл настроек корреспондента (*.xml)';
												|en = 'Peer settings file (*.xml)';") + "|*.xml");
...
Procedure ConnectionSettingsFileNameToExportStartChoice(Item, ChoiceData, StandardProcessing)
	...
	DialogSettings.Insert("Title", NStr("ru = 'Укажите файл для сохранения настроек подключения';
												|en = 'Select file to save connection settings';"));
	DialogSettings.Insert("Filter",    NStr("ru = 'Файл настроек подключения (*.xml)';
												|en = 'Connection settings file (*.xml)';") + "|*.xml");
	...
	ElsIf ConnectionKind = "PassiveMode" Then
		...
		If IsBlankString(XDTOCorrespondentSettingsFileName) Then
			CommonClient.MessageToUser(
				NStr("ru = 'Выберите файл с настройками программы-корреспондента.';
					|en = 'Please select a file with peer application settings.';"),
...
Procedure OnCompleteConnectionTestOnline()
	...
	Else
		...
		Else
			CommonClient.MessageToUser(
				NStr("ru = 'Не удалось подключиться к программе. Проверьте настройки подключения.';
					|en = 'Cannot connect to the application. Please check the connection settings.';"));
		EndIf;
	EndIf;
		...
		Else
			CommonClient.MessageToUser(
				StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Не удалось подключиться к программе. Проверьте настройки подключения %1.';
						|en = 'Cannot connect to the application. Please check the connection settings %1.';"), TransportKindToCheck));
		EndIf;
...
Procedure GetConnectionSettingsFileCompletion(ObtainedFiles, AdditionalParameters) Export
	...
	If ObtainedFiles = Undefined Then
		Result.Cancel = True;
		Result.ErrorMessage = NStr("ru = 'Не удалось сохранить настройки подключения в файл.';
											|en = 'Couldn''t save the connection settings to a file.';");
	EndIf;
...
Procedure SaveConnectionSettingsCompletion(Result, AdditionalParameters) Export
	...
	Else
		...
		If IsBlankString(MessageText) Then
			MessageText = NStr("ru = 'Не удалось сохранить настройки подключения.';
									|en = 'Couldn''t save the connection settings.';");
		EndIf;
...
Procedure OnStartSaveConnectionSettingsAtServer(ContinueWait)
	...
	If IsExchangeWithApplicationInService Then
		...
		ConnectionSettings.PeerInfobaseName = PeerInfobaseName;
		...
		If XDTOSetup Then
			...
			ConnectionSettings.XDTOCorrespondentSettings.SupportedObjects = SupportedPeerInfobaseFormatObjects;
		EndIf;
...
Procedure FillWizardConnectionParametersStructure(WizardSettingsStructure, WithoutCorrespondent = False)
	...
	WizardSettingsStructure.Insert("SecondInfobaseDescription", PeerInfobaseName);
	...
	WizardSettingsStructure.Insert("SupportedObjectsInFormat", SupportedPeerInfobaseFormatObjects);
...
Procedure ReadWizardConnectionParametersStructure(WizardSettingsStructure)
	...
	If CorrespondentDescriptionChangeAvailable Then
		PeerInfobaseName = WizardSettingsStructure.SecondInfobaseDescription;
	EndIf;
...
Procedure FillCorrespondentParameters(CorrespondentParameters, CorrespondentInSaaS = False)
	...
	If Not CorrespondentInSaaS Then
		If ValueIsFilled(CorrespondentParameters.InfobaseDescription) Then
			PeerInfobaseName = CorrespondentParameters.InfobaseDescription;
		Else
			PeerInfobaseName = CorrespondentParameters.DefaultInfobaseDescription;
		EndIf;
	EndIf;
	...
	If XDTOSetup Then
		...
		SupportedPeerInfobaseFormatObjects = New ValueStorage(
...
Procedure FillXDTOCorrespondentSettingsFromXMLAtServer(AddressInStorage, Cancel, ErrorMessage = "")
	...
	If Not Cancel Then
		...
		SupportedPeerInfobaseFormatObjects = New ValueStorage(XDTOCorrespondentSettings.SupportedObjects,
			New Deflation(9));
...
Procedure OnCompletePutConnectionSettingsFileForImport(FileThatWasPut, AdditionalParameters) Export
	...
	If Result.Cancel Then
		If IsBlankString(Result.ErrorMessage) Then
			Result.ErrorMessage = NStr("ru = 'Не удалось загрузить файл с настройками подключения.';
												|en = 'Cannot load the connection settings file.';");
		EndIf;
	Else
...
Procedure OnCompletePutXDTOCorrespondentSettingsFile(FileThatWasPut, AdditionalParameters) Export
	...
	If Result.Cancel Then
		If IsBlankString(Result.ErrorMessage) Then
			Result.ErrorMessage = NStr("ru = 'Не удалось загрузить файл с настройками корреспондента.';
												|en = 'Cannot load the file with peer application settings.';");
		EndIf;
	EndIf;
...
Procedure InitializeFormProperties()
	...
	If DIBSetup Then
		Title = NStr("ru = 'Настройка распределенной информационной базы';
						|en = 'Configure distributed infobase';");
	Else
		Title = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Настройка подключения к ""%1""';
				|en = 'Configure connection to %1';"),
...
Procedure InitializeFormAttributes()
	...
	Else
		...
		PeerInfobaseName = CorrespondentConfigurationDescription;
	EndIf;
...
Function Attachable_ConnectionSetupMethodPageOnGoNext(Cancel)
	...
	If ConnectionSetupOptionInService = "InternetAuto" 
		...
		If CurrentData = Undefined Then
			...
			CommonClient.MessageToUser(
				NStr("ru = 'Выберите приложение из списка для продолжения настройки подключения.';
					|en = 'To continue the connection setup, select an application.';"),
		...
		Else
			...
			PeerInfobaseName   = CurrentData.ApplicationDescription;
	...
	ElsIf ConnectionSetupOptionInService = "InternetManually" Then
		...
		If ConnectionKind = "RegularCommunicationChannels"
			...
			If IsBlankString(ConnectionSettingsFileNameToImport) Then
				CommonClient.MessageToUser(
					NStr("ru = 'Выберите файл с настройками подключения.';
						|en = 'Please select a file with connection settings.';"),
...
Function Attachable_InternetConnectionParametersPageOnGoNext(Cancel)
	...
	If IsBlankString(InternetWebAddress) And ConnectionSetupOptionInService <> "InternetAuto" Then
		CommonClient.MessageToUser(
			NStr("ru = 'Укажите адрес программы в Интернет.';
				|en = 'Please enter the web application address.';"),
...
Function Attachable_ConnectionParametersExternalConnectionPageOnGoNext(Cancel)
	...
	If ExternalConnectionInfobaseOperationMode = "File" Then
		...
		If IsBlankString(ExternalConnectionInfobaseDirectory) Then
			CommonClient.MessageToUser(
				NStr("ru = 'Выберите каталог расположения информационной базы.';
					|en = 'Please select an infobase directory.';"),
	...
	ElsIf ExternalConnectionInfobaseOperationMode = "ClientServer1" Then
		...
		If IsBlankString(ExternalConnectionServerCluster) Then
			CommonClient.MessageToUser(
				NStr("ru = 'Укажите имя кластера серверов 1С:Предприятия.';
					|en = 'Please specify a 1C:Enterprise server cluster name.';"),
		...
		If IsBlankString(ExternalConnectionInfobaseName) Then
			CommonClient.MessageToUser(
				NStr("ru = 'Укажите имя информационной базы в кластере серверов 1С:Предприятия.';
					|en = 'Please specify an infobase name in 1C:Enterprise server cluster.';"),
...
Function Attachable_CommonCommunicationChannelsConnectionParametersPageOnGoNext(Cancel)
	...
	If Not RegularCommunicationChannelsFILEUsage
		...
		CommonClient.MessageToUser(
			NStr("ru = 'Выберите хотя бы один способ передачи файлов с данными.';
				|en = 'Please select at least one method of transferring data files.';"),
	...
	If RegularCommunicationChannelsArchiveFiles
		...
		CommonClient.MessageToUser(
			NStr("ru = 'Укажите пароль для архивации файлов.';
				|en = 'Please enter the archive password.';"),
	...
	If RegularCommunicationChannelsFILEUsage Then
		...
		If IsBlankString(RegularCommunicationChannelsFILEDirectory) Then
			CommonClient.MessageToUser(
				NStr("ru = 'Выберите каталог для передачи файлов с данными.';
					|en = 'Please select a file transfer directory.';"),
	...
	If RegularCommunicationChannelsFTPUsage Then
		...
		If IsBlankString(RegularCommunicationChannelsFTPPath) Then
			CommonClient.MessageToUser(
				NStr("ru = 'Укажите адрес каталога для передачи файлов с данными.';
					|en = 'Please specify a file transfer directory.';"),
		...
		If RegularCommunicationChannelsFTPEnableFileSizeLimit
			...
			CommonClient.MessageToUser(
				NStr("ru = 'Укажите максимальный допустимый размер файлов с данными.';
					|en = 'Please specify the file size limit.';"),
	...
	If RegularCommunicationChannelsEMAILUsage Then
		...
		If Not ValueIsFilled(RegularCommunicationChannelsMAILUserAccount) Then
			CommonClient.MessageToUser(
				NStr("ru = 'Выберите учетную запись электронной почты для отправки сообщений с данными.';
					|en = 'Please select an email account.';"),
		...
		If RegularCommunicationChannelsEMAILEnableAttachmentSizeLimit
			...
			CommonClient.MessageToUser(
				NStr("ru = 'Укажите максимальный допустимый размер почтового вложения.';
					|en = 'Please specify the attachment size limit.';"),
...
Function Attachable_GeneralSynchronizationSettingsPageOnOpen(Cancel, SkipPage, IsMoveNext)
	...
	If ThisInfobaseHasPeerInfobaseNode Or ThisNodeExistsInPeerInfobase Then
		...
		If ThisNodeExistsInPeerInfobase Then
			ToolTipText = NStr("ru = '- Новый код для текущего узла информационной базы';
									|en = '- New code for the current infobase node';");
			Hints.Add(ToolTipText);
		EndIf;
		
		If ThisInfobaseHasPeerInfobaseNode Then
			ToolTipText = NStr("ru = '- Удаление узла ""%1""';
									|en = '- Deleting the ""%1"" node';");
...
Function Attachable_GeneralSynchronizationSettingsPageOnGoNext(Cancel)
	...
	If Not ConnectionKind = "PassiveMode" Then
		...
		If DescriptionChangeAvailable
			...
			CommonClient.MessageToUser(
				NStr("ru = 'Укажите наименование этой программы.';
					|en = 'Please specify the description of this application.';"),
				, "Description", , Cancel);
		EndIf;
		...
		If PrefixChangeAvailable
			...
			CommonClient.MessageToUser(
				NStr("ru = 'Укажите префикс этой программы.';
					|en = 'Please specify the prefix for this application.';"),
				, "Prefix", , Cancel);
		EndIf;
	...
	If CorrespondentDescriptionChangeAvailable
		And IsBlankString(PeerInfobaseName) Then
		CommonClient.MessageToUser(
			NStr("ru = 'Укажите наименование программы-корреспондента.';
				|en = 'Please specify the description of the peer application.';"),
			, "PeerInfobaseName", , Cancel);
	EndIf;
	...
	If Not ConnectionKind = "PassiveMode" Then
		...
		If CorrespondentPrefixChangeAvailable
			...
			CommonClient.MessageToUser(
				NStr("ru = 'Укажите префикс программы-корреспондента.';
					|en = 'Please specify the peer application prefix.';"),
	...
	If SaveConnectionParametersToFile
		...
		CommonClient.MessageToUser(
			NStr("ru = 'Укажите путь к файлу для сохранения настроек подключения.';
				|en = 'Please specify the path for saving the connection settings file.';"),
