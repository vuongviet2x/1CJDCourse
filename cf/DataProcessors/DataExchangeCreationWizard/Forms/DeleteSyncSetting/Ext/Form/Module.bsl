...
Procedure BeforeClose(Cancel, Exit, WarningText, StandardProcessing)
	...
	WarningText = NStr("ru = 'Отказаться от удаления настройки синхронизации данных?';
								|en = 'Do you want to cancel the deletion of the data synchronization setting?';");
	...
	Else
		...
		Else
			CommonClient.MessageToUser(
				NStr("ru = 'Не удалось удалить настройку синхронизации данных.';
					|en = 'Cannot delete the data synchronization setting.';"));
		EndIf;
	EndIf;
...
Function Attachable_BeginningPageOnOpen1(Cancel, SkipPage, IsMoveNext)
	...
	If IsExchangeWithExternalSystem Then
		Items.DeleteSettingItemInCorrespondent.Title = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Удалить настройку обмена с системой ""%1"" на Портале 1С:ИТС.';
				|en = 'Delete exchange with %1 at 1C:ITS Portal';"),
			PeerInfobaseName);
	Else
		Items.DeleteSettingItemInCorrespondent.Title = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Удалить настройку также в программе ""%1"".';
				|en = 'Also delete the setting in %1';"),
			PeerInfobaseName);
	EndIf;
