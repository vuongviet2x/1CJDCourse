...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If IsBlankString(Details) Then
		Title = StrReplace(NStr("ru = 'Регистрация %1';
									|en = 'Register %1';"), "%1", CurrentObject.RepresentationOfTheReference(ObjectAsString));
	Else
		Title = StrReplace(NStr("ru = 'Регистрация %1 (%2)';
									|en = 'Register %1 (%2)';"), "%1", CurrentObject.RepresentationOfTheReference(ObjectAsString));
...
Procedure EditRegistration(Command)
	...
	Text = NStr("ru = 'Изменить регистрацию ""%1""
	             |на узлах?';
				|en = 'Do you want to change registration state
				|of %1 at all nodes?';");
...
Procedure EditRegistrationCompletion(Val QuestionResult, Val AdditionalParameters) Export
	...
	If Count > 0 Then
		Text = NStr("ru = 'Регистрация %1 была изменена на %2 узлах';
					|en = 'Registration state of %1 changed at %2 nodes.';");
		NotificationTitle = NStr("ru = 'Изменение регистрации:';
									|en = 'Registration state changed:';");
...
Procedure SetConditionalAppearance()
	...
	Item.Appearance.SetParameterValue("Text", NStr("ru = 'ДеревоУзловОбменаНомерСообщения';
																|en = 'ExchangeNodesTreeMessageNumber';"));
