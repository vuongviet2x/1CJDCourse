...
Function GenerateMetadataStructure(ExchangePlanName = Undefined) Export
	...
	CurRowNumber = 1;
	GenerateMetadataLevel(CurRowNumber, CurParameters, 1,  2,  False,   "Constants",               NStr("ru = 'Константы';
																												|en = 'Constants';"));
	GenerateMetadataLevel(CurRowNumber, CurParameters, 3,  4,  True, "Catalogs",             NStr("ru = 'Справочники';
																												|en = 'Catalogs';"));
	GenerateMetadataLevel(CurRowNumber, CurParameters, 5,  6,  True, "Sequences",      NStr("ru = 'Последовательности';
																												|en = 'Sequences';"));
	GenerateMetadataLevel(CurRowNumber, CurParameters, 7,  8,  True, "Documents",               NStr("ru = 'Документы';
																												|en = 'Documents';"));
	GenerateMetadataLevel(CurRowNumber, CurParameters, 9,  10, True, "ChartsOfCharacteristicTypes", NStr("ru = 'Планы видов характеристик';
																												|en = 'Charts of characteristic types';"));
	GenerateMetadataLevel(CurRowNumber, CurParameters, 11, 12, True, "ChartsOfAccounts",             NStr("ru = 'Планы счетов';
																												|en = 'Charts of accounts';"));
	GenerateMetadataLevel(CurRowNumber, CurParameters, 13, 14, True, "ChartsOfCalculationTypes",       NStr("ru = 'Планы видов расчета';
																												|en = 'Charts of calculation types';"));
	GenerateMetadataLevel(CurRowNumber, CurParameters, 15, 16, True, "InformationRegisters",        NStr("ru = 'Регистры сведений';
																												|en = 'Information registers';"));
	GenerateMetadataLevel(CurRowNumber, CurParameters, 17, 18, True, "AccumulationRegisters",      NStr("ru = 'Регистры накопления';
																												|en = 'Accumulation registers';"));
	GenerateMetadataLevel(CurRowNumber, CurParameters, 19, 20, True, "AccountingRegisters",     NStr("ru = 'Регистры бухгалтерии';
																												|en = 'Accounting registers';"));
	GenerateMetadataLevel(CurRowNumber, CurParameters, 21, 22, True, "CalculationRegisters",         NStr("ru = 'Регистры расчета';
																												|en = 'Calculation registers';"));
	GenerateMetadataLevel(CurRowNumber, CurParameters, 23, 24, True, "BusinessProcesses",          NStr("ru = 'Бизнес-процессы';
																												|en = 'Business processes';"));
	GenerateMetadataLevel(CurRowNumber, CurParameters, 25, 26, True, "Tasks",                  NStr("ru = 'Задачи';
																												|en = 'Tasks';"));
...
Function ExternalDataProcessorInfo() Export
	...
	Info.Insert("Description", NStr("ru = 'Регистрация изменений для обмена данными';
										|en = 'Data registration manager';"));
	Info.Insert("Version",       "1.0");
	Info.Insert("SSLVersion",    "1.2.1.4");
	Info.Insert("Information",    NStr("ru = 'Обработка для управления регистрацией объектов на узлах обмена до формирования выгрузки. При работе в составе конфигурации с БСП версии 2.1.2.0 и старше производит контроль ограничений миграции данных для узлов обмена.';
										|en = 'The data processor is intended for managing registration of objects at exchange nodes before exporting data. When used in configurations with SSL version 2.1.2.0 or later, it manages data migration restrictions.';"));
	...
	Command.Presentation = NStr("ru = 'Редактирование регистрации изменений объекта';
								|en = 'Change item registration state';");
...
Function RecordSetDimensions(TableName, AllDimensions = False) Export
	...
	If Metadata.AccumulationRegisters.Contains(Meta)
	 ...
	 Or Metadata.Sequences.Contains(Meta) Then
		...
		String.Title   = NStr("ru = 'Регистратор';
									|en = 'Recorder';");
	EndIf;
	...
	If Metadata.CalculationRegisters.Contains(Meta.Parent()) Then
		...
		String.ValueType = Documents.AllRefsType();
		String.Title   = NStr("ru = 'Объект перерасчета';
									|en = 'Recalculation object';");
	EndIf;
...
Function CheckSettingsCorrectness(SettingsKey = "") Export
	...
	Else
		...
		If Metadata.DataProcessors.Find(QueryExternalDataProcessorAddressSetting) = Undefined Then
			Text = NStr("ru = 'Обработка ""%1"" не найдена в составе конфигурации';
						|en = 'Data processor %1is not found in the configuration';");
