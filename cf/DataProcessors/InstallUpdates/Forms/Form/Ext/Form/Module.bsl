...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Not ConfigurationUpdate.HasRightsToInstallUpdate() Then
		Raise(NStr("ru = 'Недостаточно прав для обновления приложения. Обратитесь к администратору.';
								|en = 'Insufficient rights to update the application. Please contact the administrator.';"),
	...
	If IsWebClient Or IsSubordinateDIBNode Then
		SelectionOptionTitle = NStr("ru = 'Укажите файл исправлений:';
										|en = 'Specify a patch file:';");
...
Procedure PatchInstallationErrorLabelURLProcessing(Item, FormattedStringURL, StandardProcessing)
	...
	LogFilter.Insert("EventLogEvent", NStr("ru = 'Исправления.Установка';
															|en = 'Patch.Install';"));
...
Procedure RestartApplicationOnChange(Item)
	...
	If RestartApplication Then
		Items.NextButton.Title = NStr("ru = 'Перезапустить';
												|en = 'Restart';");
	Else
...
Procedure DecorationNotifyUsersURLProcessing(Item, FormattedStringURL, StandardProcessing)
	...
	ChoiceList.Add("5", NStr("ru = '5 минут';
									|en = '5 minutes';"));
	ChoiceList.Add("10", NStr("ru = '10 минут';
									|en = '10 minutes';"));
	ChoiceList.Add("15", NStr("ru = '15 минут';
									|en = '15 minutes';"));
	ChoiceList.Add("30", NStr("ru = '30 минут';
									|en = '30 minutes';"));
	...
	If Common.SubsystemExists("StandardSubsystems.AccountingAudit") Then
		...
		If Information.WarnSecondCheckRequired Then
			If ValueIsFilled(Information.LastCheckDate) Then
				ToolTip = NStr("ru = 'Последняя системная проверка выполнялась %1.';
								|en = 'Last system check was on %1.';");
...
Procedure BeforeOpenPage(Val NewPage = Undefined)
	...
	NextButton.Title = ?(NextButtonFunction, NStr("ru = 'Далее >';
														|en = 'Next >';"), NStr("ru = 'Готово';
																				|en = 'Finish';"));
	CloseButton.Title = ?(CloseButtonFunction, NStr("ru = 'Отмена';
															|en = 'Cancel';"), NStr("ru = 'Закрыть';
																					|en = 'Close';"));
...
#If WebClient Then
	...
	Properties.Title = NStr("ru = 'Выбор исправлений';
								|en = 'Select patches';");
	Properties.Filter = NStr("ru = 'Файлы исправлений (*.cfe*;*.zip)|*.cfe*;*.zip';
							|en = 'Patch files (*.cfe*;*.zip)|*.cfe*;*.zip';");
...
#Else
	...
	Else
		...
		Properties.Title = NStr("ru = 'Выбор поставки обновления конфигурации';
									|en = 'Select configuration updates';");
		
		DescriptionOfTheDialogFilter = New Array;
		
		DescriptionOfTheDialogFilter.Add(NStr("ru = 'Файлы поставки конфигурации (*.cf)|*.cf';
											|en = 'Configuration files (*.cf)|*.cf';"));
		DescriptionOfTheDialogFilter.Add(NStr("ru = 'Файлы поставки обновления конфигурации(*.cfu)|*.cfu';
											|en = 'Update files(*.cfu)|*.cfu';"));
		
		DescriptionOfTheDialogFilter.Insert(0, NStr("ru = 'Все файлы (*.cf*;*.cfu;*.cfe;*.zip)|*.cf*;*.cfu;*.cfe;*.zip';
												|en = 'All files (*.cf*;*.cfu;*.cfe;*.zip)|*.cf*;*.cfu;*.cfe;*.zip';"));
		DescriptionOfTheDialogFilter.Add(NStr("ru = 'Файлы исправлений (*.cfe*;*.zip)|*.cfe*;*.zip';
											|en = 'Patch files (*.cfe*;*.zip)|*.cfe*;*.zip';"));
...
Procedure NavigateFromUpdateFilePage()
	...
	For Each FileName In SelectedFilesNames Do
		If Not ThisIsTheFixFile(FileName) Then
			If Not IsBlankString(Object.UpdateFileName) Then
				Raise NStr("ru = 'Допустимо выбирать только один файл обновления.';
										|en = 'Please select a single update file.';");
			EndIf;
	...
	If UpdateFileRequired = 1 Then
		If Not ValueIsFilled(SelectedFiles) Then
			CommonClient.MessageToUser(NStr("ru = 'Укажите файл поставки обновления конфигурации.';
															|en = 'Please select an update file.';"),,"Object.UpdateFileName");
			CurrentItem = Items.UpdateFileField;
			Return;
		EndIf;
		If Not IsBlankString(Object.UpdateFileName) Then
			...
			If Not File.Exists() Or Not File.IsFile() Then
				CommonClient.MessageToUser(NStr("ru = 'Файл поставки обновления конфигурации не найден.';
																|en = 'The update file is not found.';"),,"Object.UpdateFileName");
				CurrentItem = Items.UpdateFileField;
				Return;
			EndIf;
		EndIf;
...
Procedure CheckUpdateFilesApplicability(Result, AdditionalParameters) Export
	...
	Status(NStr("ru = 'Проверка применимости...';
					|en = 'Checking applicability…';"), 75);
...
Procedure ContinueInstallUpdates(PlacedFiles, AdditionalParameters) Export
	...
	Items.NextButton.Title = NStr("ru = 'Готово';
											|en = 'Finish';");
	Items.CloseButton.Title = NStr("ru = 'Закрыть';
											|en = 'Close';");
	...
	If HasErrors Then
		If PatchesInstalled1 > 0 Then
			LabelText = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Установлено исправлений: %1 из %2, изменения вступят в силу после перезапуска приложения';
					|en = 'Patches installed: %1 out of %2. The changes will be applied after you restart the application.';"),
				PatchesInstalled1,
				PlacedFiles.Count());
...
Procedure ProceedToUpdateModeSelection(IsMoveNext = False)
	...
	If AdministrationParameters = Undefined Then
		...
		If IsFileInfobase Then
			NoteLabel = NStr("ru = 'Для установки обновления введите
		...
		Else
			NoteLabel = NStr("ru = 'Для установки обновления введите параметры
...
Procedure AfterGetAdministrationParameters(Result, IsMoveNext) Export
	...
	Else
		...
		WarningText = NStr("ru = 'Для установки обновления введите параметры администрирования.';
									|en = 'To install the update, enter the administration parameters.';");
		ShowMessageBox(, WarningText);
		
		MessageText = NStr("ru = 'Не удалось установить обновление приложения, т.к. введены неправильные имя или пароль администратора 
			|(или другие параметры администрирования клиент-серверной информационной базы).';
			|en = 'Cannot install the application update as the specified administrator name or password is invalid
...
Procedure AfterNotificationIntervalSelected(Result, AdditionalParameters) Export
	...
	TitleTemplate1 = NStr("ru = 'Уведомлять пользователей о планируемом обновлении за <a href=""Ссылка"">%1 минут</a>';
							|en = 'Notify users about an update <a href=""Ссылка"">%1 minutes</a> in advance';");
