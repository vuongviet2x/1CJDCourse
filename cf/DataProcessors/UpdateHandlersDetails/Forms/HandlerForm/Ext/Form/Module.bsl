...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	OrderArray.Add(NStr("ru = 'Текущий';
								|en = 'Current';"));
	OrderArray.Add(NStr("ru = 'Конфликтующий';
								|en = 'Conflicting';"));
	OrderArray.Add(NStr("ru = 'Любой';
								|en = 'Any';"));
...
Procedure BeforeClose(Cancel, Exit, WarningText, StandardProcessing)
	...
	If Modified Then
		...
		ShowQueryBox(ResponseHandler1, NStr("ru = 'Данные были изменены. Сохранить изменения?';
...
Procedure CheckProcedureStartChoice(Item, ChoiceData, StandardProcessing)
	...
	If Object.ObjectsToRead.FindRows(New Structure("LockInterface", True)).Count() > 0
		...
		MessageText = NStr("ru = 'Процедура ""%1"" не предназначена для проверки блокировки в обработчиках, которые контролируют блокировку читаемых данных или прочих объектов.';
								|en = 'Do not use the ""%1"" procedure to check locks in handlers that control locking of data to read or other objects.';");
...
Procedure CommentStartChoice(Item, ChoiceData, StandardProcessing)
	...
	CommonClient.ShowMultilineTextEditingForm(
		Notification, Items.Comment.EditText, NStr("ru = 'Служебный комментарий';
																	|en = 'Internal comment';"));
...
Procedure InfoAboutLock(Command)
	...
	WarningText = NStr("ru = '• Проверка на блокировку предыдущими очередями выполняется только для тех объектов которые зарегистрированы на плане обмена к обновлению для текущего обработчика;
		|
		...
		|en = '• Check for lock by previous queues is performed only for objects registered on the exchange plan for update for the current handler.
		|
...
Procedure AddAttributeTypesToObjectsToLock(Command)
	...
	ObjectsNames.ShowChooseItem(ChoiceHandler, NStr("ru = 'Выберите объект метаданных';
																|en = 'Select metadata object';"));
...
Function FilledCorrectly()
	...
	If CurrentHandler.ExecutionMode = "Deferred" 
		...
		MessageText = NStr("ru = 'Отложенный обработчик не может быть записан с пустой табличной частью ""Изменяемые объекты""';
								|en = 'Couldn''t save the deferred handler. ""Objects to change"" is empty.';");
...
#EndIf
			MessageText = NStr("ru = 'Не удалось распознать вид объекта метаданных ""%1""
				|Вид объекта должен быть во множественном числе.';
				|en = 'Cannot recognize the ""%1"" metadata object kind.
...
Procedure ImportLowPriorityReading(LowPriorityReading)
	...
	If LowPriorityReading.Count() > 0 Then
		...
		For Each Readable In Object.ObjectsToRead Do
			...
			EndIf;
		EndDo;
		MessageText = NStr("ru = 'Среди читаемых объектов обработчика имеются объекты, которые обрабатываются обработчиками с более низким приоритетом чем текущий.
		|Это приведет к тому что текущий обработчик будет ждать их завершения. Необходимо устранить это несоответствие.';
		|en = 'Readable handler objects include objects that are processed by handlers with a lower priority than the current one.
...
Procedure AddAttributeTypes(TableName, ObjectName = "")
	...
	If IsBlankString(ObjectName) Then
		If Items[TableName].CurrentData = Undefined Then
			ShowMessageBox(,NStr("ru = 'Не выбран объект метаданных';
										|en = 'Metadata object is not selected';"));
			Return;
		EndIf;
...
Procedure SelectTheMetadataObjectSDetails(Attributes, TableName, ObjectName)
	...
	Attributes.ShowChooseItem(ChoiceHandler, NStr("ru = 'Выберите реквизит объекта';
															|en = 'Select an object attribute';"));
...
Function TextWarningChangedCheckProcedure(DoQueryBox = False)
	...
	TextHat = NStr("ru = 'Использование своей процедуры проверки, отличной от стандартной, допустимо
		|только в крайних случаях, когда ее возможностей недостаточно.';
		|en = 'Use a custom check procedure responsibly,
		|when the standard check procedure is insufficient.';");
	TextHat = StrConcat(StrSplit(TextHat, Chars.LF), " ");
	
	TextContinued = NStr("ru = 'При этом необходимо соблюдать следующие правила при ее написании:
		...
		|en = 'Follow these development recommendations:
	...
	If DoQueryBox Then
		WarningText = WarningText + Chars.LF + Chars.LF + NStr("ru = 'Продолжить?';
