...
#If WebClient Then
	Raise NStr("ru = 'Работа в web-клиенте не поддерживается.';
							|en = 'Web client usage is not supported.';");
#EndIf
...
Procedure BeforeClose(Cancel, Exit, WarningText, StandardProcessing)
	...
	If Modified And Not Exit Then
		...
		ShowQueryBox(ResponseHandler1, NStr("ru = 'Данные были изменены. Сохранить изменения в репозиторий?';
												|en = 'The data has been changed. Do you want to save the changes to the repository?';"), QuestionDialogMode.YesNoCancel);
	EndIf;
...
#If Not WebClient Then
		...
		FileSystemClient.SelectDirectory(FileSelectionHandler, NStr("ru = 'Укажите каталог SRC';
																		|en = 'Select a SRC directory';"), Object.SRCDirectory);
...
#If Not WebClient Then
		...
		FileSystemClient.SelectDirectory(FileSelectionHandler, NStr("ru = 'Укажите каталог файлов ошибок';
																		|en = 'Specify an error files directory';"), ErrorFileDirectory);
#EndIf
EndProcedure
...
Procedure UpdateHandlersBeforeDeleteRow(Item, Cancel)
	...
	QueryText = NStr("ru = 'Удалить описание обработчика?';
						|en = 'Do you want to delete handler details?';");
...
Procedure SaveAllHandlersToRepository(Command)
	...
	QueryText = NStr("ru = 'Внимание! Будут перезаписаны все процедуры описания обработчиков обновления.
						|Продолжить?';
						|en = 'Warning. All update handler details procedures will be overwritten.
						|Continue?';");
	ShowQueryBox(ResponseHandler1, QueryText, QuestionDialogMode.YesNo);
...
Procedure SetBuildNumberForHandlers(Command)
	...
	ShowInputNumber(ResponseHandler1, Number(VersionNumbers[3])+1, NStr("ru = 'Новый номер сборки';
																		|en = 'New build number';"));
...
#If Not WebClient Then
		...
		FileDialog.Title = NStr("ru = 'Укажите имя файла';
											|en = 'Specify a file name';");
...
#If Not WebClient Then
		...
		ImportParameters.Dialog.Title = NStr("ru = 'Укажите файл';
													|en = 'Select file';");
...
Procedure ShowPrioritiesHandlerConflicts(Command)
	...
	If Items.UpdateHandlers.CurrentData = Undefined Then
		ShowMessageBox(,NStr("ru = 'Не выбран обработчики обновления';
									|en = 'Update handler is not selected';"));
		Return;
	EndIf;
	...
	If Command <> Undefined Then
		...
		If Not ValueIsFilled(ErrorFileDirectory) Then
			CommonClient.MessageToUser(
				NStr("ru = 'Не указан каталог файлов ошибок.';
					|en = 'Error files directory is not specified.';"),
				,
				,
				"ErrorFileDirectory");
				Return;
...
Function TheSRCDirectoryIsSpecifiedCorrectly()
	...
	If Not ValueIsFilled(Object.SRCDirectory) Then
		CommonClient.MessageToUser(
			NStr("ru = 'Не указан путь к каталогу SRC';
				|en = 'SRC directory path is not specified';"),
			,
			,
			"Object.SRCDirectory");
		Result = False;
	...
	If Not DirectoryOnHardDrive.Exists() Then
		CommonClient.MessageToUser(
			NStr("ru = 'Не найден каталог SRC';
				|en = 'SRC directory is not found';"),
			,
			,
			"Object.SRCDirectory");
		Result = False;
...
Function AddModuleProcedure(Code)
	...
	If IsManagerModule And ModuleStrings.Count() = 0 Then
		...
		Template = NStr("ru = 'В модуле ""%1"" добавлена процедура ""%2""';
						|en = 'The ""%2"" procedure is added to the ""%1"" module';");
...
#EndIf
	...
	ProcedureName = ProcedureName(Code.ProcedureTitle);
	If VersionSet Then
		Template = NStr("ru = 'В модуле ""%1"" установлена новая версия ""%2""';
						|en = 'In the ""%1"" module, new version ""%2"" is installed';");
	...
	EndIf;
	If WasReplaced Then
		Template = NStr("ru = 'В модуле ""%1"" заменена процедура ""%2""';
						|en = 'In the ""%1"" module, the ""%2"" procedure is replaced';");
...
#If Not WebClient Then
	...
	If VersionSet Then
		Template = NStr("ru = 'Установлена новая версия ""%1"" в корне конфигурации';
						|en = 'New version ""%1"" is installed in the configuration root';");
...
Procedure FillQuickFilters(Val HandlersDetails = Undefined)
	...
	AddQuickFilter(HandlersDetails, RowsArray, "ExecutionMode", NStr("ru = 'Отложенно';
																					|en = 'Deferred';"), "Deferred");
	AddQuickFilter(HandlersDetails, RowsArray, "ExecutionMode", NStr("ru = 'Монопольно';
																					|en = 'Exclusive';"), "Exclusively");
	AddQuickFilter(HandlersDetails, RowsArray, "ExecutionMode", NStr("ru = 'Оперативно';
																					|en = 'Real-time';"), "Seamless");
	AddQuickFilter(HandlersDetails, RowsArray, "TechnicalDesign", NStr("ru = 'Тех.проект';
																						|en = 'Technical design';"), "TechnicalDesign");
	
	AddAnotherQuickFilter(HandlersDetails, "DeferredHandlersExecutionMode", NStr("ru = 'Параллельно';
																									|en = 'Parallel';"), "Parallel");
	AddAnotherQuickFilter(HandlersDetails, "DeferredHandlersExecutionMode", NStr("ru = 'Последовательно';
																									|en = 'Sequentially';"), "Sequentially");
	AddAnotherQuickFilter(HandlersDetails, "InitialFilling", NStr("ru = 'Начальное заполнение';
																				|en = 'Initial population';"), "InitialFilling");
	AddAnotherQuickFilter(HandlersDetails, "ExecuteInMandatoryGroup", NStr("ru = 'Обязательный';
																							|en = 'Required';"), "IsRequired");
	AddAnotherQuickFilter(HandlersDetails, "Multithreaded", NStr("ru = 'Многопоточный';
																			|en = 'Multi-threaded';"), "Multithreaded");
	AddAnotherQuickFilter(HandlersDetails, "DataToReadWriter", NStr("ru = 'Запись читаемых';
																			|en = 'Writing of readable objects';"), "DataToReadWriter");
	AddAnotherQuickFilter(HandlersDetails, "WriteAgain", NStr("ru = 'Повторная запись';
																			|en = 'Rewriting';"), "WriteAgain");
	
	StatusText = NStr("ru = 'Прочие';
						|en = 'Other';");
	...
	StatusText = StringFunctionsClientServer.SubstituteParametersToString("%1 (%2)", NStr("ru = 'Все';
																							|en = 'All';"), HandlersDetails.Count());
	HyperlinkString = New FormattedString(
		StatusText, 
    	, 
		StyleColors.ButtonTextColor, ,
...
#EndRegion
	...
	If OnlySelectedItems And Not ExtendedFormMode Then
		HandlersCode = ModuleCodeDetails(NStr("ru = 'Выделенные обработчики';
													|en = 'Selected handlers';"));
		For Each ModuleCode In ModulesTexts Do
...
Function ReadYAMLFile(FileName)
	...
	File = New File(FileName);
	If Not File.Exists() Then
		Template = NStr("ru = 'Не обнаружен файл настроек ""%1""';
						|en = 'Settings file ""%1"" was not found';");
...
#If Not WebClient Then
	...
	Except
		...
		MessageText = Chars.LF + StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Не удалось прочитать файл %1.';
																									|en = 'Cannot read file %1.';"), FileName);
		MessageText = MessageText  + Chars.LF + NStr("ru = 'По причине';
															|en = 'Reason';")+":" + Chars.LF;
...
Procedure ReadYAMLStrings(Result, FileText, LineNumber = 0, Level = 0, ReadMultilineString = False)
	...
	Except
		ErrorText = ErrorProcessing.DetailErrorDescription(ErrorInfo());
		MessageText = Chars.LF + StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Не удалось разобрать строку: %1';
																									|en = 'Cannot parse string: %1';"), FileString);
		MessageText = MessageText  + Chars.LF + NStr("ru = 'По причине';
															|en = 'Reason';")+":" + Chars.LF;
		MessageText = MessageText  + ErrorText;
		Raise MessageText;
...
Procedure WriteError(ObjectName, ErrorDescription = "",
	...
	Error.ErrorType              = NStr("ru = 'Ошибка построения очереди отложенных обработчиков обновления';
										|en = 'An error occurred while building the queue of deferred update handlers';");
...
Function GetSettingsTexts()
	...
	SettingsTexts.Add(
		...
		NStr("ru = 'Файл cut_tags.yml сохранить в каталог .settings локальной папки проекта в кодировке UTF-8';
			|en = 'Save the cut_tags.yml file to the .settings directory of the local design folder in the UTF-8 encoding';"));
...
Procedure SetSimplifiedMode()
	...
	Items.SwitchFormMode.Title = NStr("ru = 'Перейти в расширенный режим';
													|en = 'Go to advanced mode';");
...
Procedure SetAdvancedMode()
	...
	Items.SwitchFormMode.Title = NStr("ru = 'Перейти в упрощенный режим';
													|en = 'Go to simplified mode';");
	SetFormModeItemsVisibility();
...
Procedure InitializeDataProcessorConstants()
	...
	AnalysisRequiredStatus = NStr("ru = 'Необходим анализ';
								|en = 'Analysis required';");
	StatusChangedCheckProcedure = NStr("ru = 'Изменена процедура проверки';
											|en = 'Check procedure is changed';");
	LowPriorityReadingStatus = NStr("ru = 'Чтение низкого порядка';
										|en = 'Low order read';");
...
Procedure ApplySettingsGroupAppearance()
	...
	TemplateOfPresentation = NStr("ru = 'Сохранять в %1';
								|en = 'Save to %1';");
	Receiver = Object.SRCDirectory;
	If Not ValueIsFilled(Object.SRCDirectory) Then
		Receiver = NStr("ru = '<Укажите каталог src репозитория>';
						|en = '<Specify a src directory in the repository>';");
	EndIf;
	...
	If Items.ErrorFileDirectory.Visible Then
		TemplateOfPresentation =  TemplateOfPresentation + " " + NStr("ru = 'Ошибки: %2';
																|en = 'Errors: %2';");
		ErrorsDirectory = ErrorFileDirectory;
		If Not ValueIsFilled(ErrorFileDirectory) Then
			ErrorsDirectory = NStr("ru = '<Укажите каталог ошибок>';
								|en = '<Specify an errors directory>';");
		EndIf;
...
Function FilterBackupFiles()
	Return StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Описание обработчиков (*.handlers)|*.handlers|Все файлы (%1)|%1';
																		|en = 'Handler details (*.handlers)|*.handlers|All files (%1)|%1';"), GetAllFilesMask());
EndFunction
...
Procedure SetQuickFilter(QuickFilterName)
	...
	ElsIf QuickFilterName = "Deferred" Then
		Filter = New Structure("ExecutionMode", NStr("ru = 'Отложенно';
														|en = 'Deferred';"));
		
	ElsIf QuickFilterName = "Exclusively" Then
		Filter = New Structure("ExecutionMode", NStr("ru = 'Монопольно';
														|en = 'Exclusive';"));
		
	ElsIf QuickFilterName = "Seamless" Then
		Filter = New Structure("ExecutionMode", NStr("ru = 'Оперативно';
														|en = 'Real-time';"));
		
	ElsIf QuickFilterName = "Sequentially" Then
		Filter = New Structure("DeferredHandlersExecutionMode", NStr("ru = 'Последовательно';
																				|en = 'Sequentially';"));
		
	ElsIf QuickFilterName = "Parallel" Then
		Filter = New Structure("DeferredHandlersExecutionMode", NStr("ru = 'Параллельно';
																				|en = 'Parallel';"));
