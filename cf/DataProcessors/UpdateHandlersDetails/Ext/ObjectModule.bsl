...
Function TempTablesQuery()
	...
	Query.SetParameter("NoIssues", NStr("ru = 'Нет проблем';
												|en = 'No issues';"));
...
Procedure ShiftRecursivelyLinkedHandlers(HandlersQueue, HandlersAvailabilityInQueue, Handler, MaxQueue, RecursionData)
	...
	For Each LinkedHandler In LinkedHandlers Do
		...
		Else
			ExceptionText = NStr("ru = 'Ошибка при формировании очереди.';
									|en = 'An error occurred while building the queue.';");
			Raise ExceptionText;
		EndIf;
...
Function HasQueueBuildingErrors(Query, ReportErrors = True)
	...
	While Selection.Next() Do
		MessageText = NStr("ru = 'Ошибка в алгоритме построения очереди: обработчик %Handler1% должен быть отнесен к очереди 1.';
								|en = 'An error occurred in the queue building algorithm: the %Handler1% handler is to be placed in queue 1.';");
		If Selection.Issue1 = "IssueInHandlersOrder" Then
			MessageText = NStr("ru = 'Ошибка в алгоритме построения очереди: обработчики %Handler1% и %Handler2% размещены в неправильном порядке.';
									|en = 'An error occurred in the queue building algorithm: the %Handler1% and %Handler2% handlers are placed in the incorrect order.';");
...
Function CanBuildQueue()
	...
	While EmptyPriorities.Next() Do
		MessageText = NStr("ru = 'Не завершены работы по паре обработчиков %Handler% - %LinkedHandler%.';
								|en = 'Operations with the pair of handlers %Handler% - %LinkedHandler% are not completed.';");
		MessageText = MessageText + Chars.LF + NStr("ru = 'Статус проблемы: %QueuingOrder%';
															|en = 'Issue status: %QueuingOrder%';");
		...
		MessageText = StrReplace(MessageText, "%QueuingOrder%", NStr("ru = 'Не задан приоритет выполнения.';
																						|en = 'Execution priority is not set.';"));
	...
	For Each Handler In WrongPriorities Do
		MessageText = NStr("ru = 'Среди читаемых объектов обработчика %Handler% 
		...
		|en = 'Readable objects of the %Handler% handler
...
Procedure OutputErrorMessage(RaiseException = True)
	...
	If RaiseException Then
		EventName = NStr("ru = 'Построение очереди обработчиков обновления';
							|en = 'Build the update handlers queue';", Common.DefaultLanguageCode());
		...
		For Each Error In Errors Do
			...
			WriteLogEvent(
				...
				Error.Message);
		EndDo;
		ExceptionText = NStr("ru = 'При построении очереди отложенных обработчиков обнаружены ошибки.
			...
			|en = 'Some errors were found upon building the queue of deferred update handlers.
...
Function HasHandlersExecutionCycle(TheHandlerBeingChecked = Undefined, Val HandlerPriorities = Undefined, ReportErrors = False)
	...
	If HasCycle Then
		...
		For Each GraphCycle In GraphCycles Do
			...
			MessageText = NStr("ru = 'Обнаружен цикл порядка выполнения:';
									|en = 'An execution order cycle is found:';") + Chars.LF + "%Path%";
