...
Procedure TerminateSession(Command)
	...
	If SelectedLinesNumber = 0 Then
		ShowMessageBox(,NStr("ru = 'Не выбраны пользователи для завершения сеансов.';
									|en = 'Please select users.';"));
		Return;
	ElsIf SelectedLinesNumber = 1 Then
		If Items.UsersList.CurrentData.Session = InfoBaseSessionNumber Then
			ShowMessageBox(,NStr("ru = 'Невозможно завершить текущий сеанс. Для выхода из приложения можно закрыть главное окно приложения.';
										|en = 'Cannot close the current session. To exit the app, close its main window.';"));
			Return;
		EndIf;
	EndIf;
	...
	Else
		If PromptForIBAdministrationParameters Then
			...
			FormCaption = NStr("ru = 'Завершение сеанса';
									|en = 'Close session';");
			NoteLabel = NStr("ru = 'Для завершения сеанса введите параметры
				|администрирования кластера серверов';
				|en = 'To end the session, enter
...
Procedure OpenEventLog()
	...
	If SelectedRows.Count() = 0 Then
		ShowMessageBox(,NStr("ru = 'Выберите пользователей для просмотра журнала регистрации.';
									|en = 'Select users to view their event log records.';"));
		Return;
	EndIf;
...
Procedure AfterSessionTermination(Result, AdditionalParameters) Export
	...
	If Result = DialogReturnCode.OK Then
		...
		If AdditionalParameters.SessionsNumbers.Count() > 1 Then
			...
			NotificationText1 = NStr("ru = 'Сеансы %1 завершены.';
									|en = 'Sessions %1 are closed.';");
			...
			ShowUserNotification(NStr("ru = 'Завершение сеансов';
												|en = 'Sessions closed';"),, NotificationText1);
			
		Else
			
			NotificationText1 = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Сеанс %1 завершен.';
					|en = 'Session %1 is closed.';"), AdditionalParameters.SessionsNumbers[0]);
			ShowUserNotification(NStr("ru = 'Завершение сеанса';
												|en = 'Session closed';"),, NotificationText1);
