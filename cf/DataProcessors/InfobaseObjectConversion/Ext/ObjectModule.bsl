...
Var TypeDescriptionOfTypes;           // Type("TypeDescription ")
...
Function ExportByRule(
	...
	If CommentObjectProcessingFlag Then
		...
		ActionName = ?(GetRefNodeOnly, NStr("ru = 'Конвертация ссылки на объект';
														|en = 'Converting object reference';"), NStr("ru = 'Конвертация объекта';
																									|en = 'Converting object';"));
		
		MessageText = NStr("ru = '[ActionName]: [Object]([ObjectType]), ПКО: [OCR](НаименованиеПКО)';
								|en = '[ActionName]: [Object]([ObjectType]), ПКО: [OCR](OCRDescription)';");
	...
	// A circular reference to the object.
	// Also, take into account the setting in the rules.
	...
	ElsIf OCR.Owner().Columns.Find("AnObjectWithRegisteredRecords") <> Undefined
		...
		// If an object is exported with register records, don't post the object.
		WriteMode = "Record";
	...
	If HasBeforeConvertObjectGlobalHandler Then
		...
		Except
			WriteInfoOnOCRHandlerExportError(64, ErrorProcessing.DetailErrorDescription(ErrorInfo()),
				OCR, Source, NStr("ru = 'ПередКонвертациейОбъекта (глобальный)';
									|en = 'BeforeConvertObject (global)';"));
		EndTry;
		...
		If RefNode = Undefined Then
			...
			If PropertyStructure.TypeName = "Enum"
				...
				If Common.IsReference(TypeOf(RefToSource))
					...
					WP.Text               = NStr("ru = 'В правиле конвертации значений (ПКЗ) сопоставьте значение Источника значению Приемника.
														|Если подходящего значения приемника нет, то укажите пустое значение.';
														|en = 'Map the Source value to the Destination value in the value conversion rule.
...
//
//  Returns:
//       QueryResult, Undefined - a result of the query to export data cleaning.
//
...
//
//  Returns:
//       QueryResultSelection, Undefined - generated selection for data clearing.
//
...
Procedure ExportDataByRule(Rule) Export
	...
	If CommentObjectProcessingFlag Then
		...
		MessageString = NStr("ru = 'ПРАВИЛО ВЫГРУЗКИ ДАННЫХ: %1 (%2)';
								|en = 'DATA EXPORT RULE: %1 (%2)';");
...
//   Receiver - XMLWriter - a destination object XML node.
//   Type - String
//       - Array of String - a single exported type or a list of string exported types.
...
// Parameters:
//  Ref - CatalogRef
//         - DocumentRef - an object to be deleted.
...
// Parameters:
//  Ref - CatalogRef
//         - DocumentRef - an object to be registered.
// 
...
//         - DocumentObject - Object being written.
...
Procedure WriteObjectToIB(Object, Type, WriteObject = False, Val SendBack = False) Export
	...
	If Common.DataSeparationEnabled()
		...
		If Not IsSeparatedMetadataObject Then 
			...
			ErrorMessageString = NStr("ru = 'Попытка изменения неразделенных данных (%1) в разделенном сеансе.';
											|en = 'Attempting to modify shared data (%1) in a separated session.';");
...
// Parameters:
//  Object - CatalogObject
//         - DocumentObject - an object to set deletion mark for.
...
//
// Returns:
//  Map - map with data tables of the received data exchange message.
//
...
//
// Returns:
//  ValueTable - a value table with incoming exchange message statistics and extra information.
//
...
Procedure RunDataExport(DataProcessorForDataImport = Undefined) Export
	...
	If IsExchangeOverExternalConnection() Then
		...
		If DataProcessorForDataImport().FlagErrors() Then
			...
			MessageString = NStr("ru = 'Ошибка в базе-корреспонденте: %1';
									|en = 'Peer infobase error: %1';");
...
Procedure UnloadRegister(RecordSetForExport, 
	...
	// Write the filter followed by writing the record set.
	// 
...
Procedure RunDataImport() Export
	...
	Try
		...
		Try
			...
			If FlagErrors() Then
				Raise NStr("ru = 'Возникли ошибки при загрузке данных.';
										|en = 'Data import errors.';");
			EndIf;
...
Procedure ExecuteDataImportForInfobase(TablesToImport) Export
	...
	// Record in the event log.
	MessageString = NStr("ru = 'Выполняемое действие: %1;
		|Статус завершения: %2;
		|Обработано объектов: %3.';
		|en = 'Action to execute: %1;
		|Completion status: %2;
		|Objects processed: %3.';",
...
Procedure ExecuteExchangeMessageAnalysis(AnalysisParameters = Undefined) Export
	...
	Try
		...
		Try
			...
			If FlagErrors() Then
				Raise NStr("ru = 'Возникли ошибки при анализе данных.';
										|en = 'Data analysis errors.';");
			EndIf;
	...
	If Not IsBlankString(Conversion.BeforeImportData) Then
		...
		Except
			WriteErrorInfoConversionHandlers(22, ErrorProcessing.DetailErrorDescription(ErrorInfo()),
				NStr("ru = 'ПередЗагрузкойДанных (конвертация)';
					|en = 'BeforeImportData (conversion)';"));
			Cancel = True;
		EndTry;
		...
		If Not IsBlankString(Conversion.AfterImportData) Then
			...
			Except
				WriteErrorInfoConversionHandlers(23, ErrorProcessing.DetailErrorDescription(ErrorInfo()),
					NStr("ru = 'ПослеЗагрузкиДанных (конвертация)';
						|en = 'AfterImportData (conversion)';"));
			EndTry;
...
Function RulesInformation(IsCorrespondentRules = False) Export
	...
	If IsCorrespondentRules Then
		InfoString = NStr("ru = 'Правила конвертации корреспондента (%1) от %2';
								|en = 'Peer (%1) conversion rules created on %2';");
	Else
		InfoString = NStr("ru = 'Правила конвертации этой информационной базы (%1) от %2';
								|en = 'This infobase (%1) conversion rules created on %2';");
	EndIf;
...
//
// Returns:
//  String - supplemented with the specified symbol to the specified length.
//
			...
			ElsIf Upper(JSONReader.CurrentValue) = Upper("typeFeatures") Then
				...
				Else 
					...
					Try
						...
						// An attempt must be made as renaming a object will result in a type access error.
						// For example when renaming LeasingContract (2.4) into LeasingAgreement (2.5).
						ArrayOfNamesOfTypeDescriptions.Add(Type(TypeNameByString));
						
					Except
						
						// Skip the exception for renamed and deleted types.
						CommentText1 = NStr("ru = 'Метаданные не обнаружены: %1.';
												|en = 'Metadata not found: %1.';", Common.DefaultLanguageCode());
...
//
// Returns:
//   XMLWriter - object of the new XML node.
//
...
Procedure InitializeKeepExchangeProtocol()
	...
	Except
		...
		MessageString = NStr("ru = 'Ошибка при попытке записи в файл протокола данных: %1. Описание ошибки: %2';
								|en = 'Failed to log to: %1. Error details: %2';",
			Common.DefaultLanguageCode());
...
Procedure ImportConversionRule(ExchangeRules, XMLWriter)
	...
	If NewRow.Properties.Count() > 0
		...
		// Deleting the UUID value from the array of search fields.
...
Procedure StartReadMessage(MessageReader, DataAnalysis = False)
	...
	If ExchangeFile.NodeType <> XMLNodeType.StartElement Then
		Raise NStr("ru = 'Ошибка формата сообщения обмена.';
								|en = 'Exchange message format error.';");
	EndIf;
	...
	If ConversionRulesTable.Count() = 0 Then
		...
		If FlagErrors() Then
			Raise NStr("ru = 'При загрузке правил обмена данными возникли ошибки.';
									|en = 'Cannot load data exchange rules.';");
		EndIf;
	Else
		deSkip(ExchangeFile);
	EndIf;
	...
	If Not IsBlankString(Conversion.BeforeImportData) Then
		...
		If Cancel Then
			Raise NStr("ru = 'Отказ от загрузки сообщения обмена в обработчике ПередЗагрузкойДанных (конвертация).';
									|en = 'Exchange message import canceled in the BeforeImportData handler (conversion).';");
		EndIf;
	...
	If ExchangeFile.LocalName = "DataTypeInformation" Then
		...
		Else
			...
			If FlagErrors() Then
				Raise NStr("ru = 'При загрузке информации о типах данных возникли ошибки.';
										|en = 'Errors occurred while importing information about the data types.';");
			EndIf;
		EndIf;
		ExchangeFile.Read();
	EndIf;
	...
	If TransactionActive() Then
		Raise NStr("ru = 'Блокировка получения данных не может быть установлена в активной транзакции.';
								|en = 'Cannot set a data receipt lock in an active transaction.';");
	EndIf;
	...
	Try
		...
		If Cancel = True Then
			...
			If Not IsBlankString(CancelReason) Then
				...
				MessageString = NStr("ru = 'Отказ от загрузки сообщения обмена в обработчике ПослеЗагрузкиПараметров (конвертация) по причине: %1';
										|en = 'The exchange message import is canceled in the AfterImportParameters (conversion) handler. Reason: %1';");
				MessageString = StringFunctionsClientServer.SubstituteParametersToString(MessageString, CancelReason);
				Raise MessageString;
			Else
				Raise NStr("ru = 'Отказ от загрузки сообщения обмена в обработчике ПослеЗагрузкиПараметров (конвертация).';
										|en = 'The exchange message import is canceled in the AfterImportParameters (conversion) handler.';");
			EndIf;
...
Procedure ReplaceUUIDIfNecessary(
	...
	If QueryResult.IsEmpty() Then
		...
		// The value will be defined after the object is written.
		// The object might assigned a map upon identifying it by search fields.
...
//
// Parameters:
//  RefTypeString1 - String - an object type as a string, for example, CatalogRef.Products.
// 
// Returns:
...
Function FindObjectByRef(ObjectType, 
	...
	// Object identification takes up to 5 steps. The next step triggers
	// if the current step comes out with no results.
	// An object is searched:
	//
	// 1. In the infobase object map register.
	// 2. By the predefined item name.
	// 3. By the reference UUID.
	// 4. Using an arbitrary search algorithm.
	// 5. By search fields.
	// 
	...
	If ObjectMapFound Then
		...
		// 1. Search for an object by an infobase object map register gave a positive result.
	...
	If PredefinedNameProperty <> Undefined Then
		...
		If    ObjectReference <> Undefined
			...
			// 2. Search for an object of a predefined item name gave a positive result.
			Return ObjectReference;
	...
	If UUIDProperty <> Undefined Then
		...
		If    ObjectReference <> Undefined
			...
			// 3. Search for an object by a reference UUID gave a positive result.
			Return ObjectReference;
	...
	While SearchVariantNumber <= 10
		...
		Except
			...
			WriteInfoOnOCRHandlerImportError(73, ErrorProcessing.DetailErrorDescription(ErrorInfo()), "", "", 
				ObjectType, Undefined, NStr("ru = 'Последовательность полей поиска';
												|en = 'Search field sequence';"));
	...
	If Not HasSearchAlgorithm Then
		...
		// 5. Search for an object by search fields.
		ObjectReference = FindItemBySearchProperties(ObjectType, ObjectTypeName, SearchProperties, PropertyStructure, 
					SearchPropertyNameString, SearchByEqualDate);
...
Procedure ImportTabularSection(Object, TabularSectionName, GeneralDocumentTypeInformation, ObjectParameters, OCR)
	...
	Except
		...
		Text = NStr("ru = 'Имя табличной части: %1';
					|en = 'Table name: %1';");
...
Procedure ObjectImportComments(NBSp, RuleName, Source, ObjectType, Gsn = 0)
	...
	If CommentObjectProcessingFlag Then
		...
		MessageString = NStr("ru = 'Загрузка объекта № %1';
								|en = 'Importing object #%1';");
	...
	If HasBeforeImportObjectGlobalHandler Then
		...
		Except
			...
			WriteInfoOnOCRHandlerImportError(53, ErrorProcessing.DetailErrorDescription(ErrorInfo()),
				RuleName, Source, 
			ObjectType, Undefined, NStr("ru = 'ПередЗагрузкойОбъекта (глобальный)';
											|en = 'BeforeImportObject (global)';"));
	...
	While ExchangeFile.Read() Do
		...
		ElsIf NodeName = "RecordSetRows" Then
			...
			If ExchangeFile.NodeType <> XMLNodeTypeEndElement Then
				...
				// The "OnImportObject" event handler triggers before reading the first record in the set.
				// 
...
Function ReadObject(UUIDAsString1 = "")
	...
	// A temporary workaround: Save the document records in the accounting registers to the value table.
	// After the document is written, write the records.
	//
	...
	If HasBeforeImportObjectGlobalHandler Then
		...
		Except
			...
			WriteInfoOnOCRHandlerImportError(53, ErrorProcessing.DetailErrorDescription(ErrorInfo()),
				RuleName, Source, ObjectType, Undefined, NStr("ru = 'ПередЗагрузкойОбъекта (глобальный)';
																	|en = 'BeforeImportObject (global)';"));
	...
	// The flag identifies whether the object was found using the search fields in the object mapping mode.
	// If the flag is raised, add the information on mapping the reference GUID in the source and destination
	// to the map register.
	ObjectFoundBySearchFields = False;
			...
			ElsIf NodeName = "RecordSet" Then
				...
				If Metadata.AccountingRegisters.Find(Name) <> Undefined Then
					...
					If LoadedRegisteredRecords.Count() > 0 Then
						...
						// A workaround for 1C:Enterprise bug #30178033.
						// The order of extra dimensions in the accounting registers
						// doesn't match the order specified in the bookkeeping account.
			...
			If HasAfterObjectImportGlobalHandler Then
				...
				Except
					...
					WriteInfoOnOCRHandlerImportError(54, ErrorProcessing.DetailErrorDescription(ErrorInfo()), 
						RuleName, Source, ObjectType, Object, NStr("ru = 'ПослеЗагрузкиОбъекта (глобальный)';
																		|en = 'AfterImportObject (global)';"));
			...
			If ObjectTypeName = "Document" Then
				...
				If DataImportToInfobaseMode() Then
					...
					Try
						...
						// Write the document. Information whether to post it or undo posting is recorded separately.
						
						// If documents just need to be posted, post them as they are.
						If WriteMode = DocumentWriteMode.Write Then
							
							// Disable the object registration mechanism when unposting the document.
							// ORM will run during the deferred posting (for data import optimization).
							// 
						...
						ElsIf WriteMode = DocumentWriteMode.Posting Then
							...
							// If the document is written and a reference is created,
							// add the document to the posting queue.
							If WriteObject
								And Object <> Undefined
								And Object.Ref <> Undefined Then
					...
					Except
						...
						MessageString = NStr("ru = 'Ошибка при записи документа: %1. Описание ошибки: %2';
												|en = 'Failed to save document: %1. Error details: %2';");
			...
			ElsIf ObjectTypeName <> "Enum" Then
				...
				If DataImportToInfobaseMode() Then
					...
					If Not (ObjectTypeName = "InformationRegister"
						...
						// If the document is written and a reference is created,
						// add the object to the writing queue.
						If WriteObject
							And Object <> Undefined
							And Object.Ref <> Undefined Then
...
Procedure SwitchToNewExchange()
	...
	MessageString = NStr("ru = 'Выполнен автоматический переход на синхронизацию данных через формат EnterpriseData.';
							|en = 'Automatic switching to EnterpriseData data synchronization format.';");
	WriteEventLogDataExchange1(MessageString, EventLogLevel.Information);
	ExchangeResultField = Enums.ExchangeExecutionResults.Canceled;
	Raise NStr("ru = 'Синхронизация данных по старой настройке отменена.';
							|en = 'Data synchronization with outdated settings is canceled.';");
EndProcedure
...
//  IncomingData	     - arbitrary auxiliary data that is passed to
//                         the conversion rule.
...
Procedure ExportPropertyGroup(Source, Receiver, IncomingData, OutgoingData, OCR, PGCR, PropertyCollectionNode, 
	...
	If ExportGroupToFile Then
		TempRecordFile.WriteLine("</" + MasterNodeName + ">"); // Close the node.
...
Procedure GetPropertyValue(Value, CollectionObject, OCR, PCR, IncomingData, Source, DataSelection = Undefined)
	...
	Try
		...
		// Serialize TypeDescription to JSON.
		...
		Else
			...
			If ExportObjectByRef(ExtDimension, NodeForExchange) Then
				...
				If Not ObjectPassesAllowedObjectFilter(ExtDimension) Then
					...
					// Add the object to the array of objects imported by reference.
					// The objects will be registered on the current node and assigned
					// a number of the current sent exchange message.
...
Procedure FireEventsBeforeExportObject(Object, Rule, Properties=Undefined, IncomingData=Undefined, 
	...
	If CommentObjectProcessingFlag Then
		...
		EventName = NStr("ru = 'Выгрузка объекта: %1';
							|en = 'Export object: %1';");
	...
	If HasBeforeExportObjectGlobalHandler Then
		...
		Except
			WriteErrorInfoDERHandlers(65, ErrorProcessing.DetailErrorDescription(ErrorInfo()), 
				Rule.Name, NStr("ru = 'ПередВыгрузкойОбъектаВыборки (глобальный)';
									|en = 'BeforeExportSelectionObject (global)';"), Object);
		EndTry;
	...
	If HasAfterExportObjectGlobalHandler Then
		...
		Except
			WriteErrorInfoDERHandlers(69, ErrorProcessing.DetailErrorDescription(ErrorInfo()),
				Rule.Name, NStr("ru = 'ПослеВыгрузкиОбъектаВыборки (глобальный)';
									|en = 'AfterExportSelectionObject (global)';"), Object);
		EndTry;
	EndIf;
...
// Returns:
//   ReportBuilder - a builder object for dataset.
// 
		...
		If Not IsBlankString(EventText) Then
			...
			Except
				ErrorMessageString = WriteErrorInfoConversionHandlers(76, 
					ErrorProcessing.DetailErrorDescription(ErrorInfo()), 
					NStr("ru = 'ПередОтправкойИнформацииОбУдалении (конвертация)';
						|en = 'BeforeSendDeletionInfo (conversion)';"));
...
Procedure InitConversionRuleTable()
	...
	// The "Values" property is not used.
	// Columns.Add("Values", deTypeDetails("Map"));
	
	// Map.
	// Key - Predefined item value in this infobase.
	// Value - String presentation of the predefined value in the destination.
	Columns.Add("PredefinedDataValues", deTypeDetails("Map"));
	
	// Structure.
	// Key - String presentation of the item value in this infobase.
	// Value - String presentation of the predefined value in the destination.
...
Function InitializeDataProcessors()
	...
	If ExportHandlersDebug Or ImportHandlersDebug Then 
		Raise
			NStr("ru = 'Внешняя обработка отладки, загружаемая из файла на диске, не поддерживается.';
				|en = 'The external data processor (debugger) is not supported.';");
	EndIf;
...
Procedure InitMessages()
	...
	ErrorsMessages.Insert(5,  NStr("ru = 'Ошибка открытия файла обмена';
										|en = 'An error occurred when opening an exchange file';"));
	...
	ErrorsMessages.Insert(24, NStr("ru = 'Ошибка при удалении объекта';
										|en = 'An error occurred when deleting the object';"));
	ErrorsMessages.Insert(25, NStr("ru = 'Ошибка при записи документа';
										|en = 'An error occurred when saving the document';"));
	ErrorsMessages.Insert(26, NStr("ru = 'Ошибка записи объекта';
										|en = 'An error occurred when saving the object';"));
	...
	ErrorsMessages.Insert(173, NStr("ru = 'Ошибка блокировки узла обмена. Возможно синхронизация данных уже выполняется';
											|en = 'Cannot lock the exchange node. Probably the synchronization is already running.';"));
	...
	ErrorsMessages.Insert(1000, NStr("ru = 'Ошибка при создании временного файла выгрузки данных';
											|en = 'Cannot create a temporary data export file.';"));
...
Procedure ImportSingleTypeData(ExchangeRules, TypeMap, LocalItemName)
	...
	Else
		...
		If ExchangeRules.Value = "TypeDefinition" Then
			...
			TypeMap.Insert(NodeName, Type("TypeDescription")); // Deserialize JSON to TypeDescription.
	...
	If ExchangeMode <> "Load" And Conversion.Property("AfterImportExchangeRules", AfterExchangeRulesImportEventText)
		...
		Except
			ErrorMessageString = WriteErrorInfoConversionHandlers(75, 
				ErrorProcessing.DetailErrorDescription(ErrorInfo()), 
				NStr("ru = 'ПослеЗагрузкиПравилОбмена (конвертация)';
					|en = 'AfterImportExchangeRules (conversion)';"));
	...
	If Not IsBlankString(Conversion.OnGetDeletionInfo) Then
		...
		Except
			ErrorMessageString = WriteErrorInfoConversionHandlers(77,
				ErrorProcessing.DetailErrorDescription(ErrorInfo()),
				NStr("ru = 'ПриПолученииИнформацииОбУдалении (конвертация)';
					|en = 'OnGetDeletionInfo (conversion)';"));
					...
					If TableRow = Undefined Then
						...
						For Each Column In ExchangeMessageDataTable.Columns Do
							...
							If Column.ValueType.ContainsType(StringType) Then
								...
								TableRow[Column.Name] = NStr("ru = 'Удаление объекта';
																	|en = 'Object deletion';");
...
Procedure ReadDataInAnalysisMode(MessageReader, AnalysisParameters = Undefined)
	...
	While ExchangeFile.Read() Do
		...
		If NodeName = "Object" Then
			...
			If ObjectTypeString <> "ConstantsSet" Then
				...
				ElsIf Not (OCR.SynchronizeByID And OCR.SearchBySearchFieldsIfNotFoundByID) And OCR.SynchronizeByID Then
					...
					// Objects that are auto-mapped during an exchange (classifiers) are hidden from the statistics table. 
					// Therefore, don't gather statistics.
					// 

					// Objects identified by search fields (not by reference UUID) are hidden.
					// 
					CollectStatistics = True;
					IsClassifier   = False;
		...
		ElsIf NodeName = "ObjectDeletion" Then
			...
			TableRow.SearchFields = ""; // Search fields are assigned in object mapping processing.
			
			// Define values for the "TableFields" column.
				...
				Try
					...
					If Cancel = True Then
						...
						If Not IsBlankString(CancelReason) Then
							...
							MessageString = NStr("ru = 'Загрузка данных отменена по причине: %1';
													|en = 'Data import canceled. Reason: %1';");
							MessageString = StringFunctionsClientServer.SubstituteParametersToString(MessageString, CancelReason);
							Raise MessageString;
						Else
							Raise NStr("ru = 'Загрузка данных отменена';
													|en = 'Data import canceled';");
						EndIf;
...
Procedure ReadDataViaExchange(MessageReader, DataAnalysis)
	...
	// Check if the destination node is present.
	// Check if the destination node in the exchange message is valid.
	If Not ValueIsFilled(ExchangeNodeRecipient)
		Or ExchangeNodeRecipient <> ExchangeNodeDataImport Then
		
		MessageString = NStr("ru = 'Не найден узел обмена для загрузки данных. План обмена: %1, код: %2.';
								|en = 'Exchange node for data import is not found. Exchange plan: %1, code: %2.';");
	...
	If DataImportToInfobaseMode() Then
		...
		If ReceivedNo >= MessageReader.MessageNo Then // The message number is less than or equal to the previously received one.
			...
			Raise NStr("ru = 'Сообщение обмена было принято ранее';
									|en = 'The exchange message was received earlier.';");
		EndIf;
		...
		If DeleteChangeRecords Then // Delete change records.
			...
			If TransactionActive() Then
				Raise NStr("ru = 'Удаление регистрации изменений данных не может быть выполнено в активной транзакции.';
										|en = 'Cannot unregister items in an active transaction.';");
			EndIf;
	...
	If Not IsBlankString(Conversion.AfterGetExchangeNodesInformation) Then
		...
		Except
			Raise WriteErrorInfoConversionHandlers(176,
				ErrorProcessing.DetailErrorDescription(ErrorInfo()),
				NStr("ru = 'ПослеПолученияИнформацииОбУзлахОбмена (конвертация)';
					|en = 'AfterGetExchangeNodesInformation (conversion)';"));
		EndTry;
...
Procedure ReadCommonNodeData(MessageReader)
	...
	If DataExchangeEvents.DataDiffers1(CommonNode, CommonNode.Ref.GetObject()) Then
		...
		// Close the transaction that was started to get data in the "Load" mode.
		// Otherwise, the lock set on "CommonNodeDataChanges" when calling the method
		// "DeleteChangeRecords" is applied while the transaction is open.
		// This will prevent both interactive and programmatical (from other sessions)
		// modification of the exchange plan. 
		// 
		// 
		// 
		...
		// Restart the transaction that had started to get data in the "Load" mode if it was committed before calling
		// the "DeleteChangesRegistration" method of the "CommonNodeDataChanges" information register.
	...
	If Not IsBlankString(Conversion.BeforeExportData) Then
		...
		Except
			WriteErrorInfoConversionHandlers(62, 
				ErrorProcessing.DetailErrorDescription(ErrorInfo()),
				NStr("ru = 'ПередВыгрузкойДанных (конвертация)';
					|en = 'BeforeExportData (conversion)';"));
	...
	If Not Cancel And Not IsBlankString(Conversion.AfterExportData) Then
		...
		Except
			WriteErrorInfoConversionHandlers(63,
				ErrorProcessing.DetailErrorDescription(ErrorInfo()), NStr("ru = 'ПослеВыгрузкиДанных (конвертация)';
																						|en = 'AfterExportData (conversion)';"));
		EndTry;
...
Procedure ExportObjectMappingRegister(WriteMessage1, ErrorMessageString)
	...
	While ChangesSelection.Next() Do
		...
		// Export the registered information from the register "InfobaseObjectsMaps".
		// The register conversion rules are specified in the handler's code.
...
Procedure ExecuteRegisteredDataExport(WriteMessage1, ErrorMessageString, UsedExportRulesTable)
	...
	If Not IsBlankString(Conversion.BeforeGetChangedObjects) Then
		...
		Except
			WriteErrorInfoConversionHandlers(175, ErrorProcessing.DetailErrorDescription(ErrorInfo()),
				NStr("ru = 'ПередПолучениемИзмененныхОбъектов (конвертация)';
					|en = 'BeforeGetChangedObjects (conversion)';"));
			Return;
		EndTry;
	...
	// The "InfobaseObjectsMaps" information register is exported separately.
	// Therefore, it should be excluded from the selection.
	...
	Except
		...
		Raise(NStr("ru = 'Ошибка при отправке данных';
								|en = 'Cannot send the data';") + ": " + ErrorProcessing.DetailErrorDescription(ErrorInfo()));
	...
	If IsExchangeOverExternalConnection() Then
		If DataImportExecutedInExternalConnection Then
			If DataProcessorForDataImport().UseTransactions
				...
				// If transactions are used and the number of items per transaction is set,
				// control the number of imported objects when reading exchange messages.
				// When the numbers match, commit the transaction and open a new one. 
				// 
				// 
