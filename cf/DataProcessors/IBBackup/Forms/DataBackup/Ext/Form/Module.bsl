...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If BackupSettings1.LatestBackupDate = Date(1, 1, 1) Then
		TitleText = NStr("ru = 'Резервное копирование еще ни разу не проводилось';
								|en = 'The infobase has never been backed up.';");
	Else
		TitleText = NStr("ru = 'В последний раз резервное копирование проводилось: %1';
	...
	If ManualStart1 Then
		...
		EndIf;
		Items.Next.Title = NStr("ru = 'Сохранить резервную копию';
										|en = 'Save backup';");
	EndIf;
...
Procedure BeforeClose(Cancel, Exit, WarningText, StandardProcessing)
	...
	WarningText = NStr("ru = 'Прервать подготовку к резервному копированию данных?';
...
Procedure GoToPageBackupAfterInfobaseAccessCheck(ConnectionResult, Context) Export
	...
	IBConnectionsServerCall.SetConnectionLock(
		NStr("ru = 'Выполняется резервное копирование информационной базы.';
...
Procedure GoToBackupResultsPage()
	...
	Items.Cancel.Title = NStr("ru = 'Закрыть';
									|en = 'Close';");
...
#If WebClient Then
	MessageText = NStr("ru = 'Создание резервной копии не доступно в веб-клиенте.';
							|en = 'Web client does not support data backup.';");
...
#Else
	...
	If IsBlankString(Object.BackupDirectory) Then
		MessageText = NStr("ru = 'Не выбран каталог для резервной копии.';
	...
	ElsIf FindFiles(Object.BackupDirectory).Count() = 0 Then
		MessageText = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Указан несуществующий каталог ""%1"". 
			...
			|en = 'A non-existent directory specified: %1.
	...
	Else
		...
		Except
			MessageText = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Нет прав на запись в каталог с резервными копиями.
...
Procedure Timeout2()
	...
	DetachIdleHandler("CheckForSingleConnection");
	QueryText = NStr("ru = 'Не удалось отключить всех пользователей от базы. Провести резервное копирование? (возможны ошибки при архивации)';
						|en = 'Cannot terminate all user sessions. Are you sure you still want to back up the data? The backup might contain errors.';");
	ExplanationText = NStr("ru = 'Не удалось отключить пользователя.';
...
Procedure SetButtonTitleNext(ThisButtonNext)
	...
	Items.Next.Title = ?(ThisButtonNext, NStr("ru = 'Далее >';
...
Function GetPath(DialogMode)
	...
	If Mode = FileDialogMode.ChooseDirectory Then
		OpenFileDialog.Title= NStr("ru = 'Выберите каталог';
											|en = 'Select directory';");
	Else
		OpenFileDialog.Title= NStr("ru = 'Выберите файл';
											|en = 'Select file';");
	EndIf;	
...
#If Not WebClient And Not MobileClient Then
	...
	EventLogClient.AddMessageForEventLog(
		...
		"Information", 
		NStr("ru = 'Выполняется резервное копирование информационной базы:';
			|en = 'Backing up the infobase backup:';") + " " + MainScriptFileName);
...
Function GenerateSplashText()
	...
	TextParameters["[SplashTitle]"] = NStr("ru = 'Резервное копирование информационной базы...';
	...
	TextParameters["[Step1Initialization]"] = NStr("ru = 'Инициализация';
													|en = 'Initialization';");
	TextParameters["[Step2BackupCreation]"] = NStr("ru = 'Создание резервной копии информационной базы';
															|en = 'Creating infobase backup';");
	TextParameters["[Step3AwaitingCompletion]"] = NStr("ru = 'Ожидание завершения резервного копирования';
	...
	TextParameters["[Step5Completion]"] = NStr("ru = 'Завершение';
												|en = 'Completion';");
	TextParameters["[ProcessIsAborted]"] = NStr("ru = 'Внимание: процесс резервного копирования был прерван, и информационная база осталась заблокированной.';
