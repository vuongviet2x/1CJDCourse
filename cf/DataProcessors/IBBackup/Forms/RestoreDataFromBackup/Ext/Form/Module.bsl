...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Common.IsWebClient()
		...
		Raise NStr("ru = 'Резервное копирование недоступно в веб-клиенте и мобильном клиенте.';
								|en = 'Web client and mobile client do not support data backup.';");
	EndIf;
	
	If Not Common.FileInfobase() Then
		Raise NStr("ru = 'В клиент-серверном варианте работы резервное копирование следует выполнять сторонними средствами (средствами СУБД).';
...
Procedure BeforeClose(Cancel, Exit, WarningText, StandardProcessing)
	...
	WarningText = NStr("ru = 'Прервать подготовку к восстановлению данных?';
								|en = 'Do you want to cancel data restore preparations?';");
...
Procedure FinishAfterCheckInfobaseAccess(ConnectionResult, Context) Export
	...
	IBConnectionsServerCall.SetConnectionLock(
		NStr("ru = 'Выполняется восстановление информационной базы.';
			|en = 'Restoring the infobase…';"),
		"Backup");
...
Procedure SelectBackupFile()
	...
	OpenFileDialog.Filter = NStr("ru = 'Резервная копия базы (*.zip, *.1CD)|*.zip;*.1cd';
										|en = 'Infobase backup (*.zip, *.1CD)|*.zip;*.1cd';");
	OpenFileDialog.Title= NStr("ru = 'Выберите файл резервной копии';
										|en = 'Select a backup file';");
...
#If WebClient Or MobileClient Then
	MessageText = NStr("ru = 'Восстановление не доступно в веб-клиенте и мобильном клиенте.';
							|en = 'Web client and mobile client do not support data backup.';");
...
#Else
	...
	If PasswordRequired And IsBlankString(IBAdministratorPassword) Then
		MessageText = NStr("ru = 'Не задан пароль администратора.';
	...
	If IsBlankString(FileName) Then
		MessageText = NStr("ru = 'Не выбран файл с резервной копией.';
								|en = 'Backup file is not provided.';");
	...
	If Upper(ArchiveFile1.Extension) <> ".ZIP" And Upper(ArchiveFile1.Extension) <> ".1CD"  Then
		...
		MessageText = NStr("ru = 'Выбранный файл не является архивом с резервной копией.';
								|en = 'The selected file is not a backup file.';");
		CommonClient.MessageToUser(MessageText,, "Object.BackupImportFile");
		Return False;
	...
	If Upper(ArchiveFile1.Extension) = ".1CD" Then
		...
		If Upper(ArchiveFile1.BaseName) <> "1CV8" Then
			MessageText = NStr("ru = 'Выбранный файл не является резервной копией (неправильное имя файла информационной базы).';
									|en = 'The selected file is not a valid backup file for this infobase. It contains another infobase name.';");
			CommonClient.MessageToUser(MessageText,, "Object.BackupImportFile");
			Return False;
		EndIf;
	...
	Else 
		...
		If ZipFile.Items.Count() <> 1 Then
			...
			MessageText = NStr("ru = 'Выбранный файл не является архивом с резервной копией (содержит более одного файла).';
									|en = 'The selected file is not a valid backup file. It contains more than one file.';");
			CommonClient.MessageToUser(MessageText,, "Object.BackupImportFile");
			Return False;
		...
		If Upper(FileInArchive.Extension) <> "1CD" Then
			...
			MessageText = NStr("ru = 'Выбранный файл не является архивом с резервной копией (не содержит файл информационной базы).';
									|en = 'The selected file is not a valid backup file. It does not contain any infobase.';");
			CommonClient.MessageToUser(MessageText,, "Object.BackupImportFile");
			Return False;
		...
		If Upper(FileInArchive.BaseName) <> "1CV8" Then
			...
			MessageText = NStr("ru = 'Выбранный файл не является архивом с резервной копией (неправильное имя файла информационной базы).';
									|en = 'The selected file is not a valid backup file. The infobase name is not correct.';");
			CommonClient.MessageToUser(MessageText,, "Object.BackupImportFile");
			Return False;
...
#If Not WebClient And Not MobileClient Then
	...
	EventLogClient.AddMessageForEventLog(
		...
		"Information",
		NStr("ru = 'Выполняется восстановление данных информационной базы:';
			|en = 'Restoring the infobase:';") + " " + MainScriptFileName);
...
Function GenerateSplashText()
	...
	TextParameters["[SplashTitle]"] = NStr("ru = 'Восстановление данных из резервной копии...';
													|en = 'Restoring data from backup…';");
	TextParameters["[SplashText]"] = 
		NStr("ru = 'Пожалуйста, подождите.
	...
	TextParameters["[Step1Initialization]"] = NStr("ru = 'Инициализация';
													|en = 'Initializing';");
	TextParameters["[Step2DataRecovery]"] = NStr("ru = 'Восстановление данных';
														|en = 'Restoring data';");
	TextParameters["[Step3AwaitingCompletion]"] = NStr("ru = 'Ожидание завершения восстановления данных';
														|en = 'Waiting for data restore to complete';");
	TextParameters["[Step4AllowConnections]"] = NStr("ru = 'Разрешение подключений новых соединений';
															|en = 'Allowing new connections';");
	TextParameters["[Step5Completion]"] = NStr("ru = 'Завершение';
												|en = 'Completing';");
	TextParameters["[ProcessIsAborted]"] = NStr("ru = 'Внимание: процесс восстановления данных был прерван, и информационная база осталась заблокированной.';
												|en = 'Warning! The data restore was interrupted and the infobase is still locked.';");
