...
Function PrepareFormData()
	...
	For Each PagesPackage In PreparedPackages Do
		PagesPackage.FormCaption = NStr("ru = 'Информация';
											|en = 'Information';");
		...
		If StrStartsWith(PagesPackage.Section, "_") Then
			...
			Else
				...
				If CurrentSubmenu = Undefined Then
					Raise StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Не найдена группа ""%1""';
																									|en = 'Group ""%1"" is not found';"), SubmenuName);
				EndIf;
		...
		ElsIf CurrentSectionDescription <> PagesPackage.Section Then
			...
			IsMain1 = (PagesPackage.Section = NStr("ru = 'Главная';
													|en = 'Main';"));
