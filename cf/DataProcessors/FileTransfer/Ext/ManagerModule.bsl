...
Function ExecuteFileTransfer(FilesToTransfer, Parameters) Export
	...
	If Parameters.Action = "MoveBetweenVolumes" Then
		ActionForLog = NStr("ru = 'между томами на сервере.';
									|en = 'between volumes on a server.';");
	ElsIf Parameters.Action = "MoveToVolumes" Then
		ActionForLog = NStr("ru = 'в тома на сервере.';
									|en = 'to volume on a server.';")
	Else
		ActionForLog = NStr("ru = 'в информационную базу.';
									|en = 'to the infobase.';")
	EndIf;
	
	WriteLogEvent(NStr("ru = 'Файлы.Начало переноса файлов.';
									|en = 'Files.Start moving files.';", Common.DefaultLanguageCode()),
		EventLogLevel.Information,,,
		NStr("ru = 'Начат перенос файлов';
			|en = 'Started moving files';") + " " + ActionForLog);
	...
	RecordOnCompletionText = NStr("ru = 'Завершен перенос файлов %1
		|Перенесено файлов: %2';
		|en = 'The files are moved %1
		|Total files: %2';");
	...
	If FilesTransferred < FilesToMoveCount Then
		RecordOnCompletionText = RecordOnCompletionText + "
			|" + NStr("ru = 'Количество ошибок при переносе: %1';
						|en = 'Total errors: %1.';");
		RecordOnCompletionText = StringFunctionsClientServer.SubstituteParametersToString(
			RecordOnCompletionText, Format(FilesToMoveCount - FilesTransferred, "NZ=0; NG="));
	EndIf;
	
	WriteLogEvent(NStr("ru = 'Файлы.Завершение переноса файлов.';
									|en = 'Files.End moving files.';", Common.DefaultLanguageCode()),
...
Procedure TransferFile(File, VolumeProperties, Parameters, TransferErrors, FilesTransferred)
	...
	Try
		...
		Else
			Raise NStr("ru = 'Неподдерживаемая операция.';
	...
	Except
		...
		WriteLogEvent(NStr("ru = 'Файлы.Ошибка переноса файла.';
										|en = 'Files.File moving error.';", Common.DefaultLanguageCode()),
			EventLogLevel.Error,, FileRef,
			StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Не удалось перенести файл %1
				|%2
				|по причине:
				|%3"".';
				|en = 'Couldn''t move file %1
				|%2
				|due to:
				|%3"".';"),
...
Procedure MoveBetweenVolumes(FileObject1, VolumeProperties, Parameters)
	...
	If Not FileOnHardDrive.Exists() Then
		...
		NStr("ru = 'Файл ""%1"" не найден.';
			|en = 'File ""%1"" is not found.';"), CurrentFilePath);
	EndIf;
...
Procedure ThrowAnExceptionWhenTheVolumeSizeIsExceeded(Val VolumeProperties, Val FileObject1, Val Parameters)
	...
	If Parameters.MoveToVolume
		...
		Raise NStr("ru = 'Превышен максимальный размер тома.';
								|en = 'Volume size limit exceeded.';");
	EndIf;
