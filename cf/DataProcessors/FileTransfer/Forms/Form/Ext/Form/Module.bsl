...
Procedure OnOpen(Cancel)
	...
	If StorageVolumesCount = 0 Then
		...
		ShowMessageBox(NotifyDescription, NStr("ru = 'Невозможно перенести файлы между томами, т.к. нет ни одного тома для размещения файлов.';
														|en = 'Cannot move the files between volumes since there is no volume available for storing files.';"));	
		Return;
	EndIf;
...
Procedure ActionOnChange(Item)
	...
	If Action = "MoveBetweenVolumes"
		...
		ShowMessageBox(, NStr("ru = 'Невозможно перенести файлы между томами: в настройках указан один том для размещения файлов.';
										|en = 'Cannot move the files between volumes as only one file storage volume is defined in the settings.';"));
	EndIf;
...
Procedure ExecuteTransfer(Command)
	...
	If Action = "MoveBetweenVolumes" And Not ValueIsFilled(DestinationStorageVolume) Then
		CommonClient.MessageToUser(
			NStr("ru = 'Не заполнен том, в который будут перенесены файлы.';
				|en = 'Please specify the destination volume.';"), , "DestinationStorageVolume");
		Return;
	EndIf;
...
Procedure ExecuteTransferCompletion(Result, AdditionalParameters) Export
	...
	If TransferResult.TransferErrors.Count() = 0 Then
		If TransferResult.FilesTransferred > 0 Then
			Explanation = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Завершен перенос файлов. Перенесено файлов %1.';
																					|en = 'The files are moved. Total files: %1.';"),
				Format(TransferResult.FilesTransferred, "NZ=0; NG="));
		Else
			Explanation = NStr("ru = 'Нет ни одного файла, подходящего под заданные условия.';
							|en = 'There are no files matching the specified conditions.';");
			If AdditionalConditions.Settings.Filter.Items.Count() > 1 Then
				Explanation = Explanation + Chars.LF + NStr("ru = 'Обратите внимание, что дополнительные условия объединяются по ""И"", если они не сгруппированы в группу ""Или"".';
															|en = 'Consider that additional conditions are grouped by ""AND"" if they are not grouped into an ""Or"" group.';");
			EndIf;
		EndIf;
		ShowMessageBox(, Explanation);
	Else
		Explanation = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Перенесено файлов: %1, не удалось перенести: %2';
				|en = 'Moved files: %1, failed to move: %2';"),
