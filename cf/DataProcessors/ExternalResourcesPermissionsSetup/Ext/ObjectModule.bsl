...
Function NewSecurityProfileDetails(Val ProgramModuleType, Val ModuleID)
	...
	If IsBlankString(IBName) Then
		Raise NStr("ru = 'Строка соединения информационной базы должна содержать информационной базы';
								|en = 'Infobase connection string must contain the infobase.';");
	EndIf;
	
	If ProgramModuleType = Catalogs.MetadataObjectIDs.EmptyRef() Then
		Return StringFunctionsClientServer.SubstituteParametersToString(Template, IBName,
			NStr("ru = 'Профиль безопасности для информационной базы';
				|en = 'Security profile for infobase';"), InfoBaseConnectionString());
	Else
