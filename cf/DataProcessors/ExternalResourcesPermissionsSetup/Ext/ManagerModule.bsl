...
Function ExecuteApplicabilityCheckRequestsProcessing() Export
	...
	If TransactionActive() Then
		Raise NStr("ru = 'Транзакция активна';
								|en = 'Transaction is active';");
	EndIf;
...
Procedure LockRegistersOfGrantedPermissions(Val ProgramModule = Undefined, Val LockAttachmentModes = True) Export
	...
	If Not TransactionActive() Then
		Raise NStr("ru = 'Должна быть активная транзакция';
								|en = 'There must be an active transaction';");
	EndIf;
...
Function PermissionsTableRow(Val PermissionsTable, Val Filter, Val AddIfAbsent = True) Export
	...
	Else
		...
		Raise StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Нарушение уникальности строк в таблице разрешений по отбору %1';
																						|en = 'Rows uniqueness violation in permission table by the filter %1';"),
