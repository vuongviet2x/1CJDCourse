...
Function StartRequestsProcessing(Val Queries, Val EnablingMode, DisablingMode, Val RecoveryMode, Val ApplicabilityCheckMode)
	...
	Job = BackgroundJobs.Execute("Common.ExecuteConfigurationMethod",
			...
			,
			NStr("ru = 'Обработка запросов на использование внешних ресурсов...';
...
Function RequestsProcessed(Val JobID)
	...
	If Job = Undefined Then
		Raise(NStr("ru = 'Непредвиденная ситуация при обработке запросов разрешений на использование внешних ресурсов.';
	...
	If Job.State = BackgroundJobState.Failed Then
		...
		Else
			Raise(NStr("ru = 'Не удалось обработать запросы разрешений на использование внешних ресурсов.';
									|en = 'Cannot process permission requests to use external resources.';"));
		EndIf;
	ElsIf Job.State = BackgroundJobState.Canceled Then
		Raise(NStr("ru = 'Не удалось обработать запросы разрешений на использование внешних ресурсов, т.к. задание было отменено администратором.';
...
Procedure CancelRequestsProcessing(Val JobID)
	...
	Except
		...
		WriteLogEvent(NStr("ru = 'Настройка разрешений на использование внешних ресурсов.Отмена выполнения фонового задания';
										|en = 'External resource permission setup.Cancel background job';", Common.DefaultLanguageCode()),
