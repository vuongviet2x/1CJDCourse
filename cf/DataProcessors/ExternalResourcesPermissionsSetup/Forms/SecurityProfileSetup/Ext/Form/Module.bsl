...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Not SafeModeManagerInternal.SecurityProfilesUsageAvailable() Then
		Raise NStr("ru = 'Недоступно использование профилей безопасности.';
								|en = 'Security profiles are unavailable';");
	EndIf;
	
	If Not SafeModeManagerInternal.CanSetUpSecurityProfiles() Then
		Raise NStr("ru = 'Настройка профилей безопасности недоступна.';
								|en = 'Setting of security profiles is unavailable.';");
	EndIf;
...
Procedure StartApplyingSecurityProfilesSettings(Val UUID)
	...
	If Not SafeModeManagerInternal.SecurityProfilesUsageAvailable() Then
		Raise NStr("ru = 'Внимание! Включение автоматического запроса разрешений недоступно';
								|en = 'Warning! Enabling automatic permission request is not available.';");
	EndIf;
