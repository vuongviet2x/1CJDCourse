...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	Items.FormAllow.Title = NStr("ru = 'Далее >';
											|en = 'Next >';");
...
#If MobileClient Then
	ShowMessageBox(, NStr("ru = 'Отчет недоступен в мобильном клиенте. Запустите тонкий клиент или веб-клиент.';
									|en = 'The report is unavailable in mobile client. Start thin client or web client.';"));
	Cancel = True;
	Return;
#EndIf
...
Procedure FillCheckProcessingAtServer(Cancel, CheckedAttributes)
	...
	If InfobaseAdministrationParametersRequired Then
		...
		If IBUser = Undefined Then
			Common.MessageToUser(NStr("ru = 'Указанный пользователь не имеет доступа к информационной базе.';
														|en = 'This user is not allowed to access the infobase.';"),,
				FieldName,,Cancel);
			Return;
		EndIf;
		
		If Not Users.IsFullUser(IBUser, True) Then
			Common.MessageToUser(NStr("ru = 'У пользователя нет административных прав.';
														|en = 'This user has no administrative rights.';"),,
				FieldName,,Cancel);
			Return;
		EndIf;
...
Procedure Next(Command)
	...
	If Items.GroupPages.CurrentPage = Items.PermissionsPage Then
		...
		Items.FormAllow.Title = NStr("ru = 'Настроить разрешения в кластере серверов';
												|en = 'Set up permissions in server cluster';");
	...
	For Each ScenarioItem In PermissionsApplyingScenario Do
		...
		If ScenarioItem.Operation = OperationKinds.Creating Then
			...
			If ClusterAdministration.SecurityProfileExists(ClusterAdministrationParameters, ScenarioItem.Profile) Then
				...
				Common.MessageToUser(
					StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Профиль безопасности %1 уже присутствует в кластере серверов. Настройки в профиле безопасности будут замещены...';
																				|en = 'Security profile %1 already exists in the server cluster. Settings in the security profile will be replaced…';"), ScenarioItem.Profile));
		...
		ElsIf ScenarioItem.Operation = OperationKinds.Delete Then
			...
			Else
				...
				Common.MessageToUser(
					StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Профиль безопасности %1 отсутствует в кластере серверов. Возможно, профиль безопасности был удален ранее...';
																				|en = 'Security profile %1 does not exist in the server cluster. Security profile might have been deleted earlier…';"), ScenarioItem.Profile));
