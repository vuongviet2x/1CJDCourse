...
Function RulesInformation() Export
	...
	InfoString = NStr("ru = 'Правила регистрации объектов этой информационной базы (%1) от %2';
							|en = 'Object registration rules in this infobase (%1) created on %2';");
...
Procedure CheckExchangePlanExists()
	...
	If Registration.ExchangePlanName <> ExchangePlanNameForImport Then
		...
		ErrorDescription = NStr("ru = 'В правилах регистрации указан план обмена %1, а загрузка выполняется для плана обмена %2';
								|en = 'The exchange plan name specified in the registration rules (%1) does not match the exchange plan name whose data is imported (%2)';");
...
Function InitMessages()
	...
	Messages.Insert(0, NStr("ru = 'Внутренняя ошибка';
								|en = 'Internal error';", DefaultLanguageCode));
	Messages.Insert(1, NStr("ru = 'Ошибка открытия файла правил';
								|en = 'Cannot open the exchange rules file.';", DefaultLanguageCode));
	Messages.Insert(2, NStr("ru = 'Ошибка при загрузке правил';
								|en = 'Cannot load the exchange rules.';", DefaultLanguageCode));
	Messages.Insert(3, NStr("ru = 'Ошибка формата правил';
								|en = 'Exchange rule format error';", DefaultLanguageCode));
	Messages.Insert(4, NStr("ru = 'Ошибка при получении файла правил для чтения';
								|en = 'Cannot get the exchange rules file.';", DefaultLanguageCode));
	Messages.Insert(5, NStr("ru = 'Загружаемые правила регистрации не предназначены для текущего плана обмена.';
								|en = 'The registration rules are not intended for the current exchange plan.';", DefaultLanguageCode));
