...
Procedure UseCloudSignatureServiceOnChange(Item)
	...
	If ConstantsSet.UseDSSService Then
		...
		ListOfCommands.Add("OK", NStr("ru = 'Подтвердить';
										|en = 'Confirm';"));
		ListOfCommands.Add("None", NStr("ru = 'Отмена';
											|en = 'Cancel';"), True);
		QueryText = NStr("ru = 'Внимание, если сервис подписи DSS будет использоваться для формирования квалифицированной электронной подписи, то нужно самостоятельно произвести настройки для выполнения <a href = ""%1"">требований</a>, предъявляемых к такой подписи.';
							|en = 'Attention. If you plan to use the DSS signature service to generate a qualified digital signature, set it up on your own to meet the <a href = ""%1"">requirements</a> for such signature.';") 
			+ Chars.LF + Chars.LF
			+ NStr("ru = 'Подтверждаете использование сервиса?';
					|en = 'Do you confirm the service use?';");
		...
		TheDSSCryptographyServiceModuleClient.OutputQuestion(
			...
			ListOfCommands,
			,
			NStr("ru = 'Требуются дополнительные настройки';
				|en = 'Additional settings are required';"));
...
Procedure InfobasePublicationURLStartChoiceCompletion(Var_AttributeName)
	...
	Else
		ShowMessageBox(, NStr("ru = 'Не удалось автоматически заполнить поле, т.к. клиентское приложение не подключено через веб-сервер.';
										|en = 'Cannot populate the field. The client application is not connected over the web server.';"));
	EndIf;
