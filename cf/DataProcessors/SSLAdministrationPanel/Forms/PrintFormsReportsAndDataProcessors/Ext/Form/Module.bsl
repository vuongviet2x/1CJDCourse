...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Common.SubsystemExists("StandardSubsystems.ReportMailing") Then
		...
		If Common.SubsystemExists("StandardSubsystems.Interactions") Then
			ToolTipText = NStr("ru = 'Позволяет узнать, когда, кому и с каким результатом были разосланы отчеты. Для ограничения объема хранимых данных
		...
		EndIf;
		NumberFormat_ = NStr("ru = '%Number% %OfMonths%';
							|en = '%Number% %OfMonths%';");
		NumberFormat_ = StrReplace(NumberFormat_, "%Number%", "Ch");
		NumberFormat_ = StrReplace(NumberFormat_, "%OfMonths%", NStr("ru = 'мес.';
																|en = 'months';"));
...
Function TitleOfTheTextTranslationServiceSettings(TextTranslationService)
	...
	If ValueIsFilled(TextTranslationService) Then
		Template = NStr("ru = 'Перевод текста с помощью <a href=""%1"">%2</a>';
						|en = 'Translate with <a href=""%1"">%2</a>';");
	Else
		Template = NStr("ru = 'Перевод текста с помощью внешнего сервиса';
