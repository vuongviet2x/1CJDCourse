	...
	If MessageReader.Sender = ExchangePlans.MasterNode() Then
		...
		Try
			...
			If ValueIsFilled(DuplicatesOfPredefinedItems) Then
				WriteLogEvent(
					NStr("ru = 'Предопределенные элементы.Найдены не уникальные записи.';
						|en = 'Predefined items.Duplicates';",
		...
		Except
			...
			ErrorMessage = NStr("ru = 'Ошибка при загрузке приоритетных данных: ""%1"" (тип %2)';
									|en = 'An error occurred when importing priority data: ""%1"" (type %2)';", Common.DefaultLanguageCode());
...
Procedure AddPredefinedItemDuplicateDetails(WrittenObject, DuplicatesOfPredefinedItems, Cancel, CancelDetails)
	...
	If WriteToLog Then
		If DuplicateCount = 1 Then
			Template = NStr("ru = '(загружаемая ссылка: %1, ссылка дубля: %2)';
							|en = '(reference to import: %1, duplicate reference: %2)';");
		Else
			Template = NStr("ru = '(загружаемая ссылка: %1, ссылки дублей: %2)';
							|en = '(reference to import: %1, duplicate references: %2)';");
		EndIf;
...
Function ErrorOpeningExchangeMessageFile()
	...
	Return NStr("ru = 'Ошибка открытия файла сообщения обмена';
				|en = 'Cannot open the exchange message file.';", Common.DefaultLanguageCode());
	
EndFunction

Function ErrorStartRedingTheExchangeMessageFile()
	
	Return NStr("ru = 'Ошибка при начале чтения файла сообщения обмена';
				|en = 'Cannot start reading the exchange message file.';", Common.DefaultLanguageCode());
	
EndFunction

Function ErrorStartWritingTheExchangeMessageFile()
	
	Return NStr("ru = 'Ошибка при начале записи файла сообщения обмена';
				|en = 'Cannot start saving the exchange message file';", Common.DefaultLanguageCode());
	
EndFunction

Function ErrorReadingExchangeMessageFile()
	
	Return NStr("ru = 'Ошибка чтения файла сообщения обмена';
				|en = 'Cannot read the exchange message file.';", Common.DefaultLanguageCode());
	
EndFunction

Function ErrorSavingExchangeMessageFile()
	
	Return NStr("ru = 'Ошибка записи данных в файл сообщения обмена';
				|en = 'Failed to save data to the exchange message file.';");
	
EndFunction

Function DataExchangeKindError()
	
	Return NStr("ru = 'Обмен не по правилам конвертации не поддерживается';
				|en = 'The exchange must follow the conversion rules.';", Common.DefaultLanguageCode());
