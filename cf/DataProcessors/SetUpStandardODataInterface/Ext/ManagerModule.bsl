...
Procedure WriteAuthorizationSettingsForStandardODataInterface(Val AuthorizationSettings) Export
	...
	If AuthorizationSettings.Used Then
		...
		Try
			...
			StandardODataInterfaceUser.Description = NStr("ru = 'Автоматический REST-сервис';
																		|en = 'Automatic REST service';");
			...
			Comment = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Записан пользователь для стандартного интерфейса OData с результатом: %1
					|Описание пользователя:
					|%2';
					|en = 'A user for the standard OData interface is saved with the result: %1
					|User details:
					|%2';"),
			...
			WriteLogEvent(
				NStr("ru = 'Настройка стандартного интерфейса OData.Запись пользователя';
					|en = 'Configure the standard OData interface.Save user';", Common.DefaultLanguageCode()),
		...
		Except
			...
			Comment = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Не удалось записать пользователя для стандартного интерфейса OData по причине:
					|%1
					|
					|Описание пользователя:
					|%2';
					|en = 'Cannot save a user for the standard OData interface due to:
					|%1
					|
					|User details:
					|%2';"),
...
Function ODataRoleCompositionErrors(ErrorsByObjects = Undefined) Export
	...
	If ExcessRights.Count() > 0 Then
		ErrorText = Chars.NBSp + NStr("ru = 'Следующие права избыточно включены в состав роли:';
										|en = 'The following rights are excessively included in the role:';") + Chars.LF + Chars.CR
			+ ExcessOrMissingRightsPresentation(ExcessRights, 2);
		Errors.Add(ErrorText);
	EndIf;
	
	If MissingRights.Count() > 0 Then
		ErrorText = Chars.NBSp + NStr("ru = 'Следующие права должны быть включены в состав роли:';
										|en = 'The following rights must be included in the role:';") + Chars.LF + Chars.CR
	...
	If TypeOf(ErrorsByObjects) = Type("Map") Then
		For Each KeyAndValue In ExcessRights Do
			...
			ErrorsByObjects.Insert(FullName, StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Следующие права на объект %1 избыточно включены в состав роли %2: %3';
					|en = 'The following rights to object %1 are redundantly included in role %2: %3';"),
		...
		For Each KeyAndValue In MissingRights Do
			...
			ErrorsText = ErrorsText + StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Следующие права на объект %1 должны быть включены в состав роли %2: %3';
					|en = 'The following rights to object %1 must be included in role %2: %3';"),
				FullName, Role.Name, StrConcat(KeyAndValue.Value, ", "));
...
Function StandardODataInterfaceUserProperties()
	...
	If Not AccessRight("DataAdministration", Metadata) Then
		Raise(NStr("ru = 'Недостаточно прав доступа для настройки автоматического REST-сервиса.';
								|en = 'Insufficient access rights to configure automatic REST service.';"),
...
Procedure CheckCanCreateUserForStandardODataInterfaceCalls()
	...
	If UsersCount = 0 Then
		Raise NStr("ru = 'Нельзя создать отдельные логин и пароль для использования автоматического REST-сервиса, т.к. в приложении отсутствуют другие пользователи.';
								|en = 'Cannot create a separate username and password for the automatic REST service, as there are no app users.';");
	EndIf;
