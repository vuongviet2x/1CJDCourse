...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	ElsIf Not Users.IsFullUser() Then
		Raise(NStr("ru = 'Недостаточно прав для открытия загрузки данных из файла';
								|en = 'Insufficient rights to import data from spreadsheets';"),
...
Procedure BeforeClose(Cancel, Exit, WarningText, StandardProcessing)
	...
	If Exit Then
		WarningText = NStr("ru = 'Введенные данные не будут записаны.';
									|en = 'The data that you entered will be lost.';");
		Return;
	EndIf;
		
	If Cancel Then
		Notification = New NotifyDescription("FormClosingCompletion1", ThisObject);
		QueryText = NStr("ru = 'Введенные данные не будут записаны. Закрыть форму?';
							|en = 'Changes are not saved. Close the form?';");
...
Procedure CancelMapping(Command)
	...
	ShowQueryBox(Notification, NStr("ru = 'Отменить сопоставление?';
									|en = 'Do you want to clear the mapping?';"), QuestionDialogMode.YesNo);
EndProcedure
...
Procedure StepBack()
	...
	If Items.WizardPages.CurrentPage = Items.FillTableWithData Then
		...
		Title = NStr("ru = 'Загрузка данных в справочник';
						|en = 'Import data to catalog';");
		ClearTable();
	...
	ElsIf Items.WizardPages.CurrentPage = Items.DataToImportMapping
		...
		Items.Next.Title = ?(ImportType = "PastingFromClipboard",
				NStr("ru = 'Вставить в список';
					|en = 'Add to list';"), NStr("ru = 'Далее >';
														|en = 'Next >';"));
...
Procedure ProceedToNextStepOfDataImport()
	...
	ElsIf Items.WizardPages.CurrentPage = Items.MappingResults Then
		Items.WizardPages.CurrentPage = Items.DataToImportMapping;
		Items.AddToList.Visible = False;
		Items.Next.Title = NStr("ru = 'Вставить в список';
										|en = 'Add to list';");
		Items.Next.DefaultButton = True;
		Items.Back.Title = NStr("ru = '< В начало';
										|en = '< To Beginning';");
	ElsIf Items.WizardPages.CurrentPage = Items.DataToImportMapping Then
		...
		If ImportType = "TabularSection" Then
			...
			If Rows.Count() > 0 Then
				...
				ShowQueryBox(Notification, NStr("ru = 'Строки, в которых не заполнены обязательные колонки, будут пропущены.';
												|en = 'Rows that contain empty required cells will be skipped.';")
					+ Chars.LF + NStr("ru = 'Продолжить?';
										|en = 'Do you want to continue?';"), QuestionDialogMode.YesNo);
				Return;
			EndIf;
		...
		ElsIf ImportType = "PastingFromClipboard" Then
			Items.Back.Title = NStr("ru = '< В начало_';
											|en = '< To Beginning';");
			CloseFormAndReturnRefArray();
		Else
...
Procedure RunMapping()
	...
	ShowUserNotification(NStr("ru = 'Выполнено сопоставление';
										|en = 'Mapping completed';"),, NStr("ru = 'Сопоставлено элементов:';
																				|en = 'Items mapped:';") + " " + String(ItemsMappedByColumnsCount));
	ShowMappingStatisticsImportFromFile();
EndProcedure
...
Procedure ShowMappingStatisticsImportFromFile()
	...
	AllText = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Все (%1)';
																			|en = 'All (%1)';"), Statistics.Total);
	
	Items.CreateIfUnmapped.Title = NStr("ru = 'Несопоставленные (';
															|en = 'Unmapped items (';") + Statistics.Incomparable + ")";
	Items.UpdateExistingItems.Title       = NStr("ru = 'Сопоставленные элементы (';
															|en = 'Mapped items (';") + String(Statistics.Mapped2) + ")";
	
	ChoiceList = Items.MappingTableFilter.ChoiceList;
	ChoiceList.Clear();
	ChoiceList.Add("All", AllText, True);
	ChoiceList.Add("Unmapped", StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Несопоставленные (%1 из %2)';
																											|en = 'Unmapped items (%1 out of %2)';"),
		Statistics.Incomparable, Statistics.Total));
	ChoiceList.Add("Mapped1", StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Сопоставленные (%1 из %2)';
																										|en = 'Mapped items (%1 out of %2)';"),
		Statistics.Mapped2, Statistics.Total));
	ChoiceList.Add("Ambiguous", StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Неоднозначные (%1 из %2)';
																										|en = 'Ambiguous items (%1 out of %2)';"),
		Statistics.Ambiguous1, Statistics.Total));
	
	If Statistics.Ambiguous1 > 0 Then
		Items.ConflictDetails.Visible = True;
		Items.ConflictDetails.Title = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = '(неоднозначностей: %1)';
																													|en = '(conflicts: %1)';"),
...
Procedure ExportTemplateToFileCompletion(Attached, AdditionalParameters) Export
	...
	If Attached Then
		...
		FileDialog.Filter                      = NStr("ru = 'Книга Excel 97 (*.xls)|*.xls|Книга Excel 2007 (*.xlsx)|*.xlsx|Электронная таблица OpenDocument (*.ods)|*.ods|Текстовый документ c разделителями (*.csv)|*.csv|Табличный документ (*.mxl)|*.mxl';
															|en = 'Excel Workbook 97 (*.xls)|*.xls|Excel Workbook 2007 (*.xlsx)|*.xlsx|OpenDocument Spreadsheet (*.ods)|*.ods|Comma-separated values file (*.csv)|*.csv|Spreadsheet document (*.mxl)|*.mxl';");
...
Procedure InsertFromClipboardInitialization()
	...
	If ValueIsFilled("FieldPresentation") Then
		Title = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Вставка из буфера обмена (%1)';
																				|en = 'Paste (%1)';"), Parameters.FieldPresentation);
	Else
		Title = NStr("ru = 'Вставка из буфера обмена';
						|en = 'Paste';");
	EndIf;
...
Procedure SetFormItemsVisibility()
	...
	Title = ?(IsBlankString(Parameters.Title), NStr("ru = 'Загрузка данных в справочник';
															|en = 'Import data to catalog';"), Parameters.Title);
	...
	If Common.IsWebClient() Then
		...
		|en = 'Select a catalog you want to import data from a spreadsheet file to.
		|';");
	EndIf;
	...
	If ImportType = "PastingFromClipboard" Then
		...
		Items.Close.Title = NStr("ru = 'Отмена';
											|en = 'Cancel';");
	ElsIf ImportType = "TabularSection" Then
		Items.WizardPages.CurrentPage = Items.FillTableWithData;
		Items.MappingSettingsGroup.Visible = False;
...
Procedure ExecuteStepFillTableWithDataAtServer(SelectionRowDetails)
	...
	EndIf;
	Title = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Загрузка данных в справочник ""%1""';
																			|en = 'Import data to catalog ""%1""';"), WindowTitle);
...
Procedure ExecuteDataToImportMappingStepAfterMapAtServer(ResultAddress)
	...
	If Result.Count() = MappingTable.Count() Then
		Explanation = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Данные сопоставлены. При необходимости возможно сопоставить данные вручную, заполнив колонку ""%1"".';
				|en = 'Data is mapped. If necessary, you can map the data manually by filling in the ""%1"" column.';"),
...
Procedure MapDataToImport()
	...
	If ImportType = "PastingFromClipboard" Then
		If IsBlankString(TemplateWithDataSingleColumn) Then
			ShowMessageBox(, (NStr("ru = 'Для вставки сопоставленных данных в список, заполните текстовое поле.';
											|en = 'To add mapped data to the list, fill in the text field.';")));
			Return;
		EndIf;
	...
	Else
		...
		If EmptyDataTable() Then
			...
			If ImportOption = 0 Then
				ShowMessageBox(, (NStr("ru = 'Для сопоставления и загрузки данных, заполните таблицу.';
												|en = 'To map and import data, fill in the table.';")));
			Else
				...
				|en = 'Cannot import data from the spreadsheet.
	...
	If UnfilledColumnsList.Count() > 0 Then
		If UnfilledColumnsList.Count() = 1 Then
			TextAboutColumns = NStr("ru = 'Обязательная колонка""';
									|en = 'Required column ""';") + " " + UnfilledColumnsList[0]
				+ NStr("ru = '"" содержит незаполненные строки, эти строки будут пропущены при загрузке.';
						|en = '"" contains blank cells. Rows with these cells will be skipped.';");
		Else
			TextAboutColumns = NStr("ru = 'Обязательные колонки""';
									|en = 'Required columns ""';") + " " + StrConcat(UnfilledColumnsList,", ")
				+ NStr("ru = '"" содержат незаполненные строки, эти строки будут пропущены при загрузке.';
						|en = '"" contain blank cells. Rows with these cells will be skipped.';");
		EndIf;
		TextAboutColumns = TextAboutColumns + Chars.LF + NStr("ru = 'Продолжить?';
...
Procedure ShowReport(ResultAddress)
	...
	Items.FilterReport.ChoiceList.Add("AllItems", NStr("ru = 'Все (';
																	|en = 'All (';") + Report.Total + ")");
	Items.FilterReport.ChoiceList.Add("New_Items", NStr("ru = 'Новые (';
															|en = 'New items (';") + Report.CreatedOn+ ")");
	Items.FilterReport.ChoiceList.Add("Updated2", NStr("ru = 'Обновленные (';
																	|en = 'Updated items (';") + Report.Updated3+ ")");
	Items.FilterReport.ChoiceList.Add("Skipped2", NStr("ru = 'Пропущенные (';
																	|en = 'Skipped items (';") + Report.Skipped3+ ")");
...
Procedure ExecuteDataToImportMappingStepClient()
	...
	If ImportType = "PastingFromClipboard" Then
		...
		If Statistics.Mapped2 > 0 Then
			TextFound = NStr("ru = 'Из %1 введенных строк в список будут вставлены: %2.';
								|en = '%2 out of %1 entered lines will be added to the list.';");
			...
			If Statistics.Ambiguous1 > 0 And Statistics.NotFound4 > 0 Then 
				...
				|en = 'Total rows skipped: %3. Details:
				| • No matches found: %1
			...
			ElsIf Statistics.Ambiguous1 > 0 Then
				TextNotFound = NStr("ru = 'Строки, для которых в приложении имеется несколько вариантов, будут пропущены: %1';
										|en = 'Rows that has multiple matches will be skipped: %1';");
			...
			ElsIf Statistics.NotFound4 > 0 Then
				TextNotFound = NStr("ru = 'Строки, для которых в приложении нет соответствующих данных, будут пропущены: %1';
										|en = 'Rows that has no matches will be skipped: %1';");
				TextNotFound = StringFunctionsClientServer.SubstituteParametersToString(TextNotFound, Statistics.NotFound4);
			EndIf;
			TextNotFound = TextNotFound + Chars.LF + NStr("ru = 'Для просмотра пропущенных строк и подбора данных для вставки нажмите ""Далее"".';
																|en = 'To view skipped rows and map them manually, click ""Next"".';");
			...
			ShowMappingStatisticsImportFromFile();
			SetAppearanceForMappingPage(False, Items.RefSearchNote, False, NStr("ru = 'Далее >';
																											|en = 'Next >';"));
		Else
			Items.WizardPages.CurrentPage = Items.NotFound4;
			Items.Close.Title = NStr("ru = 'Закрыть';
												|en = 'Close';");
			Items.Back.Visible = True;
			Items.AddToList.Visible = False;
	...
	Else 
		...
		If ImportType = "UniversalImport" Then
			SetAppearanceForMappingPage(True, Items.DataMappingNote, True, NStr("ru = 'Загрузить данные >';
																														|en = 'Import data >';"));
		ElsIf ImportType = "TabularSection" Then
			...
			SetAppearanceForMappingPage(False, Items.TabularSectionNote, True, NStr("ru = 'Загрузить данные';
																												|en = 'Import data';"));
			SetAppearanceForConflictFields(FilterConflict1);
		Else
			SetAppearanceForMappingPage(False, Items.AppliedImportNote, False, NStr("ru = 'Загрузить данные >';
																													|en = 'Import data >';"));
		EndIf;
	EndIf;
...
Procedure ExecuteDataImportReportStepClient()
	...
	Items.Close.Title = NStr("ru = 'Готово';
										|en = 'Finish';");
	Items.Next.Visible = False;
	Items.Back.Visible = False;
...
Procedure SetDataAppearance(ColumnsList = Undefined)
	...
	If ValueIsFilled(ColumnsList) Then
		...
		For Each ColumnName In ColumnsList Do
			...
			ConditionalAppearanceItem.Appearance.SetParameterValue("Text", NStr("ru = '<неоднозначность>';
																							|en = '<ambiguity>';"));
		EndDo;
	Else
		ConditionalAppearanceItem = ConditionalAppearance.Items.Add();
...
Function ObjectManager(MappingObjectName)
	...
	Else
		Raise StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Объект ""%1"" не найден';
																						|en = 'Object ""%1"" is not found.';"), MappingObjectName);
	EndIf;
...
Procedure ShowInfoBarAboutRequiredColumns()
	...
	If Items.FillWithDataPages.CurrentPage = Items.ImportFromFileOptionPage Then
		ToolTipText = NStr("ru = 'Для загрузки данных необходимо, сохранить бланк в файл для заполнения в другом приложении.
		...
		|en = 'Save the template to a file, open it in a spreadsheet editor, and enter the data.
	...
	Else
		ToolTipText = NStr("ru = 'Для заполнения таблицы скопируйте данные в таблицу из внешнего файла через буфер обмена.';
								|en = 'To fill in the table, copy and paste data to the table from an external file.';") + Chars.LF;
	EndIf;
	...
	If RequiredColumns2.Count() > 0 Then 
		...
		If RequiredColumns2.Count() = 1 Then
			ToolTipText = ToolTipText + NStr("ru = 'Колонка, обязательная для заполнения:';
													|en = 'Required column:';") + " " + ColumnsList;
		Else
			ToolTipText = ToolTipText + NStr("ru = 'Колонки, обязательные для заполнения:';
													|en = 'Required columns:';") + " " + ColumnsList;
		EndIf;
...
Procedure AddStandardColumnsToMappingTable(TemporarySpecification, MappingObjectStructure, AddID,
	...
	If AddID Then 
		TemporarySpecification.Columns.Add("Id", New TypeDescription("Number"), NStr("ru = 'п/п';
																						|en = '#';"));
	EndIf;
	...
	If AddRowMappingResult Then 
		TemporarySpecification.Columns.Add("RowMappingResult", New TypeDescription("String"), NStr("ru = 'Результат';
																										|en = 'Status';"));
	EndIf;
	If AddErrorDescription Then
		TemporarySpecification.Columns.Add("ErrorDescription", New TypeDescription("String"), NStr("ru = 'Причина';
																							|en = 'Reason';"));
	EndIf;
		...
		If StrStartsWith(Column.Name, TablePartPrefix) Then
			...
			If GroupColumns.Count() = 1 Then
				...
				If ColumnLevel2 <> Undefined Then 
					...
					If ColumnType <> Undefined And StrFind(ColumnType.FullName(), "Catalog") > 0 Then
						NewItem.Title = NStr("ru = 'Данные из файла';
														|en = 'File';");
					Else
						NewItem.Title = " ";
					EndIf;
			...
			ElsIf GroupColumns.Count() > 1 Then
				...
				Prefix = NStr("ru = 'Данные из файла:';
								|en = 'File:';");
	...
	If Result <> Undefined Then
		...
		If ValueIsFilled(SelectedFile.Name) Then
			...
			Else
				...
				Else
					ShowMessageBox(, NStr("ru = 'Шаблон файла не был сохранен.';
													|en = 'The file template is not saved.';"));
					Return;
				EndIf;
...
Function ImportFileWithDataToSpreadsheetDocumentAtServer(TempStorageAddress, Extension)
	...
	ExecutionParameters.BackgroundJobDescription = StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = 'Подсистема %1: Выполнение серверного метода загрузка данных из файла';
			|en = '%1 subsystem: import data from file using the server method';"), "ImportDataFromFile");
	ExecutionParameters.RefinementErrors =
		NStr("ru = 'Не удалось загрузить данные по причине:';
			|en = 'Could not import data due to:';");
...
Function MapDataToImportAtServerUniversalImport()
	...
	ExecutionParameters.BackgroundJobDescription =
		NStr("ru = 'Заполнение таблицы сопоставления загруженными данными из файла.';
			|en = 'Populate mapping table with data imported from file.';");
	ExecutionParameters.RefinementErrors =
		NStr("ru = 'Не удалось сопоставить данные по причине:';
			|en = 'Could not map data due to:';");
...
Function RecordDataToImportReportUniversalImport()
	...
	ExecutionParameters = TimeConsumingOperations.BackgroundExecutionParameters(UUID);
	ExecutionParameters.BackgroundJobDescription = NStr("ru = 'Запись загруженных данных из файла';
															|en = 'Save data imported from file';");
	ExecutionParameters.RefinementErrors = NStr("ru = 'Не удалось записать данные по причине:';
												|en = 'Could not save data due to:';");
...
Function GenerateReportOnImport(ReportType = "AllItems",  CalculateProgressPercent = False)
	...
	ExecutionParameters = TimeConsumingOperations.BackgroundExecutionParameters(UUID);
	ExecutionParameters.BackgroundJobDescription = NStr("ru = 'Создание отчета о загрузке данных из файла';
															|en = 'Create report on data import from file';");
