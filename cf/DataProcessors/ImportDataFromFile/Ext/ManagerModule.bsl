...
Procedure CreateCatalogsListForImport(CatalogsListForImport) Export
	...
	CatalogsToImport = CatalogsInformation();
	...
	For Each MetadataObjectForOutput In Metadata.Catalogs Do
		...
		If CanImportDataFromFile(MetadataObjectForOutput) Then
			...
			CatalogInformation = CatalogsToImport.Add();
			CatalogInformation.Presentation = MetadataObjectForOutput.Presentation();
	...
	For Each CatalogInformation In CatalogsToImport Do
		...
		TableRow = CatalogsInformation.Find(CatalogInformation.FullName, "FullName");
		...
		ElsIf TableRow.AppliedImport Then
			Continue;
		EndIf;
		
		TableRow.AppliedImport = CatalogInformation.AppliedImport; 
		...
		TableRow.ImportTypeInformation = ImportTypeInformation;
	...
	For Each CatalogInformation In CatalogsInformation Do 
		...
		Presentation = "";
		If CatalogInformation.AppliedImport And IsBlankString(CatalogInformation.Presentation) Then
...
// 
Function CatalogsInformation()
	
	StringType = New TypeDescription("String");
	
	Result = New ValueTable;
	Result.Columns.Add("FullName",          StringType);
	Result.Columns.Add("Presentation",      StringType);
	Result.Columns.Add("AppliedImport", New TypeDescription("Boolean"));
...
Function ExtendedCatalogsInformation()
	...
	Result = CatalogsInformation();
	Result.Columns.Add("ImportTypeInformation", StructureType);
...
Procedure SetInsertModeFromClipboard(TemplateWithData, ColumnsInformation, TypeDescription) Export
	...
	For Each Type In TypeDescription.Types() Do
		...
		ColumnTitle = NStr("ru = 'Введенные данные';
								|en = 'Entered data';");
...
Function DocumentByPresentation(Presentation, Types)
	...
	For Each Type In Types Do
		...
		If StrFind(Presentation, ItemPresentation) > 0 Then
			...
			PositionFrom = StrFind(Lower(PresentationNumberAndDate), NStr("ru = 'от';
																	|en = 'dated';"));
	...
	For Each Attribute In CatalogMetadata.Attributes Do
		...
		If Attribute.Type.ContainsType(Type("Boolean")) Then 
			ColumnTypeDetails = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Флаг, %1 или 1 / Нет или 0';
					|en = 'Check box, %1 or 1 / No or 0';"), ImportDataFromFile.PresentationOfTextYesForBoolean());
		ElsIf Attribute.Type.ContainsType(Type("Number")) Then 
			ColumnTypeDetails =  StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Число, Длина: %1, Точность: %2';
																								|en = 'Digit, Length: %1, Precision: %2';"),
		...
		ElsIf Attribute.Type.ContainsType(Type("String")) Then
			If Attribute.Type.StringQualifiers.Length > 0 Then
				...
				ColumnTypeDetails =  StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Строка, макс. количество символов: %1';
																									|en = 'String, length limit: %1';"), StringLength);
			Else
				ColumnTypeDetails = NStr("ru = 'Строка неограниченной длины';
											|en = 'String of unlimited length';");
			EndIf;
		...
		ElsIf Attribute.Type.ContainsType(Type("UUID")) Then
			ColumnTypeDetails = NStr("ru = 'Уникальный идентификатор';
										|en = 'UUID';");
		EndIf;
...
Procedure CreateStandardAttributesColumn(ColumnsInformation, CatalogMetadata, ColumnName, Position)
	...
	If DataType = Type("String") Then 
		TypePresentation = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Строка (не более %1 символов)';
																						|en = 'String (up to %1 characters)';"), TypeDetails.StringQualifiers.Length);
	...
	ElsIf DataType = Type("Number") Then
		TypePresentation = NStr("ru = 'Число';
								|en = 'Number';");
	Else
		...
		Else
			...
			If MetadataObject = Undefined Then
				ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Загрузка данных из файла в табличную часть не поддерживается для объекта с типом: %1';
			...
			If MetadataObject.Parent().Templates.Find(ImportParameters.Template) = Undefined Then
				ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Загрузка данных из файла в табличную часть не поддерживается, т.к. отсутствует макет %1 у объекта с типом: %2';
...
Procedure DetermineColumnsInformationTabularSection(Val ColumnsInformation, Template, TableHeaderArea, Val ImportParameters)
	...
	If MetadataObject = Undefined Then
		ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Загрузка данных из файла в табличную часть не поддерживается для объектов типа: %1';
				|en = 'Cannot import data from a file to a table for objects of the %1 type';"),
				MetadataObjectName);
		Raise ErrorText;
	EndIf;
	...
	Else
		Raise NStr("ru = 'Не найден макет для загрузки данных из файла';
								|en = 'Cannot find a template for importing data.';");
	EndIf;
		...
		Else
			...
			If ValueIsFilled(AttributeName) Then
				...
				NoteInTheColumnHeader = Cell.Comment.Text + ?(IsRequiredInfo,
					Chars.LF + NStr("ru = 'Обязательно для заполнения.';
										|en = 'Required.';"), "");
...
Function PredefinedLayoutAreas()
	...
	PredefinedLayoutAreas.Insert("AdditionalAttributes", NStr("ru = '<Дополнительные реквизиты>';
																			|en = '<Additional attributes>';"));
	PredefinedLayoutAreas.Insert("ContactInformation", NStr("ru = '<Контактная информация>';
...
Procedure ImportCSVFileToTable(FileName, TemplateWithData, ColumnsInformation)
	...
	If String = Undefined Then 
		MessageText = NStr("ru = 'Не получилось загрузить данные из этого файла. Убедитесь в корректности данных в файле.';
								|en = 'Cannot import data from the file. The data may be corrupt.';");
		Raise MessageText;
	EndIf;
	...
	For Each TableRow In MappedData Do
		...
		Try
			If ValueIsFilled(TableRow.MappingObject) Then
				...
				If CatalogItem = Undefined Then
					Raise StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Номенклатура с артикулом %1 не существует.';
																									|en = 'Product with product ID %1 does not exist.';"),
					TableRow.SKU);
				EndIf;
			Else
		...
		Except
			...
			NStr("ru = 'Не удалось записать элемент справочника %1 по причине:
				|%2';
				|en = 'Couldn''t save the item of the ""%1"" catalog. Reason:
				|%2';"), 
...
Procedure GenerateReportTemplate(TableReport, TemplateWithData)
	...
	FillTemplateHeaderCell(Cell, NStr("ru = 'Результат';
												|en = 'Status';"), 12, NStr("ru = 'Результат загрузки данных';
																				|en = 'Data import result';"), True);
...
Function ImportStatusPresentation(Status)
	...
	If Status = "Created" Then
		Return NStr("ru = 'Создан';
					|en = 'Created';");
	ElsIf Status = "Updated" Then
		Return NStr("ru = 'Обновлен';
					|en = 'Updated';");
	ElsIf Status = "Skipped" Then
		Return NStr("ru = 'Пропущен';
					|en = 'Skipped';");
	EndIf;
...
Function EventLogEvent() 
	...
	Return NStr("ru = 'Загрузка данных из файла';
				|en = 'Import data from spreadsheet';", Common.DefaultLanguageCode());
