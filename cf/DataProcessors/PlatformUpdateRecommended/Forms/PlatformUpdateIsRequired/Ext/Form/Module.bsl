...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Not Common.FileInfobase()
		...
		Refinement = NStr("ru = 'Для этого обратитесь к администратору.';
	...
	If Not Common.IsLinuxClient()
		...
		If ValueIsFilled(PlatformDirectory) Then
			...
			ClarificationRestart = Chars.LF + Chars.LF + NStr("ru = 'Перезапустите приложение на подходящей версии платформы (кнопка <b>Перезапустить</b>).';
																|en = 'Run the app on the supported 1C:Enterprise version (click <b>Restart</b>).';");
		EndIf;
	EndIf;
