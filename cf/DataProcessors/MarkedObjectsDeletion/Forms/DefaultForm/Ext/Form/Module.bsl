...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Not Users.IsFullUser() Then
		ErrorText = NStr("ru = 'Недостаточно прав для выполнения операции.';
	...
	If Common.DataSeparationEnabled() And Not Common.SeparatedDataUsageAvailable() Then
		ErrorText = NStr("ru = 'Для удаления помеченных войдите в область данных.';
							|en = 'To delete marked objects, log in to the data area.';");
		Return; // Cancel is set in OnOpen.
	EndIf;
...
Procedure BeforeClose(Cancel, Exit, WarningText, StandardProcessing)
	...
	If ShowDialogBeforeClose Then
		...
		QueryText = NStr("ru = 'Выполняется %1.
							|Прервать?';
							|en = 'In progress %1.
							|Do you want to stop it?';");
		Buttons = New ValueList;
		Buttons.Add(DialogReturnCode.Abort);
		Buttons.Add(DialogReturnCode.Ignore, NStr("ru = 'Не прерывать';
															|en = 'Do not stop';"));
...
Procedure TotalObjectsToSelect(Command)
	...
	If ActionsTable.Count() > 0 Then
		ShowQueryBox(Notification, 
			NStr("ru = 'Не завершено удаление объектов.
			|Вернуться к списку помеченных на удаление?';
			|en = 'Object deletion is not completed.
...
Procedure NotDeletedItemRelationsActionChoiceProcessing(Item, ValueSelected, StandardProcessing)
	...
	ElsIf ValueSelected = "Delete" And Not SelectedRow.ReferenceType Then
		ShowMessageBox(, StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Нельзя удалить выбранное значение: %1';
				|en = 'Cannot delete selected value: %1';"),
...
Procedure ExecuteActionsAndDelete(Command)
	...
	If SelectedCountTotal = 0 Then
		WarningText = NStr("ru = 'Необходимо выбрать хотя бы один элемент для удаления.';
									|en = 'Select at least one item to be deleted.';");
...
Procedure InstallDelete(Command)
	...
	ElsIf NonReferenceValues.Count() > 1 And SelectedTableRows.Count() > 0 Then
		ShowMessageBox(, StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Нельзя удалить все выбранные значения (%1 из %2).';
				|en = 'Cannot delete some of the selected values (%1 out of %2).';"),
			NonReferenceValues.Count(), TableRowsCount));
 	ElsIf NonReferenceValues.Count() > 1 Then
		ShowMessageBox(, StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Нельзя удалить выбранные значения (%1 шт.).';
				|en = 'Cannot delete the selected values (%1).';"),
...
Procedure SetReplaceWith(Command)
	...
	If CurrentData = Undefined Then
		Status(NStr("ru = 'Для выбранной строки нельзя указать действие.';
						|en = 'Cannot specify an action for the selected row.';"),,, PictureLib.DialogExclamation);
		Return;
	EndIf;
...
Procedure DeleteSelectedItems(Command)
	...
	SelectedCountTotal = InformationAboutTheSelectedObjects.SelectedCount;
	If SelectedCountTotal = 0 Then
		QueryText = NStr("ru = 'Необходимо выбрать хотя бы один элемент для удаления.';
							|en = 'Select at least one item to be deleted.';");
		ShowMessageBox(, QueryText);
		Return;
	ElsIf SelectedCountTotal = 1 Then
		QueryText = NStr("ru = 'Удалить элемент, помеченный на удаление?';
							|en = 'Delete the item marked for deletion?';");
	ElsIf SelectedCountTotal = InformationAboutTheSelectedObjects.TotalCount1 Then
		QueryText = NStr("ru = 'Удалить все элементы, помеченные на удаление?';
							|en = 'Delete all items marked for deletion?';");
	Else
		QueryText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Удалить элементы, помеченные на удаление (%1)?';
				|en = 'Delete the items marked for deletion (%1)?';"),
			SelectedCountTotal);
	EndIf;
...
Procedure SetClearDeletionMark(Command)
	...
	Else
		If SetATag Then
			QueryText = NStr("ru = 'Пометить выделенные объекты (%1) на удаление?';
								|en = 'Mark the selected objects (%1) for deletion?';");
		Else
			QueryText = NStr("ru = 'Снять с выделенных объектов (%1) пометку на удаление?';
								|en = 'Clear the deletion mark from the selected objects (%1)?';");
		EndIf;
...
Procedure SetClearDeletionMarkFollowUp(Result, ReferencesToProcess) Export
	...
	Except
		...
		ErrorText = NStr("ru = 'Не удалось изменить пометку удаления по причине:';
							|en = 'Cannot change the deletion mark due to:';")
			+ Chars.LF + ErrorText;
		ShowMessageBox(, ErrorText);
	EndTry;
...
Function SetUncheckDeleteOnTheServer(References)
	...
	Except
		WriteLogEvent(
			NStr("ru = 'Удаление помеченных.Установка / снятие пометки удаления';
				|en = 'Delete marked objects.Toggle deletion mark';", Common.DefaultLanguageCode()),
	...
	For Each TableRowID_ In TableRowsIDs Do
		...
		Else
			If IsBlankString(MessageText) Then
				MessageText = NStr("ru = 'Для места использования %1 нельзя выбрать действие %2.';
										|en = 'Object %1 does not support action ""%2"".';");
...
Procedure ToOpenTheFormCompleteTheUserExperience()
	...
	If CommonClient.SubsystemExists("StandardSubsystems.UsersSessions") Then
		...
		FormParameters.Title = NStr("ru = 'Не удалось удалить помеченные объекты';
										|en = 'Cannot delete marked objects';");
		FormParameters.ErrorMessageText = NStr("ru = 'Невозможно выполнить удаление помеченных объектов, т.к. в приложении работают другие пользователи:';
													|en = 'Cannot delete the marked objects because other users are logged in:';");
		FormParameters.ErrorTextExitFailed = NStr("ru = 'Невозможно выполнить удаление помеченных объектов, т.к. не удалось завершить работу пользователей:';
																	|en = 'Cannot delete the marked objects because the following users are still logged in:';");
		FormParameters.ShouldCloseAllSessionsButCurrent = True;
		FormParameters.LoginMessage = NStr("ru = 'Работа с приложением временно недоступна для удаления помеченных объектов.';
															|en = 'The app is temporarily unavailable while deleting objects marked for deletion.';");
...
Procedure SetConditionalAppearance()
	...
	AppearanceItem.Appearance.SetParameterValue("Text", NStr("ru = 'Выберите действие';
																			|en = 'Select an action';"));
...
Procedure SetUsageInstancesActionsList(Form)
	...
	Form.NotDeletedItemsUsageInstancesActions.Add("Delete", NStr("ru = 'Удалить';
																		|en = 'Delete';"));
	Form.NotDeletedItemsUsageInstancesActions.Add("ReplaceRef", NStr("ru = 'Заменить %PresentationRef% на';
																				|en = 'Replace %PresentationRef% with';"));
...
Procedure FillNotDeletedObjectsUsageInstances(Val ActionsTable = Undefined)
	...
	If TreeRow = Undefined Or TreeRow.PictureNumber < 1 Then
		...
		ErrorText = NStr("ru = 'Выберите объект, чтобы узнать причину,
			|по которой его не удалось удалить.';
			|en = 'Select an object to view the reason
...
Function ReplaceWithCommandPresentation(CurrentData, Parameter = Undefined)
	...
	If Parameter <> Undefined Then
		Result = StrReplace(NStr("ru = 'Заменить %1 на %2';
									|en = 'Replace %1 with %2';"), "%1", UpperLevelRowPresentation);
		Return StrReplace(Result, "%2", Parameter);
	Else
		Return StrReplace(NStr("ru = 'Заменить %1 на...';
								|en = 'Replace %1 with…';"), "%1", UpperLevelRowPresentation);
	EndIf;
...
Function ViewOfTheDeleteCommand(CurrentData)
	Presentation = NStr("ru = 'Удалить';
						|en = 'Delete';") + Chars.NBSp + CurrentData.Presentation;
	Return Presentation;
EndFunction
...
Function MetadataFilterPresentation(FilterValue)
	...
	Return ?(IsBlankString(FilterPresentation), NStr("ru = 'Все помеченные на удаление';
														|en = 'All objects marked for deletion';"), FilterPresentation);
EndFunction
...
Procedure StartMarkedObjectsSearch(SearchForTechnologicalObjects)
	...
	PresentationOperation = NStr("ru = 'Поиск помеченных на удаление';
								|en = 'Search for objects marked for deletion';");
	ToStartSearchingForTheMarkedSettingOfTheForm(ThisObject);
...
Procedure ToStartSearchingForTheMarkedSettingOfTheForm(Form)
	...
	Form.Items.LongRunningOperationPresentationDecoration.Title = NStr("ru = 'Выполняется поиск помеченных на удаление...';
...
Function StartMarkedObjectsSearchServer(MetadataFilter, FormUniqueID, SearchForTechnologicalObjects = False)
	...
	MethodParameters.BackgroundJobDescription = NStr("ru = 'Поиск помеченных на удаление';
														|en = 'Search for objects marked for deletion';");
...
Procedure FillBackgroundJobErrorInfo(Form, Result)
	...
	Form.Items.CompletionPresentationFailedDecoration.Title = 
		?(IsExclusiveModeSettingError(Result.ErrorInfo),
			NStr("ru = 'Не удалось установить монопольный режим';
				|en = 'Couldn''t set exclusive mode';"),
...
Procedure StartMarkedObjectsDeletion(Parameter = Undefined)
	PresentationOperation = NStr("ru = 'Удаление помеченных';
								|en = 'Delete marked objects';");
	Items.StatePresentationPages.CurrentPage = Items.RunningState;
	Items.LongRunningOperationPresentationDecoration.Title = NStr("ru = 'Выполняется удаление помеченных...';
																		|en = 'Deleting objects marked for deletion…';");
	SetObjectsMarkedForDeletionSelectionStateWithStatePanel();
...
Function StartMarkedObjectsDeletionServer(FormUniqueID, PreviousStepResult, RepeatSearch = False)
	...
	ExecutionParameters.BackgroundJobDescription = NStr("ru = 'Удаление помеченных объектов';
...
Procedure ProcessDeletionExecutionResult(DeletionResultsInfo)
	...
	ElsIf DeletionResultsInfo.DeletedItemsCount1 > 0 Then
		...
		Items.CompletionPresentationSuccessDecoration.Title = NStr("ru = 'Удаление успешно завершено.';
...
Procedure StartAdditionalDataProcessorExecution(Parameter)
	...
	PresentationOperation = NStr("ru = 'Дополнительная обработка мешающих удалению объектов';
								|en = 'Additional processing of objects preventing deletion';");
	SetStateUnsuccessfulDeletionWithStatePanel();
	
	Items.StatePresentationPages.CurrentPage = Items.RunningState;
	Items.LongRunningOperationPresentationDecoration.Title = NStr("ru = 'Выполняется дополнительная обработка причин неудаления...';
																		|en = 'Additional processing of reasons preventing deleting…';");
	Items.CommandBarForm.Enabled = False;
	Items.InformationPages.ReadOnly = True;
...
Function StartAdditionalDataProcessorExecutionServer()
	...
	MethodParameters.BackgroundJobDescription = NStr("ru = 'Дополнительная обработка мешающих удалению объектов';
														|en = 'Additional processing of objects preventing deletion';");
...
Function IsExclusiveModeSettingError(ErrorInfo)
	If ErrorInfo = Undefined Then
		Return False;
	EndIf;
	ErrorTextExclusive = NStr("ru = 'Ошибка разделенного доступа к базе';
								|en = 'Error of separated infobase access';");
