...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	Except
		...
		Raise NStr("ru = 'В информационной базе не поддерживается синхронизация данных через универсальный формат';
								|en = 'The infobase does not support universal data synchronization format.';");
	EndTry;
	...
	If FormOpenOption = "ImportOnly" Then
		ThisObject.Title = NStr("ru = 'Загрузка данных EnterpriseData';
	...
	Else
		...
		ThisObject.Title = NStr("ru = 'Выгрузка и загрузка данных EnterpriseData';
	...
	If OperationKind = "Load" Then
		If ImportSource = 1 Then // A text field.
			If Not ValueIsFilled(DataForXDTOImport) Then
				CommonClient.MessageToUser(NStr("ru = 'Текстовое поле с данными для загрузки не заполнено.';
...
Procedure AbortExportImport(Command)
	...
	Message.Text = NStr("ru = 'Выполнение операции прервано.';
							|en = 'The operation is canceled.';");
	Message.Message();
...
Procedure ShouldSaveSettings(Command)
	...
	FileSystemClient.SaveFile(
		...
		NStr("ru = 'Файл настроек выгрузки.xml';
			|en = 'Export settings file.xml';"),
		SavingParameters);
...
Procedure ImportMessage()
	...
	Items.TimeConsumingOperationNoteTextDecoration.Title = NStr("ru = 'Выполняется загрузка данных...';
																		|en = 'Importing data…';");
...
Procedure OnCompleteImport(Result, AdditionalParameters) Export
	If Result = Undefined Then
		Raise NStr("ru = 'Загрузка данных завершена неудачно.';
								|en = 'Cannot import data.';");
...
Procedure ExportData()
	Items.TimeConsumingOperationNoteTextDecoration.Title = NStr("ru = 'Выполняется выгрузка данных...';
																		|en = 'Exporting data…';");
	Items.ExportImport.CurrentPage = Items.Waiting;
	SetVisibilityAvailabilityOfButtons(False);
	ArrayOfMessages = Undefined;
...
Procedure OnCompleteExport(Result, AdditionalParameters) Export
	If Result = Undefined Then
		Raise NStr("ru = 'Выгрузка данных завершена неудачно. Отсутствует результат выгрузки.';
								|en = 'Cannot export data. The export result is missing.';");
	ElsIf Result.Status = "Error" Then
		Raise Result.BriefErrorDescription;
	EndIf;
...
Procedure SelectFileForImportAtClient(ImportAfterChoice = False)
	...
	OpenFileDialog.Filter = NStr("ru = 'Данные загрузки (*.xml)|*.xml';
...
Procedure SelectFileForExportAtClient(ExportAfterChoice = False, ResultStorageAddress = "")
	...
	OpenFileDialog.Filter = NStr("ru = 'Данные выгрузки (*.xml)|*.xml';
...
Function ExportDataAtServer()
	...
	ExecutionParameters.BackgroundJobDescription = NStr("ru = 'Выгрузка данных EnterpriseData';
...
Function ImportDataAtServer()
	...
	ModuleTimeConsumingOperations = Common.CommonModule("TimeConsumingOperations");
	ExecutionParameters = ModuleTimeConsumingOperations.BackgroundExecutionParameters(UUID);
	ExecutionParameters.BackgroundJobDescription = NStr("ru = 'Загрузка данных EnterpriseData';
															|en = 'Import EnterpriseData data';");
	BackgroundJobResult = ModuleTimeConsumingOperations.ExecuteInBackground(MethodToExecute, JobParameters, ExecutionParameters);
	Return BackgroundJobResult;
	...
	For Each MetadataGroupString In Object.ExportRulesTable.GetItems() Do
		For Each MetadataString In MetadataGroupString.GetItems() Do
			...
			ElsIf AdditionStrings.Count() = 0 Then
				...
				NewString.FilterAsString = NStr("ru = 'Все объекты';
												|en = 'All objects';");
...
Procedure ManagerModuleStartChoice(ManagerModuleAttribute, StandardProcessing, UpdateExport)
	...
	OpenFileDialog = New FileDialog(FileDialogMode.Open);
	OpenFileDialog.Filter = NStr("ru = 'Внешняя обработка (*.epf)|*.epf';
...
Procedure RefreshExportRulesAtServer()
	...
	Items.FormatExtension.ChoiceList.Add("", NStr("ru = '<без расширения>';
																|en = '<without extension>';"));
...
Procedure UpdateTheUploadRulesOnTheServer()
	...
	Items.FormatExtensionImport.ChoiceList.Add("", NStr("ru = '<без расширения>';
																		|en = '<without extension>';"));
	For Each Extension In FormatExtensions.FindRows(New Structure("BaseVersion", Object.FormatVersion)) Do
...
Procedure ReportOperationEnd(Load = False)
	...
	Message = New UserMessage();
	If Load Then
		Message.Text = NStr("ru = 'Загрузка данных завершена.';
								|en = 'Data import completed.';");
	Else
		Message.Text = NStr("ru = 'Выгрузка данных завершена.';
...
Function ComplianceOfSKDSelections()
	...
	MatchingSelections.Insert("In group from_ list0", DataCompositionComparisonType.InListByHierarchy);
	...
	MatchingSelections.Insert("Not In group from_ list0", DataCompositionComparisonType.NotInListByHierarchy);
...
Procedure FillInTheDefaultFormatVersion(AvailableVersionsArray)
	...
	If AvailableVersionsArray.Count() = 0 Then
		...
		StringSupportedFormatVersions = NStr("ru = 'Не обнаружены поддерживаемые версии формата';
												|en = 'No supported format versions are found.';", Common.DefaultLanguageCode());
	...
	Else
		...
		StringSupportedFormatVersions = StrTemplate(NStr("ru = 'Поддерживаемые версии формата: %1';
															|en = 'Supported format versions: %1';"), VersionsListPresentation);
