...
Function ExchangeComponents(ExchangeDirection, ExchangeFormatVersionOnImport1 = "", FormatExtensionOnImport = "") 
	...
	EndIf;
	ExchangeComponents.EventLogMessageKey = NStr("ru = 'Перенос данных через буфер обмена';
															|en = 'Clipboard data transfer';", Common.DefaultLanguageCode());
	...
	If ExchangeManagerInternal Then
		...
		If ExchangeComponents.ExchangeManager = Undefined Then
			Raise StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Не поддерживается версия формата обмена: <%1>.';
																							|en = 'The exchange format version is not supported: %1.';"), CurrFormatVersion);
		EndIf;
	EndIf;
...
Function ExportToXMLResult(ParametersStructure, AddressToPlaceResult = Undefined) Export
	...
	If ExportResult.HasErrors Then
		MessageText = NStr("ru = 'В ходе выполнения операции возникли ошибки';
								|en = 'Operation execution errors';") + ": "
			+ Chars.LF + ExportResult.ErrorText
			+ Chars.LF + NStr("ru = 'Данные могут быть выгружены не полностью.';
								|en = 'Some data might not be exported.';");
		Common.MessageToUser(MessageText);
	ElsIf Not ExportResult.HasExportedObjects Then
		MessageText = NStr("ru = 'Не найдено ни одного объекта к выгрузке.';
								|en = 'There are no objects to export.';");
		Common.MessageToUser(MessageText);
	EndIf;
...
Function ExportDataToXML(ExchangeFileName = "")
	...
	If ListExportAddition.Count() > 0 Then
		...
		EventStart = NStr("ru = 'ВыгрузкаЗагрузкаEnterpriseData.Начало';
							|en = 'ExportImportEnterpriseData.Start';", Common.DefaultLanguageCode());
		EventEnd = NStr("ru = 'ВыгрузкаЗагрузкаEnterpriseData.Окончание';
								|en = 'ExportImportEnterpriseData.End';", Common.DefaultLanguageCode());
...
Function ImportDataFromXML(XMLReader)
	...
	If XMLReader.NodeType <> XMLNodeType.StartElement
		...
		ImportResult1.ErrorText = NStr("ru = 'Ошибка чтения сообщения загрузки. Неверный формат сообщения.';
											|en = 'Cannot read the import message. Invalid message format.';");
...
Procedure FillExportRules() Export
	...
	For Each DPRRow In ExchangeComponents.DataProcessingRules Do
		...
		If Metadata.Catalogs.Contains(CurrMetadata) Then
			...
			Presentation = NStr("ru = 'Справочник %1';
								|en = '""%1"" catalog';");
		ElsIf Metadata.Documents.Contains(CurrMetadata) Then
			...
			Presentation = NStr("ru = 'Документ %1';
								|en = '""%1"" document';");
		ElsIf  Metadata.ChartsOfCharacteristicTypes.Contains(CurrMetadata) Then
			...
			Presentation = NStr("ru = 'План видов характеристик %1';
								|en = '""%1"" chart of characteristic types';");
		Else
...
Procedure AddTabularSectionCompositionAdditionalSets(SourceItems, AddingOptions)
	...
	If ObjectMetadata = Undefined Then
		Raise StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Некорректное имя метаданных ""%1""';
																						|en = 'Invalid metadata name: ""%1"".';"),
				NameOfTableToAdd);
	EndIf;
...
Function ParseExchangeFormat(Val ExchangeFormat)
	...
	If FormatItems.Count() = 0 Then
		Raise StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Неканоническое имя формата обмена <%1>';
																						|en = 'Noncanonical exchange format name: ""%1"".';"), ExchangeFormat);
	EndIf;
	...
	If Versions.Count() = 0 Then
		Raise StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Неканоническое представление версии формата обмена: <%1>.';
																						|en = 'Non-canonical presentation of the exchange format v.%1.';"), Result.Version);
	EndIf;
