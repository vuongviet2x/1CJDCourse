...
Procedure UserRefOnChange(Item)
	...
	If UserRef <> Undefined
		...
		If SettingsRecipientsUsers.UsersArray.Find(UserRef) <> Undefined Then
			ShowMessageBox(,NStr("ru = 'Нельзя копировать настройки пользователя самому себе,
					...
					|en = 'Cannot copy user settings to the source user.
	...
	SettingsCount = 0;
	Items.SelectSettings.Title = NStr("ru = 'Выбрать';
												|en = 'Select';");
...
Procedure Copy(Command)
	...
	If UserRef = Undefined Then
		CommonClient.MessageToUser(
			NStr("ru = 'Выберите пользователя, чьи настройки необходимо скопировать.';
				|en = 'Select the source user.';"), , "UserRef");
		Return;
	EndIf;
	
	If UsersCount = 0 And CopySettingsToRadioButtons <> "ToAllUsers" Then
		CommonClient.MessageToUser(
			NStr("ru = 'Выберите одного или несколько пользователей, которым необходимо скопировать настройки.';
				|en = 'Select one or several destination users.';"), , "Receiver");
		Return;
	EndIf;
	
	If SettingsToCopyRadioButton = "CopySelectedSettings1" And SettingsCount = 0 Then
		CommonClient.MessageToUser(
			NStr("ru = 'Выберите настройки, которые необходимо скопировать.';
				|en = 'Select the settings to copy.';"), , "SettingsToCopyRadioButton");
		Return;
	EndIf;
	...
	For Each OpenWindow In OpenWindows Do
		...
		If StrFind(AllSettingsToCopyRow, OpenFormName) > 0 Then
			OpenFormsRow = ?(ValueIsFilled(OpenFormsRow),
				...
				NStr("ru = 'Открытые окна';
					|en = 'The open windows';") + ":" + Chars.LF + "- " + OpenWindow.Caption)
		EndIf;
...
Procedure CopySettings(CommandName)
	...
	Else
		SettingsCopiedToNote = NStr("ru = 'всем пользователям';
												|en = 'all users';");
	EndIf;
	...
	Else
		...
		If Not SettingsCopied Then
			...
			ShowMessageBox(, StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Настройки не были скопированы, так как у пользователя ""%1"" не было сохранено ни одной настройки.';
					|en = 'The settings were not copied because user ""%1"" does not have any saved settings.';"),
				String(UserRef)));
			Return;
		EndIf;
