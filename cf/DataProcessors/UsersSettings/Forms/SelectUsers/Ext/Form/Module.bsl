...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If SettingsClearing Then
		Title =
			NStr("ru = 'Выбор пользователей для очистки настроек';
				|en = 'Select users to clear settings';");
		Items.Label.Title =
			NStr("ru = 'Выберите пользователей, которым необходимо очистить настройки';
				|en = 'Select the users whose settings you want to clear:';");
	EndIf;
...
Procedure Select(Command)
	...
	If UsersDestination.Count() = 0 Then
		ShowMessageBox(,NStr("ru = 'Отметьте одного или несколько пользователей.';
									|en = 'Select one or several users.';"));
		Return;
	EndIf;
	...
	If MarkedUser <> Undefined And CheckMarkValue <> UserListRow.Check Then
		...
		UsersGroup.GroupDescriptionAndUserMarkCount = 
			StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = '%1 (%2)';
																		|en = '%1 (%2)';"), String(UsersGroup.Group), UsersGroup.MarkedUsersCount);
	EndIf;
