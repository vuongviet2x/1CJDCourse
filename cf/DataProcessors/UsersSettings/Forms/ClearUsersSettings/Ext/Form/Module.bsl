	...
	If SettingsToClearRadioButton = "SelectedSettings2" Then
		If UsersToClearSettingsRadioButtons <> "SelectedUsers1"
		 Or UsersCount <> 1 Then
			...
			ShowMessageBox(,NStr("ru = 'Очистка отдельных настроек доступна только при выборе одного пользователя.';
										|en = 'To clear individual settings, select one user.';"));
		Else
...
Procedure UpdateUsersSelectionRef(Form)
	...
	If Form.UsersCount = 0 Then
		Form.Items.SelectSettings.Title = NStr("ru = 'Выбрать';
														|en = 'Select';");
...
Function SettingsClearing()
	...
	If UsersToClearSettingsRadioButtons = "SelectedUsers1" Then
		...
		If UsersCount = 1 Then
			SettingsClearedForNote = NStr("ru = 'пользователя ""%1""';
													|en = 'user ""%1""';");
	...
	If SettingsToClearRadioButton = "SelectedSettings2"
		...
		CommonClient.MessageToUser(
			NStr("ru = 'Выберите настройки, которые необходимо очистить.';
				|en = 'Select the settings that you want to clear.';"), , "SettingsToClearRadioButton");
		Return False;
	EndIf;
	...
	If SettingsToClearRadioButton = "SelectedSettings2" Then
		...
		If SettingsCount = 1 Then
			...
			ExplanationText = NStr("ru = '""%1"" очищена у %2';
									|en = '""%1"" cleared for %2';");
		...
		Else
			...
			ExplanationText = NStr("ru = 'Очищено %1 у %2';
									|en = '%1 cleared for %2';");
	...
	ElsIf SettingsToClearRadioButton = "ClearAll" Then
		...
		ExplanationText = NStr("ru = 'Очищены все настройки %1';
								|en = 'All settings are cleared for %1';");
	...
	ElsIf SettingsToClearRadioButton = "ObsoleteSettings" Then
		...
		ExplanationText = NStr("ru = 'Очищены устаревшие настройки %1';
								|en = 'Obsolete settings %1 are cleared';");
		ExplanationText = StringFunctionsClientServer.SubstituteParametersToString(ExplanationText, SettingsClearedForNote);
		ShowUserNotification(NStr("ru = 'Очистка настроек';
											|en = 'Clear settings';"), , ExplanationText, PictureLib.DialogInformation);
	EndIf;
