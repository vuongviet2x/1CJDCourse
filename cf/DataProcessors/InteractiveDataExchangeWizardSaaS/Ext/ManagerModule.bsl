...
Procedure OnWaitForExportData(HandlerParameters, ContinueWait) Export
	...
	Else
		...
		If Not ContinueWait
				...
				If InfoBaseAdmParams.NodeExists Then
					If InfoBaseAdmParams.Property("DataSynchronizationSetupCompleted")
						...
						HandlerParameters.ErrorMessage = StringFunctionsClientServer.SubstituteParametersToString(
							NStr("ru = 'Перед запуском обмена данными завершите настройку синхронизации в программе ""%1"".';
								|en = 'Before starting the data exchange, finish the synchronization setup in ""%1"" application.';"),
					...
					If InfoBaseAdmParams.Property("MessageReceivedForDataMapping")
						...
						HandlerParameters.Cancel = True;
						HandlerParameters.ErrorMessage = StringFunctionsClientServer.SubstituteParametersToString(
							NStr("ru = 'Перед запуском обмена данными завершите сопоставление данных в программе ""%1"".';
								|en = 'Before starting the data exchange, finish data mapping in ""%1"" application.';"),
							String(ExportSettings1.Peer));
						ContinueWait = False;
						Return;
					EndIf;
			...
			BackgroundJobKey = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Обмен данными (%1)';
					|en = 'Data exchange (%1)';"),
				ExportSettings1.Peer);

			If HasActiveBackgroundJobs(BackgroundJobKey) Then
				Raise StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Обмен данными с ""%1"" уже выполняется.';
						|en = 'Data exchange with ""%1"" is already in progress.';"),
					ExportSettings1.Peer);
			EndIf;
