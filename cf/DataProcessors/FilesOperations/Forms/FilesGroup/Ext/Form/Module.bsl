...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	Else
		...
		Else
			Raise NStr("ru = 'Не предусмотрено непосредственное создание группы файлов.';
...
Procedure BeforeClose(Cancel, Exit, WarningText, StandardProcessing)
	...
	If Modified Then
		...
		ShowQueryBox(ResponseNotification, NStr("ru = 'Данные были изменены. Сохранить изменения?';
...
Procedure SetUpFormObject(Val NewObject)
	...
	CreatedStatus = StringFunctions.FormattedString(NStr("ru = '<a href=""%1"">%2</a>';
																|en = '<a href=""%1"">%2</a>';"),
...
Procedure RefreshTitle()
	...
	If ValueIsFilled(CurrentRefToFile) Then
		Title = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = '%1 (Группа файлов)';
				|en = '%1 (File group)';"), String(CurrentRefToFile));
	Else
		Title = NStr("ru = 'Группа файлов (Создание)';
...
Function WriteFile(Val ParameterObject = Undefined)
	...
	Except
		...
		RollbackTransaction();
		WriteLogEvent(NStr("ru = 'Файлы.Ошибка записи группы присоединенных файлов';
										|en = 'Files.Error writing group of attachments';", Common.DefaultLanguageCode()),
...
Procedure RefreshInformationAboutChange()
	...
	ChangedStatus = StringFunctions.FormattedString(NStr("ru = '<a href=""%1"">%2</a>';
																|en = '<a href=""%1"">%2</a>';"),
