...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Parameters.FileOwner = Undefined Then
		Raise NStr("ru = 'Список присоединенных файлов можно посмотреть
									...
									|en = 'You can view the list of attachments
	...
	If Parameters.ChoiceMode Then
		...
		Title = NStr("ru = 'Выбор присоединенного файла';
	...
	If Common.IsMobileClient() Then
		...
		Items.AddFileFromScanner.Title = NStr("ru = 'С камеры устройства...';
...
Procedure SetDeletionMark(Command)
	...
	Else
		QueryText = ?(CurrentData.DeletionMark,
			NStr("ru = 'Снять с выбранных файлов пометку на удаление?';
				|en = 'Do you want to clear the deletion mark from the selected files?';"),
			NStr("ru = 'Пометить выбранные файлы на удаление?';
				|en = 'Do you want to mark the selected files for deletion?';"));
	EndIf;
