...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	Telegram1.Description = NStr("ru = 'Чаты Telegram';
									|en = 'Telegram chats';");
	Telegram1.Active = -1;
	Telegram1.Type = ExternalSystemsTypes.Telegram;
	
	VKontakte = ConnectionsList.GetItems().Add();
	VKontakte.Description = NStr("ru = 'Чаты ВКонтакте';
									|en = 'VK chats';");
	VKontakte.Active = -1;
	...
	For Each Integration In CollaborationSystem.GetIntegrations() Do
		...
		Else
			WriteLogEvent(ConversationsInternal.EventLogEvent(),
				EventLogLevel.Error,,,
				NStr("ru = 'Не поддерживаемый тип внешней интеграции';
					|en = 'Unsupported type of external integration';"));
		EndIf;
