...
Function SearchForDuplicatesAreaManager(Val DataSearchAreaName) Export
	...
	Raise StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = 'Некорректный тип объекта метаданных ""%1""';
			|en = 'Invalid type of ""%1"" metadata object.';"), DataSearchAreaName);
EndFunction
	...
	While NextSelectionItem(QuerySchema.SampleObjectsSelection) Do
		...
		If RequestStructure.FieldsNamesToCompareForSimilarity.Count() > 0 Then
			...
			Except
				Result.ErrorDescription = 
					NStr("ru = 'Не удалось подключить компоненту для нечеткого поиска дублей. Подробнее см. в журнале регистрации.';
						|en = 'Cannot attach the add-in for fuzzy search for duplicates. For more information, see the event log.';");
				Return Result;
			EndTry;
		...
		If ReturnedBatchSize > 0 And (DuplicatesTable.Count() > ReturnedBatchSize) Then
				Result.ErrorDescription = StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Найдено слишком много дублей. Показаны только первые %1.';
						|en = 'Too many duplicates were found. First %1 items are shown.';"), ReturnedBatchSize); 
	...
	For Each FieldName In RequestStructure.FieldsNamesInChoice Do
		...
		If AvailableDCField = Undefined Then
			WriteLogEvent(DuplicateObjectsDetection.SubsystemDescription(False),
				...
				StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Поле ""%1"" не существует.';
																			|en = 'Field %1 does not exist.';"), String(DCField)));
			Continue;
		EndIf;
