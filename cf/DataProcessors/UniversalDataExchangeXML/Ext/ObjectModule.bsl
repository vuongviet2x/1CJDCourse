...
Var ExchangeFile Export;            // A sequentially writable/readable exchange file.

Var AdditionalDataProcessorParameters Export;  // Structure with parameters of external data processors in use.

Var ParametersInitialized Export;  // If True, the required conversion parameters are initialized.
...
Var HandlersNames; // Structure containing names of all exchange rule handlers.
...
Var mConversionRuleMap; // A map for defining object conversion rules by the given object type.
...
//
// Returns:
//  String - the right part of the string - before the separator character.
// 
...
//
// Returns:
//   Array of String - received array of values.
// 
...
// Parameters:
//  TypeValue - String
//               - Type - contains a type name or value of the Type type.
//  
// Returns:
//  TypeDescription - the Type details object.
//
...
// Parameters:
//  Type          - String
//               - Type - a type name or value of the Type type.
//
// Returns:
...
//                 - DocumentManager - manager of the object to be searched;
//  Property       - String - a property to implement the search: Name, Code, 
//                   Description, or a name of an indexed attribute.
//  Value       - String
//                 - Number
//                 - Date - value of a property to be used for searching the object.
//  FoundByUUIDObject - CatalogObject
//                                             - DocumentObject - an infobase object 
//                   that was found by UUID while executing function.
...
Function deValueTypeAsString(ValueOrType) Export
	...
	Else
		...
		If Manager = Undefined Then
			...
			Text= NStr("ru = 'Неизвестный тип:';
						|en = 'Unknown type:';") + String(TypeOf(ValueType));
			MessageToUser(Text);
...
//
// Returns:
//  XMLWriter - XML node added to the specified parent node.
//
...
//
// Returns:
//  XMLWriter - a copy of the specified node.
//
...
Function ExportByRule(Source					= Undefined,
	...
	If CommentObjectProcessingFlag Then
		...
		StringForUser = ?(GetRefNodeOnly, NStr("ru = 'Конвертация ссылки на объект: %1';
																|en = 'Converting object reference: %1';"), NStr("ru = 'Конвертация объекта: %1';
																												|en = 'Converting object: %1';"));
	...
	If HasBeforeConvertObjectGlobalHandler Then
		...
		Except
			...
			HandlerName = NStr("ru = '%1 (глобальный)';
									|en = '%1 (global-level)';");
...
Procedure SendWriteInformationToDestination(InformationToWriteToFile, ErrorStringInDestinationInfobase = "") Export
	...
	If Not IsBlankString(ErrorStringInDestinationInfobase) Then
		...
		MessageString = SubstituteParametersToString(NStr("ru = 'Загрузка в приемнике: %1';
															|en = 'Importing in destination: %1';"), ErrorStringInDestinationInfobase);
...
//
// Returns:
//  String - current value of the data processor version.
//
	...
	ElsIf HandlersDebugModeFlag Then
		...
		Try
			...
			Except
				...
				Raise
					NStr("ru = 'Ошибка при подключении внешней обработки отладки.';
						|en = 'Error attaching the external debug data processor.';");
...
Procedure DeleteTempFiles(TempFileName) Export
	...
	If Not IsBlankString(TempFileName) Then
		...
		Except
			WriteLogEvent(NStr("ru = 'Универсальный обмен данными в формате XML';
											|en = 'Conversion Rule Data Exchange in XML format';", DefaultLanguageCode()),
...
Function WriteTextToTemporaryFile(TempFileList)
	...
	Except
		WriteErrorInfoConversionHandlers(1000,
			...
			NStr("ru = 'Ошибка при создании временного файла для выгрузки данных';
...
Function ReadTextFromTemporaryFile(TempFileName)
	...
	Except
		WriteErrorInfoConversionHandlers(1000,
			ErrorProcessing.DetailErrorDescription(ErrorInfo()),
			NStr("ru = 'Ошибка при открытии временного файла для переноса данных в файл обмена';
				|en = 'An error occurred when opening the temporary file to transfer data to the exchange file';"));
		Raise;
	EndTry;
...
// Returns:
//   ValueTree - A collection of data export rules:
//     * Enable - Number
//     * IsFolder - Boolean
//     * Name - String
//     * Description - String
...
Procedure ExportEventHandlers(Cancel) Export
	...
	If IsInteractiveMode Then
		...
		If FlagErrors Then
			...
			MessageToUser(NStr("ru = 'При выгрузке обработчиков были обнаружены ошибки.';
										|en = 'Error exporting event handlers.';"));
			
		Else
			
			MessageToUser(NStr("ru = 'Обработчики успешно выгружены.';
										|en = 'Event handlers exported.';"));
...
Procedure GetFullAlgorithmScriptRecursively()
	...
	// Populate the structure of integrated algorithms.
...
//  
// Returns:
//  String - modified algorithm script taking nested algorithms into account.
// 
Function ReplaceAlgorithmCallsWithTheirHandlerScript(Handler, AlgorithmOwner, RequestedItemArray, Val PrefixString = "")
	...
	WriteAlgorithmBlockTitle(HandlerNew, AlgorithmOwner, PrefixString, NStr("ru = '{НАЧАЛО АЛГОРИТМА}';
																							|en = '{ALGORITHM START}';"));
		...
		If HandlerAlgorithms.Count() <> 0 Then // There are algorithm calls in the line.
			...
			For Each Algorithm In HandlerAlgorithms Do
				...
				If RequestedItemArray.Find(Upper(Algorithm)) <> Undefined Then // Recursive algorithm call.
					...
					WriteAlgorithmBlockTitle(HandlerNew, Algorithm, PrefixStringForInlineCode, NStr("ru = '{РЕКУРСИВНЫЙ ВЫЗОВ АЛГОРИТМА}';
																													|en = '{RECURSIVE ALGORITHM CALL}';"));
					
					OperatorString = NStr("ru = '%1 ""РЕКУРСИВНЫЙ ВЫЗОВ АЛГОРИТМА: %2"";';
											|en = '%1 ""RECURSIVE ALGORITHM CALL: %2"";';");
	...
	WriteAlgorithmBlockTitle(HandlerNew, AlgorithmOwner, PrefixString, NStr("ru = '{КОНЕЦ АЛГОРИТМА}';
																							|en = '{ALGORITHM END}';"));
...
//  
// Returns:
//  Array - an array received by copying from the passed array.
// 
...
//  
// Returns:
//  Array - an array with names of algorithms that the passed handler contains.
//
...
// Returns:
//  String - algorithm script total offset mode.
// 
...
//
// Parameters:
//  Rows         - A collection of value tree rows.
// 
...
//
// Parameters:
//  Object   - an object whose property is being set.
...
//
// Returns:
//   - AnyRef - if object is not found.
//   - Undefined - if object is not found.
// 
...
//
// Parameters:
//  Object         - The object whose table is being imported.
//  Name            - tabular section name.
//  Clear       - If set to True, the table will be pre-cleaned.
// 
...
Procedure ObjectImportComments(NBSp, RuleName, Source, ObjectType, Gsn = 0)
	...
	If CommentObjectProcessingFlag Then
		...
		If NBSp <> 0 Then
			MessageString = SubstituteParametersToString(NStr("ru = 'Загрузка объекта № %1';
																|en = 'Importing object #%1';"), NBSp);
		Else
			MessageString = SubstituteParametersToString(NStr("ru = 'Загрузка объекта № %1';
																|en = 'Importing object #%1';"), Gsn);
		EndIf;
...
Procedure ExecuteWriteNotWrittenObjects()
	...
	For Each DataString1 In mNotWrittenObjectGlobalStack Do
		...
		Object = DataString1.Value.Object; // CatalogObject, DocumentObject, etc.
...
//
// Parameters:
//   DataElement	  - "CatalogObject", "DocumentObject", "InformationRegisterRecordSet", etc.
...
//  Rule        - A reference to a data export rule.
...
Procedure ExportSelectionObject(Object, Rule, Properties=Undefined, IncomingData=Undefined, SelectionForDataExport = Undefined)
	...
	If CommentObjectProcessingFlag Then
		...
		MessageString = SubstituteParametersToString(NStr("ru = 'Выгрузка объекта: %1';
															|en = 'Exporting object: %1';"), ObjectRul);
...
Procedure ExportDataByRule(Rule)
	...
	If CommentObjectProcessingFlag Then
		...
		MessageString = SubstituteParametersToString(NStr("ru = 'Правило выгрузки данных: %1 (%2)';
															|en = 'Data export rule: %1 (%2)';"), TrimAll(Rule.Name), TrimAll(Rule.Description));
...
// Returns:
//   ValueTreeRow - A row of the data export rules tree.:
//     * Name - String
//     * Description - String
//
...
Procedure ImportDataExchangeParameterValues()
	...
	If EventsAfterParametersImport.Property(Name, AfterParameterImportAlgorithm)
		...
		If HandlersDebugModeFlag Then
			...
			Raise NStr("ru = 'Отладка обработчика ""После загрузки параметра"" не поддерживается.';
									|en = 'Event handler AfterParameterImport doesn''t support debugging.';");
	...
	If Conversion.Property("AfterImportExchangeRules", AfterExchangeRulesImportEventText)
		...
		Try
			...
			If HandlersDebugModeFlag Then
				...
				Raise NStr("ru = 'Отладка обработчика ""После загрузки правил обмена"" не поддерживается.';
										|en = 'Event handler AfterImportExchangeRules doesn''t support debugging.';");
...
Procedure RunReadingData(ErrorInfoResultString = "") Export
				...
				If Not IsBlankString(AlgorithmText) Then
					...
					Try
						...
						If HandlersDebugModeFlag Then
							...
							Raise NStr("ru = 'Отладка обработчика ""После загрузки параметров"" не поддерживается.';
													|en = 'Event handler AfterImportParameters doesn''t support debugging.';");
						...
						If Cancel = True Then
							...
							If Not IsBlankString(CancelReason) Then
								ExceptionString = SubstituteParametersToString(NStr("ru = 'Загрузка данных отменена по причине: %1';
																					|en = 'Data import canceled. Reason: %1';"), CancelReason);
				...
				If Not IsBlankString(AlgorithmText) Then
					...
					Try
						...
						If HandlersDebugModeFlag Then
							...
							Raise NStr("ru = 'Отладка глобального алгоритма не поддерживается.';
													|en = 'Global algorithms don''t support debugging.';");
	...
	Except
		...
		ErrorString = SubstituteParametersToString(NStr("ru = 'Ошибка при загрузке данных: %1';
...
//
//  Returns:
//     Boolean - True if the data can be imported from file. Otherwise, False.
//
Function ExecuteActionsBeforeReadData(DataString1 = "") Export
	...
	MessageString = SubstituteParametersToString(NStr("ru = 'Начало загрузки: %1';
														|en = 'Import started at: %1';"), CurrentSessionDate());
	...
	If Not IsBlankString(Conversion.BeforeImportData) Then
		...
		Except
			...
			HandlerName = NStr("ru = '%1 (конвертация)';
									|en = '%1 (conversion)';"); 
...
Procedure ExecuteActionsAfterDataReadingCompleted() Export
	...
	WriteToExecutionProtocol(SubstituteParametersToString(
		NStr("ru = 'Окончание загрузки: %1';
			|en = 'Import finished at: %1';"), CurrentSessionDate()), , False, , , True);
	WriteToExecutionProtocol(SubstituteParametersToString(
		NStr("ru = 'Загружено объектов: %1';
			|en = 'Objects imported: %1';"), mImportedObjectCounter), , False, , , True);
	
	FinishKeepExchangeProtocol();
	
	If IsInteractiveMode Then
		MessageToUser(NStr("ru = 'Загрузка данных завершена.';
...
Procedure CompressResultingExchangeFile()
	...
	Try
		...
		Archiver = New ZipFileWriter(ExchangeFileName, ExchangeFileCompressionPassword, NStr("ru = 'Файл обмена данными';
																							|en = 'Data exchange file';"));
...
Procedure ExecuteExport() Export
	...
	WriteToExecutionProtocol(SubstituteParametersToString(
		NStr("ru = 'Начало выгрузки: %1';
			|en = 'Export started at: %1';"), CurrentSessionDate()), , False, , , True);
	...
	Except
		...
		WriteToExecutionProtocol(SubstituteParametersToString(
			NStr("ru = 'Ошибка при выгрузке данных: %1';
				|en = 'Data export error: %1';"), ErrorString), Undefined, True, , , True);
	...
	WriteToExecutionProtocol(SubstituteParametersToString(
		NStr("ru = 'Окончание выгрузки: %1';
			|en = 'Export finished at: %1';"), CurrentSessionDate()), , False, , ,True);
	WriteToExecutionProtocol(SubstituteParametersToString(
		NStr("ru = 'Выгружено объектов: %1';
			|en = 'Objects exported: %1';"), mExportedObjectCounter), , False, , , True);
	...
	If IsInteractiveMode Then
		MessageToUser(NStr("ru = 'Выгрузка данных завершена.';
...
Procedure InitMessages()
	...
	deMessages.Insert(2,  NStr("ru = 'Ошибка распаковки файла обмена. Файл заблокирован';
									|en = 'Error extracting exchange file. File is locked.';"));
	deMessages.Insert(3,  NStr("ru = 'Указанный файл правил обмена не существует';
									|en = 'The exchange rules file does not exist.';"));
	
	ErrorText = NStr("ru = 'Ошибка при создании COM-объекта %1';
						|en = 'An error occurred while creating the %1 COM object';");
	deMessages.Insert(4,  SubstituteParametersToString(ErrorText,"Msxml2.DOMDocument"));
	
	deMessages.Insert(5,  NStr("ru = 'Ошибка открытия файла обмена';
									|en = 'Error opening exchange file';"));
	deMessages.Insert(6,  NStr("ru = 'Ошибка при загрузке правил обмена';
									|en = 'Error importing exchange rules';"));
	deMessages.Insert(7,  NStr("ru = 'Ошибка формата правил обмена';
									|en = 'Exchange rule format error';"));
	deMessages.Insert(8,  NStr("ru = 'Некорректно указано имя файла для выгрузки данных';
									|en = 'Invalid data export file name';"));
	deMessages.Insert(9,  NStr("ru = 'Ошибка формата файла обмена';
									|en = 'Exchange file format error';"));
	deMessages.Insert(10, NStr("ru = 'Не указано имя файла для выгрузки данных (Имя файла данных)';
									|en = 'Data export file name is not specified';"));
	deMessages.Insert(11, NStr("ru = 'Ссылка на несуществующий объект метаданных в правилах обмена';
									|en = 'Exchange rules reference a metadata object that does not exist';"));
	deMessages.Insert(12, NStr("ru = 'Не указано имя файла с правилами обмена (Имя файла правил)';
									|en = 'Exchange rules file name is not specified';"));
	
	deMessages.Insert(13, NStr("ru = 'Ошибка получения значения свойства объекта (по имени свойства источника)';
									|en = 'Error getting value of object property by property name in source infobase';"));
	deMessages.Insert(14, NStr("ru = 'Ошибка получения значения свойства объекта (по имени свойства приемника)';
									|en = 'Error getting value of object property by property name in destination infobase';"));
	
	deMessages.Insert(15, NStr("ru = 'Не указано имя файла для загрузки данных (Имя файла для загрузки)';
									|en = 'Data import file name is not specified';"));
	
	deMessages.Insert(16, NStr("ru = 'Ошибка получения значения свойства подчиненного объекта (по имени свойства источника)';
									|en = 'Error getting value of subordinate object property by property name in source infobase';"));
	deMessages.Insert(17, NStr("ru = 'Ошибка получения значения свойства подчиненного объекта (по имени свойства приемника)';
									|en = 'Error getting value of subordinate object property by property name in destination infobase';"));
	
	ErrorText = NStr("ru = 'Ошибка в обработчике события %1';
						|en = 'Event handler error: %1';");
	...
	ErrorText = NStr("ru = 'Ошибка в обработчике события %1 (конвертация)';
						|en = 'Event handler error (data conversion): %1';");
	...
	deMessages.Insert(24, NStr("ru = 'Ошибка при удалении объекта';
									|en = 'Error deleting object';"));
	deMessages.Insert(25, NStr("ru = 'Ошибка при записи документа';
									|en = 'Error writing document';"));
	deMessages.Insert(26, NStr("ru = 'Ошибка записи объекта';
									|en = 'Error writing object';"));
	...
	deMessages.Insert(39, NStr("ru = 'Ошибка при выполнении алгоритма, содержащегося в файле обмена';
									|en = 'Exchange file algorithm execution error';"));
	...
	deMessages.Insert(45, NStr("ru = 'Не найдено правило конвертации объектов';
									|en = 'Object conversion rule not found';"));
	
	ErrorText = NStr("ru = 'Ошибка в обработчике события %1 группы свойств';
						|en = 'Event handler error: %1 of property group';");
	...
	ErrorText = NStr("ru = 'Ошибка в обработчике события %1 (объекта коллекции)';
						|en = 'Event handler error: %1 (of collection object)';");
	...
	ErrorText = NStr("ru = 'Ошибка в глобальном обработчике события %1 (конвертация)';
						|en = 'Global event handler error (data conversion): %1';"); 
	...
	ErrorText = NStr("ru = 'Ошибка в обработчике события %1 (свойства)';
						|en = 'Event handler error: %1 (of property)';");
	...
	deMessages.Insert(66, NStr("ru = 'Ошибка получения коллекции подчиненных объектов из входящих данных';
									|en = 'Error getting collection of subordinate objects from incoming data';"));
	deMessages.Insert(67, NStr("ru = 'Ошибка получения свойства подчиненного объекта из входящих данных';
									|en = 'Error getting property of subordinate object from incoming data';"));
	deMessages.Insert(68, NStr("ru = 'Ошибка получения свойства объекта из входящих данных';
	...
	deMessages.Insert(71, NStr("ru = 'Не найдено соответствие для значения Источника';
									|en = 'Cannot find a match for the source value';"));
	
	deMessages.Insert(72, NStr("ru = 'Ошибка при выгрузке данных для узла плана обмена';
	...
	deMessages.Insert(74, NStr("ru = 'Необходимо перезагрузить правила обмена для выгрузки данных';
									|en = 'Reloading exchange rules is required';"));
	
	deMessages.Insert(75, NStr("ru = 'Ошибка при выполнении алгоритма после загрузки значений параметров';
	...
	deMessages.Insert(77, NStr("ru = 'Не указан файл внешней обработки с подключаемыми процедурами обработчиков событий';
									|en = 'External data processor file not specified';"));
	
	deMessages.Insert(78, NStr("ru = 'Ошибка создания внешней обработки из файла с процедурами обработчиков событий';
									|en = 'Error creating external data processor from file with event handlers';"));
	...
	deMessages.Insert(79, NStr("ru = 'Код алгоритмов не может быть интегрирован в обработчик из-за обнаруженного рекурсивного вызова алгоритмов. 
							...
							|en = 'Cannot integrate algorithms'' code into the handler.
	...
	deMessages.Insert(80, NStr("ru = 'Обмен данными можно проводить только под полными правами';
									|en = 'You must have full rights to run the data exchange';"));
	
	deMessages.Insert(1000, NStr("ru = 'Ошибка при создании временного файла выгрузки данных';
...
Function DetermineIfEnoughInfobaseConnectionParameters(ConnectionStructure, StringForConnection = "", ErrorMessageString = "")
	...
	If ConnectionStructure.FileMode  Then
		...
		If IsBlankString(ConnectionStructure.IBDirectory) Then
			...
			ErrorMessageString = NStr("ru = 'Не задан каталог информационной базы-приемника';
											|en = 'Destination infobase directory not specified';");
	...
	Else
		...
		If IsBlankString(ConnectionStructure.ServerName) Then
			...
			ErrorMessageString = NStr("ru = 'Не задано имя сервера 1С:Предприятия информационной базы-приемника';
											|en = 'Server of destination infobase not specified';");
		...
		If IsBlankString(ConnectionStructure.IBNameAtServer) Then
			...
			ErrorMessageString = NStr("ru = 'Не задано имя информационной базы-приемника на сервере 1С:Предприятия';
											|en = 'Infobase name on 1C:Enterprise server not specified';");
...
Function ConnectToInfobase(ConnectionStructure, ErrorMessageString = "")
	...
	Except
		...
		ErrorMessageString = NStr("ru = 'При попытке соединения с COM-сервером произошла следующая ошибка:
			|%1';
			|en = 'Error when connecting to COM server:
			|%1';");
...
// 
//  Returns:
//    DataProcessorObject.UniversalDataExchangeXML - processing the destination infobase to which data is imported.
//
Function EstablishConnectionWithDestinationIB() Export
	...
	Except
		...
		Text = NStr("ru = 'При попытке создания обработки %1 произошла ошибка: %2';
					|en = 'An error occurred while trying to create the %1 data processor: %2';");
...
Procedure InitHandlersNamesStructure()
	...
	// A global structure containing handler names.
...
// Returns:
//   Structure - Value type manager details:
//     * Name - String
//     * TypeName - String
//     * RefTypeString1 - String
//     * Manager - Arbitrary
