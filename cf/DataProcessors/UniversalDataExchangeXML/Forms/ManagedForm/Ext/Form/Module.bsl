...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	FormCaption = NStr("ru = 'Универсальный обмен данными в формате XML (%DataProcessorVersion%)';
							|en = 'Conversion Rule Data Exchange in XML format (%DataProcessorVersion%)';");
...
#EndIf
	...
	If SavedImportMode
		...
		If Object.AutomaticDataImportSetup = 1 Then
			...
			ShowQueryBox(NotifyDescription, NStr("ru = 'Выполнить загрузку данных из файла обмена?';
													|en = 'Do you want to import data from the exchange file?';"), QuestionDialogMode.YesNo, , DialogReturnCode.Yes);
...
Procedure InfobaseToConnectDirectoryStartChoice(Item, ChoiceData, StandardProcessing)
	...
	FileDialog.Title = NStr("ru = 'Выберите каталог информационной базы';
										|en = 'Select an infobase directory';");
...
Procedure AfterExistenceCheckRulesFileName(Exists, AdditionalParameters) Export
	...
	If Not Exists Then
		MessageToUser(NStr("ru = 'Не найден файл правил обмена';
									|en = 'Exchange rules file not found';"), "RulesFileName");
	...
	ShowQueryBox(NotifyDescription, NStr("ru = 'Загрузить правила обмена данными?';
											|en = 'Do you want to import data exchange rules?';"), QuestionDialogMode.YesNo, , DialogReturnCode.Yes);
...
Procedure GetExchangeFileInfoCompletion(Result, Address, SelectedFileName, AdditionalParameters) Export
	...
	If Result Then
		...
		Except
			...
			MessageToUser(NStr("ru = 'Не удалось прочитать файл обмена.';
										|en = 'Cannot read the exchange file.';"));
			ClearDataImportFileData();
...
Procedure DeletionDelete(Command)
	...
	ShowQueryBox(NotifyDescription, NStr("ru = 'Удалить выбранные данные в информационной базе?';
											|en = 'Do you want to delete the selected data from the infobase?';"), QuestionDialogMode.YesNo, , DialogReturnCode.No);
...
Procedure ReadExchangeRules(Command)
	...
	If Not IsWindowsClient() And DirectExport = 1 Then
		ShowMessageBox(,NStr("ru = 'Прямое подключение к информационной базе поддерживается только в клиенте под управлением ОС Windows.';
...
Function RuleAndExchangeFileNamesMatch()
	...
	If Upper(TrimAll(RulesFileName)) = Upper(TrimAll(DataFileName)) Then
		...
		|en = 'An exchange rules file cannot be the same as a data file.
		|Select another file.';"));
		Return True;
...
Procedure FillTypeAvailableToDeleteList()
	...
	TreeRow.Presentation = NStr("ru = 'Справочники';
										|en = 'Catalogs';");
	...
	TreeRow = DataTree.Rows.Add();
	TreeRow.Presentation = NStr("ru = 'Планы видов характеристик';
	...
	TreeRow = DataTree.Rows.Add();
	TreeRow.Presentation = NStr("ru = 'Документы';
										|en = 'Documents';");
...
Procedure EstablishConnectionWithDestinationIBAtServer()
	...
	If ConnectionResult <> Undefined Then
		...
		MessageToUser(NStr("ru = 'Подключение успешно установлено.';
	...
	Else
		...
		If Not FileOnServer.Exists() Then
			...
			MessageToUser(NStr("ru = 'Не найден файл обмена на сервере.';
										|en = 'Exchange file not found on the server.';"), "ExchangeFileName");
			Return;
...
Procedure ExecuteImportAtServer(FileAddress, FileNameForExtension)
	...
	If Object.SafeImport Then
		...
		Else
			MessageToUser(NStr("ru = 'Не указан файл правил для загрузки данных.';
										|en = 'Data import file is not specified.';"));
			Return;
		EndIf;
	EndIf;
	...
	Except
		WriteLogEvent(NStr("ru = 'Универсальный обмен данными в формате XML';
										|en = 'Conversion Rule Data Exchange in XML format';", Common.DefaultLanguageCode()),
...
Function FileNameAtServerOrClient(Var_AttributeName ,Val FileAddress, Val FileNameForExtension = ".xml",
	...
	Else
		...
		If Not FileOnServer.Exists() And CheckForExistence Then
			...
			MessageToUser(NStr("ru = 'Указанный файл не существует.';
...
Procedure ExecuteExportFromForm()
	...
	If IsClient And Not DirectExport And Not Object.FlagErrors Then
		...
		FileToSaveName = ?(Object.ArchiveFile, NStr("ru = 'Файл выгрузки.zip';
																|en = 'Export file.zip';"),NStr("ru = 'Файл выгрузки.xml';
																								|en = 'Export file.xml';"));
...
Function ExecuteExportAtServer()
	...
	If Not DirectExport Then
		...
		If TempDataFileName = Undefined Then
			...
			MessageToUser(NStr("ru = 'Не определен файл данных';
										|en = 'Data file not specified';"));
			Return Undefined;
	...
	If ObjectForServer.HandlersDebugModeFlag Then
		...
		If Not File.Exists() Then
			...
			MessageToUser(NStr("ru = 'Файл внешней обработки отладчиков событий не существует на сервере';
										|en = 'The external event debugger file does not exist on the server';"));
			Return Undefined;
		...
		If Cancel Then
			...
			MessageToUser(NStr("ru = 'Не удалось выгрузить обработчики событий';
										|en = 'Cannot export event handlers';"));
			Return "";
...
Procedure ChangeProcessingMode(WorkMode)
	...
	?(WorkMode, NStr("ru = 'Режим работы (на клиенте)';
						|en = 'Mode (client)';"), NStr("ru = 'Режим работы (на сервере)';
																	|en = 'Mode (server)';"));
...
Function EmptyAttributeValue(Attribute, DataPath, Var_Title)
	...
	If IsBlankString(Attribute) Then
		...
		MessageText = NStr("ru = 'Поле ""%1"" не заполнено';
								|en = '""%1"" is required';");
