...
Procedure PathToBackupDirectoryStartChoice(Item, ChoiceData, StandardProcessing)
	...
	OpenFileDialog.Title= NStr("ru = 'Выберите каталог для сохранения резервных копий';
										|en = 'Choose a directory to save backups to';");
...
Procedure NavigateFromSettingPageAfterCheckAccessToInfobase(ConnectionResult, Context) Export
	...
	ShowUserNotification(NStr("ru = 'Резервное копирование';
										|en = 'Backup';"), SettingsFormName,
		NStr("ru = 'Резервное копирование настроено.';
			|en = 'Backup is all set up.';"));
...
#If WebClient Or MobileClient Then
	MessageText = NStr("ru = 'Действие недоступно в веб-клиенте или в мобильном клиенте.
		|Запустите тонкий клиент.';
		|en = 'Cannot perform the operation in web client or mobile client.
		|Start thin client.';");
...
#Else
	...
	If IsBlankString(Object.BackupDirectory) Then
		...
		MessageText = NStr("ru = 'Не выбрана папка для резервной копии.';
								|en = 'Select a folder for backup.';");
		CommonClient.MessageToUser(MessageText,, "Object.BackupDirectory");
		AttributesFilled = False;
		
	ElsIf FindFiles(Object.BackupDirectory).Count() = 0 Then
		
		MessageText = NStr("ru = 'Указана несуществующая папка.';
								|en = 'Non-existent folder is specified.';");
		CommonClient.MessageToUser(MessageText,, "Object.BackupDirectory");
		AttributesFilled = False;
	...
	Else
		...
		Try
			...
			TestFile.Close();
		Except
			MessageText = NStr("ru = 'Нет доступа к папке с резервными копиями.';
									|en = 'Cannot access the backup folder.';");
			CommonClient.MessageToUser(MessageText,, "Object.BackupDirectory");
			AttributesFilled = False;
		EndTry;
