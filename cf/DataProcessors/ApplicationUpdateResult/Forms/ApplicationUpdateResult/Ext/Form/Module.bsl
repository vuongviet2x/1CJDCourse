...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Not Users.IsFullUser(, True) Then
		...
		Items.WhereToFindThisFormHint.Title = 
			NStr("ru = 'Ход обработки данных версии приложения можно также проконтролировать из раздела
		               |""Информация"" на рабочем столе, команда ""Описание изменений приложения"".';
						|en = 'To view the progress of processing app data,
	...
	Else
		UpdateCompletedTitle = NStr("ru = 'Версия приложения успешно обновлена на версию %1';
											|en = 'The application is updated to version %1.';");
	...
	Else
		...
		TitleTemplate1 = NStr("ru = 'Дополнительные процедуры обработки данных завершены %1 в %2';
								|en = 'Additional data processing procedures were completed on %1 at %2.';");
	...
	TroubleHintWithData = NStr("ru = 'Проблемы с данными могут мешать их обработке при переходе на новую версию.
		...
		|en = 'When the application is updating, data issues might prevent the processing.
...
Procedure ExplanationUpdateNotRunningURLProcessing(Item, FormattedStringURL, StandardProcessing)
	...
	ElsIf FormattedStringURL = "Enable" Then
		EnableScheduledJob();
		MessageText = NStr("ru = 'Задание включено. Статус обновления скоро обновится.';
								|en = 'Duty is enabled. The status will refresh soon.';");
...
Function UpdateResultMessage()
	...
	If Progress.TotalHandlerCount = Progress.CompletedHandlersCount Then
		...
		If Progress.TotalHandlerCount = 0 Then
			...
			MessageText = "";
		Else
			MessageText = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Все процедуры обновления выполнены успешно (%1)';
																							|en = 'All update procedures are completed (%1).';"), Progress.CompletedHandlersCount);
		EndIf;
		Items.CompletedPicture.Picture = PictureLib.Success32;
		UpdateCompletedSuccessful = True;
	Else
		MessageText = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Не все процедуры удалось выполнить (выполнено %1 из %2)';
																						|en = 'Some of the update procedures are not completed (%1 out of %2 completed)';"), 
...
Procedure UpdateInformationOnIssues()
	...
	If NumberofProblemsInHandlers <> 0 And Not UpdateCompletedSuccessful Then
		TextIndicator = NStr("ru = 'Проблемы с обработчиками';
								|en = 'Handler issues found';");
	Else
		TextIndicator = NStr("ru = 'Проблем с обработчиками не обнаружено';
								|en = 'No handler issues found';");
	EndIf;
	...
	If NumberofProblemswithData <> 0 Then
		TextIndicator = NStr("ru = 'Проблемы с данными (%1)';
								|en = 'Data issues (%1)';");
		TextIndicator = StringFunctionsClientServer.SubstituteParametersToString(TextIndicator, NumberofProblemswithData);
	Else
		TextIndicator = NStr("ru = 'Проблем с данными не обнаружено';
								|en = 'No data issues found';");
	EndIf;
...
Procedure ChangeScheduleAfterSetUpSchedule(NewSchedule, AdditionalParameters) Export
	...
	If NewSchedule <> Undefined Then
		If NewSchedule.RepeatPeriodInDay = 0 Then
			...
			QuestionButtons.Add("SetUpSchedule", NStr("ru = 'Настроить расписание';
																|en = 'Set schedule';"));
			QuestionButtons.Add("RecommendedSettings1", NStr("ru = 'Установить рекомендуемые настройки';
...
Procedure ProcessPatchInstallResult(PatchInfo)
	...
	If PatchInfo.Unspecified > 0 Then
		...
		Ref = New FormattedString(NStr("ru = 'Не удалось установить исправления';
													|en = 'Cannot install the patches';"),,,, "UnsuccessfulInstallation");
		PatchesLabel = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = '(%1 из %2)';
																							|en = '(%1 out of %2).';"), InstalledSuccessfully, TotalPatchCount);
	...
	Else
		Ref = New FormattedString(NStr("ru = 'Исправления (патчи)';
													|en = 'The patches';"),,,, "InstalledPatches");
		PatchesLabel = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'успешно установлены (%1)';
				|en = 'are installed (%1).';"), TotalPatchCount);
		PatchesLabel = New FormattedString(Ref, " ", PatchesLabel);
...
Procedure CheckPerformDeferredUpdate(UpdateInfo)
	...
	If Job.Use Then
		...
		MessageText = NStr("ru = 'Регламентное задание <b>Отложенное обновление</b> включено, но
			|не выполняется. Вероятно включена блокировка выполнения регламентных заданий.';
			|en = 'Scheduled job <b>Deferred update</b> is active but not running.
		...
		If Common.SubsystemExists("StandardSubsystems.UsersSessions") Then
			Messages.Add(NStr("ru = '<a href=""%1"">Проверить блокировку регламентных заданий</a>';
									|en = '<a href=""%1"">Check scheduled job lock</a>';"));
			IdentifierHyperlinks = "CheckLock";
		EndIf;
	Else
		MessageText = NStr("ru = 'Дополнительные процедуры обработки данных не выполняются,
			|т.к. отключено регламентное задание <b>Отложенное обновление</b>.';
			|en = 'Additional procedures of data processing are not running
			|because the <b>Deferred update</b> scheduled job is disabled.';");
		Messages.Add(StrConcat(StrSplit(MessageText, Chars.LF), " "));
		Messages.Add(NStr("ru = '<a href=""%1"">Включить</a>';
								|en = '<a href=""%1"">Enable</a>';"));
