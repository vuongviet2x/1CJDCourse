...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Items.AutomaticTextsExtractionGroup.Visible Then
		...
		If Common.FileInfobase() Then
			...
			ChoiceList[0].Presentation = NStr("ru = 'Все рабочие станции работают под управлением ОС Windows';
												|en = 'All workstations run on Windows.';");
			
			ChoiceList = Items.ExtractFilesTextsAtLinuxServer.ChoiceList;
			ChoiceList[0].Presentation = NStr("ru = 'Одна или несколько рабочих станций работают под управлением ОС Linux';
												|en = 'One or more workstations run on Linux.';");
		EndIf;
	...
	Else
		AutoTitle = False;
		Title = NStr("ru = 'Управление полнотекстовым поиском';
						|en = 'Full-text search management';");
		Items.SectionDetails.Title =
			NStr("ru = 'Включение и отключение полнотекстового поиска, обновление индекса полнотекстового поиска.';
				|en = 'Full-text search toggle, search index update.';");
	EndIf;
...
Procedure UpdateIndex(Command)
	UpdateIndexServer();
	ShowUserNotification(NStr("ru = 'Полнотекстовый поиск';
										|en = 'Full-text search';"),, NStr("ru = 'Индекс успешно обновлен';
																				|en = 'Index has been updated';"));
EndProcedure

&AtClient
Procedure ClearIndex(Command)
	QueryText = NStr("ru = 'Индекс поиска будет очищен, что приведет к невозможности
		|полнотекстового поиска в приложении.
		|Для включения полнотекстового поиска потребуется выполнить обновление индекса.
		|
		|Продолжить?';
		|en = 'The search index will be cleared, and you
...
Procedure CheckIndex(Command)
	...
	CheckIndexServer();
	ShowUserNotification(NStr("ru = 'Полнотекстовый поиск';
										|en = 'Full-text search';"),, NStr("ru = 'Индекс содержит корректные данные';
																				|en = 'Index is up to date';"));
EndProcedure
...
Procedure Attachable_OnChangeAttribute(Item, ShouldRefreshInterface = True)
	...
	If Result.Property("CannotEnableFullTextSearchMode") Then
		...
		QueryText = NStr("ru = 'Для изменения режима полнотекстового поиска требуется завершение сеансов всех пользователей, кроме текущего.';
							|en = 'To change the full-text search mode, close all sessions, except for the current user session.';");
...
Procedure ClearTheIndexAfterAnsweringTheQuestion(Result, AdditionalParameters) Export
	...
	If Result = DialogReturnCode.Yes Then
		ClearIndexServer();
		ShowUserNotification(NStr("ru = 'Полнотекстовый поиск';
											|en = 'Full-text search';"),, NStr("ru = 'Индекс успешно очищен';
																					|en = 'Index has been cleaned up';"));
	EndIf;
...
Procedure CheckIndexServer()
	...
	Except
		ErrorMessageText = 
			NStr("ru = 'В настоящее время проверка индекса невозможна, так как выполняется его очистка или обновление.';
				|en = 'Cannot check index status. The index is being updated or cleaned up.';");
...
Function SaveAttributeValue(DataPathAttribute)
	...
	If NameParts.Count() <> 2 Then
		...
		ElsIf DataPathAttribute = "IndexedDataMaxSize"
			...
			Except
				WriteLogEvent(
					NStr("ru = 'Полнотекстовый поиск';
						|en = 'Full-text search';", Common.DefaultLanguageCode()),
					EventLogLevel.Error,
					,
					,
...
Procedure SetAvailability(DataPathAttribute = "", IndexChecked = False)
	...
	If DataPathAttribute = ""
		...
		If UseFullTextSearch = 1 Then
			...
			If IndexChecked And Not IndexContainsCorrectData Then
				IndexStatus = NStr("ru = 'Требуется очистка и обновление';
									|en = 'Cleanup and update required';");
			ElsIf IndexTrue Then
				IndexStatus = NStr("ru = 'Обновление не требуется';
									|en = 'No update required';");
			Else
				IndexStatus = NStr("ru = 'Требуется обновление';
									|en = 'Update required';");
			EndIf;
		Else
			IndexUpdateDate = '00010101';
			IndexTrue = False;
			IndexStatus = NStr("ru = 'Полнотекстовый поиск отключен';
	...
	If Items.AutomaticTextsExtractionGroup.Visible
		...
		Else
			SchedulePresentation = NStr("ru = 'Автоматическое извлечение текстов не выполняется.';
											|en = 'Automatic text extraction is not scheduled.';");
		EndIf;
