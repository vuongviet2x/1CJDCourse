...
Procedure SendMessage(Val Author, Val Recipients, Message, ConversationContext = Undefined) Export
	...
	If Author = Undefined Then
		Raise NStr("ru = 'Не указан автор сообщения';
								|en = 'Message author is not specified';");
	EndIf;
	
	If Recipients.Count() = 0 Then
		Raise NStr("ru = 'Не указаны получатели сообщения';
	...
	If ConversationContext <> Undefined 
		...
		If Not ValueIsFilled(ConversationContext) Then
			Raise NStr("ru = 'Передан пустой контекст обсуждения.';
									|en = 'Empty conversation context is passed.';");
		EndIf;
...
Procedure SendNotification(Val Author, Message, ConversationContext) Export
	...
	ElsIf ConversationContext = Undefined Then
		...
		Raise NStr("ru = 'Не указан идентификатор обсуждения или контекст.';
								|en = 'Conversation ID or context is not specified.';");
...
Function InfoBaseUsers(CollaborationSystemUsers)Export
	...
	If Errors.Count() > 0 Then
		...
		WriteLogEvent(ConversationsInternal.EventLogEvent(
			NStr("ru = 'Пользователи информационной базы';
				|en = 'Infobase users';", Common.DefaultLanguageCode())),
			EventLogLevel.Error,,,
...
Function InfoBaseUser(CollaborationSystemUser) Export
	...
	If Not ValueIsFilled(Result) Then
		...
		ErrorTemplate = NStr("ru = 'Не удалось получить пользователя информационной базы по идентификатору пользователя системы взаимодействия (%1)';
							|en = 'Cannot get an infobase user by collaboration system user ID (%1)';");
...
Function CollaborationSystemUsers(Var_InfoBaseUsers) Export
	...
	If Errors.Count() > 0 Then
		...
		WriteLogEvent(ConversationsInternal.EventLogEvent(
			NStr("ru = 'Пользователи системы взаимодействия';
				|en = 'Collaboration system users';", Common.DefaultLanguageCode())),
			EventLogLevel.Error,,,
...
Function CollaborationSystemUser(User, IDOnly = False) Export
	...
	Else
		...
		If Not ValueIsFilled(InfoBaseUserID) Then
			...
			Else
				ErrorTemplate = NStr("ru = 'Не удалось получить идентификатор пользователя (%1)';
									|en = 'Cannot get user ID (%1)';");
		...
		Except
			...
			ErrorDescription = ?(Not ValueIsFilled(CollaborationSystemIDGettingError), "",
				StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Не удалось получить идентификатор пользователя системы взаимодействия для пользователя %1 (%2)
					           |по причине:
					           |%3';
								|en = 'Cannot receive a collaboration system user ID for the %1 (%2) user
								|due to:
								|%3';"),
					String(User),
			...
			ErrorDescription = ErrorDescription + StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Не удалось создать пользователя системы взаимодействия для пользователя %1 (%2)
					           |по причине:
					           |%3';
								|en = 'Cannot create a collaboration system user for the %1 (%2) user
								|due to:
								|%3';"),
					String(User),
					String(InfoBaseUserID),
...
Procedure UpdateUserInCollaborationSystem(User) Export
	...
	Try
		...
		UpdateUserDetailsInCollaborationSystem(
			...
			UserDetails);
	Except
		ErrorDescription = NStr("ru = 'Не удалось обновить пользователя системы взаимодействия.';
								|en = 'Cannot update the collaboration system user.';");
		Error = ErrorInfo();
		WriteLogEvent(ConversationsInternal.EventLogEvent(
			NStr("ru = 'Обновить описание в системе взаимодействия';
				|en = 'Update details in the collaboration system';", Common.DefaultLanguageCode())),
			EventLogLevel.Error,
			User.Metadata(),
			User.Ref,
...
Function NewCollaborationSystemUser(User)
	...
	If Not ValueIsFilled(UserDetails.IBUserID) Then
		Raise NStr("ru = 'Не существует пользователь информационной базы';
								|en = 'Infobase user does not exist';");
	EndIf;
