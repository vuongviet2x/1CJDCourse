...
Function AddInBundleFilesChecksum(Val TemplateName) Export
		...
		Raise StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Не удалось сформировать разрешение на использование внешней компоненты:
				  |некорректное имя макета %1.';
					|en = 'Cannot generate a permission to use the add-in:
	...
	Except
		WriteLogEvent(NStr("ru = 'Работа в безопасном режиме.Не удалось удалить временный файл';
										|en = 'Safe mode manager.Cannot create temporary file';", Common.DefaultLanguageCode()), 
...
Procedure FillPermissionsToUpdatesProtectionCenter(PermissionsRequests)
	...
	Resolution = SafeModeManager.PermissionToUseInternetResource("HTTPS", "1cv8update.com",, 
		NStr("ru = 'Сайт ""Центр защиты обновлений"" (ЦЗО) для проверки правомерности использования и обновления программного продукта.';
			|en = 'The ""Update protection center"" (UPC) site for checking legitimacy of the software usage and updating.';"));
...
Procedure OnSaveInternalData(Object) Export
	...
	If SafeModeManager.SafeModeSet() Then
		...
		Raise StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Запись объекта %1 недоступна: установлен безопасный режим: %2.';
				|en = 'Couldn''t save %1. Safe mode is set: %2.';"),
