...
Procedure HandleError(ErrorInfo) Export
	...
	WriteLogEvent(
		NStr("ru = 'Серверные оповещения.Ошибка получения или обработки оповещений';
	...
	If SendImmediately
	   And ServerNotificationsInternalCached.IsSessionSendServerNotificationsToClients() Then
		...
		ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'В процедуре %1 недопустимо указывать значение %2 для параметра %3
			           |при вызове из процедур %4.';
						|en = 'In procedure %1, you cannot specify value %2 for parameter %3
...
Procedure PrepareServerNotifications(SendStatus, MaxIntervalByUser = Undefined)
	...
	For Each KeyAndValue In RecurringNotifications Do
		...
		EndIf;
		Try
			SendingModule.OnSendServerNotification(NameOfAlert, ProcedureParameters);
		Except
			ErrorInfo = ErrorInfo();
			ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Процедура ""%1"" не выполнена по причине:
				           |%2';
				...
				ErrorProcessing.DetailErrorDescription(ErrorInfo));
			WriteLogEvent(
				NStr("ru = 'Серверные оповещения.Ошибка работы фонового задания';
					|en = 'Server notifications.Background job error';",
					Common.DefaultLanguageCode()),
...
Procedure UpdateSendStatus(NewSendStatus, PropertiesNames)
	...
	Except
		...
		ErrorInfo = ErrorInfo();
		ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Не удалось установить константу %1 по причине:
			           |%2';
						|en = 'Cannot set constant %1 due to:
						|%2';"),
...
Function SendStatusOnBackgroundJobStart()
	...
	Except
		...
		ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
			...
			ErrorProcessing.DetailErrorDescription(ErrorInfo));
		WriteLogEvent(
			NStr("ru = 'Серверные оповещения.Ошибка запуска фонового задания';
				|en = 'Server notifications.Background job startup error';",
				Common.DefaultLanguageCode()),
			EventLogLevel.Error,,, ErrorText);
	EndTry;
...
Procedure StartDeliverDeferredServerNotifications(Launched = False)
	...
	JobDescription =
		...
		+ NStr("ru = 'Доставка серверных оповещений с отсрочкой';
				|en = 'Delayed server notification delivery';", Common.DefaultLanguageCode()) + " ("
		+ StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'из сеанса %1 от %2';
				|en = 'from session %1 started on %2';", Common.DefaultLanguageCode()),
...
Procedure ConfigureJobSendServerNotificationsToClients(Enable, RepeatPeriod = 0, OnStart = False)
	...
	Except
		...
		ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
			...
			ErrorProcessing.DetailErrorDescription(ErrorInfo));
		WriteLogEvent(
			NStr("ru = 'Серверные оповещения.Ошибка настройки регламентного задания';
				|en = 'Server notifications.Scheduled job setup error';",
				Common.DefaultLanguageCode()),
			EventLogLevel.Error,,, ErrorText);
	EndTry;
	 ...
	 Or Not OnStart
	   And InfoBaseUsers.GetUsers().Count() <> 0 Then
		...
		Except
			ErrorInfo = ErrorInfo();
			ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Не удалось создать служебного пользователя ""%1"" по причине:
				           |%2';
							|en = 'Cannot create utility user ""%1"" due to:
							|%2';"),
				InternalUsername(),
				ErrorProcessing.DetailErrorDescription(ErrorInfo));
			Raise ErrorText;
...
Procedure SetUsageOfJobSendServerNotificationsToClients(Use)
	...
	Except
		ErrorInfo = ErrorInfo();
		ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Не удалось изменить использование регламентного задания
			           |""%1"" по причине:
			           |%2';
						|en = 'Cannot change the usage of
...
Function IsCurrentUserRegisteredInInteractionSystem(UserIDCollaborationSystem = Undefined)
	...
	Except
		...
		EndIf;
		ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Не удалось зарегистрировать текущего пользователя
			           |""%1 (%2)""
			           |в системе взаимодействия по причине:
			           |%3';
						|en = 'Cannot register current user
						|""%1 (%2)""
			...
			ErrorProcessing.DetailErrorDescription(ErrorInfo));
		WriteLogEvent(
			NStr("ru = 'Серверные оповещения.Ошибка регистрации пользователя в системе взаимодействия';
				|en = 'Server notifications.An error occurred when registering the user in the collaboration system';",
				Common.DefaultLanguageCode()),
...
Function IsInteractionSystemTemporarilyUnavailable(CheckAvailability = False)
	...
	If PreviousValue2 = CurrentValue Then
		WriteLogEvent(
			NStr("ru = 'Серверные оповещения.Система взаимодействия недоступна';
				|en = 'Server notifications.Collaboration system is unavailable';",
				Common.DefaultLanguageCode()),
...
Function PersonalChatID(IBUserID = Undefined,
	...
	Except
		ErrorInfo = ErrorInfo();
		ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Не удалось создать личное обсуждение для пользователя ""%1 (%2)"" по причине:
			           |%3';
						|en = 'Cannot create a personal conversation for user ""%1 (%2)"" due to:
						|%3';"),
			InfoBaseUsers.CurrentUser().Name,
			Lower(IBUserID),
			ErrorProcessing.DetailErrorDescription(ErrorInfo));
		WriteLogEvent(
			NStr("ru = 'Серверные оповещения.Ошибка создания личного обсуждения';
				|en = 'Server notifications.An error occurred when creating a personal conversation';",
				Common.DefaultLanguageCode()),
...
Function GlobalChatID()
	...
	Except
		ErrorInfo = ErrorInfo();
		WriteLogEvent(
			NStr("ru = 'Серверные оповещения.Ошибка создания общего обсуждения';
				|en = 'Server notifications.An error occurred when creating a common conversation';",
				Common.DefaultLanguageCode()),
			EventLogInteractionSystemErrorSeverity(ErrorInfo),,,
			ErrorProcessing.DetailErrorDescription(ErrorInfo));
		Return Undefined;
...
Function SendMessage(Data, ConversationID)
	...
	Except
		ErrorInfo = ErrorInfo();
		ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Не удалось отправить сообщение в обсуждение %1 по причине:
			           |%2';
						|en = 'Cannot send the message to conversation %1 due to:
						|%2';"),
			Lower(ConversationID),
			ErrorProcessing.DetailErrorDescription(ErrorInfo));
		WriteLogEvent(
			NStr("ru = 'Серверные оповещения.Ошибка отправки сообщения';
				|en = 'Server notifications.Send message error';",
				Common.DefaultLanguageCode()),
			EventLogInteractionSystemErrorSeverity(ErrorInfo),,, ErrorText);
		Return False;
	EndTry;
...
//
// Returns:
//  Boolean - If "True", the message is accepted for delivery.
//
	...
	If ValueIsFilled(Data.SMSMessageRecipients) Then
		...
		For Each KeyAndValue In Data.SMSMessageRecipients Do
			If TypeOf(KeyAndValue.Value) <> Type("Array") Then
				...
				Break;
			EndIf;
			For Each SessionKey In KeyAndValue.Value Do
				SessionKeyParts = StrSplit(SessionKey, " ", False);
				SessionNumberAsString = ?(SessionKeyParts.Count() > 1,
					SessionKeyParts.Get(SessionKeyParts.UBound()), "0");
				SessionNumber = NumberDetails.AdjustValue(SessionNumberAsString);
				If ValueIsFilled(SessionNumber) Then
...
Procedure CleanUpObsoleteMessages(SendStatus)
	...
	Except
		ErrorInfo = ErrorInfo();
		WriteLogEvent(
			NStr("ru = 'Серверные оповещения.Ошибка очистки устаревших сообщений';
				|en = 'Server notifications.An error occurred when clearing obsolete messages';",
				Common.DefaultLanguageCode()),
			EventLogInteractionSystemErrorSeverity(ErrorInfo),,,
			ErrorProcessing.DetailErrorDescription(ErrorInfo));
	EndTry;
