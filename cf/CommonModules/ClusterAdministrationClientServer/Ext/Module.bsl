...
Procedure AddFilterCondition(Filter, Val Property, Val ValueComparisonType, Val Value) Export
	...
	Else
		...
		Raise NStr("ru = 'Некорректно задано описание фильтра.';
								|en = 'Invalid filter description.';");
...
Function CheckFilterConditions(Val ObjectToCheck, Val Filter = Undefined) Export
	...
	For Each Condition In Filter Do
		...
		Else
			...
			Raise NStr("ru = 'Некорректно задан фильтр.';
									|en = 'Invalid filter.';");
...
Function AdministrationManager(Val AdministrationParameters)
	...
	Else
		...
		Raise StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Неизвестный тип подключения: %1.';
																						|en = 'Unknown connection type: %1.';"), AdministrationParameters.AttachmentType);
