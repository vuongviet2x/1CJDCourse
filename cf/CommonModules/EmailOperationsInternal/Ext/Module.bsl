...
Procedure SaveYourAccountSettingsForPasswordRecovery(Settings) Export
	...
	If TypeOf(Settings) <> Type("Structure") Then
		Raise NStr("ru = 'Неверный тип настроек почты для восстановления пароля.';
								|en = 'Incorrect account settings type for password recovery.';");
	EndIf;
...
Procedure OnAddUpdateHandlers(Handlers) Export
	...
	Handler.Comment = NStr("ru = 'Обновляет сведения об учетных записях электронной почты.
		|До завершения обработки список учетных записей электронной почты может быть неполным.';
		|en = 'Updates email account data.
...
Procedure OnFillAccessKinds(AccessKinds) Export
	...
	AccessKind.Presentation = NStr("ru = 'Учетные записи электронной почты';
									|en = 'User email accounts';");
...
Procedure OnFillTypesExcludedFromExportImport(Types) Export
	...
	Types.Add(Metadata.Constants.PasswordRecoveryAccount);
...
Function DownloadMessages(Val UserAccountOrConnection, Val ImportParameters = Undefined) Export
	...
	If Account = Undefined Then
		...
		TransportSettings.Insert("KeepMessageCopiesAtServer", True);
		TransportSettings.Insert("KeepMailAtServerPeriod", 0);
	Else
...
Procedure CheckSendReceiveEmailAvailability(Account, ErrorMessage, AdditionalMessage) Export
	...
	If AccountSettings1.UseForSending Then
		...
		If ValueIsFilled(ErrorText) Then
			ErrorMessage = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Не удалось подключиться к серверу SMTP: %1';
																							|en = 'Cannot connect to SMTP server: %1';") + Chars.LF, ErrorText);
		EndIf;
		If Not AccountSettings1.UseForReceiving Then
			AdditionalMessage = Chars.LF + NStr("ru = '(Выполнена проверка отправки электронных сообщений.)';
														|en = '(The check whether the mail is sent is performed.)';");
		EndIf;
	EndIf;
	...
	If AccountSettings1.UseForReceiving 
		...
		If ValueIsFilled(ErrorText) Then
			...
			ErrorMessage = ErrorMessage + StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Не удалось подключиться к серверу %1:
				|%2';
				|en = 'Cannot connect to %1 server:
				|%2';"), AccountSettings1.ProtocolForIncomingMail, ErrorText);
		EndIf;
		
		If Not AccountSettings1.UseForSending Then
			AdditionalMessage = Chars.LF + NStr("ru = '(Выполнена проверка получения электронных сообщений.)';
														|en = '(The check whether the mail is received is performed.)';");
		EndIf;
...
Procedure PrepareAttachments(Attachments, SettingsForSaving) Export
	...
	For IndexOf = -Attachments.UBound() To 0 Do
		...
		For Each SelectedFormat In SelectedSaveFormats Do
			...
			Else
				FileNameForArchive = NStr("ru = 'Документы';
										|en = 'Documents';") + ".zip";
			EndIf;
...
Function EmailPresentation(EmailSubject, EmailDate)
	...
	TemplateOfPresentation = NStr("ru = '%1 от %2';
								|en = '%1, %2';");
	
	Return StringFunctionsClientServer.SubstituteParametersToString(TemplateOfPresentation,
		?(IsBlankString(EmailSubject), NStr("ru = '<Без темы>';
										|en = '<No subject>';"), EmailSubject),
	...
	If Account <> Undefined Then
		...
		Except
			...
			ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Не удалось подключиться к серверу IMAP:
				|%1';
				|en = 'Cannot connect to IMAP server:
		...
		For Each MailMessage In Emails Do
			...
			If WrongRecipients.Count() > 0 Then
				...
				ErrorText = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Отправка письма следующим получателям не выполнена:
					|%1';
					|en = 'The message was not sent to the following recipients:
...
Function EventNameSendEmail()
	...
	Return NStr("ru = 'Работа с почтовыми сообщениями.Отправка почты';
				|en = 'Email management.Send message';", Common.DefaultLanguageCode());
...
Function Permissions() Export
	...
	Port = Undefined;
	LongDesc = NStr("ru = 'Поиск настроек почты и диагностика ошибок подключения.';
					|en = 'Search for email settings and run connection error troubleshooting.';");
	...
	For Each Address In DNSServerAddresses() Do
		Protocol = "TCP";
		Port = 53;
		LongDesc = NStr("ru = 'Поиск настроек почты.';
						|en = 'Search for email settings.';");
	...
	Permissions.Add(ModuleSafeModeManager.PermissionToUseOperatingSystemApplications(CommandTemplate,
		NStr("ru = 'Разрешение для nslookup.';
			|en = 'Permission for nslookup.';", Common.DefaultLanguageCode())));
...
Function ExplanationOnError(ErrorText, Val LanguageCode = Undefined, ForSetupAssistant = False) Export
	...
	If Not ValueIsFilled(PossibleReasons) Then
		If Not ValueIsFilled(ErrorsDetails) Then
			PossibleReasons.Add(NStr("ru = 'Отсутствует подключение к Интернету.';
											|en = 'No Internet connection.';"));
		EndIf;
		PossibleReasons.Add(NStr("ru = 'Неверные настройки подключения к почтовому серверу.';
										|en = 'Invalid email server connection settings.';"));
		PossibleReasons.Add(NStr("ru = 'Неполадки на почтовом сервере.';
										|en = 'Mail server malfunction.';"));
	EndIf;
	
	If Not ValueIsFilled(MethodsToFixError) Then
		If Not ValueIsFilled(ErrorsDetails) Then
			MethodsToFixError.Add(NStr("ru = 'Проверьте подключение к Интернету.';
											|en = 'Check the Internet connection.';"));
		EndIf; 
		
		If ForSetupAssistant Then
			MethodsToFixError.Add(NStr("ru = 'Проверьте введенные настройки.';
											|en = 'Check the specified settings.';"));
		Else
		...
		MethodsToFixError.Add(NStr("ru = 'Повторите действие еще раз через некоторое время.';
										|en = 'Try again later.';"));
		MethodsToFixError.Add(NStr("ru = 'Обратитесь к администратору локальной сети.';
										|en = 'Contact the network administrator.';"));
		MethodsToFixError.Add(NStr("ru = 'Обратитесь к администратору почтового сервера.';
										|en = 'Contact the email server administrator.';"));
	EndIf;
...
Function ExecuteQuery(ServerAddress, ResourceAddress, QueryOptions, PutParametersInQueryBody = True) Export
	...
	If HTTPResponse <> Undefined Then
		If HTTPResponse.StatusCode <> 200 Then
			ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Запрос ""%1"" не выполнен. Код состояния: %2.';
					|en = 'Request failed: %1. Status code: %2.';"), ResourceAddress, HTTPResponse.StatusCode) + Chars.LF
...
Function EventNameAuthorizationByProtocolOAuth() Export
	...
	Return NStr("ru = 'Работа с почтовыми сообщениями.Авторизация на почтовом сервере';
				|en = 'Email management. Email server authorization';", Common.DefaultLanguageCode());
...
Function RefreshAccessToken(Val Account, Val UpdateToken)
	...
	If Not ValueIsFilled(AttributesValues.EmailServiceName) Then
		ErrorText = NStr("ru = 'Не указано имя почтового сервиса для авторизации. Необходимо перенастроить почту.';
							|en = 'Email service for authorization is not specified. Reconfigure your account.';");
	...
	If Not ValueIsFilled(AuthorizationSettings.AppID) Then
		ErrorText = NStr("ru = 'Настройки авторизации Интернет-сервиса ""%1"" не найдены для домена ""%2"". Необходимо перенастроить почту.';
							|en = 'Authorization settings of the ""%1"" online service are not found for domain ""%2"". Reconfigure your account.';");
		WriteLogEvent(EventNameAuthorizationByProtocolOAuth(), EventLogLevel.Error, , Account,
			ErrorText);
			Return "";
	EndIf;
	...
	If ValueIsFilled(ErrorCode) Then
		ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Не удалось получить ключи доступа к учетной записи почты %1 по причине:
			|%2
			|Ответ сервера:
			|%3';
			|en = 'Cannot get access keys to the %1 email account due to:
			|%2
			|Server response:
					...
					If StrFind(Message.Header, Selection.RecipientAddress) > 0 And StrFind(Message.Header, "X-Failed-Recipients") > 0  Then
						...
						If CharNumberReasonLine > 0 Then 
							...
							Cause = StringFunctionsClientServer.SubstituteParametersToString(
								NStr("ru = 'Письмо не доставлено по причине: %1.';
									|en = 'The message is not delivered due to: %1.';"), Cause);
						Else
								Cause = NStr("ru = 'Письмо не доставлено.';
												|en = 'The message is not delivered.';");
						EndIf;
					...
					If Message.Texts.Count() > 0 Then
						For Each EmailText In Message.Texts Do
							...
							If StrFind(EmailText["Text"], Selection.EmailID) > 0
								...
								Cause = StringFunctionsClientServer.SubstituteParametersToString(
								NStr("ru = 'Письмо не доставлено по причине: %1.';
									|en = 'The message is not delivered due to: %1.';"), TrimAll(Cause));
...
Function CharCodeToOrdinalNum(Val Code)
	...
	ElsIf Code - CodeA < 26 Then
		Return Code - CodeA;
	Else
		Raise NStr("ru = 'Плохие данные на вход';
								|en = 'Bad input data';");
	EndIf;
EndFunction
