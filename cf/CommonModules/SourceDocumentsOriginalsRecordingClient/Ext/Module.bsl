...
Procedure Attachable_SetOriginalState(Ref, Parameters) Export
	...
	If List.SelectedRows.Count() = 0 Then
		ShowMessageBox(, NStr("ru = 'Не выделено ни одного документа, для которого можно установить выбранное состояние.';
	...
	If List.SelectedRows.Count() > 1 Then
		QueryText = NStr("ru = 'У выделенных в списке документов будет установлено состояние оригинала ""%StateName%"". Продолжить?';
							|en = 'The ""%StateName%"" original state will be set for documents selected in the list. Continue?';");
		QueryText = StrReplace(QueryText, "%StateName%", StateName);

		Buttons = New ValueList;
		Buttons.Add(DialogReturnCode.Yes,NStr("ru = 'Установить';
													|en = 'Set';"));
		Buttons.Add(DialogReturnCode.No,NStr("ru = 'Не устанавливать';
													|en = 'Do not set';"));
	...
	ElsIf SourceDocumentsOriginalsRecordingServerCall.IsAccountingObject(List.CurrentData.Ref) Then 
		SetOriginalStateCompletion(DialogReturnCode.Yes, AdditionalParameters);
	Else
		ShowMessageBox(, NStr("ru = 'Для данного документа учет оригиналов не ведется.';
...
Procedure OpenStateSelectionMenu(Val Form, Val Source = Undefined) Export 
	...
	If TypeOf(Source) = Type("FormTable") Then
		...
		If UnpostedDocuments.Count() = 1 Then
			ShowMessageBox(, NStr("ru = 'Для выполнения команды предварительно проведите документ.';
		...
		If RecordData.OverallState Or Not ValueIsFilled(RecordData.SourceDocumentOriginalState) Then
			If ClarifyByPrintForms = Undefined Then
				Form.OriginalStatesChoiceList.Add("ClarifyByPrintForms",
					NStr("ru = 'Уточнить по печатным формам...';
...
Procedure ProcessBarcode(Barcode, EventName) Export
	...
	If EventName = "ScanData" Then
		Status(NStr("ru = 'Выполняется установка состояния оригинала по штрихкоду...';
...
Procedure NotifyUserOfStatesSetting(ProcessedItemsCount, DocumentRef = Undefined, StateName = Undefined) Export
	...
	If ProcessedItemsCount > 1 Then
		MessageText = NStr("ru = 'Для всех выделенных в списке документов установлено состояние оригинала ""%StateName%""';
								|en = 'The ""%StateName%"" original state is set for all documents selected in the list';");
		MessageText = StrReplace(MessageText, "%StateName%", StateName);
		
		TitleText = NStr("ru = 'Состояние оригинала ""%StateName%"" установлено';
								|en = 'The ""%StateName%"" original state is set';");
	...
	Else
		...
		ShowUserNotification(NStr("ru = 'Изменение состояния оригинала:';
											|en = 'Original state changed:';"),NotifyDescription,DocumentRef,PictureLib.DialogInformation,UserNotificationStatus.Important);
	EndIf;
...
Procedure SetOriginalStateCompletion(Response, AdditionalParameters) Export
	...
	If IsChanged = "NotPosted" Then
		ShowMessageBox(, NStr("ru = 'Для установки состояния оригиналов предварительно проведите выделенные документы.';
										|en = 'To set the original state, post the selected documents first.';"));
		Return;
