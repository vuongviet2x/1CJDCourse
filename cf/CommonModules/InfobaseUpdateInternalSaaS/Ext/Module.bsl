		...
		If LibraryID = Metadata.Name Then
			...
			While Selection.Next() Do
				...
				If Not Selection.Updated1 Then
					...
					CommentTemplate = NStr("ru = 'Обновление версии конфигурации было выполнено до обновления версии библиотеки %1';
											|en = 'Configuration version update was performed before updating %1 library version';");
				...
				If LibraryUpdatePlanDetails = Undefined Then
					...
					CommentTemplate = NStr("ru = 'Не существует план обновления библиотеки %1';
											|en = 'The update plan for library %1 does not exist';");
					CommentText1 = StringFunctionsClientServer.SubstituteParametersToString(CommentTemplate, Selection.Name);
					WriteLogEvent(
						InfobaseUpdate.EventLogEvent(),
						EventLogLevel.Error,
				...
				If LibraryUpdatePlanDetails.VersionTo1 <> Selection.Version Then
					...
					CommentTemplate = NStr("ru = 'Обнаружен некорректный план обновления библиотеки %1
						...
						|en = 'Incorrect update plan of the %1 library is detected.
	...
	If Common.DataSeparationEnabled()
		...
		If InfobaseUpdateInternal.UpdateRequired(Metadata.Version, SharedDataVersion) Then
			Message = NStr("ru = 'Не выполнена общая часть обновления информационной базы.
				...
				|en = 'General part of the infobase update is not performed.
...
Function DataAreaLockResult(RecordKey, AttemptNumber)
	...
	Except
		...
		WriteLogEvent(InfobaseUpdate.EventLogEvent() + "." 
			+ NStr("ru = 'Обновление области данных';
					|en = 'Data area update';", Common.DefaultLanguageCode()),
			EventLogLevel.Error,,,
			ErrorProcessing.DetailErrorDescription(ErrorInfo()));
		Raise(NStr("ru = 'Ошибка обновления области данных. Запись версий области данных заблокирована.';
								|en = 'An error occurred when updating the data area. Saving data area versions is locked.';"));
	EndTry;
