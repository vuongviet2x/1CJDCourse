...
Procedure OpenReportForm(Val OwnerForm, Val Variant, Val AdditionalParameters = Undefined) Export
	...
	If Not ValueIsFilled(OpeningParameters.ReportType) Then
		Raise StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Не определен тип отчета в %1';
																						|en = 'Report type is not specified in %1';"), "ReportsOptionsClient.OpenReportForm");
	EndIf;
	...
	ElsIf OpeningParameters.ReportType = "Additional" Then
		...
		EndIf;
	Else
		ShowMessageBox(, NStr("ru = 'Вариант внешнего отчета можно открыть только из формы отчета.';
										|en = 'You can open external report options only from report forms.';"));
		Return;
	EndIf;
	
	If Not ValueIsFilled(OpeningParameters.ReportName) Then
		Raise StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Не определено имя отчета в %1';
																						|en = 'Report name is not specified in %1';"), "ReportsOptionsClient.OpenReportForm");
	EndIf;
...
Procedure OnStart(Parameters) Export 
	...
	Except
		...
		EventLogClient.AddMessageForEventLog(
			NStr("ru = 'Варианты отчетов.Ошибка подключения обработчика действия сообщения';
				|en = 'Report options.An error occurred when attaching a message action handler';",
...
Procedure OpenOptionArrangeInSectionsDialog(Variants, Owner = Undefined) Export
	...
	If TypeOf(Variants) <> Type("Array") Or Variants.Count() < 1 Then
		ShowMessageBox(, NStr("ru = 'Выберите варианты отчетов для размещения в разделах.';
										|en = 'Select report options to add to sections.';"));
		Return;
	EndIf;
...
Procedure OnMessageActionHandlerError(ErrorInfo, StandardProcessing, Context) Export 
	...
	EventLogClient.AddMessageForEventLog(
		NStr("ru = 'Варианты отчетов.Ошибка обработки действия сообщения';
			|en = 'Report options.An error occurred when processing a message action';",
			CommonClient.DefaultLanguageCode()),
		"Error",
		ErrorProcessing.DetailErrorDescription(ErrorInfo));
...
Function ReportOptionUsersPickingParameters(Form, PickUsersGroups, ExternalUsersGroupsPickup = False)
	...
	PickingParameters.Insert("PickFormHeader", NStr("ru = 'Подбор пользователей варианта отчета';
															|en = 'Pick report option users';"));
...
Procedure UpdateReportOptionFromFiles(ReportOptionProperties, FormIdentifier) Export
	...
	ImportParameters.Dialog.Filter = NStr("ru = 'Сведения об отчете (*.zip)|*.zip';
											|en = 'Report information (*.zip)|*.zip';");
...
Procedure UpdateReportOptionsFromFilesCompletion(FilesDetails, BaseReportOptionProperties) Export 
	...
	NotificationText2 = StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = 'Загружены из файла: %1';
			|en = 'Imported from the file: %1';"), ReportsOptionsProperties.Count());
	
	ShowUserNotification(NStr("ru = 'Отчеты обновлены';
										|en = 'The reports are updated';"),, NotificationText2);
...
Procedure UpdateReportOptionFromFile(FileDetails, BaseReportOptionProperties) 
	...
	If BaseReportOptionProperties.ReportName = ReportOptionProperties.ReportName Then 
		...
		ShowUserNotification(NStr("ru = 'Отчет обновлен из файла';
											|en = 'The report is updated from the file';"), 
	...
	Else
		...
		QuestionTextTemplate = NStr("ru = 'Выбраны настройки варианта отчета ""%1"",
			...
			|en = 'The selected settings of the ""%1"" report option
...
Procedure ShareUserSettings(SettingsDescription) Export 
	...
	If SettingsDescription.Settings.Items.Count() = 0 Then 
		...
		CommonClient.MessageToUser(NStr("ru = 'Настройки (пользовательские) не установлены.';
														|en = 'The user settings are not specified.';"));
		Return;
	...
	PickingParameters.Insert("PickFormHeader", NStr("ru = 'Поделиться настройками отчета с пользователями';
															|en = 'Share report settings with users';"));
	PickingParameters.Insert("PickingCompletionButtonTitle", NStr("ru = 'Поделиться';
																		|en = 'Share';"));
...
Procedure ShareUserSettingsAfterUsersChoice(SelectedUsers, SettingsDescription) Export 
	...
	ShowUserNotification(NStr("ru = 'Настройки переданы';
										|en = 'The settings are shared';"),, Explanation);
