	...
	If CorrespondentEndpoint.IsEmpty() Then
		Raise StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Не найдена конечная точка корреспондента с кодом ""%1"".';
				|en = 'Endpoint with ID %1 not found in peer infobase.';"),
			Body.EndPoint);
	EndIf;
...
Procedure ImportExchangeMessage(Message, Sender) Export
	...
	Try
		...
		If Cancel Then
			Raise StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'В процессе загрузки данных из приложения  ""%1"" возникли ошибки.';
					|en = 'Errors occurred during data import from %1.';"),
				String(Peer));
		EndIf;
