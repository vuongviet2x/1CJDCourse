...
Function InvalidBankNote(Bank) Export
	...
	If ValueIsFilled(Bank) And ValueIsFilled(NewBankDetails) Then
		Result = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'БИК банка изменился на <a href = ""%1"">%2</a>';
				|en = 'Bank code was changed to <a href = ""%1"">%2</a>';"),
			GetURL(NewBankDetails.Ref), NewBankDetails.BIC);
	Else
		Result = NStr("ru = 'Деятельность банка прекращена';
						|en = 'Bank activity is ceased';");
	EndIf;
...
Procedure OnFillToDoList(ToDoList) Export
	...
	For Each Section In Sections Do
		...
		ToDoItem.Presentation  = NStr("ru = 'Справочник БИК устарел';
									|en = 'Bank codes catalog is outdated';");
		ToDoItem.ToolTip      = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Последнее обновление %1 назад';
																							|en = 'The last update was %1 ago.';"), Result.AmountOfDelayByLine);
