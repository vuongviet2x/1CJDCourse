...
Procedure CreateContact(LongDesc, Address, Basis, ContactsTypes) Export
	...
	ContactsTypes.ShowChooseItem(HandlerNotifications, NStr("ru = 'Выбор типа контакта';
...
Procedure PromptOnChangeMessageFormatToPlainText(Form, AdditionalParameters = Undefined) Export
	...
	MessageText = NStr("ru = 'При преобразовании этого сообщения в обычный текст будут утеряны все элементы оформления, картинки и прочие вставленные элементы. Продолжить?';
							|en = 'If you change the message format to plain text, all images and formatting will be lost. Continue?';");
	ShowQueryBox(OnCloseNotifyHandler, MessageText, QuestionDialogMode.YesNo, , DialogReturnCode.No, NStr("ru = 'Изменение формата письма';
																															|en = 'Change mail format';"));
	...
	If Copy Then
		...
		If TypeOf(CurrentData.Ref) = Type("DocumentRef.IncomingEmail") 
			...
			If Not OnlyEmail Then
				ShowMessageBox(, NStr("ru = 'Копирование электронных писем запрещено';
												|en = 'Copying messages is not allowed';"));
			EndIf;
		EndIf;
...
Procedure CheckOfDeferredSendingAttributesFilling(Object, Cancel) Export
	...
	If Object.DateToSendEmail > Object.EmailSendingRelevanceDate And (Not Object.EmailSendingRelevanceDate = Date(1,1,1)) Then
		...
		Cancel = True;
		MessageText= NStr("ru = 'Дата актуальности отправки меньше чем дата отправки.';
							|en = '""Schedule send"" date cannot be later than ""Don''t send after"" date.';");
	...
	If Not Object.EmailSendingRelevanceDate = Date(1,1,1)
		...
		Cancel = True;
		MessageText= NStr("ru = 'Указанная дата актуальности меньше текущей даты, такое сообщение никогда не будет отправлено';
							|en = '""Don''t send after"" date is earlier than today. This message will never be sent.';");
		CommonClient.MessageToUser(MessageText,, "Object.EmailSendingRelevanceDate");
