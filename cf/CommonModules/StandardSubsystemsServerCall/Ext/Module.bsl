...
Function CalculationCellsIndicators(Val SpreadsheetDocument, SelectedAreas, UUID = Undefined) Export 
	...
	ExecutionParameters.BackgroundJobDescription = NStr("ru = 'Расчет показателей ячеек';
															|en = 'Cell indicator calculation';");
...
Procedure WriteErrorToEventLogOnStartOrExit(Shutdown, Val Event, Val ErrorText) Export
	...
	If Event = "Run" Then
		EventName = NStr("ru = 'Запуск приложения';
							|en = 'Startup';", Common.DefaultLanguageCode());
		If Shutdown Then
			ErrorDescriptionBeginning = NStr("ru = 'Запуск приложения невозможен по причине:';
										|en = 'Startup failed due to:';");
		Else
			ErrorDescriptionBeginning = NStr("ru = 'Возникла исключительная ситуация при запуске приложения:';
										|en = 'Exception occurred during startup:';");
		EndIf;
	Else
		EventName = NStr("ru = 'Завершение приложения';
							|en = 'Exit';", Common.DefaultLanguageCode());
		ErrorDescriptionBeginning = NStr("ru = 'Возникла исключительная ситуация при завершении приложения:';
									|en = 'Exception occurred while exiting the app:';");
	EndIf;
