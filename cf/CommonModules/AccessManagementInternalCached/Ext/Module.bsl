...
Function AllowedAccessKey() Export
	...
	If Not ValueIsFilled(RefInDatabase) Then
		...
		AllowedKey.Description = NStr("ru = 'Разрешенный ключ доступа';
											|en = 'Allowed access key';");
...
Function AllowedBlankAccessGroupsSet() Export
	...
	If Not ValueIsFilled(RefInDatabase) Then
		...
		AllowedBlankSet.Description = NStr("ru = 'Разрешенный пустой набор групп доступа';
													|en = 'Allowed empty access group set';");
...
Function AccessKeyDimensions() Export
	...
	If SimilarItemsInCollectionCount(KeyMetadata.Attributes, "Value") <> 5 Then
		ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'В справочнике %1 должно быть 5 реквизитов %2 и не более.';
				|en = 'Catalog ""%1"" can contain the maximum of 5 ""%2"" attributes.';"),
			"AccessKeys", "Value" + "*");
		Raise ErrorText;
	EndIf;
	...
	If KeyMetadata.TabularSections.Find("Header") = Undefined
	 Or SimilarItemsInCollectionCount(KeyMetadata.TabularSections.Header.Attributes, "Value", 6) <> 5 Then
		ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'В справочнике %1 должна быть табличная часть %2 с 5 реквизитами %3 и не более.';
				|en = 'Catalog ""%1"" can contain tabular section ""%2"" with the maximum of 5 ""%3"" attributes.';"),
	...
	If TabularSectionsCount < 1 Or TabularSectionsCount > 12 Then
		ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'В справочнике %1 должно быть от 1 до 12 табличных частей %2.';
				|en = 'Catalog ""%1"" can contain 1 to 12 tabular sections ""%2"".';"),
	...
	If TabularSectionAttributesCount = 0 Then
		ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'В справочнике %1 в табличных частях %2
			           |допустимо только одинаковое количество реквизитов %3 и не более 15.';
						|en = 'Tabular sections ""%2"" of catalog ""%1""
...
Function BasicRegisterFieldsCount(Val RegisterName = "") Export
	...
	If RegisterName = "" Or RegisterName = "AccessKeysForRegisters" Then
		...
		If Dimensions.Count() < 1 Or Dimensions[0].Name <> "Register" Then
			ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'В регистре сведений %1
				           |первым измерением должно быть %2.';
							|en = 'In information register ""%1"",
		...
		If Dimensions.Count() < 2 Or Dimensions[1].Name <> "AccessOption" Then
			ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'В регистре сведений %1
				           |вторым измерением должно быть %2.';
							|en = 'In information register ""%1"",
	...
	If Dimensions.Count() <= IndexOfTheFieldDimension
	 Or Dimensions[IndexOfTheFieldDimension].Name <> "Field1" Then
		ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'В регистре сведений %1
			           |после измерения %2 должно быть %3.';
						|en = 'In information register ""%1"",
			...
			ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'В регистре сведений %1
				           ...
				           |а на месте измерения %3 обнаружено %4';
							|en = 'In the %1 information register,
	...
	If MetadataObject = Undefined Then
		ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Не удалось получить имя предопределенного идентификатора объекта метаданных
			           ...
			           |""%1"".';
						|en = 'Cannot get the name of a predefined metadata object ID
...
Function RolesNamesBasicAccess(ForExternalUsers) Export
	...
	For Each Role In Metadata.Roles Do
		...
		// _Demo Example Start
		If StrStartsWith(NameOfRole, "_Demo") Then
			NameOfRole = Mid(NameOfRole, StrLen("_Demo") + 1);
		EndIf;
		// _Demo Example End
