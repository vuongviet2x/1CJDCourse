	...
	If Not Common.SubsystemExists("OnlineUserSupport.GetAddIns") Then
		Raise StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Действие недоступно, т.к. отсутствует подсистема ""%1"".';
				|en = 'The operation is unavailable as the ""%1"" subsystem is required.';"),
			"OnlineUserSupport.GetAddIns");
	EndIf;
		...
		For Each ResultString1 In AddInsData Do
			...
			If ValueIsFilled(ErrorCode) Then
				...
				If ErrorCode = "LatestVersion" Then
					ExecutionResult.Success.Insert(ResultString1.Id,
						...
						NStr("ru = '%1 - актуальная версия.';
							|en = '%1: the latest version.';"), AddInPresentation));
					Continue;
				EndIf;
				
				ErrorInfo = "";
				If ErrorCode = "ComponentNotFound" Then 
					ErrorInfo = NStr("ru = 'В сервисе отсутствует данная внешняя компонента';
												|en = 'This add-in is missing in the service';");
				ElsIf ErrorCode = "FileNotImported" Then 
					ErrorInfo = NStr("ru = 'Файл внешней компоненты не загружен из сервиса';
												|en = 'The add-in file is not imported from the service';");
				EndIf;
				
				ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Не удалось загрузить внешнюю компоненту %1 из сервиса:
					           |%2';
								|en = 'Cannot import the %1 add-in from the service:
								|%2';"),
			...
			If Selection.FindNext(Filter) Then 
				...
				If Selection.VersionDate > ResultString1.VersionDate Then 
					...
					ExecutionResult.Success.Insert(ResultString1.Id,
						StringFunctionsClientServer.SubstituteParametersToString(
							NStr("ru = '%1 - в приложении более новая версия, чем в сервисе (%2 от %3).';
								|en = '%1: the app contains a version that is newer than the version in the service (%2 dated %3).';"), 
			...
			Try
				...
				ExecutionResult.Success.Insert(ResultString1.Id, StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = '%1 - успешно обновлена.';
						|en = '%1: updated.';"), AddInPresentation));
