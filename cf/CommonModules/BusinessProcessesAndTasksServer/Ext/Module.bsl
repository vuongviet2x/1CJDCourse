...
Procedure TaskFormOnCreateAtServer(BusinessTaskForm, TaskObject, 
	...
	If TaskObject.Executed Then
		...
		Item.Title = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Задача выполнена %1 пользователем %2.';
																						|en = 'The task is completed on %1 by user %2.';"),
	...
	If BusinessProcessesAndTasksServerCall.IsHeadTask(TaskObject.Ref) Then
		...
		If Item = Undefined Then
			...
			Item.Type = FormDecorationType.Label;
			Item.Title = NStr("ru = 'Это ведущая задача для вложенных бизнес-процессов. Она будет выполнена автоматически при их завершении.';
									|en = 'This is a head task for nested business processes. It will be completed automatically upon their completion.';");
			Item.Height = 0; // Auto height.
			Item.AutoMaxWidth = False;
		EndIf;
	EndIf;
...
Procedure SetTaskAppearance(Val TaskListOrItsConditionalAppearance) Export
	...
	AppearanceColorItem.Value = NStr("ru = 'Срок не указан';
											|en = 'Due date is not specified';");
	AppearanceColorItem.Use = True;
	...
	If Users.IsExternalUserSession() Then
			...
			AppearanceColorItem = ConditionalAppearanceItem.Appearance.Items.Find("Text");
			AppearanceColorItem.Value = NStr("ru = 'Представитель организации';
...
Procedure SetBusinessProcessesAppearance(Val BusinessProcessesConditionalAppearance) Export
	...
	ConditionalAppearanceItem.Appearance.SetParameterValue("Text", NStr("ru = 'Без описания';
																					|en = 'No details';"));
	...
	If PreviousState <> BusinessProcessObject.State Then
		...
		If Not HasRightsToStopBusinessProcess(BusinessProcessObject) Then 
			MessageText = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Недостаточно прав для остановки бизнес-процесса ""%1"".';
					|en = 'Insufficient rights to suspend business process ""%1"".';"),
		...
		If PreviousState = Enums.BusinessProcessStates.Running Then
			...
			If BusinessProcessObject.Completed Then
				Raise NStr("ru = 'Невозможно остановить завершенные бизнес-процессы.';
										|en = 'Cannot suspend the completed business processes.';");
			EndIf;
				
			If Not BusinessProcessObject.Started Then
				Raise NStr("ru = 'Невозможно остановить не стартовавшие бизнес-процессы.';
										|en = 'Cannot suspend the business processes that are not started yet.';");
			EndIf;
			
		ElsIf PreviousState = Enums.BusinessProcessStates.Suspended Then
			
			If BusinessProcessObject.Completed Then
				Raise NStr("ru = 'Невозможно сделать активными завершенные бизнес-процессы.';
										|en = 'Cannot activate the completed business processes.';");
			EndIf;
				
			If Not BusinessProcessObject.Started Then
				Raise NStr("ru = 'Невозможно сделать активными не стартовавшие бизнес-процессы.';
										|en = 'Cannot activate the business processes that are not started yet.';");
			EndIf;
...
Procedure StartDeferredProcess(BusinessProcess) Export
	...
	Except
		...
		LongDesc = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Не удалось отложенно запустить процесс по причине:
			|%1
			|Попробуйте запустить процесс вручную, а не отложенно.';
			|en = 'Cannot delay the process startup due to:
			|%1
...
Procedure NotifyPerformersOnNewTasks() Export
	...
	ErrorDescription = "";
	MessageKind = NStr("ru = 'Бизнес-процессы и задачи.Уведомление о новых задачах';
						|en = 'Business processes and tasks.New task notification';", Common.DefaultLanguageCode());
	...
	WriteLogEvent(MessageKind, EventLogLevel.Information,,,
		StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Начато регламентное уведомление о новых задачах за период %1 - %2';
																	|en = 'Scheduled notification of new tasks for the period %1–%2 is started';"),
	...
	WriteLogEvent(MessageKind, EventLogLevel.Information,,,
		StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Завершено регламентное уведомление о новых задачах (уведомлено исполнителей: %1)';
																	|en = 'Scheduled notification of new tasks is completed (notified assignees: %1)';"),
...
Procedure CheckTasks() Export
	...
	If Not SystemEmailAccountIsSetUp(ErrorDescription) Then
		MessageKind = NStr("ru = 'Бизнес-процессы и задачи.Мониторинг задач';
							|en = 'Business processes and tasks.Task monitoring';", Common.DefaultLanguageCode());
		WriteLogEvent(MessageKind, EventLogLevel.Error,
...
Procedure FinishUpdateAccessValuesSetsPortions(Parameters) Export
	...
	If Parameters.ObjectsProcessed = 0 Then
		MessageText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Процедуре ""%1"" не удалось обновить права доступа для некоторых объектов (пропущены): %2';
				|en = 'Procedure ""%1"" cannot update access rights for some objects (skipped): %2';"),
	...
	WriteLogEvent(InfobaseUpdate.EventLogEvent(), EventLogLevel.Information,
		...
		StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Процедура ""%1"" обновила права доступа для очередной порции объектов: %2';
																	|en = 'The ""%1"" procedure has updated access rights for objects: %2';"), 
...
Procedure OnAddUpdateHandlers(Handlers) Export
	...
	Handler.Comment = NStr("ru = 'Заполнение реквизита Состояние в регистре Бизнес-процессы.';
									|en = 'Populate the State attribute in the ""Business processes"" information register.';");
...
Procedure OnFillToDoList(ToDoList) Export
	...
	For Each Section In Sections Do
		...
		ToDoItem.Presentation  = NStr("ru = 'Мои задачи';
									|en = 'My tasks';");
		...
		ToDoItem.Presentation  = NStr("ru = 'просроченные';
									|en = 'overdue';");
		...
		ToDoItem.Presentation  = NStr("ru = 'сегодня';
									|en = 'today';");
		...
		ToDoItem.Presentation  = NStr("ru = 'на этой неделе';
									|en = 'this week';");
		...
		ToDoItem.Presentation  = NStr("ru = 'на следующей неделе';
									|en = 'next week';");
...
Procedure SendNotifAboutOverdueTask(MailMessage)
	...
	If IsBlankString(MailMessage.MailAddress) Then
		MessageText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Уведомление не было отправлено, так как не задан адрес электронной почты у исполнителя задачи %1.';
				|en = 'The notification is not sent as the email address of the %1 task assignee is not specified.';"), 
			Common.SubjectString(MailMessage.Recipient));
		WriteLogEvent(NStr("ru = 'Бизнес-процессы и задачи.Уведомление о просроченных задачах';
										|en = 'Business processes and tasks.Overdue task notification';", 
	...
	If MailMessage.EmailType = "ToPerformer" Then
		...
		EmailSubjectText = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Не выполненные в срок задачи (%1)';
																						|en = 'Overdue tasks (%1)';"),
	...
	ElsIf MailMessage.EmailType = "ToAuthor" Then
		MessageBodyText = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'По введенным задачам прошел крайний срок:
			| 
			|%1';
			|en = 'Deadline for the specified tasks expired:
			| 
			|%1';"), MailMessage.EmailText);
		EmailParameters.Insert("Body", MessageBodyText);
		
		EmailSubjectText = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'По задачам истек контрольный срок (%1)';
																						|en = 'Task deadline expired (%1)';"),
			String(MailMessage.TaskCount));
		EmailParameters.Insert("Subject", EmailSubjectText);
	ElsIf MailMessage.EmailType = "ToCoordinator" Then
		MessageBodyText = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Прошел крайний срок по задачам:
			| 
			|%1';
			|en = 'Task deadline expired:
			| 
			|%1';"), MailMessage.EmailText);
		EmailParameters.Insert("Body", MessageBodyText);
		
		EmailSubjectText = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Истек контрольный срок задач (%1)';
																						|en = 'Task deadline expired (%1)';"),
			String(MailMessage.TaskCount));
		EmailParameters.Insert("Subject", EmailSubjectText);
	EndIf;
	...
	Except
		...
		MessageText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Не отправлены уведомления о просроченных задачах по причине: 
				|%1.';
				|en = 'Overdue task notifications are not sent due to: 
				|%1.';"),
			ErrorDescription);
	...
	If IsBlankString(MessageText) Then
		...
		MessageText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Уведомление о просроченных задачах успешно отправлено на адрес %1.';
				|en = 'Overdue task notification sent to %1.';"), Whom);
...
Procedure CreateTaskForSettingRoles(TaskRef, EmployeesResponsible)
	...
	For Each EmployeeResponsible In EmployeesResponsible Do
		...
		TaskObject.LongDesc = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Задача не может быть исполнена, так как у роли не задано ни одного исполнителя:
		    |%1';
			|en = 'The task cannot be completed as no assignees are assigned for the role:
			|%1';"), String(TaskRef));
		TaskObject.Description = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Назначить исполнителей: задача не может быть исполнена %1';
																								|en = 'Set assignees: task %1 cannot be executed';"), String(TaskRef));
		TaskObject.Write();
	EndDo;
...
Function SendNotificationOnNewTasks(Performer, TasksByExecutive, RecipientsAddresses)
	...
	If IsBlankString(RecipientEmailAddress) Then
		WriteLogEvent(NStr("ru = 'Бизнес-процессы и задачи.Уведомление о новых задачах';
										|en = 'Business processes and tasks.New task notification';", Common.DefaultLanguageCode()),
			EventLogLevel.Information,,,
			StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Уведомление не отправлено, так как не указан почтовый адрес у исполнителя задачи %1.';
					|en = 'The notification is not sent as the email address of the %1 task assignee is not specified.';"), 
	...
	EndDo;
	EmailSubject = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Направлены задачи - %1';
																				|en = 'Tasks sent- %1';"), Metadata.BriefInformation);
	...
	Except
		WriteLogEvent(NStr("ru = 'Бизнес-процессы и задачи.Уведомление о новых задачах';
										|en = 'Business processes and tasks.New task notification';",
			Common.DefaultLanguageCode()), 
			EventLogLevel.Error,,,
			StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Уведомления о новых задачах не отправлены по причине:
				|%1';
				|en = 'New task notifications are not sent due to:
				|%1';"), 
				ErrorProcessing.DetailErrorDescription(ErrorInfo())));
		Return False;
	EndTry;
	...
	WriteLogEvent(NStr("ru = 'Бизнес-процессы и задачи.Уведомление о новых задачах';
		...
		Common.DefaultLanguageCode()),
		EventLogLevel.Information,,,
		StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Уведомления успешно отправлены на адрес %1.';
																	|en = 'Notifications sent to %1.';"), 
...
Function GenerateTaskPresentation(TaskStructure)
	...
	Result = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = '%1
		...
		|Deadline: %2';") + Chars.LF,
		TaskStructure.Ref, 
		Format(TaskStructure.TaskDueDate, NStr("ru = 'ДЛФ=DD; ДП=''не указан''';
													|en = 'DLF=DD; DE=''not specified''';")));
	If ValueIsFilled(TaskStructure.Performer) Then
		Result = Result + StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Исполнитель: %1';
																							|en = 'Assignee: %1';"), 
			TaskStructure.Performer) + Chars.LF;
	EndIf;
	If ValueIsFilled(TaskStructure.PerformerRole) Then
		Result = Result + StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Роль: %1';
																							|en = 'Role: %1';"), 
			TaskStructure.PerformerRole) + Chars.LF;
	EndIf;
	If ValueIsFilled(TaskStructure.MainAddressingObject) Then
		Result = Result + StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Основной объект адресации: %1';
																							|en = 'Main business object: %1';"), 
			TaskStructure.MainAddressingObject) + Chars.LF;
	EndIf;
	If ValueIsFilled(TaskStructure.AdditionalAddressingObject) Then
		Result = Result + StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Доп. объект адресации: %1';
																							|en = 'Additional business object: %1';"), 
			TaskStructure.AdditionalAddressingObject) + Chars.LF;
	EndIf;
	If ValueIsFilled(TaskStructure.Author) Then
		Result = Result + StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Автор: %1';
																							|en = 'Author: %1';"), 
			TaskStructure.Author) + Chars.LF;
	EndIf;
	If ValueIsFilled(TaskStructure.LongDesc) Then
		Result = Result + Chars.LF + StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = '%1';
																											|en = '%1';"), 
...
Function EventLogEvent() Export
	Return NStr("ru = 'Бизнес-процессы и задачи';
				|en = 'Business processes and tasks';", Common.DefaultLanguageCode());
EndFunction
...
Procedure SetMyTasksListParameters(List) Export
	...
	List.Parameters.SetParameterValue("Overdue", " " + NStr("ru = 'Просрочено';
																			|en = 'Overdue';")); // Insert a space for sort purposes.
	List.Parameters.SetParameterValue("Today", NStr("ru = 'В течение сегодня';
																|en = 'Today';"));
	List.Parameters.SetParameterValue("ThisWeek", NStr("ru = 'До конца недели';
																	|en = 'Till the end of the week';"));
	List.Parameters.SetParameterValue("NextWeek", NStr("ru = 'На следующей неделе';
																		|en = 'Next week';"));
	List.Parameters.SetParameterValue("Later", NStr("ru = 'Позднее';
																|en = 'Later';"));
...
Procedure FillEmployeeResponsibleForCompletionControl() Export
	...
	RoleObject1.Description = NStr("ru = 'Координатор выполнения задач';
									|en = 'Task control manager';");
...
Function SystemEmailAccountIsSetUp(ErrorDescription)
	...
	If Not Common.SubsystemExists("StandardSubsystems.EmailOperations") Then
		ErrorDescription = NStr("ru = 'Отправка почты не предусмотрена в приложении.';
								|en = 'Sending email messages is not supported in the app.';");
	Else
		ModuleEmailOperations = Common.CommonModule("EmailOperations");
		If ModuleEmailOperations.AccountSetUp(ModuleEmailOperations.SystemAccount(), True, False) Then
			Return True;
		EndIf;
		ErrorDescription = NStr("ru = 'Не настроена электронная почта для рассылки уведомлений.';
								|en = 'The service email account is not set up.';");
	EndIf;
...
Procedure FillPredefinedItemDescriptionAllAddressingObjects() Export
	...
	Try
		...
		AllAddressingObjects.Description = NStr("ru = 'Все объекты адресации';
												|en = 'All business objects';");
