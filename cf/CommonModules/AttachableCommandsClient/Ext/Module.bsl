...
Procedure ContinueCommandExecution(ExecutionParameters)
	...
	If ExecutionParameters.Form.ReadOnly And ExecutionParameters.CommandDetails.ChangesSelectedObjects Then
		ShowMessageBox(, NStr("ru = 'Для выполнения этого действия необходимо разрешить редактирование в форме.';
										|en = 'To perform this action, you must allow editing in the form.';"));
		Return;
	EndIf;
	...
	If ExecutionParameters.WritingRequired Then
		...
		If Source.Ref.IsEmpty()
			...
			If ExecutionParameters.PostingRequired Then
				QuestionTemplate = NStr("ru = 'Для выполнения команды ""%1""
					|документ будет проведен. Продолжить?';
					|en = 'The document will be posted in order to run the ""%1"" command.
					|Do you want to continue?';");
				Buttons.Add(DialogReturnCode.OK, NStr("ru = 'Провести и продолжить';
			...
			Else
				...
				Buttons.Add(DialogReturnCode.OK, NStr("ru = 'Записать и продолжить';
		...
		If DocumentsInfo.Unposted.Count() > 0 Then
			If DocumentsInfo.HasRightToPost Then
				If DocumentsInfo.Unposted.Count() = 1 Then
					QueryText = NStr("ru = 'Для выполнения команды предварительно проведите документ. Выполнить проведение документа и продолжить?';
										|en = 'Cannot run the command for unposted documents. Do you want to post the document and continue?';");
				Else
					QueryText = NStr("ru = 'Для выполнения команды предварительно проведите документы. Выполнить проведение документов и продолжить?';
										|en = 'Cannot run the command for unposted documents. Do you want to post the document and continue?';");
				EndIf;
			...
			Else
				If DocumentsInfo.Unposted.Count() = 1 Then
					WarningText = NStr("ru = 'Для выполнения команды предварительно проведите документ. Недостаточно прав для проведения документа.';
												|en = 'Cannot run the command for unposted documents. You are not authorized to post the document.';");
				Else
					WarningText = NStr("ru = 'Для выполнения команды предварительно проведите документы. Недостаточно прав для проведения документов.';
												|en = 'Cannot run the command for unposted documents. You are not authorized to post the document.';");
				EndIf;
...
Procedure ContinueCommandExecutionAfterConfirmPosting(Response, Context) Export
	...
	If UnpostedDocuments.Count() > 0 Then
		...
		If Context.ReferencesArrray.Count() = 0 Then
			DialogButtons.Add(DialogReturnCode.Cancel, NStr("ru = 'ОК';
																	|en = 'OK';"));
		Else
