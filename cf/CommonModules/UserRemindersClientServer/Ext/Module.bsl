...
Function TimePresentation(Val Time, FullPresentation = True, OutputSeconds = True) Export
	...
	SecondsPresentation = NStr("ru = ';%1 секунду;;%1 секунды;%1 секунд;%1 секунды';
								|en = ';%1 second;;;;%1 seconds';");
...
Function ReplaceUnitOfMeasureByMultiplier(Val Unit)
	...
	AllowedChars = NStr("ru = 'абвгдеёжзийклмнопрстуфхцчшщъыьэюя';
	...
	WordFormsForWeek = StrSplit(NStr("ru = 'нед,н';
										|en = 'wk,w,wee';"), ",", False);
	WordFormsForDay = StrSplit(NStr("ru = 'ден,дне,дня,дн,д';
										|en = 'day,d';"), ",", False);
	WordFormsForHour = StrSplit(NStr("ru = 'час,ч';
										|en = 'hrs,hr,h,hou';"), ",", False);
	WordFormsForMinute = StrSplit(NStr("ru = 'мин,м';
										|en = 'min,m';"), ",", False);
	WordFormsForSecond = StrSplit(NStr("ru = 'сек,с';
											|en = 'sec,s';"), ",", False);
...
Function EnumPresentationOnOccurrence() Export
	...
	Return NStr("ru = 'при наступлении';
				|en = 'on occurrence';");
...
Function ReminderTimePresentation(Reminder) Export
	...
	If Reminder.ReminderTimeSettingMethod = PredefinedValue("Enum.ReminderTimeSettingMethods.RelativeToSubjectTime") Then
		...
		Else
			Return StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'за %1';
					|en = '%1 before';"), TimePresentation(Reminder.ReminderInterval));
		EndIf;
	ElsIf Reminder.ReminderTimeSettingMethod = PredefinedValue("Enum.ReminderTimeSettingMethods.AtSpecifiedTime")
		...
		Return StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = '%1';
				|en = '%1';"), Format(Reminder.ReminderTime, "DLF=DT;"));
	Else
