...
Procedure BeforeExit(Cancel, Warnings) Export
	...
	If SessionTerminationInProgress() Then
		...
		WarningParameters.HyperlinkText = NStr("ru = 'Блокировка работы пользователей';
														|en = 'User access';");
		WarningParameters.WarningText = NStr("ru = 'Из текущего сеанса выполняется завершение работы пользователей';
															|en = 'User sessions are being closed from the current session.';");
...
Procedure SessionTerminationModeManagement(CurrentMode)
	...
	Template = NStr("ru = 'Рекомендуется завершить текущую работу и сохранить все свои данные. Работа приложения будет завершена %1 в %2. 
		|%3';
		|en = 'Please save your data. The app will be temporarily unavailable starting %1, %2.
		|%3';");
...
Procedure EndUserSessions(CurrentMode)
	...
	If CurrentMoment < LockBeginTime Then
		...
		MessageText = NStr("ru = 'Блокировка работы пользователей запланирована на %1.';
								|en = 'The application will be temporarily unavailable since %1.';");
		MessageText = StringFunctionsClientServer.SubstituteParametersToString(MessageText, LockBeginTime);
		ShowUserNotification(NStr("ru = 'Завершение работы пользователей';
	...
	Except
		...
		ShowUserNotification(NStr("ru = 'Завершение работы пользователей';
			...
			NStr("ru = 'Завершение сеансов не выполнено. Подробности см. в Журнале регистрации.';
				|en = 'Cannot close sessions. For more information, see the event log.';"),
	...
	ShowUserNotification(NStr("ru = 'Завершение работы пользователей';
										|en = 'Closing user sessions';"),
		ClickNotification,
		NStr("ru = 'Завершение сеансов выполнено успешно';
			|en = 'All user sessions are closed.';"),
...
Procedure TerminateThisSession(OutputQuestion1 = True)
	...
	MessageText = NStr("ru = 'Работа пользователей с приложением запрещена. Завершить работу этого сеанса?';
							|en = 'User access is restricted. Exit the app?';");
	Title = NStr("ru = 'Завершение работы текущего сеанса';
					|en = 'Close current session';");
...
Procedure BeforeStartInteractiveHandler(Parameters, Context) Export
	...
	If Not IsBlankString(QueryText) Then
		...
		Buttons.Add(DialogReturnCode.Yes, NStr("ru = 'Войти';
													|en = 'Log in';"));
		If ClientParameters.CanUnlock Then
			Buttons.Add(DialogReturnCode.No, NStr("ru = 'Снять блокировку и войти';
														|en = 'Remove lock and log in';"));
		EndIf;
...
Procedure ShowWarningOnExit(MessageText, LockBeginTime)
	...
	If Not InformParameters.IsNotificationDisplayed Then
		ShowUserNotification(NStr("ru = 'Работа приложения будет завершена';
											|en = 'App will be closed';"),, MessageText,, 
...
Procedure ClearUserNotifications()
	...
	ShowUserNotification(NStr("ru = 'Завершение работы отменено';
										|en = 'Closing the application is canceled';"),, ,,
		UserNotificationStatus.Important, "UserSessionsEndControl");
	...
	If TheKeyIsContainedInTheStartupParameters(StartupParameters, ParameterNameAllowUsers) Then
		...
		If Not IBConnectionsServerCall.AllowUserAuthorization() Then
			MessageText = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Параметр запуска %1 не отработан. Нет прав на администрирование информационной базы.';
					|en = 'The %1 parameter is ignored because you do not have administrative rights.';"),
		...
		EventLogClient.AddMessageForEventLog(EventLogEvent(),,
			StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Выполнен запуск с параметром %1. Работа приложения будет завершена.';
					|en = 'App was started with parameter %1. App will be closed.';"),
...
Function EventLogEvent() Export
	...
	Return NStr("ru = 'Завершение работы пользователей';
				|en = 'User sessions';", CommonClient.DefaultLanguageCode());
