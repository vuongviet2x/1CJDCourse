...
Procedure ShowPutFileOnAttachFileSystemExtension(ExtensionAttached, Context) Export
	...
	If Not ExtensionAttached
		...
		Raise NStr("ru = 'Невозможно загрузить файл без установленного расширения для работы с 1С:Предприятием.';
								|en = 'Cannot upload the file because 1C:Enterprise Extension is not installed.';");
	EndIf;
...
Procedure OpenFileInViewer(PathToFile, Val Notification = Undefined,
	...
	SuggestionText = NStr("ru = 'Для открытия файла установите расширение для работы с 1С:Предприятием.';
							|en = 'To open the file, install 1C:Enterprise Extension.';");
...
Procedure OpenFileInViewerAfterCheckFileSystemExtension(ExtensionAttached, Context) Export
	...
	Else
		...
		ErrorDescription = NStr("ru = 'Не установлено расширение для работы с 1С:Предприятием, открытие файла недоступно.';
								|en = 'Cannot open the file because 1C:Enterprise Extension is not installed.';");
...
Procedure OpenFileInViewerAfterCheckIfExists(Exists, Context) Export
	...
	Else 
		...
		ErrorDescription = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Не существует открываемый файл:
			           |%1';
						|en = 'The file to open does not exist:
						|%1';"),
			FileInfo3.FullName);
		OpenFileInViewerNotifyOnError(ErrorDescription, Context);
...
Procedure OpenExplorerAfterCheckFileSystemExtension(ExtensionAttached, Context) Export
	...
	If ExtensionAttached Then
		...
		FileInfo3.BeginCheckingExistence(Notification);
	Else
		ErrorDescription = NStr("ru = 'Для того чтобы открыть папку, установите расширение для работы с 1С:Предприятием.';
...
Procedure OpenExplorerAfterCheckIfExists(Exists, Context) Export 
	...
	Else 
		ErrorDescription = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Каталог, который требуется открыть в проводнике, не существует:
			           |""%1""';
						|en = 'The directory to be opened in the Explorer, does not exist:
						|""%1""';"),
			FileInfo3.FullName);
		OpenExplorerNotifyOnError(ErrorDescription, Context);
	EndIf;
...
Procedure OpenURLAfterCheckFileSystemExtension(ExtensionAttached, Context) Export
	...
	Else
		ErrorDescription = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Расширение для работы с 1С:Предприятием не установлено, переход по ссылке ""%1"" невозможен.';
				|en = 'Cannot follow link ""%1"" because 1C:Enterprise Extension is not installed.';"),
			URL);
...
Procedure StartApplicationAfterCheckFileSystemExtension(ExtensionAttached, Context) Export
	...
	Else
		ErrorDescription = NStr("ru = 'Не установлено расширение для работы с 1С:Предприятием, запуск приложения невозможен.';
								|en = 'Cannot start the app because 1C:Enterprise Extension is not installed.';");
...
Procedure StartApplicationAfterCheckIfExists(Exists, Context) Export
	...
	Else 
		...
		ErrorDescription = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Не удалось запустить приложение
			           |%1
	...
	If Context.WaitForCompletion And ReturnCode = Undefined Then
		Result.ErrorDescription = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Возникла непредвиденная ситуация при запуске приложения
				|%1';
				|en = 'An unexpected error occurred upon the startup of
				|%1';"),
		...
		ErrorDescription = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Возникла непредвиденная ситуация при запуске приложения:
				...
				|en = 'An unexpected error occurred upon the startup:
				|Command line: %1
				|Directory: %2
				|Return code: %3
		...
		EventLogClient.AddMessageForEventLog(
			NStr("ru = 'Стандартные подсистемы';
				|en = 'Standard subsystems';", CommonClient.DefaultLanguageCode()),
...
Procedure CreateTemporaryDirectoryAfterCheckFileSystemExtension(ExtensionAttached, Context) Export
	...
	Else
		CreateTemporaryDirectoryNotifyOnError(NStr("ru = 'Не удалось установить расширение для работы с 1С:Предприятием.';
														|en = 'Cannot install 1C:Enterprise Extension.';"), Context);
	EndIf;
