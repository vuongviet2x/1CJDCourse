...
Function CreateEmail(Message, Account, SendImmediately = True) Export
	...
	Except
		...
		MessageTextTemplate = NStr("ru = 'Не удалось сформировать письмо по причине:
			|%1';
			|en = 'Cannot generate a mail. Reason:
			|%1';");
	...
	Except
		...
		MessageTextTemplate = NStr("ru = 'Не удалось отправить письмо по причине:
				|%1';
				|en = 'Cannot send the mail. Reason:
				|%1';");
	...
	If Not MailMessage.DeleteAfterSend Then
		...
		Except
			...
			ErrorInfo = ErrorInfo();
			MessageTextTemplate = NStr("ru = 'Не удалось сохранить письмо в приложении после успешной отправки по причине:
				|%1';
				|en = 'Couldn''t save outgoing mail. Reason:
				|%1';");
...
Procedure CreateAndSendSMSMessage(Message) Export
	...
	SMSMessage.Comment = NStr("ru = 'Создано и отправлено по шаблону';
									|en = 'Created from template and sent';") + " - " + Message.AdditionalParameters.Description;
...
Procedure OnFillToDoList(ToDoList) Export
	...
	For Each Section In Sections Do
		...
		UserTaskParent.Presentation  = NStr("ru = 'Почта';
											|en = 'Mailbox';");
...
Procedure OnAddUpdateHandlers(Handlers) Export
	...
	Handler.Comment =
		NStr("ru = 'Заполняет в справочнике ""Папки электронных писем"" тип предопределенной папки';
			|en = 'Fills in the predefined folder type in the ""Email folders"" catalog';");
	...
	Handler.Comment =
		NStr("ru = 'Заполнение реквизита ""Текст"" документа ""Электронное письмо исходящее"" для писем в формате HTML у которых он ранее не был ошибочно заполнен';
			|en = 'Filling in the ""Text"" attribute of the ""Outbox e-mail"" document for HTML messages which were not previously filled in with it by mistake';");
	Handler.ExecutionMode = "Deferred";
...
Procedure OnSearchForReferenceReplacement(ReplacementPairs, UnprocessedOriginalsValues) Export
	...
	For Each UnprocessedDuplicate In UnprocessedOriginalsValues Do
		...
		Except
			RollbackTransaction();
			WriteLogEvent(NStr("ru = 'Поиск и замена ссылок';
											|en = 'Find and replace references';", Common.DefaultLanguageCode()),
				EventLogLevel.Error,
...
Function AvailableSearchesList(FTSEnabled, Parameters, FormItems, ForAddressBook) Export
	...
	AddSearchOption(AllSearchLists, FormItems.SearchOptions, "ByEmail", NStr("ru = 'По email';
																						|en = 'In email address';"), Address);
	AddSearchOption(AllSearchLists, FormItems.SearchOptions, "ByDomain", NStr("ru = 'По доменному имени';
																							|en = 'In domain name';"), DomainAddress);
	
	If Not ForAddressBook And (Parameters.Property("EmailOnly") And Not Parameters.EmailOnly) Then
		AddSearchOption(AllSearchLists, FormItems.SearchOptions, "ByPhone", NStr("ru = 'По телефону';
																								|en = 'In phone number';"), Address);
	EndIf;
	...
	If FTSEnabled Then
		AddSearchOption(AllSearchLists, FormItems.SearchOptions, "ByLine",
			NStr("ru = 'По строке';
				|en = 'In description';"), SearchByStringOptions);
	EndIf;
	
	AddSearchOption(AllSearchLists, FormItems.SearchOptions, "BeginsWith", NStr("ru = 'Начинается с';
																							|en = 'Begins with';"), Presentation);
...
Function FullTextContactsSearchByRow(Val SearchString, FoundContacts, Val ForAddressBook = False) Export
	...
	If FullTextSearch.GetFullTextSearchMode() = FullTextSearchMode.Disable
		Or Not GetFunctionalOption("UseFullTextSearch") Then
		Return NStr("ru = 'Использование полнотекстового поиска данных запрещено.';
					|en = 'Full-text search is not available.';");
	EndIf;
	...
	Except
		Return NStr("ru = 'Не удалось выполнить поиск, попробуйте изменить выражение поиска.
		              ...
		              |Подробно о поисковых выражениях полнотекстового поиска см. справку.';
						|en = 'Nothing was found. Please try another search expression.
	...
	Return ?(FoundItemsCount1 < 101, "", NStr("ru = 'Уточните параметры поиска. В списке отображены не все найденные контакты.';
															|en = 'Refine the search parameters. The search result is too big to accommodate in the list.';"));
...
Procedure GenerateChoiceListInteractionTypeEmailOnly(Item)
	...
	Item.ChoiceList.Add("AllEmails", NStr("ru = 'Все письма';
													|en = 'All mail';"));
	Item.ChoiceList.Add("IncomingMessages", NStr("ru = 'Входящие';
														|en = 'Inbox';"));
	Item.ChoiceList.Add("MessageDrafts", NStr("ru = 'Черновики';
															|en = 'Drafts';"));
	Item.ChoiceList.Add("OutgoingMessages", NStr("ru = 'Исходящие';
															|en = 'Outbox';"));
	Item.ChoiceList.Add("SentMessages", NStr("ru = 'Отправленные';
														|en = 'Sent';"));
	Item.ChoiceList.Add("DeletedMessages", NStr("ru = 'Удаленные';
															|en = 'Trash';"));
...
Procedure SetEmailFormHeader(Form) Export
	...
	If Not ObjectEmail.Ref.IsEmpty() Then
		...
		FormCaption = ?(IsBlankString(ObjectEmail.Subject), NStr("ru = 'Письмо без темы (%1)';
																|en = 'No-subject email (%1)';"), ObjectEmail.Subject + " (%1)");
		Form.Title  = StringFunctionsClientServer.SubstituteParametersToString(FormCaption,
			?(TypeOf(ObjectEmail.Ref) = Type("DocumentRef.IncomingEmail"), NStr("ru = 'Входящее';
																									|en = 'Incoming';"), NStr("ru = 'Исходящее';
																														|en = 'Outgoing';")));
			
	Else
		If TypeOf(ObjectEmail.Ref) = Type("DocumentRef.OutgoingEmail") Then
			Form.AutoTitle = False;
			Form.Title = NStr("ru = 'Исходящее письмо (создание)';
									|en = 'Mail message (Create)';");
		EndIf;
	EndIf;
...
Procedure AddContactsPickupFormPages(Form) Export
	...
	For Each ContactDescription In ContactsDetails Do
		...
		If HasAddressColumns[ContactDescription.Name] Then
			...
			AddressColumn.Title = NStr("ru = 'Адрес';
											|en = 'Email';");
		EndIf;
...
Procedure InitializeInteractionsListForm(Form, Parameters) Export
	...
	If Form.OnlyEmail Then
		Form.Title = NStr("ru = 'Электронная почта';
								|en = 'Email';");
		...
		Form.Commands.SubjectOf.Title = NStr("ru = 'Установить предмет переписки';
												|en = 'Choose topic';");
		Form.Commands.SubjectOf.ToolTip = NStr("ru = 'Установить предмет переписки';
												|en = 'Choose topic';");
		...
		If Form.Commands.Find("SubjectList") <> Undefined Then
			Form.Commands.SubjectList.Title = NStr("ru = 'Установить предмет переписки';
														|en = 'Choose topic';");
			Form.Commands.SubjectList.ToolTip = NStr("ru = 'Установить предмет переписки';
														|en = 'Choose topic';");
		EndIf;
	EndIf;
...
Function GenerateEmailHeaderDataItem(ParentElement, EmailHeader1, OnlyBySenderPresentation = False)
	...
	Subject = ?(IsBlankString(EmailHeader1.Subject), NStr("ru = '<Без Темы>';
													|en = '<No Subject>';"), EmailHeader1.Subject);
...
Procedure AddAttachmentFooterToEmailBody(HTMLDocument, Attachments) Export
	...
	AttachmentsCountString = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Вложений: %1.';
																							|en = '%1 attachments.';"), Attachments.Count());
	AddTextNode(FontItem, AttachmentsCountString, True, True);
	
	For Each Attachment In Attachments Do 
		
		AttachmentPresentation = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = '%1 (%2).';
																							|en = '%1 (%2).';"), Attachment.FileName, Attachment.SizePresentation);
...
Function SendSMSMessageByDocument(Document) Export
	...
	If Not SendSMSMessage.SMSMessageSendingSetupCompleted() Then
		Common.MessageToUser(NStr("ru = 'Не выполнены настройки отправки SMS.';
													|en = 'SMS settings not configured.';"),, "Object");
...
Procedure SendSMS() Export
	...
	If Not SendSMSMessage.SMSMessageSendingSetupCompleted() Then
		WriteLogEvent(EmailManagement.EventLogEvent(), 
			EventLogLevel.Error, , ,
			NStr("ru = 'Не выполнены настройки отправки SMS.';
				|en = 'SMS settings not configured.';", Common.DefaultLanguageCode()));
		Return;
	EndIf;
	...
	While DocumentsSelection.Next() Do
		...
		Except
			MessageText = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Не удалось отправить %1 по причине: 
				|%2';
				|en = 'Cannot send %1 due to: 
				|%2';"),
		...
		Except
			...
			MessageText = StringFunctionsClientServer.SubstituteParametersToString(MessageText, 
				NStr("ru = 'Не удалось записать факт отправки %1 по причине: 
				|%2';
				|en = 'Cannot record sending of %1 due to: 
				|%2';"),
...
Procedure SMSDeliveryStatusUpdate() Export
	...
	ProcessedDocuments_ = New Map;
	MessagesIDs = New Array;
	...
	For Each DocumentToProcess In ProcessedDocuments_ Do
		...
		Try
			...
			For Each Addressee In SMSMessageRecipients Do
				...
				If MessageState <> Addressee.MessageState Then
					...
					NewRow.LineNumber        = Addressee.LineNumber;
		...
		Except
			...
			RollbackTransaction();
			MessageText = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Не удалось обновить статусы доставки %1 по причине: %2';
					|en = 'Failed to update delivery status: %1. Reason: %2';"),
				DocumentPresentation_, ErrorProcessing.DetailErrorDescription(ErrorInfo()));
			WriteLogEvent(EmailManagement.EventLogEvent(),
...
Procedure ReportSMSMessageSendingResultsInDocument(DocumentObject, SendingResult)
	...
	For Each Addressee In DocumentObject.SMSMessageRecipients Do
		...
		Else
			Addressee.MessageState = Enums.SMSMessagesState.NotDelivered;
		EndIf;
	EndDo;
...
Function DefineFolderForEmail(MailMessage) Export
	...
	While Selection.Next() Do
		...
		Except
			...
			ErrorMessageTemplate = NStr("ru = 'Не удалось применить правило обработки писем ""%1"" для почты ""%2"" по причине: 
			                                |%3
			                                |Требуется исправить правило.';
											|en = 'Cannot apply the ""%1"" mailbox rule to the ""%2"" account due to: 
											|%3
...
Procedure FillTimeSelectionList(FormInputField, Interval = 3600) Export
	...
	While BegOfHour(ListTime) <= BegOfHour(WorkdayEnd) Do
		...
		Else
			TimePresentation = Format(ListTime, NStr("ru = 'ДФ=ЧЧ:мм';
															|en = 'DF=HH:mm';"));
		EndIf;
...
Procedure ProcessFilterByInteractionsTypeSubmenu(Form) Export
	...
	TitleTemplate1 = NStr("ru = 'Показывать: %1';
							|en = 'Show %1';");
...
Function FiltersListByInteractionsType(OnlyEmail) Export
	...
	FiltersList.Add("All", NStr("ru = 'Все';
										|en = 'All';"));
	FiltersList.Add("AllEmails", NStr("ru = 'Все письма';
											|en = 'All mail';"));
	If Not OnlyEmail Then
		FiltersList.Add("Meetings", NStr("ru = 'Встречи';
												|en = 'Appointments';"));
		FiltersList.Add("PhoneCalls", NStr("ru = 'Телефонные звонки';
														|en = 'Phone calls';"));
		FiltersList.Add("PlannedInteractions", NStr("ru = 'Запланированные взаимодействия';
																	|en = 'Scheduled interactions';"));
		FiltersList.Add("SMSMessages", NStr("ru = 'Сообщения SMS';
													|en = 'Text messages';"));
	EndIf;
	FiltersList.Add("IncomingMessages", NStr("ru = 'Входящие';
													|en = 'Inbox';"));
	FiltersList.Add("MessageDrafts", NStr("ru = 'Черновики';
													|en = 'Drafts';"));
	FiltersList.Add("OutgoingMessages", NStr("ru = 'Исходящие';
													|en = 'Outbox';"));
	FiltersList.Add("SentMessages", NStr("ru = 'Отправленные';
												|en = 'Sent';"));
	FiltersList.Add("DeletedMessages", NStr("ru = 'Удаленные';
													|en = 'Trash';"));
	If Not OnlyEmail Then
		FiltersList.Add("OutgoingCalls", NStr("ru = 'Исходящие звонки';
														|en = 'Outgoing calls';"));
		FiltersList.Add("IncomingCalls", NStr("ru = 'Входящие звонки';
														|en = 'Incoming calls';"));
	EndIf;
...
Function StatusesList() Export
	...
	StatusesList.Add("All", NStr("ru = 'Все';
										|en = 'All';"));
	StatusesList.Add("ToReview", NStr("ru = 'К рассмотрению';
													|en = 'Pending review';"));
	StatusesList.Add("Deferred3", NStr("ru = 'Отложенные';
												|en = 'Deferred';"));
	StatusesList.Add("ReviewedItems", NStr("ru = 'Рассмотренные';
													|en = 'Reviewed';"));
...
Function EmailPresentation(EmailSubject, EmailDate) Export
	...
	Return StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = '%1 от %2';
																		|en = '%1, %2';"), 
...
Function SendingPausedWarningText() Export
	...
	If Not Users.IsFullUser() Then
		Return NStr("ru = 'Получение и отправка писем приостановлены. Обратитесь к администратору.';
					|en = 'Mail sync is paused. Contact the Administrator.';");
	EndIf;
...
Procedure FillChoiceListForReviewAfter(ChoiceList) Export
	...
	ChoiceList.Clear();
	ChoiceList.Add(15*60,    NStr("ru = 'Через 15 мин.';
										|en = 'Snooze for 15 min';"));
	ChoiceList.Add(30*60,    NStr("ru = 'Через 30 мин.';
										|en = 'Snooze for 30 min';"));
	ChoiceList.Add(60*60,    NStr("ru = 'Через 1 час';
										|en = 'Snooze for 1 hour';"));
	ChoiceList.Add(3*60*60,  NStr("ru = 'Через 3 часа';
										|en = 'Snooze for 3 hours';"));
	ChoiceList.Add(24*60*60, NStr("ru = 'Завтра';
										|en = 'Snooze for 24 hours';"));
