...
Function SecurityProfile(Val ClusterAdministrationParameters, Val ProfileName, Val ClusterID = Undefined) Export
	...
	If SecurityProfiles.Count() <> 1 Then
		Raise StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'В кластере серверов %1 не зарегистрирован профиль безопасности %2';
																						|en = 'Security profile %2 is not registered in server cluster %1.';"), ClusterID, ProfileName);
	EndIf;
...
Procedure CreateSecurityProfile(Val ClusterAdministrationParameters, Val SecurityProfileProperties) Export
	...
	If SecurityProfiles.Count() = 1 Then
		Raise StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'В кластере серверов %1 уже зарегистрирован профиль безопасности %2';
																						|en = 'Security profile %2 is already registered in server cluster %1.';"), ClusterID, ProfileName);
	EndIf;
...
Function InfoBaseID(Val ClusterID, Val ClusterAdministrationParameters, Val InfobaseAdministrationParameters) Export
	...
	Else
		Raise StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'В кластере серверов %1 не зарегистрирована информационная база %2';
																						|en = 'Infobase %2 is not registered in server cluster %1.';"), ClusterID, InfobaseAdministrationParameters.NameInCluster);
	EndIf;
...
Function ClusterID(Val ClusterAdministrationParameters) Export
	...
	Else
		Raise StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Не обнаружен кластер серверов с портом %1';
																						|en = 'Cannot find a server cluster with port %1.';"), ClusterAdministrationParameters.ClusterPort);
	EndIf;
...
Function CastOutputItem(OutputItem, ElementType)
	...
	ElsIf ElementType = Type("Number") Then
		...
		Except
			Raise NStr("ru = 'Неправильный формат.';
									|en = 'Invalid format.';");
		EndTry;
	...
	Else
		...
		Raise NStr("ru = 'Неверный тип элемента.';
...
Function RunCommand(Command, ClusterAdministrationParameters, Dictionary = Undefined, Filter = Undefined, PropertyTypes = Undefined)
	...
	If Not ClientFile.Exists() Then
		...
		Raise StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Невозможно выполнить операцию администрирования кластера серверов по причине: не существует файл %1
			           |
			           ...
			           |- Для компьютеров с ОС Linux установите пакет 1c-enterprise83-server*.';
						|en = 'Cannot perform the operation of server cluster administration as the %1 file does not exist.
						|
...
Function ProfileSupportedProperties(ClusterAdministrationParameters)
	...
	ProfileDetails = NStr("ru = 'Служебный профиль для проверки поддерживаемых свойств.';
							|en = 'Service profile for testing supported properties.';");
