...
Procedure RecordKeyOperationDuration(Parameters)
	...
	If Not ValueIsFilled(KeyOperationStartDate) Then
		WriteLogEvent(NStr("ru = 'Невозможно зафиксировать замер с пустой датой начала';
										|en = 'Sampling error. Start date required';",
...
Procedure WriteTimeMeasurements(MeasurementsArray)
	...
	If RecordSet.Count() > 0 Then
		...
		Except
			WriteLogEvent(NStr("ru = 'Оценка производительности.Ошибка сохранения замеров';
											|en = 'Performance monitor.Error saving measurements';",
...
Procedure LoadPerformanceMonitorFile(FileName, StorageAddress) Export
	...
	Except
		...
		StringPattern = NStr("ru = 'Не удалось распаковать архив %1 %2';
							|en = 'Cannot extract the archive %1 %2';");
