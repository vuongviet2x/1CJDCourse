...
Procedure ImportCurrencyRates() Export
	...
	If Descriptors.Descriptor.Count() < 1 Then
		Raise StringFunctionsClientServer.SubstituteParametersToString(NStr(
			"ru = 'В менеджере сервиса отсутствуют данные вида ""%1""';
...
Procedure CopyCurrencyRates(Val CurrencyCode_) Export
	...
	If CurrencyRef.IsEmpty() Then
		ErrorText = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Валюта с кодом %1 не найдена в справочнике, загрузка курсов отменена.';
																					|en = 'Currency with code %1 is not found in the catalog. Exchange rate import is canceled.';"), CurrencyCode_);
	...
	If ExRates.Count() = 0 Then
		ErrorText = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Отсутствуют курсы для валюты с кодом %1 в поставляемых данных.';
																					|en = 'There are no exchange rates for the currency with code %1 in the default master data.';"), CurrencyCode_);
		WriteLogEvent(NStr("ru = 'Поставляемые данные.Распространение курсов валют по областям данных';
										|en = 'Default master data.Distribute exchange rates to data areas';", Common.DefaultLanguageCode()),
			EventLogLevel.Error,,,
			ErrorText);
	...
	For Each DataArea In AreasForUpdate Do
		...
		Except
			...
			ErrorText = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Не удалось установить разделение сеанса %1 по причине:
				|%2';
				|en = 'Couldn''t set session separation for %1. Reason:
		...
		Except
			...
			ErrorText = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Не удалось обновить курсы валют в области %1 по причине:
				|%2';
				|en = 'Cannot update the exchange rates in data area ""%1"". Reason:
				|%2';", Common.DefaultLanguageCode()),
				Format(DataArea, "NZ=0; NG=0"),
				ErrorProcessing.DetailErrorDescription(ErrorInfo()));
			WriteLogEvent(NStr("ru = 'Поставляемые данные.Распространение курсов валют по областям данных';
	...
	While CurrencySelection1.Next() Do
		...
		While CommonSelection.Next() Do
			...
			If Write Then
				RecordSet.Write();
			Else
				Comment = NStr("ru = 'Загрузка курса валюты %1 на дату %2 отменена из-за нарушения даты запрета изменений.';
									|en = 'The %1 exchange rate import as of %2 is canceled due to a period-end closing date violation.';"); 
				Comment = StringFunctionsClientServer.SubstituteParametersToString(Comment, CurrencySelection1.Code, CommonSelection.Date);
				EventName = NStr("ru = 'Поставляемые данные.Отмена загрузки курсов валюты';
									|en = 'Default master data.Cancel exchange rates import';", Common.DefaultLanguageCode());
