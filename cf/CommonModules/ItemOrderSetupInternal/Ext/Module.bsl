...
Function MoveItem(ItemList, CurrentItemRef, Direction) Export
	...
	If Not AccessParameters.Accessibility Then
		Return NStr("ru = 'Недостаточно прав для изменения порядка элементов.';
					|en = 'You are not authorized to change the item sequence.';");
	EndIf;
	...
	If Information.HasParent And RepresentedAsList And Not ListContainsFilterByParent(DataCompositionSettings) Then
		Return NStr("ru = 'Для изменения порядка элементов установите режим просмотра ""Дерево"" или ""Иерархический список"".';
					|en = 'To change the item sequence, set the view mode to Tree or Hierarchical list.';");
	EndIf;
	...
	If Information.HasOwner And Not ListContainsFilterByOwner(DataCompositionSettings) Then
		Return NStr("ru = 'Для изменения порядка элементов установите отбор по полю ""Владелец"".';
					|en = 'To change the item sequence, filter the list by the Owner field.';");
	EndIf;
	...
	If Information.HasGroups Then
		...
		If IsFolder And Not Information.ForGroups Or Not IsFolder And Not Information.ForItems Then
			Return NStr("ru = 'Выбранный элемент нельзя перемещать.';
						|en = 'Cannot move the selected item.';");
		EndIf;
	EndIf;
...
Function CheckSortingInList(DataCompositionSettings)
	...
	If Not SortingCorrect Then
		Return StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Для перемещения элементов настройте сортировку
			|в списке по полю ""%1"" (по возрастанию)';
			|en = 'To transfer items, sort the
