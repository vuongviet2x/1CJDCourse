...
Function ExecuteReportsMailing(BulkEmail, LogParameters = Undefined, AdditionalSettings = Undefined) Export
	...
	If Not BulkEmailObject.IsPrepared
		...
		If Not BulkEmailObject.IsPrepared Then
			Cause = Cause + Chars.LF + NStr("ru = 'Рассылка не подготовлена';
													|en = 'Distribution not prepared';");
		EndIf;
		If BulkEmailObject.DeletionMark Then
			Cause = Cause + Chars.LF + NStr("ru = 'Рассылка помечена на удаление';
													|en = 'The distribution is marked for deletion.';");
		EndIf;
		
		LogRecord(LogParameters, EventLogLevel.Warning,
			NStr("ru = 'Завершение';
				|en = 'Completing';"), TrimAll(Cause));
		Return False;
	...
	EndIf;
	If DefaultFormats.Count() = 0 Then
		Raise NStr("ru = 'Не установлены форматы по умолчанию.';
								|en = 'Default formats are not set.';");
	EndIf;
	...
	If Not DeliveryParameters.UseFolder
		...
		LogRecord(LogParameters, EventLogLevel.Warning, NStr("ru = 'Не выбран способ доставки.';
																						|en = 'Delivery method is not selected.';"));
		Return False;
	EndIf;
	...
	DeliveryParameters.ResultAddress = CommonClientServer.StructureProperty(AdditionalSettings, "ResultAddress");
...
Function ExecuteBulkEmail(Var_Reports, DeliveryParameters, MailingDescription = "", LogParameters = Undefined) Export
	...
	MessageText = StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = 'Инициализирована рассылка ''%1'', автор: ''%2''';
			|en = 'Report distribution %1 is started by %2';"),
	...
	If ReportsTree.Rows.Count() = 0 Then
		...
		LogRecord(LogParameters,
			...
			NStr("ru = 'Рассылка отчетов не выполнена, так как отчеты пустые или не сформированы из-за ошибок.';
				|en = 'Report distribution failed. Reports are empty or cannot be generated.';"));
		Return False;
	EndIf;
	...
	LogRecord(LogParameters,
		EventLogLevel.Note,
		NStr("ru = 'Начало доставки отчетов получателям.';
			|en = 'Start report distribution to recipients.';"));
	...
	For Each RecipientRow In ReportsTree.Rows Do
		...
		If DeliveryParameters.ShouldAttachReports Then
			...
			// Encrypt each file (unless archiving is selected).
				If ValueIsFilled(DeliveryParameters.CertificateToEncrypt) And Not DeliveryParameters.Archive Then
					...
					For Each Attachment In RecipientsAttachments Do
						...
						Except
							HandleEncryptionError(DeliveryParameters.ResultAddress, DeliveryParameters.CertificateToEncrypt, ErrorInfo(), LogParameters);
							Raise;
						EndTry;
			...
			// Archive attachments.
			ArchiveAttachments(RecipientsAttachments, DeliveryParameters, RecipientRow.Value, LogParameters);
		...
		Try  
			...
			If SendHiddenCopiesToSender Then
				AdditionalInfo = NStr("ru = 'Копия направлена отправителю.';
												|en = 'A copy was sent to the sender.';");
			EndIf;
			MessageText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Отчеты отправлены ''%1'' от %2. %3';
				|en = 'Reports are sent from %2 to ''%1''. %3';"), RecipientPresentation1, SenderSRepresentation,
				AdditionalInfo);
		...
		Except
			MessageText = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Не удалось отправить отчеты получателю ''%1'':';
					|en = 'Cannot send reports to %1:';"), RecipientPresentation1);
	...
	If SharedAttachments.Count() > 0 Or DeliveryParameters.ReportsForEmailText.Count() > 0 Then
		...
		If (DeliveryParameters.UseEmail And DeliveryParameters.ShouldAttachReports)
			...
			// Archive attachments.
			ArchiveAttachments(SharedAttachments, DeliveryParameters, DeliveryParameters.TempFilesDir, LogParameters);
		...
		If ExecuteDelivery(LogParameters, DeliveryParameters, SharedAttachments) Then
			...
			If DeliveryParameters.Recipients <> Undefined Then
				...
				MessageText = StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Отчеты отправлены %1 получателям от %2. %3
					|%4';
					|en = 'Reports are sent from %2 to %1 users. %3
					|%4';"),
	...
	If MailingExecuted Then
		LogRecord(LogParameters, , NStr("ru = 'Рассылка выполнена.';
												|en = 'Report distribution completed.';"));
	Else
		LogRecord(LogParameters, , NStr("ru = 'Рассылка не выполнена.';
...
Function LogParameters(BulkEmail = Undefined) Export
	...
	LogParameters.Insert("EventName", NStr("ru = 'Рассылка отчетов. Запуск по требованию';
												|en = 'Report distribution. Manual start';", Common.DefaultLanguageCode()));
...
Procedure SendBulkEmailsInBackgroundJob(ExecutionParameters, ResultAddress) Export
	...
	If PreliminarySettings = Undefined Then
		PreliminarySettings = New Structure;
	EndIf;
	PreliminarySettings.Insert("ResultAddress", ResultAddress);
	...
	If Total = 0 Then
		MessageText = NStr("ru = 'Выбранные группы не содержат рассылок отчетов.';
								|en = 'The selected groups contain  no report distributions.';");
	ElsIf Total <= 5 Then
		MessageText = "";
		For Each TableRow In MailingsTable Do
			If Not TableRow.IsPrepared Then
				MessageTemplate = NStr("ru = 'Рассылка ""%1"" не подготовлена.';
										|en = 'Report distribution ""%1"" not prepared.';");
			ElsIf Not TableRow.Executed Then
				MessageTemplate = NStr("ru = 'Рассылка ""%1"" не выполнена.';
										|en = 'Report distribution ""%1"" not completed.';");
			ElsIf TableRow.WithErrors Then
				MessageTemplate = NStr("ru = 'Рассылка ""%1"" выполнена частично';
										|en = 'The ""%1"" report distribution is partially completed';");
			Else
				MessageTemplate = NStr("ru = 'Рассылка ""%1"" выполнена.';
										|en = 'Report distribution ""%1"" completed successfully.';");
			EndIf;
	...
	Else
		MessageText = StringFunctionsClientServer.SubstituteParametersToString(
			...
			|Частично: %4
			|Не выполнено: %5';
			|en = 'Prepared %1 out of %2 report distributions
			|Completed: %3
			|Partially: %4
			|Not completed: %5';"),
...
Procedure ReportFormAddCommands(Form, Cancel, StandardProcessing) Export
	...
	CreateCommand.Title = NStr("ru = 'Создать рассылку отчетов...';
									|en = 'Create report distribution';");
	CreateCommand.ToolTip = NStr("ru = 'Создать новую рассылку отчетов и добавить в нее отчет с текущими настройками.';
									|en = 'Include the report with the current settings in a newly created report distribution.';");
	...
	AttachCommand.Title = NStr("ru = 'Включить в существующую рассылку отчетов...';
										|en = 'Choose existing report distribution';");
	AttachCommand.ToolTip = NStr("ru = 'Присоединить отчет с текущими настройками к существующей рассылке отчетов.';
										|en = 'Include the report with the current settings in an existing report distribution.';");
	...
	If MailingsWithReportsNumber > 0 Then
		...
		MailingsCommand.Title = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Рассылки отчета (%1)';
																								|en = 'Report distribution (%1)';"), 
			MailingsWithReportsNumber);
		MailingsCommand.ToolTip = NStr("ru = 'Открыть список рассылок, в которые включен отчет.';
										|en = 'Show the list of distributions that contain the report.';");
...
Procedure OnFillToDoList(ToDoList) Export
	...
	For Each Section In Sections Do
		...
		ToDoItem.Presentation  = NStr("ru = 'Проблемы с рассылками отчетов';
									|en = 'Report distribution issues';");
...
Procedure OnAddUpdateHandlers(Handlers) Export
	...
	Handler.Comment = NStr("ru = 'Установка шаблонов наименований отчетов по умолчанию в рассылках отчетов, 
		|установка флага ""Прикреплять отчеты во вложения"". До завершения обработки, не рекомендуется использовать рассылки отчетов.';
		|en = 'Set default report name templates in report distributions. 
...
Procedure ExecuteScheduledMailing(BulkEmail) Export
	...
	If Not AccessRight("Read", Metadata.Catalogs.ReportMailings) Then
		Raise(NStr("ru = 'Недостаточно прав для просмотра рассылок отчетов.';
								|en = 'Insufficient rights to view the report distribution.';"), ErrorCategory.AccessViolation);
	EndIf;
	...
	If Not Selection.Next() Then
		Raise(NStr("ru = 'Недостаточно прав для просмотра рассылки отчетов.';
								|en = 'Insufficient rights to view the report distribution.';"), ErrorCategory.AccessViolation);
	EndIf;
	If Not Selection.ExecuteOnSchedule Then
		Raise StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'У рассылки отчетов ""%1"" отключен флажок ""Выполнять по расписанию""
				...
				|en = 'The ""Schedule"" check box is cleared for report distribution ""%1"". 
...
//
Function GenerateMailingRecipientsList(BulkEmail, LogParameters = Undefined, DeliveryParameters = Undefined) Export
	...
	If RecipientsType = Type("CatalogRef.Users") Then
		...
		|	MAX(User.Description) AS Description,
	...
	ErrorMessageTextForEventLog = StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = 'Не удалось сформировать список получателей ""%1"" по причине:';
			|en = 'Cannot generate recipient list ""%1"" due to:';"), String(RecipientsType));
	...
	If RecipientsWithoutAnAddress.Count() > 0 And LogParameters <> Undefined Then
		...
		PatternOfTheWarningText = NStr("ru = 'Отчеты не отправлены следующим получателям, так как не заполнен адрес электронной почты:
			|- %1.';
			|en = 'The reports were not sent to the following recipients as the email address is not filled in:
			|-%1.';");
		   ...
		   And DeliveryParameters <> Undefined Then
			For Each RecipientWithoutAddress In RecipientsWithoutAnAddress Do
				...
				If DeliveryParameters.NotifyOnly Then
					HistoryFields.Comment = NStr("ru = 'Уведомления не отправлены по причине: ""Адрес электронной почты не указан"".';
													|en = 'Couldn''t send notifications: Email address not specified.';");
				Else
					HistoryFields.Comment = NStr("ru = 'Отчеты не отправлены по причине: ""Адрес электронной почты не указан"".';
													|en = 'Couldn''t send notifications: Email address not specified.';");
				EndIf;
				HistoryFields.Executed = False;
	...
	If RecipientsList.Count() = 0 Then
		...
		|en = 'Cannot generate recipient list ""%1"". Possible reasons:
...
Function IsDistributionRecipientsSelected(BulkEmail, DeliveryParameters, LogParameters, AdditionalSettings)
	...
	Recipients = GenerateMailingRecipientsList(BulkEmail, LogParameters, DeliveryParameters);
...
Function InitializeReport(LogParameters, ReportParameters, PersonalizationAvailable, FormUniqueID = Undefined) Export
	...
	Except
		ReportParameters.Errors = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Не удалось подключить и загрузить настройки отчета ""%1"".';
				|en = 'Cannot import settings from the ""%1"" report.';"),
	...
	If ReportMailingCached.ReportsToExclude().Find(Connection.RefOfReport) <> Undefined Then
		ReportParameters.Errors = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Отчет ""%1"" не предназначен для рассылки.';
				|en = 'Report ""%1"" is not intended for distribution.';"), String(Connection.RefOfReport));
...
Function GenerateReport(LogParameters, ReportParameters, Recipient = Undefined)
	...
	If Not ReportParameters.Property("Initialized") Then
		LogRecord(LogParameters, ,
			StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Отчет ''%1'' не инициализирован';
																		|en = 'Report ''%1'' not initialized';"), String(ReportParameters.Report)));
		Return Result;
	EndIf;
	...
	If Not Generation1.Success Then
		LogRecord(LogParameters, EventLogLevel.Error,
			StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Отчет ""%1"":';
																		|en = 'Report ""%1"":';"),
...
Function ExecuteDelivery(LogParameters, DeliveryParameters, Attachments) Export
	Result = False;
	ErrorMessageTemplate = NStr("ru = 'Отчеты не доставлены';
									|en = 'Reports are not delivered';");
		...
		Try
			...
			If GetFunctionalOption("RetainReportDistributionHistory") 
			   And TypeOf(LogParameters.Data) = Type("CatalogRef.ReportMailings") Then
				...
				"ru = 'Отчеты помещены в сетевой каталог ''%1''.';
	...
	If DeliveryParameters.UseEmail Then
		...
		If DeliveryParameters.NotifyOnly Then
			ErrorMessageTemplate = NStr("ru = 'Невозможно отправить уведомление о рассылке по электронной почте:';
											|en = 'Cannot send report distribution notification by email:';");
			EmailAttachments1 = New Map;
		ElsIf Not DeliveryParameters.ShouldAttachReports Then
			EmailAttachments1 = New Map;
		Else
			ErrorMessageTemplate = NStr("ru = 'Невозможно отправить отчет по электронной почте:';
											|en = 'Cannot send report by email:';");
...
Procedure LogRecord(LogParameters, Val LogLevel = Undefined, Val Text = "", Val IssueDetails = Undefined) Export
	...
	If TypeOf(IssueDetails) = Type("ErrorInfo") Then
		If WriteToLog Then
			TextForLog = TextForLog + Chars.LF + Chars.LF + ErrorProcessing.DetailErrorDescription(IssueDetails);
		EndIf;	
		TextForUser = TextForUser + Chars.LF + Chars.LF + ErrorProcessing.BriefErrorDescription(IssueDetails);
	ElsIf TypeOf(IssueDetails) = Type("String") Then
		If WriteToLog Then
			TextForLog = TextForLog + Chars.LF + Chars.LF + IssueDetails;
		EndIf;
	...
	If BulkEmail.UseNetworkDirectory Then
		If ValueIsFilled(BulkEmail.NetworkDirectoryWindows) Then
			Item = ModuleSafeModeManager.PermissionToUseFileSystemDirectory(
				...
				True,
				True,
				NStr("ru = 'Сетевой каталог для публикации отчетов с сервера Windows.';
					|en = 'Network directory to publish reports from a Windows server.';"));
		...
		If ValueIsFilled(BulkEmail.NetworkDirectoryLinux) Then
			Item = ModuleSafeModeManager.PermissionToUseFileSystemDirectory(
				...
				True,
				True,
				NStr("ru = 'Сетевой каталог для публикации отчетов с сервера Linux.';
					|en = 'Network directory to publish reports from a Linux server.';"));
			Permissions.Add(Item);
		EndIf;
	EndIf;
	If BulkEmail.UseFTPResource Then
		If ValueIsFilled(BulkEmail.FTPServer) Then
			Item = ModuleSafeModeManager.PermissionToUseInternetResource(
				...
				BulkEmail.FTPPort,
				NStr("ru = 'FTP ресурс для публикации отчетов.';
					|en = 'FTP server to publish reports.';"));
			Permissions.Add(Item);
		EndIf;
	EndIf;
	Return Permissions;
...
Function DeliveryMethodsPresentation(DeliveryParameters) Export
	...
	If Not DeliveryParameters.NotifyOnly Then
		...
		+ ?(PresentationText = "", Prefix, " " + NStr("ru = 'и';
														|en = 'and';")) 
		+ " "
		+ NStr("ru = 'отправлен по почте (см. вложения)';
				|en = 'sent by email (see attachment)';");
	...
	If DeliveryParameters.ExecutedToFolder Then
		...
		+ ?(PresentationText = "", Prefix, " " + NStr("ru = 'и';
														|en = 'and';")) 
		+ " "
		+ NStr("ru = 'доставлен в папку';
				|en = 'delivered to folder';")
		+ " ";
	...
	If DeliveryParameters.ExecutedToNetworkDirectory Then
		...
		+ ?(PresentationText = "", Prefix, " " + NStr("ru = 'и';
														|en = 'and';")) 
		+ " "
		+ NStr("ru = 'доставлен в сетевой каталог';
				|en = 'delivered to network directory';")
		+ " ";
	...
	If DeliveryParameters.ExecutedAtFTP Then
		...
		+ ?(PresentationText = "", Prefix, " " + NStr("ru = 'и';
														|en = 'and';")) 
		+ " "
		+ NStr("ru = 'доставлен на FTP ресурс';
				|en = 'delivered to FTP resource';")
		+ " ";
		...
		While Position1 > 0 Do
			...
			Try
				If TypeOf(KeyAndValue.Value) = Type("StandardPeriod") Then
					ValueWithFormat = NStr("ru = '%StartDate% - %EndDate%';
...
Procedure GenerateAndSaveReport(LogParameters, ReportParameters, ReportsTree, DeliveryParameters, RecipientRef)
		...
		If GetFunctionalOption("RetainReportDistributionHistory")
		   And TypeOf(LogParameters.Data) = Type("CatalogRef.ReportMailings") Then
			If Result.Generated1 Then
				MessageText = StringFunctionsClientServer.SubstituteParametersToString(NStr(
				"ru = '- ""%1"" не был отправлен, так как пустой.';
	...
	// Save a spreadsheet document in formats.
		...
		// Save a report by the built-in subsystem tools.
			If StandardProcessing = True Then
				ErrorTitle = NStr("ru = 'Ошибка записи отчета ''%1'' в формат ''%2'':';
										|en = 'Error saving report %1 as %2:';");
				...
				If FormatParameters.FileType = Undefined Then
					LogRecord(LogParameters, EventLogLevel.Error,
						...
						FormatParameters.Name), NStr("ru = 'Формат не поддерживается.';
													|en = 'Format is not supported.';"));
					Continue;
				EndIf;
...
Function CheckAndFillExecutionParameters(ReportsTable, DeliveryParameters, MailingDescription, LogParameters)
	...
	If Unavailable2.Count() > 0 Then
		LogRecord(LogParameters, EventLogLevel.Error,
			StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'В рассылке есть недоступные отчеты (%1):%2';
																		|en = 'Report distribution contains unavailable reports (%1):%2';"),
			Unavailable2.Count(),
	...
	If DeliveryParameters.UseFolder Then
		If Not ValueIsFilled(DeliveryParameters.Folder) Then
			...
			LogRecord(LogParameters, EventLogLevel.Warning,
				NStr("ru = 'Папка не заполнена, доставка в папку отключена.';
					|en = 'Directory not specified. Delivery to directory is disabled.';"));
		Else
			If Common.SubsystemExists("StandardSubsystems.FilesOperations") Then
				ModuleFilesOperationsInternal = Common.CommonModule("FilesOperationsInternal");
			...
			If Not AccessRight Then
				...
				SetPrivilegedMode(False);
				LogRecord(LogParameters, EventLogLevel.Error,
					StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Недостаточно прав для создания файлов в папке ""%1"".';
																				|en = 'Insufficient rights to create files in directory ""%1"".';"),
					FoldersPresentation));
				Return False;
			EndIf;
		EndIf;
	...
	If DeliveryParameters.UseNetworkDirectory Then
		If Not ValueIsFilled(DeliveryParameters.NetworkDirectoryWindows) 
			...
			If ValueIsFilled(DeliveryParameters.NetworkDirectoryWindows) Then
				SubstitutionValue = NStr("ru = 'Linux';
											|en = 'Linux';");
			ElsIf ValueIsFilled(DeliveryParameters.NetworkDirectoryLinux) Then
				SubstitutionValue = NStr("ru = 'Windows';
											|en = 'Windows';");
			Else
				SubstitutionValue = NStr("ru = 'Windows и Linux';
											|en = 'Windows and Linux';");
			EndIf;
			
			DeliveryParameters.UseNetworkDirectory = False;
			LogRecord(LogParameters, EventLogLevel.Error,
				StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Сетевой каталог %1 не выбран, доставка в сетевой каталог отключена.';
																			|en = 'Network directory %1 not selected. Delivery to network directory is disabled.';"),
				SubstitutionValue));
	...
	If DeliveryParameters.UseFTPResource And Not ValueIsFilled(DeliveryParameters.Server) Then
		...
		LogRecord(LogParameters, EventLogLevel.Error,
			NStr("ru = 'FTP сервер не заполнен, доставка в папку на FTP ресурс отключена.';
				|en = 'FTP server not specified. Delivery to FTP directory is disabled.';"));
	EndIf;
	
	If DeliveryParameters.UseEmail And Not ValueIsFilled(DeliveryParameters.Account) Then
		DeliveryParameters.UseEmail = False;
		LogRecord(LogParameters, EventLogLevel.Error,
			NStr("ru = 'Почта не выбрана, доставка по электронной почте отключена.';
				|en = 'Email account is not selected. Email delivery is disabled.';"));
	EndIf;
	
	If DeliveryParameters.Personalized Then
		If Not DeliveryParameters.UseEmail Then
			LogRecord(LogParameters, EventLogLevel.Error,
				NStr("ru = 'Персонализированная рассылка может быть отправлена только по электронной почте.';
					|en = 'Individual reports can be distributed via email only.';"));
			Return False;
		EndIf;
	...
	If DeliveryParameters.UseEmail Then
		If DeliveryParameters.NotifyOnly
			...
			And Not DeliveryParameters.UseFTPResource Then
			LogRecord(LogParameters, EventLogLevel.Warning,
				NStr("ru = 'Использование уведомлений по электронной почте возможно только совместно с другими способами доставки.';
					|en = 'Email notifications are available only with other delivery methods.';"));
			Return False;
		EndIf;
		   ...
		   And TypeOf(LogParameters.Data) = Type("CatalogRef.ReportMailings") Then
			...
			ElsIf DeliveryParameters.Recipients <> Undefined Then
				For Each Recipient In DeliveryParameters.Recipients Do
					If DeliveryParameters.NotifyOnly Then
						MessageText = StringFunctionsClientServer.SubstituteParametersToString(
						NStr("ru = 'Уведомления отправлены ''%1'' от %2. %3';
							|en = 'Notifications are sent from %2 to %1. %3';"), RecipientPresentation1,
						SenderSRepresentation);
					Else
...
//
Procedure ArchiveAttachments(Attachments, DeliveryParameters, TempFilesDir, LogParameters)
	...
	If DeliveryParameters.ShouldSetPasswordsAndEncrypt And ValueIsFilled(DeliveryParameters.CertificateToEncrypt) Then
		ArchiveNameTooltip = NStr("ru = '(Необходимо расшифровать)';
										|en = '(Decrypt)';");
		If Lower(Right(DeliveryParameters.ArchiveName, 4)) <> ".zip" Then
	...
	If DeliveryParameters.UseEmail Then
		If DeliveryParameters.FillGeneratedReportsInMessageTemplate Then
			DeliveryParameters.RecipientReportsPresentation = 
				...
				+ Chars.LF 
				+ Chars.LF
				+ NStr("ru = 'Файлы отчетов запакованы в архив';
...
Function OutputRight(LogParameters)
	...
	If Not OutputRight Then
		LogRecord(LogParameters, EventLogLevel.Error,
			StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'У пользователя ''%1'' недостаточно прав на вывод информации.';
																		|en = '%1 has insufficient rights to export data.';"),
			Users.CurrentUser()));
	EndIf;
	Return OutputRight;
EndFunction
...
Function MessagesToUserString(Errors = Undefined, SeeEventLog = True) Export
	...
	If AllErrors <> "" And SeeEventLog Then
		AllErrors = AllErrors + Indent + "---" + Indent + NStr("ru = 'Подробности см. в журнале регистрации.';
...
Function TestOfSuccessfulReportDistribution(DeliveryParameters, RecipientRow, RecipientPresentation1, SenderSRepresentation, AdditionalInfo = "")
	...
	If GeneratedReports.Count() = 1 Then 
		MessageText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = '- ""%1"" отправлен от %2. %3';
				|en = '- ""%1"" is sent from %2. %3';"), GeneratedReports[0],
			SenderSRepresentation, AdditionalInfo);	
	Else
		...
		MessageText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = '%1 отправлены от %2. %3';
				|en = '%1 are sent from %2. %3';"), ReportsAsString, SenderSRepresentation,
				AdditionalInfo);

	EndIf;

	If DeliveryParameters.Archive Then
		PackedToArchive = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Упаковано в архив ""%1""';
																						|en = 'Packed in the ""%1"" archive';"), DeliveryParameters.ArchiveName);
...
Procedure SendBulkSMSMessagesWithReportDistributionArchivePasswordsInBackgroundJob(Parameters, ResultAddress) Export
	...
	For Each PrepareSMS In Parameters.PreparedSMSMessages Do
		...
		Else	
			SentCount = SentCount + 1;
			RecipientResult.Comment = NStr("ru = 'SMS отправлено.';
													|en = 'The text message is sent.';");
		EndIf;
	...
	MessageText = StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = 'Отправлены SMS с паролями архивов. Отправлено: %1; не отправлено: %2.';
			|en = 'Text messages with archive passwords are sent. Sent: %1. Not sent: %2.';"), SentCount,
		UnsentCount);
...
Procedure HandleEncryptionError(ResultAddress, CertificateToEncrypt, ErrorInfo, LogParameters)
	
	Refinement = CommonClientServer.ExceptionClarification(ErrorInfo);
	ModuleDigitalSignature = Common.CommonModule("DigitalSignature");
	ClassifierError = ModuleDigitalSignature.ClassifierError(Refinement.Text, True);
	...
	If IsKnownError Then
		...
		If ValueIsFilled(ClassifierError.Cause) Then
			Reasons = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Возможные причины: %1';
																					|en = 'Possible reasons: %1';"), ClassifierError.Cause);
			StringsErrors.Add(Reasons);
		EndIf;
		If ValueIsFilled(ClassifierError.Decision) Then
			Decision = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Рекомендации: %1';
																					|en = 'Recommendations: %1';"), ClassifierError.Decision);
			StringsErrors.Add(Decision);
		EndIf;
		ErrorText = StrConcat(StringsErrors, Chars.LF);
		
		LogRecord(LogParameters, EventLogLevel.Error, ErrorText);
	EndIf;
...
Function DistributionReceiptMethod(DeliveryParameters, Recipient, MailAddr = "")
	...
	If DeliveryParameters.UseNetworkDirectory Then
		...
		MethodOfObtaining = StringFunctionsClientServer.SubstituteParametersToString(NStr(
			"ru = 'Публиковать в сетевой каталог: ""%1"".';
	...
	If DeliveryParameters.UseFTPResource Then
		FTPResource = "ftp://" + DeliveryParameters.Server + ":" + Format(DeliveryParameters.Port, "NZ=0; NG=0")
			+ DeliveryParameters.Directory;
		MethodOfObtaining = StringFunctionsClientServer.SubstituteParametersToString(NStr(
			"ru = 'Публиковать на FTP ресурс: ""%1"".';
			|en = 'Publish to the FTP resource: ""%1"".';"), FTPResource);
	EndIf;

	If DeliveryParameters.UseFolder Then
		MethodOfObtaining = StringFunctionsClientServer.SubstituteParametersToString(NStr(
			"ru = 'Публиковать в папку: ""%1"".';
...
Function ReportDistributionProgressText(DeliveryParameters, SentCount, QuantityToSend)
	...
	If DeliveryParameters.UseNetworkDirectory Then
		Text = NStr("ru = 'Выполняется рассылка отчетов.
			|Отчеты помещены в сетевой каталог %1 из %2.
			|Пожалуйста, подождите...';
			|en = 'Distributing the reports.
...
Procedure ResendByEmail(ReportsTable, DeliveryParameters, BulkEmail, LogParameters)
	...
	LogRecord(LogParameters,
		EventLogLevel.Note,
		NStr("ru = 'Повторная доставка на электронную почту.';
			|en = 'Resend the reports by email.';"));
...
Function RunReportsGenerationInSingleThread(Var_Reports, DeliveryParameters, MailingDescription, LogParameters)
	...
	For Each RowReport In Var_Reports Do
		LogText = NStr("ru = 'Отчет ''%1'' формируется...';
		...
		If DeliveryParameters.Personalized And Not ReportParameters.IsPersonalized Then
			ReportParameters.Errors = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Отчет ''%1'' не может сформирован, так как в его настройках не указан отбор по получателю рассылки.';
					|en = 'Cannot generate report ""%1"". Recipient is required.';"),
				ReportPresentation);
		...
		Try
			...
			MessageText = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Отчет ''%1'' успешно сформирован.';
					|en = 'The ""%1"" report is successfully generated.';"), ReportPresentation);
			
			LogRecord(LogParameters, EventLogLevel.Note, MessageText);
		Except
			MessageText = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Отчет ''%1'' не сформирован:';
					|en = 'Report ""%1"" was not generated:';"), ReportPresentation);
...
Procedure AddToToDoListSetTempFilesDirectory(ToDoList)
	...
	For Each Section In Sections Do
		...
		ToDoItem.Presentation  = NStr("ru = 'Рассылки отчетов могут работать быстрее';
									|en = 'Reports can be sent faster';");
		ToDoItem.Owner       = Section;
		ToDoItem.ToolTip  = NStr("ru = 'Для рассылки отчетов в несколько потоков необходимо указать каталог временных файлов кластера серверов 1С:Предприятие.';
								|en = 'To send reports in multiple threads, specify a directory of temporary files of the 1C:Enterprise server cluster.';");	
...
Function GenerateReportsInMultipleThreads(Var_Reports, DeliveryParameters, MailingDescription, LogParameters)
	...
	ExecutionParameters.BackgroundJobDescription = NStr("ru = 'Рассылка отчетов';
