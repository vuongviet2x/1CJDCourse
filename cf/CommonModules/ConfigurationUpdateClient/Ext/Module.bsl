...
Function UpdatesInstallationSupported() Export
	...
	If CommonClient.DataSeparationEnabled() Then
		Result.ErrorDescription = 
			NStr("ru = 'Установка обновлений для приложения в Интернете выполняется централизованно через Менеджер сервиса.';
				|en = 'Cloud applications support only centralized updates initiated from the service manager.';");
		Return Result;
	EndIf;
	
	If Not UsersClient.IsFullUser(True) Then
		Result.ErrorDescription = NStr("ru = 'Для установки обновления требуются права администрирования.';
										|en = 'You need the administrator rights to install the update.';");
		Return Result;
	EndIf;
...
#If WebClient Then
	...
	Result.ErrorDescription =
		NStr("ru = 'В веб-клиенте доступна только установка исправлений.
			|Для установки обновления, установите
			|полный дистрибутив технологической платформы 1С:Предприятие для Windows.';
			|en = 'Only patch installation is available in the web client.
			|To install the update, install
			|the distribution package of 1C:Enterprise for Windows.';");
	
#ElsIf MobileClient Then
	Result.ErrorDescription = NStr("ru = 'Установка обновления доступна только в ОС Windows.';
									|en = 'The update is available only on Windows.';");
#Else
	...
	If CommonClient.ClientConnectedOverWebServer() Then 
		...
		Result.ErrorDescription = Result.ErrorDescription
			+ NStr("ru = 'При подключении через веб-сервер доступна только установка исправлений.
				|Для установки обновления, установите
				|полный дистрибутив технологической платформы 1С:Предприятие для Windows.';
				|en = 'Only patch installation is available from the web server.
				|To install the update, install
				|the distribution package of 1C:Enterprise for Windows.';");
	EndIf;
...
Function BackupCreationTitle(Parameters) Export
	...
	If Parameters.CreateDataBackup = 0 Then
		Return NStr("ru = 'Не создавать резервную копию ИБ';
					|en = 'Do not back up the infobase';");
	ElsIf Parameters.CreateDataBackup = 1 Then
		If Parameters.RestoreInfobase Then 
			Return NStr("ru = 'Создавать временную резервную копию ИБ и выполнять откат при нештатной ситуации';
						|en = 'Create a temporary infobase backup and roll back if any issues occur';");
		Else 
			Return NStr("ru = 'Создавать временную резервную копию ИБ и не выполнять откат при нештатной ситуации';
						|en = 'Create a temporary infobase backup and do not roll back if any issues occur';");
		EndIf;
	ElsIf Parameters.CreateDataBackup = 2 Then
		If Parameters.RestoreInfobase Then 
			Return NStr("ru = 'Создавать резервную копию ИБ и выполнять откат при нештатной ситуации';
						|en = 'Create an infobase backup and roll back if any issues occur';");
		Else 
			Return NStr("ru = 'Создавать резервную копию ИБ и не выполнять откат при нештатной ситуации';
						|en = 'Create an infobase backup and do not roll back if any issues occur';");
		EndIf;
	Else
		Return "";
	EndIf;
	...
	If IsBlankString(ScriptDirectory) Then
		EventLogClient.AddMessageForEventLog(EventLogEvent(),
			"Warning", StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Журнал обновления см. в папке временных файлов %1.(цифры).';
					|en = 'See the update log in the temporary file folder: %1.(digits).';"), "%temp%\1Cv8Update"),, True);
		UpdateResult = True; // Consider the update being successful.
	Else 
		EventLogClient.AddMessageForEventLog(EventLogEvent(),
			"Information", 
			StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Папка с журналом обновления: %1';
					|en = 'Update log folder: %1';"), ScriptDirectory),, True);
...
Procedure BeforeExit(Cancel, Warnings) Export
	...
	If ApplicationParameters["StandardSubsystems.SuggestInfobaseUpdateOnExitSession"] = True Then
		...
		WarningParameters.CheckBoxText  = NStr("ru = 'Установить обновление конфигурации';
													|en = 'Install configuration update';");
		WarningParameters.WarningText  = NStr("ru = 'Запланирована установка обновления.';
															|en = 'Update installation is scheduled.';");
...
Function UpdateInstallationPossible(Parameters, AdministrationParameters)
	...
	If IsFileInfobase And Parameters.CreateDataBackup = 2 Then
		...
		If Not File.Exists() Or Not File.IsDirectory() Then // ACC:566 Synchronous calls outside of the web client are allowed.
			ShowMessageBox(,
				NStr("ru = 'Укажите существующую папку для сохранения резервной копии ИБ.';
					|en = 'Please specify an existing folder for storing the infobase backup.';"));
			Return False;
		EndIf;
	EndIf;
	...
	If Parameters.UpdateMode = 0 Then // Update now.
		...
		If IsFileInfobase
			...
			ShowMessageBox(,
				NStr("ru = 'Невозможно продолжить обновление конфигурации,
				           |так как не завершены все соединения с информационной базой.';
							|en = 'Cannot proceed with configuration update
	...
	ElsIf Parameters.UpdateMode = 2 Then
		...
		If InvalidEmailSpecified Then
			ShowMessageBox(,
				NStr("ru = 'Укажите допустимый адрес электронной почты.';
					|en = 'Please specify a valid email address.';"));
			Return False;
		EndIf;
		
		If Not JobSchedulerSupported() Then
			ShowMessageBox(,
				NStr("ru = 'Планировщик заданий поддерживается только начиная с операционной системы версии 6.0 Vista.';
					|en = 'Job scheduler supports Windows Vista 6.0 or later.';"));
			Return False;
		EndIf;
	EndIf;
...
Function UpdateDateCorrect(Parameters)
	...
	If Parameters.UpdateDateTime < CurrentDate Then
		MessageText = NStr("ru = 'Обновление конфигурации может быть запланировано только на будущую дату и время.';
								|en = 'A configuration update can be scheduled only for a future date and time.';");
	ElsIf Parameters.UpdateDateTime > AddMonth(CurrentDate, 1) Then
		MessageText = NStr("ru = 'Обновление конфигурации может быть запланировано не позднее, чем через месяц относительно текущей даты.';
								|en = 'A configuration update cannot be scheduled to a date later than one month from the current date.';");
	EndIf;
...
Function GetUpdateAdministratorAuthenticationParameters(AdministrationParameters)
	...
	If StrFind(ConnectionString, "'") > 0 Then
		...
		If FileInfobase1 Then
			...
			ErrorText = NStr("ru = 'Невозможно выполнить обновление на новую версию приложения, т.к. каталог
				|информационной базы содержит недопустимый символ одинарной кавычки "" '' "":
				|%1%2';
				|en = 'Cannot update the application as the infobase
				|directory contains an incorrect single quote character "" '' "":
				|%1%2';");
			Postfix = NStr("ru = 'Переместите информационную базу в другой каталог и повторите попытку обновления.';
							|en = 'Move the infobase to another directory and retry the update.';");
		Else
			PathToTheDatabase = ConnectionString;
			ErrorText = NStr("ru = 'Невозможно выполнить обновление на новую версию приложения, т.к. в адресе
				|сервера или имени информационной базы содержится недопустимый символ одинарной кавычки "" '' "":
				|%1%2';
				|en = 'Cannot update the application as the server address
				|or the infobase name contains an incorrect single quote character "" '' "":
				|%1%2';");
			Postfix = NStr("ru = 'Переместите информационную базу на другой сервер или переименуйте ее, затем повторите попытку обновления.';
							|en = 'Move the infobase to another server or rename it and then retry the update.';");
		EndIf;
...
Function ScheduleServiceTaskName(Val TaskCode)
	...
	Return StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Обновление конфигурации (%1)';
																		|en = 'Update configuration (%1)';"), Format(TaskCode, "NG=0"));
...
Function EventLogEvent() Export
	Return NStr("ru = 'Обновление конфигурации';
				|en = 'Configuration update';", CommonClient.DefaultLanguageCode());
EndFunction
...
#If Not WebClient And Not MobileClient Then
	...
	If SettingsOfUpdate.ConfigurationChanged Then
		ShowUserNotification(NStr("ru = 'Обновление конфигурации';
											|en = 'Configuration update';"),
			"e1cib/app/DataProcessor.InstallUpdates",
			NStr("ru = 'Конфигурация отличается от основной конфигурации информационной базы.';
				|en = 'The configuration is different from the main infobase configuration.';"), 
...
Function GenerateUpdateScriptFiles(Val InteractiveMode, Parameters, AdministrationParameters)
	...
	If IsDeferredUpdate Then 
		...
		TaskDetails1 = NStr("ru = 'Обновление конфигурации 1С:Предприятие';
								|en = 'Update 1C:Enterprise configuration';");
...
Procedure RunUpdateScriptAfterObsoleteDataPurge(Result, Context) Export
	...
	If Result <> True Then
		If Result <> False Then
			ShowMessageBox(, NStr("ru = 'Обновление конфигурации отменено.';
											|en = 'Configuration update is canceled.';"));
		EndIf;
		Return;
	EndIf;
...
Procedure RunUpdateScriptCompletion(Parameters, AdministrationParameters)
	...
	EventLogClient.AddMessageForEventLog(EventLogEvent(), "Information",
		NStr("ru = 'Выполняется процедура обновления конфигурации:';
			|en = 'Updating the configuration:';") + " " + MainScriptFileName);
