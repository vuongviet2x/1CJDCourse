		...
		While ConnectionAttempt <= 2 Do
			...
			Except
				ExceptionText = StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Не удалось подключиться с компьютера %1 к кластеру серверов по причине:
						|%2.';
						|en = 'Cannot connect to the server cluster from computer %1. Reason:
						|%2.';"),
				...
				ExceptionText = ExceptionText + Chars.LF + Chars.LF + StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Если в сообщении указано, что ""класс не зарегистрирован"" или не совпадает версия компоненты ""comcntr"" с требуемой,
						...
						|en = 'If you get the ""Class not registered"" or comcntr version mismatch error,
...
Procedure DeleteInfobaseSessions(Val ClusterAdministrationParameters, Val IBAdministrationParameters, Val Filter = Undefined) Export
	...
	If Not AllSessionsTerminated Then
		...
		Raise NStr("ru = 'Не удалось удалить сеансы.';
								|en = 'Cannot delete sessions.';");
...
Procedure TerminateInfobaseConnections(Val ClusterAdministrationParameters, Val IBAdministrationParameters, 
	...
	If Not AllConnectionsTerminated Then
		...
		Raise NStr("ru = 'Не удалось разорвать соединения.';
								|en = 'Cannot close connections.';");
...
Function COMConnector()
	...
	If SafeMode() <> False Then
		Raise NStr("ru = 'Администрирование кластера невозможно в безопасном режиме';
								|en = 'Safe mode does not support cluster administration.';");
	EndIf;
	
	If Common.DataSeparationEnabled() Then
		Raise NStr("ru = 'В модели сервиса недопустимо выполнение прикладной информационной базой функций администрирования кластера';
								|en = 'SaaS mode does not support cluster administration.';");
	EndIf;
...
Function RegisterCOMConnector(Val Message = "")
	...
	Comment = StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = '%1Выполнена перерегистрация компоненты ""comcntr"" на компьютере %2.
			|Команда: %3
			|Код возврата %4, сообщение:
			|%5';
			|en = '%1The comcntr component is reregistered on computer %2.
			|Command: %3
			|Return code:%4, message:
			|%5';"), 
		Message, ComputerName(), CommandText, RunResult.ReturnCode, RunResult.OutputStream);
	WriteLogEvent(NStr("ru = 'Подключение к кластеру серверов';
									|en = 'Connect to server cluster';", Common.DefaultLanguageCode()),
...
Function GetCluster(IServerAgentConnection, Val ClusterPort, Val ClusterAdministratorName, Val ClusterAdministratorPassword)
	...
	Raise StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'На рабочем сервере %1 не существует кластер %2';
																					|en = 'Cluster %2 does not exist on production server %1';"),
...
Function WorkingProcessesConnections(COMConnector, IServerAgentConnection, Cluster)
	...
	If Result.Count() = 0 Then
		Raise StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'В кластере серверов %1:%2 не существует активных рабочих процессов.';
																						|en = 'There are no active working processes on server cluster %1:%2.';"),
			Cluster.HostName,
...
Function GetIBDetails(IServerAgentConnection, Cluster, Val NameInCluster)
	...
	Raise StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'В кластере серверов %1:%2 не существует информационная база ""%3""';
																					|en = 'Infobase ""%3"" does not exist on server cluster %1:%2';"),
		Cluster.HostName,
		Format(Cluster.MainPort, "NG=0"),
		NameInCluster);
...
Function GetIB(WorkingProcessesConnections, Cluster, Val NameInCluster, Val IBAdministratorName, Val IBAdministratorPassword)
	...
	If InfobaseFound Then	
		Raise StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Неправильные имя и пароль администратора информационной базы %1 в кластере серверов %2:%3 (имя: ""%4"").';
				|en = 'Incorrect administrator name or password for infobase %1, server cluster %2:%3 (name: ""%4"").';"),
			NameInCluster, Cluster.HostName, Cluster.MainPort, IBAdministratorName);
	Else		
		Raise StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'В кластере серверов %1:%2 не существует информационная база ""%3""';
				|en = 'Infobase ""%3"" does not exist on server cluster %1:%2';"),
...
Function GetSecurityProfile(IServerAgentConnection, Cluster, ProfileName)
	...
	Raise StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'В кластере серверов %1:%2 не существует профиль безопасности ""%3""';
																					|en = 'Security profile ""%3"" does not exist on server cluster %1:%2';"),
		Cluster.HostName,
		Format(Cluster.MainPort, "NG=0"),
		ProfileName);
