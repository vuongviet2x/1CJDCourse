	...
	For Each Action In RemedyActions Do
		...
		If Action = "SetListOfCertificateRevocation" And Not ErrorAtServer And ThereIsReceiptOfFilesFromInternet Then
			Decision = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = '• <a href=%1>Установите список отзыва</a> удостоверяющего центра автоматически.
				|%2';
				|en = '• <a href=%1>Install a revocation list</a> of a certificate authority automatically.
				|%2';"), Action, Decision);
		ElsIf Action = "InstallRootCertificate" And Not ErrorAtServer Then
			Decision = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = '• <a href=%1>Установите корневой сертификат</a> удостоверяющего центра автоматически.
				|%2';
				|en = '- <a href=%1>Install a root certificate</a> of a certificate authority automatically.
				|%2';"), Action, Decision);
		ElsIf Action = "InstallCertificate" And Not ErrorAtServer Then
			Decision = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = '• <a href=%1>Установите сертификат</a> в Личное хранилище сертификатов пользователя операционной системы автоматически.
				|%2';
				|en = '- <a href=%1>Install the certificate</a> in the Current user/Personal certificate store automatically.
				|%2';"), Action, Decision);
		ElsIf Action = "InstallCertificateIntoContainer" And Not ErrorAtServer Then
			Decision = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = '• <a href=%1>Установите сертификат в контейнер</a> прямо из приложения.
				|%2';
				|en = '• <a href=%1>Install certificate on container</a> from the app.
				|%2';"), Action, Decision);
		EndIf;
