...
Function RunFilesRecovery(Volume, FormUniqueID) Export
	...
	ExecutionParameters.BackgroundJobDescription = NStr("ru = '""Восстановление файлов""';
															|en = '""File recovery""';");
...
Function CreateFile(Val Owner, Val FileInfo1)
	...
	File.Description = CommonClientServer.ReplaceProhibitedCharsInFileName(FileInfo1.BaseName);
	...
	File.Extension = CommonClientServer.ReplaceProhibitedCharsInFileName(FileInfo1.ExtensionWithoutPoint);
	File.Size = FileInfo1.Size;
	File.UniversalModificationDate = FileInfo1.ModificationTimeUniversal;
	If Not ValueIsFilled(File.UniversalModificationDate)
		Or File.UniversalModificationDate > CurrentUniversalDate() Then
		File.UniversalModificationDate = CurrentUniversalDate();
	EndIf;
...
Function SaveChangesAndUnlockFile(FileData, FileInfo1, DontChangeRecordInWorkingDirectory,
	...
	If Not FileDataCurrent.CurrentUserEditsFile And Not FileToSynchronizeByCloudService(
		FileData.Ref) Then
		Raise NStr("ru = 'Файл не занят текущим пользователем';
								|en = 'The file is not locked by the current user.';");
	EndIf;
...
Function FilesDeletionResult(FilesOrVersions, UUID) Export
	...
	For Each FileOrVersion In FilesOrVersions Do
		...
		If Common.ObjectAttributeValue(FileOrVersion, "Author") = AuthorizedUser Then
			...
			EndIf;
		Else
			Result.WarningText = ?(IsFileVersion, NStr("ru = 'Только автор может удалить версию файла.';
																	|en = 'Only the author can delete the file version.';"),
				NStr("ru = 'Только автор может удалить файл.';
					|en = 'Only the author can delete the file.';"));
		EndIf;
...
Function TechnicalInformation() Export
	...
	Result = New Structure;
	ScanLogEvent = NStr("ru = 'Сканирование изображений';
										|en = 'Scan images';", Common.DefaultLanguageCode());
