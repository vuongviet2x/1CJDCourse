...
Function AddInInformation(Id, Version = Undefined) Export
	...
	If Information.State = "NotFound1" Then
		Result.ErrorDescription = NStr("ru = 'Внешняя компонента не существует';
										|en = 'The add-in does not exist';");
		Return Result;
	EndIf;
	
	If Information.State = "DisabledByAdministrator" Then
		Result.ErrorDescription = NStr("ru = 'Внешняя компонента отключена';
										|en = 'Add-in is disabled';");
		Return Result;
	EndIf;
