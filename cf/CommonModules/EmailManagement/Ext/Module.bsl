...
Procedure SendReceiveEmails() Export
	...
	WriteLogEvent(EventLogEvent(), 
		...
		NStr("ru = 'Начато регламентное получение и отправка электронной почты';
			|en = 'Mail synchronization started';", Common.DefaultLanguageCode()));
	...
	WriteLogEvent(EventLogEvent(), 
		EventLogLevel.Information, , ,
		NStr("ru = 'Закончено регламентное получение и отправка электронной почты';
			|en = 'Mail synchronization completed';", Common.DefaultLanguageCode()));
	...
	While AccountsSelection.Next() Do
		...
		While EmailSelection.Next() Do
			...
			Except
				...
				ErrorMessageTemplate = NStr("ru = 'Письмо %1 не подготовлено к отправке по причине:
					|%2';
					|en = 'The %1 email is not prepared for sending due to:
		...
		Except
			...
			ErrorMessageTemplate = NStr("ru = 'Не удалось подключиться к почте %1 по причине:
				|%2';
				|en = 'Cannot connect to the %1 account due to:
				|%2';", Common.DefaultLanguageCode());
			...
			For Each EmailToSend In Emails Do
				...
				If SendingResult[EmailToSend] = Undefined Then
					...
					If DataFromFirstUnsentEmail <> Undefined Then
						...
						ErrorProcessingParameters.IncrementAttemptsCount = True;
						ErrorProcessingParameters.InformUser              = Interactively;
						ErrorProcessingParameters.ErrorText                       = NStr("ru = 'Непредвиденная ошибка при отправке письма';
																							|en = 'Unknown error when sending the email message';");
...
Function ProcessEmailSendingError(ErrorProcessingParameters, Val WrongRecipients) Export
	...
	If IsIssueOfEmailAddressesServerRejection Then
		...
		If Not AllEmailAddresseesRejectedByServer Then
			ErrorMessageTemplate = NStr("ru = 'Следующие адресаты письма ""%1"" не приняты почтовым сервером:
				...
				|en = 'Some recipients of the message ""%1"" were rejected by the server:
				|%2. The message was sent to other recipients.';", Common.DefaultLanguageCode());
		Else
			ErrorMessageTemplate = NStr("ru = 'Не удалось отправить письмо ""%1"".
				|Следующие адресаты письма не приняты почтовым сервером:
				|%2.';
				|en = 'Cannot send message ""%1"".
	...
	Else
		...
		ErrorMessageTemplate = NStr("ru = 'Не удалось отправить письмо ""%1"".
			|По причине: %2.';
			|en = 'Cannot send the email message ""%1"".
...
Procedure SendNotoficationsOnReading(ForCurrentUser)
	...
	While AccountsSelection.Next() Do
		...
		While EmailSelection.Next() Do
			...
			EmailParameters.Insert("Subject",NStr("ru = 'Уведомление о прочтении';
												|en = 'Read receipt';") + " / " +"Reading Confirmation");
...
Procedure LoadUserEmail(Result)
	...
	TimeConsumingOperations.ReportProgress(, NStr("ru = 'Получение почты';
												|en = 'Receive mail';"));
	...
	If Result.UserAccountsAvailable = 0 Then
		Common.MessageToUser(NStr("ru = 'Нет доступных учетных записей для получения почты.';
													|en = 'No available email accounts to receive mail.';"));
...
Procedure SendUserEmail(Result)
	...
	TimeConsumingOperations.ReportProgress(, NStr("ru = 'Отправка почты';
												|en = 'Send mail';"));
...
Procedure GetEmails(Val AccountData, HasErrors, ReceivedEmails, EmailsReceived)
	...
	Except
		...
		ErrorMessageText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Не удалось подключиться к почте %1 по причине:
				|%2';
				|en = 'Cannot connect to the %1 account due to:
				|%2';", Common.DefaultLanguageCode()),
				AccountData.Ref,
		...
		While MessagesToImportIDs.Count() > (EmailsReceived1 + ErrorsCountOnWrite + ObsoleteMessagesCount) Do
			...
			For Each Message In Messages Do
				...
				Except
					...
					RollbackTransaction();
					ErrorMessageText = StringFunctionsClientServer.SubstituteParametersToString(
						NStr("ru = 'Не удалось получить письмо %1 от %2, отправленное c адреса %3, по причине:
						|%4';
	...
	For Each ActiveFolderName In ActiveFoldersNames Do
		...
		Except
			MessageText = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Не удалось получить заголовки писем для папки ""%1"" учетной записи ""%2"", по причине: %3';
					|en = 'Couldn''t get the email headers. Folder: %1. Account: %2. Reason: %3';"),
		...
		MessageText = StrTemplate(NStr("ru = 'Начало получения писем из папки ""%1"" учетной записи ""%2"". Писем к загрузке: %3';
										|en = 'Receiving emails from folder ""%1"" for %2. Pending messages: %3';"),
		...
		If IDsTable.Count() > 0 Or TitlesWithEmptyIDs.Count() > 0 Then
			...
			MessageText = StrTemplate(NStr("ru = 'Получено писем из папки ""%1"" учетной записи ""%2"": %3';
											|en = 'Messages received from folder ""%1"" for %2: %3';"),
...
Procedure SynchronizeReviewedFlagWithServer(Mail, AccountData, ImportedEmailsArray)
	...
	Except
		...
		RollbackTransaction();
		MessageText = NStr("ru = 'Не удалось изменить информацию о признаке рассмотрено для писем по причине : %Cause%.';
								|en = 'Couldn''t change the message status due to: %Cause%.';");
...
Procedure DeleteIDsOfPreviouslyReceivedEmails(Account, IDsAtServer, IDsDelete)
	...
	Except
		...
		RollbackTransaction();
		MessageText = NStr("ru = 'Не удалось очистить информацию о ранее полученных идентификаторах по причине: %Cause%.';
								|en = 'Could not clean up ID data due to: %Cause%.';");
		MessageText = StrReplace(MessageText, "%Cause%", ErrorProcessing.DetailErrorDescription(ErrorInfo()));
		WriteLogEvent(EventLogEvent(),
			EventLogLevel.Error,
...
Function ImportedEmailSubjectAndFolder(MailMessage, Account, IsOutgoingEmail1, PutEmailInBaseEmailFolder)
	...
	If QueryExecutionTime > 1 Then
		...
		LogRecordText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Запрос по получению данных писем основания загружаемого письма выполнялся дольше %1 сек, это дольше ожидаемого.';
				|en = 'The request to retrieve parent email data for the loaded email took more than %1 seconds, which is longer than expected.';"),
			QueryExecutionTime);
		WriteLogEvent(EventLogEvent(), EventLogLevel.Information,
...
Procedure DeterminePreviouslyImportedSubordinateEmails(Account, EmailsReceived);
	...
	While Selection.Next() Do
		...
		Except
			RollbackTransaction();
			MessageText = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Не установить письмо основание у %1 по причине %2';
					|en = 'Could not identify the original message for %1 due to: %2';"), 
...
Procedure WriteEmailAttachment(Object, Attachment, AttachmentSignatures, CountOfBlankNamesInAttachments)
	...
	If HasSignatures Then
		...
		For Each AttachmentsSignature In AttachmentSignatures Do
			...
			Except
				EventText = NStr("ru = 'Не удалось прочитать данные подписи вложения %1: %2';
									|en = 'Cannot read the %1 attachment signature data: %2';");
			...
			SignatureData.Signature = AttachmentSignatureData;
			ResultOfReadSignatureProperties = ModuleDigitalSignature.SignatureProperties(AttachmentSignatureData);
			...
			SignatureData.Comment = NStr("ru = 'Вложение электронного письма';
											|en = 'Email attachment';");
...
Function WriteEmailAttachmentFromTempStorage(MailMessage, AddressInTempStorage,
	...
	If IsBlankString(BaseName) Then
		...
		BaseName = NStr("ru = 'Вложение без имени';
								|en = 'Untitled attachment';") 
...
Function EventLogEvent() Export
	...
	Return NStr("ru = 'Взаимодействия';
				|en = 'Business interactions';", Common.DefaultLanguageCode());
