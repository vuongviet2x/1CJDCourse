...
Function ParametersOfSearchForSimilarStrings(AttachAddInSSL = True) Export
	...
	If AttachAddInSSL Then
		...
		If FuzzySearch1 = Undefined Then
			Raise NStr("ru = 'Не удалось подключить компоненту нечеткого поиска. Подробнее см. в журнале регистрации.';
									|en = 'Cannot attach the fuzzy search add-in. See the Event log for details.';");
		EndIf;
	EndIf;
...
Procedure OnDefineCommandsAttachedToObject(FormSettings, Sources, AttachedReportsAndDataProcessors, Commands) Export
	...
	For Each AttachedObject In AttachedObjects Do
		...
		If AccessRight("Update", AttachedObject)
			...
			Command.Presentation = NStr("ru = 'Объединить выделенные...';
										|en = 'Merge selected items…';");
			...
			Command.Kind = "Administration";
			Command.Importance = "SeeAlso";
			Command.Presentation = NStr("ru = 'Заменить выделенные...';
										|en = 'Replace selected items…';");
			Command.WriteMode = "NotWrite";
			Command.VisibilityInForms = "ListForm";
			Command.MultipleChoice = True;
...
Function SubsystemDescription(ForUser) Export
	...
	Return NStr("ru = 'Поиск и удаление дублей';
				|en = 'Duplicate cleaner';", LanguageCode);
EndFunction
	...
	For Each AttributeName In KeyAttributes Do
		...
		Else 
			ErrorDescription = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Не существует поле связи %1 в объекте метаданных %2';
					|en = 'Link field %1 does not exist in metadata object %2';"), 
