...
Procedure OnAddUpdateHandlers(Handlers) Export
	...
	If StandardSubsystemsServer.IsBaseConfigurationVersion() And Not SeparationByDataAreasEnabled() Then
		...
		Handler.Comment     = NStr("ru = 'Включает отправку сведений об использовании приложения в фирму ""1С"". Отключить отправку сведений можно в разделе Администрирование / Интернет-поддержка и сервисы / Центр мониторинга';
											|en = 'Enables sending usage data to 1C company. You can disable this option in Administration > Online support and services > Monitoring center.';");
			...
			If ActiveBackgroundJobs.Count() = 0 Then                                                                      
				...
				BackgroundJobs.Execute("MonitoringCenterInternal.SendTestPackage",
					...
					NStr("ru = 'Центр мониторинга: отправка тестового пакета';
						|en = 'Monitoring center: send test package';"));
			EndIf;
		EndIf;
	EndIf;
			...
			If ActiveBackgroundJobs.Count() = 0 Then
				...
				BackgroundJobs.Execute("MonitoringCenterInternal.CollectAndSendDumps",
					...
					NStr("ru = 'Сбор и отправка отчетов об ошибках';
						|en = 'Collect and send error reports';"));
				EndIf;
		Else
...
Procedure OnFillToDoList(ToDoList) Export
	...
	For Each Section In Sections Do
		...
		ToDoItem.Presentation  = NStr("ru = 'Предоставить отчеты об ошибках';
									|en = 'Provide error reports';");
		ToDoItem.Count     = 0;
		ToDoItem.ToolTip      = NStr("ru = 'Зарегистрированы аварийные завершения работы приложения. Пожалуйста, расскажите нам об этой проблеме.';
									|en = 'Abnormal terminations of the app were detected. Please contact us on this issue.';");
	...
	For Each Section In Sections Do
		...
		ToDoItem.ToolTip      = NStr("ru = 'Отчеты об аварийном завершении собраны и подготовлены. Пожалуйста, согласуйте их отправку.';
									|en = 'Crash reports are collected and prepared. Please approve reports submission.';");
...
Procedure MonitoringCenterScheduledJob() Export
	...
	If (MonitoringCenterParameters.EnableMonitoringCenter Or MonitoringCenterParameters.ApplicationInformationProcessingCenter) And IsMasterNode1() Then 
		If MonitoringCenterParameters.RegisterDumps And StartDate2 >= MonitoringCenterParameters.DumpRegistrationNextCreation Then
			...
			Except
				...
				WriteLogEvent(NStr("ru = 'Центр мониторинга.Регистрация дампов';
												|en = 'Monitoring center.Register dumps';", 
		...
		If MonitoringCenterParameters.RegisterBusinessStatistics And StartDate2 >= MonitoringCenterParameters.BusinessStatisticsNextSnapshot Then
			...
			Except
				Comment = ErrorProcessing.DetailErrorDescription(ErrorInfo());
				WriteLogEvent(NStr("ru = 'Центр мониторинга.Регистрация операций статистики';
												|en = 'Monitoring center.Register statistics operations';",
					Common.DefaultLanguageCode()), EventLogLevel.Error,,, Comment);
		...
		If (MonitoringCenterParameters.RegisterConfigurationStatistics Or MonitoringCenterParameters.RegisterConfigurationSettings) And StartDate2 >= MonitoringCenterParameters.ConfigurationStatisticsNextGeneration Then
			...
			Except
				Comment = ErrorProcessing.DetailErrorDescription(ErrorInfo());
				WriteLogEvent(NStr("ru = 'Центр мониторинга.Собрать статистику конфигурации';
												|en = 'Monitoring center.Collect configuration statistics';",
					Common.DefaultLanguageCode()), EventLogLevel.Error,,, Comment);
		...
		If StartDate2 >= MonitoringCenterParameters.SendDataNextGeneration Then
			...
			Except
				Comment = ErrorProcessing.DetailErrorDescription(ErrorInfo());
				WriteLogEvent(NStr("ru = 'Центр мониторинга.Сформировать пакет для отправки';
												|en = 'Monitoring center.Generate a package for sending';",
					Common.DefaultLanguageCode()), EventLogLevel.Error,,, Comment);
			...
			Except
				Comment = ErrorProcessing.DetailErrorDescription(ErrorInfo());
				WriteLogEvent(NStr("ru = 'Центр мониторинга.Отправить данные мониторинга';
												|en = 'Monitoring center.Send monitoring data';",
					Common.DefaultLanguageCode()), EventLogLevel.Error,,, Comment);
...
Procedure AggregateStatisticsOperationsMeasurements(CurrentDate)
	...
	If ProcessRecordsUntil > AggregationBoundary Then
		...
		Except
			RollbackTransaction();
			WriteLogEvent(NStr("ru = 'Центр мониторинга.Агрегировать замеры операций статистики';
											|en = 'Monitoring center.Aggregate measurements of statistics operations ';", 
...
Procedure DeleteObsoleteStatisticsOperationsData()
	...
	Except
		RollbackTransaction();
		WriteLogEvent(NStr("ru = 'Центр мониторинга.Удалить устаревшие данные операций статистики';
										|en = 'Monitoring center.Delete obsolete data of statistics operations ';", 
			Common.DefaultLanguageCode()), EventLogLevel.Error,,, 
			ErrorProcessing.DetailErrorDescription(ErrorInfo()));
		Raise;
	EndTry;
				...
				If InformationRegisters.StatisticsAreas.ShouldCollectStatistics(DataAreaRow) Then
					...
					Except
						WriteLogEvent(NStr("ru = 'Центр мониторинга.Статистика конфигурации переопределяемая';
														|en = 'Monitoring center.Configuration statistics overridable ';", 
							Common.DefaultLanguageCode()), EventLogLevel.Error,,,
							NStr("ru = 'Не удалось установить разделение сеанса. Область данных';
								|en = 'Couldn''t set session separation. Data area';", Common.DefaultLanguageCode()) 
...
Function SetSendingParameters(Parameters)
	...
	Except
		...
		WriteLogEvent(NStr("ru = 'Центр мониторинга.Установка параметров отправки';
										|en = 'Monitoring center.Set sending parameters';", 
			Common.DefaultLanguageCode()), EventLogLevel.Error,,, 
			ErrorProcessing.DetailErrorDescription(ErrorInfo()));
		Raise;
	EndTry;
...
Procedure SetMonitoringCenterParameters(NewParameters)
	...
	Except
		...
		WriteLogEvent(NStr("ru = 'Центр мониторинга.Установка параметров центра мониторинга';
										|en = 'Monitoring center.Set Monitoring center parameters';", 
			Common.DefaultLanguageCode()), EventLogLevel.Error, 
...
Procedure DeleteMonitoringCenterParameters()
	...
	Except
		WriteLogEvent(NStr("ru = 'Центр мониторинга.Удалить параметры центра мониторинга';
										|en = 'Monitoring center.Delete Monitoring center parameters';", 
			Common.DefaultLanguageCode()), EventLogLevel.Error, 
			Metadata.Constants.MonitoringCenterParameters,, ErrorProcessing.DetailErrorDescription(ErrorInfo()));
		Raise;
	EndTry;
	...
	If File.Exists() Then
		...
		Except
			Message = NStr("ru = 'Ошибка чтения файла настроек технологического журнала';
							|en = 'An error occurred when reading a setting file of the technological log';");
			...
			Message = Message + NStr("ru = 'Скорее всего файл поврежден. Регистрация дампов не возможна. Удалите поврежденный файл или восстановите настройки.';
										|en = 'The file is likely corrupt. Cannot register dumps. Delete the corrupt file or reset settings.';");
			WriteLogEvent(NStr("ru = 'Центр мониторинга';
											|en = 'Monitoring center';", Common.DefaultLanguageCode()), 
		...
		If DumpsParameters <> Undefined Then
			If Not DumpsParameters.Property("location") Or Not DumpsParameters.Property("create") Or Not DumpsParameters.Property("type") Then
				Message = NStr("ru = 'Ошибка секции сбора дампов в файле настроек технологического журнала';
								|en = 'Dump collection section error in the setting file of technological log';");
				Message = Message + " """ + File.FullName + """." + Chars.LF;
				Message = Message + NStr("ru = 'Регистрация дампов не возможна. Удалите файл или восстановите настройки.';
											|en = 'Cannot register dumps. Remove the file or restore the settings.';");
				XMLReader.Close();
				WriteLogEvent(NStr("ru = 'Центр мониторинга';
												|en = 'Monitoring center';", Common.DefaultLanguageCode()), 
					EventLogLevel.Warning,,, Message);
	...
	Else
		...
		If DumpsDirectory.Path = Undefined Then
			DumpsDirectory.ErrorDescription = NStr("ru = 'Ошибка создания файла настроек технологического журнала. Регистрация дампов не возможна.';
												|en = 'An error occurred while creating the setting file of the technological log. Cannot register dumps.';");
		EndIf;
	EndIf;
		...
		If Upper(FirstChild.NodeName) = "CONFIG" Then
			...
			Except
				Message = NStr("ru = 'Ошибка записи файла настроек технологического журнала. Регистрация дампов не возможна.';
								|en = 'An error occurred while saving the setting file of the technological log. Cannot register dumps.';");
				Message = Message + " """ +File.FullName + """." + Chars.LF;
	...
	While SearchForDirectory = True Do
		...
		If SettingsDirectories.Find(DirectoryPath) <> Undefined Then
			...
			SettingsDirectory.ErrorDescription = NStr("ru = 'Обнаружена циклическая ссылка';
													|en = 'Circular ref is found';", Common.DefaultLanguageCode());
			...
			Else
				...
				Else
					...
					SettingsDirectory.ErrorDescription = NStr("ru = 'Файл конфигурации настроек в каталоге не существует';
															|en = 'The setting configuration file does not exist in the directory.';", 
...
Function GetDirectoryFromSettingsConfigurationFile(SettingsConfigurationFile)
	...
	Else
		...
		SettingsDirectory.ErrorDescription = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Не найдена секция %1 в файле %2';
				|en = 'Section %1 is not found in file %2';", Common.DefaultLanguageCode()),
...
Function EventLogEventMonitoringCenterDumpDeletion()
	Return NStr("ru = 'Центр мониторинга.Удаление дампа';
				|en = 'Monitoring center.Removing the dump';", Common.DefaultLanguageCode());
EndFunction

Function LogLogEventParseOperationBufferStatistics()
	Return NStr("ru = 'Центр мониторинга.Разобрать буфер операций статистики';
				|en = 'Monitoring center.Parse the buffer of statistics operations';", Common.DefaultLanguageCode());
EndFunction
#EndRegion
...
Procedure CollectAndSendDumps(FromClientAtServer = False, JobID = "") Export
	...
	If DumpsCollectionAndSendingParameters.SendDumpsFiles = 0 Then
		...
		SetMonitoringCenterParameter("SendingResult", NStr("ru = 'Пользователь отказал в предоставлении дампов.';
																		|en = 'User refused to submit dumps.';"));
		SetPrivilegedMode(False);
		StopFullDumpsCollection();
		Return;
	EndIf;
	...
	If SeparatorPosition = 0 Then
		...
		SetMonitoringCenterParameter("SendingResult", NStr("ru = 'Не удалось определить букву диска';
																		|en = 'Cannot determine the drive letter';"));
		SetPrivilegedMode(False);
		StopFullDumpsCollection();
		Return;
	EndIf;
	...
	If DumpsCollectionAndSendingParameters.FullDumpsCollectionEnabled[ComputerName] = True Then
		...
		If MeasurementResult.Value/1024 < DumpsCollectionAndSendingParameters.SpaceReserveEnabled
			...
			SetMonitoringCenterParameter("SendingResult", NStr("ru = 'Недостаточно свободного места для хранения дампов. Сбор дампов будет отключен.';
																			|en = 'There is not enough free space to store dumps. Dump collection will be disabled.';"));	
			SetPrivilegedMode(False);
			StopFullDumpsCollection();
			Return;
		EndIf;
	...
	Else
		...
		If MeasurementResult.Value/1024 < DumpsCollectionAndSendingParameters.SpaceReserveDisabled
			...
			SetMonitoringCenterParameter("SendingResult", NStr("ru = 'Недостаточно свободного места для сбора дампов.';
																			|en = 'There is not enough free space to collect dumps.';"));	
			SetPrivilegedMode(False);
			StopFullDumpsCollection();
			Return;
		EndIf;
	...
	Else 
		...
		Else
			...
			QueryResult.ErrorDescription = NStr("ru = 'Не удалось разобрать результат typeperf';
													|en = 'Cannot parse the result typeperf';");
		EndIf;
	EndIf;
	...
	If TestPackageSendingPossible(MonitoringCenterParameters, StartDate2) And ExecutionParameters.TestPackageSending
		...
		Except
			...
			ExecutionResult.BriefErrorDescription = NStr("ru = 'Ошибка при формировании пакета.';
																	|en = 'An error occurred while generating the package.';");
			Comment = ErrorProcessing.DetailErrorDescription(ErrorInfo());
			WriteLogEvent(NStr("ru = 'Центр мониторинга.Сформировать тестовый пакет для отправки';
											|en = 'Monitoring center.Generate a test package for sending';",
				Common.DefaultLanguageCode()), EventLogLevel.Error,,, Comment);
		EndTry;
		...
		Try
			...
			Else
				...
				ExecutionResult.BriefErrorDescription = NStr("ru = 'Ошибка при отправке пакета.';
																		|en = 'An error occurred while sending a package.';");
				Template = NStr("ru = 'Ошибка HTTP при отправке пакета. Код %1';
								|en = 'An HTTP error occurred while sending a package. Code %1';");
				Comment = StringFunctionsClientServer.SubstituteParametersToString(Template, HTTPResponse.StatusCode); 
				WriteLogEvent(NStr("ru = 'Центр мониторинга.Отправить тестовые данные мониторинга';
												|en = 'Monitoring center.Send monitoring test data';",
					Common.DefaultLanguageCode()), EventLogLevel.Error,,, Comment);
			EndIf;
		Except
			ExecutionResult.Success = False;
	...
	If ReceivedParameters.Property("ErrorMessageDisplayVariant") 
		...
		MessageString = StringFunctions.FormattedString(
			StringFunctionsClientServer.SubstituteParametersToString(	
				NStr("ru = '<span style=""font:%1"">К сожалению, возникла непредвиденная ситуация</span>';
					|en = '<span style=""font:%1"">Oops… An unexpected situation has occurred</span>';"),
				"VeryLargeFont"));
