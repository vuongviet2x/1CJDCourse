...
Procedure AddSignature(Object, Val SignatureProperties, FormIdentifier = Undefined,
	...
	Except
		...
		If ValueIsFilled(EventLogMessage) Then
			WriteLogEvent(
				NStr("ru = 'Электронная подпись.Ошибка добавления подписи';
					|en = 'Digital signature. An error occurred while adding a signature';", Common.DefaultLanguageCode()),
...
Procedure DeleteSignature(Object, SequenceNumber, FormIdentifier = Undefined,
	...
	Except
		...
		If ValueIsFilled(EventLogMessage) Then
			WriteLogEvent(
				NStr("ru = 'Электронная подпись.Ошибка удаления подписи';
					|en = 'Digital signature.Signature deletion error';", Common.DefaultLanguageCode()),
				EventLogLevel.Information,
				Object.Metadata(),
				Object.Ref,
				EventLogMessage + "
...
Function WriteCertificateToCatalog(Val Certificate, AdditionalParameters = Undefined) Export
	...
	If Not AccessRightInsert And Not ValueIsFilled(CertificateReference) Then
		Raise(NStr("ru = 'Недостаточно прав для использования сертификата.';
								|en = 'insufficient rights to use the certificate.';"), ErrorCategory.AccessViolation);
	EndIf;
	...
	Try
		...
		If CertificateObject.CertificateData.Get() <> CertificateBinaryData Then
			If Not AccessRightInsert Then
				RollbackTransaction();
				Raise(NStr("ru = 'Недостаточно прав для изменения данных сертификата.';
										|en = 'Insufficient rights to modify certificate data.';"), ErrorCategory.AccessViolation);
			EndIf;
			CertificateObject.CertificateData = New ValueStorage(CertificateBinaryData);
		EndIf;
...
Function EnhanceSignature(Signature, SignatureType, AddArchiveTimestamp = False,
	...
	Except
		Result.ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Не удалось прочитать данные подписи: %1';
				|en = 'Cannot read the signature data: %1';"), ErrorProcessing.BriefErrorDescription(ErrorInfo()));
		Return Result;
	EndTry;
	...
	If ParametersCryptoSignatures.CertificateLastTimestamp = Undefined Then
		Result.ErrorText = NStr("ru = 'Не удалось получить сертификат подписи';
	...
	If Not EnhancementParameters.ShouldIgnoreCertificateValidityPeriod
		...
		Result.ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Истек срок действия подписи: %1';
				|en = 'The signature expired: %1';"), ParametersCryptoSignatures.DateActionLastTimestamp);
		Return Result;
	EndIf;
	...
	If CertificateVerificationResult Then
		If ValueIsFilled(SignatureType) And DigitalSignatureInternalClientServer.ToBeImproved(
			...
			Except
				Result.ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
						NStr("ru = 'Не удалось усовершенствовать подпись: %1';
							|en = 'Couldn''t enhance the signature: %1';"), ErrorProcessing.BriefErrorDescription(
					ErrorInfo()));
				Return Result;
			EndTry;
		ElsIf AddArchiveTimestamp And (ParametersCryptoSignatures.SignatureType
			...
			Except
				Result.ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
						NStr("ru = 'Не удалось добавить архивную метку времени в подпись: %1';
							|en = 'Cannot add an archive timestamp to the signature: %1';"), ErrorProcessing.BriefErrorDescription(
					ErrorInfo()));
				Return Result;
			EndTry;
	...
	Else
		...
		Result.ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Не действует сертификат подписи: %1
				|%2';
				|en = 'The signature certificate is invalid: %1
	...
	Except
		Result.ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Не удалось прочитать данные усовершенствованной подписи: %1';
					|en = 'Couldn''t read the enhanced signature data: %1';"), ErrorProcessing.BriefErrorDescription(
			ErrorInfo()));
		Return Result;
	EndTry;
	...
	Else
		...
		ErrorDescription = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Не прошел проверку сертификат полученной метки времени: %1
					 |%2';
					|en = 'The certificate of the received timestamp is invalid: %1
					|%2';"), ErrorDescription, InformationAboutCertificate);
...
Function ImproveObjectSignature(SignedObject, SequenceNumber, SignatureType, AddArchiveTimestamp = False,
	...
	If SetSignatures.Count() = 0 Then
		Result.ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Не удалось найти подпись с порядковым номером %1 у %2';
				|en = 'Cannot find a signature with sequence number %1 for %2';"), SequenceNumber, SignedObject);
		Return Result;
	EndIf;
...
Function VerifySignature(CryptoManager, RawData, Signature, ErrorDescription = Null, OnDate = Undefined, ResultStructure = Undefined) Export
	...
	ElsIf CryptoManagerToCheck = Undefined
		...
		If SignatureProperties.Certificate = Undefined Then
			ErrorDescription = NStr("ru = 'Сертификат не существует в данных подписи.';
									|en = 'The certificate does not exist in signature data.';");
			FillSignatureVerificationResult(ErrorDescription, ResultStructure);
			If RaiseException1 Then
				Raise ErrorDescription;
			EndIf;
	...
	If Base64Row <> Undefined Then
		...
		Except
			...
			Raise StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Не удалось получить данные из файла подписи по причине:
						|%1';
						|en = 'Cannot get data from the signature file. Reason:
...
Function Encrypt(Data, Certificate, EncryptAlgorithm = "") Export
	...
	If Not CommonSettings().UseEncryption Then
		Raise NStr("ru = 'Шифрование недоступно.';
								|en = 'Encryption unavailable.';");
	EndIf;
	...
	If ValueIsFilled(AttributesValues.Application) Then
		...
		ElsIf Not Common.DataSeparationEnabled()
			...
			If ValueIsFilled(ErrorAtServer) Then
				Raise StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Не удалось выполнить шифрование сертификатом ""%1"":
						 |%2';
						|en = 'Cannot encrypt with the ""%1"" certificate:
			...
			If CryptoManager = Undefined Then
				Raise StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Не удалось выполнить шифрование сертификатом ""%1"":
						|Не удалось создать менеджер криптографии';
						|en = 'Cannot encrypt with the ""%1"" certificate:
		...
		ErrorList.Add(NStr("ru = 'Недоступно шифрование на сервере.';
									|en = 'Encryption on the server is unavailable.';"));
	...
	If Not Common.DataSeparationEnabled() And (CommonSettings().GenerateDigitalSignaturesAtServer
		...
		If Not ValueIsFilled(ErrorDescription) Then
			...
			If CryptoManager = Undefined Then
				ErrorDescription = NStr("ru = 'Не удалось создать менеджер криптографии';
										|en = 'Cannot create a crypto manager';");
			Else
	...
	Raise StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Не удалось выполнить шифрование сертификатом ""%1""';
						|en = 'Cannot encrypt with the ""%1"" certificate';"), Certificate);
EndFunction
	...
	If Not Common.DataSeparationEnabled() Then
		If RequireImprovementSignatures Or RequiredAddArchiveTags Then
			...
			If TimestampServersAddresses.Count() = 0 Then
				Raise NStr("ru = 'Не указаны адреса серверов меток времени.';
										|en = 'Timestamp server addresses are not specified.';");
			EndIf;
		EndIf;
...
Procedure AddSignatureRows(DataObject, PropertiesSignatures, EventLogMessage)
	...
	For Each SignatureProperties In SignaturePropertiesArray Do
		...
		WriteLogEvent(
				NStr("ru = 'Электронная подпись.Добавление подписи';
					|en = 'Digital signature.Add signature';", Common.DefaultLanguageCode()),
...
Procedure DeleteSignatureRows(SignedObject, SequenceNumbers, EventLogMessage)
	...
	Try
		...
		If SelectionDetailRecords.Count() <> SequenceNumbersArray.Count() Then
			Raise NStr("ru = 'Строка с подписью не существует.';
									|en = 'A signature row does not exist.';");
		EndIf;
		...
		While SelectionDetailRecords.Next() Do
			...
			If Not HasRights Then
				Raise(NStr("ru = 'Недостаточно прав на удаление подписи.';
										|en = 'Insufficient rights to delete the signature.';"), ErrorCategory.AccessViolation);
			EndIf;
			...
			WriteLogEvent(NStr("ru = 'Электронная подпись.Удаление подписи';
											|en = 'Digital signature.Delete signature';", Common.DefaultLanguageCode()),
