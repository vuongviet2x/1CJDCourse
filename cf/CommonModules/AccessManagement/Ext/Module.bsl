		 ...
		 Or Upper(String.AccessKind) = Upper("EditRight") Then
			...
			If Upper(String.AccessKind) = Upper("EditRight") Then
				ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Ошибка в функции %1 модуля %2.
					           ...
					           |может быть зависимость только от права Чтения.';
								|en = 'Error in function ""%1"" of module ""%2"".
...
Function HasRight(Right, ObjectReference, Val User = Undefined) Export
	...
	If RightsDetails = Undefined Then
		ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Нет описания возможных прав для таблицы ""%1""';
				|en = 'Details about rights valid for table ""%1"" are missing.';"),
	...
	If RightDetails = Undefined Then
		ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Нет описания права ""%1"" для таблицы ""%2""';
				|en = 'Details about right ""%1"" for table ""%2"" are missing.';"),
...
Procedure OnCreateAccessValueForm(Form, AdditionalParameters = Undefined,
	...
	If TypeOf(CreateNewAccessValue) <> Type("Boolean") Then
		...
		Except
			...
			ErrorText = ErrorTitle + Chars.LF + Chars.LF + StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Параметр %1 не указан, а автоматическое заполнение
	...
	If Attribute = Undefined Then
		...
		Except
			ErrorInfo = ErrorInfo();
			ErrorText = ErrorTitle + Chars.LF + Chars.LF + StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Параметр Реквизит не указан, а автоматическое заполнение
				           |из реквизита формы ""%2"" недоступно по причине:
				           |%3';
	...
	Else
		...
		If PointPosition = 0 Then
			...
			Except
				ErrorInfo = ErrorInfo();
				ErrorText = ErrorTitle + Chars.LF + Chars.LF + StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Не удалось получить значение реквизита формы ""%1"",
	...
	If ValuesGroupsForChange.Count() = 0
	   And CreateNewAccessValue Then
		...
		ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Для добавления требуются разрешенные ""%1"".';
...
Procedure SetDynamicListFilters(List, FiltersDetails) Export
	...
	If TypeOf(List) <> Type("DynamicList") Then
		ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Ошибка вызова процедуры %1 общего модуля %2.
			           |Значение параметра %3 ""%4"" не является динамическим списком.';
						|en = 'Error calling procedure ""%1"" of common module ""%2"".
...
Procedure ReplaceRightsInObjectsRightsSettings(RenamedTable) Export
	...
	Try
		...
		If RepeatedNewNames.Count() > 0 Then
			...
			EndDo;
			ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Ошибка в параметрах процедуры %1
				           |общего модуля %2.
				           |
...
Procedure AddUpdateHandlerToEnableUniversalRestriction(Version, Handlers,
	...
	Handler.Comment = NStr("ru = 'Включение универсального ограничения доступа на уровне записей.';
									|en = 'Enables universal record-level access restriction.';");
...
//
// Parameters:
//  ObjectReference - AnyRef - Reference to the object from DefinedType.AccessValuesSetsOwnerObject
//                                 whose access value sets will be populated.
//
//  Table - See AccessValuesSetsTable
//
			...
			ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Объект ""%1"" сформировал набор значений доступа,
				           ...
				           |""%3"", ""%4"", если они используются.';
							|en = 'Object ""%1"" generated an access value set
		...
		ErrorText =
			NStr("ru = 'Данная операция возможна только для упрощенного
			           |интерфейса настройки прав доступа.';
						|en = 'This operation is available only in the simplified
	...
	If TypeOf(User) <> Type("CatalogRef.Users")
	   And TypeOf(User) <> Type("CatalogRef.ExternalUsers") Then
		...
		ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
			...
			ExpectedTypes, 
			?(ParameterValue <> Undefined, ParameterValue, NStr("ru = 'Неопределено';
																		|en = 'Undefined';")),
						...
						ErrorText =
							NStr("ru = 'Профиль Администратор должен быть хотя бы у одного пользователя,
							           |которому разрешен вход в приложение.';
										|en = 'At least one user authorized to log in
