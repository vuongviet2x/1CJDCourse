...
Procedure BeforeImportData(Container) Export
	...
	If ReaderStream.NodeType <> XMLNodeType.StartElement
		...
		Raise StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Неверный формат файла XML. Ожидается начало элемента %1.';
				|en = 'Invalid XML file format. Start of ""%1"" element is expected.';"), "Data");
		
	EndIf;
	
	If Not ReaderStream.Read() Then
		Raise NStr("ru = 'Неверный формат файла XML. Обнаружено завершение файла.';
								|en = 'Invalid XML file format. File end is detected.';");
	EndIf;
