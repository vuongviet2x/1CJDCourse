...
Function TranslateTheTexts(Texts, TranslationLanguage = Undefined, SourceLanguage = Undefined) Export
	...
	If Not QueryResult.QueryCompleted Then
		Raise ErrorText(NStr("ru = 'Не удалось выполнить перевод текста.';
											|en = 'Cannot translate text.';"));
	EndIf;
...
Function NewIAMToken()
	...
	If Not QueryResult.QueryCompleted Then
		Raise ErrorText(StringFunctionsClientServer.SubstituteParametersToString(NStr(
			"ru = 'Не удалось авторизоваться в сервисе %1.';
...
Function AvailableLanguages() Export
	...
	If Not QueryResult.QueryCompleted Then
		Raise ErrorText(NStr("ru = 'Не удалось получить список доступных языков.';
											|en = 'Cannot retrieve the list of available languages.';"));
	EndIf;
...
Function ExecuteQuery(Val HTTPRequest, Val HostName)
	...
	Except
		WriteErrorToEventLog(StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Не удалось установить соединение с сервером %1 по причине:
			|%2';
			|en = 'Cannot establish a connection with server %1 due to:
	...
	If HTTPResponse.StatusCode <> 200 Then
		ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Запрос ""%1"" не выполнен. Код состояния: %2.';
				|en = 'Request ""%1"" failed. Status code: %2.';"),
...
Procedure OnDefineSettings(Settings) Export
	...
	Parameter.Presentation = NStr("ru = 'OAuth-токен';
									|en = 'OAuth token';");
	Parameter.ToolTipRepresentation = ToolTipRepresentation.ShowTop;
	
	Parameter = Settings.AuthorizationParameters.Add();
	Parameter.Name = "DirectoryID";
	Parameter.Presentation = NStr("ru = 'Идентификатор каталога';
									|en = 'Directory ID';");
	Parameter.ToolTipRepresentation = ToolTipRepresentation.ShowTop;
...
Procedure WriteErrorToEventLog(Comment)
	...
	WriteLogEvent(NStr("ru = 'Перевод текста';
									|en = 'Translator';", Common.DefaultLanguageCode()),
...
Function ErrorText(ErrorText)
	...
	If Users.IsFullUser() Then
		Return ErrorText + Chars.LF + NStr("ru = 'Подробности см. в журнале регистрации.';
												|en = 'See the Event log for details.';");
	EndIf;
	
	Return ErrorText + Chars.LF + NStr("ru = 'Обратитесь к администратору.';
...
Function Permissions() Export
	...
	Port = Undefined;
	LongDesc = NStr("ru = 'Сервис перевода текста Яндекс.Переводчик';
					|en = 'Yandex Translate translation service';");
