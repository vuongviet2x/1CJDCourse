		...
		If SourcesCommaSeparated = "" Then
			Raise StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'В формах отчетов, обработок и в общих формах
					...
					|en = 'To call the %1 procedure in report forms,
...
Function AttachableObjectSettings(FullName, InterfaceSettings4 = Undefined) Export
	...
	Except
		ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Не удалось прочитать настройки объекта %1 из модуля менеджера:';
				|en = 'Cannot read %1 object settings from the manager module:';"), FullName);
		...
		If WriteError1 Then
			WriteLogEvent(
				NStr("ru = 'Подключаемые команды';
					|en = 'Attachable commands';", Common.DefaultLanguageCode()),
				EventLogLevel.Error,
	...
	For Each Setting In InterfaceSettings4 Do
		...
		Except
			ErrorText = NStr("ru = 'Не удалось зарегистрировать настройку программного интерфейса подключаемых объектов.
				...
				|en = 'Cannot register a setting for attachable objects application interface.
...
Procedure CheckCommandsKindName(KindName) Export
	...
	If Not CommonClientServer.NameMeetPropertyNamingRequirements(KindName) Then
		ErrorText = NStr("ru = 'Имя вида команд ""%1"" не удовлетворяет требованиям именования переменных.';
							|en = 'Command kind name ""%1"" does not meet naming requirements for variables.';");
...
Function RegisterSubmenu(Items, InfoOnAllSubmenus, SubmenuName, NewSubmenuTemplate = Undefined, 
	...
	If ValueIsFilled(SubmenuName) Then
		...
		If Not Groups.Property("Important") Then
			...
			GroupImportant.Title = Popup.Title + " (" + NStr("ru = 'Важное';
																	|en = 'Important';") + ")";
		...
		If Not Groups.Property("Ordinary") Then
			...
			DefaultGroup.Title = Popup.Title + " (" + NStr("ru = 'Обычное';
																			|en = 'Standard';") + ")";
		...
		If Not Groups.Property("SeeAlso") Then
			...
			GroupSeeAlso.Title = Popup.Title + " (" + NStr("ru = 'См. также';
																		|en = 'See also:';") + ")";
...
Function CommandDetails(CommandNameInForm, SettingsAddress) Export
	...
	If Command = Undefined Then
		Raise StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Сведения о команде ""%1"" не существуют.';
				|en = 'Information on command ""%1"" does not exist.';"),
			CommandNameInForm);
	EndIf;
