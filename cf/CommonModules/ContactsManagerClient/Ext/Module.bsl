...
Procedure CreateEmailMessage(Val FieldValues, Val EmailParameters = Undefined,
	...
	If IsBlankString(MailAddr) Then
		...
		If IsBlankString(MailAddr) Then
			ErrorText= NStr("ru = 'Для отправки письма введите адрес электронной почты.';
								|en = 'To send an email, enter an email address.';");
		ElsIf TypeOf(MailAddr) <> Type("String") Or InformationType = Undefined Then
			ErrorText =  NStr("ru = 'Невозможно отправить письмо, т.к. значение не является адресом электронной почты.';
								|en = 'Cannot send an email as the value is not an email address.';");
		ElsIf InformationType <> PredefinedValue("Enum.ContactInformationTypes.Email") Then
			ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Нельзя создать письмо по контактной информацию с типом ""%1"".';
					|en = 'Cannot create an email from contact information of the ""%1"" type.';"), InformationType);
		Else
...
Procedure CreateSMSMessage(Val FieldValues, Val SMSParameters = Undefined,
	...
	If Not CommonClient.SubsystemExists("StandardSubsystems.SendSMSMessage") Then
		Raise NStr("ru = 'Отправка SMS недоступна.';
								|en = 'Text messaging is not available.';");
	EndIf;
	...
	If IsBlankString(SMSParameters.Presentation) Then
		...
		If InformationType <> PredefinedValue("Enum.ContactInformationTypes.Phone") Then
			MessageText = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Невозможно отправить SMS, т.к. значение не является номером телефона (""%1"").';
					|en = 'Cannot send the text message as the phone number you provided is invalid: %1.';"), 
				InformationType);
		ElsIf FieldValues = "" And IsBlankString(SMSParameters.Presentation) Then
			ErrorText = NStr("ru = 'Для отправки SMS введите номер телефона.';
								|en = 'To send a text message, enter a phone number.';");
		EndIf;
...
#If Not WebClient Then
		...
		If AvailableProtocolName = Undefined Then
			StringWithWarning = New FormattedString(
					NStr("ru = 'Для совершения звонка установите приложение телефонии, например';
						|en = 'To make a call, install a telecom app. For example,';"),
...
Procedure GoToWebLink(Val FieldValues, Val Presentation = "", ExpectedKind = Undefined) Export
	...
	If InformationType <> PredefinedValue("Enum.ContactInformationTypes.WebPage") Then
		Raise StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Нельзя открыть ссылку по контактной информации с типом ""%1""';
	...
	If TypeOf(HyperlinkAddress) <> Type("String") Then
		Raise NStr("ru = 'Ошибка получения ссылки, неверный тип контактной информации';
								|en = 'Error getting URL. Invalid contact information type.';");
	EndIf;
...
#If Not WebClient Then
		If IsBlankString(TelephonyApplicationInstalled("skype")) Then
			ShowMessageBox(Undefined, NStr("ru = 'Для совершения звонка по Skype установите приложение.';
														|en = 'Install Skype to make Skype calls.';"));
			Return;
		EndIf;
#EndIf
...
Procedure BeforePhoneCall(ContactInformation, AdditionalParameters) Export
	...
	If IsBlankString(ContactInformation.Presentation) Then
		CommonClient.MessageToUser(
			NStr("ru = 'Для совершения звонка введите номер телефона.';
				|en = 'To start a call, enter a phone number.';"), , AdditionalParameters.AttributeName);
		Return;
	EndIf;
...
Procedure BeforeCreateSMS(ContactInformation, AdditionalParameters) Export
	...
	If IsBlankString(ContactInformation.Presentation) Then
		CommonClient.MessageToUser(
			NStr("ru = 'Для отправки SMS введите номер телефона.';
				|en = 'To send a text message, enter a phone number.';"), , AdditionalParameters.AttributeName);
		Return;
	EndIf;
	SMSParameters = SMSAndEmailParameters();
