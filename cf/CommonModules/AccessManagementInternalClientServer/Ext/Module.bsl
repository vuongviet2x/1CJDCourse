...
Procedure OnChangeCurrentAccessKind(Form, ProcessingAtClient = True) Export
		...
		If CurrentData.AccessKind = Form.AccessKindUsers
		 Or CurrentData.AccessKind = Form.AccessKindExternalUsers Then
			...
			LabelPattern = ?(CurrentData.AllAllowed,
				NStr("ru = 'Запрещенные значения (%1) - авторизованный пользователь и его группы всегда разрешены';
					|en = 'Forbidden values (%1): Authorized user and their groups are always allowed';"),
				NStr("ru = 'Разрешенные значения (%1) - авторизованный пользователь и его группы всегда разрешены';
					|en = 'Allowed values (%1): Authorized user and their groups are always allowed';") );
		Else
			LabelPattern = ?(CurrentData.AllAllowed,
				NStr("ru = 'Запрещенные значения (%1)';
					|en = 'Denied values (%1)';"),
				NStr("ru = 'Разрешенные значения (%1)';
					|en = 'Allowed values (%1)';") );
		EndIf;
	...
	If Form.CurrentTypesOfValuesToSelect.Count() = 0 Then
		Form.CurrentTypesOfValuesToSelect.Add(Undefined, NStr("ru = 'Неопределено';
																		|en = 'Undefined';"));
	EndIf;
	...
	While AccessKindIndex > 0 Do
		...
		If AccessKindRow.AccessKind = Undefined Then
			CommonClientServer.AddUserError(Errors,
				...
				NStr("ru = 'Вид доступа не выбран.';
					|en = 'The access kind is not selected.';"),
				"AccessKinds",
				AccessKinds.Find(AccessKindRow),
				NStr("ru = 'Вид доступа в строке %1 не выбран.';
		...
		If Parameters.Purpose <> Undefined
		  And Not AccessKindMatchesProfileAssignment(AccessKindRow.AccessKind, ProfileAssignment) Then
			...
			CommonClientServer.AddUserError(Errors,
				Parameters.PathToTables + "AccessKinds[%1].AccessKind",
				StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Вид доступа ""%1"" не соответствует назначению профиля.';
																			|en = 'Access kind ""%1"" does not match the profile assignment.';"),
					AccessKindRow.AccessKindPresentation),
				"AccessKinds",
				AccessKinds.Find(AccessKindRow),
				StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Вид доступа ""%1"" в строке %2 не соответствует назначению профиля.';
		...
		If FoundAccessKinds.Count() > 1 Then
			CommonClientServer.AddUserError(Errors,
				Parameters.PathToTables + "AccessKinds[%1].AccessKind",
				StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Вид доступа ""%1"" повторяется.';
																			|en = 'Duplicate access kind: ""%1"".';"),
					AccessKindRow.AccessKindPresentation),
				"AccessKinds",
				AccessKinds.Find(AccessKindRow),
				StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Вид доступа ""%1"" в строке %2 повторяется.';
		...
		While AccessValueIndex > 0 Do
			...
			If AccessValueRow.AccessValue = Undefined Then
				...
				CommonClientServer.AddUserError(Errors,
					...
					NStr("ru = 'Значение не выбрано.';
						|en = 'No value is selected.';"),
					"AccessValues",
					AccessValues.Find(AccessValueRow),
					NStr("ru = 'Значение в строке %1 не выбрано.';
			...
			If FoundValues.Count() > 1 Then
				...
				CommonClientServer.AddUserError(Errors,
					Parameters.PathToTables + "AccessValues[%1].AccessValue",
					NStr("ru = 'Значение повторяется.';
						|en = 'Duplicate value.';"),
					"AccessValues",
					AccessValues.Find(AccessValueRow),
					NStr("ru = 'Значение в строке %1 повторяется.';
