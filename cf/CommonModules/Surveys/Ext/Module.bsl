...
Procedure OnAddUpdateHandlers(Handlers) Export
	...
	Handler.Comment = NStr("ru = 'Заполнение значения нового реквизита ""Режим анкетирования"" у документов ""Анкета"" прошлых периодов.
								  |До завершения обработки ""Режим анкетирования"" данных документов будет отображаться некорректно.';
									|en = 'The new attribute ""Survey mode"" is being populated in the ""Questionnaire"" documents.
	...
	Handler.Comment = NStr("ru = 'Заполнение значения нового реквизита ""Способ отображения подсказки"" в справочнике ""Вопросы шаблона анкеты"".
								  |До завершения обработки ""Способ отображения подсказки"" в данном справочнике будет отображаться некорректно.';
									|en = 'New attribute ""Tooltip display mode"" is being populated in the ""Questionnaire template questions"" catalog.
									|Until the population is complete, the attribute might be displayed incorrectly.';");
	Handler.ExecutionMode = "Deferred";
	...
	Handler.Comment = NStr("ru = 'Заполнение значений новых реквизитов ""Вид"", ""Использовать минимальное значение"", ""Использовать максимальное значение"" в вопросах для анкетирования.
								  |До завершения обработки вид вопросов для анкетирования будет отображаться некорректно.';
									|en = 'Fill the new ""Display format"", ""Use minimum value"", and ""Use maximum value"" attributes in survey questions.
...
Procedure CreateFillingFormBySection(Form, CurrentDataSectionsTree) Export
	...
	If CurrentDataSectionsTree.RowType = "Section" Then
		...
		ItemLabelIntroduction.Title  = NStr("ru = 'Раздел';
													|en = 'Section';") + " " + CurrentDataSectionsTree.Wording;
	...
	Else
		...
		Introduction = ?(IsBlankString(Form.Introduction), NStr("ru = 'Нажмите далее для заполнения анкеты.';
															|en = 'Click Next to fill out the questionnaire.';"),
			Form.Introduction);
		ClosingStatement = ?(IsBlankString(Form.ClosingStatement), NStr("ru = 'Спасибо за то что заполнили анкету.';
															|en = 'Thank you for filling out the questionnaire.';"),
...
Procedure DeleteFillingFormItems(Form, AttributesToBeDeleted)
	...
	For Each AttributeToDelete In AttributesToBeDeleted Do
		...
		QuestionName = Left(AttributeToDelete.Value, CharCountBeforePostfix);
