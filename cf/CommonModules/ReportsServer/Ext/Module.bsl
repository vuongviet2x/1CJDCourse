...
Procedure InitializeSettingsComposer(SettingsComposer, Schema, Report = Undefined, ReportVariant = Undefined) Export 
	...
	Except
		EventName = NStr("ru = 'Ошибка инициализации компоновщика настроек компоновки данных.';
							|en = 'Settings Composer failed to initialize.';",
...
Procedure OutputCommand(ReportForm, CommandOrCommands, GroupType, ToGroupBeginning = False, OnlyInAllActions = False, SubgroupSuffix = "") Export
	...
	Else
		Raise StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'При вызове процедуры ""%1"" передано недопустимо значение параметра ""%2"".';
				|en = 'Invalid value is passed in parameter %2 on calling procedure %1.';"),
...
Function ReportObject(Id) Export
	...
	Else
		Raise StrReplace(NStr("ru = 'Некорректное полное имя отчета ""%1"".';
											|en = 'Report %1 has invalid name.';"), "%1", FullName);
	EndIf;
	...
	Else
		Raise StrReplace(NStr("ru = '""%1"" не является отчетом.';
											|en = '%1 is not a report.';"), "%1", FullName);
	EndIf;
EndFunction
...
Procedure RegroupSettingsFormItems(Form, Val ItemsHierarchyNode, ItemsProperties, AttributesNames, StyylizedItemsKinds)
	...
	For Each GroupID In GroupsIDs Do 
		...
		If Group = Undefined Then 
			...
			Group.Title = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Строка %1';
																							|en = 'Row %1';"), NumberOfGroup);
		EndIf;
...
Procedure AddColumnMargins(Form, SettingsItems, ItemsHierarchyNode)
	...
	For ColumnNumber = 2 To ColumnsCount Do 
		...
		ColumnFieldGroup.Title = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Поля колонки %1';
																									|en = 'The %1 column fields';"), ColumnNumber);
...
Function SettingsFormItemsHierarchy(Form, Parent, Signs, LineNumber, ColumnNumber)
	...
	String.Title = StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = 'Строка %1.%2';
			|en = 'Row %1.%2';"), ColumnNumber, LineNumber);
	...
	Column.Title = StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = 'Колонка %1.%2.1';
			|en = 'Column %1.%2.1';"), ColumnNumber, LineNumber);
...
Function PeriodElementHeader(Property, SettingItemTitle, ShouldOutputPeriodTitles)
	...
	Return StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = '%1 (дата %2)';
			|en = '%1 (date %2)';"),
		SettingItemTitle,
		?(StrEndsWith(Lower(Property), Lower("StartDate")), NStr("ru = 'начала';
																		|en = 'start';"), NStr("ru = 'окончания';
																								|en = 'end';")));
...
Procedure AddAPeriodShiftCommand(Form, Group, NameTemplate, ThisIsTheStandardRepresentation, ShiftDirection = 1)
	...
	If ShiftDirection > 0 Then 
		...
		Command.Title = ">";
		Command.ToolTip = NStr("ru = 'Сдвинуть вперед';
								|en = 'Move forward';");
	Else
		Command.Action = "Attachable_MoveThePeriodBack";
		Command.Title = "<";
		Command.ToolTip = NStr("ru = 'Сдвинуть назад';
								|en = 'Move back';");
	EndIf;
...
Procedure AddPeriodChoiceCommand(Form, Group, NameTemplate, Period, ThisIsTheStandardRepresentation)
	...
	Command.Title = NStr("ru = 'Выбрать период...';
							|en = 'Select period…';");
...
Procedure AddListCommands(Form, SettingItem, SettingsItems, ListName)
	...
	If HasReferenceType Then
		...
		CommandTitle = NStr("ru = 'Вставить из буфера обмена...';
