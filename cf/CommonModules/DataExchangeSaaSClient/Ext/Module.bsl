...
Procedure OnStart(Parameters) Export
	...
	If ClientRunParameters.IsStandaloneWorkplace Then
		...
		If ClientRunParameters.SynchronizeDataWithWebApplicationOnStart Then
			...
			ShowUserNotification(NStr("ru = 'Автономная работа';
												|en = 'Standalone mode';"), "e1cib/app/DataProcessor.DataExchangeExecution",
				NStr("ru = 'Рекомендуется синхронизировать данные с приложением в Интернете.';
					|en = 'It is recommended that you synchronize the workstation data with the web application.';"), PictureLib.Information32);
...
Procedure BeforeExit(Cancel, Warnings) Export
	...
	If ApplicationParameters["StandardSubsystems.SuggestDataSynchronizationWithWebApplicationOnExit"] = True
		...
		WarningParameters.ExtendedTooltip = NStr("ru = 'В некоторых случаях синхронизация данных может занять длительное время:
		...
		WarningParameters.WarningText = NStr("ru = 'Не выполнена синхронизация данных с приложением в Интернете.';
															|en = 'Data is not synchronized with the web application.';");
		WarningParameters.CheckBoxText = NStr("ru = 'Синхронизировать данные с приложением в Интернете';
