	...
	If TypeOf(Validation) = Type("String") Then
		...
		If CheckToExecute.IsEmpty() Then
			Raise StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Проверка ведения учета с идентификатором ""%1"" не существует (см. %2)';
					|en = 'Data integrity check with ID %1 does not exist (see %2).';"),
				Validation,
...
Procedure ExecuteChecksInContext(AccountingChecksContext) Export
	...
	ExecutionParameters.BackgroundJobDescription = NStr("ru = 'Контроль ведения учета';
															|en = 'Data integrity';");
	...
	If MethodParameters.Count() <> Results.Count() Then
		ErrorText = NStr("ru = 'Не все проверки были выполнены';
							|en = 'Some checks were not performed';");
		Raise ErrorText;
	EndIf;
...
Procedure OnCreateListFormAtServer(Form, ListsNames, AdditionalProperties = Undefined) Export
	...
	For Each ListName In NamesList Do
		...
		ErrorIndicatorColumn.Title          = NStr("ru = 'Индикатор ошибки';
														|en = 'Error indicator';");
...
Function EventLogEvent()
	...
	Return NStr("ru = 'Контроль ведения учета';
