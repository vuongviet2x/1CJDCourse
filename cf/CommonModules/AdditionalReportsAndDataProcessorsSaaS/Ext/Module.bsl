		...
		For Each AttributeToControl In AttributesToControl Do
			...
			If SourceAttribute <> ResultingAttribute Then
				Raise StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Недопустимая попытка изменения значения реквизита %1 для дополнительной обработки %2, 
						|полученной из каталога дополнительных обработок менеджера сервиса.';
						|en = 'Invalid attempt to change attribute value %1 for additional data processor %2
...
Procedure OnAttachExternalDataProcessor(Val Ref, StandardProcessing, Result) Export
	...
	If TypeOf(Ref) <> Type("CatalogRef.AdditionalReportsAndDataProcessors")
		...
		Raise NStr("ru = 'Запрошено подключение несуществующей дополнительной обработки.';
								|en = 'A non-existing additional data processor connection requested.';");
	EndIf;
...
Procedure OnCreateExternalDataProcessor(Val Ref, StandardProcessing, Result) Export
	...
	If DataProcessorName = Undefined Then
		Raise NStr("ru = 'Запрошено создание объекта несуществующей дополнительной обработки.';
								|en = 'Creation is requested for an object of a non-existing additional data processor.';");
	EndIf;
...
Procedure BeforeUpdateJob(Object, Command, Job, Changes) Export
	...
	If Not Constants.AllowScheduledJobsExecutionSaaS.Get() Then
		Raise NStr("ru = 'Периодическое выполнение команд дополнительных обработок в качестве заданий запрещено администратором сервиса.';
								|en = 'It is prohibited by the service administrator to periodically run additional data processor commands as jobs.';");
	EndIf;
	...
	If Job.Schedule.ExecutionRequired(DateToCheck, SourceDate1) Then
		Raise StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Расписание, задаваемое для выполнения команд дополнительного отчета или обработки в качестве заданий, должно быть не чаще, чем 1 раз в %1 секунд.';
				|en = 'Schedule set for execution of commands of an additional report or a data processor as jobs must be maximum once in %1 seconds.';"), 
			MinInterval);
	EndIf;
...
Procedure InstallSuppliedDataProcessorToDataArea(Val InstallationDetails, Val QuickAccess, Val Jobs, Val Sections, 
	...
	WriteLogEvent(
		NStr("ru = 'Поставляемые дополнительные отчеты и обработки.Инициирована установка поставляемой обработки в область данных';
			|en = 'Built-in additional reports and data processors.Installation of the built-in data processor is initiated in the data area';",
	...
	Try
		...
		If SuppliedDataProcessorExists Then
			...
			WriteLogEvent(NStr("ru = 'Поставляемые дополнительные отчеты и обработки.Установка в область данных';
											|en = 'Additional built-in reports and data processors.Installation to the data area';",
				Common.DefaultLanguageCode()),
				EventLogLevel.Information,
				,
		...
		Else
			...
			WriteLogEvent(NStr("ru = 'Поставляемые дополнительные отчеты и обработки.Установка в область данных отложена';
											|en = 'Additional built-in reports and data processors. Installation to the data area is deferred';",
				Common.DefaultLanguageCode()),
				EventLogLevel.Information,
				,
				String(InstallationDetails.Id),
...
Procedure DeleteSuppliedDataProcessorFromDataArea(Val SuppliedDataProcessor, Val IDOfDataProcessorToUse) Export
	...
	Try
		...
		WriteLogEvent(NStr("ru = 'Поставляемые дополнительные отчеты и обработки.Удаление из области данных';
										|en = 'Built-in additional reports and data processors.Delete from the data area';",
			Common.DefaultLanguageCode()),
			EventLogLevel.Information,
			,
			SuppliedDataProcessor,
	...
	If Not IsBlankString(ExceptionText) Then
		...
		WriteLogEvent(NStr("ru = 'Поставляемые дополнительные отчеты и обработки.Ошибка удаления из область данных';
										|en = 'Built-in additional reports and data processors. An error occurred when deleting from the data area';",
			Common.DefaultLanguageCode()),
			EventLogLevel.Error,
			,
			SuppliedDataProcessor,
...
Procedure ProcessErrorOfInstallingAdditionalDataProcessorToDataArea(Val SuppliedDataProcessor, Val InstallationID, Val ExceptionText) Export
	...
	WriteLogEvent(NStr("ru = 'Поставляемые дополнительные отчеты и обработки.Ошибка установки в область данных';
									|en = 'Additional built-in reports and data processors. An error occurred during installation to the data area';",
		Common.DefaultLanguageCode()),
		EventLogLevel.Error,
		,
		SuppliedDataProcessor,
...
Procedure AppliedDataProcessorSettingsUpdate(Val Ref) Export
	...
	Try
		...
		If Not ValueIsFilled(SuppliedDataProcessor) Then
			Raise StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Дополнительная обработка с идентификатором %1 не является поставляемой.';
					|en = 'Additional data processor with %1 ID is not built-in.';"),
				String(Ref.UUID()));
		EndIf;
...
Procedure NewDataAvailable(Val Descriptor, ToImport) Export
	...
	If Descriptor.DataType = SuppliedDataKindID() Then
		...
		Else
			...
			WriteLogEvent(
				NStr("ru = 'Поставляемые дополнительные отчеты и обработки.Загрузка поставляемой обработки отменена';
					|en = 'Built-in additional reports and data processors.Built-in data processor import canceled';", 
				Common.DefaultLanguageCode()),
				EventLogLevel.Information,
				,
				,
				NStr("ru = 'Поставляемая обработка несовместима с данной конфигурацией';
					|en = 'The built-in data processor is not compatible with this configuration';") + Chars.LF + Chars.CR + SuppliedDataProcessorDetails.Compatibility);
...
Function SuppliedDataProcessor(DataProcessorToUse)
	...
	If Not Common.SeparatedDataUsageAvailable() Then
		MessageText = NStr("ru = 'Использование функции %1 доступно только сеансов с установленным разделением данных.';
								|en = 'You can use function %1 only in sessions with enabled data separation.';");
...
Procedure ProcessSuppliedAdditionalReportsAndDataProcessors(Descriptor, PathToFile)
	...
	WriteLogEvent(NStr("ru = 'Поставляемые дополнительные отчеты и обработки.Загрузка поставляемой обработки';
									|en = 'Built-in additional reports and data processors.Import built-in data processor';", 
		Common.DefaultLanguageCode()),
		EventLogLevel.Information,
		,
		,
		NStr("ru = 'Инициирована загрузка поставляемой обработки';
			|en = 'Built-in data processor import initiated';") + Chars.LF + Chars.CR + SuppliedDataProcessorDetails.Manifest);
	...
	WriteLogEvent(NStr("ru = 'Поставляемые дополнительные отчеты и обработки.Загрузка поставляемой обработки завершена';
									|en = 'Built-in additional reports and data processors.Built-in data processor is imported';",
		Common.DefaultLanguageCode()),
		EventLogLevel.Information,
		,
		SuppliedDataProcessor.Ref,
		NStr("ru = 'Завершена загрузка поставляемой обработки';
			|en = 'Built-in data processor import is completed';") + Chars.LF + Chars.CR + SuppliedDataProcessorDetails.Manifest);
	...
	For Each DataProcessorInstallation In DataProcessorsToUse Do
		...
		WriteLogEvent(NStr("ru = 'Поставляемые дополнительные отчеты и обработки.Запланирована актуализация настроек поставляемой обработки';
										|en = 'Built-in additional reports and data processors.Update of the built-in data processor settings is scheduled';",
			Common.DefaultLanguageCode()),
			EventLogLevel.Information,
			,
			SuppliedDataProcessor.Ref,
			NStr("ru = 'Область данных:';
				|en = 'Data area:';") + DataProcessorInstallation.DataArea);
	...
	For Each QueueItem In InstallationsQueue Do
		...
		WriteLogEvent(
			NStr("ru = 'Поставляемые дополнительные отчеты и обработки.Запланирована отложенная установка поставляемой обработки в область данных';
				|en = 'Additional built-in reports and data processors.Deferred installation of the built-in data processor to the data area is scheduled';",
			Common.DefaultLanguageCode()),
			EventLogLevel.Information,
			,
			SuppliedDataProcessor.Ref,
			NStr("ru = 'Область данных:';
				|en = 'Data area:';") + QueueItem.DataArea);
...
Procedure CheckCanExecute(Ref)
	...
	If Not IsSuppliedDataProcessor(Ref) Then
		If Not GetFunctionalOption("IndependentUsageOfAdditionalReportsAndDataProcessorsSaaS") Then
			Raise NStr("ru = 'Этот дополнительный отчет или обработка не может быть использован в сервисе.';
									|en = 'This additional report or data processor cannot be used in SaaS.';");
		EndIf;
		Return;
	EndIf;
