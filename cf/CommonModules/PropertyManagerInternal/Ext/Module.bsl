	...
	If CatalogMetadata.TabularSections.Find("AdditionalAttributes") <> Undefined Then
		...
		For Each Property In Properties Do
			If Not Property.IsAdditionalInfo Then
				...
				ColumnsInfoRow.Group                   = NStr("ru = 'Доп. реквизиты';
				...
				If TypeOf(Values) = Type("Array") And Values.Count() > 0 Then
					ColumnsInfoRow.Note = ColumnsInfoRow.Note  + Chars.LF + NStr("ru = 'Варианты значений:';
																											|en = 'Available values:';") + Chars.LF;
		...
		For Each Property In AdditionalInfo Do
			...
			ColumnsInfoRow.IsRequiredInfo = Property.RequiredToFill;
			ColumnsInfoRow.Position                  = Position;
			ColumnsInfoRow.Group                   = NStr("ru = 'Доп. свойства';
...
Procedure OnAddUpdateHandlers(Handlers) Export
	...
	Handler.Comment = NStr("ru = 'Заполняет уникальное имя и обновляет зависимости дополнительных реквизитов и сведений.
		|Редактирование дополнительных реквизитов и сведений будет недоступно до завершения обновления.';
		|en = 'Provides a unique name and updates the dependencies of additional attributes and information records.
	...
	Handler.Comment = NStr("ru = 'Реструктуризация дополнительных реквизитов и сведений.
		...
		|en = 'Restructures additional attributes and information records.
...
Procedure OnFillAccessKinds(AccessKinds) Export
	...
	AccessKind.Presentation = NStr("ru = 'Дополнительные сведения';
	...
	For Each UnprocessedDuplicate In UnprocessedOriginalsValues Do
		...
		Except
			...
			WriteLogEvent(NStr("ru = 'Поиск и замена ссылок';
											|en = 'Reference search and replacement';", Common.DefaultLanguageCode()),
				EventLogLevel.Error,
...
Procedure AdditionalAttributesFillCheckProcessing(Source, Cancel, CheckedAttributes) Export
	...
	For Each Item In Dependencies Do
		...
		If FillingRequirementDependencies.Count() = 0 Then
			Text = NStr("ru = 'Реквизит ""%1"" не заполнен.';
						|en = 'Attribute ""%1"" is required.';");
...
Procedure DeleteSettingFromStorage(Filter)
	...
	While NextSettingsItem(SettingsSelection) Do
		...
		Except
			ErrorInfo = ErrorInfo();
			ErrorText = NStr("ru = 'При очистке настроек в обработчике %1
				|возникла ошибка:
				|%2';
...
Function NextSettingsItem(Selection)
	...
	Except
		ErrorInfo = ErrorInfo();
		ErrorText = NStr("ru = 'При чтении настроек в обработчике %1
			|возникла ошибка:
			|%2';
			|en = 'An error occurred
...
Function DescriptionAlreadyUsed(Property, PropertiesSet, Description) Export
	...
	If Selection.IsAdditionalInfo Then
		QueryText = NStr("ru = 'Существует дополнительное сведение с наименованием
		                          |""%1"".';
									|en = 'The additional information record with description
									|""%1"" already exists.';");
	Else
		QueryText = NStr("ru = 'Существует дополнительный реквизит с наименованием
		                          |""%1"".';
									|en = 'The additional attribute with description
									|""%1"" already exists.';");
	EndIf;
	...
	QueryText = StringFunctionsClientServer.SubstituteParametersToString(
		QueryText + Chars.LF + Chars.LF
								...
								|en = 'It is recommended that you enter another description,
...
Function NameAlreadyUsed(Val Name, Val CurrentProperty) Export
	...
	If NewName <> Name
		...
		QueryText = NStr("ru = 'Имя (группа Для разработчиков) должно состоять из одного слова, начинаться с буквы и не содержать специальных символов, кроме ""_"".';
							|en = 'The name (the For developing purpose group) must be a single word that starts with a letter and can contain digits, letters, and underscores ( _ ).';");
		Return QueryText;
	EndIf;
	...
	If Selection.IsAdditionalInfo Then
		QueryText = NStr("ru = 'Существует дополнительное сведение с именем
		                          |""%1"".';
									|en = 'The additional information record with name
									|""%1"" already exists.';");
	Else
		QueryText = NStr("ru = 'Существует дополнительный реквизит с именем
		                          |""%1"".';
									|en = 'The additional attribute with name
									|""%1"" already exists.';");
	EndIf;
	...
	QueryText = StringFunctionsClientServer.SubstituteParametersToString(
		QueryText + Chars.LF + Chars.LF
								...
								|en = 'It is recommended that you enter another name,
								|otherwise, the application might not work properly.
								|
...
Function IDForFormulasAlreadyUsed(Val IDForFormulas, Val CurrentProperty) Export
	...
	If Upper(IDForFormulas) <> Upper(VerificationID) Then
		QueryText = NStr("ru = 'Идентификатор ""%1"" не соответствует правилам именования переменных.
	...
	If Selection.IsAdditionalInfo Then
		QueryText = NStr("ru = 'Существует дополнительное сведение с идентификатором для формул
		                          |""%1"".';
									|en = 'An additional information record with ID for formulas
									|""%1"" already exists.';");
	Else
		QueryText = NStr("ru = 'Существует дополнительный реквизит с идентификатором для формул
		                          |""%1"".';
									|en = 'An additional attribute with ID for formulas
									|""%1"" already exists.';");
	EndIf;
	
	QueryText = StringFunctionsClientServer.SubstituteParametersToString(
		QueryText + Chars.LF + Chars.LF
		                         + NStr("ru = 'Рекомендуется использовать другой идентификатор для формул,
		                         |иначе приложение может работать некорректно.';
								|en = 'It is recommended to use another ID for formulas.
		...
		If Selection.ValueType <> NULL
		   And ValueTypeContainsPropertyValues(Selection.ValueType) Then
			...
			If TopValues = Undefined Then 
				If HasVal = True Then
					ValuesPresentation = NStr("ru = 'Значения помечены на удаление';
												|en = 'Values are marked for deletion';");
				Else
					ValuesPresentation = NStr("ru = 'Значения еще не введены';
												|en = 'Values are not entered yet';");
				EndIf;
			Else
