...
Function DatesByCalendar(Val WorkScheduleCalendar, Val DateFrom, Val DaysArray, Val CalculateNextDateFromPrevious = False, RaiseException1 = True) Export
	...
	If Not ValueIsFilled(WorkScheduleCalendar) Then
		If RaiseException1 Then
			Raise NStr("ru = 'Не указан график работы или производственный календарь.';
	...
	If Selection.Count() < ShiftDays.Maximum Then
		If RaiseException1 Then
			Raise StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Производственный календарь ""%1"" не заполнен с даты %2 на указанное количество рабочих дней.';
					|en = 'Business calendar ""%1"" is not filled in for the specified number of workdays after %2.';"), 
...
Function DateDiffByCalendar(Val WorkScheduleCalendar, Val StartDate, Val EndDate, RaiseException1 = True) Export
	...
	If Not ValueIsFilled(WorkScheduleCalendar) Then
		If RaiseException1 Then
			Raise NStr("ru = 'Не указан производственный календарь.';
									|en = 'Business calendar is not specified.';");
		EndIf;
		Return Undefined;
	EndIf;
	...
	If Query.Execute().Unload().Count() <> Years.Count() Then
		If RaiseException1 Then
			ErrorMessage = NStr("ru = 'График работы ""%1"" не заполнен на период %2.';
									|en = 'The ""%1"" work schedule is blank for period: %2.';");
...
Function NearestWorkDates(BusinessCalendar, InitialDates, ReceivingParameters = Undefined) Export
	...
	CommonClientServer.Validate(
		...
		NStr("ru = 'Не указан график или производственный календарь.';
			|en = 'The schedule or business calendar is not specified.';"), 
		"CalendarSchedules.NearestWorkDates");
	...
	While Selection.Next() Do
		...
		ElsIf ReceivingParameters.RaiseException1 Then
			Raise StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Невозможно определить ближайшую рабочую дату для даты %1, 
					 |возможно, производственный календарь не заполнен.';
					|en = 'Cannot determine the workday nearest to %1. 
...
Function WorkSchedulesForPeriod(Schedules, StartDate, EndDate) Export
	...
	Raise NStr("ru = 'Подсистема ""Графики работы"" не обнаружена.';
							|en = 'The ""Work schedules"" subsystem is not found.';");
...
Procedure OnAddClassifiers(Classifiers) Export
	...
	LongDesc.Description = NStr("ru = 'Календари';
								|en = 'Calendars';");
...
Procedure CheckItemStart(Val XMLReader, Val Name)
	...
	If XMLReader.NodeType <> XMLNodeType.StartElement Or XMLReader.Name <> Name Then
		EventName = NStr("ru = 'Календарные графики.Обработка файла классификатора';
							|en = 'Calendar schedules.Process classifier file';", Common.DefaultLanguageCode());
		MessageText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Неверный формат файла данных. Ожидается начало элемента %1';
				|en = 'Invalid data file format. Start of ""%1"" element is expected.';"), 
			Name);
...
Procedure CheckItemEnd(Val XMLReader, Val Name)
	...
	If XMLReader.NodeType <> XMLNodeType.EndElement Or XMLReader.Name <> Name Then
		EventName = NStr("ru = 'Календарные графики.Обработка файла классификатора';
							|en = 'Calendar schedules.Process classifier file';", Common.DefaultLanguageCode());
		MessageText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Неверный формат файла данных. Ожидается конец элемента %1';
				|en = 'Invalid data file format. End of the ""%1"" element is expected.';"), 
			Name);
		WriteLogEvent(EventName, EventLogLevel.Error, , , MessageText);
		Raise MessageText;
	EndIf;
...
Procedure AddHandlerOfDataDependentOnBusinessCalendars(Handlers)
	...
	Handler.Comment = NStr("ru = 'Обновление графиков работы и др. данных, зависимых от производственных календарей.';
									|en = 'Updates work schedules and other data that depends on business calendars.';");
		...
		Else
			CommonClientServer.Validate(False,
				StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Недопустимый тип значения элемента в параметре %1:
					           |%2';
								|en = 'Invalid item value type in the %1 parameter:
