...
Function ValidateAddress(Address, AddressCheckParameters = Undefined) Export
	...
	If TypeOf(Address) <> Type("String") Then
		...
		CheckResult.ErrorList.Add("AddressFormat", NStr("ru = 'Некорректный формат адреса';
...
Procedure OnAddUpdateHandlers(Handlers) Export
	...
	Handler.Comment = NStr("ru = 'Обновление сведений о странах мирах в соответствии с общероссийским классификатором стран мира.
		...
		|en = 'Updates Countries details against the Country classifier.
...
Procedure OnDefineChecks(ChecksGroups, Checks) Export
	...
	ChecksGroup.Description                 = NStr("ru = 'Контактная информация';
	...
	Validation.Description                 = NStr("ru = 'Выявление некорректных настроек видов контактной информации';
												|en = 'Identifying incorrect contact information kind settings';");
	Validation.Reasons                      = NStr("ru = 'В карточке или документе отсутствуют поля контактной информации, или возникает ситуация, блокирующая работу с ними.';
												|en = 'There are no contact information fields in the card or in the document, or a situation arises that blocks operations with them.';");
	Validation.Recommendation                 = NStr("ru = 'Выполнить частичное автоматическое восстановление видов контактной информации (для этого нажать ссылку ниже).
	|
	...
	|en = 'Perform partial automatic restoration of contact information kinds (to do this, click the link below).
	|
...
Procedure CheckContactInformationKinds(Validation, CheckParameters) Export
	...
	CheckExecutionParameters = ModuleAccountingAudit.CheckExecutionParameters("ContactInformation", NStr("ru = 'Виды контактной информации';
																														|en = 'Contact information kinds';", Common.DefaultLanguageCode()));
	...
	While StringFromQuery.Next() Do
		...
		Issue1.IssueSummary = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Не заполнена группа определяющая владельца контактной информации';
				|en = 'The group that determines a contact information owner is required';"), StringFromQuery.Description);
...
Procedure UpdateExistingWorldCountries() Export
	...
	For Each ClassifierRow In ModuleAddressManager.TableOfClassifier() Do
		...
		Except
			RollbackTransaction();
			Info = ErrorInfo();
			ErrorText = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Ошибка записи страны мира %1 (код %2) при обновлении классификатора, %3';
																						|en = 'Error saving country %1 (code %2) while updating classifier, %3';"),
...
Procedure ContactInformationValidationProcessing(Source, Cancel, CheckedAttributes) Export
	...
	For Each ContactInformationKind In ContactInformationKinds Do
		...
		If FoundRows.Count() = 0 Then
			...
			Text = StringFunctionsClientServer.SubstituteParametersToString( NStr("ru = 'Поле ""%1"" не заполнено.';
																					|en = 'Please fill in the ""%1"" field.';"),
...
Function EventLogEvent() Export
	...
	Return NStr("ru = 'Контактная информация';
				|en = 'Contact information';", Common.DefaultLanguageCode());
	
EndFunction

Function ContactsByPresentation(Presentation, ExpectedKind, SplitByFields = False, RecognizeAddress = True) Export
...
Procedure UpdateAddressPresentation(Address, IncludeCountryInPresentation)
	...
	If TypeOf(Address) <> Type("Structure") Then
		Raise NStr("ru = 'Для формирования представления адреса передан некорректный тип адреса';
								|en = 'Cannot generate address. Invalid address type passed.';");
	EndIf;
...
Function CheckContactsKindParameters(ContactInformationKind) Export
	...
	If Not ValueIsFilled(ContactInformationKind.Description) Then
		...
		Result.ErrorText = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Не заполнено обязательное поле Наименование у вида контактной информации ""%1"".';
																							|en = 'Field ""Description"" of the ""%1"" contact information kind is empty. The field is required.';"),
	...
	If Not ValueIsFilled(ContactInformationKind.Type) Then
		Result.HasErrors = True;
		Result.ErrorText = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Не заполнено обязательное поле Тип у вида контактной информации ""%1"".';
																							|en = 'Field ""Type"" of the ""%1"" contact information kind is empty. The field is required.';"),
...
//   Replace - Boolean
// 
Procedure AddContactInformation(Object, ValueOrPresentation, ContactInformationKind, Val Date, Val Replace, RecognizeAddress = True) Export
	...
	If ObjectMetadata = Undefined
		...
		Raise NStr("ru = 'Добавление контактной информации невозможно, у объекта нет таблицы с контактной информацией.';
								|en = 'Cannot add contact information. The object does not have a contact information table.';");
	EndIf;
...
Function ContactInformationTypeFromRow(Val ContactInformationTypeAsString) Export
...
Function JSONStringToStructure1(Value) Export
	...
	Except
		ErrorText = NStr("ru = 'Ошибка конвертации контактной информации из формата JSON.';
							|en = 'An error occurred while converting contact information from JSON.';");
