...
Procedure SetAdditionalReportOrDataProcessor(Val Message, Val Sender)
	...
	Try
		...
		If ValueIsFilled(Body.Assignments) Then
			...
			AssignmentToSectionsType = AdditionalReportsAndDataProcessorsSaaSManifestInterface.AssignmentToSectionsType(ManifestPackage());
			AssignmentToCatalogsAndDocumentsType = AdditionalReportsAndDataProcessorsSaaSManifestInterface.AssignmentToCatalogsAndDocumentsType(ManifestPackage());
			For Each Assignment In Body.Assignments Do
				
				If Assignment.Type() = AssignmentToSectionsType Then
				...
				ElsIf Assignment.Type() = AssignmentToCatalogsAndDocumentsType Then
					
					For Each AssignmentObject In Assignment.Objects Do
						AssignmentRow1 = AssignmentSettings.Add();
						RelatedObject = Common.MetadataObjectID(AssignmentObject.ObjectName, False);
						AssignmentRow1.RelatedObject = ?(ValueIsFilled(RelatedObject), RelatedObject, Catalogs.MetadataObjectIDs.EmptyRef());
					EndDo;
