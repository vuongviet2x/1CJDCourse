		...
		ExceptionText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Приложение было перемещено.
			           |Регламентное задание ""%1"", работающее с внешними ресурсами, отключено.';
						|en = 'The application has been moved.
		...
		ExceptionText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Изменилась строка соединения информационной базы.
			           ...
			           |Регламентное задание ""%1"" отключено.';
						|en = 'The infobase connection string has changed.
...
Function SetExternalResourcesOperationsLock()
	...
	If DataSeparationChanged Then
		MessageText = NStr("ru = 'Информационная база была перемещена из приложения в Интернете.';
								|en = 'The infobase has been moved from a web application.';");
	...
	If MovedBetweenFileAndClientServerMode Then
		MessageText = 
			?(IsFileInfobase, 
				NStr("ru = 'Информационная база была перемещена из клиент-серверного режима работы в файловый.';
					|en = 'The infobase has been moved from the client/server mode to the file mode.';"),
				NStr("ru = 'Информационная база была перемещена из файловый режима работы в клиент-серверный.';
					|en = 'The infobase has been moved from the file mode to the client/server mode.';"));
		SetFlagShowsNecessityOfLock(LockParameters, MessageText);
		Return True;
	EndIf;
	...
	If IsFileInfobase Then
		...
		If Not FileInfobaseIDCheckFileExists() Then
			MessageText = NStr("ru = 'В папке информационной базы отсутствует файл проверки %1.';
									|en = 'The infobase folder does not contain check file %1.';");
		...
		If InfobaseIDChanged Then
			MessageText = 
				NStr("ru = 'Идентификатор информационной базы в файле проверки %1 не соответствует идентификатору в текущей базе.';
					|en = 'The infobase ID in check file %1 does not match ID in the current infobase.';");
			MessageText = StringFunctionsClientServer.SubstituteParametersToString(MessageText, "DoNotCopy.txt");
			SetFlagShowsNecessityOfLock(LockParameters, MessageText);
			Return True;
		EndIf;
...
Function LockReasonPresentation(LockParameters)
	...
	Return StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = 'Блокировка выполнена на сервере <b>%1</b> <b>%2</b> в <b>%3</b> %4.
		           |
		           |Размещение информационной базы изменилось с
		           |<b>%5</b>
		           |на 
		           |<b>%6</b>';
					|en = 'The lock was set on server <b>%1</b> on <b>%2</b> at <b>%3</b> %4.
					|
...
Function CurrentOperationPresentation()
	...
	If IsScheduledJobSession Then
		Return StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'при попытке выполнения регламентного задания <b>%1</b>';
				|en = 'on attempt to execute scheduled job <b>%1</b>';"),
			BackgroundJob.ScheduledJob.Description);
	EndIf;
	
	Return StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = 'при входе пользователя <b>%1</b>';
...
Function EventLogEventName() Export 
	...
	Return NStr("ru = 'Работа с внешними ресурсами заблокирована';
				|en = 'Online activities are disabled';", Common.DefaultLanguageCode());
