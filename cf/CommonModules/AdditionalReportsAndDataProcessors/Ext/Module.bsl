...
#EndIf
	...
	WriteComment(Ref,
		StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Подключение, %1 = ""%2"".';
																	|en = 'Attachment, %1 = ""%2"".';"), "SafeMode",
			AttachmentMode));
...
Procedure OnFillToDoList(ToDoList) Export
	...
	For Each Section In Sections Do
		...
		ToDoItem.Presentation = NStr("ru = 'Дополнительные отчеты и обработки';
...
Procedure OnFillAccessKinds(AccessKinds) Export
	...
	AccessKind.Presentation = NStr("ru = 'Дополнительные отчеты и обработки';
									|en = 'Additional reports and data processors';");
...
Procedure OnGetOtherSettings(UserInfo, Settings) Export
	...
	SettingName1 = NStr("ru = 'Настройки быстрого доступа к дополнительным отчетам и обработкам';
							|en = 'Settings for additional report and data processor quick access';");
...
Procedure OnDefineCommandsAttachedToObject(FormSettings, Sources, AttachedReportsAndDataProcessors, Commands) Export
	...
	If ObjectFillingTypes.Count() > 0 Then
		...
		If Common.SubsystemExists("StandardSubsystems.AttachableCommands") Then
			...
			Command.Presentation = NStr("ru = 'Дополнительные обработки заполнения...';
										|en = 'Object filling additional data processors…';");
			Command.Importance      = "SeeAlso";
		Else
			Command.Kind           = "CommandBar";
			Command.Presentation = NStr("ru = 'Заполнение...';
										|en = 'Filling…';");
		EndIf;
	...
	If ReportsTypes.Count() > 0 Then
		...
		If Common.SubsystemExists("StandardSubsystems.ReportsOptions") Then
			...
			Command.Importance      = "SeeAlso";
			Command.Presentation = NStr("ru = 'Дополнительные отчеты...';
										|en = 'Additional reports…';");
		Else
			Command.Kind           = "CommandBar";
			Command.Presentation = NStr("ru = 'Отчеты...';
										|en = 'Reports…';");
		EndIf;
		Command.Order            = 50;
		Command.Handler         = "AdditionalReportsAndDataProcessorsClient.OpenCommandList";
	...
	If RelatedObjectsCreationTypes.Count() > 0 Then
		...
		Else
			...
			Command.Presentation      = NStr("ru = 'Создание связанных объектов...';
												|en = 'Create related objects…';");
...
Procedure OnAttachAdditionalReport(Ref, ReportParameters, Result, GetMetadata) Export
	If Not GetFunctionalOption("UseAdditionalReportsAndDataProcessors") Then
		ReportParameters.ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Элемент ""%1"" не подключен, потому что ""Дополнительные отчеты и обработки"" отключены в настройках приложения.';
				|en = 'Cannot attach %1. Additional reports and data processors are disabled in the application settings.';"),
			"'" + String(Ref) + "'");
		Return;
	EndIf;
		...
		Except
			ReportParameters.ErrorText = 
				StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Не удалось подключить дополнительный отчет ""%1"" по причине:';
						|en = 'Cannot attach the ""%1"" additional report due to:';"), String(Ref))
	...
	Else
		...
		ReportParameters.ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Элемент %1 не является дополнительным отчетом';
				|en = '%1 is not an additional report.';"),
			"'"+ String(Ref) +"'");
...
Procedure OnAttachReport(Context) Export
	...
	If TypeOf(Ref) <> Type("CatalogRef.AdditionalReportsAndDataProcessors") Then
		Raise StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'В процедуру ""%1"" не передан отчет';
				|en = 'No report is passed to the ""%1"" procedure.';"),
			"AdditionalReportsAndDataProcessors.OnAttachReport");
	EndIf;
	
	If Not GetFunctionalOption("UseAdditionalReportsAndDataProcessors") Then
		Raise NStr("ru = '""Дополнительные отчеты и обработки"" отключена в настройках приложения.';
								|en = 'The ""Additional reports and data processors"" feature is disabled in the application settings.';");
	EndIf;
	...
	Else
		Raise StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Объект ""%1"" не является дополнительным отчетом';
				|en = '""%1"" is not an additional report.';"), String(Ref));
	EndIf;
		...
		For Each TableRow In FoundItems Do
			If Not SubsystemEnabled Then
				TableRow.Presentation = StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Отчет ""%1"" недоступен, т.к. дополнительные отчеты и обработки отключены в настройках приложения.';
						|en = 'The ""%1"" report is unavailable as additional reports and data processors are disabled in the application settings.';"),
			...
			ElsIf Not HasReadRight Then
				TableRow.Presentation = StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Отчет ""%1"" недоступен, т.к. отсутствует право чтения дополнительных отчетов и обработок.';
						|en = 'The ""%1"" report is unavailable as you do not have the right to read additional reports and processors.';"),
					TableRow.Presentation);
			ElsIf Not IsSuppliedDataProcessor(Report) Then
				TableRow.Presentation = StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Отчет ""%1"" недоступен в модели сервиса.';
						|en = 'The ""%1"" report is unavailable in SaaS mode.';"),
					TableRow.Presentation);
			Else
...
Procedure ExecuteDataProcessorByScheduledJob(ExternalDataProcessor, CommandID) Export
	...
	WriteInformation(ExternalDataProcessor, 
		StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Команда %1: Запуск.';
																	|en = 'Command %1: Start.';"), CommandID));
	...
	Except
		Refinement = CommonClientServer.ExceptionClarification(ErrorInfo(),
			StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Команда %1 не выполнена по причине:';
					|en = 'Couldn''t execute command %1 due to:';"), CommandID));
		Raise(Refinement.Text, Refinement.Category,,, ErrorInfo());
	EndTry;
	
	// Event log record.
	WriteInformation(ExternalDataProcessor, 
		StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Команда %1: Завершение.';
																	|en = 'Command %1: Complete.';"), CommandID));
...
Function SubsystemDescription()
	Return NStr("ru = 'Дополнительные отчеты и обработки';
				|en = 'Additional reports and data processors';", Common.DefaultLanguageCode());
EndFunction
...
Function ExecuteExternalObjectCommand(ExternalObject, CommandID, CommandParameters, ResultAddress)
	...
	If CommandDetails = Undefined Then
		Raise StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Команда %1 не обнаружена.';
																						|en = 'Command %1 is not found.';"), CommandID);
	EndIf;
...
Procedure ExportReportsAndDataProcessorsToFiles(ReportsAndDataProcessors)
	...
	If Not Upload0.Success Then
		ErrorText = TrimAll(
			NStr("ru = 'Не удалось выгрузить отчеты и обработки конфигурации во внешние файлы:';
				|en = 'Failed to export reports and configuration data processors to external files:';")
	...
	While ReverseIndex > 0 Do
		...
		Else
			WriteError(TableRow.Ref, 
				StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Неподдерживаемый вид объектов метаданных: ""1""';
						|en = 'Invalid metadata object kind: ""1""';"), TableRow.Kind));
			ReportsAndDataProcessors.Delete(ReverseIndex);
			Continue;
		EndIf;
		...
		If SchemaText = Undefined Then
			WriteError(TableRow.Ref, 
				StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Не обнаружен файл ""%1"".';
																			|en = 'Cannot find file: %1.';"), 
...
Function DesignerBatchRun(Parameters, PassedStartupCommands)
	...
	If Not ValueIsFilled(Parameters.ConfigurationPath) Then
		...
		If DesignerIsOpen() Then
			...
			Else
				Result.Brief1 = NStr("ru = 'Для выгрузки модулей закройте конфигуратор.';
	...
	Result.Brief1 = StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = 'Не удалось выгрузить конфигурацию в XML (код ошибки ""%1"")';
			|en = 'Cannot dump configuration to XML. Error code: %1.';"),
		ReturnCode);
		...
		If Not Result.Success Then
			If Conflicting.Count() <> 0 Then
				Result.ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Имя ""%1"" занято объектами ""%2""';
						|en = 'Name %1 is assigned to objects %2.';"),
					ObjectName,
					String(Conflicting));
			EndIf;
			WriteLogEvent(
		...
		For Each KeyAndValue In JobsSearch Do
			...
			Except
				WriteLogEvent(InfobaseUpdate.EventLogEvent(),
					...
					StringFunctionsClientServer.SubstituteParametersToString(
						NStr("ru = 'Регламентное задание ""%1"" не было удалено по причине:
							|%2';
							|en = 'Cannot delete the ""%1"" scheduled job due to:
							|%2';"),
						KeyAndValue.Value,
...
Function RegisterDataProcessor(Val Object, Val RegistrationParameters) Export
	...
	If RegistrationData.Kind = Enums.AdditionalReportsAndDataProcessorsKinds.PrintForm
		...
		Result.ErrorText = NStr("ru = 'Работа с печатными формами недоступна.';
									|en = 'Operations with print forms are unavailable.';");
		Return Result;
	EndIf;
	
	If Not RegistrationData.SafeMode And Not Users.IsFullUser(, True) Then
		Result.ErrorText = NStr("ru = 'Для подключения обработки, запускаемой в небезопасном режиме, требуется роль ""Администратор системы"".';
									|en = 'Cannot attach the data processor. Only users with the ""System administrator"" role can attach data processors that require disabling safe mode.';");
		Return Result;
	EndIf;
	...
	ElsIf RegistrationParameters.IsReport <> IsExternalReport Then
		Result.ErrorText = NStr("ru = 'Вид обработки, указанный в сведениях о внешней обработке, не соответствует ее расширению.';
									|en = 'The data processor type specified in the data processor details does not match the actual extension.';");
		Return Result;
	EndIf;
	...
	For Each DetailsCommand In RegistrationData.Commands Do
		...
		If Not ValueIsFilled(DetailsCommand.StartupOption) Then
			Common.MessageToUser(StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Для команды ""%1"" не определен способ запуска.';
					|en = 'Startup option is not specified for command %1.';"), DetailsCommand.Presentation));
		EndIf;
...
#If ThickClientOrdinaryApplication Then
	...
	Except
		WarningText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Не был удален временный файл ""%1"" по причине:
			|%2';
			|en = 'Cannot delete the ""%1"" temporary file due to:
			|%2';"),
			RegistrationResult.ObjectName,
			ErrorProcessing.DetailErrorDescription(ErrorInfo()));
...
#EndIf
	If ErrorInfo <> Undefined Then
		...
		Else
			...
			|It might not be compatible with this application version.';");
		EndIf;
		ErrorText = ErrorText + Chars.LF + Chars.LF + NStr("ru = 'Техническая информация:';
																	|en = 'Technical information:';") + Chars.LF;
