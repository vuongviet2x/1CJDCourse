...
Procedure ConfigureExchangeStep1(Message, Sender)
	...
	Try
		...
		If Not IsBlankString(ThisNodeCode)
			...
			If ThisNodeAlias <> Body.Code Then
				MessageString = StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Ожидаемый код предопределенного узла в этом приложении ""%1""
					|не соответствует фактическому ""%2"" или псевдониму ""%3"". План обмена: %4.';
					|en = 'Expected code of the predefined node in the %1
		...
		If CorrespondentEndpoint.IsEmpty() Then
			Raise StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Не найдена конечная точка корреспондента с кодом ""%1"".';
					|en = 'Endpoint with ID %1 not found.';"),
				Body.EndPoint);
		EndIf;
		...
		If Cancel Then
			Raise StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Возникли ошибки в процессе выгрузки справочников для корреспондента %1.';
					|en = 'Error exporting catalogs to peer infobase %1.';"),
				String(Peer));
		EndIf;
...
Procedure ConfigureExchangeStep2(Message, Sender)
	...
	Try
		...
		// Registering all infobase data for exporting, except for catalogs
...
Procedure ImportExchangeMessage(Message, Sender)
	...
	Try
		...
		If Cancel Then
			Raise StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Возникли ошибки в процессе загрузки справочников от корреспондента %1.';
					|en = 'Error importing catalogs from peer infobase %1.';"),
				String(Peer));
		EndIf;
...
Function ExchangeCorrespondent(Val ExchangePlanName, Val Code)
	...
	If Not ValueIsFilled(Result) Then
		MessageString = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Не найден узел плана обмена; имя плана обмена %1; код узла %2.';
				|en = 'Node not found. Exchange plan: %1. Node ID: %2.';"), ExchangePlanName, Code);
		Raise MessageString;
	EndIf;
