...
Procedure EnableObjectVersioning(ObjectName, Val VersioningMode = Undefined) Export
	...
	ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = 'Неизвестный вариант версионирования: ""%1""';
			|en = 'Unknown versioning option: %1.';"), VersioningMode);
...
//  Sender - ExchangePlanObject, ExchangePlanRef - SSL exchange plan.
//
...
Function ChangeHistoryCommand(Form) Export
	...
	Command.Title = NStr("ru = 'История изменений объекта';
							|en = 'Object change history';");
	Command.ToolTip = NStr("ru = 'История изменений объекта';
							|en = 'Object change history';");
...
Procedure OnReceiveDataFromSlave(DataElement, ItemReceive, SendBack, Sender) Export
	...
	Try
		...
		If ConflictVersionNumber = Undefined Then
			...
			Record.SynchronizationWarning = NStr("ru = 'Отклоненная версия (автоматическое разрешение конфликта).';
														|en = 'The rejected version (automatic conflict resolving).';", Common.DefaultLanguageCode());
			RecordSet.Write();
...
Procedure OnFillToDoList(ToDoList) Export
	...
	ToolTip = NStr("ru = 'Устаревших версий: %1 (%2)';
					|en = 'Obsolete versions: %1 (%2)';");
	...
	For Each Section In Sections Do
		...
		ToDoItem.Presentation = NStr("ru = 'Устаревшие версии объектов';
									|en = 'Obsolete object versions';");
		...
		If Not Object.IsNew() And (PostingChanged And ObjectVersionInfo.VersionNumber > 1 Or CurrentAndPreviousVersionMismatch(Object)) Then
			...
			If ObjectVersionInfo.VersionNumber = 1 Then
				If ObjectIsVersioned(Object.Ref) Then
					...
					VersionParameters.Insert("Comment", NStr("ru = 'Версия создана по уже имеющемуся объекту';
																|en = 'Version was created from existing object.';"));
...
Procedure CheckObjectEditRights(MetadataObject)
	...
	If Not PrivilegedMode() And Not AccessRight("Update", MetadataObject)Then
		MessageText = NStr("ru = 'Недостаточно прав на изменение ""%1"".';
								|en = 'Insufficient rights to modify %1.';");
...
Function ReportOnObjectVersion(ObjectReference, Val ObjectVersion = Undefined, CustomVersionNumber = Undefined) Export
	...
	LongDesc = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = '№ %1 / (%2) / %3';
																			|en = '#%1 / (%2) / %3';"), CustomVersionNumber,
...
Function RestoreObjectByXML(ObjectData, ErrorMessageText = "")
	...
	Except
		WriteLogEvent(NStr("ru = 'Версионирование';
										|en = 'Versioning';", Common.DefaultLanguageCode()),
...
Function ObjectVersionInfo(Val Ref, Val VersionNumber) Export
	...
	If Result.ObjectVersion = Undefined Then
		Raise NStr("ru = 'Выбранная версия объекта отсутствует в приложении.';
								|en = 'Selected object version is not available in the app.';");
	EndIf;
...
Function OutputParsedObjectAttributes(Result, ObjectVersion, ObjectReference)
	...
	OutputTextToReport(Result, Section3, "R1C2", NStr("ru = 'Реквизиты';
														|en = 'Attributes';"), StyleFonts.LargeTextFont);
...
Function RestoreVersionServer(Ref, VersionNumber, ErrorMessageText, UndoPosting = False) Export
	...
	Try
		...
		Object.AdditionalProperties.Insert("ObjectsVersioningVersionComment",
			StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Выполнен переход к версии №%1 от %2';
...
Procedure CreateRejectedItemsOwnerVersion(LastRejectedVersionNumber, ConflictVersionNumber, Object, VersionAuthor)
	...
	Record.VersionDate = CurrentSessionDate();
	Record.SynchronizationWarning = NStr("ru = 'Отклоненная версия (автоматическое разрешение конфликта).';
												|en = 'Rejected version (automatic conflict resolving).';");
	Record.VersionAuthor = VersionAuthor;
