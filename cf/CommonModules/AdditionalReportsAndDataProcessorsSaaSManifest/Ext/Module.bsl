...
Function GenerateManifest(Val DataProcessorObject2, Val VersionObject1, Val ReportOptions = Undefined, 
	...
	If Not ValueIsFilled(XDTOKind) Then
		Raise StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Вид дополнительных отчетов и обработок %1 не поддерживается в модели сервиса.';
				|en = 'Additional report (data processor) type %1 is not supported in SaaS.';"),
			VersionObject1.Kind);
	EndIf;
	...
	If VersionObject1.Commands.Count() > 0 Then
		...
		For Each CommandDetails In VersionObject1.Commands Do
			...
			If Not ValueIsFilled(XDTOStartType) Then
				Raise StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Способ запуска дополнительных отчетов и обработок %1 не поддерживается в модели сервиса.';
						|en = 'Launch method of additional reports and data processors %1 is not supported in SaaS.';"),
