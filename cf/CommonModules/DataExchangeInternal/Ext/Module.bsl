...
Function ExportFilePart(FileID, FilePartToExportNumber, ErrorMessage) Export
	...
	Else
		ErrorMessage = NStr("ru = 'Часть файла с указанным номером не найдена.';
								|en = 'The file part with the ID is not found.';");
	EndIf;
...
Function PrepareFileForImport(FileID, ErrorMessage) Export
	...
	Else
		MessageTemplate = NStr("ru = 'Не найден ни один фрагмент сессии передачи с идентификатором %1.
							...
							|en = 'No parts of the transfer session with the %1 ID are found.
	...
	If Dearchiver.Items.Count() = 0 Then
		...
		ErrorMessage = NStr("ru = 'Файл архива не содержит данных.';
...
Procedure ImportXDTODateToInfobase(ProcedureParameters, StorageAddress) Export
	...
	If Not Cancel Then
		...
		Except
			...
			ErrorMessage = NStr("ru = 'Ошибка при загрузке данных: %1';
		...
		Except
			DisableAccessKeysUpdate(False);
			Information = ErrorInfo();
			ErrorMessage = NStr("ru = 'Ошибка при удалении временных объектов, созданных по ссылкам: %1';
									|en = 'Cannot delete temporary objects created by references: %1';");
			ErrorMessage = StringFunctionsClientServer.SubstituteParametersToString(
				ErrorMessage, ErrorProcessing.DetailErrorDescription(Information));
			DataExchangeXDTOServer.WriteToExecutionProtocol(ExchangeComponents, ErrorMessage, , , , , True);
			ExchangeComponents.FlagErrors = True;
...
Procedure CheckInfobaseLockForUpdate() Export
	...
	If ValueIsFilled(InfobaseUpdateInternal.InfobaseLockedForUpdate()) Then
		...
		Raise NStr("ru = 'Синхронизация данных временно недоступна в связи с обновлением приложения.';
								|en = 'Data synchronization is temporarily unavailable due to the application update.';");
...
Function GenerateEventLogMessageKey(ExchangeDirection, WebServiceParameters)
	...
	If ExchangeDirection = "Receive" Then
		MessageKeyTemplate = NStr("ru = 'Загрузка данных через Web-сервис %1';
									|en = 'Import data over web service %1';", Common.DefaultLanguageCode());
	Else
		MessageKeyTemplate = NStr("ru = 'Выгрузка данных через Web-сервис %1';
									|en = 'Export data over web service %1';", Common.DefaultLanguageCode());
	EndIf;
