...
Function CurrentBackupSetting() Export
	...
	If BackupSettings1 = Undefined Then
		Return NStr("ru = 'Для настройки резервного копирования обратитесь к администратору.';
					|en = 'To set up backup, contact your administrator.';");
	EndIf;
	
	CurrentSetting = NStr("ru = 'Резервное копирование не настроено, информационная база подвергается риску потери данных.';
							|en = 'Backup is not set up. Configure backup settings to prevent data loss.';");
	...
	If Common.FileInfobase() Then
		...
		If BackupSettings1.CreateBackupAutomatically Then
			...
			If BackupSettings1.ExecutionOption = "OnExit" Then
				CurrentSetting = NStr("ru = 'Резервное копирование выполняется регулярно при завершении работы.';
										|en = 'Backups are created automatically on exit.';");
			ElsIf BackupSettings1.ExecutionOption = "Schedule3" Then // On schedule.
				...
				If Not IsBlankString(Schedule) Then
					CurrentSetting = NStr("ru = 'Резервное копирование выполняется регулярно по расписанию: %1';
											|en = 'Backups are created on schedule: %1.';");
		...
		Else
			...
			If BackupSettings1.BackupConfigured Then
				CurrentSetting = NStr("ru = 'Резервное копирование не выполняется (организовано сторонними приложениями).';
										|en = 'Backups are created by third-party backup tools.';");
			EndIf;
	...
	Else
		...
		CurrentSetting = NStr("ru = 'Резервное копирование не выполняется (организовано средствами СУБД).';
								|en = 'Backups are created by the means of DBMS.';");
...
Procedure OnFillToDoList(ToDoList) Export
	...
	For Each Section In Sections Do
		...
		If Not DisabledNotificationOfBackupSettings Then
			...
			ToDoItem.Presentation  = NStr("ru = 'Настроить резервное копирование';
										|en = 'Set up backup';");
		...
		If Not DisabledNotificationOfBackupExecution Then
			...
			ToDoItem.Presentation  = NStr("ru = 'Резервное копирование не выполнено';
										|en = 'Backup required';");
			ToDoItem.Important         = True;
...
Function ScriptMessages()
	...
	Messages["[MessageLaunchResult]"] = NStr("ru = 'Код возврата: {0}';
													|en = 'Return code: {0}';");
	Messages["[StartupFailureMessage]"] = NStr("ru = 'Запускаемый файл не существует: {0}';
												|en = 'The executable file does not exist: {0}';");
	Messages["[MessageLogging1S]"] = NStr("ru = 'Исключение при записи журнала: {0}, {1}';
												|en = 'Exception when writing Event log: {0}, {1}';");
	Messages["[TheMessageIsThePathToTheScriptFile]"] = NStr("ru = 'Файл скрипта: {0}';
													|en = 'Script file: {0}';");
	Messages["[MessagePathToTheBackupFile]"] = NStr("ru = 'Файл резервной копии: {0}';
															|en = 'Backup file: {0}';");
	Messages["[TheMessageIsTheResultOfCreatingABackupCopyOfTheDatabase]"] = NStr("ru = 'Резервная копия успешно создана';
																		|en = 'Backed up';");
	Messages["[TheMessageFailureToCreateABackupCopyOfTheDatabase]"] = NStr("ru = 'Не удалось создать резервную копию базы';
																	|en = 'Cannot back up the infobase';");
	...
	Messages["[MessageOSBitnessUndefined]"] = NStr("ru = '<Неопределена>';
															|en = '<Undefined>';");
	Messages["[MessageCOMConnectorVersion]"] = StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = 'Версия %1: {0} {1}';
			|en = 'Version %1: {0} {1}';"), "comcntr.dll");
	Messages["[TheMessageConnectionFailureWithTheDatabase]"] = NStr("ru = 'Исключение при создании COM-соединения на шаге: {0}, {1}, {2}';
														|en = 'Exception during COM connection creation at the step: {0}, {1}, {2}';");
	Messages["[TheMessageLoggingFailure1S]"] = NStr("ru = 'Исключение при записи в журнал регистрации: {0}, {1}';
														|en = 'Exception when writing data to Event log: {0}, {1}';");
	Messages["[TheMessageFailureWhenCallingCompleteBackup]"] = StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = 'Исключение при вызове %1: {0}, {1}';
			|en = 'Exception when calling %1: {0}, {1}';"), "IBBackupServer.FinishBackup");
	Messages["[TheMessageDatabaseBackupResult]"] = NStr("ru = 'Резервное копирование базы выполнено успешно.';
																		|en = 'Infobase is backed up.';");
	Messages["[TheMessageDatabaseBackupFailure]"] = NStr("ru = 'Непредвиденная ситуация при резервном копировании информационной базы.';
																	|en = 'An unexpected error occurred while backing up the infobase.';");
	Messages["[TheMessageDatabaseParameters]"] = NStr("ru = 'Параметры информационной базы: {0}.';
												|en = 'Infobase parameters: {0}.';");
	Messages["[MessageBackupLogging1S]"] = NStr("ru = 'Протокол резервного копирования сохранен в журнал регистрации.';
																	|en = 'The backup protocol is saved to the event log.';");
	Messages["[LoggingFailureMessage]"] = NStr("ru = 'Исключение при записи в журнал: {0}, {1}';
													|en = 'Exception when writing data to Event log: {0}, {1}';");
	Messages["[MessageBackupFileSizeInMb]"] = StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = 'Размер файла базы %1: {0} Мб';
			|en = 'Infobase ""%1"" file size: {0} MB';"), "1Cv8.1CD");
	Messages["[TheMessageFailedToCompressTheBackupFileINZIP]"] = StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = 'Размер файла базы %1 превышает 2 Гб, поэтому сжатие в zip не используется';
			|en = 'Infobase ""%1"" file size exceeds 2 GB. It will not be compressed to a ZIP archive.';"), "1Cv8.1CD");
	Messages["[TheMessageTheBeginningOfCreatingABackupCopyOfTheDatabase]"] = NStr("ru = 'Начато создание резервной копии...';
																	|en = 'Backup started…';");
	...
	Messages["[TheMessageAssumptionOfADatabaseBackupError]"] = NStr("ru = 'За 15 минут файл резервной копии все еще не начал создаваться (размер {0} байт), предположительно, из-за возникшей ошибки. Резервное копирование прервано.';
																				|en = 'After 15 minutes of runtime, no backup file has been created (size is {0} byte). Probably, an error has occurred. Back up is canceled.';");
	Messages["[MessageErrorDeletingTheLockFile]"] = StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = 'Не был удален файл блокировок %1: {0}, {1}';
			|en = 'The ""%1"" lock file is not deleted: {0}, {1}';"), "1Cv8.CDN");
	...
	Messages["[TheMessageFailureWhenCallingCompleteRecovery]"] = StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = 'Исключение при вызове %1: {0}, {1}';
			|en = 'Exception when calling %1: {0}, {1}';"), "IBBackupServer.CompleteRestore");
	Messages["[TheMessageDatabaseRecoveryResult]"] = NStr("ru = 'Восстановление информационной базы прошло успешно.';
																|en = 'Infobase is restored.';");
	Messages["[DatabaseRecoveryFailureMessage]"] = NStr("ru = 'Непредвиденная ситуация при восстановлении информационной базы.';
															|en = 'An unexpected error occurred while restoring the infobase.';");
	Messages["[MessageRecoveryLogging1S]"] = NStr("ru = 'Протокол восстановления сохранен в журнал регистрации.';
																|en = 'The restore protocol is saved to the event log.';");
	Messages["[TheMessageFailureToTransferTheDatabaseFileToATemporaryDirectory]"] = NStr("ru = 'Файл базы не был перенесен во временный каталог (возможно в приложении есть активные сеансы): {0}, {1}.';
																			|en = 'The infobase file is not transferred to a temporary directory. The app might have active sessions: {0}, {1}.';");
	Messages["[MessageAttemptToTransferADatabaseFileToATemporaryDirectory]"] = NStr("ru = 'Попытка переноса файла базы во временный каталог ({0} из 5): {1}, {2}.';
																				|en = 'Attempting to transfer an infobase file to a temporary directory ({0} out of 5): {1}, {2}.';");
...
Procedure SetTheGeneralParametersOfTheScreenSaver(Parameters) Export 
	...
	Parameters["[ProductName]"] = NStr("ru = '1С:ПРЕДПРИЯТИЕ 8.3';
