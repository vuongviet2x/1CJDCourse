...
Procedure OnSendFilesViaEmail(SendOptions, FilesToSend, FilesOwner, UUID) Export
	...
	// Adding attachments to the message text.
	
	ListOfFiles = New Array;
	For Each FileToSend In FilesToSend Do
		ListOfFiles.Add(FileToSend.Description + "." + FileToSend.Extension);
	EndDo;
	FilesListPresentation = StrConcat(ListOfFiles, Chars.LF);
	
	LetterTemplate = "
	|%1:
	|%2";
	TextAboutFiles = NStr("ru = 'Список приложенных файлов';
						|en = 'Attachments';");
	
	SendOptions.Text = StringFunctionsClientServer.SubstituteParametersToString(LetterTemplate, TextAboutFiles, FilesListPresentation);
