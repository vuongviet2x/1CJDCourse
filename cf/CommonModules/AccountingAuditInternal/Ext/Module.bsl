...
Procedure OnAddUpdateHandlers(Handlers) Export
	...
	Handler.Comment     = NStr("ru = 'Обновляются правила проверки учета в соответствие с изменениями в новой версии приложения.
		|До завершения обработки часть проверок ведения учета будет недоступна.';
		|en = 'Updating data integrity checks to reflect the changes in the new version of the application.
	...
	Handler.Comment                         = StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = 'Начальное заполнение признака %1 и расчет контрольных сумм в регистре сведений ""Результаты проверки учета"" для повышения производительности.';
			|en = 'Initial setting of the %1 flag and calculation of checksums in the ""Data integrity check results"" information register for better performance.';"),
		"IgnoreIssue");
	...
	Handler.ExecutionPriorities                = InfobaseUpdate.HandlerExecutionPriorities();
	Handler.CheckProcedure                   = "InfobaseUpdate.DataUpdatedForNewApplicationVersion";
	Handler.Comment                         = NStr("ru = 'Отключение автоматического запуска системных проверок в подсистеме контроля ведения учета.';
															|en = 'Disabling automatic start of system checks in the ""Data integrity"" subsystem.';");
...
Procedure OnFillToDoList(ToDoList) Export
	...
	For Each Section In Sections Do
		...
		If FullUser Then
			...
			ToDoItem.Presentation  = NStr("ru = 'Проблемы ведения учета';
		...
		ToDoItem.Presentation  = NStr("ru = 'Некорректные данные';
									|en = 'Invalid data';");
		...
		If ValueIsFilled(LastCheckInformation.LastCheckDate) Then
			ToolTip = NStr("ru = 'Последняя проверка выполнялась %1.';
							|en = 'Last checked on %1.';");
		...
		ToDoItem.Important         = False;
		ToDoItem.Owner       = Section;
		ToDoItem.Presentation  = NStr("ru = 'Проверка ведения учета давно не выполнялась';
									|en = 'Data integrity has not been checked for a while';");
...
Procedure WriteIssue(CheckError, CheckParameters = Undefined) Export
	...
	If Not ValueIsFilled(CheckError.CheckKind) Then
		Raise StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'При записи проблемы по проверке ""%1"" не указан вид проверки.';
				|en = 'Check kind is not specified when saving the issue for the ""%1"" check.';"), 
...
Procedure AddChecksGroups(ChecksGroups)
	...
	ChecksInfoRecords = DataIntegrityChecksInfoRecords(ChecksGroups);
	
	For Each ChecksGroup In ChecksInfoRecords Do
		
		ChecksGroupByID = ChecksGroup.Validation;
		
		If Not ValueIsFilled(ChecksGroupByID) Then
			ChecksGroupObject = Catalogs.AccountingCheckRules.CreateFolder();
		Else
			
			If ChecksGroup.AccountingCheckIsChanged Then
				Continue;
			EndIf;
		...
		CheckGroupParent        = ChecksGroup.CheckParent1;
		ChecksGroupObject.Parent = CheckGroupParent;
		
		If ValueIsFilled(CheckGroupParent) Then
			ChecksGroupObject.AccountingChecksContext = ChecksGroup.DataIntegrityCheckParentContext;
		Else
...
Procedure AddChecks(Checks)
	...
	ChecksInfoRecords = DataIntegrityChecksInfoRecords(Checks);
	
	For Each CheckInfoRecords In ChecksInfoRecords Do
		
		If Not ValueIsFilled(CheckInfoRecords.Validation) Then
		...
		Else
			...
			If CheckInfoRecords.AccountingCheckIsChanged Then
				Continue;
			EndIf;
			
			CheckObject1 = CheckInfoRecords.Validation.GetObject();
			If CheckObject1.DeletionMark Then
				CheckObject1.SetDeletionMark(False);
			EndIf;
			
		EndIf;
		
		FillPropertyValues(CheckObject1, CheckInfoRecords);
		
		CheckParent1        = CheckInfoRecords.CheckParent1;
		CheckObject1.Parent = CheckParent1;
		
		If ValueIsFilled(CheckParent1) Then
			CheckObject1.AccountingChecksContext = CheckInfoRecords.DataIntegrityCheckParentContext;
		Else
			CheckObject1.AccountingChecksContext = CheckInfoRecords.AccountingChecksContext;
		EndIf;
		
		CheckObject1.Use = Not CheckInfoRecords.isDisabled;
		
		If ValueIsFilled(CheckInfoRecords.IssuesLimit) Then
			CheckObject1.IssuesLimit = CheckInfoRecords.IssuesLimit;
		Else
			CheckObject1.IssuesLimit = 1000;
		EndIf;
		
		If ValueIsFilled(CheckInfoRecords.CheckStartDate) Then
...
Procedure SpecifiedItemsUniquenessCheck(ChecksGroups, Checks)
	...
	While Result.Next() Do
		...
		ExceptionText = ExceptionText + ?(ValueIsFilled(ExceptionText), Chars.LF, "")
			+ StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Дублирующийся идентификатор: ""%1""';
																			|en = 'Duplicate ID: ""%1""';"), Result.Id);
	...
	If ValueIsFilled(ExceptionText) Then
		Raise StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'В процедуре %1 у следующих проверок совпадают идентификаторы:
		|%2';
		|en = 'In %1 procedure, the following checks have identical IDs:
...
Procedure CheckUnfilledRequiredAttributes(Validation, CheckParameters) Export
	...
	If CheckExecutionParameters <> Undefined Then
		If CheckExecutionParameters.ValidationArea = "Registers" Then
			...
			MetadataObject = CheckExecutionParameters.MetadataObject; // MetadataObjectAccumulationRegister, MetadataObjectInformationRegister, MetadataObjectCalculationRegister, MetadataObjectAccountingRegister 
...
Procedure CheckPredefinedExchangePlanNodeAvailability(Validation, CheckParameters) Export
	...
	For Each MetadataExchangePlan In MetadataExchangePlans Do
		...
		Issue1.IssueSummary = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'В плане обмена ""%1"" отсутствует предопределенный узел (%2 = Неопределено).';
																									|en = 'Predefined node is missing from exchange plan ""%1"" (%2 = Undefined).';"), MetadataExchangePlan.Name, "ThisNode()");
...
Function PlaceErrorIndicatorGroup(Form, NamesUniqueKey, GroupParentName = Undefined, OutputAtBottom = False) Export
	...
	Else
		...
		If GroupParent = Undefined Then
			Raise StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Не существует указанная группа формы: ""%1""';
																							|en = 'The specified form group ""%1"" does not exist';"), GroupParentName);
		EndIf;
...
Procedure FillErrorIndicatorGroup(Form, ErrorIndicatorGroup, NamesUniqueKey,
	...
	ToolTip = NStr("ru = '(По состоянию на %1)';
					|en = '(As of %1)';");
...
Function GenerateCommonStringIndicator(Form, ObjectReference, Settings) Export
	...
	Else
		NoteLabel = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'В этом %1';
																						|en = 'In this %1';"), ObjectPresentationByType(ObjectReference));
	EndIf;
	
	If IssuesIndicatorHyperlink <> Undefined Then
		Hyperlink = IssuesIndicatorHyperlink;
	Else
		Hyperlink = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'найдены проблемы (%1)';
																					|en = 'issues found (%1)';"), Format(IssuesCountByObject, "NG=0"));
	EndIf;
...
Procedure ExecuteCheck(Validation, CheckExecutionParameters = Undefined, ObjectsToCheck = Undefined) Export
	...
	If SessionParameters.AccountingIssuesCounter > 0
		...
		Comment = NStr("ru = 'При выполнении проверки ""%1"" обнаружены ошибки (%2)';
							|en = 'Errors (%2) occurred while checking ""%1""';");
		...
		WriteLogEvent(NStr("ru = 'Контроль ведения учета';
										|en = 'Data integrity';", Common.DefaultLanguageCode()),
			EventLogLevel.Error,
			,
			,
			Comment);
...
Function DataIntegrityChecksInfoRecords(Checks)
	
	ColumnsNames = New Array;
	For Each Column In Checks.Columns Do
		If Column.Name = "Id"
			Or Column.Name = "GroupID" Then
			Continue;
		EndIf;
		ColumnsNames.Add("Checks" + "." + Column.Name);
	EndDo;
	
	Query = New Query(
	"SELECT
	|	Checks.Id AS Id,
	|	Checks.GroupID AS GroupID,
	|	&ColumnsToSelect
	|INTO VTChecks
	|FROM
	|	&Checks AS Checks
	...
	|	Checks.GroupID AS GroupID,
	|	&ColumnsToSelect,
	|	ISNULL(AccountingChecks_.Ref, VALUE(Catalog.AccountingCheckRules.EmptyRef)) AS Validation,
	|	AccountingChecks_.AccountingCheckIsChanged AS AccountingCheckIsChanged,
	|	DataIntegrityCheckParents.Ref AS CheckParent1,
	|	DataIntegrityCheckParents.AccountingChecksContext AS DataIntegrityCheckParentContext
	|FROM
	|	VTChecks AS Checks
	|		LEFT JOIN Catalog.AccountingCheckRules AS AccountingChecks_
	|		ON Checks.Id = AccountingChecks_.Id
	|			AND (NOT AccountingChecks_.DeletionMark)
	|		LEFT JOIN Catalog.AccountingCheckRules AS DataIntegrityCheckParents
	|		ON Checks.GroupID = DataIntegrityCheckParents.Id
	|			AND (NOT DataIntegrityCheckParents.DeletionMark)");
	Query.Text = StrReplace(Query.Text, "&ColumnsToSelect", StrConcat(ColumnsNames, ","));
	...
	For IndexOf = 2 To PropertiesCount Do
		...
		Else
			If LastValuableParameterFound Then
				MessageText = NStr("ru = 'Параметры выполнения проверки заданы не по порядку в %1.';
										|en = 'The check parameters in %1 are not in correct order.';");
	...
	While Result.Count() > 0 Do
		...
		For Each ResultString1 In Result Do
				...
				If IsDeadRef(RefToCheck, CheckedRefs) Then
					IssueSummary = IssueSummary + ?(ValueIsFilled(IssueSummary), Chars.LF, "")
						+ StringFunctionsClientServer.SubstituteParametersToString(
							NStr("ru = 'У объекта ""%1"" в реквизите ""%2"" указана ссылка на несуществующий элемент: ""%3"" (%4).';
								|en = 'The ""%2"" attribute of the ""%1"" object references an item that does not exist: ""%3"" (%4).';"), 
							ObjectReference, 
					...
					For Each TSRow In ObjectTabularSection Do
						For Each CurrentColumn In ObjectTabularSection.Columns Do
							...
							If IsDeadRef(DataToCheck1, CheckedRefs) Then
								...
								IssueSummary = IssueSummary + ?(ValueIsFilled(IssueSummary), Chars.LF, "")
									+ StringFunctionsClientServer.SubstituteParametersToString(
										NStr("ru = 'У объекта ""%1"" в реквизите ""%2"" табличной части ""%3"" (строка № %4) указана ссылка на несуществующий элемент: ""%5"" (%6).';
											|en = 'The ""%2"" attribute of the ""%3"" table (row #%4) in the ""%1"" object references an item that does not exist: ""%5"" (%6).';"),
			...
			If MoreIssues > 0 Then
				IssueSummary = IssueSummary + Chars.LF + NStr("ru = 'и еще проблем';
																			|en = 'More issues';") + ": " + String(MoreIssues);
			EndIf;
			...
			Issue1.IssueSummary = NStr("ru = 'Нарушена ссылочная целостность:';
												|en = 'Reference integrity violation:';") + Chars.LF + IssueSummary;
...
Procedure FindDeadRefsInAccumulationRegisters(MetadataObject, CheckParameters, CheckedRefs)
	...
	While Result.Count() > 0 Do
		...
		For Each ResultString1 In Result Do
			...
			CurrentRecordSet = RegisterManager.CreateRecordSet(); // AccumulationRegisterRecordSet
			...
			Issue1.IssueSummary = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'У регистра накопления ""%1"" в записях с номерами %2 по регистратору ""%3"" указаны ссылки на несуществующие данные.';
					|en = 'Records ""%2"" in recorder ""%3"" for accumulation register ""%1"" reference data that does not exist.';"),
...
Procedure FindDeadRefsInSubordinateInformationRegisters(MetadataObject, CheckParameters, CheckedRefs)
	...
	While Result.Count() > 0 Do
		...
		For Each ResultString1 In Result Do
			...
			Issue1 = IssueDetails(Common.MetadataObjectID(MetadataObject), CheckParameters); // @skip-check query-in-loop - A batch-wise data integrity check
			Issue1.IssueSummary = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'У регистра сведений ""%1"" в записях с номерами %2 по регистратору ""%3"" указаны ссылки на несуществующие данные.';
					|en = 'Records ""%2"" in recorder ""%3"" for information register ""%1"" reference data that does not exist.';"),
				MetadataObject.Presentation(), ProblemRecordsNumbers, ResultString1.RecorderAttributeRef);
	...
	While Result.Count() > 0 Do
		...
		For Each ResultString1 In Result Do
			...
			For Each AttributeInformation In RegisterInformation Do
				...
				Issue1 = IssueDetails(Common.MetadataObjectID(MetadataObject), CheckParameters); // @skip-check query-in-loop - A batch-wise data integrity check
				Issue1.IssueSummary = StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'У регистра сведений ""%1"" в %2, по комбинации измерений ""%3"" указана ссылка на несуществующий элемент: ""%4"" (%5).';
						|en = 'The ""%1"" information register in %2, the ""%3"" combination of dimensions, references an item that does not exist: ""%4"" (%5).';"),
...
Procedure FindDeadRefsInAccountingRegisters(MetadataObject, CheckParameters, ExtDimensionTypes, CheckedRefs)
	...
	While Result.Count() > 0 Do
		...
		For Each ResultString1 In Result Do
			...
			CurrentRecordSet = RegisterManager.CreateRecordSet(); // AccountingRegisterRecordSet
			CurrentRecordSet.Filter.Recorder.Set(ResultString1.RecorderAttributeRef);
			CurrentRecordSet.Read();
			...
			Issue1 = IssueDetails(Common.MetadataObjectID(MetadataObject), CheckParameters); // @skip-check query-in-loop - A batch-wise data integrity check
			Issue1.IssueSummary = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'У регистра бухгалтерии ""%1"" в записях с номерами %2 по регистратору ""%3"" указаны ссылки на несуществующие данные.';
					|en = 'Records ""%2"" in recorder ""%3"" for accounting register ""%1"" reference data that does not exist.';"),
				MetadataObject.Presentation(), ProblemRecordsNumbers, ResultString1.RecorderAttributeRef);
...
Procedure FindDeadRefsInCalculationRegisters(MetadataObject, CheckParameters, CheckedRefs)
	...
	While Result.Count() > 0 Do
		...
		For Each ResultString1 In Result Do
			...
			CurrentRecordSet = RegisterManager.CreateRecordSet(); // CalculationRegisterRecordSet
			CurrentRecordSet.Filter.Recorder.Set(ResultString1.RecorderAttributeRef);
			CurrentRecordSet.Read();
			...
			Issue1.IssueSummary = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'У регистра расчета ""%1"" в записях с номерами %2 по регистратору ""%3"" указаны ссылки на несуществующие данные.';
					|en = 'Records ""%2"" in recorder ""%3"" for calculation register ""%1"" reference data that does not exist.';"),
				MetadataObject.Presentation(), ProblemRecordsNumbers, ResultString1.RecorderAttributeRef);
...
Function IndependentRegisterInformation(MetadataObject, GetDimensions = True, GetResources = True, GetAttributes = True)
		...
		For Each Dimension In Dimensions Do
			...
			FillPropertyValues(RegisterInformation.Add(),
				New Structure("MetadataTypeInNominativeCase, MetadataTypeInInstrumentalCase, NameOfMetadataObjects", 
					"Dimension", NStr("ru = 'измерении';
										|en = 'dimension';"), DimensionName));
		EndDo;
	EndIf;
	...
	If GetResources Then
		...
		For Each Resource In Resources Do
			...
			FillPropertyValues(RegisterInformation.Add(),
				New Structure("MetadataTypeInNominativeCase, MetadataTypeInInstrumentalCase, NameOfMetadataObjects", 
					"Resource", NStr("ru = 'ресурсе';
									|en = 'resource';"), ResourceName));
		EndDo;
	EndIf;
	...
	If GetAttributes Then
		...
		For Each Attribute In Attributes Do
			...
			FillPropertyValues(RegisterInformation.Add(),
				New Structure("MetadataTypeInNominativeCase, MetadataTypeInInstrumentalCase, NameOfMetadataObjects", 
					"Attribute", NStr("ru = 'реквизите';
									|en = 'attribute';"), AttributeName));
		EndDo;
	EndIf;
...
Procedure FindNotFilledRequiredAttributes(MetadataObject, CheckParameters)
	...
	While Result.Count() > 0 Do
		...
		For Each ResultString1 In Result Do
			...
			Issue1.IssueSummary = NStr("ru = 'Не заполнены реквизиты, обязательные к заполнению:';
												|en = 'Attributes required:';") + Chars.LF + ObjectFillingErrors();
...
Procedure FindNotFilledRequiredAttributesInSubordinatePeriodicalRegisters(MetadataObject, CheckParameters)
	...
	While Result.Count() > 0 Do
		...
		For Each ResultString1 In Result Do
			...
			Issue1.IssueSummary = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'У записи с полями:
				|%1,
				|обнаружены незаполненные данные, обязательные к заполнению: %2';
				|en = 'Error in record with the following fields:
				|%1
				|Some values are required: %2';"),
				" • " + NStr("ru = 'Регистратор:';
							|en = 'Recorder:';") + " = """ + ResultString1.RecorderAttributeRef, Chars.LF + ObjectFillingErrors());
			Issue1.AdditionalInformation = New ValueStorage(AdditionalInformation);
	...
	While Result.Count() > 0 Do
		...
		For Each ResultString1 In Result Do
			...
			RecordSetFilterPresentation = StringFunctionsClientServer.SubstituteParametersToString(" • %1 = ""%2""",
				NStr("ru = 'Период';
					|en = 'Period';"), ResultString1.Period);
			For Each Dimension In Dimensions Do
...
Procedure FindMissingPredefinedItems(MetadataObject, CheckParameters)
	...
	For Each PredefinedItem In PredefinedItems Do
		...
		Issue1 = IssueDetails(Common.MetadataObjectID(MetadataObject), CheckParameters); // @skip-check query-in-loop - A batch-wise data integrity check
		Issue1.IssueSummary = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Предопределенный элемент ""%1"" отсутствует в информационной базе.';
																									|en = 'Predefined item ""%1"" missing in information database.';"), PredefinedItem);
		WriteIssue(Issue1, CheckParameters); // @skip-check query-in-loop - A batch-wise data integrity check
...
Procedure FindPredefinedItemsDuplicates(MetadataKind, CheckParameters)
	...
	While Result.Next() Do
		...
		While ResultByPredefinedItemName.Next() Do
			...
			IssueSummary = IssueSummary + ?(ValueIsFilled(IssueSummary), Chars.LF, "")
				+ NStr("ru = 'Имя предопределенного элемента:';
						|en = 'Name of the predefined item:';") + " """ + PredefinedDataName + """"
				+ Chars.LF + NStr("ru = 'Ссылки на предопределенный элемент:';
									|en = 'References to the predefined item:';");
