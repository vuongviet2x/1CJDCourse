...
Procedure SendSMS(SendOptions, Result) Export
	...
	If SendOptions.Provider = Enums.SMSProviders._DemoAnotherProvider Then
		// Send a text message via "_DemoAnotherProvider".
		// …
		
		// Generate a send result.
		For Each RecipientNumber In SendOptions.RecipientsNumbers Do
			Result.SentMessages.Add(New Structure("RecipientNumber,MessageID", RecipientNumber, "1234567890"));
		EndDo;
...
Procedure DeliveryStatus(MessageID, Provider, Login, Password, Result) Export 
	...
	If Provider = Enums.SMSProviders._DemoAnotherProvider Then
		// Check the status of the text message sent via "_DemoAnotherProvider".
		// …
		If MessageID = "1234567890" Then
			Result = "Delivered";
		Else
			Result = "Pending";
		EndIf;
