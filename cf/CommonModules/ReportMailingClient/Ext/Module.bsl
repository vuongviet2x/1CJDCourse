...
Procedure SelectRecipient(ResultHandler, Object, MultipleChoice, ReturnsMap) Export
	...
	If MultipleChoice Then
		PossibleRecipients.Insert(0, Undefined, NStr("ru = 'Всем получателям';
	...
	PossibleRecipients.ShowChooseItem(Handler, NStr("ru = 'Выбор получателя';
																|en = 'Select recipient';"));
...
Procedure ExecuteNowInBackground(Recipients, Parameters) Export
	...
	If Parameters.IsItemForm Then
		...
		EndIf;
		StateText = NStr("ru = 'Выполняется рассылка отчетов.';
								|en = 'Distributing reports.';");
	Else
		StateText = NStr("ru = 'Выполняются рассылки отчетов.';
								|en = 'Distributing reports.';");
	EndIf;
	...
	Else
		...
		If Result <> Undefined And Result.ThereAreErrorsInEncryption
		   And CommonClient.SubsystemExists("StandardSubsystems.DigitalSignature") Then
			...
			FormParameters = New Structure;
			FormParameters.Insert("WarningTitle", NStr("ru = 'Не удалось выполнить рассылку отчетов';
		...
		Else
			StandardSubsystemsClient.OutputErrorInfo(Job.ErrorInfo);
		EndIf;
	EndIf;
...
Function ScheduleFillingOptionsList() Export
	...
	VariantList.Add(1, NStr("ru = 'Каждый день';
									|en = 'Every day';"));
	VariantList.Add(2, NStr("ru = 'Каждый второй день';
									|en = 'Every second day';"));
	VariantList.Add(3, NStr("ru = 'Каждый четвертый день';
									|en = 'Every fourth day';"));
	VariantList.Add(4, NStr("ru = 'По будням';
									|en = 'On weekdays';"));
	VariantList.Add(5, NStr("ru = 'По выходным';
									|en = 'On weekends';"));
	VariantList.Add(6, NStr("ru = 'По понедельникам';
									|en = 'On Mondays';"));
	VariantList.Add(7, NStr("ru = 'По пятницам';
									|en = 'On Fridays';"));
	VariantList.Add(8, NStr("ru = 'По воскресеньям';
									|en = 'On Sundays';"));
	VariantList.Add(9, NStr("ru = 'В первый день месяца';
									|en = 'On the first day of the month';"));
	VariantList.Add(10, NStr("ru = 'В последний день месяца';
										|en = 'On the last day of the month';"));
	VariantList.Add(11, NStr("ru = 'Каждый квартал десятого числа';
										|en = 'On the 10th day of every quarter';"));
	If Not CommonClient.DataSeparationEnabled() Then
		VariantList.Add(12, NStr("ru = 'Другое...';
											|en = 'Other…';"));
	EndIf;
...
Procedure SendBulkSMSMessagesInBackground(Recipients, Parameters) Export
	...
	WaitSettings = TimeConsumingOperationsClient.IdleParameters(Parameters.Form);
	WaitSettings.OutputIdleWindow = True;
	WaitSettings.MessageText = NStr("ru = 'Выполняется рассылка SMS с паролями для рассылки отчетов.';
...
Procedure SendBulkSMSMessagesInBackgroundCompletion(Job, Parameters) Export
	...
	If Job.Status = "Completed2" Then
		...
		Form.Title = NStr("ru = 'Результат отправки SMS с паролями архивов';
...
Procedure ClearUpReportDistributionHistoryInBackground(Parameters, AdditionalParameters) Export
	...
	WaitSettings.OutputIdleWindow = True;
	WaitSettings.MessageText = NStr("ru = 'Выполняется очистка истории рассылки отчетов.';
