...
Procedure OnDefineObjectsWithFIllingCommands(Objects) Export
	...
	Objects.Add(Metadata.Documents._DemoReceivedGoodsRecording);
	Objects.Add(Metadata.Documents._DemoCustomerProformaInvoice);
	Objects.Add(Metadata.DocumentJournals._DemoWarehouseDocuments);
	// _Demo Example End
EndProcedure
...
Procedure BeforeAddFillCommands(FillingCommands, Parameters, StandardProcessing) Export
	// _Demo Example Start
	If StrStartsWith(Parameters.FormName, "Catalog._DemoCounterparties")
		Or StrStartsWith(Parameters.FormName, "Catalog._DemoCompanies") Then
		Command = FillingCommands.Add();
		Command.Presentation = NStr("ru = 'Заполнить наименование (из конфигурации, на клиенте, без записи)';
									|en = 'Fill in name (from configuration, on the client, or without saving)';");
		Command.Id = "FillFullDescription";
		Command.Order       = 20;
		Command.Importance      = "Important";
		Command.VisibilityInForms = "ItemForm";
		Command.WriteMode      = "NotWrite";
		Command.Handler       = "_DemoStandardSubsystemsClient.FillDescription";
	EndIf;
	// _Demo Example End
EndProcedure
