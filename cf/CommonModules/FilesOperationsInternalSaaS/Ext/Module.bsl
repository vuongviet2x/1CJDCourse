...
Procedure BeforeExportObject(Container, ObjectExportManager, Serializer, Object, Artifacts, Cancel) Export
	...
	Except
		...
		If Common.SubsystemExists("CloudTechnology.Core") Then
			...
			If CommonClientServer.CompareVersions(ModuleSaaSTechnology.LibraryVersion(), "2.0.2.15") >= 0 Then
				Warning = StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Выгрузка файла %1 (тип %2) пропущена по причине
					|%3';
					|en = 'Export of file %1 (type %2) is skipped due to 
					|%3';"),
					Object,
		...
		WriteLogEvent(
			NStr("ru = 'Файлы.Выгрузка данных для перехода в сервис';
				|en = 'Files.Export data for cloud migration';", Common.DefaultLanguageCode()),
			EventLogLevel.Error,
			Object.Metadata(),
			Object.Ref,
...
Procedure HandleTextExtractionQueue() Export
	...
	If TransactionActive() Then
		Raise(NStr("ru = 'Транзакция активна. Выполнение запроса в транзакции невозможно.';
								|en = 'The transaction is active. Cannot execute a query in the transaction.';"));
	EndIf;
