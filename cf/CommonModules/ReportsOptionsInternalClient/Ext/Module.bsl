...
Procedure WhenActivatingTheReportResult(Form, Item) Export 
	...
	If Items.HeadingAreaContextMenu.Visible Then 
		...
		ButtonTitle = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Сгруппировать по полю %1';
				|en = 'Group by the %1 field';"), TitleProperties.Text);
...
Procedure HighlightInRed(Form, Command, TitleProperties = Undefined, Value = Undefined) Export 
	...
	If CommandAction = "FormatNegativeValues" Then 
		...
		ColoringOptions.Presentation = NStr("ru = 'Оформить отрицательные красным';
												|en = 'Make negative values red';");
	...
	Else
		...
		ColoringOptions.Presentation = NStr("ru = 'Оформить красным';
												|en = 'Highlight in red';");
...
Procedure HighlightInGreen(Form, Command, TitleProperties = Undefined, Value = Undefined) Export 
	...
	If CommandAction = "FormatPositiveValues" Then 
		...
		ColoringOptions.Presentation = NStr("ru = 'Оформить положительные зеленым';
												|en = 'Make positive values green';");
	...
	Else
		...
		ColoringOptions.Presentation = NStr("ru = 'Оформить зеленым';
...
Procedure SetRowHeight(Form, Command, TitleProperties = Undefined,
	...
	If RowHeight = Undefined Then 
		...
		ShowInputNumber(Handler, LineHeightParameters.Size, NStr("ru = 'Высота строки';
																		|en = 'Row height';"), 5);
...
Procedure SetColumnWidth(Form, Command, TitleProperties = Undefined,
	...
	If ColumnWidth = Undefined Then 
		...
		ShowInputNumber(Handler, ColumnWidthParameters.Size, NStr("ru = 'Ширина колонки';
																			|en = 'Column width';"), 5);
...
Procedure DisableFilter(Form, TitleProperties, DetailsData = Undefined) Export 
	...
	If Filter = Undefined Then 
		...
		WarningText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'По полю ""%1"" отсутствуют фильтры';
				|en = 'No filters by the ""%1"" field';"), TitleProperties.Text);
...
Function TheSortTypeOfTheSectionIsAvailable(Command, TitleProperties)
	...
	WarningText = "";
	WarningTemplate = NStr("ru = 'Поле ""%1"" уже отсортировано по %2';
								|en = 'Field ""%1"" is already sorted by %2';");
	
	If StrEndsWith(CommandAction(Command), "Ascending")
		And Not TitleProperties.SortAsc Then 
		
		WarningText = StringFunctionsClientServer.SubstituteParametersToString(
			WarningTemplate, TitleProperties.Text, NStr("ru = 'возрастанию';
																|en = 'ascending';"));
	...
	ElsIf StrEndsWith(CommandAction(Command), "Descending")
		...
		WarningText = StringFunctionsClientServer.SubstituteParametersToString(
			WarningTemplate, TitleProperties.Text, NStr("ru = 'убыванию';
																|en = 'descending';"));
...
Procedure HighlightInYellow(Form, Command, TitleProperties = Undefined, Value = Undefined)
	...
	ColoringOptions.Appearance.Add(Appearance);
	ColoringOptions.Presentation = NStr("ru = 'Оформить желтым';
...
Function ReportFields(Form, Command, CollectionName)
	...
	If ReportFields.FindByValue("More") = Undefined Then 
		ReportFields.Add("More", NStr("ru = 'Еще...';
										|en = 'More…';"));
	EndIf;
...
Function IndexOfTheSelectedField(SourceFieldIndex, FinalCollectionOfFields, Field, MovementParameters)
	...
	If NewField <> Undefined Then 
		...
		If MovementParameters.Response = Undefined Then 
			...
			QuestionTextTemplate = NStr("ru = 'Группировка уже содержит подобное поле - %1.
				|Удалить поле?';
				|en = 'The grouping already contains a similar field: %1.
				|Delete the field?';");
...
Function HeadingAreaContextMenu()
	...
	ContextMenu.Add(DataCompositionDetailsProcessingAction.OpenValue, NStr("ru = 'Открыть';
																								|en = 'Open';"));
	
	// Filter.
	ContextMenu.Add("DisableFilter", NStr("ru = 'Снять фильтр';
												|en = 'Clear filter';"));
	ContextMenu.Add("FilterCommand", NStr("ru = 'Фильтровать...';
												|en = 'Filter…';"),, PictureLib.DataCompositionFilter);
	
	// Sort.
	ContextMenu.Add(DataCompositionSortDirection.Asc, NStr("ru = 'Сортировать по возрастанию';
																				|en = 'Sort ascending';"),, PictureLib.SortRowsAsc);
	ContextMenu.Add(DataCompositionSortDirection.Desc, NStr("ru = 'Сортировать по убыванию';
																				|en = 'Sort descending';"),, PictureLib.SortRowsDesc);
	
	// Apply appearance.
	DesignSubmenu = New ValueList;
	DesignSubmenu.Add("SetRowHeight", NStr("ru = 'Высота строки...';
																|en = 'Row height…';"),, PictureLib.RowHeight);
	DesignSubmenu.Add("SetColumnWidth", NStr("ru = 'Ширина колонки...';
																|en = 'Column width…';"),, PictureLib.ColumnWidth);
	
	ContextMenu.Add(DesignSubmenu, NStr("ru = 'Оформить';
													|en = 'Format';"),, PictureLib.DataCompositionConditionalAppearance);
...
Function DataAreaContextMenu(TitleProperties, AvailableCompareTypes)
	...
	ContextMenu.Add(AvailableCompareTypes, NStr("ru = 'Фильтровать';
															|en = 'Filter';"),, PictureLib.DataCompositionFilter);
	...
	DesignSubmenu.Add("ClearAppearance", NStr("ru = 'Очистить оформление';
															|en = 'Clear appearance';"));
	DesignSubmenu.Add("HighlightInRed", NStr("ru = 'Красным';
														|en = 'Red';"),, DesignImages.HighlightInRed);
	DesignSubmenu.Add("HighlightInYellow", NStr("ru = 'Желтым';
														|en = 'Yellow';"),, DesignImages.HighlightInYellow);
	DesignSubmenu.Add("HighlightInGreen", NStr("ru = 'Зеленым';
														|en = 'Green';"),, DesignImages.HighlightInGreen);
	DesignSubmenu.Add("FormatNegativeValues", NStr("ru = 'Отрицательные красным';
															|en = 'Negative in red';"));
	DesignSubmenu.Add("FormatPositiveValues", NStr("ru = 'Положительные зеленым';
															|en = 'Positive in green';"));
	DesignSubmenu.Add("SetRowHeight", NStr("ru = 'Высота строки...';
																|en = 'Row height…';"),, PictureLib.RowHeight);
	DesignSubmenu.Add("SetColumnWidth", NStr("ru = 'Ширина колонки...';
																|en = 'Column width…';"),, PictureLib.ColumnWidth);
	DesignSubmenu.Add("ApplyAppearanceMore", NStr("ru = 'Еще...';
													|en = 'More…';"));
	...
	// Decrypt.
	ContextMenu.Add("DecodeByDetailedRecords", NStr("ru = 'Расшифровать по детальным записям';
																	|en = 'Drill down by detailed records';"));
	...
	If AreaProperties.ThisIsTheTitle Then 
		...
		If TypeOf(ExecutedAction) = Type("String")
			...
			MessageText = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Действие ""%1"" недоступно';
					|en = 'Action ""%1"" is unavailable';"), Menu.FindByValue(ExecutedAction));
...
Procedure OpenReportForm(Form, OpeningParameters)
	...
	VariantPresentation = StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = '%1 (Расшифровка)';
			|en = '%1 (Details)';"), Form.CurrentVariantPresentation);
...
Function TheActionIsAvailable(Form, Command = Undefined, TitleProperties = Undefined)
	...
	If TypeOf(TitleProperties) <> Type("Structure")
		...
		ShowMessageBox(, NStr("ru = 'Действие недоступно';
										|en = 'Action is unavailable';"));
		Return False;
...
Function ActionOnTheFieldIsAvailable(Action, TitleProperties)
	...
	If Not TitleProperties[Action] Then 
		...
		WarningText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Действие недоступно для поля %1';
				|en = 'Action is unavailable for the %1 field';"), TitleProperties.Text);
...
Procedure AskAboutUserNotification(NotifyDescription, UsersCount) Export 
	...
	RepresentationOfTheNumberOfUsers = StringFunctionsClientServer.StringWithNumberForAnyLanguage(
		NStr("ru = '; %1 пользователя; ; %1 пользователей; %1 пользователей; %1 пользователей';
			|en = '; %1 user; ; %1 users; %1 users; %1 users';"),
		UsersCount);
	
	QueryText = StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = 'Уведомить в чате %1 о том, что этот вариант будет отображен на их панели отчетов?';
			|en = 'Do you want to notify in chat %1 that this option will be displayed on their report panel?';"),
...
Procedure ChangeFormula(Form, Settings, DataPath, CollectionName = "SelectionAvailableFields") Export 
	...
	If TypeOf(Formula) <> Type("DataCompositionUserFieldExpression") Then
		MessageText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Редактирование поля ""%1""доступно в форме настроек для технического специалиста на закладке ""Пользовательские поля""';
				|en = 'You can edit field ""%1"" in the form of the settings for a technician on the User-defined fields tab';"),
			Formula.Title);
