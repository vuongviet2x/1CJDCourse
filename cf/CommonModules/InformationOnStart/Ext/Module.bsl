...
Procedure OnAddUpdateHandlers(Handlers) Export
	...
	Handler.Comment = NStr("ru = 'Актуализирует данные первого показа.';
									|en = 'Updates data of the first show.';");
...
Function PagesPackages(TemplatesMedia) Export
	...
	For LineNumber = 3 To SpreadsheetDocument.TableHeight Do
		...
		If Lower(TableRow.Section) = Lower(NStr("ru = 'Реклама';
													|en = 'Ads';")) Then // ACC:1391 Localizable section.
			TableRow.Priority = 0;
		Else
...
Function ExtractPackageFiles(TemplatesMedia, TemplateName) Export
	...
	Except
		MessageText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Не удалось извлечь файл из макета %1 (%2: %3) обработки %4 по причине:';
				|en = 'Cannot retrieve the file from template %1(%2:%3) of data processor %4 due to:';"),
		...
		If TemplatesMedia.Metadata().Templates.Find(LocalizedTemplateName) = Undefined Then
			MessageText = MessageText + NStr("ru = 'Макет с таким именем не существует.';
													|en = 'Template with this name does not exist.';") + Chars.LF;
		EndIf;
		MessageText = MessageText + ErrorProcessing.DetailErrorDescription(ErrorInfo());
		WriteLogEvent(
			NStr("ru = 'Информация при запуске';
				|en = 'Startup notifications';", Common.DefaultLanguageCode()),
