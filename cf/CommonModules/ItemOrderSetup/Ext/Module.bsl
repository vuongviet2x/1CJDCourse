...
Procedure OnDefineAttachableCommandsKinds(AttachableCommandsKinds) Export
	...
	Kind.Title   = NStr("ru = 'Настройка порядка элементов';
							|en = 'Item order setup';");
...
Procedure OnDefineCommandsAttachedToObject(FormSettings, Sources, AttachedReportsAndDataProcessors, Commands) Export
	...
	Command.Presentation = NStr("ru = 'Переместить элемент вверх';
								|en = 'Move item up';");
	Command.Order = 1;
	...
	Command.Presentation = NStr("ru = 'Переместить элемент вниз';
								|en = 'Move item down';");
	Command.Order = 2;
