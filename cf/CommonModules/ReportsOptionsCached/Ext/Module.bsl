...
Function CurrentUserSubsystems() Export
	...
	RootRow.Presentation = NStr("ru = 'Все разделы';
										|en = 'All sections';");
	...
	For Each ListItem In SectionsList Do
		...
		If Not (TypeOf(MetadataSection) = Type("MetadataObject") And StrStartsWith(MetadataSection.FullName(), "Subsystem"))
			...
			Raise StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Некорректно определены значения разделов в процедуре %1.';
					|en = 'Invalid section values in %1 procedure.';"), 
		...
		Else
			TitleTemplate1 = NStr("ru = 'Отчеты раздела ""%1""';
									|en = '%1 section reports';");
		EndIf;
