...
Procedure ConnectCorrespondent(Message, Sender)
	...
	If IsEndpoint.IsEmpty()
		...
		ErrorPresentation = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Конечная точка не соответствует ожидаемой. Код ожидаемой конечной точки %1. Код текущей конечной точки %2.';
				|en = 'Endpoint does not match the expected one. Expected endpoint code %1. Current endpoint code %2. ';"),
			Body.SenderId,
	...
	If Peer.IsEmpty() Then // Connecting a correspondent endpoint.
		...
		If Cancel Then // Sending an error message to the service manager.
			...
			ErrorPresentation = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Ошибка подключения конечной точки корреспондента обмена. Код конечной точки корреспондента обмена %1.';
					|en = 'Peer infobase endpoint connection error. Endpoint ID: %1.';"),
				Body.RecipientId);
		...
		If ConnectedCorrespondentCode <> Body.RecipientId Then
			...
			ErrorPresentation = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Ошибка при подключении конечной точки корреспондента обмена.
				...
				|en = 'Peer infobase endpoint connection error.
	...
	Else // Updating the correspondent and the endpoint connection settings.
		...
		If Cancel Then // Sending an error message to the service manager.
			...
			ErrorPresentation = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Ошибка обновления параметров подключения этой конечной точки и конечной точки корреспондента обмена.
				|Код этой конечной токи %1
				|Код конечной точки корреспондента обмена %2';
				|en = 'Endpoint connection failed.
...
Procedure SetTransportSettings(Message, Sender)
	...
	If Peer.IsEmpty() Then
		MessageString = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Не найдена конечная точка корреспондента с кодом ""%1"".';
				|en = 'Endpoint with ID %1 is not found.';"),
			Body.RecipientId);
		Raise MessageString;
	EndIf;
...
Function EventLogEventCorrespondentConnection()
	...
	Return NStr("ru = 'Обмен данными.Подключение корреспондента обмена';
				|en = 'Data exchange.Peer infobase connection';", Common.DefaultLanguageCode());
