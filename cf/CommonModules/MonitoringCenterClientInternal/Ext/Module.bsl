...
Procedure NotifyRequestForReceivingDumps() Export
	ShowUserNotification(NStr("ru = 'Отчеты об ошибках';
										|en = 'Error reports';"),
			"e1cib/app/DataProcessor.MonitoringCenterSettings.Form.RequestForErrorReportsCollectionAndSending",
			NStr("ru = 'Предоставьте отчеты о возникающих ошибках';
				|en = 'Provide reports on occurred errors';"),
...
Procedure NotifyRequestForSendingDumps() Export
	ShowUserNotification(NStr("ru = 'Отчеты об ошибках';
				...
				NStr("ru = 'Отправьте отчеты о возникающих ошибках';
					|en = 'Send reports on occurred errors';"),
				PictureLib.DialogExclamation,
				UserNotificationStatus.Important, "DumpsSendingRequest");
EndProcedure

Procedure NotifyContactInformationRequest() Export
	ShowUserNotification(NStr("ru = 'Проблемы производительности';
										|en = 'Performance issues';"),
				New NotifyDescription(
						"OnClickNotifyContactInformationRequest",
						ThisObject, True),
				NStr("ru = 'Сообщить о проблемах производительности';
