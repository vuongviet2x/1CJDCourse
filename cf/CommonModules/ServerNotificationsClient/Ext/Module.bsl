...
Procedure ProcessServerNotificationOnClient(DataReceiptStatus, ServerNotification)
	...
	Except
		...
		ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
			...
			ErrorProcessing.DetailErrorDescription(ErrorInfo));
		EventLogClient.AddMessageForEventLog(
			NStr("ru = 'Серверные оповещения.Ошибка обработки полученного оповещения';
...
Procedure AfterNewPersonalMessageHandlerAttachError(ErrorInfo, StandardProcessing, Context) Export
	...
	EventLogClient.AddMessageForEventLog(
		NStr("ru = 'Серверные оповещения.Ошибка подключения обработчика новых личных сообщений';
...
Procedure OnInteractionSystemNewPersonalMessageReceiptError(ErrorInfo, StandardProcessing, Context) Export
	...
	EventLogClient.AddMessageForEventLog(
		NStr("ru = 'Серверные оповещения.Ошибка получения нового личного сообщения';
...
Procedure AfterNewGlobalMessageHandlerAttachError(ErrorInfo, StandardProcessing, Context) Export
	...
	EventLogClient.AddMessageForEventLog(
		NStr("ru = 'Серверные оповещения.Ошибка подключения обработчика новых общих сообщений';
...
Procedure OnInteractionSystemNewGlobalMessageReceiptError(ErrorInfo, StandardProcessing, Context) Export
	...
	EventLogClient.AddMessageForEventLog(
		NStr("ru = 'Серверные оповещения.Ошибка получения нового общего сообщения';
...
Procedure IfThereIsErrorInReceivingNewClientNotification(ErrorInfo, StandardProcessing, Context) Export
	...
	EventLogClient.AddMessageForEventLog(
		NStr("ru = 'Серверные оповещения.Ошибка получения нового уведомления клиента';
			|en = 'Server notifications.Error obtaining new client notification';",
			CommonClient.DefaultLanguageCode()),
		"Error",
		ErrorProcessing.DetailErrorDescription(ErrorInfo));
