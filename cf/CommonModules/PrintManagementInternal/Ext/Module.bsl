		...
		For Each PrintFormString In TableOfPrintedForms Do
			If PrintFormString.CreateAgain Then
				...
				Else
					...
					NStr("ru = 'Ошибка: Не удалось сформировать %1';
						|en = 'Error: Cannot generate %1';"), PrintFormString.Presentation);
				EndIf;
			EndIf;
...
Function TemplateFromBinaryData(BinaryTemplateData) Export
	...
	If Extension <> "docx" Then
		ErrorText = NStr("ru = 'Макет шаблона офисного документа имеет неверный формат.';
							|en = 'Incorrect layout format for MS Word template.';");
...
Procedure CloseConnection(PrintForm) Export
	...
	Except
		...
		Raise(NStr("ru = 'Не удалось удалить временный каталог шаблона печатной формы по причине:';
								|en = 'Failed to delete temporary directory where print form template is stored. Reason:';") + Chars.LF 
...
Function GetNameWithoutDigits(Val Name)
	...
	If ArrayOfKey.Count() > 1 Then
		Raise NStr("ru = 'Неожиданный ключ';
								|en = 'Unexpected key';");
	EndIf;
	...
	If NumbersArray.Count() > 1 Then
		Raise NStr("ru = 'Неожиданный ключ, несколько номеров';
								|en = 'Unexpected key, several numbers';");
	EndIf;
...
Function ReadRecord(XMLReader, TreeRow, Hyperlinks)
	...
	While XMLReader.Read() Do
		...
		Else
			Raise (NStr("ru = 'Не известный:';
									|en = 'Unknown:';") + " " + XMLReader.NodeType);
		EndIf;
	EndDo;
	Return WholeText;
EndFunction  
...
Procedure FindAreas(DocumentStructure, ObjectTablePartNames) Export
	...
	For Each ConditionalAreaStartIndex In ConditionalAreasStartIndexes Do
		...
		If ConditionalAreaEnd = Undefined Then
			Raise StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'В макете документа нет окончания условной области ""%1"".';
					|en = 'In the document template, the end of the %1 conditional area is not specified.';"), AreaCondition);
		EndIf;
...
Procedure ParseDOCXDocumentContainer(Val FullFileName, Val FileStructurePath)
	...
	Except
		...
		WriteEventsToEventLog(EventLogEvent(), "Error", ErrorProcessing.DetailErrorDescription(ErrorInfo()));
		Raise(NStr("ru = 'Не удалось открыть файл шаблона по причине:';
								|en = 'Cannot open template file. Reason:';") + Chars.LF 
			+ ErrorProcessing.BriefErrorDescription(ErrorInfo()));
	EndTry;
	...
	Except
		...
		DeleteFiles(FullFileName);
		WriteEventsToEventLog(EventLogEvent(), "Error", ErrorProcessing.DetailErrorDescription(ErrorInfo()));
		Raise(NStr("ru = 'Не удалось выполнить разбор файла шаблона по причине:';
								|en = 'Cannot parse template file. Reason:';") + Chars.LF 
			+ ErrorProcessing.BriefErrorDescription(ErrorInfo()));
	EndTry;
...
Procedure AssembleDOCXDocumentContainer(Val FullFileName, Val FileStructurePath)
	...
	Except
		WriteEventsToEventLog(EventLogEvent(), "Error", ErrorProcessing.DetailErrorDescription(ErrorInfo()));
		Raise(NStr("ru = 'Не удалось создать файл документа по причине:';
								|en = 'Cannot create MS Word document. Reason:';") + Chars.LF 
			+ ErrorProcessing.BriefErrorDescription(ErrorInfo()));
	EndTry;
	...
	Except
		WriteEventsToEventLog(EventLogEvent(), "Error", ErrorProcessing.DetailErrorDescription(ErrorInfo()));
		Raise(NStr("ru = 'Не удалось сформировать файл документа по причине:';
								|en = 'Cannot generate MS Word document. Reason:';") + Chars.LF 
			+ ErrorProcessing.BriefErrorDescription(ErrorInfo()));
	EndTry;
...
Function EventLogEvent()
	...
	Return NStr("ru = 'Печать';
				|en = 'Print';", Common.DefaultLanguageCode());
	
EndFunction

Procedure CopyDirectoryContent(From, Where) Export
