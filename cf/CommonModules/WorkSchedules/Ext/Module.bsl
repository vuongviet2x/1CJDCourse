	...
	If Selection.Count() < ShiftDays.Maximum Then
		If RaiseException1 Then
			Raise StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'График работы ""%1"" не заполнен с даты %2 на указанное количество рабочих дней.';
					|en = 'Work schedule ""%1"" is not filled in for the specified number of workdays after %2.';"), 
...
Function NearestDatesIncludedInSchedule(WorkScheduleCalendar, InitialDates, ReceivingParameters = Undefined) Export
	...
	CommonClientServer.Validate(
		...
		NStr("ru = 'Не указан график работы.';
	...
	While Selection.Next() Do
		...
		Else 
			...
			If ReceivingParameters.RaiseException1 Then
				Raise StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Невозможно определить ближайшую дату, включенную в график, для даты %1, 
						 |возможно, график работы не заполнен.';
						|en = 'Cannot determine the nearest date included in the schedule for date %1.
...
Function DateDiffByCalendar(Val WorkScheduleCalendar, Val StartDate, Val EndDate, RaiseException1 = True) Export
	...
	If Not ValueIsFilled(WorkScheduleCalendar) Then
		If RaiseException1 Then
			Raise NStr("ru = 'Не указан график работы.';
									|en = 'Work schedule is not specified.';");
		EndIf;
		Return Undefined;
	EndIf;
...
Procedure OnAddUpdateHandlers(Handlers) Export
	...
	Handler.Comment = NStr("ru = 'Исправление включения в график рабочих и предпраздничных дней, выпадающих на субботу или воскресенье.';
									|en = 'Corrects the inclusion of working days and days before the holidays falling on Saturdays or Sundays in the schedule.';");
	...
	Handler.Comment = NStr("ru = 'Установка нового признака ""Учитывать нерабочие периоды"" в положение по умолчанию.';
									|en = 'Set the new ""Skip non-work periods"" flag to the default value.';");
	Handler.ObjectsToLock = "Catalog.Calendars";
	Handler.ExecutionPriorities = InfobaseUpdate.HandlerExecutionPriorities();
