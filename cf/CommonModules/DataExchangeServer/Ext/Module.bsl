		...
		If Attribute <> Undefined Then
			...
			If AttributeTypes.Count() <> 1 Then
				...
				MessageString = NStr("ru = 'Составной тип данных для значений по умолчанию не поддерживается.
					|Реквизит ""%1"".';
					|en = 'Default values don''t support flexible data types.
					|Attribute: %1.';");
			...
			If Not Common.IsCatalog(MetadataObject) Then
				...
				MessageString = NStr("ru = 'Выбор значений по умолчанию поддерживается только для справочников.
					|Реквизит ""%1"".';
					|en = 'Only catalogs support selection of default values.
					|Attribute: %1.';");
				MessageString = StringFunctionsClientServer.SubstituteParametersToString(MessageString, Attribute.FullName());
				Raise MessageString;
			EndIf;
...
//
// Returns:
//  String - a string presentation of date.
//
Function SynchronizationDatePresentation(Val SynchronizationDate) Export
	
	If Not ValueIsFilled(SynchronizationDate) Then
		Return NStr("ru = 'Синхронизация не выполнялась.';
					|en = 'Synchronization never started.';");
	EndIf;
	
	Return StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Последняя синхронизация: %1';
																		|en = 'Last synchronized on: %1';"), RelativeSynchronizationDate(SynchronizationDate));
EndFunction
...
Function RelativeSynchronizationDate(Val SynchronizationDate) Export
	...
	Else
		...
		If DifferenceDaysCount = 0 Then // Today.
			...
			Result = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Сегодня, %1';
																					|en = 'Today, %1';"), Format(SynchronizationDate, "DLF=T"));
			
		ElsIf DifferenceDaysCount = 1 Then // Yesterday.
			
			Result = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Вчера, %1';
																					|en = 'Yesterday, %1';"), Format(SynchronizationDate, "DLF=T"));
			
		ElsIf DifferenceDaysCount = 2 Then // Day before yesterday.
			
			Result = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Позавчера, %1';
																					|en = 'Day before yesterday, %1';"), Format(SynchronizationDate, "DLF=T"));
...
//      * ExchangeFormatVersions                    - Map of KeyAndValue - Supported format versions mapped with references to IFDE common modules.
//                                                 Intended for Interim Format Data Exchange only
//                                                 :
//         ** Key - Number - Format version.
//         ** Value - CommonModule - A reference to a common module.
...
//      *UseCacheOfPublicIdentifiers   - Boolean - Indicates that cache should be used
//                                                   for searching references during data import
...
//                                                    - A list of used message transports. 
//                                                    If empty, all transport types are available.
...
//                                                    the form within the configuration as a string to
...
//      * DataMappingSupported         - Boolean - If True, the setting option supports interactive data mapping. 
//                                                    Otherwise, the infobase will receive the messages "as is".
//                                                    
//                                                    
//                                                    
//      * FormatExtension                         - String - A format extension name applicable for the setting option.
//                                                    It is created from the URL of the format extension schema namespace,
//                                                    excluding the version. For example, for the namespace "http://v8.1c.ru/edi/edi_stnd/_DemoEnterpriseDataExt/1.1", the format extension name will be "http://v8.1c.ru/edi/edi_stnd/_DemoEnterpriseDataExt".
//                                                    
//                                                     
//                                                     
//                                                    
//      * CorrespondentExchangePlanName              - String - The name of the peer infobase's exchange plan. 
//
Function DefaultExchangeSettingOptionDetails(ExchangePlanName) Export
	...
	WizardFormTitle = StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = 'Синхронизация данных с %1 (настройка)';
			|en = 'Data synchronization with %1 (setup)';"),
		ExchangePlanSynonym);
...
//                    - Undefined - an exchange node matching the correspondent.
//                                     For the NewConnection mode it equals Undefined;
//    * ExchangePlanName - String
//                     - Undefined - a name of the exchange plan where node is created.
//    * SettingID - String
//                             - Undefined - a setting option ID
//                                              as it is specified in the exchange plan manager module.
//  ConnectionParameters - Structure - a structure of connection parameters:
//    * CorrespondentID - String - a 36-character-long string
//                                  - Undefined - a correspondent UUID.
//    * PeerInfobaseName - String
//                                 - Undefined - a correspondent name as it will be displayed
//                                                  in the list of configured synchronizations.
//    * TransportSettings - Arbitrary
//                          - Undefined - an arbitrary value describing connection parameters.
//    * SynchronizationSchedule - JobSchedule
//                              - Undefined - a schedule of synchronization startup.
//    * XDTOSettings - Structure
//                    - Undefined - A structure of peer's XDTO settings:
	...
	Try
		...
		Else
			Raise StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Не подходящий режим вызова функции: %1';
					|en = 'Invalid function call mode: %1';"), Context.Mode);
		EndIf;
			...
			If Common.DataSeparationEnabled() Then
				If Common.SubsystemExists("CloudTechnology.JobsQueue") 
					...
					JobKey = StringFunctionsClientServer.SubstituteParametersToString(
						NStr("ru = 'Обмен данными с внешней системой (%1)';
							|en = 'Data exchange with external system (%1)';"),
...
//    * PeerInfobaseName - String - a correspondent name as it is displayed
//                                            in the list of configured synchronizations.
...
//
// Returns:
//   Array of String - an array of strings with setting IDs for the correspondent.
// 
...
Function IssueMonitorHyperlinkTitleStructure(Nodes = Undefined) Export
	...
	If Count > 0 Then
		...
		Title = NStr("ru = 'Предупреждения (%1)';
						|en = 'Warnings (%1)';");
	...
	Else
		...
		Title = NStr("ru = 'Предупреждений нет';
						|en = 'No warnings';");
		Picture = New Picture;
...
Procedure ImportPriorityDataToSubordinateDIBNode(Cancel = False) Export
	...
	Except
		...
		WriteLogEvent(
			NStr("ru = 'Обмен данными.Загрузка приоритетных данных';
				|en = 'Data exchange.Priority data import';", Common.DefaultLanguageCode()),
			EventLogLevel.Error,,,
			...
			Raise
				NStr("ru = 'Загружены изменения программы, полученные из главного узла.
				           ...
				           |После этого запустите программу.';
							|en = 'Changes are imported from the main node. Do the following:
...
Procedure WriteUpdateDataToFile(Parameters, Data, DataKind, FullObjectName = "") Export
	...
	If Parameters.WriteChangesForSubordinateDIBNodeWithFilters = Undefined Then
		ExceptionText = NStr("ru = 'В обработчике неправильно организована работа с параметрами регистрации данных к обработке.';
								|en = 'The processing of the data registration parameters in the handler is poorly arranged.';");
		Raise ExceptionText;
	EndIf;
...
Procedure CheckExchangeMessageTransportDataProcessorAttachment(Cancel,
	...
	If Not DataProcessorObject.ConnectionIsSet() Then
		...
		WriteLogEvent(NStr("ru = 'Транспорт сообщений обмена';
										|en = 'Exchange message transport';", Common.DefaultLanguageCode()),
...
Procedure CheckExchangeManagementRights() Export
	...
	If Not HasRightsToAdministerExchanges() Then
		...
		Raise NStr("ru = 'Недостаточно прав для администрирования синхронизации данных.';
								|en = 'Insufficient rights to administer data synchronization.';");
...
// Parameters:
//   Receiver - ValueTable
//            - TabularSection -  destination data.
//   Source - ValueTable
...
//                              - Undefined - shows that correspondent is in SaaS.
//
// Returns:
//   Structure:
//     * CorrespondentVersion - String - a number of the peer infobase configuration version.
...
//
// Returns:
//   Structure - A set of export add-on options:
...
Procedure CheckWhetherTheExchangeCanBeStarted(ExchangeNode, Cancel) Export
	...
	Except
		...
		ErrorMessage = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Не удалось выполнить обмен данными с %1.
			|Обмен данными уже выполняется. Повторите попытку позже.';
			|en = 'Cannot start an exchange with %1.
...
// Returns the flag indicating whether the specified procedure or function is available in the exchange plan manager module.
// Calculated by exchange plan settings, the Algorithms property (see the comment to "DefaultExchangePlanSettings").
// Parameters:
...
Procedure CalculateExportPercent(ExportedCount, ObjectsToExportCount) Export
	...
	If ObjectsToExportCount = 0 Or ExportedCount > ObjectsToExportCount Then
		ProgressPercent = 95;
		Template = NStr("ru = 'Обработано: %1 объектов.';
						|en = '%1 objects processed.';");
	...
	Else
		...
		Template = NStr("ru = 'Обработано: %1 из %2 объектов.';
						|en = '%1 out of %2 objects processed.';");
...
Procedure CalculateImportPercent(ExportedCount1, ObjectsToImportCount, ExchangeMessageFileSize) Export
	...
	If ExchangeMessageFileSize <> 0 Then
		Template = NStr("ru = 'Размер сообщения %1 МБ';
						|en = 'Message size: %1 MB';");
...
Function FTPServerNameAndPath(Val StringForConnection) Export
	...
	If (Upper(Left(StringForConnection, 6)) <> "FTP://"
		...
		Raise StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Строка подключения к FTP-ресурсу не соответствует формату: ""%1""';
				|en = 'The FTP connection string has invalid format: ""%1""';"), StringForConnection);
	EndIf;
	...
	If ConnectionParameters.Count() < 3 Then
		Raise StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'В строке подключения к FTP-ресурсу не указано имя сервера: ""%1""';
				|en = 'The server name is missing from the FTP connection string: ""%1""';"), StringForConnection);
	EndIf;
...
Function SelectChanges(Val Node, Val MessageNo, Val SelectionFilter = Undefined) Export
	...
	If TransactionActive() Then
		Raise NStr("ru = 'Выборка изменений данных запрещена в активной транзакции.';
								|en = 'Cannot select data changes in an active transaction.';");
	EndIf;
...
Procedure CheckCanSynchronizeData(OnlineApplication = False) Export
	...
	If Not AccessRight("View", Metadata.CommonCommands.Synchronize) Then
		...
		If OnlineApplication Then
			...
			Raise NStr("ru = 'Недостаточно прав для синхронизации данных с приложением в Интернете.';
									|en = 'Insufficient rights to synchronize data with the web application.';");
			
		Else
			
			Raise NStr("ru = 'Недостаточно прав для синхронизации данных.';
									|en = 'Insufficient rights to synchronize data.';");
	...
	ElsIf InfobaseUpdate.InfobaseUpdateRequired()
		...
		If OnlineApplication Then
			...
			Raise NStr("ru = 'Приложение в Интернете находится в состоянии обновления.';
									|en = 'Web application is updating.';");
			
		Else
			
			Raise NStr("ru = 'Информационная база находится в состоянии обновления.';
									|en = 'Infobase is updating.';");
	...
	ElsIf Common.DataSeparationEnabled()
		...
		If Common.SubsystemExists("CloudTechnology.Core") Then
			...
			If ModuleSaaSOperations.DataAreaStatus(DataAreaNumber) <> Enums["DataAreaStatuses"]["Used"] Then
				...
				ExceptionText = NStr("ru = 'Синхронизация невозможна, потому что область данных отмечена как не рабочая.
					|Необходимо обратиться к администратору системы или в службу поддержки сервиса.';
					|en = 'Cannot sync as the data area is marked as inactive.
...
Procedure CheckDataExchangeUsage(SetUsing = False) Export
	...
	If Not GetFunctionalOption("UseDataSynchronization") Then
		...
		Else
			MessageText = NStr("ru = 'Синхронизация данных запрещена администратором.';
									|en = 'Synchronization is disabled by the administrator.';");
			WriteLogEvent(DataExchangeEventLogEvent(), EventLogLevel.Error,,,MessageText);
			Raise MessageText;
		EndIf;
...
Procedure DeleteDocumentRegisterRecords(DocumentObject) Export
	...
	For Each RegisterRecordRow In RegisterRecordTable Do
		...
		If Not AccessRight("Update", Set.Metadata()) Then
			...
			ExceptionText = NStr("ru = 'Нарушение прав доступа: %1';
									|en = 'Access rights violation: %1';");
	...
	Try
		For Each RegisterRecordRow In RecordTableRowToProcessArray Do
			...
			Except
				...
				Raise StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Операция не выполнена: %1
					|%2';
...
Procedure OnAddUpdateHandlers(Handlers) Export
	...
	Handler.Comment =
		NStr("ru = 'Первоначальное заполнение настроек обмена данными XDTO.';
			|en = 'Initial population of XDTO data exchange settings.';");
	...
	Handler.Comment =
		NStr("ru = 'Заполнение вспомогательных настроек обмена данными в регистре ""Общие настройки узлов информационных баз"".';
			|en = 'Population of auxiliary data exchange settings in the register ""Common infobase node settings"".';");
	...
	Handler.Comment =
		NStr("ru = 'Перенос результатов выполнения обменов данными в новый регистр.';
			|en = 'Moving data exchange results to a new register.';");
...
Procedure OnFillSuppliedAccessGroupProfiles(ProfilesDetails, ParametersOfUpdate) Export
	...
	ProfileDetails.Description =
		NStr("ru = 'Синхронизация данных с другими программами';
			|en = 'Synchronization with other applications';", Common.DefaultLanguageCode());
...
Function DataExchangeRulesImportEventLogEvent() Export
	...
	Return NStr("ru = 'Обмен данными.Загрузка правил';
				|en = 'Data exchange.Load rules';", Common.DefaultLanguageCode());
...
Function DataExchangeCreationEventLogEvent() Export
	...
	Return NStr("ru = 'Обмен данными.Создание обмена данными';
				|en = 'Data exchange.Create data exchange';", Common.DefaultLanguageCode());
...
Function DataExchangeDeletionEventLogEvent() Export
	...
	Return NStr("ru = 'Обмен данными.Удаление обмена данными';
				|en = 'Data exchange.Delete data exchange';", Common.DefaultLanguageCode());
...
Function RegisterDataForInitialExportEventLogEvent() Export
	...
	Return NStr("ru = 'Обмен данными.Регистрация данных для начальной выгрузки';
				|en = 'Data exchange.Register data for initial export';", Common.DefaultLanguageCode());
...
Function DataExportToMapEventLogEvent() Export
	...
	Return NStr("ru = 'Обмен данными.Выгрузка данных для сопоставления';
				|en = 'Data exchange.Export data for mapping';", Common.DefaultLanguageCode());
...
Function ExportDataToFilesTransferServiceEventLogEvent() Export
	...
	Return NStr("ru = 'Обмен данными.Сервис передачи файлов.Выгрузка данных';
				|en = 'Data exchange.File transfer service.Export data';", Common.DefaultLanguageCode());
...
Function ImportDataFromFilesTransferServiceEventLogEvent() Export
	...
	Return NStr("ru = 'Обмен данными.Сервис передачи файлов.Загрузка данных';
				|en = 'Data exchange.File transfer service.Import data';", Common.DefaultLanguageCode());
...
Procedure ExecuteDataExchangeForInfobaseNodeOverFileOrString(ExchangeParameters) Export
	...
	If ExchangeParameters.InfobaseNode = Undefined Then
		...
		If ExchangeParameters.InfobaseNode.IsEmpty()
			...
			ElsIf MigrationError Then
				ErrorMessageString = NStr("ru = 'Не удалось выполнить переход на синхронизацию данных через универсальный формат.';
												|en = 'Cannot switch to Interim Format Data Exchange.';");
				Raise ErrorMessageString;
			EndIf;
		EndIf;
		
		If ExchangeParameters.InfobaseNode.IsEmpty() Then
			ErrorMessageString = NStr("ru = 'Узел плана обмена %1 с кодом %2 не найден.';
											|en = 'Node with ID %2 not found. Exchange plan: %1.';");
	...
	If Not SynchronizationSetupCompleted(ExchangeParameters.InfobaseNode) Then
		...
		ErrorMessageString = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'В ""%1"" настройка синхронизации данных с ""%2"" (идентификатор ""%3"") еще не завершена.';
				|en = 'The setup of data synchronization with %2 (ID: %3) in %1 is not completed.';"),
	...
	If ExchangeSettingsStructure.Cancel Then
		ErrorMessageString = NStr("ru = 'Ошибка при инициализации процесса обмена данными.';
										|en = 'Cannot initialize data exchange.';");
		WriteExchangeFinish(ExchangeSettingsStructure);
		Raise ErrorMessageString;
	EndIf;
...
Procedure WriteMessageWithNodeChanges(ExchangeSettingsStructure, Val ExchangeMessageFileName = "", ExchangeMessage = "") Export
	...
	Else
		...
		If ExchangeSettingsStructure.ExchangeByObjectConversionRules Then
			...
			// Get the initialized data exchange processor.
...
Function ExportAdditionSettingsAutoSavingName() Export
	Return NStr("ru = 'Последняя отправка (сохраняется автоматически)';
				|en = 'Last data sent (autosaved)';");
EndFunction
...
// Parameters:
//     ExportAddition     - Structure
//                            - FormDataCollection - export parameters details.
//
...
Procedure WriteExchangeFinish(ExchangeSettingsStructure) Export
	...
	If ExchangeSettingsStructure.IsDIBExchange Then
		MessageString = NStr("ru = '%1, %2';
								|en = '%1, %2';", Common.DefaultLanguageCode());
	...
	Else
		MessageString = NStr("ru = '%1, %2; Объектов обработано: %3';
								|en = '%1, %2. Objects processed: %3';", Common.DefaultLanguageCode());
		MessageString = StringFunctionsClientServer.SubstituteParametersToString(MessageString,
							ExchangeSettingsStructure.ExchangeExecutionResult,
							ExchangeSettingsStructure.ActionOnExchange,
		...
		For Each MapObject In ObjectsForDeferredPosting Do
			...
			Try
				...
				Else
					...
					ErrorDescription = NStr("ru = 'Ошибка проверки заполнения реквизитов';
											|en = 'Attribute filling verification error';");
...
Procedure BlockTheExchangeNode(ExchangeNode, Cancel) Export
	...
	Except
		...
		MessageTemplate = NStr("ru = 'Не удалось заблокировать узел плана обмена [%1].
			|Вероятно, обмен данными уже выполняется. Повторите попытку позже.';
			|en = 'Cannot lock the exchange plan node [%1].
...
Procedure UnblockTheExchangeNode(ExchangeNode, Cancel) Export
	...
	Except
		...
		MessageTemplate = NStr("ru = 'Не удалось разблокировать узел плана обмена [%1].
			|Вероятно, обмен данными уже выполняется. Повторите попытку позже.';
			|en = 'Cannot unlock the exchange plan node [%1].
			|Data exchange is probably already in progress. Try again later.';", Common.DefaultLanguageCode());
...
Function GetExchangeMessageToTemporaryDirectory(Cancel, InfobaseNode, ExchangeMessagesTransportKind, OutputMessages = True) Export
	...
	If ExchangeSettingsStructure.Cancel Then
		...
		If OutputMessages Then
			NString = NStr("ru = 'При инициализации обработки транспорта сообщений обмена возникли ошибки.';
							|en = 'Failed to initialize exchange message transport processing.';");
	...
	If ExchangeSettingsStructure.ExchangeExecutionResult <> Undefined Then
		...
		If OutputMessages Then
			NString = NStr("ru = 'При получении сообщений обмена возникли ошибки.';
							|en = 'Errors occurred when receiving exchange messages.';");
			Common.MessageToUser(NString,,,, Cancel);
		EndIf;
...
Function GetExchangeMessageToTempDirectoryFromCorrespondentInfobase(Cancel, InfobaseNode, OutputMessages = True) Export
	...
	Except
		If OutputMessages Then
			Message = NStr("ru = 'Не удалось произвести обмен: %1';
							|en = 'Data exchange failed: %1';");
	...
	If Cancel Then
		...
		If OutputMessages Then
			...
			Message = NStr("ru = 'Не удалось выгрузить данные: %1';
							|en = 'Data export failed: %1';");
...
Procedure FillInitialRefsInPredefinedDataTable(ImportParameters, ErrorMessage)
	...
	ReaderSettings = New XMLReaderSettings(, , , , , , , False); // Read considering the comments
	Try
...
Procedure InitializeMessageReaderForStandardImport(ImportParameters, ExchangeExecutionResult, ErrorMessage)
	...
	If MessageReader.Sender <> ImportParameters.InfobaseNode Then
		...
		ErrorMessage = NStr("ru = 'Сообщение обмена содержит данные для другого узла информационной базы.';
								|en = 'The exchange message contains data for another infobase node.';",
...
Function IsErrorMessageNumberLessOrEqualToPreviouslyReceivedMessageNumber(ErrorDescription)
	...
	Return StrFind(Lower(ErrorDescription), Lower(NStr("ru = 'Номер сообщения меньше или равен';
													|en = 'The message number is less than or equal to';"))) > 0;
...
Procedure OnReceiveFileFromStorage(Val FileID, FileName)
	...
	If QueryResult.IsEmpty() Then
		LongDesc = NStr("ru = 'Файл с идентификатором %1 не обнаружен.';
						|en = 'File with ID %1 not found.';");
...
//  Companies - Array
//              - Undefined - a list of companies
//               data changes are to be registered for. If this parameter is not specified, companies are not taken into
//               account on changes registration.
//
		...
		If ExchangePlanContentItem.AutoRecord = AutoChangeRecord.Deny
			...
			ElsIf Common.IsRegister(ExchangePlanContentItem.Metadata) Then // Registers
				...
				If Common.IsInformationRegister(ExchangePlanContentItem.Metadata)
					...
					ElsIf FilterByExportStartDate And FilterByPeriod Then // Register by date.
						
						Selection = MainInformationRegisterFilterValuesSelectionByExportStartDate(MainFilter, FullObjectName, ExportStartDate);
						
					ElsIf FilterByCompanies And FilterByCompany Then // Register by companies.
...
Procedure ExecuteSharedDataOnWriteCheck(Val Data) Export
	...
	If Common.DataSeparationEnabled()
		...
		ExceptionText = NStr("ru = 'Недостаточно прав для выполнения действия.';
								|en = 'Insufficient rights for this operation.';", Common.DefaultLanguageCode());
...
Procedure RecordDocumentPostingError(
	...
	If Not IsBlankString(MessageText) Then
		MessageString = NStr("ru = 'Не удалось провести документ %1, полученный из другой информационной базы.
			|По причине: %2.
			|Возможно, не заполнены все реквизиты, обязательные к заполнению.';
			|en = 'Cannot post ""%1"" received from another infobase.
			|Reason: %2.
	...
	If RecordIssuesInExchangeResults Then
		...
		Except
			MessageString = NStr("ru = 'Ошибка при записи результата обмена данными для объекта ""%1:
								   |%2';
									|en = 'An error occurred when saving the data exchange result for the %1 object:
...
Procedure RecordObjectWriteError(
	...
	If Not IsBlankString(TrimAll(MessageText)) Then
		...
		ErrorReason = " " + NStr("ru = 'По причине %1.';
									|en = 'Reason: %1.';");
	...
	MessageString = NStr("ru = 'Не удалось записать объект %1, полученный из другой информационной базы.%2
		|Возможно не заполнены все реквизиты, обязательные к заполнению.';
		|en = 'Failed to save %1 received from another infobase. %2
		|Some attributes may be required.';",
		Common.DefaultLanguageCode());
...
Procedure SetDataExchangeMessageFromMasterNode(ExchangeMessage, MasterNode) Export
	...
	WriteDataReceivingEvent(MasterNode, NStr("ru = 'Сообщение обмена записано в кэш.';
													|en = 'The exchange message is cached.';"));
...
Procedure ClearDataExchangeMessageFromMasterNode() Export
	...
	WriteDataReceivingEvent(MasterNode(), NStr("ru = 'Сообщение обмена удалено из кэша.';
														|en = 'The exchange message is deleted from the cache.';"));
...
//
// Parameters:
//  ExternalModule - MetadataObjectExchangePlan - Object of "ExchangePlan" that is an external module.
//
// Returns:
...
Function ExternalModuleContainerDictionary() Export
	...
	Result.Insert("Nominative", NStr("ru = 'Настройка синхронизации данных';
											|en = 'Data synchronization setup';"));
	Result.Insert("Genitive",  NStr("ru = 'Настройки синхронизации данных';
											|en = 'Data synchronization settings';"));
...
Procedure ExecuteExchangeActionForInfobaseNodeUsingExternalConnection(Cancel, InfobaseNode,
	...
	If IsXDTOExchangePlan Then
		...
		If CheckNodeExistenceInCorrespondent Then
			...
			If Not ValueIsFilled(ExchangePlanRef.Code) Then
				...
				MessageText = NStr("ru = 'Необходим переход на синхронизацию данных через универсальный формат в базе-корреспонденте.';
										|en = 'Switch the peer infobase to Interim Format Data Exchange.';");
				...
				If ParametersStructure.Error Then
					HasErrors = True;
					NString = NStr("ru = 'Ошибка при переходе на синхронизацию данных через универсальный формат: %1. Обмен отменен.';
									|en = 'Error switching to Interim Format Data Exchange: %1. The exchange is canceled.';",
						Common.DefaultLanguageCode());
				...
				ElsIf TransferResult = Undefined Then
					HasErrors = True;
					ErrorMessageString = NStr("ru = 'Переход на синхронизацию данных через универсальный формат не выполнен';
													|en = 'Switching to Interim Format Data Exchange failed';");
				EndIf;
				If HasErrors Then
					WriteEventLogDataExchange(ErrorMessageString, ExchangeSettingsStructure, True);
					ExchangeSettingsStructure.ExchangeExecutionResult = Enums.ExchangeExecutionResults.Canceled;
					...
					Return;
				Else
					Message = NStr("ru = 'Переход на синхронизацию данных через универсальный формат завершен успешно.';
									|en = 'Switching Interim Format Data Exchange completed.';");
	...
	If ExchangeSettingsStructureExternalConnection.Property("DataSynchronizationSetupCompleted") Then
		If Not MessageForDataMapping
			...
			ErrorMessage = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Для продолжения перейдите в программу ""%1"" и завершите в ней настройку синхронизации.
				|Выполнение обмена данными отменено.';
				|en = 'To continue, set up synchronization in ""%1"".
	...
	If ExchangeSettingsStructureExternalConnection.Property("MessageReceivedForDataMapping") Then
		If Not MessageForDataMapping
			...
			ErrorMessage = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Для продолжения перейдите в программу ""%1"" и выполните загрузку сообщения для сопоставления данных.
				|Выполнение обмена данными отменено.';
	...
	If ExchangeSettingsStructure.DoDataImport Then
		If Not IsXDTOExchangePlan Then
			...
			If ObjectsConversionRules = Undefined Then
				...
				NString = NStr("ru = 'Не заданы правила конвертации во второй информационной базе для плана обмена %1. Обмен отменен.';
								|en = 'Conversion rules are not specified for exchange plan %1 in the second infobase. The exchange is canceled.';",
					Common.DefaultLanguageCode());
		...
		Else
			...
			Except
				WriteEventLogDataExchange(
					StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Возникла ошибка во второй информационной базе: %1';
																				|en = 'Error occurred in peer infobase: %1';"),
	...
	While Selection.Next() Do
		...
		Else
			...
			Else
				...
				MessageString = NStr("ru = 'Начало процесса обмена данными по настройке %1';
										|en = 'Data exchange started. Setting: %1';", Common.DefaultLanguageCode());
...
//    * OperationID - String - contains a long-running operation ID as a string.
...
Procedure BeforeReadExchangeMessage(Val Recipient, ExchangeMessage, StandardProcessing) Export
	...
	If IsSubordinateDIBNode()
		...
		If TypeOf(SavedExchangeMessage) = Type("Structure") Then
			...
			WriteDataReceivingEvent(Recipient, NStr("ru = 'Сообщение обмена получено из кэша.';
															|en = 'An exchange message is received from the cache.';"));
...
Procedure InitExchangeSettingsStructure(ExchangeSettingsStructure, ExchangeExecutionSettings, LineNumber)
	...
	ExchangeSettingsStructure.EventLogMessageKey = NStr("ru = 'Обмен данными';
																	|en = 'Data exchange';");
...
Procedure CheckMainExchangeSettingsStructureFields(ExchangeSettingsStructure)
	...
	ElsIf Not ValueIsFilled(ExchangeSettingsStructure.ExchangeTransportKind) Then
		...
		ErrorMessageString = NStr("ru = 'Не задан вид транспорта обмена. Обмен отменен.';
										|en = 'Exchange transport type is not specified. The data exchange is canceled.';",
			Common.DefaultLanguageCode());
		WriteEventLogDataExchange(ErrorMessageString, ExchangeSettingsStructure, True);
	...
	ElsIf Not ValueIsFilled(ExchangeSettingsStructure.ActionOnExchange) Then
		...
		ErrorMessageString = NStr("ru = 'Не указано выполняемое действие (выгрузка / загрузка). Обмен отменен.';
										|en = 'Direction (export or import) is not specified. The data exchange is canceled.';",
			Common.DefaultLanguageCode());
		WriteEventLogDataExchange(ErrorMessageString, ExchangeSettingsStructure, True);
...
Procedure CheckExchangeStructure(ExchangeSettingsStructure, UseTransportSettings = True)
	...
	ElsIf Common.ObjectAttributeValue(ExchangeSettingsStructure.InfobaseNode, "DeletionMark") Then
		...
		ErrorMessageString = NStr("ru = 'Узел информационной базы помечен на удаление. Обмен отменен.';
										|en = 'The infobase node is marked for deletion. The data exchange is canceled.';",
			Common.DefaultLanguageCode());
		WriteEventLogDataExchange(ErrorMessageString, ExchangeSettingsStructure, True);
	...
	ElsIf IsBlankString(ExchangeSettingsStructure.InfobaseNodeCode1)
		...
		ErrorMessageString = NStr("ru = 'Один из узлов обмена имеет пустой код. Обмен отменен.';
										|en = 'An exchange node contains no code. The data exchange is canceled.';",
			Common.DefaultLanguageCode());
		WriteEventLogDataExchange(ErrorMessageString, ExchangeSettingsStructure, True);
	...
	ElsIf ExchangeSettingsStructure.ExportHandlersDebug Then
		...
		If Not ExportDataProcessorFile.Exists() Then
			...
			ErrorMessageString = NStr("ru = 'Файл внешней обработки для отладки выгрузки не существует. Обмен отменен.';
											|en = 'The data processor file required for debugging does not exist. The data exchange is canceled.';",
				Common.DefaultLanguageCode());
			WriteEventLogDataExchange(ErrorMessageString, ExchangeSettingsStructure, True);
	...
	ElsIf ExchangeSettingsStructure.ImportHandlersDebug Then
		...
		If Not ImportDataProcessorFile1.Exists() Then
			...
			ErrorMessageString = NStr("ru = 'Файл внешней обработки для отладки загрузки не существует. Обмен отменен.';
											|en = 'The data processor file required for debugging does not exist. The data exchange is canceled.';",
				Common.DefaultLanguageCode());
			WriteEventLogDataExchange(ErrorMessageString, ExchangeSettingsStructure, True);
...
Procedure SetDataExportExchangeRules(DataExchangeXMLDataProcessor, ExchangeSettingsStructure)
	...
	If ObjectsConversionRules = Undefined Then
		...
		// Exchange rules must be specified.
		ErrorMessageString = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Не заданы правила конвертации для плана обмена %1. Выгрузка данных отменена.';
				|en = 'Conversion rules are not specified. Exchange plan: %1. The data export is canceled.';", Common.DefaultLanguageCode()),
...
Procedure SetDataImportExchangeRules(DataExchangeXMLDataProcessor, ExchangeSettingsStructure)
	...
	If ObjectsConversionRules = Undefined Then
		...
		If Not ExchangeSettingsStructure.Property("ConversionRulesAreRequired")
			...
			// Exchange rules must be specified.
			NString = NStr("ru = 'Не заданы правила конвертации для плана обмена %1. Загрузка данных отменена.';
							|en = 'Conversion rules are not specified. Exchange plan: %1. The data import is canceled.';",
				Common.DefaultLanguageCode());
...
Procedure OnFillToDoListSynchronizationWarnings(ToDoList)
	...
	For Each Section In Sections Do
		...
		ToDoItem.Presentation  = NStr("ru = 'Предупреждения';
									|en = 'Warnings';");
...
Procedure CheckLoopingWhenFillingOutToDoList(ToDoList)
	...
	For Each Section In Sections Do
		...
		ToDoItem.Presentation  = NStr("ru = 'Обнаружено зацикливание синхронизаций';
									|en = 'Synchronization loop is found';");
...
Procedure OnFillToDoListCheckCompatibilityWithCurrentVersion(ToDoList)
	...
	For Each Section In Sections Do
		...
		ToDoItem.Presentation = NStr("ru = 'Правила обмена';
									|en = 'Exchange rules';");
		...
		If ToDoGroup = Undefined Then
			...
			ToDoGroup.Presentation = NStr("ru = 'Проверить совместимость';
...
Function ExchangePlansForMonitorQueryText(QueryOptions = Undefined, ExcludeStandaloneModeExchangePlans = True) Export
	...
	If DataExchangeCached.StandaloneModeSupported()
		...
		// A separate monitor is used for the exchange plan of a standalone mode.
...
Procedure CheckExternalConnectionAvailability()
	...
	If Common.IsLinuxServer() Then
		...
		Raise NStr("ru = 'Синхронизация данных через прямое подключение на сервере под управлением ОС Linux недоступно.
			...
			|en = 'Data synchronization via direct server connection is not available on Linux.
...
Function UnpackZipFile(Val FullArchiveFileName, Val FilesUnpackPath, Val ArchivePassword = "") Export
	...
	Except
		ErrorInfo = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Ошибка при распаковке файлов архива ""%1"" в каталог ""%2"" по причине:
			|%3';
			|en = 'Cannot extract files from archive ""%1"" to directory ""%2"". Reason:
			|%3';"),
			FullArchiveFileName,
			FilesUnpackPath,
			ErrorProcessing.DetailErrorDescription(ErrorInfo()));
...
Function PackIntoZipFile(Val FullArchiveFileName, Val FilesPackingMask, Val ArchivePassword = "") Export
	...
	Except
		ErrorInfo = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Ошибка при запаковке файлов ""%1"" в архив ""%2"" по причине:
			|%3';
			|en = 'Cannot add files ""%1"" to archive ""%2"". Reason:
			|%3';"),
			FilesPackingMask,
			FullArchiveFileName,
			ErrorProcessing.DetailErrorDescription(ErrorInfo()));
...
Function EventLogMessageKey(InfobaseNode, ActionOnExchange) Export
	...
	MessageKey = NStr("ru = 'Обмен данными.[ExchangePlanName].[ActionOnExchange]';
						|en = 'Data exchange.[ExchangePlanName].[ActionOnExchange]';",
		Common.DefaultLanguageCode());
...
// 
// Returns:
//   Boolean - True if the specified attribute is standard.
//
...
Procedure CheckMandatoryFormAttributes(Form, Val Attributes)
	...
	If AbsentAttributes.Count() > 0 Then
		...
		Raise StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Отсутствуют обязательные реквизиты формы настройки узла: %1';
																						|en = 'Node settings required: %1';"),
...
Procedure ExternalConnectionUpdateDataExchangeSettings(Val ExchangePlanName, Val NodeCode, Val DefaultNodeValues) Export
	...
	If Not ValueIsFilled(InfobaseNode) Then
		Message = NStr("ru = 'Не найден узел плана обмена; имя плана обмена %1; код узла %2';
						|en = 'Node not found. Exchange plan: %1. Node ID: %2';");
...
Function StatisticsInformation(StatisticsInformation, Val EnableObjectDeletion = False) Export
	...
	// Get a tree of metadata objects.
...
Procedure CheckLoadedFromFileExchangeRulesAvailability(ExchangeRulesImportedFromFile, RegistrationRulesImportedFromFile)
	...
	If Not Result.IsEmpty() Then
		...
		MessageString = NStr("ru = 'Для планов обмена %1 используются правила обмена, загруженные из файла.
				...
				|en = 'The exchange plan ""%1"" uses rules exported from a file.
...
Function EstablishExternalConnectionWithInfobase(SettingsStructure_) Export
	...
	If Metadata.ScriptVariant = Metadata.ObjectProperties.ScriptVariant.Russian
		...
		VariantOfBuiltInCorrespondentLanguage = NStr("ru = 'Английский';
													|en = 'English';");
	...
	If Metadata.ScriptVariant = Metadata.ObjectProperties.ScriptVariant.English
		...
		VariantOfBuiltInCorrespondentLanguage = NStr("ru = 'Русский';
													|en = 'Russian';");
		EmbeddedLanguageOptionsAreDifferent = True;
	EndIf;
	...
	If EmbeddedLanguageOptionsAreDifferent Then
		...
		DetailedErrorDetails = NStr("ru = 'В подключаемой программе отличается вариант встроенного языка (%1). Подключение невозможно.';
										|en = 'The application to connect has a different 1C:Enterprise language option (%1). Connection is unavailable.';");
	...
	If NoFullAccess Then
		Result.DetailedErrorDetails = NStr("ru = 'Пользователю, указанному для подключения к другой программе, должны быть назначены роли ""Администратор системы"" и ""Полные права""';
												|en = 'The user on whose behalf connection to the peer application is established must be assigned the ""System administrator"" and ""Full access"" roles.';");
	...
	Else
		...
		If InvalidState Then
			Result.DetailedErrorDetails = NStr("ru = 'Другая программа находится в состоянии обновления.';
													|en = 'Peer application is updating.';");
			Result.BriefErrorDetails   = Result.DetailedErrorDetails;
			Result.Join = Undefined;
		EndIf;
...
Procedure UpdateStandardDataExchangeRuleVersion(ExchangeRulesImportedFromFile, RegistrationRulesImportedFromFile)
	...
	For Each ExchangePlanRecord In Result Do
		...
		If DataExchangeCached.HasExchangePlanTemplate(ExchangePlanName, "ExchangeRules")
			...
			If ExchangeRulesImportedFromFile.Find(ExchangePlanName) = Undefined Then
				...
				MessageText = StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Выполняется обновление правил конвертации данных для плана обмена %1';
						|en = 'Updating data conversion rules. Exchange plan: %1';"), ExchangePlanName);
				WriteLogEvent(DataExchangeEventLogEvent(),
		...
		Else
			...
			MessageText = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Выполняется обновление правил регистрации данных для плана обмена %1';
					|en = 'Updating data registration rules. Exchange plan: %1';"), ExchangePlanName);
	...
	If Cancel Then
		Raise NStr("ru = 'При обновлении правил обмена данными возникли ошибки (см. Журнал регистрации).';
								|en = 'Error updating data exchange rules. See the event log.';");
	EndIf;
	...
	If File.Exists() And File.IsFile() Then
		...
		If MaxMessageSize <> 0 Then
			...
			If PackageSize > MaxMessageSize Then
				...
				MessageString = NStr("ru = 'Размер исходящего пакета составил %1 Кбайт, что превышает допустимое ограничение %2 Кбайт.';
										|en = 'The outgoing package size (%1 KB) exceeds the limit (%2 KB).';");
	...
	If GetFunctionalOption("UseDataSynchronization") Then
		...
		If InfobaseNode <> Undefined Then
			...
			Try
				...
				If Cancel Then
					Raise NStr("ru = 'Получение данных из главного узла завершилось с ошибками.';
...
Function DifferentCorrespondentVersions(ExchangePlanName, EventLogMessageKey, VersionInCurrentApplication,
	...
	If ValueIsFilled(VersionInCurrentApplication) And ValueIsFilled(VersionInOtherApplication)
		...
		If VersionInCurrentApplicationWithoutBuildNumber <> VersionInOtherApplicationWithoutBuildNumber Then
			...
			MessageTemplate = NStr("ru = 'Синхронизация данных может быть выполнена некорректно, т.к. версия программы ""%1"" (%2) в правилах конвертации этой программы отличается от версии %3 в правилах конвертации в другой программе. Убедитесь, что загружены актуальные правила, подходящие для используемых версий обеих программ.';
									|en = 'Data synchronization for ""%1"" might cause errors. The application versions specified in the conversion rules do not match. Version in this app: %2. Version in peer app: %3. Ensure that the rules support both versions.';");
...
Procedure OnFillToDoListUpdateRequired(ToDoList)
	...
	For Each Section In Sections Do
		...
		ToDoItem.Important         = True;
		ToDoItem.Presentation  = NStr("ru = 'Обновить версию программы';
									|en = 'Update application version';");
		...
		Else
			...
			ToDoItem.FormParameters = New Structure("Title,TemplateName",
				NStr("ru = 'Установка обновления';
					|en = 'Install update';"), "ManualUpdateInstruction");
		EndIf;
		ToDoItem.Owner       = Section;
...
Function StatisticsTreeRowDataSynonym(TreeRow, SourceTypeString) 
	...
	If Count = 0 Or (Count = 1 And Existing[0] = TreeRow) Then
		// There has been no such description in this tree.
		Return Synonym;
	EndIf;
...
// Returns:
//   ValueTable - A collection of registers with the specified recorder records:
...
Function DetermineDocumentHasRegisterRecords(DocumentRef)
	...
	// If the number of tables is more than 256, make an additional query and supplement the table with rows.	
	QueryText = "";
	For Each Movement In MetadataOfDocument.RegisterRecords Do
...
//            Key - String - The metadata item property name.
//            Value - Array - An array of filter values.
//
// An example of "Filter" variable initialization
//
// Array = New Array;
...
// 
// Returns:
//   ValueTree - a configuration metadata tree.
//
Function ConfigurationMetadataTree(Filter = Undefined) Export
	...
	NewMetadataObjectsCollectionRow("Constants",               NStr("ru = 'Константы';
																			|en = 'Constants';"),                 PictureLib.Constant,              PictureLib.Constant,                    MetadataObjectsCollections);
	NewMetadataObjectsCollectionRow("Catalogs",             NStr("ru = 'Справочники';
																			|en = 'Catalogs';"),               PictureLib.Catalog,             PictureLib.Catalog,                   MetadataObjectsCollections);
	NewMetadataObjectsCollectionRow("Documents",               NStr("ru = 'Документы';
																			|en = 'Documents';"),                 PictureLib.Document,               PictureLib.DocumentObject,               MetadataObjectsCollections);
	NewMetadataObjectsCollectionRow("ChartsOfCharacteristicTypes", NStr("ru = 'Планы видов характеристик';
																			|en = 'Charts of characteristic types';"), PictureLib.ChartOfCharacteristicTypes, PictureLib.ChartOfCharacteristicTypesObject, MetadataObjectsCollections);
	NewMetadataObjectsCollectionRow("ChartsOfAccounts",             NStr("ru = 'Планы счетов';
																			|en = 'Charts of accounts';"),              PictureLib.ChartOfAccounts,             PictureLib.ChartOfAccountsObject,             MetadataObjectsCollections);
	NewMetadataObjectsCollectionRow("ChartsOfCalculationTypes",       NStr("ru = 'Планы видов расчета';
																			|en = 'Charts of calculation types';"),       PictureLib.ChartOfCalculationTypes,       PictureLib.ChartOfCalculationTypesObject,       MetadataObjectsCollections);
	NewMetadataObjectsCollectionRow("InformationRegisters",        NStr("ru = 'Регистры сведений';
																			|en = 'Information registers';"),         PictureLib.InformationRegister,        PictureLib.InformationRegister,              MetadataObjectsCollections);
	NewMetadataObjectsCollectionRow("AccumulationRegisters",      NStr("ru = 'Регистры накопления';
																			|en = 'Accumulation registers';"),       PictureLib.AccumulationRegister,      PictureLib.AccumulationRegister,            MetadataObjectsCollections);
	NewMetadataObjectsCollectionRow("AccountingRegisters",     NStr("ru = 'Регистры бухгалтерии';
																			|en = 'Accounting registers';"),      PictureLib.AccountingRegister,     PictureLib.AccountingRegister,           MetadataObjectsCollections);
	NewMetadataObjectsCollectionRow("CalculationRegisters",         NStr("ru = 'Регистры расчета';
																			|en = 'Calculation registers';"),          PictureLib.CalculationRegister,         PictureLib.CalculationRegister,               MetadataObjectsCollections);
	NewMetadataObjectsCollectionRow("BusinessProcesses",          NStr("ru = 'Бизнес-процессы';
																			|en = 'Business processes';"),           PictureLib.BusinessProcess,          PictureLib.BusinessProcessObject,          MetadataObjectsCollections);
	NewMetadataObjectsCollectionRow("Tasks",                  NStr("ru = 'Задачи';
																			|en = 'Tasks';"),                    PictureLib.Task,                 PictureLib.TaskObject,                 MetadataObjectsCollections);
...
//                            - UUID - an address for saving data between server calls.
//     HasNodeScenario       - Boolean                          - a flag showing whether additional setup is required.
//
// Returns:
//   Structure - Data required for handling the import difference:
//     * InfobaseNode - ExchangePlanRef - a reference to the node to be configured.
...
//     * FilterAsString - String - the filter value as a string.
...
// Returns:
//   Structure - Settings of the default export add-on option "Do not add":
...
Function StandardOptionDetailsWithoutAddition()
	...
	Result.Insert("Title",     "");
	Result.Insert("Explanation",     NStr("ru = 'Будут отправлены только данные согласно общим настройкам.';
											|en = 'Send only data selected using the common settings.';"));
...
// Returns:
//   Structure - Settings of the default export add-on option "Add all documents for the period":
//     * Use - Boolean - indicates that the option is allowed to use. The default value is True.
...
Function StandardOptionDetailsAllDocuments()
	...
	Result.Insert("Title",     "");
	Result.Insert("Explanation",     NStr("ru = 'Дополнительно будут отправлены все документы за период, удовлетворяющие условиям отбора.';
											|en = 'Also, send all documents that match the filter.';"));
...
// Returns:
//   Structure - Settings of the default export add-on option "Add data with arbitrary filter":
//     * Use - Boolean - indicates that the option is allowed to use. The default value is True.
...
Function StandardOptionDetailsCustomFilter()
	...
	Result.Insert("Title",     "");
	Result.Insert("Explanation",     NStr("ru = 'Дополнительно будут отправлены данные согласно отбору.';
											|en = 'Also, send all data that matches the filter.';"));
...
//
// Returns:
//   Structure - Settings of the default export add-on option by the node scenario:
...
// Parameters:
//     ExportAddition - Structure
//                        - FormDataStructure - Export parameters details:
...
Function ExportAdditionFilterPresentation(Val Period, Val Filter, Val EmptyFilterDetails=Undefined) Export
	...
	If IsBlankString(FilterAsString) Then
		If EmptyFilterDetails=Undefined Then
			FilterAsString = NStr("ru = 'Все объекты';
								|en = 'All objects';");
		Else
...
Function DetailedExportAdditionPresentation(Val AdditionalRegistration, Val EmptyFilterDetails=Undefined) Export
	...
	ElsIf EmptyFilterDetails=Undefined Then
		Return NStr("ru = 'Дополнительные данные не выбраны';
					|en = 'No additional data is selected';");
...
//
// Returns:
//     Array of Number - The numbers of the used filters: 
//               0 - No filters. 1 - Filter all documents. 2 - Detailed filter. 3 - A node scenario.
//
Function InteractiveExportChangeOptionFilter(Val ExportAddition) Export
	...
	If TypeOf(AdditionScenarioParameters)<>Type("Structure") Then
		// If there are no settings specified, using all options as the default settings.
		Return Undefined;
	EndIf;
	...
	If Result.Count() = 4 Then
		// All options are selected, removing a filter.
		Return Undefined;
	EndIf;
...
Procedure SetUpLoopFormElements(Form)
	...
	TextTemplate1 = NStr("ru = '<br>Обнаружено зацикливание синхронизаций. Для получения подробной информации перейдите по 
			  |<a href=""%1"">ссылке</a>.
			  |<br><br>
			  |<a href=""%2"">Объекты незарегистрированные при зацикливании</a>.';
				|en = '<br>Synchronization loop is found. For more information, follow the 
				|<a href=""%1"">link</a>.
				|<br><br>
