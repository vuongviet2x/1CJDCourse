...
Function GenerateInfoLabelMessageCharsCount(SendInTransliteration, MessageText) Export
	...
	MessageTextTemplate = NStr("ru = 'Сообщение - %1, осталось символов - %2';
								|en = 'Messages: %1. Symbols left: %2';");
...
Function GetFileSizeStringPresentation(SizeInBytes) Export
	...
	If SizeMB > 1 Then
		StringSize = Format(SizeMB, "NFD=1") + " " + NStr("ru = 'Мб';
																|en = 'MB';");
	Else
		StringSize = Format(SizeInBytes /1024, "NFD=0; NZ=0") + " " + NStr("ru = 'Кб';
																				|en = 'kB';");
	EndIf;
...
Procedure OnChangeFilterInteractionType(Form,InteractionType) Export
	...
	FilterGroup = CommonClientServer.CreateFilterItemGroup(
		Filter.Items, NStr("ru = 'Отбор по типу взаимодействий';
							|en = 'Filter by interaction category';"), DataCompositionFilterItemsGroupType.AndGroup);
...
Function InteractionSubject1(Subject) Export
	...
	Return ?(IsBlankString(Subject), NStr("ru = '<Без темы>';
										|en = '<No Subject>';"), Subject);
