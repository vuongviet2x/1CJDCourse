...
Function LongSynchronizationQuestionSetupFlag(FlagValue1 = Undefined, SettingDetails = Undefined) Export
	...
	SettingDetails.Insert("Presentation", NStr("ru = 'Показывать предупреждение о длительной синхронизации';
...
Function InstallPackageFileName() Export
	...
	Return NStr("ru = 'Автономная работа.zip';
...
Function StandaloneWorkstationsMonitor() Export
	...
	For Each SyncSetup In SynchronizationSettings Do
		...
		Else
			SyncSetup.SynchronizationDatePresentation = NStr("ru = 'не выполнялась';
																		|en = 'not performed';");
		EndIf;
...
Function StandaloneWorkstationCreationEventLogMessageText() Export
	...
	Return NStr("ru = 'Автономная работа.Создание автономного рабочего места';
				|en = 'Standalone mode.Create standalone workstation';", Common.DefaultLanguageCode());
...
Function StandaloneWorkstationDeletionEventLogMessageText() Export
	...
	Return NStr("ru = 'Автономная работа.Удаление автономного рабочего места';
				|en = 'Standalone mode.Delete standalone workstation';", Common.DefaultLanguageCode());
...
Procedure SynchronizeDataWithWebApplication() Export
	...
	If Not IsStandaloneWorkplace() Then
		...
		DetailErrorDescriptionForEventLog =
			NStr("ru = 'Эта информационная база не является автономным рабочим местом. Синхронизация данных отменена.';
				|en = 'This infobase is not a standalone workstation. Data synchronization is canceled.';",
	...
	If Cancel Then
		Raise NStr("ru = 'В процессе синхронизации данных с приложением в Интернете возникли ошибки (см. журнал регистрации).';
...
Procedure PerformStandaloneWorkstationSetupOnFirstStart(DataImport = False) Export
	...
	If Not Common.FileInfobase() Then
		Raise NStr("ru = 'Первый запуск автономного рабочего места должен выполняться в файловой информационной базе.';
								|en = 'The first start of a standalone workstation must be performed in a file infobase.';");
	EndIf;
...
Procedure DeleteObsoleteExchangeMessages(StandaloneWorkstation) Export 
	...
	EventLogEvent = NStr("ru = 'Обмен данными';
...
Procedure WriteInitialImageDataElement(DataElement, MetadataProperties1, Recipient) Export 
	...
	Except
		...
		If Common.IsRefTypeObject(ItemMetadata) Then
			Template = NStr("ru = 'Навигационная ссылка: %1';
							|en = 'URL: %1';", Common.DefaultLanguageCode());
			Ref = StrTemplate(Template, GetURL(DataElement.Ref));
		ElsIf Common.IsRegister(ItemMetadata) Then
			Template = NStr("ru = 'Отбор: %1';
							|en = 'Filter: %1';", Common.DefaultLanguageCode());
...
Function DefaultStandaloneWorkstationDescription()
	...
	Result = NStr("ru = 'Автономная работа - %1';
...
Function StandaloneWorkstationPrefixAllowedChars()
	...
	Return NStr("ru = 'АБВГДЕЖЗИКЛМНОПРСТУФХЦЧШЭЮЯабвгдежзиклмнопрстуфхцчшэюя';
				|en = 'ABCDEFGHIKLMNOPQRSTVXYZabcdefghiklmnopqrstvxyz';"); // 54 characters.
...
Procedure DoImportParametersFromInitialImage()
	...
	Except
		...
		Raise NStr("ru = 'Возможно, информационная база открыта в режиме конфигуратора.
	...
	Try
		...
		If User = Undefined Then
			Raise NStr("ru = 'Идентификация пользователя не выполнена.
		...
		User.AdditionalProperties.Insert("CreateAdministrator",
			NStr("ru = 'Начальная настройка автономного рабочего места.';
				|en = 'Initial standalone workstation setup.';"));
		User.Write();
...
Function GetOrderedListDataFiles(InfobaseDirectory)
	...
	For Each DataArchiveFile In FoundDataArchiveFiles Do
		...
		If FileNameParts.Count() <> 2 Then
			Raise NStr("ru = 'Неверный формат имени файла данных';
									|en = 'File name invalid format';");
		EndIf;
...
Function GetParametersFromInitialImage()
	...
	If IsBlankString(XMLLine) Then
		Raise NStr("ru = 'В автономное рабочее место не были переданы настройки.
									|Работа с автономным рабочим место невозможна.';
									|en = 'Settings were not transferred to a standalone workstation.
...
Function ReadDataToStructure(XMLReader)
	...
	If XMLReader.NodeType <> XMLNodeType.StartElement Then
		Raise NStr("ru = 'Ошибка чтения XML';
...
Function DataSynchronizationEventLogEvent()
	...
	Return NStr("ru = 'Автономная работа.Синхронизация данных';
				|en = 'Standalone mode.Data synchronization';", Common.DefaultLanguageCode());
