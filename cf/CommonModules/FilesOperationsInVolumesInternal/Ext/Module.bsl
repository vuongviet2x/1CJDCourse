...
Procedure FillInTheFileDetails(AttachedFile, BinaryDataOrPath, 
	...
	If TypeOf(AttachedFile) = Type("Structure")
		...
		Raise(StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Не заполнено значение свойства %1 в параметре %2 (Структура) в процедуре %3.';
				|en = 'Blank value of the %1 property the %2 parameter (Structure) of the %3 procedure.';"),
			"Ref",
			"AttachedFile",
	...
	If TypeOf(BinaryDataOrPath) = Type("String") Then
		...
		Else
			...
			NStr("ru = 'Не удалось добавить файл ""%1"" ни в один из томов, т.к. он отсутствует.
				...
				|en = 'Cannot add file ""%1"" to a volume as the file is missing.
...
Procedure CopyAttachedFile(AttachedFile, FilePathDestination) Export
	...
	If Not SourceFile1.Exists() Then
		ErrorMessage = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Данные файла были удалены. Возможно, файл очищен как ненужный или удален антивирусом.
				|%1';
				|en = 'File data was deleted. The file might have been cleaned up as unused or deleted by the antivirus software.
...
Function DeleteFile(PathToFile) Export
	...
	If FileOnHardDrive.Exists() Then
		...
		Except
			...
			WriteLogEvent(
				NStr("ru = 'Файлы.Удаление файлов в томе';
					|en = 'Files.Delete files from volume';", Common.DefaultLanguageCode()),
				EventLogLevel.Error,,,
...
Function FullFileNameInVolume(FileProperties, FileDateInVolume = Undefined) Export
	...
	Except
		...
		If Not Volume.Exists() Or Not Volume.IsDirectory() Then
			Raise StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Не существует сетевой каталог для тома хранения файлов ""%1"": %2
					|Обратитесь к администратору.';
					|en = 'A network directory for the ""%1"" storage volume does not exist: %2
...
Procedure OnDefineChecks(ChecksGroups, Checks) Export
	...
	Validation.Description                 = NStr("ru = 'Поиск ссылок на несуществующие файлы в томах хранения';
												|en = 'Search for references to missing files in storage volumes';");
	Validation.Reasons                      = NStr("ru = 'Файл был физически удален или перемещен на компьютере вследствие работы антивируса,
		|непреднамеренных действий администратора и.т.д.';
		|en = 'The file was deleted or moved by the antivirus software,
		|unintentional actions of the administrator, or similar reasons.';");
	Validation.Recommendation                 = NStr("ru = '• Пометить файл в приложении на удаление;
...
Function SetFilesStoragePaths(FilesToRecover, VolumePath) Export
	...
	For Each Batch In Portions Do
		...
		Except
			RollbackTransaction();
			WriteLogEvent(
				NStr("ru = 'Файлы.Восстановление файлов в томе';
					|en = 'Files.File recovery in volume';", Common.DefaultLanguageCode()),
				EventLogLevel.Error,,,
...
Procedure WriteTheFileDataToTheVolume(AttachedFile, BinaryDataOrPath)
	...
	Except
		...
		ErrorDescriptionTemplate = NStr("ru = 'Ошибка при добавлении файла ""%1""
			|в том ""%2"" (%3):
			|""%4"".';
		...
		WriteLogEvent(NStr("ru = 'Файлы.Добавление файла';
										|en = 'Files.Add file';", Common.DefaultLanguageCode()),
			EventLogLevel.Error,,,
...
Procedure ClearDeletedFilesInTheVolume(Directory)
	...
	Except
		RollbackTransaction();
		Error = ErrorInfo();
		WriteLogEvent(NStr("ru = 'Работа с файлами.Очистка файлов';
										|en = 'File management.File cleanup';", Common.DefaultLanguageCode()),
			EventLogLevel.Error,
...
Function ViewStatusChecks(Val CheckStatus) Export
	If CheckStatus = "OK" Then
		Return NStr("ru = 'Целостные данные';
					|en = 'Data integrity check passed';");
	ElsIf CheckStatus = "ExtraFileInTome" Then 
		Return NStr("ru = 'Лишние файлы (есть в томе, но сведения о них отсутствуют)';
					|en = 'Unreferenced files (files in the volume that have no entries in the application)';");
	ElsIf CheckStatus = "NoFileInVolume" Then
		Return NStr("ru = 'Отсутствуют файлы в томе';
					|en = 'No files in the volume';");
	ElsIf CheckStatus = "FixingPossible" Then
		Return NStr("ru = 'Поврежденные сведения о файлах';
					|en = 'Corrupted file info';");
	EndIf;
EndFunction
...
Function VolumeAvailable(Volume, VolumePresentation, Path, CheckParameters)
	...
	If IsBlankString(Path) Then
		...
		IssueSummary = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'У тома хранения файлов ""%1"" не задан путь к сетевому каталогу. Сохранение файлов в него невозможно.';
				|en = 'Couldn''t save files to file storage volume ""%1"" as it does not have a path to the network directory.';"), 
			VolumePresentation);
	...
	While Result.Count() > 0 Do
		...
		For Each ResultString1 In Result Do
			...
			If ResultString1.Owner <> Undefined Then
				IssueSummary = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Версия ""%1"" файла ""%2"" не существует в томе ""%3"".';
																								|en = 'Version ""%1"" of file ""%2"" does not exist in volume ""%3.""';"),
					ResultString1.File, ResultString1.Owner, ResultString1.Volume);
			Else
				IssueSummary = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Файл ""%1"" не существует в томе ""%2"".';
																								|en = 'File ""%1"" does not exist in volume ""%2.""';"),
...
Procedure OnAddUpdateHandlers(Handlers) Export
	...
	Handler.Comment = NStr("ru = 'Исправляет ошибочные пути хранения файлов в томе.';
									|en = 'Fixes incorrect file storage paths in a volume.';");
	...
	While Selection.Next() Do
		...
		Except
			...
			MessageText = StringFunctionsClientServer.SubstituteParametersToString(
				...
				|%2';
				|en = 'Couldn''t process file storage volume %1. Reason:
				|%2';"), 
...
Function FreeVolume(AttachedFile)
	...
	If Result.IsEmpty() Then
		ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Не удалось добавить файл ""%1"", 
				|т.к. не настроено ни одного тома для хранения файлов.
				|Обратитесь к администратору.';
				|en = 'Cannot add the %1 file 
