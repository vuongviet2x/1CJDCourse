...
#Else
			Raise ErrorProcessing.BriefErrorDescription(ErrorInfo()) + Chars.LF + Chars.LF
				+ StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'В случае ошибки о несоответствии версии компоненты ""comcntr"" зарегистрируйте ее на компьютере %1
					|для учетной записи ОС Windows, от имени которой выполняется 1С:Предприятие. Например:
					|regsvr32.exe ""%2\comcntr.dll""';
					|en = 'If the comcntr version mismatch error occurs, register comcntr on computer %1
...
#If Server Or ThickClientOrdinaryApplication Or ExternalConnection Then 
	If SafeMode() <> False Then
		Raise NStr("ru = 'Внимание! Администрирование кластера невозможно в безопасном режиме';
								|en = 'Warning! Cluster administration is unavailable in safe mode.';");
	EndIf;
	
	If Common.DataSeparationEnabled() Then
		Raise NStr("ru = 'Внимание! В модели сервиса недопустимо выполнение прикладной информационной базой функций администрирования кластера';
								|en = 'Warning! The infobase features related to cluster administration are unavailable in SaaS mode.';");
	EndIf;
	
	Return New COMObject(CommonClientServer.COMConnectorName());
#ElsIf MobileClient Then
	Raise NStr("ru = 'Внимание! Администрирование кластера невозможно в мобильном клиенте';
							|en = 'Warning! The mobile client does not support cluster administration.';");
#Else
	Return New COMObject(CommonClientServer.COMConnectorName());
#EndIf
	...
	For Each InfoBase In IWorkingProcessConnection.GetInfoBases() Do
		...
		If Lower(InfoBase.Name) = Lower(NameInCluster) Then
			...
			If Not ValueIsFilled(InfoBase.DBMS) Then
				...
				Raise StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Неправильные имя и пароль администратора информационной базы %1 в кластере серверов %2:%3 (имя: ""%4"").';
