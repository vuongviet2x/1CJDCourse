...
Procedure ExecuteCommandInBackground(Val CommandID, Val CommandParameters, Val Handler) Export
	...
	If Not ValueIsFilled(AccompanyingText1) Then
		AccompanyingText1 = NStr("ru = 'Команда выполняется.';
									|en = 'Command running.';");
	EndIf;
...
Procedure ShowNotificationOnCommandExecution(CommandToExecute)
	If CommandToExecute.ShouldShowUserNotification Then
		ShowUserNotification(NStr("ru = 'Команда выполняется...';
											|en = 'Command running…';"), , CommandToExecute.Presentation);
	EndIf;
EndProcedure
...
Procedure ExportToFile(ExportingParameters) Export
	...
	SavingParameters.SuggestionText = NStr("ru = 'Для сохранения внешней обработки (отчета) в файл рекомендуется установить расширение для работы с 1С:Предприятием.';
												|en = 'It is recommended that you install 1C:Enterprise Extension before you save the external report or data processor to a file.';");
