	...
	For Each Batch In TransferQueue Do
		...
		Except
			WriteLogEvent(NStr("ru = 'Перевод текста';
											|en = 'Translator';", Common.DefaultLanguageCode()), EventLogLevel.Error,
			...
			Else
				ErrorText = NStr("ru = 'Не удалось выполнить операцию. Обратитесь к администратору.';
									|en = 'Cannot perform the operation. Contact the Administrator.';");
			EndIf;
...
Procedure CheckSettings()
	...
	If TextTranslationServiceModule = Undefined Or Not TextTranslationServiceModule.SetupExecuted() Then
		Raise NStr("ru = 'Не указаны настройки сервиса перевода текстов.';
								|en = 'Text translation service settings are not specified.';");
	EndIf;
...
Function SplitTextByDelimiter(Val Text, Val TextPartsMaxSize, Val Separators)
	...
	For IndexOf = 0 To PieceOfText.UBound() Do
		...
		If FragmentSize > TextPartsMaxSize Then
			...
			Else
				Raise NStr("ru = 'Не удалось разделить текст на фрагменты.';
										|en = 'Cannot split the text into parts.';");
			EndIf;
