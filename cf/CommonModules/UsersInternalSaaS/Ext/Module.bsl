...
Function GetActionsWithSaaSUser(Val User = Undefined,
	...
	Else
		If HasRightToAddUsers() Then
			Return ActionsWithNewSaaSUser();
		Else
			ErrorText = NStr("ru = 'Недостаточно прав доступа для добавления новых пользователей';
								|en = 'Insufficient rights to add users';");
...
Procedure BeforeStartIBUserProcessing(Val UserObject, ProcessingParameters) Export
	...
	If TypeOf(UserObject) = Type("CatalogObject.ExternalUsers") Then
		ErrorText = NStr("ru = 'Внешние пользователи не поддерживаются в модели сервиса.';
							|en = 'SaaS applications don''t support external users.';");
		Raise ErrorText;
	EndIf;
	...
	If AdditionalProperties.Property("RemoteAdministrationChannelMessageProcessing") Then
		...
		If Not ModuleSaaSOperations.SessionWithoutSeparators() Then
			ErrorText =
				NStr("ru = 'Изменение сведений о пользователях через удаленное администрирование
				           |доступно только неразделенным пользователям.';
							|en = 'Only shared users can edit
	   ...
	   And AutoAttributes.ServiceUserID <> OldUser.ServiceUserID Then
		...
		If ValueIsFilled(OldUser.ServiceUserID) Then
			ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Запрещено изменять идентификатор пользователя сервиса для ""%1"".';
					|en = 'Cannot change the service user ID for ""%1"".';"),
		...
		If UsersInternal.UserByIDExists(
			...
			ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Запрещено устанавливать идентификатор пользователя сервиса ""%1"" для ""%2"",
				           |т.к. он уже указан в другом ""%3"".';
							|en = 'Cannot set the ""%1"" service user ID for ""%2""
...
Procedure BeforeEndIBUserProcessing(UserObject, ProcessingParameters) Export
	...
	If AutoAttributes.ServiceUserID <> UserObject.ServiceUserID Then
		ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Не допускается изменять реквизит %1 для ""%2"".
			           |Значение реквизита обновится автоматически.';
						|en = 'Cannot change the %1 attribute for ""%2"".
...
Function ActionsWithExsistingSaaSUser(Val User)
	...
	If Not Common.SubsystemExists("CloudTechnology.Core") Then
		ErrorText = NStr("ru = 'Работа в модели сервиса не предусмотрена.';
							|en = 'SaaS mode is not supported.';");
		Raise ErrorText;
	EndIf;
	...
	For Each ObjectAccessRightsXDTO In ObjectsAccessRightsXDTO.Item Do
		...
		ElsIf ObjectAccessRightsXDTO.Object.Type() = ContactInformationType Then
			...
			If ContactInformationKind = Undefined Then
				ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Неизвестный вид контактной информации: %1';
						|en = 'Unknown contact information kind: %1';"),
		...
		Else
			...
			ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Неизвестный тип объектов доступа: %1';
					|en = 'Unknown access object type: %1';"), TypePresentation);
			Raise ErrorText;
		EndIf;
...
Function InternalUserFullName(Val Id = Undefined) Export
	...
	Result = "<" + NStr("ru = 'Служебный пользователь %1';
							|en = 'Utility user ""%1""';") + ">";
...
Function IsSharedIBUser()
	...
	If Common.SeparatedDataUsageAvailable() Then
		...
		If Not UserRegisteredAsShared(UserIdentificator) Then
			ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Пользователь с идентификатором %1 не зарегистрирован в качестве неразделенного.';
					|en = 'User with ID %1 is not a shared user.';"),
...
Function SharedUserCannotBeWrittenExceptionText()
	...
	Return NStr("ru = 'Запись неразделенных пользователей при включенном использовании разделителей запрещена.';
				|en = 'It is prohibited to save shared users when the separator use is enabled.';");
