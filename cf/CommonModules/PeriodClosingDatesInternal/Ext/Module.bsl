	...
	For Each TableRow In DataToCheck Do
		...
		If SectionProperties = Undefined Then
			ErrorText = ErrorTitle + StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'В параметре %1 указан несуществующий раздел ""%2"".';
					|en = '%1 parameter contains non-existent section: ""%2"".';"),
				"DataToCheck",
				TableRow.Section);
			Raise ErrorText;
		EndIf;
...
Function ErrorTextImportRestrictionDatesNotImplemented() Export
	...
	Return NStr("ru = 'Даты запрета загрузки данных прошлых периодов из других приложений
	                   |не предусмотрены ни для одного плана обмена.';
						|en = 'No exchange plan is subject to data import restrictions
			...
			If PredefinedItemsNames.Find(Selection.PredefinedDataName) <> Undefined Then
				...
				Else
					...
					ElsIf Selection.Ref <> SectionProperties.Ref Then
						...
						Object.Description = "(" + NStr("ru = 'не используется';
														|en = 'not applicable';") + ") " + SectionProperties.Presentation;
					EndIf;
				EndIf;
...
Function PeriodEndMessages(PeriodEnds, PeriodEndMessageParameters, SectionsProperties, SearchImportRestrictions, StructuralDetails)
	...
	If ValueIsFilled(Text) Then
		If SearchImportRestrictions Then
			If NewVersion Then
				Template = NStr("ru = '%1 невозможно загрузить в запрещенный период.';
								|en = 'Cannot import %1 after period-end closing date.';");
			Else
				Template = NStr("ru = '%1 в запрещенном периоде невозможно заменить загружаемыми данными.';
								|en = 'Imported data cannot replace %1 after period-end closing date.';");
			EndIf;
		Else
			If NewVersion Then
				Template = NStr("ru = '%1 невозможно поместить в запрещенный период.';
								|en = 'Cannot store %1 after period-end closing date.';");
			ElsIf IsDataSetDeletion(PeriodEndMessageParameters.Data) Then
				Template = NStr("ru = '%1 невозможно удалить в запрещенном периоде.';
								|en = 'Cannot delete %1 after period-end closing date.';");
			Else
				Template = NStr("ru = '%1 невозможно изменить в запрещенном периоде.';
								|en = 'Cannot change %1 after period-end closing date.';");
			EndIf;
		EndIf;
...
Function PeriodEndMessage(Val Prohibition, Val SearchImportRestrictions, Val SectionsProperties, Val BlankSection)
	...
	If Not SearchImportRestrictions Then
		If      Not BySection And Not ByObject And ForUnspecifiedOne Then
			...
			Text = NStr("ru = 'Запрещено изменять данные с датой ранее %5 (включительно).
			|[SeparatorReadMore]Подробнее:
			|Изменение недоступно, поскольку дата %1 входит в запрещенный период по %5';
			|en = 'Cannot change data with a date earlier than %5 (inclusive).
	...
	Else
		If      Not BySection And Not ByObject And ForInfobase Then
			...
			Text = NStr("ru = 'Запрещено загружать данные с датой ранее %5 (включительно).
			|[SeparatorReadMore]Подробнее:
			|Для информационной базы ""%4"" запрещена загрузка данных,
			|поскольку дата %1 входит в запрещенный период по %5';
			|en = 'Cannot import data with a date earlier than %5 (inclusive).
			|[SeparatorReadMore]Details:
	...
	If Not SectionsProperties.NoSectionsAndObjects Then
		If ValueIsFilled(Prohibition.Section) Then
			If Prohibition.Object = Prohibition.Section Then
				Text = Text + " " + NStr("ru = '(запрет установлен на раздел ""%2"")';
											|en = 'The restriction is applied to section %2.';");
			ElsIf ValueIsFilled(SectionsProperties.SingleSection) Then
				Text = Text + " " + NStr("ru = '(запрет установлен на объект ""%3"")';
											|en = 'The restriction is applied to object %3.';");
			Else
				Text = Text + " " + NStr("ru = '(запрет установлен на объект ""%3"" раздела ""%2"")';
											|en = 'The restriction is applied to object %3 of section %2.';");
			EndIf;
		Else
			Text = Text + " " + NStr("ru = '(установлена общая дата запрета)';
										|en = 'Common-date restriction is applied.';");
		EndIf;
	EndIf;
		...
		If MetadataObject = Undefined Then
			ErrorText = ErrorTitle + StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Для проверки запрета изменения в источнике данных неверно указана таблица
				           |""%1"".';
							|en = 'Period-end closing date check error. Data source contains invalid table:
							|%1.';"),
				TableRow.Table);
			Raise ErrorText;
		EndIf;
			...
			If SectionProperties = Undefined Then
				ErrorText = ErrorTitle + StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Для проверки запрета изменения в источнике данных указан несуществующий
					           ...
					           |(см. %3).';
								|en = 'Non-existent section ""%1"" for table ""%2"" is specified
		...
		If FieldKind = "DateField" Then
			ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Для проверки запрета изменения 
				           ...
				           |не задано поле даты.';
							|en = 'Period-end closing date check error.
		...
		ErrorText =
			NStr("ru = 'Выгрузка или загрузка записей регистра сведений ""Даты запрета изменения""
			           |допускается только в разрезе измерения Пользователь.';
						|en = 'Period-end closing dates information register supports
