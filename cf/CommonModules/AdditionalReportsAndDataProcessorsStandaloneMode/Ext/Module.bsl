	...
	If Common.IsStandaloneWorkplace() Then
		...
		If (Source.DeletionMark Or Source.Publication = Enums.AdditionalReportsAndDataProcessorsPublicationOptions.isDisabled) And IsServiceProcessing(Source.Ref) Then
			...
			Raise StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Дополнительный отчет или обработка %1 был загружен из сервиса и не может быть отключен из автономного рабочего места.
					...
					|en = 'Additional report or data processor %1 was imported from the service and cannot be disabled from the standalone workstation.
