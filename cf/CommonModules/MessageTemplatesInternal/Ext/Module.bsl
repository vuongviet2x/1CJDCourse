...
Function GenerateMessageAndSend(SendOptions) Export
	...
	If SendOptions.Template.ForSMSMessages Then
		If Message.Recipient.Count() = 0 Then
			Result.ErrorDescription  = NStr("ru = 'Для отправки сообщения введите номера телефонов получателей.';
											|en = 'To send the message, enter recipient phone numbers.';");
			Return Result;
		EndIf;
		...
		If Common.SubsystemExists("StandardSubsystems.SendSMSMessage") Then
			...
			Else
				...
				Result.ErrorDescription = NStr("ru = 'Сообщение SMS не может быть отправлено сразу.';
												|en = 'Cannot send the text message right away.';");
	...
	ElsIf SendOptions.Template.ForEmails Then
		If Message.Recipient.Count() = 0 Then
			Result.ErrorDescription  = NStr("ru = 'Введите адрес электронной почты, чтобы отправить сообщение сразу.';
											|en = 'Enter an email address to send the message right away.';");
			Return Result;
		EndIf;
		...
		If Common.SubsystemExists("StandardSubsystems.EmailOperations") Then
			...
			Else
				...
				Result.ErrorDescription  = NStr("ru = 'Сообщение не может быть отправлено сразу.';
												|en = 'Cannot send the message right away.';");
				Return Result;
...
Procedure OnDefineCommandsAttachedToObject(FormSettings, Sources, AttachedReportsAndDataProcessors, Commands) Export
	...
	If AvailableOptions.Mail Then
		...
		Command.Kind = "Send";
		Command.Presentation = NStr("ru = 'Почта';
									|en = 'Email';");
	...
	If AvailableOptions.SMS Then
		Command = Commands.Add();
		Command.Kind = "Send";
		Command.Presentation = NStr("ru = 'SMS';
									|en = 'SMS';");
		Command.WriteMode = "NotWrite";
		Command.Order = 50;
		Command.ParameterType = Metadata.DefinedTypes.MessageTemplateSubject.Type;
...
Procedure DefineAttributesAndAttachmentsList(TemplateInfo, TemplateParameters)
	...
	If ValueIsFilled(TemplateParameters.FullAssignmentTypeName) Then
		...
		If MetadataObject3 <> Undefined Then
			...
			ObjectReference.Presentation = NStr("ru = 'Ссылка на';
												|en = 'Ref to';") + " """ + Presentation + """";
			ObjectReference.Name           = Prefix + "ExternalObjectRef";
			ObjectReference.Type  = New TypeDescription("String");
			ObjectReference.FullPresentation = Presentation + "." + NStr("ru = 'Ссылка на';
																			|en = 'Ref to';") + " """ + Presentation + """";
	...
	For Each ArbitraryParameter In TemplateParameters.Parameters Do
		...
		If ArbitraryParameter.Value.TypeDetails.Types().Count() > 0 Then
			...
			Else
				Arbitrary_ParametersPresentation = NStr("ru = 'Произвольные';
												|en = 'Custom';");
...
Function CommentByTemplateDescription(TemplateDescription)
	...
	Return NStr("ru = 'Создано и отправлено по шаблону';
				|en = 'Created from template and sent';") + " - " + TemplateDescription;
...
Function TemplatesKinds() Export
	...
	AvailableOptions = MessageTemplatesAvailableSendOutOptions();
	If AvailableOptions.Mail Then
		TemplatesTypes.Add(MessageTemplatesClientServer.EmailTemplateName(), NStr("ru = 'Электронного письма';
																								|en = 'Mail template';"));
	EndIf;
	If AvailableOptions.SMS Then
		TemplatesTypes.Add(MessageTemplatesClientServer.SMSTemplateName(), NStr("ru = 'Сообщения SMS';
																				|en = 'Text template';"));
	EndIf;
	...
	If Common.SubsystemExists("StandardSubsystems.FilesOperations") Then
		...
		If IsBlankString(BaseName) Then
			...
			BaseName = NStr("ru = 'Вложение без имени';
									|en = 'Untitled attachment';") + ?(CountOfBlankNamesInAttachments = 0, ""," " + String(CountOfBlankNamesInAttachments + 1));
		...
		If AttachmentPrintForm.Status = "PrintForm" And NameOfParameterWithPrintFormInTemplate <> Undefined Then
			...
			If Common.SubsystemExists("StandardSubsystems.Print") Then
				...
				Except
					...
					WriteLogEvent(
						EventLogEventName(),
						EventLogLevel.Error,,, NStr("ru = 'Ошибка при создании внешней печатной формы. По причине:';
																|en = 'Error creating external print form due to:';") + Chars.LF
...
Procedure FillAttributesByDCS(Attributes, SubjectOf, TemplateParameters) Export
	...
	If BlankParameters.Count() > 0 Then
		Raise StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Не удалось сформировать сообщение, т.к. отсутствуют данные заполнения параметров: %1 для %2';
				|en = 'Cannot generate the message as the following parameter filling data is missing: %1 for %2';"), 
...
Function CommonAttributesTitle() Export
	Return NStr("ru = 'Общие реквизиты';
...
Function DetermineCommonAttributes() Export
	...
	AddCommonAttribute(CommonRowAttributes, "CurrentDate", NStr("ru = 'Текущая дата';
																	|en = 'Current date';"), New TypeDescription("Date"));
	AddCommonAttribute(CommonRowAttributes, "SystemTitle", NStr("ru = 'Заголовок системы';
																		|en = 'Application title';"));
	AddCommonAttribute(CommonRowAttributes, "InfobaseInternetAddress", NStr("ru = 'Адрес базы в Интернете';
																			|en = 'Infobase web address';"), New TypeDescription("String"));
	AddCommonAttribute(CommonRowAttributes, "InfobaseLocalAddress", NStr("ru = 'Адрес базы в локальной сети';
																				|en = 'Infobase LAN address';"), New TypeDescription("String"));
	AddCommonAttribute(CommonRowAttributes, "CurrentUser", NStr("ru = 'Текущий пользователь';
																			|en = 'Current user';"), New TypeDescription("CatalogRef.Users"));
...
Function EventLogEventName()
	...
	Return NStr("ru = 'Формирование шаблона сообщений';
				|en = 'Create message template';", Common.DefaultLanguageCode());
