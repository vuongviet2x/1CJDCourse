...
Procedure CustomizeReportInManagerModule(Settings, ReportMetadata) Export
	...
	Except
		ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Недопустимое значение параметра %1 в процедуре %2.
...
//           ** Value - String           - The settings of placing the report in a subsystem or a group. Valid values are:
//               "" - Display the name as normal text in the subsystem.
//               "Important" - Display the name as bold text in the subsystem.
//               "SeeAlso" - Display the name in the "See also" group.
...
Function DescriptionOfReport(Settings, Report) Export
	...
	If Result.Count() <> 1 Then
		Raise StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Недопустимое значение параметра %1 в функции %2.
			|Отчет ""%3"" не подключен к подсистеме ""%4"". Проверьте свойство ""Хранилище вариантов"" в свойствах отчета.';
			|en = 'Invalid value of ""%1"" parameter is specified. Function ""%2"".
...
//           ** Value - String           - The settings of placing the report option in a subsystem or a group. Valid values are:
//               "" - Display the name as normal text in the subsystem.
//               "Important" - Display the name as bold text in the subsystem.
//               "SeeAlso" - Display the name in the "See also" group.
//       * FunctionalOptions - Array of String - Names of the report option's functional options.
...
Procedure ResetCustomSettings(Var_Key, SettingsTypes1 = Undefined) Export
	...
	ObjectsKeys = New Array; // The final list of keys to be cleared.
	...
	Else
		Raise NStr("ru = 'Некорректный тип параметра ""Отчет""';
								|en = 'Invalid type of Report parameter';");
	EndIf;
		...
		While True Do
			...
			Except
				...
				WriteToLog(EventLogLevel.Error, 
					NStr("ru = 'Не удалось прочитать пользовательские настройки отчетов по причине:';
						|en = 'Cannot read custom report settings due to:';")
					+ Chars.LF
					+ ErrorProcessing.DetailErrorDescription(ErrorInfo()));
			EndTry;
...
Procedure MoveUsersOptionsFromStandardStorage(ReportsNames = "") Export
	ProcedurePresentation = NStr("ru = 'Прямая конвертация вариантов отчетов';
									|en = 'Direct conversion of report options';");
	...
	While True Do
		...
		Except
			...
			SuccessiveReadingErrors = SuccessiveReadingErrors + 1;
			WriteToLog(EventLogLevel.Error,
				NStr("ru = 'Не удалось прочитать варианты отчетов из стандартного хранилища по причине:';
					|en = 'Cannot read report options from the standard storage due to:';")
				+ Chars.LF
				+ ErrorProcessing.DetailErrorDescription(ErrorInfo()));
		EndTry;
...
Procedure ImportUserOptions(UserOptions1 = Undefined) Export
	...
	ProcedurePresentation = NStr("ru = 'Завершить конвертацию вариантов отчетов';
									|en = 'Finalize report option conversion';");
	WriteProcedureStartToLog(ProcedurePresentation);
...
Function AttachReportAndImportSettings(Val Parameters) Export
	...
	If MustReadReportRef Or MustReadSettings Then
		If TypeOf(Result.OptionRef1) <> Type("CatalogRef.ReportsOptions")
			...
			If Result.OptionRef1 = Undefined Then
				Result.ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'При вызове процедуры ""%1"" не указаны значения параметров ""%2"".';
						|en = 'Values of parameters ""%2"" are not specified when calling the ""%1"" procedure.';"),
		...
		PropertiesNames = "VariantKey,Custom,Parent,Parent.VariantKey" + ?(MustReadReportRef, ",Report", "")
			+ ?(MustReadSettings, ",Settings", "");  // ACC:1297 - These are attribute names. They do not require NStr.
	...
	If MustReadSettings Then
		...
		If DCSettingsOption = Undefined Then
			Result.ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Вариант ""%1"" (ключ ""%2"") не существует в схеме отчета ""%3"".';
					|en = 'The report option ""%1"" (key: ""%2"") is not found in the schema of the ""%3"" report.';"),
...
Procedure OnWriteAdditionalReport(CurrentObject, Cancel, ExternalObject) Export
	...
	If Not ReportsOptionsCached.InsertRight1() Then
		ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Недостаточно прав доступа для записи вариантов дополнительного отчета ""%1"".';
				|en = 'Insufficient rights to save options of additional report %1.';"),
...
Function OpeningParameters(OptionRef) Export
	...
	Else
		...
		If Not Selection.Next() Then
			...
			If Selection.Next() Then
				Raise(NStr("ru = 'Недостаточно прав для просмотра.';
										|en = 'Insufficient rights to view the report.';"), ErrorCategory.AccessViolation);
			Else
				Raise NStr("ru = 'Вариант отчета не существует.';
...
Function GenerateReport(Val Parameters, Val CheckFilling, Val GetCheckBoxEmpty) Export
	...
	If Not Connection.Success Then
		Result.ErrorText = NStr("ru = 'Не удалось сформировать отчет:';
									|en = 'Cannot generate the report:';") + Chars.LF + Connection.ErrorText;
		Return Result;
	EndIf;
	...
	If CheckFilling Then
		...
		If Not CheckPassed Then
			Result.ErrorText = NStr("ru = 'Отчет не прошел проверку заполнения:';
	...
	Except
		ErrorText = NStr("ru = 'Не удалось определить используемые таблицы:';
	...
	Except
		...
		WriteLogEvent(
			NStr("ru = 'Варианты отчетов.Формирование отчета';
				|en = 'Report options.Generate report';", Common.DefaultLanguageCode()),
			EventLogLevel.Error,
			Result.Metadata,
			Result.OptionRef1,
			ErrorProcessing.DetailErrorDescription(ErrorInfo()));
...
//     * FullNameOfExternalReportFile - String - Data for execution in a long-running operation.
...
Function ReportGenerationParameters() Export
	...
	Result.Insert("FullNameOfExternalReportFile", "");
	...
	If OptionsReferences.Count() > 0 Then
		...
		For Each Ref In OptionsReferences Do
			...
			If ReportValue = Undefined Then
				TableRow.Presentation = NStr("ru = 'Недостаточно прав для работы с вариантом отчета.';
													|en = 'Insufficient rights to access the report option.';");
			Else
...
Function ReportInformation(Val ReportFullName, Val RaiseException1 = False) Export
		...
		If Result.ReportMetadata = Undefined Then
			...
			WriteToLog(EventLogLevel.Warning,
				StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Отчет ""%1"" не существует в приложении, он будет значиться как внешний.';
						|en = 'The report %1 was added as an external report.';"),
					ReportFullName));
		ElsIf Not AccessRight("View", Result.ReportMetadata) Then
			Result.ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Недостаточно прав доступа к отчету ""%1"".';
					|en = 'Insufficient rights to access report ""%1.""';"),
				ReportFullName);
		EndIf;
	...
	Else
		Result.ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Неизвестный тип отчета ""%1"" (ожидался ""%2"" или ""%3"").';
				|en = 'Report ""%1"" has unknown type. Expected types: ""%2"" or ""%3"".';"),
...
Function AdditionalReportOptionsStorageCorrect(MetadataOfReport, WarningText = "") Export 
	...
	If MetadataOfReport.VariantsStorage <> Undefined Then 
		WarningText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Свойство отчета ""Хранилище вариантов"" заполнено не корректно.
			|Должно быть установлено в значение %1.';
			|en = 'The ""Option storage"" report property has invalid value.
			|Valid value: %1.';"), Metadata.SettingsStorages.ReportsVariantsStorage.FullName());
	Else
		WarningText = NStr("ru = 'Свойство отчета ""Хранилище вариантов"" не заполнено.';
									|en = 'The ""Option storage"" report property is not specified.';");
	EndIf;

	WarningText = WarningText + Chars.LF + Chars.LF
		+ NStr("ru = 'Сохранение и выбор вариантов отчета будет работать с ограничениями.
		|Обратитесь к разработчику дополнительного (внешнего) отчета.';
		|en = 'Saving and selecting report options may have some limitations.
...
Function PredefinedReportsOptions(ReportsType = "BuiltIn", ConnectedToTheStorage = True) Export
	...
	For Each ReportMetadata In Metadata.Reports Do 
		...
		If DescriptionOfReport.UsesDCS Then
			...
			Except
				ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Не удалось прочитать схему отчета %1:
						|%2';
						|en = 'Cannot read the %1 report scheme:
			...
			If DCSchema <> Undefined Then
				...
				Except
					If Common.DataSeparationEnabled() Then
						ErrorTextTemplate = NStr("ru = 'Не удалось прочитать список вариантов отчета %1 в неразделенном сеансе,
							...
							|%2';
							|en = 'Cannot read the %1 report option list in a shared session
					...
					Else
						ErrorTextTemplate = NStr("ru = 'Не удалось прочитать список вариантов отчета %1:
							|%2';
							|en = 'Cannot read the %1 report option list:
							|%2';");
			...
			If SettingVariants = Undefined Then
				...
				Except
					ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
						NStr("ru = 'Не удалось прочитать список вариантов отчета %1 из модуля менеджера:
							|%2';
							|en = 'Cannot read the %1 report option list from the manager module:
							|%2';"), ReportMetadata.Name, ErrorProcessing.DetailErrorDescription(ErrorInfo()));
					WriteToLog(EventLogLevel.Error, ErrorText, ReportMetadata);
				EndTry;
			EndIf;
			...
			If SettingVariants <> Undefined Then
				For Each DCSettingsOption In SettingVariants Do
					...
					OptionDetails.ReportFullName = DescriptionOfReport.ReportFullName;
						...
						Except
							ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
								NStr("ru = 'Не удалось прочитать настройки варианта ""%1"":
									|%2';
									|en = 'Cannot read the settings of the ""%1"" report option:
		...
		If IsBlankString(DescriptionOfReport.MainOption) Then
			OptionDetails = Result.Add();
			FillPropertyValues(OptionDetails, DescriptionOfReport, "Report, Description, Purpose, ReportFullName");
...
Procedure OnAddUpdateHandlers(Handlers) Export
	...
	If SharedDataIndexingAllowed() Then
		...
		Handler.Comment   = NStr("ru = 'Обновление индекса поиска отчетов, предусмотренных в приложении.';
										|en = 'Update search index for predefined reports.';");
	EndIf;
	...
	Handler.Comment     = NStr("ru = 'Обновление индекса поиска отчетов, сохраненных пользователями.';
										|en = 'Update search index for custom reports.';");
	...
	Handler.Comment = NStr("ru = 'Установка источника данных в настройках вариантов универсального отчета.
		...
		|en = 'Set a data source in the universal report option settings.
	...
	Handler.CheckProcedure = "InfobaseUpdate.DataUpdatedForNewApplicationVersion";
	Handler.Comment = NStr("ru = 'Перенос всех настроек доступности вариантов отчетов в регистр сведений ""Настройки вариантов отчетов"".
		...
		|en = 'Moves the report access restrictions to the ""Report option settings"" information register.
...
Function SubsystemDescription(LanguageCode)
	Return NStr("ru = 'Варианты отчетов';
				|en = 'Report options';", ?(LanguageCode = Undefined, Common.DefaultLanguageCode(), LanguageCode));
EndFunction
...
Function AttachReportObject(RefOfReport, GetMetadata)
	...
	If RefOfReport = Undefined Then
		Result.ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'В методе ""%1"" не указан параметр ""%2"".';
				|en = 'Method %1 is missing parameter %2.';"),
			"AttachReportObject",
			"RefOfReport");
		Return Result;
	Else
	...
	If TypeOf(Result.Ref) = Type("String") Then
		Result.ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Отчет ""%1"" записан как внешний и не может быть подключен из приложения';
				|en = 'Cannot attach report %1 from the app. Reason: The report was added as an external report.';"),
			Result.Ref);
		Return Result;
	EndIf;
	...
	If TypeOf(Result.Ref) = Type("CatalogRef.MetadataObjectIDs")
	 Or TypeOf(Result.Ref) = Type("CatalogRef.ExtensionObjectIDs") Then
		...
		If TypeOf(Result.Metadata) <> Type("MetadataObject") Then
			Result.ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Отчет ""%1"" не существует в приложении.';
					|en = 'The report %1 is not a part of the app.';"),
				Result.Name);
			Return Result;
		EndIf;
		Result.Name = Result.Metadata.Name;
		If Not AccessRight("Use", Result.Metadata) Then
			Result.ErrorText = NStr("ru = 'Недостаточно прав доступа';
		...
		Try
			...
			Result.Success = True;
		Except
			ErrorText = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Не удалось подключить отчет %1:';
																						|en = 'Cannot attach report %1:';"),
				Result.Metadata);
			ErrorText = ErrorText + Chars.LF + ErrorProcessing.DetailErrorDescription(ErrorInfo());
...
Procedure SetReportOutputModeInReportsPanels(Settings, Report, GroupByReports)
	...
	Else
		...
		If DescriptionOfReport.Count() <> 1 Then
			WriteToLog(EventLogLevel.Warning, 
				StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Отчет ""%1"" не подключен к подсистеме.';
						|en = 'Report %1 is not attached to the subsystem.';"), Report.Name));
			Return;
		EndIf;
...
Function KeysChanges()
	...
	For Each Update In Changes Do
		...
		If FoundItems.Count() > 0 Then
			...
			Raise StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Конфликт переименования варианта отчета ""%1"":
				|Актуальное имя варианта ""%2"" (старое имя ""%3"")
				|также числится как старое имя ""%4"" (актуальное имя ""%5"").';
				|en = 'A conflict occurred when renaming the ""%1"" report option:
...
Function GlobalSettings() Export
	...
	Result.Search.Insert("InputHint", NStr("ru = 'Наименование, поле или автор отчета';
													|en = 'Report description, field, or author';"));
...
Procedure PopulateExternalReportBinaryData(ReportObject, Parameters)
	...
	BinaryDataStructure.Insert("FullNameOfExternalReportFile", Parameters.FullNameOfExternalReportFile);
...
Procedure WriteProcedureStartToLog(ProcedureName)
	...
	EventLog.AddMessageForEventLog(SubsystemDescription(Undefined),
		...
		StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Запуск процедуры ""%1"".';
																	|en = 'Starting %1.';"), ProcedureName)); 
...
Procedure WriteProcedureCompletionToLog(ProcedureName, ObjectsChanged = Undefined)
	...
	Text = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Завершение процедуры ""%1"".';
																		|en = 'Finishing %1.';"), ProcedureName);
	If ObjectsChanged <> Undefined Then
		Text = Text + " " 
			+ StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Изменено %1 объектов.';
																			|en = '%1 objects have been modified.';"), ObjectsChanged);
	EndIf;
	EventLog.AddMessageForEventLog(SubsystemDescription(Undefined),
...
Procedure InternalUserNonexclusiveUpdate() Export 
	...
	Except
		...
		WriteLogEvent(
			NStr("ru = 'Варианты отчетов.Обновление служебного пользователя';
				|en = 'Report options.Update utility user';", Common.DefaultLanguageCode()),
			EventLogLevel.Error,
...
Function UpdateSearchIndex(Mode, IndexSchema)
	...
	Refinement = Lower(ModePresentation(Mode)) + ", " + ?(IndexSchema, NStr("ru = 'полное';
																					|en = 'full';"), NStr("ru = 'по изменениям';
																											|en = 'by changes';"));
	
	ProcedurePresentation = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Обновление индекса поиска (%1)';
																							|en = 'Updating search index (%1)';"), Refinement);
	WriteProcedureStartToLog(ProcedurePresentation);
	...
	While Selection.Next() Do
		...
		Try
			...
			If SharedData Then
				...
				If FoundItems.Count() = 0 Then
					ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
						NStr("ru = 'Вариант ""%1"" не существует для отчета ""%2""';
							|en = 'The ""%1"" option of the ""%2"" report does not exist.';"), 
			...
			Except
				ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Не удалось перестроить индекс поиска для варианта ""%1"" отчета ""%2"". Возможно, отчет неисправен.';
						|en = 'Cannot rebuild the search index for option %1, report%2. The report might be corrupted.';"), 
					OptionObject.VariantKey, OptionObject.Report);
...
Procedure UpdateKeysOfPredefinedItems(Mode, Result)
	...
	ProcedurePresentation = StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = 'Обновление ключей вариантов отчетов (%1)';
			|en = 'Updating report option keys (%1)';"), 
		?(Mode = "ConfigurationCommonData", NStr("ru = 'метаданные конфигурации';
													|en = 'configuration metadata';"), NStr("ru = 'метаданные расширений';
																							|en = 'extension metadata';")));
	WriteProcedureStartToLog(ProcedurePresentation);
...
Procedure MarkDeletedPredefinedItems(Mode, Result)
	...
	ProcedurePresentation = StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = 'Обновление настроек предопределенных (%1)';
			|en = 'Updating predefined settings (%1)';"), 
		?(Mode = "ConfigurationCommonData", NStr("ru = 'метаданные конфигурации';
													|en = 'configuration metadata';"), NStr("ru = 'метаданные расширений';
																							|en = 'extension metadata';")));
	WriteProcedureStartToLog(ProcedurePresentation);
...
Procedure GenerateOptionsFunctionalityTable(Mode, Result)
	...
	For Each OptionDetails In Result.ReportsOptions Do
		...
		If IsBlankString(DescriptionOfReport.MainOption) Or OptionDetails.VariantKey = DescriptionOfReport.MainOption Then
			MainOptionKey = OptionDetails.ReportFullName + "." + OptionDetails.VariantKey;
		...
		Else
			...
			MainOptionKey = MainOption.ReportFullName + "." + MainOption.VariantKey;
	...
	Try
		...
		For Each Section In OptionDetails.Location Do
			If Section.Key = Undefined Then
				MessageText = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Для варианта отчета ""%1"" (%2) 
					|не указана подсистема для размещения. См. процедуры %3, %4.';
					|en = 'A subsystem to add the %1 report option (%2) to is not specified.
...
Procedure UpdateReportsOptionsByPredefinedOnes(Mode, Result)
	...
	ProcedurePresentation = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Обновление вариантов отчетов (%1)';
																							|en = 'Updating report options (%1)';"), 
...
Procedure MarkOptionsOfDeletedReportsForDeletion(Mode, Result)
	...
	ProcedurePresentation = StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = 'Удаление вариантов удаленных отчетов (%1)';
			|en = 'Deleting options of deleted reports (%1)';"), 
		Lower(ModePresentation(Mode)));
	WriteProcedureStartToLog(ProcedurePresentation);
...
Procedure WriteFunctionalOptionsTable(Mode, Result)
	...
	EndIf;
	ProcedurePresentation = NStr("ru = 'Запись неразделенного кэша в регистр';
									|en = 'Save shared cache to register';");
	WriteProcedureStartToLog(ProcedurePresentation);
...
Procedure RecordCurrentExtensionsVersion()
	...
	ProcedurePresentation = NStr("ru = 'Запись регистра версий расширений';
									|en = 'Save extension version register';");
	WriteProcedureStartToLog(ProcedurePresentation);
...
// Handler of the "PredefinedReportOptionsUpdate" scheduled job.
...
Function ReportsOptionsPresentationsFillingParameters(Languages = Undefined, CurrentLanguageIndex = 0)
	...
	JobDescription = StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = 'Заполнение представлений предопределенных вариантов отчетов для языка %1';
			|en = 'Populating presentations of predefined report options for the %1 language';"), LanguageCode);
...
Function InternalUser(Val LanguageCode)
	...
	Except
		...
		WriteLogEvent(
			NStr("ru = 'Варианты отчетов.Создание служебного пользователя';
				|en = 'Report options.Create utility user';", Common.DefaultLanguageCode()),
			EventLogLevel.Error,
			Metadata.Catalogs.Users,,
			ErrorProcessing.DetailErrorDescription(ErrorInfo()));
...
Function ModePresentation(Mode)
	...
	Modes = New Map;
	Modes.Insert("ConfigurationCommonData", NStr("ru = 'Общие данные конфигурации';
													|en = 'Shared configuration data';"));
	Modes.Insert("ExtensionsCommonData", NStr("ru = 'Общие данные расширений';
													|en = 'Shared extension data';"));
	Modes.Insert("SeparatedConfigurationData", NStr("ru = 'Разделенные данные конфигурации';
															|en = 'Separate configuration data';"));
	Modes.Insert("SeparatedExtensionData", NStr("ru = 'Разделенные данные расширений';
														|en = 'Separate extension data';"));
...
Procedure SetConditionalAppearanceOfReportOptionUsersList(Form) Export 
	...
	Item.Appearance.SetParameterValue("Text", NStr("ru = 'Все пользователи';
																|en = 'All users';"));
...
Function FillFieldsForSearch(OptionObject, ReportInfo = Undefined) Export
	...
	If DCSchema = Undefined Then
		...
		If IsPredefined Then
			ErrorText = ErrorText + Chars.LF
				+ StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Подробнее см. процедуру %1.';
																				|en = 'For more details, see procedure ""%1"".';"),
	...
	If TypeOf(DCSettings) <> Type("DataCompositionSettings")
		...
		Except
			MessageTemplate = NStr("ru = 'Не удалось прочитать настройки пользовательского варианта отчета. 
				...
				|%1';
				|en = 'Cannot read custom report option settings. 
	...
	If TypeOf(DCSettings) <> Type("DataCompositionSettings") Then
		If TypeOf(OptionObject) = Type("CatalogObject.PredefinedReportsOptions")
			...
			WriteToLog(EventLogLevel.Error, 
				StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Не удалось прочитать настройки предопределенного варианта отчета ""%1"".';
						|en = 'Cannot read settings of a predefined report option: %1.';"), OptionObject.MeasurementsKey),
				OptionObject.Ref);
		EndIf;
		Return False;
	EndIf;
...
Function ItIsAcceptableToSetContext(Form) Export
	...
	Return (Form.ParametersForm.InitialKeyOfPredefinedOption = Form.ReportSettings.PredefinedOptionKey)
		Or (Form.ParametersForm.InitialOptionKey = Form.CurrentVariantKey);
...
Function ReportSettings(ReportRef, VariantKey, ReportObject)
	...
	If ReportObject = Undefined Then
		...
		Else
			Text = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Не удалось получить настройки отчета ""%1"":';
					|en = 'Failed to get settings for report %1:';") + Chars.LF + Connection.ErrorText,
				ReportRef);
	...
	Except
		...
		Else
			...
			ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Отчет ""%1"" (%2) подключен к подсистеме
				           ...
				           |
				           |%4';
							|en = 'The ""%1"" (%2) report is attached to
...
Function PrepareSearchConditionByRow(Val Query, QueryText, Val HasSearchString, Val SearchParameters, Val AuthorsReadRight)
	...
	If HasSearchString Then
		...
		EndDo;
		SearchTemplate = StrConcat(SearchTemplates, " OR "); // Do not localize. This is a query fragment.
		...
		EndDo;
		OptionsAndSubsystemsBySearchString = "(" + StrConcat(SearchFields, " OR ") + ")"; // Do not localize. This is a query fragment.
...
Procedure FindReportOptionsForOutput(FillParameters, ResultAddress) Export
	...
	RowEmptySubsystem.Presentation = NStr("ru = 'Не включенные в разделы';
												|en = 'Not included in sections';");
...
Function DataIsBeingUpdatedMessage() Export
	Return NStr("ru = 'Отчет может содержать некорректные данные, так как не завершен переход на новую версию приложения. Если отчет долгое время недоступен, обратитесь к администратору.';
				|en = 'The report might contain incorrect data since the migration to the new version is not completed. If the report is not available for a while, contact the administrator.';");
EndFunction
		...
		For Each TableRow In FoundItems Do
			...
			ElsIf Not FoundInApplication Then
				TableRow.Presentation = StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = '<Отчет ""%1"" не существует в приложении>';
						|en = '<Report %1 doesn''t exist in app>';"),
					ReportName);
			ElsIf Not AvailableByRights Then
				TableRow.Presentation = StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = '<Недостаточно прав для работы с отчетом ""%1"">';
						|en = '<Insufficient rights to access report %1>';"),
					ReportName);
			ElsIf Not AvailableByOptions Then
				TableRow.Presentation = StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = '<Отчет ""%1"" отключен в настройках приложения>';
						|en = '<Report %1 disabled in settings>';"),
					ReportName);
			Else
...
Function UpdateReportOptionsFromFiles(FilesDetails) Export 
	...
	For Each FileDetails In FilesDetails Do 
		...
		Ref = UpdateReportOptionByDetails(ReportOptionDetails); // @skip-check query-in-loop - Save data object-by-object.
...
#Region ReadSettingsDetailsFile
	...
	HeaderDescriptionErrors = NStr("ru = 'Некорректный формат описания настроек:';
								|en = 'Incorrect settings description format:';") + Chars.LF;
	...
	Except
		SettingsDescription.ErrorDescription = HeaderDescriptionErrors + StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'отсутствует файл %1.';
				|en = 'File %1 is missing.';"), FileName);
...
#Region ReadCommonSettingsDetails
	...
	If DOMDocument.ChildNodes.Count() = 0 Then 
		SettingsDescription.ErrorDescription = HeaderDescriptionErrors + StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'отсутствует элемент %1.';
				|en = 'Item ""%1"" is missing.';"), "SettingsDescription");
		Return SettingsDescription;
	EndIf;
	...
	If ReportName = Undefined Then 
		SettingsDescription.ErrorDescription = HeaderDescriptionErrors + StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'отсутствует атрибут %1.';
				|en = 'Attribute ""%1"" is missing.';"), TagName);
		Return SettingsDescription;
	EndIf;
...
#Region ReadMainSettingsDetails
	...
	If VariantKey = Undefined Then 
		SettingsDescription.ErrorDescription = HeaderDescriptionErrors + StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'отсутствует атрибут %1 элемента %2.';
				|en = 'Item ""%2"" is missing attribute ""%1"".';"), AttributeName, TagName);
		Return SettingsDescription;
	EndIf;
...
Function DeserializedSettings(FileName)
	...
	Except
		...
		Raise StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Некорректный формат настроек: отсутствует файл %1';
				|en = 'Invalid settings format: the %1 file is missing.';"),
...
Procedure ShareUserSettings(SelectedUsers, SettingsDetailsTemplate) Export 
	...
	If UsersProperties.Valid2.Count() = 0 Then 
		...
		SettingsDetailsTemplate.Insert("Warning", NStr("ru = 'Выбранные пользователи недействительны';
																|en = 'The selected users are inactive.';"));
		Return;
	...
	If UsersProperties.Invalid1.Count() > 0 Then 
		...
		NoteTemplate = NStr("ru = 'Некоторые из выбранных пользователей недействительны: %1';
								|en = 'Some of the selected users are inactive: %1.';");
...
Procedure SetUserSettingsPresentation(SettingsDetailsTemplate, CurrentUser)
	...
	TemplateOfPresentation = NStr("ru = 'Настройки %1';
								|en = '%1''s settings';");
...
#Region NewSettingsUsersNotificationGeneration
	...
	If ValueIsFilled(SettingsDescription.Presentation) Then 
		...
		Text = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Предоставлены настройки отчета ""%1""';
				|en = 'The report settings are available: ""%1.""';"), SettingsDescription.Presentation);
	Else
		Text = NStr("ru = 'Предоставлены настройки отчета';
					|en = 'The report settings are available.';");
	EndIf;
	...
	Message.Actions.Add(
		...
		NStr("ru = 'Применить настройки?';
			|en = 'Do you want to apply the settings?';"));
	...
	Except
		...
		WriteLogEvent(
			NStr("ru = 'Варианты отчетов.Оповещение о предоставлении доступа к настройкам отчета';
				|en = 'Report options.Report settings availability notification';", DefaultLanguageCode),
			EventLogLevel.Error,,
...
Function PredefinedReportsOptionsCollection()
	...
	Result.Columns.Add("ReportFullName", TypesDetailsString());
...
Function DefaultReportDetails(ReportsDetails, MetadataOfReport, RefOfReport,
	...
	DescriptionOfReport.ReportFullName = MetadataOfReport.FullName();
