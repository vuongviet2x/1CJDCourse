...
Function DefaultSettings()
	...
	SubsystemSettings.Insert("UnlockCommandPlacement",
		NStr("ru = 'Блокировку также можно снять позднее в разделе <b>Администрирование - Обслуживание</b>.';
			|en = 'You can release the lock later in <b>Administration > Support and service</b>.';"));
...
Function ExecuteScheduledJobManually(Val Job) Export
	...
	If LastBackgroundJobProperties <> Undefined
	   And LastBackgroundJobProperties.State = BackgroundJobState.Active Then
		...
		EndIf;
	Else
		BackgroundJobDescription = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Запуск вручную: %1';
																									|en = 'Manual start: %1';"), ScheduledJobPresentation(Job));
...
Procedure CancelBackgroundJob(Id) Export
	...
	Else
		Raise NStr("ru = 'Фоновое задание не существует.';
								|en = 'The background job does not exist.';");
	EndIf;
	
	If BackgroundJob.State <> BackgroundJobState.Active Then
		Raise NStr("ru = 'Задание не выполняется, его нельзя отменить.';
								|en = 'The job is not running. It cannot be canceled.';");
	EndIf;
