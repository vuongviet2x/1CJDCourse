...
Function SendSMS(RecipientsNumbers, Val Text, SenderName = Undefined, Transliterate = False) Export
	...
	If Not ValueIsFilled(StrConcat(RecipientsNumbers, "")) Then
		Result.ErrorDescription = NStr("ru = 'Не указан номер получателя SMS.';
										|en = 'Text message recipient number is not specified.';");
		Return Result;
	EndIf;
	...
	If Not SMSMessageSendingSetupCompleted() Then
		Result.ErrorDescription = NStr("ru = 'Неверно заданы настройки провайдера для отправки SMS.';
										|en = 'Invalid SMS provider settings.';");
		Return Result;
	EndIf;
		...
		If Result.SentMessages.Count() > 0 Then
			CommonClientServer.Validate(
				...
				StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Неверный тип значения в коллекции %1:
						|ожидается тип ""Структура"", передан тип ""%2""';
						|en = 'Invalid value type in %1 collection.
...
Function DefaultProviderAuthorizationFields()
	...
	AuthorizationFields.Add("Login", NStr("ru = 'Логин';
											|en = 'Username';"));
	AuthorizationFields.Add("Password", NStr("ru = 'Пароль';
											|en = 'Password';"), True);
...
Function DefaultAuthorizationMethods()
	...
	Result.Add("ByUsernameAndPassword", NStr("ru = 'По логину и паролю';
...
Function DefaultProviderSettings()
	...
	Result.Insert("DeliveryStatusesInOneRequest", False);
