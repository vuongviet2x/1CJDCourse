...
Procedure OnDefineAttachableCommandsKinds(AttachableCommandsKinds) Export
	...
	// _Demo Example Start
	Kind = AttachableCommandsKinds.Add();
	Kind.Name         = "SendPollInvitation";
	Kind.SubmenuName  = "SubmenuSendPollInvitation";
	Kind.Title   = NStr("ru = 'Отправка приглашения';
							|en = 'Send invitation';");
	Kind.Picture    = PictureLib.EmailMessage;
	Kind.Representation = ButtonRepresentation.PictureAndText;
	
	Kind = AttachableCommandsKinds.Add();
	Kind.Name        = "Surveys";
	Kind.SubmenuName = "SurveySubmenu";
	Kind.Title   = NStr("ru = 'Анкетирование';
							|en = 'Surveys';");
	Kind.Picture    = PictureLib.IsEmpty;
	Kind.Representation = ButtonRepresentation.Text;
	
	Kind = AttachableCommandsKinds.Add();
	Kind.Name = "MarkingDeletionsWithDisplay";
	Kind.Title   = NStr("ru = 'Демо: Пометка удаления';
							|en = 'Demo: Deletion mark';");
	Kind.Representation = ButtonRepresentation.PictureAndText;
...
Procedure OnDefineAttachableObjectsSettingsComposition(InterfaceSettings4) Export
	...
	// _Demo Example Start
	Setting = InterfaceSettings4.Add();
	Setting.Key          = "AddSendInvitationCommands";
	Setting.TypeDescription = New TypeDescription("Boolean");
	Setting.AttachableObjectsKinds = "DataProcessor";
	
	Setting = InterfaceSettings4.Add();
	Setting.Key          = "AddPollSendingCommands";
	Setting.TypeDescription = New TypeDescription("Boolean");
	Setting.AttachableObjectsKinds = "DataProcessor";
	// _Demo Example End
...
Procedure OnDefineCommandsAttachedToObject(FormSettings, Sources, AttachedReportsAndDataProcessors, Commands) Export
	...
	// _Demo Example Start
	_DemoStandardSubsystems.OnDefineCommandsAttachedToObject(FormSettings, Sources, AttachedReportsAndDataProcessors, Commands);
	// _Demo Example End
