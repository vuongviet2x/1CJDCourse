...
Procedure OnReceiveFileFromStorage(Val FileID, FileName) Export
	...
	If QueryResult.IsEmpty() Then
		LongDesc = NStr("ru = 'Файл с идентификатором %1 не обнаружен.';
						|en = 'The file with ID %1 is not found.';");
...
Procedure BeforeWriteCommonData(Object, Cancel) Export
	...
	If ReadOnly Then
		ErrorString = NStr("ru = 'Изменение неразделенных данных (%1), загружаемых из приложения, в Автономном рабочем месте запрещено.
		|Обратитесь к администратору.';
		|en = 'Standalone workstations don''t support modification of imported shared data (%1).
...
Procedure ChangeTheIndicationOfTheNeedForDataExchangeInTheServiceModel(ItIsNecessaryToPerformAnExchange, AdditionalParameters = Undefined) Export
	...
	If Not TheValueOfTheConstantHasBeenChanged Then
		...
		ExceptionText = NStr("ru = 'Ошибка блокировки константы ""Зарегистрированы изменения данных"".
			|Повторите попытку позже.';
			|en = 'Cannot lock the ""Data changes recorded"" constant.
...
Function GeneratePredefinedNodeDescription() Export
	...
	DefaultApplicationName = NStr("ru = 'Приложение в Интернете';
									|en = 'Web application';");
...
Function ExchangePlanNodeCodeInService(Val AreaNumber) Export
	...
	If TypeOf(AreaNumber) <> Type("Number") Then
		Raise NStr("ru = 'Неправильный тип параметра номер [1].';
								|en = 'Invalid type in parameter number [1].';");
	EndIf;
...
Procedure ExecuteDataExchangeScenarioActionInFirstInfobase(ScenarioRowIndex, DataExchangeScenario) Export
	...
	ActionToBeExecutedDetailsEL = StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = 'Выполнение шага сценария синхронизации (%1/%2).';
			|en = 'Running synchronization scenario… Step %1/%2.';"), ScenarioRowIndex + 1, DataExchangeScenario.Count())
		...
		Try
			...
			If Not DataExchangeServer.SynchronizationSetupCompleted(InfobaseNode) Then
				Raise StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Настройка синхронизации данных в приложении ""%1"" не завершена.';
						|en = 'Data synchronization setup in ""%1"" is not completed.';"),
					InfobaseNode);
			ElsIf DataExchangeServer.MessageWithDataForMappingReceived(InfobaseNode) Then
				Raise StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'В приложение ""%1"" загружено сообщение для ручного сопоставления.';
						|en = 'A message that requires manual mapping has been imported to ""%1"".';"),
					InfobaseNode);
			...
			Else
				Raise StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Неизвестное действие ""%1"" при обмене данными между областями данных.';
						|en = 'Unknown action (%1) is detected during data exchange between data areas.';"),
	...
	ElsIf ScenarioRow.InfobaseNumber = 2 Then
		...
		Try
			...
			If WSProxyDetails.WSProxy = Undefined Then
				Raise StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Не удалось получить описание WS-прокси для приложения ""%1"".';
						|en = 'Cannot get WS proxy details for ""%1"".';"),
					InfobaseNode);
			Else
...
Procedure ExecuteDataExchangeScenarioActionInSecondInfobase(ScenarioRowIndex, DataExchangeScenario) Export
	...
	Try
		...
		Else
			Raise StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Неизвестное действие ""%1"" при обмене данными между областями данных.';
					|en = 'Unknown action (%1) is detected during data exchange.';"),
				ScenarioRow.CurrentAction);
		EndIf;
	Except
		ResultingStructure.Cancel = True;
...
Procedure DeleteSynchronizationSetting(ExchangePlanName, CorrespondentNodeCode) Export
	...
	MessageText = StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = 'Обработка сообщения удаления настройки синхронизации ""%1"" с приложением ""%2"".';
			|en = 'Processing the message that will delete synchronization between ""%1"" and ""%2"".';"),
...
Procedure NewDataAvailable(Val Descriptor, ToImport) Export
	...
	If Descriptor.DataType = SuppliedDataKindID()
		...
		Else
			...
			MessageText = NStr("ru = 'Поставляемые правила обмена не подходят для текущей конфигурации и предназначены для плана обмена %1 конфигурации %2 версии %3';
									|en = 'The built-in exchange rules are not intended for the current configuration. They are intended for exchange plan %1 of configuration %2 v.%3';");
			...
			WriteLogEvent(NStr("ru = 'Поставляемые правила обмена данными.Загрузка поставляемых правил отменена';
											|en = 'Built-in data exchange rules.Import of built-in rules is canceled';",
...
Function EndpointsExchangePlanManager() Export
	...
	If MetadataMessagesExchange = Undefined Then
		Raise NStr("ru = 'Не определен план обмена конечных точек.';
								|en = 'Endpoint exchange plan is not defined.';");
	EndIf;
...
Procedure FinishDataExchangeScenarioExecution(ScenarioRowIndex, DataExchangeScenario, ResultingStructure = Undefined)
	...
	WriteLogEvent(DataSyncronizationLogEvent(),
		EventLogLevel.Information, , , NStr("ru = 'Завершение выполнения сценария синхронизации.';
														|en = 'Finishing the synchronization scenario.';"));
...
Function DataExchangeScenarioRowDetails(ScenarioRowIndex, DataExchangeScenario)
	...
	LongDesc = StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = 'ПорядковыйНомер: %1, Приложение: %2, Действие: %3(%4), Режим: %5.';
			|en = 'SequenceNumber: %1, Application: %2, Action: %3(%4), Mode: %5.';"),
...
Procedure CreateExchangeSetting(ConnectionSettings,
	...
	If IsBlankString(ThisNodeCode) Then
		...
		MessageString = NStr("ru = 'Для предопределенного узла плана обмена ""%1"" не задан код.';
								|en = 'Code of the predefined exchange plan node %1 is not specified.';");
	...
	Try
		...
		If IsBlankString(GetFunctionalOption("InfobasePrefix")) Then
			If IsBlankString(ConnectionSettings.Prefix) Then
				Raise StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Для продолжения настройки синхронизации данных необходимо
					|задать префикс информационной базы в приложении ""%1"".';
					|en = 'To continue data synchronization setup,
		...
		If CheckCode And ConnectionSettings.CorrespondentCode <> ActualCorrespondentCode Then
			...
			MessageString = NStr("ru = 'Ошибка назначения кода узла корреспондента.
				|Назначенное значение ""%1""
				|Фактическое значение ""%2"".';
				|en = 'Error assigning ID to a peer infobase node.
				|Assigned ID: %1.
				|Actual ID: %2.';");
...
Procedure CreateExchangeSetting_3_0_1_1(ConnectionSettings,
	...
	If IsBlankString(ThisNodeCode) Then
		// The node code is set in the infobase update handler.
		Raise StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Для предопределенного узла плана обмена ""%1"" не задан код.';
				|en = 'Code of the predefined exchange plan node %1 is not specified.';"),
...
Procedure UpdateDataAreaTransportSettings(Parameters)
	...
	If TransportSettings.DefaultExchangeMessagesTransportKind = Enums.ExchangeMessagesTransportTypes.FILE Then
		...
		If IsBlankString(FILECommonInformationExchangeDirectory) Then
			...
			MessageString = NStr("ru = 'Не задан каталог обмена информацией для конечной точки ""%1"".';
									|en = 'The data exchange directory for the endpoint %1 is not specified.';");
		...
		If Not CommonDirectory.Exists() Then
			...
			MessageString = NStr("ru = 'Каталог обмена информацией ""%1"" не существует.';
									|en = 'The exchange directory %1 does not exist.';");
	...
	Else
		Raise StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Вид транспорта сообщений обмена ""%1"" для конечной точки %2 не поддерживается.';
				|en = 'The endpoint %2 doesn''t support the exchange message transport type %1.';"),
...
Procedure SaveSessionData(Val Message, Val Presentation = "") Export
	...
	If Not IsBlankString(Presentation) Then
		Presentation = " "+ StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = '{%1}';
																							|en = '{%1}';"), Presentation);
	EndIf;
	
	MessageString = NStr("ru = 'Сессия обмена сообщениями системы ""%1"" успешно завершена.%2';
							|en = 'Message exchange session %1 is completed. %2';",
...
Procedure CommitUnsuccessfulSession(Val Message, Val Presentation = "") Export
	...
	MessageString = NStr("ru = 'Ошибка выполнения сессии обмена сообщениями системы ""%1"".%2
		|Описание ошибки из корреспондента: %3';
		|en = 'Message exchange failed. Session: %1. %2.
...
Function EventLogEventDataSynchronizationSetup() Export
	...
	Return NStr("ru = 'Обмен данными в модели сервиса.Настройка синхронизации данных';
				|en = 'Data exchange SaaS.Data synchronization setup';",
		Common.DefaultLanguageCode());
...
Function EventLogEventDataSynchronizationMonitor() Export
	...
	Return NStr("ru = 'Обмен данными в модели сервиса.Монитор синхронизации данных';
				|en = 'Data exchange SaaS.Data synchronization monitor';",
		Common.DefaultLanguageCode());
...
Function DataSyncronizationLogEvent() Export
	...
	Return NStr("ru = 'Обмен данными в модели сервиса.Синхронизация данных';
				|en = 'Data exchange SaaS.Data synchronization';",
		Common.DefaultLanguageCode());
...
Function EventLogEventSystemMessagesExchangeSessions()
	...
	Return NStr("ru = 'Обмен данными в модели сервиса.Сессии обмена сообщениями системы';
				|en = 'Data exchange SaaS.Message exchange sessions';",
		Common.DefaultLanguageCode());
...
Function FindInfobaseNode(Val ExchangePlanName, Val NodeCode)
	...
	If Result = Undefined Then
		...
		Message = NStr("ru = 'Не найден узел плана обмена. Имя плана обмена %1; код узла %2 или %3';
						|en = 'Cannot find a node in the exchange plan %1. Node ID %2 or %3';");
...
Procedure OnCreateStandaloneWorkstation() Export
	...
	If UsersInternalSaaS.UserRegisteredAsShared(
		...
		Raise NStr("ru = 'Создать автономное рабочее место можно только от имени разделенного пользователя.
			|Текущий пользователь является неразделенным.';
			|en = 'You can create a standalone workstation only under a separated user.
