...
Procedure SaveProcessing(ReportKey, VariantKey, Settings, SettingsDescription, User)
	If Not ReportsOptionsCached.InsertRight1() Then
		Raise(NStr("ru = 'Недостаточно прав для сохранения вариантов отчетов.';
								|en = 'Insufficient rights to save report options.';"),
...
Function AddExternalReportOptions(ReportOptions, FullReportName, ReportName) Export
	...
	Except
		MessageTemplate = NStr("ru = 'Не удалось получить список предопределенных вариантов внешнего отчета ""%1"":%2%3';
								|en = 'Failed to get the list of predefined options of external report %1: %2%3';");
