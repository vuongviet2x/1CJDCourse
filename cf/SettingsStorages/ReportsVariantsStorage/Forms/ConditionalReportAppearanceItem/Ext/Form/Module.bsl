...
Function TheDesignElementIsTheOriginalOne()
	...
	Else
		...
		If DesignSource = Undefined Then
			Raise NStr("ru = 'Не существует узел отчета.';
									|en = 'The report node does not exist.';");
		EndIf;
		
		DesignElementSource = DesignSource.GetObjectByID(DCID);
		If DesignElementSource = Undefined Then
			Raise NStr("ru = 'Не существует элемент условного оформления.';
									|en = 'The conditional appearance item does not exist.';");
		EndIf;
...
Procedure AddAdditionalField(List)
	...
	List.Add(AdditionalField(), NStr("ru = 'Еще...';
												|en = 'More…';"),, PictureLib.IsEmpty);
