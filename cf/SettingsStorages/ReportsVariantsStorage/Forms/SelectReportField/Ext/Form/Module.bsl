...
Procedure SetTheCollectionName()
	...
	If FieldsCollectionName = Undefined Then		
		Raise StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Некорректное значение параметра ""Режим"": ""%1"".';
				|en = 'Mode parameter contains invalid value: %1.';"), String(Mode));		
	EndIf;
...
Procedure SelectAndClose()
	...
	If TypeOf(ListBox.CurrentData) = Type("FormDataTreeItem") And ListBox.CurrentData.Folder Then
		CommonClient.MessageToUser(NStr("ru = 'Выберите поле отчета, а не группу.';
														|en = 'Select a report field, not a group.';"));
		Return;
	ElsIf TypeOf(ListBox.CurrentData) <> Type("FormDataTreeItem") Then
		CommonClient.MessageToUser(NStr("ru = 'Выберите поле отчета.';
														|en = 'Select a report field.';"));
		Return;
	EndIf;
...
Function NewFieldDescr()
	...
	For FieldNumber = 1 To 100 Do
		FieldDescription = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Поле %1';
																						|en = 'Field %1';"), FieldNumber);
...
Function AdditionalFieldOfTheFormulaGroup()
	...
	Field.Presentation = NStr("ru = 'Формулы';
								|en = 'Formulas';");
	Field.Order = 99;
...
Function AdditionalFieldOfDetailedRecords()
	...
	Field.Presentation = NStr("ru = '<Детальные записи>';
								|en = '<Detailed records>';");
