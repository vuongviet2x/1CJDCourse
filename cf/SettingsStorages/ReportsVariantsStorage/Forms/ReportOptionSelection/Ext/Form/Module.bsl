...
Procedure ReportOptionsTreeBeforeDeleteRow(Item, Cancel)
	...
	If Variant.PictureIndex = 4 Then
		QueryText = NStr("ru = 'Снять с ""%1"" пометку на удаление?';
							|en = 'Do you want to remove the deletion mark from ""%1""?';");
	Else
...
Procedure SelectAndClose()
	...
	If Variant.PictureIndex = 4 Then
		QueryText = NStr("ru = 'Выбранный вариант отчета помечен на удаление.
...
Procedure OpenOptionForChange()
	...
	If Not OptionChangeRight(Variant, FullRightsToOptions) Then
		WarningText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Недостаточно прав для изменения варианта ""%1"".';
				|en = 'Insufficient rights to modify option ""%1"".';"), 
			Variant.Description);
	...
	For Each OptionInfo In VariantsTable Do
		...
		If TreeRowsSet = Undefined Then
			...
			If OptionInfo.Order = 3 Then
				TreeGroup.Description = NStr("ru = 'Помеченные на удаление';
