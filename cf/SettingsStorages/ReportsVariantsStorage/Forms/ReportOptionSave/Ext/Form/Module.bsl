...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Context.IsExternal Then
		...
		Items.NextStepInfoNewOptionDecoration.Title = NStr("ru = 'Будет сохранен новый вариант отчета.';
																|en = 'The report option will be saved as a new option.';");
		Items.NextStepInfoOverwriteOptionDecoration.Title = NStr("ru = 'Будет перезаписан существующий вариант отчета.';
																	|en = 'The current report option will be overwritten.';");
...
Procedure FillCheckProcessingAtServer(Cancel, CheckedAttributes)
	...
	If Not ValueIsFilled(Object.Description) Then
		Common.MessageToUser(
			NStr("ru = 'Поле ""Наименование"" не заполнено';
				|en = 'Description is not populated';"),, "Description");
		Cancel = True;
	ElsIf ReportsOptions.DescriptionIsUsed(Context.ReportRef, OptionRef, Object.Description) Then
		Common.MessageToUser(
			StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = '""%1"" занято, необходимо указать другое Наименование.';
...
Procedure LongDescStartChoice(Item, ChoiceData, StandardProcessing)
	...
	CommonClient.ShowMultilineTextEditingForm(
		Notification, Items.LongDesc.EditText, NStr("ru = 'Описание';
																|en = 'Details';"));
...
Procedure ExecuteBatch(Result, Package) Export
	...
	If Package.CheckPage1 = True Then
		...
		If Not ValueIsFilled(Object.Description) Then
			ErrorText = NStr("ru = 'Поле ""Наименование"" не заполнено';
								|en = 'Description is not populated';");
		...
		If Not Package.OptionIsNew Then
			...
			If Not RightToWriteOption(Variant, Context.FullRightsToOptions) Then
				ErrorText = NStr("ru = 'Недостаточно прав для изменения варианта ""%1"". Необходимо выбрать другой вариант или изменить наименование.';
									|en = 'Insufficient rights to modify option ""%1"". Save it under a different name or select another report option.';");
...
Function AskAboutOverwritingAReportVariant(Package, Variant)
	...
	If Variant.DeletionMark = True Then
		...
		QuestionTextTemplate = NStr("ru = 'Вариант отчета ""%1"" помечен на удаление.
	...
	Else
		...
		QuestionTextTemplate = NStr("ru = 'Заменить ранее сохраненный вариант отчета ""%1""?';
									|en = 'Do you want to overwrite report option %1?';");
	EndIf;
...
Function GenerateFreeDescription(Variant, ReportOptions)
	...
	OptionNameTemplate = TrimAll(Variant.Description) + " - " + NStr("ru = 'копия';
																		|en = 'copy';");
