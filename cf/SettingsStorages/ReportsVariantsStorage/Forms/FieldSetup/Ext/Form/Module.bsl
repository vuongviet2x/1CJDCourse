...
Procedure RenameField(Command)
	...
	If Not ValueIsFilled(FieldPresentation) Then 
		...
		CommonClient.MessageToUser(
			NStr("ru = 'Необходимо ввести новый заголовок поля';
				|en = 'Enter new field header';"),, "FieldPresentation");
		Return;
...
Procedure SetRowHeight(Command)
	...
	If Not ValueIsFilled(RowHeight) Then 
		...
		CommonClient.MessageToUser(
			NStr("ru = 'Необходимо ввести высоту строки';
				|en = 'Enter row height';"),, "RowHeight");
		Return;
...
Procedure SetColumnWidth(Command)
	...
	If Not ValueIsFilled(ColumnWidth) Then 
		...
		CommonClient.MessageToUser(
			NStr("ru = 'Необходимо ввести ширину колонки';
				|en = 'Enter column width';"),, "ColumnWidth");
		Return;
...
Procedure SetConditionalAppearance()
	...
	Item.Appearance.SetParameterValue("Text", NStr("ru = '(Пустые)';
																|en = '(Empty)';"));
...
Procedure Initialize()
	...
	Title = StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = 'Настройка поля: %1';
			|en = 'Field setting: %1';"), FieldTitle);
...
Function StartFillingInTheValues(JobID)
	...
	Items.TimeConsumingOperationPresentation.Title = StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = 'Выполняется чтение строк поля: %1';
			|en = 'Reading field lines: %1';"), FieldPresentation);
	
	MethodParameters = TimeConsumingOperations.FunctionExecutionParameters(JobID);
	MethodParameters.BackgroundJobDescription = NStr("ru = 'Заполнение фильтра по значению';
														|en = 'Fill in filter by value';");
	MethodParameters.RefinementErrors = NStr("ru = 'Не удалось заполнить фильтр по значению по причине:';
											|en = 'Cannot fill in filter by value due to:';");
...
Procedure AfterFillingInTheValues(Result, AdditionalParameters) Export
	...
	If Result.Status = "Error" Then
		...
		CommentTemplate = NStr("ru = 'Не удалось заполнить фильтр по значению по причине: %1';
								|en = 'Cannot fill in filter by value due to: %1';");	
...
Procedure ToSpecifyAFilterCondition(Filter, Cancel)
	...
	If GroupsAreUsed
		...
		WarningText = NStr("ru = 'В списке выбраны группы и элементы.
			|Необходимо выбрать только группы или только элементы.';
			|en = 'The list contains both groups and items.
...
Procedure ShowStatisticsOfFillingInValues()
	...
	Items.FilterByValueStatistics.Title = StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = 'Выведено %1 первых значений.';
			|en = 'The first %1 values are displayed.';"), Values.Count());
	
	Items.OutputAllReportSectionValues.ExtendedTooltip.Title = StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = 'Выведено %1 уникальных значений из первых %2 строк отчета. Всего строк в разделе отчета - %3.';
			|en = '%1 unique values out of the first %2 lines of the report are displayed. The total number of lines in the report section is %3.';"),
		Values.Count(),
