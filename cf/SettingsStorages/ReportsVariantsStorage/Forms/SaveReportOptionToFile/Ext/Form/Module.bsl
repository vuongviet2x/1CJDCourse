...
Procedure Save(Command) 
	...
	SavingParameters.Dialog.Filter = NStr("ru = 'Архив (*.zip)|*.zip';
											|en = 'Archive (*.zip)|*.zip';");
	SavingParameters.Dialog.Title = NStr("ru = 'Укажите файл';
												|en = 'Select file';");
...
Procedure FillInTheDescriptionOfTheReportOptions()
	...
	If ReportsOptionsDetails.Count() > 1 Then
		...
		Title = NStr("ru = 'Сохранение вариантов отчетов в файл';
						|en = 'Save report options to file';");
		
	ElsIf ReportsOptionsDetails.Count() = 1 Then
		
		Items.SaveOptions.CurrentPage = Items.OneReportOption;
		Title = NStr("ru = 'Сохранение варианта отчета в файл';
						|en = 'Save report option to file';");
...
Procedure FillReportOptionDetails(ReportVariant, DataOfTheReportVariant,
	...
	If Report.DataCompositionSchema = Undefined Then 
		...
		Common.MessageToUser(StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Вариант отчета ""%1"" не предназначен для сохранения.';
				|en = 'Report option ""%1"" cannot be saved.';"), DataOfTheReportVariant.Presentation));
		Return;
	EndIf;
...
Procedure ReadUserSettings()
	...
	If CurrentUserSettings = Undefined Then 
		...
		PersonalSettings.Add("", NStr("ru = 'У отчета нет персональных настроек.';
												|en = 'The report has no personal settings.';"));
		Return;
	EndIf;
...
Procedure CompressReportOptionSettingsCompletion(Files, AdditionalParameters) Export 
	...
	ShowUserNotification(NStr("ru = 'Вариант отчета сохранен в файл';
										|en = 'Report option saved to file';"),, Explanation);
...
Function SuggestionText()
	...
	Return NStr("ru = 'Для сохранения варианта отчета в файл рекомендуется установить расширение для работы с 1С:Предприятием.';
				|en = 'It is recommended that you install 1C:Enterprise Extension before you save the report option to a file.';");
