...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Not ValueIsFilled(HandlerName) Then
		Raise NStr("ru = 'Не передано имя обработчика.';
								|en = 'Handler name wasn''t passed.';");
	EndIf;
	
	Title = NStr("ru = 'Зарегистрированные данные по обработчику %1';
					|en = 'Data registered on handler ""%1""';");
	Title = StringFunctionsClientServer.SubstituteParametersToString(Title, HandlerName);
	
	TextSummaryInformation = NStr("ru = 'Осталось обработать объектов %1 из %2, прогресс - %3%. За указанный период обработано %4.';
	...
	If Not ValueIsFilled(Number(Parameters.ProcessedForPeriod)) Then
		TextSummaryInformation = NStr("ru = 'Осталось обработать объектов %1 из %2, прогресс - %3%. За указанный период нет обработанных данных.';
