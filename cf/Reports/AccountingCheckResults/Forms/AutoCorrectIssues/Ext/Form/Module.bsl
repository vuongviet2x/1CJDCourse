...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If CheckID = "StandardSubsystems.CheckCircularRefs1" Then
		QueryText = NStr("ru = 'Исправление циклических ссылок может занять продолжительное время. Выполнить исправление?';
							|en = 'Fixing the circular references can take a long time. Do you want to fix these?';");
	ElsIf CheckID = "StandardSubsystems.CheckNoPredefinedItems" Then
		QueryText = NStr("ru = 'Создать отсутствующие предопределенные элементы заново?';
							|en = 'Create the missing predefined items?';");
	EndIf;
...
Function ResolveIssueInBackground(CheckID)
	...
	ExecutionParameters.BackgroundJobDescription = NStr("ru = 'Исправление циклических ссылок';
															|en = 'Fixing circular references';");
