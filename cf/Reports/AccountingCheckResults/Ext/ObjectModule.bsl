...
Procedure OnCreateAtServer(Form, Cancel, StandardProcessing) Export
	...
	If Users.IsFullUser() Then
		...
		Command.Title = NStr("ru = 'Игнорировать проблему';
								|en = 'Ignore issue';");
		Command.ToolTip = NStr("ru = 'Игнорировать выбранную проблему';
								|en = 'Ignore the selected issue';");
...
Procedure BeforeLoadVariantAtServer(Form, NewDCSettings) Export
	...
	For Each Filter In NewDCSettings.Filter.Items Do
		...
		RightFilterValue.Add(Catalogs.Users.EmptyRef(), NStr("ru = 'Без ответственного';
																					|en = 'Without person responsible';"));
...
Function LastCheckTooltip()
	...
	If ValueIsFilled(LastCheckInformation.LastCheckDate) Then
		ToolTip = NStr("ru = 'выполнялась %1.';
						|en = 'was performed earlier %1.';");
		ToolTip = StringFunctionsClientServer.SubstituteParametersToString(ToolTip, 
			Format(LastCheckInformation.LastCheckDate, "DLF=D"));
	Else
		ToolTip = NStr("ru = 'еще ни разу не выполнялась.';
						|en = 'has not been performed yet.';");
	EndIf;
	If LastCheckInformation.WarnSecondCheckRequired Then
		ToolTip = " " + NStr("ru = 'Рекомендуется выполнить проверку, чтобы просмотреть актуальные результаты.';
								|en = 'It is recommended that you perform a check to view the relevant results.';");
	EndIf;
	...
	For RowsIndex = 1 To TableHeight Do
		...
		For ColumnsIndex = 1 To TableWidth Do
			...
			If StrStartsWith(AreaText, "%") And StrEndsWith(AreaText, "%") Then
				...
				ReportsServer.OutputHyperlink(Area, DetailsStructure2, NStr("ru = 'Выполнить исправление';
																					|en = 'Fix issue';"));
...
Function LocalizedParametersStructure()
	...
	LocalizedParametersStructure.Insert("LabelError",            NStr("ru = 'Ошибка';
																				|en = 'Error';"));
	LocalizedParametersStructure.Insert("LabelPossibleCauses",  NStr("ru = 'Возможные причины';
																				|en = 'Possible causes';"));
	LocalizedParametersStructure.Insert("LabelRecommendations",      NStr("ru = 'Рекомендации';
																				|en = 'Recommendations';"));
	LocalizedParametersStructure.Insert("LabelDecision",           NStr("ru = 'Решение';
																				|en = 'Solution';"));
	LocalizedParametersStructure.Insert("ObjectsWithIssuesLabel", NStr("ru = 'Проблемные объекты';
																				|en = 'Objects with issues';"));
...
Procedure SetFilterByIssuesList(DCSSettingsFilter, UserSettings, FilterValue)
	...
	FilterElement.Presentation    = NStr("ru = 'Правило проверки В списке';
											|en = 'Check rule In list';") + " """ + FilterPresentation + """";
