...
Function TitleByRefSet(Val RefSet)
	...
	If TypeOf(RefSet) = Type("ValueList") Then
		...
		If TotalRefs = 1 Then
			Return StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Места использования %1';
					|en = 'Occurrences of %1';"), Common.SubjectString(RefSet[0].Value));
		ElsIf TotalRefs > 1 Then
			...
			If EqualType Then
				Return StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Места использования элементов ""%1"" (%2)';
						|en = 'Occurrences of %1 (%2)';"), 
					RefSet[0].Value.Metadata().Presentation(),
					TotalRefs);
			Else		
				Return StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Места использования элементов (%1)';
						|en = 'Occurrences (%1)';"), 
					TotalRefs);
			EndIf;
		EndIf;
	...
	Return NStr("ru = 'Места использования элементов';
				|en = 'Item occurrences';");
