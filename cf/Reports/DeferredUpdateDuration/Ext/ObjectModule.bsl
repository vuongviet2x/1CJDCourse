...
Procedure OnComposeResult(ResultDocument, DetailsData, StandardProcessing, StorageAddress)
	...
	If UpdateInfo.DeferredUpdateCompletedSuccessfully <> Undefined Then
		LastCheckInformation = NStr("ru = 'Отчет сформирован %1';
...
Procedure GanttChart(StatisticsTable1, ResultDocument)
	...
	Colors = New Map;
	Series = GanttChart.SetSeries(NStr("ru = 'Длительность';
												|en = 'Duration';"));
	TooltipTemplate = NStr("ru = '%1 сек., с %2 по %3 %4';
	...
	For Each UpdateStep In DurationOfUpdateSteps Do
		...
		If UpdateCompleted Then
			...
			Var_54_Template = NStr("ru = '%1, %2% от общей длительности';
		...
		DurationInterval.Text = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Начало: %1
			|Конец: %2
			|Длительность: %3';
			|en = 'Start: %1
			|End: %2
