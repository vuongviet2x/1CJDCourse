...
Procedure BeforeImportSettingsToComposer(Context, SchemaKey, VariantKey, NewDCSettings, NewDCUserSettings) Export
	...
	If ReportParameters.OwnerDocument = Undefined Then
		Raise NStr("ru = 'Отчет о движениях документа возможно открыть из формы документа.';
								|en = 'You can open the document record history from the document form.';");
	EndIf;
...
Procedure OnCreateAtServer(Form, Cancel, StandardProcessing) Export
	...
	If Form.Parameters.VariantPresentation = "Details" Then
		Raise NStr("ru = 'Выбранное действие в данном отчете не доступно.';
								|en = 'The selected action is unavailable in this report.';");
	EndIf;
...
Function DataSetForAccountingRegister(RegisterProperties, RegisterMetadata)
	...
	DebitSubmission    = NStr("ru = 'Дт';
								|en = 'Dr';");
	SubmissionCredit   = NStr("ru = 'Кт';
								|en = 'Cr';");
	RepresentationOfSubconto = NStr("ru = 'Субконто';
								|en = 'Extra dimension';");
	AccountSubmission     = NStr("ru = 'Счет';
								|en = 'Account';");
...
Procedure PrepareAHorizontalVersionOfTheAccumulationAndDataRegisters(DCSettings, DCDataSet, DataElement)
	...
	If StandardAttributesStructure.Count() > 0 Then
		...
		GroupParameters.Insert("SelectedGroupPresentation", NStr("ru = 'Стандартные реквизиты';
	...
	If DimensionStructure.Count() > 0 Then
		...
		GroupParameters.Insert("SelectedGroupLocation",  Undefined);
		GroupParameters.Insert("SelectedGroupEmpty",         False);
		GroupParameters.Insert("SelectedGroupPresentation", NStr("ru = 'Измерения';
																		|en = 'Dimensions';"));
	...
	If ResourcesStructure.Count() > 0 Then
		...
		GroupParameters.Insert("SelectedGroupLocation",  Undefined);
		GroupParameters.Insert("SelectedGroupEmpty",         False);
		GroupParameters.Insert("SelectedGroupPresentation", NStr("ru = 'Ресурсы';
																		|en = 'Resources';"));
	...
	If AttributesStructure1.Count() > 0 Then
		...
		GroupParameters.Insert("SelectedGroupLocation",  Undefined);
		GroupParameters.Insert("SelectedGroupEmpty",         False);
		GroupParameters.Insert("SelectedGroupPresentation", NStr("ru = 'Реквизиты';
																		|en = 'Attributes';"));
...
Procedure PrepareHorizontalOptionOfCalculationRegisters(DCSettings, DCDataSet, DataElement)
	...
	UserHeader = NStr("ru = 'Регистр расчета';
									|en = 'Calculation register';") + " " + RegisterPresentation;
...
Procedure PrepareVerticalOptionOfCalculationRegisters(DCSettings, DCDataSet, DataElement)
	...
	UserHeader = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Регистр расчета %1';
																							|en = 'Calculation register %1';"),
...
Procedure PrepareHorizontalOptionOfAccountingRegisters(DCSettings, DCDataSet, DataElement)
	...
	UserHeader = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Регистр бухгалтерии %1';
																							|en = 'Accounting register %1';"),
		RegisterPresentation);
	...
	For LineNumber = 1 To ResultDocument.TableHeight Do
		...
		If NotesBorder = StringHeight Then
			...
			NoteText2 = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'В этой строке: %1';
					|en = 'In this line: %1';"), NoteProperties.Note + ".");
...
Procedure GetRegistersProperties(Context, OwnerDocument = Undefined)
	...
	For Each Movement In DocumentRegisterRecords Do
		...
		If Common.IsAccumulationRegister(RegisterMetadata) Then
			...
			RegisterProperties.RegisterKindPresentation = NStr("ru = 'Регистр накопления';
		...
		ElsIf Common.IsInformationRegister(RegisterMetadata) Then
			...
			RegisterProperties.RegisterKindPresentation = NStr("ru = 'Регистр сведений';
		...
		ElsIf Common.IsAccountingRegister(RegisterMetadata) Then
			...
			RegisterProperties.RegisterKindPresentation = NStr("ru = 'Регистр бухгалтерии';
		...
		ElsIf Common.IsCalculationRegister(RegisterMetadata) Then
			...
			RegisterProperties.RegisterKindPresentation = NStr("ru = 'Регистр расчета';
	...
	For Each RegisterProperties In RegistersProperties Do 
		...
		For IndexOf = 0 To StringHeight - 1 Do
			...
			For Each GroupOfFields In SelectedFieldsGroups Do
				...
				Title = "";
				HeaderOfStandardBankDetails = NStr("ru = 'Стандартные реквизиты';
														|en = 'Standard attributes.';");
				MeasurementHeader = NStr("ru = 'Измерения';
											|en = 'Dimensions';");
				ResourceHeader = NStr("ru = 'Ресурсы';
										|en = 'Resources';");
				TitleOfTheBankDetails = NStr("ru = 'Реквизиты';
											|en = 'Attributes';");
				TitleOfTheCalculationData = NStr("ru = 'Данные расчета';
												|en = 'Calculation data';");
				
				If GroupOfFields.Title = HeaderOfStandardBankDetails Then
					Title = NStr("ru = 'Стандартный реквизит';
									|en = 'Standard attribute';");
				ElsIf GroupOfFields.Title = MeasurementHeader Then
					Title = NStr("ru = 'Измерение';
									|en = 'Dimension';");
				ElsIf GroupOfFields.Title = ResourceHeader Then
					Title = NStr("ru = 'Ресурс';
									|en = 'Resource';");
				ElsIf GroupOfFields.Title = TitleOfTheBankDetails Then
					Title = NStr("ru = 'Реквизит';
									|en = 'Attribute';");
				ElsIf GroupOfFields.Title = TitleOfTheCalculationData Then
					Title = NStr("ru = 'Данные расчета';
									|en = 'Calculation data';");
				EndIf;
