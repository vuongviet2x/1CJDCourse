...
Procedure OnCreateAtServer(Form, Cancel, StandardProcessing) Export
	...
	If AccessManagementInternal.SimplifiedAccessRightsSetupInterface() Then
		...
		Field.Title = NStr("ru = 'Профиль пользователя';
								|en = 'User profile';");
...
Procedure OnComposeResult(ResultDocument, DetailsData, StandardProcessing)
	...
	If Not Common.SubsystemExists("StandardSubsystems.ReportsOptions") Then
		ErrorText = NStr("ru = 'Для использования отчета требуется внедрить подсистему Варианты отчетов.';
							|en = 'To use the report, deploy the Report options subsystem.';");
		Raise ErrorText;
	EndIf;
...
Procedure FinishOutput(ResultDocument, DetailsData, RightsSettings)
	...
	AccessGroupTitle = NStr("ru = 'Группа доступа';
									|en = 'Access group';");
	If AccessManagementInternal.SimplifiedAccessRightsSetupInterface() Then
		AccessGroupTitle = NStr("ru = 'Профиль пользователя';
										|en = 'User profile';");
	EndIf;
	...
	// ACC:163-on
	TextRightNotAssigned = NStr("ru = '●';
								|en = '●';");
	TextRightAllowed   = NStr("ru = '✔';
								|en = '✔';");
	TextRightForbidden   = NStr("ru = '✘';
								|en = '✘';");
...
Function UsersRights()
	...
	ElsIf VariantWithRestrictedAccess Then
		...
		Query.SetParameter("TextAllowed", " (" + NStr("ru = 'Разрешенные';
																	|en = 'Allowed';")+ ")");
		Query.SetParameter("TextForbidden", " (" + NStr("ru = 'Запрещенные';
																	|en = 'Denied';") + ")");
		Query.SetParameter("TextAllowedUsers", " (" + NStr("ru = 'Разрешенные';
																				|en = 'Allowed';") + ") - "
			+ NStr("ru = 'авторизованный пользователь и его группы всегда разрешены';
					|en = 'Authorized user and their groups are always allowed';"));
		Query.SetParameter("TextForbiddenUsers", " (" + NStr("ru = 'Запрещенные';
																				|en = 'Denied';") + ") - "
			+ NStr("ru = 'авторизованный пользователь и его группы всегда разрешены';
					|en = 'Authorized user and their groups are always allowed';"));
		Query.SetParameter("TextRestrictionWithoutAccessTypes", "<" + NStr("ru = 'Ограничение без видов доступа';
																				|en = 'Restriction without access kinds';")+ ">");
		Query.SetParameter("TextUnlimited", "<" + NStr("ru = 'Без ограничения';
																	|en = 'No restriction';") + ">");
		Query.SetParameter("TextAllAllowed", "<" + NStr("ru = 'Все разрешены';
																	|en = 'All allowed';") + ">");
		Query.SetParameter("TextAllForbidden", "<" + NStr("ru = 'Все запрещены';
																	|en = 'All denied';") + ">");
...
Function RightsSettingsOnObjects()
	...
	Query.SetParameter("ViewPersonal",  NStr("ru = 'Персонально';
																|en = 'Personal';"));
	Query.SetParameter("ViewUndefined", NStr("ru = 'Неопределено';
																|en = 'Undefined';"));
	Query.SetParameter("ViewUserGroup",
		" (" + NStr("ru = 'Группа пользователей';
					|en = 'User group';") + ")");
	Query.SetParameter("ExternalUserGroupView",
		" (" + NStr("ru = 'Группа внешних пользователей';
					|en = 'External user group';") + ")");
...
Function DescriptionColumnsForSubfolders()
	...
	Result.Insert("Title", NStr("ru = 'Для подпапок';
										|en = 'For subfolders';"));
	Result.Insert("ToolTip",
		NStr("ru = 'Права не только для текущей папки, но и для ее нижестоящих папок';
...
Function SettingsRightsLegend(TitlesRight, HasHierarchy)
	...
	If HasHierarchy Then
		...
		NewRow.HintPermissions = NStr("ru = 'Наследование прав от вышестоящих папок';
