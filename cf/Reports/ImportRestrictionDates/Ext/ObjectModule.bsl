...
Function ClosingDatesPrepared(DataParameters)
	...
	For Each String In Table Do
		...
		ElsIf ValueIsFilled(String.Section) Then
			String.ObjectPresentation = NStr("ru = 'Для всех объектов, кроме указанных';
												|en = 'For all objects except for the specified ones';");
		Else
			String.ObjectPresentation = NStr("ru = 'Для всех разделов и объектов, кроме указанных';
												|en = 'For all sections and objects except for the specified ones';");
		EndIf;
		
		If ValueIsFilled(String.Section) Then
			String.SectionPresentation = String(String.Section);
		Else
			String.SectionPresentation = "<" + NStr("ru = 'Общая дата';
													|en = 'Common date';") + ">";
		EndIf;
		
		If String.SettingsRecipient = Enums.PeriodClosingDatesPurposeTypes.ForAllInfobases Then
			String.SettingsRecipientPresentation = NStr("ru = 'Для всех информационных баз, кроме указанных';
														|en = 'For all infobases except for the specified ones';");
		Else
			String.SettingsRecipientPresentation = String(TypeOf(String.SettingsRecipient)) + ":"
				+ ?(ValueIsFilled(String.SettingsRecipient),
					...
					NStr("ru = 'Все информационные базы';
						|en = 'All infobases';"));
		EndIf;
		
		If String.SettingsOwner = Enums.PeriodClosingDatesPurposeTypes.ForAllInfobases Then
			String.SettingsOwnerPresentation = NStr("ru = 'Для всех информационных баз, кроме указанных';
														|en = 'For all infobases except for the specified ones';");
		Else
