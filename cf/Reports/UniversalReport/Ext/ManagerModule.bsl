...
Procedure CustomizeReportOptions(Settings, ReportSettings) Export
	...
	OptionSettings.LongDesc = NStr("ru = 'Универсальный отчет по справочникам, документам, регистрам.';
										|en = 'Universal report on catalogs, documents, and registers.';");
...
Function AvailableMetadataObjectsTypes()
	...
	If HasMetadataTypeObjects(Metadata.Catalogs) Then
		AvailableValues.Add("Catalogs", NStr("ru = 'Справочник';
														|en = 'Catalog';"), , PictureLib.Catalog);
	EndIf;
	
	If HasMetadataTypeObjects(Metadata.Documents) Then
		AvailableValues.Add("Documents", NStr("ru = 'Документ';
													|en = 'Document';"), , PictureLib.Document);
	EndIf;
	
	If HasMetadataTypeObjects(Metadata.InformationRegisters) Then
		AvailableValues.Add("InformationRegisters", NStr("ru = 'Регистр сведений';
															|en = 'Information register';"), , PictureLib.InformationRegister);
	EndIf;
	...
	If HasMetadataTypeObjects(Metadata.AccumulationRegisters) Then
		AvailableValues.Add("AccumulationRegisters", NStr("ru = 'Регистр накопления';
																|en = 'Accumulation register';"), , PictureLib.AccumulationRegister);
	EndIf;
	
	If HasMetadataTypeObjects(Metadata.AccountingRegisters) Then
		AvailableValues.Add("AccountingRegisters", NStr("ru = 'Регистр бухгалтерии';
																|en = 'Accounting register';"), , PictureLib.AccountingRegister);
	EndIf;
	
	If HasMetadataTypeObjects(Metadata.CalculationRegisters) Then
		AvailableValues.Add("CalculationRegisters", NStr("ru = 'Регистр расчета';
															|en = 'Calculation register';"), , PictureLib.CalculationRegister);
	EndIf;
	
	If HasMetadataTypeObjects(Metadata.ChartsOfCalculationTypes) Then
		AvailableValues.Add("ChartsOfCalculationTypes", NStr("ru = 'Планы видов расчета';
															|en = 'Charts of calculation types';"), , PictureLib.ChartOfCalculationTypes);
	EndIf;
	
	If HasMetadataTypeObjects(Metadata.Tasks) Then
		AvailableValues.Add("Tasks", NStr("ru = 'Задачи';
													|en = 'Tasks';"), , PictureLib.Task);
	EndIf;
	
	If HasMetadataTypeObjects(Metadata.BusinessProcesses) Then
		AvailableValues.Add("BusinessProcesses", NStr("ru = 'Бизнес-процессы';
															|en = 'Business processes';"), , PictureLib.BusinessProcess);
	EndIf;
...
Function AvailableTables(MetadataObjectType, MetadataObjectName)
	...
	ElsIf MetadataObjectType = "InformationRegisters" Then 
		If MetadataObject.InformationRegisterPeriodicity
			...
			AvailableValues.Add("SliceLast", NStr("ru = 'Срез последних';
															|en = 'Last values slice';"));
			AvailableValues.Add("SliceFirst", NStr("ru = 'Срез первых';
															|en = 'First values slice';"));
		EndIf;
	ElsIf MetadataObjectType = "AccumulationRegisters" Then
		If MetadataObject.RegisterType = Metadata.ObjectProperties.AccumulationRegisterType.Balance Then
			AvailableValues.Add("BalanceAndTurnovers", NStr("ru = 'Остатки и обороты';
																|en = 'Balances and turnovers';"));
			AvailableValues.Add("Balance", NStr("ru = 'Остатки';
														|en = 'Balances';"));
			AvailableValues.Add("Turnovers", NStr("ru = 'Обороты';
														|en = 'Turnovers';"));
		Else
			AvailableValues.Add("Turnovers", NStr("ru = 'Обороты';
														|en = 'Turnovers';"));
	...
	ElsIf MetadataObjectType = "AccountingRegisters" Then
		...
		If MetadataObject.Correspondence Then
			AvailableValues.Add("DrCrTurnovers", NStr("ru = 'Обороты Дт/Кт';
															|en = 'Dr/Cr turnovers';"));
		EndIf;
		AvailableValues.Add("RecordsWithExtDimensions", NStr("ru = 'Движения с субконто';
															|en = 'Records with extra dimensions';"));
	ElsIf MetadataObjectType = "CalculationRegisters" Then 
		If MetadataObject.ActionPeriod Then
			AvailableValues.Add("ScheduleData", NStr("ru = 'Данные графика';
															|en = 'Chart data';"));
			AvailableValues.Add("ActualActionPeriod", NStr("ru = 'Фактический период действия';
																		|en = 'Actual validity period';"));
		EndIf;
	ElsIf MetadataObjectType = "ChartsOfCalculationTypes" Then
		If MetadataObject.DependenceOnCalculationTypes
			...
			AvailableValues.Add("BaseCalculationTypes", NStr("ru = 'Базовые виды расчета';
																	|en = 'Baseline calculation types.';"));
		EndIf;
		
		AvailableValues.Add("LeadingCalculationTypes", NStr("ru = 'Ведущие виды расчета';
																|en = 'Primary calculation types';"));
		
		If MetadataObject.ActionPeriodUse Then 
			AvailableValues.Add("DisplacingCalculationTypes", NStr("ru = 'Вытесняющие виды расчета';
																		|en = 'Overriding calculation types';"));
		EndIf;
	EndIf;
...
Procedure AddObjectTotals(Val ReportParameters, Val DataCompositionSchema)
	...
	If ValueIsFilled(ReferenceDetails.Synonym) And ReferenceDetails.Synonym <> NStr("ru = 'Ссылка';
																					|en = 'Reference';") Then // ACC:1391 - If the synonym of the "Ref" attribute differs from the standard "Ref", the column header displays the object presentation.
...
Procedure AddRegisterTotals(Val ReportParameters, Val DataCompositionSchema)
	...
	If ReportParameters.TableName = "BalanceAndTurnovers" Then
		...
		DataSetField.Title = NStr("ru = 'Регистратор';
											|en = 'Recorder';");
	...
	If ReportParameters.MetadataObjectType = "AccountingRegisters" Then
		...
		AccountField = AddDataSetField(DataCompositionSchema.DataSets[0], "Account", NStr("ru = 'Счет';
																								|en = 'Account';"));
		...
		For ExtDimensionNumber = 1 To ExtDimensionCount Do
			ExtDimensionField = AddDataSetField(DataCompositionSchema.DataSets[0], "ExtDimension" + ExtDimensionNumber, NStr("ru = 'Субконто';
																															|en = 'Extra dimension';") + " " + ExtDimensionNumber);
	...
	For Each Resource In MetadataObject.Resources Do
		...
		If ReportParameters.TableName = "Turnovers" Then
			...
			If ReportParameters.MetadataObjectType = "AccountingRegisters" Then
				AddDataSetField(DataCompositionSchema.DataSets[0], Resource.Name + "TurnoverDr", Resource.Synonym + " " + NStr("ru = 'оборот Дт';
																																	|en = 'Dr turnover';"), Resource.Name + "TurnoverDr");
				AddTotalField(DataCompositionSchema, Resource.Name + "TurnoverDr");
				AddDataSetField(DataCompositionSchema.DataSets[0], Resource.Name + "TurnoverCr", Resource.Synonym + " " + NStr("ru = 'оборот Кт';
																																	|en = 'Cr turnover';"), Resource.Name + "TurnoverCr");
				AddTotalField(DataCompositionSchema, Resource.Name + "TurnoverCr");
				
				If Not Resource.Balance Then
					AddDataSetField(DataCompositionSchema.DataSets[0], Resource.Name + "BalancedTurnover", Resource.Synonym + " " + NStr("ru = 'кор. оборот';
																																			|en = 'corr. turnover';"), Resource.Name + "BalancedTurnover");
					AddTotalField(DataCompositionSchema, Resource.Name + "BalancedTurnover");
					AddDataSetField(DataCompositionSchema.DataSets[0], Resource.Name + "BalancedTurnoverDr", Resource.Synonym + " " + NStr("ru = 'кор. оборот Дт';
																																			|en = 'Dr corr. turnover';"), Resource.Name + "BalancedTurnoverDr");
					AddTotalField(DataCompositionSchema, Resource.Name + "BalancedTurnoverDr");
					AddDataSetField(DataCompositionSchema.DataSets[0], Resource.Name + "BalancedTurnoverCr", Resource.Synonym + " " + NStr("ru = 'кор. оборот Кт';
																																			|en = 'Cr corr. turnover';"), Resource.Name + "BalancedTurnoverCr");
		...
		ElsIf ReportParameters.TableName = "RecordsWithExtDimensions" Then
			...
			Else
				AddDataSetField(DataCompositionSchema.DataSets[0], Resource.Name + "Dr", Resource.Synonym + " " + NStr("ru = 'Дт';
																																|en = 'Dr';"), Resource.Name + "Dr");
				AddTotalField(DataCompositionSchema, Resource.Name + "Dr");
				AddDataSetField(DataCompositionSchema.DataSets[0], Resource.Name + "Cr", Resource.Synonym + " " + NStr("ru = 'Кт';
																																|en = 'Cr';"), Resource.Name + "Cr");
				AddTotalField(DataCompositionSchema, Resource.Name + "Cr");
			EndIf;
			
		ElsIf ReportParameters.TableName = "BalanceAndTurnovers" Then
			
			SetField = AddDataSetField(DataCompositionSchema.DataSets[0], Resource.Name + "OpeningBalance", Resource.Synonym + " " + NStr("ru = 'нач. остаток';
																																						|en = 'open balance';"), Resource.Name + "OpeningBalance");
			...
			If ReportParameters.MetadataObjectType = "AccountingRegisters" Then
				...
				SetField = AddDataSetField(DataCompositionSchema.DataSets[0], Resource.Name + "OpeningBalanceDr", Resource.Synonym + " " + NStr("ru = 'нач. остаток Дт';
																																							|en = 'Dr open balance';"), Resource.Name + "OpeningBalanceDr");
				SetField.Role.Balance = True;
				SetField.Role.BalanceType = DataCompositionBalanceType.OpeningBalance;
				...
				SetField = AddDataSetField(DataCompositionSchema.DataSets[0], Resource.Name + "OpeningBalanceCr", Resource.Synonym + " " + NStr("ru = 'нач. остаток Кт';
																																							|en = 'Cr open balance';"), Resource.Name + "OpeningBalanceCr");
				SetField.Role.Balance = True;
				SetField.Role.BalanceType = DataCompositionBalanceType.OpeningBalance;
				...
				SetField = AddDataSetField(DataCompositionSchema.DataSets[0], Resource.Name + "OpeningSplittedBalanceDr", Resource.Synonym + " " + NStr("ru = 'нач. развернутый остаток Дт';
																																										|en = 'Dr open balance detailed';"), Resource.Name + "OpeningSplittedBalanceDr");
				SetField.Role.Balance = True;
				SetField.Role.BalanceType = DataCompositionBalanceType.OpeningBalance;
				...
				SetField = AddDataSetField(DataCompositionSchema.DataSets[0], Resource.Name + "OpeningSplittedBalanceCr", Resource.Synonym + " " + NStr("ru = 'нач. развернутый остаток Кт';
																																										|en = 'Cr open balance detailed';"), Resource.Name + "OpeningSplittedBalanceCr");
				SetField.Role.Balance = True;
				SetField.Role.BalanceType = DataCompositionBalanceType.OpeningBalance;
				SetField.Role.AccountingBalanceType = DataCompositionAccountingBalanceType.None;
			...
			AddDataSetField(DataCompositionSchema.DataSets[0], Resource.Name + "Turnover", Resource.Synonym + " " + NStr("ru = 'оборот';
																																|en = 'turnover';"), Resource.Name + "Turnover");
			AddTotalField(DataCompositionSchema, Resource.Name + "Turnover");
			
			If ReportParameters.MetadataObjectType = "AccumulationRegisters" Then
				AddDataSetField(DataCompositionSchema.DataSets[0], Resource.Name + "Receipt", Resource.Synonym + " " + NStr("ru = 'приход';
																																	|en = 'income';"), Resource.Name + "Receipt");
				AddTotalField(DataCompositionSchema, Resource.Name + "Receipt");
				
				AddDataSetField(DataCompositionSchema.DataSets[0], Resource.Name + "Expense", Resource.Synonym + " " + NStr("ru = 'расход';
																																	|en = 'expense';"), Resource.Name + "Expense");
			...
			SetField = AddDataSetField(DataCompositionSchema.DataSets[0], Resource.Name + "ClosingBalance", Resource.Synonym + " " + NStr("ru = 'кон. остаток';
																																					|en = 'close balance';"), Resource.Name + "ClosingBalance");
			...
			If ReportParameters.MetadataObjectType = "AccountingRegisters" Then
				...
				SetField = AddDataSetField(DataCompositionSchema.DataSets[0], Resource.Name + "ClosingBalanceDr", Resource.Synonym + " " + NStr("ru = 'кон. остаток Дт';
																																							|en = 'Dr close balance';"), Resource.Name + "ClosingBalanceDr");
				SetField.Role.Balance = True;
				SetField.Role.BalanceType = DataCompositionBalanceType.ClosingBalance;
				SetField.Role.AccountingBalanceType = DataCompositionAccountingBalanceType.Debit;
				SetField.Role.AccountField = "Account";
				...
				SetField = AddDataSetField(DataCompositionSchema.DataSets[0], Resource.Name + "ClosingBalanceCr", Resource.Synonym + " " + NStr("ru = 'кон. остаток Кт';
																																							|en = 'Cr close balance';"), Resource.Name + "ClosingBalanceCr");
				SetField.Role.Balance = True;
				SetField.Role.BalanceType = DataCompositionBalanceType.ClosingBalance;
				SetField.Role.AccountingBalanceType = DataCompositionAccountingBalanceType.Credit;
				SetField.Role.AccountField = "Account";
				...
				SetField = AddDataSetField(DataCompositionSchema.DataSets[0], Resource.Name + "ClosingSplittedBalanceDr", Resource.Synonym + " " + NStr("ru = 'кон. развернутый остаток Дт';
																																										|en = 'Dr close balance detailed';"), Resource.Name + "ClosingSplittedBalanceDr");
				SetField.Role.Balance = True;
				SetField.Role.BalanceType = DataCompositionBalanceType.ClosingBalance;
				SetField.Role.AccountingBalanceType = DataCompositionAccountingBalanceType.None;
				SetField.Role.BalanceGroup = "DetldBal" + Resource.Name + "Dr";
				...
				SetField = AddDataSetField(DataCompositionSchema.DataSets[0], Resource.Name + "ClosingSplittedBalanceCr", Resource.Synonym + " " + NStr("ru = 'кон. развернутый остаток Кт';
																																										|en = 'Cr close balance detailed';"), Resource.Name + "ClosingSplittedBalanceCr");
				SetField.Role.Balance = True;
				SetField.Role.BalanceType = DataCompositionBalanceType.ClosingBalance;
				SetField.Role.AccountingBalanceType = DataCompositionAccountingBalanceType.None;
				SetField.Role.BalanceGroup = "DetldBal" + Resource.Name + "Cr";
		...
		ElsIf ReportParameters.TableName = "Balance" Then
			AddDataSetField(DataCompositionSchema.DataSets[0], Resource.Name + "Balance", Resource.Synonym + " " + NStr("ru = 'остаток';
																																|en = 'balance';"), Resource.Name + "Balance");
			AddTotalField(DataCompositionSchema, Resource.Name + "Balance");
			
			If ReportParameters.MetadataObjectType = "AccountingRegisters" Then
				AddDataSetField(DataCompositionSchema.DataSets[0], Resource.Name + "BalanceDr", Resource.Synonym + " " + NStr("ru = 'остаток Дт';
																																		|en = 'Dr balance';"), Resource.Name + "BalanceDr");
				AddTotalField(DataCompositionSchema, Resource.Name + "BalanceDr");
				
				AddDataSetField(DataCompositionSchema.DataSets[0], Resource.Name + "BalanceCr", Resource.Synonym + " " + NStr("ru = 'остаток Кт';
																																		|en = 'Cr balance';"), Resource.Name + "BalanceCr");
				AddTotalField(DataCompositionSchema, Resource.Name + "BalanceCr");
				
				AddDataSetField(DataCompositionSchema.DataSets[0], Resource.Name + "SplittedBalanceDr", Resource.Synonym + " " + NStr("ru = 'развернутый остаток Дт';
																																				|en = 'Dr balance detailed';"), Resource.Name + "SplittedBalanceDr");
				AddTotalField(DataCompositionSchema, Resource.Name + "SplittedBalanceDr");
				
				AddDataSetField(DataCompositionSchema.DataSets[0], Resource.Name + "SplittedBalanceCr", Resource.Synonym + " " + NStr("ru = 'развернутый остаток Кт';
																																				|en = 'Cr balance detailed';"), Resource.Name + "SplittedBalanceCr");
...
Procedure AddSubordinateRecordsCountTotals(DataCompositionSchema)
	...
	FieldToEval.Title = NStr("ru = 'Количество записей';
...
Function AddPeriodFieldsInDataSet(DataSet, ThereIsFieldLogger)
	...
	PeriodsList.Add("SecondPeriod",   NStr("ru = 'Период секунда';
													|en = 'Period second';"));
	PeriodsList.Add("MinutePeriod",    NStr("ru = 'Период минута';
													|en = 'Period minute';"));
	PeriodsList.Add("HourPeriod",       NStr("ru = 'Период час';
													|en = 'Period hour';"));
	PeriodsList.Add("DayPeriod",      NStr("ru = 'Период день';
													|en = 'Period day';"));
	PeriodsList.Add("WeekPeriod",    NStr("ru = 'Период неделя';
													|en = 'Period week';"));
	PeriodsList.Add("TenDaysPeriod",    NStr("ru = 'Период декада';
													|en = 'Period ten-day';"));
	PeriodsList.Add("MonthPeriod",     NStr("ru = 'Период месяц';
													|en = 'Period month';"));
	PeriodsList.Add("QuarterPeriod",   NStr("ru = 'Период квартал';
													|en = 'Period quarter';"));
	PeriodsList.Add("HalfYearPeriod", NStr("ru = 'Период полугодие';
													|en = 'Period half-year';"));
	PeriodsList.Add("YearPeriod",       NStr("ru = 'Период год';
													|en = 'Period year';"));
	...
	DataSetFieldsFolder.Title   = NStr("ru = 'Периоды';
												|en = 'Periods';");
...
Procedure AddIndicators(ReportParameters, DCSettings)
	...
	If ReportParameters.TableName = "BalanceAndTurnovers" Then
		...
		SelectedFieldsOpeningBalance.Title = NStr("ru = 'Нач. остаток';
														|en = 'Open balance';");
		...
		If ReportParameters.MetadataObjectType = "AccumulationRegisters" Then
			...
			SelectedFieldsReceipt.Title = NStr("ru = 'Приход';
												|en = 'Income';");
			...
			SelectedFieldsExpense.Title = NStr("ru = 'Расход';
												|en = 'Expense';");
		...
		ElsIf ReportParameters.MetadataObjectType = "AccountingRegisters" Then
			...
			SelectedFieldsTurnovers.Title = NStr("ru = 'Обороты';
													|en = 'Turnovers';");
		...
		SelectedFieldsClosingBalance.Title = NStr("ru = 'Кон. остаток';
														|en = 'Close balance';");
	...
	ElsIf ReportParameters.MetadataObjectType = "AccountingRegisters" Then
		For Each Resource In MetadataObject.Resources Do
			...
			SelectedFields = DCSettings.Selection;
			If ReportParameters.TableName = "Turnovers" Then
				ReportsServer.AddSelectedField(SelectedFields, Resource.Name + "TurnoverDr", Resource.Synonym + " " + NStr("ru = 'оборот Дт';
																														|en = 'Dr turnover';"));
				ReportsServer.AddSelectedField(SelectedFields, Resource.Name + "TurnoverCr", Resource.Synonym + " " + NStr("ru = 'оборот Кт';
																														|en = 'Cr turnover';"));
			ElsIf ReportParameters.TableName = "DrCrTurnovers" Then
				If Resource.Balance Then
					ReportsServer.AddSelectedField(SelectedFields, Resource.Name + "Turnover", Resource.Synonym + " " + NStr("ru = 'оборот';
																														|en = 'turnover';"));
				Else
					ReportsServer.AddSelectedField(SelectedFields, Resource.Name + "TurnoverDr", Resource.Synonym + " " + NStr("ru = 'оборот Дт';
																															|en = 'Dr turnover';"));
				...
				EndIf;
			ElsIf ReportParameters.TableName = "Balance" Then
				ReportsServer.AddSelectedField(SelectedFields, Resource.Name + "BalanceDr", Resource.Synonym + " " + NStr("ru = 'ост. Дт';
																														|en = 'Dr balance';"));
				ReportsServer.AddSelectedField(SelectedFields, Resource.Name + "BalanceCr", Resource.Synonym + " " + NStr("ru = 'ост. Кт';
																														|en = 'Cr balance';"));
			ElsIf ReportParameters.TableName = "BalanceAndTurnovers" Then
				ReportsServer.AddSelectedField(SelectedFieldsOpeningBalance, Resource.Name + "OpeningBalanceDr", Resource.Synonym + " " + NStr("ru = 'нач. ост. Дт';
																																				|en = 'Dr open balance';"));
				ReportsServer.AddSelectedField(SelectedFieldsOpeningBalance, Resource.Name + "OpeningBalanceCr", Resource.Synonym + " " + NStr("ru = 'нач. ост. Кт';
																																				|en = 'Cr open balance';"));
				ReportsServer.AddSelectedField(SelectedFieldsTurnovers, Resource.Name + "TurnoverDr", Resource.Synonym + " " + NStr("ru = 'оборот Дт';
																																|en = 'Dr turnover';"));
				ReportsServer.AddSelectedField(SelectedFieldsTurnovers, Resource.Name + "TurnoverCr", Resource.Synonym + " " + NStr("ru = 'оборот Кт';
																																|en = 'Cr turnover';"));
				ReportsServer.AddSelectedField(SelectedFieldsClosingBalance, Resource.Name + "ClosingBalanceDr", " " + Resource.Synonym + NStr("ru = 'кон. ост. Дт';
																																				|en = 'Dr close balance';"));
				ReportsServer.AddSelectedField(SelectedFieldsClosingBalance, Resource.Name + "ClosingBalanceCr", " " + Resource.Synonym + NStr("ru = 'кон. ост. Кт';
																																				|en = 'Cr close balance';"));
			ElsIf ReportParameters.TableName = "RecordsWithExtDimensions" Or ReportParameters.TableName = "" Then
				...
				Else
					ReportsServer.AddSelectedField(SelectedFields, Resource.Name + "Dr", Resource.Synonym + " " + NStr("ru = 'Дт';
																													|en = 'Dr';"));
					ReportsServer.AddSelectedField(SelectedFields, Resource.Name + "Cr", Resource.Synonym + " " + NStr("ru = 'Кт';
																													|en = 'Cr';"));
				EndIf;
			EndIf;
		EndDo;
...
Function DataSource(ManagerType, ObjectName)
	...
	If Common.MetadataObjectByFullName(FullObjectName) = Undefined Then 
		WriteLogEvent(NStr("ru = 'Варианты отчетов.Установка источника данных универсального отчета';
										|en = 'Report options. Configure universal report data source';", 
			Common.DefaultLanguageCode()),
			EventLogLevel.Error,
			Metadata.Catalogs.ReportsOptions,,
			StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Источник данных %1 отсутствует';
																		|en = 'Cannot find data source for %1';"), 
				FullObjectName));
		Return Undefined;
	EndIf;
...
Function DataSourceMetadata(DataSource, Context)
	...
	Except
		MessageText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Отчет не может быть сформирован по причине: %1 %2';
				|en = 'Cannot generate the report due to: %1%2';"),
			ErrorProcessing.BriefErrorDescription(ErrorInfo()),
			NStr("ru = 'Выберите другой справочник или документ.';
				|en = 'Select another catalog or document.';"));
		WriteLogEvent(NStr("ru = 'Формирование варианта универсального отчета';
										|en = 'Generate a universal report option';", Common.DefaultLanguageCode()),
			EventLogLevel.Warning, Metadata.Reports.UniversalReport,, MessageText);
		If TypeOf(Context) = Type("ClientApplicationForm") Then
			RecommendationText = NStr("ru = 'Отчет не может быть сформирован. Выберите другой справочник или документ.';
									|en = 'Cannot generate the report. Select another catalog or document.';");
...
Procedure SetStandardReportHeader(Context, Settings, FixedParameters, AvailableValues) Export 
	...
	ElsIf ParameterValues.TableName = Undefined
		...
		Title = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Универсальный отчет: %1 ""%2""';
				|en = 'Universal report: %1 ""%2""';"),
	...
	ElsIf ParameterValues.TableName <> Undefined
		...
		Title = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Универсальный отчет: %1 ""%2"" - таблица ""%3""';
				|en = 'Universal report: %1 ""%2"" - table ""%3""';"),
			ParameterValues.MetadataObjectType,
			ParameterValues.MetadataObjectName,
	...
	ElsIf ParameterValues.TableName = Undefined
		...
		Title = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Универсальный отчет: %1 ""%2"" за %3';
				|en = 'Universal report: %1 ""%2"" for %3';"),
			ParameterValues.MetadataObjectType,
			ParameterValues.MetadataObjectName,
			PeriodPresentation);
	Else
		Title = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Универсальный отчет: %1 ""%2"" - таблица ""%3"" за %4';
				|en = 'Universal report: %1 ""%2"" - table ""%3"" for %4';"),
			ParameterValues.MetadataObjectType,
			ParameterValues.MetadataObjectName,
			ParameterValues.TableName,
			PeriodPresentation);
...
Function MainDataTableNamePresentation()
	...
	Return NStr("ru = 'Основные данные';
				|en = 'Main data';");
