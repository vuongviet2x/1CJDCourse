...
Procedure OutputAnswerAnswersOptions(ReportTable,TreeRow,Template,FullCode)
	...
	If TreeRow.Rows[0].ReplyType = Enums.TypesOfAnswersToQuestion.Boolean Then
		...
		Area.Parameters.AnswerOption = NStr("ru = 'Да';
												|en = 'Yes';");
		...
		Area = Template.GetArea("AnswersOptions");
		Area.Parameters.AnswerOption = NStr("ru = 'Нет';
												|en = 'No';");
...
Function ExecuteQueryOnSurveysRespondentsWithSpecificCompositionOfRespondents(Survey)
	...
	Query.SetParameter("Survey",Survey);
	Query.SetParameter("Filled2",NStr("ru = 'Заполнил';
												|en = 'Respondent';"));
	Query.SetParameter("StartedFilling",NStr("ru = 'Начал заполнять';
													|en = 'Started filling';"));
	Query.SetParameter("NotFilled2",NStr("ru = 'Не заполнял';
												|en = 'Not filled';"));
...
Function SurveyPresentationForHeader(SurveyDetails)
	...
	If SurveyDetails.StartDate <> Date(1, 1, 1) And SurveyDetails.EndDate <> Date(1, 1, 1) Then
		Result = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Опрос проводился в период с %1 по %2 на основании документа %3.';
				|en = 'A survey is held from %1 to %2 on a basis of a document %3.';"),
	...
	ElsIf SurveyDetails.StartDate <> Date(1, 1, 1) Then
		Result = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Опрос проводился в период с %1 на основании документа %2.';
				|en = 'A survey is held from %1 on a basis of a document %2.';"),
			Format(SurveyDetails.StartDate, "DLF=DD"), 
			SurveyDetails.Presentation);
	ElsIf SurveyDetails.EndDate <> Date(1, 1, 1) Then
		Result = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Опрос проводился в период по %1 на основании документа %2.';
				|en = 'A survey is held until %1 on a basis of a document %2.';"),
			Format(SurveyDetails.EndDate, "DLF=DD"), 
			SurveyDetails.Presentation);
	Else
		Result = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Опрос на основании документа %1.';
				|en = 'A survey is held on a basis of a document %1.';"),
			SurveyDetails.Presentation);
	EndIf; 
	Return Result;
