...
Procedure ConfigureJobSchedule(Command)
	...
	Else
		ShowMessageBox(,NStr("ru = 'Невозможно изменить расписание регламентного задания: регламентное задание не существует в этой версии приложения.';
									|en = 'Cannot change the job''s schedule: The scheduled job does not exist in this app version.';"));
	EndIf;
...
Procedure GoToEventLogCommand(Command)
	...
	For Each Area In SelectedAreas Do
		...
		If Details = Undefined
			...
			ShowMessageBox(,NStr("ru = 'Выберите строку или ячейку нужного сеанса задания';
										|en = 'Select the job session line or cell.';"));
			Return;
		EndIf;
		StartDate = Details.Get(0);
		EndDate = Details.Get(1);
