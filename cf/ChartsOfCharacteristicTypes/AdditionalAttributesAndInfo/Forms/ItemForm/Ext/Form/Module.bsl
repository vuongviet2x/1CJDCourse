...
Procedure BeforeWrite(Cancel, WriteParameters)
	...
	If Not WriteParameters.Property("DescriptionChangeConfirmed") Then
		If ValueIsFilled(CurrentTitle) And CurrentTitle <> Object.Title Then
			QueryText = NStr("ru = 'Наименование было изменено. Потребуется заново настроить отображение
				                      |дополнительного реквизита в списках и отчетах, а также использование
				                      |в отборах.';
										|en = 'If you change the attribute''s description, you have to configure its view
										|in all of the lists, reports, and filters
										|that include the attribute.';");
			QueryText = StrReplace(QueryText, Chars.LF, " ");
			
			Buttons = New ValueList;
			Buttons.Add("ContinueWrite",            NStr("ru = 'Переименовать';
																|en = 'Rename';"));
			Buttons.Add("ReturnDescription", NStr("ru = 'Отмена';
														|en = 'Cancel';"));
	...
	If Not WriteParameters.Property("WhenDescriptionAlreadyInUse") Then
		...
		If ValueIsFilled(QueryText) Then
			Buttons = New ValueList;
			Buttons.Add("ContinueWrite",            NStr("ru = 'Продолжить запись';
																|en = 'Save';"));
			Buttons.Add("BackToDescriptionInput", NStr("ru = 'Вернуться к вводу наименования';
																|en = 'Edit description';"));
	...
	If Not WriteParameters.Property("WhenNameAlreadyInUse")
		...
		If ValueIsFilled(QueryText) Then
			Buttons = New ValueList;
			Buttons.Add("ContinueWrite",            NStr("ru = 'Продолжить';
																|en = 'Continue';"));
			Buttons.Add("BackToNameInput", NStr("ru = 'Отмена';
														|en = 'Cancel';"));
	...
	If Not WriteParameters.Property("WhenIDForFormulasIsAlreadyUsed")
		...
		If ValueIsFilled(QueryText) Then
			...
			Buttons.Add("ContinueWrite",              NStr("ru = 'Продолжить';
																	|en = 'Continue';"));
			Buttons.Add("BackToIDInput", NStr("ru = 'Отмена';
																	|en = 'Cancel';"));
...
Procedure AdditionalValuesWithWeightOnChange(Item)
		...
		QueryText =
			NStr("ru = 'Очистить введенные весовые коэффициенты?
			           ...
			           |Данные будут записаны.';
						|en = 'Do you want to clear the weight coefficients?
						|
						|The data will be saved.';");
		
		Buttons = New ValueList;
		Buttons.Add("ClearAndWrite", NStr("ru = 'Очистить и записать';
													|en = 'Clear and save';"));
	...
	Else
		QueryText = NStr("ru = 'Данные будут записаны.';
							|en = 'Do you want to save the data?';");
		
		Buttons = New ValueList;
		Buttons.Add("Write", NStr("ru = 'Записать';
										|en = 'Save';"));
		Buttons.Add("Cancel", NStr("ru = 'Отмена';
										|en = 'Cancel';"));
...
#EndIf
	...
	If PageIndex = 0
		...
		WarningText = NStr("ru = 'Элемент не выбран.';
...
Procedure FillChoicePage()
	...
	If PropertyKind = Enums.PropertiesKinds.AdditionalInfo Then
		Items.UnusedAttributes.Title = NStr("ru = 'Неиспользуемые дополнительные сведения';
															|en = 'Unused additional information records';");
	ElsIf PropertyKind = Enums.PropertiesKinds.Labels Then
		Items.UnusedAttributes.Title = NStr("ru = 'Неиспользуемые метки';
															|en = 'Unused labels';");
	Else
		Items.UnusedAttributes.Title = NStr("ru = 'Неиспользуемые дополнительные реквизиты';
															|en = 'Unused additional attributes';");
	EndIf;
	...
	If PropertyKind = PredefinedValue("Enum.PropertiesKinds.AdditionalInfo") Then
		ListPresentation = NStr("ru = 'Неиспользуемые дополнительные сведения';
									|en = 'Unused additional information records';");
	ElsIf PropertyKind = PredefinedValue("Enum.PropertiesKinds.AdditionalAttributes") Then
		ListPresentation = NStr("ru = 'Неиспользуемые дополнительные реквизиты';
									|en = 'Unused additional attributes';");
	ElsIf PropertyKind = PredefinedValue("Enum.PropertiesKinds.Labels") Then
		ListPresentation = NStr("ru = 'Неиспользуемые метки';
									|en = 'Unused labels';");
	EndIf;
...
Procedure WriteObject(QuestionTextVariant, FollowUpHandler, AdditionalParameters = Undefined, WriteParameters = Undefined)
	...
	If QuestionTextVariant = "DeletionMarkEdit" Then
		If Modified Then
			If Object.DeletionMark Then
				QueryText = NStr("ru = 'Для снятия пометки удаления запишите внесенные изменения. Записать данные?';
									|en = 'Save the changes before clearing the deletion mark. Do you want to save the changes?';");
			Else
				QueryText = NStr("ru = 'Для установки пометки удаления запишите внесенные изменения. Записать данные?';
									|en = 'Save the changes before marking for deletion. Do you want to save the changes?';");
			EndIf;
		Else
	...
	If QuestionTextVariant = "GoToValueList" Then
		QueryText = NStr("ru = 'Перед переходом к списку значений данные будут записаны.';
							|en = 'Do you want to save the data and open the list of values?';");
	Else
		QueryText = NStr("ru = 'Данные будут записаны.';
							|en = 'Do you want to save the data?';")
	EndIf;
...
Procedure EditValueFormatCompletion(Text, Context) Export
	...
	If Text <> Undefined Then
		...
		WarningText = NStr("ru = 'Следующие настройки формата автоматически не применяются в большинстве мест:';
									|en = 'The following format settings are not applied automatically in most cases:';");
		...
		For Each Substring In Array Do
			If StrFind(Substring, "ДП=") > 0 Or StrFind(Substring, "DE=") > 0 Then // @Non-NLS
				WarningText = WarningText + Chars.LF
					+ " - " + NStr("ru = 'представление пустой даты';
									|en = 'Blank date presentation';");
				Continue;
			EndIf;
			If StrFind(Substring, "ЧН=") > 0 Or StrFind(Substring, "NZ=") > 0 Then // @Non-NLS
				WarningText = WarningText + Chars.LF
					+ " - " + NStr("ru = 'представление пустого числа';
									|en = 'Blank number presentation';");
				Continue;
			EndIf;
			...
			If StrFind(Substring, "ДЛФ=") > 0 Or StrFind(Substring, "DLF=") > 0 Then // @Non-NLS
				If StrFind(Substring, "ДД") > 0 Or StrFind(Substring, "DD") > 0 Then // @Non-NLS
					WarningText = WarningText + Chars.LF
						+ " - " + NStr("ru = 'длинная дата (месяц прописью)';
										|en = 'Long date (month in words)';");
				EndIf;
			EndIf;
		EndDo;
...
Procedure SetWizardSettings(CurrentPage = Undefined)
	...
	If CurrentPage = Items.SelectAttribute Then
		...
		If PassedFormParameters.PropertyKind =
			...
			Title = NStr("ru = 'Добавление дополнительного сведения';
							|en = 'Add additional information record';");
			ListHeaderTemplate =
				NStr("ru = 'Выберите дополнительное сведение для включения в набор ""%1""';
					|en = 'Select an additional information record to include in the ""%1"" set';");
			RadioButtonHeaderTemplate =
				NStr("ru = 'Выберите вариант добавления дополнительного сведения ""%1"" в набор ""%2""';
					|en = 'Select an option to add the ""%1"" additional information record to the ""%2"" set';");
		ElsIf PassedFormParameters.PropertyKind =
			PredefinedValue("Enum.PropertiesKinds.Labels") Then
			Title = NStr("ru = 'Добавление метки';
							|en = 'Add label';");
			ListHeaderTemplate =
				NStr("ru = 'Выберите метку для включения в набор ""%1""';
					|en = 'Select a label to include in the ""%1"" set';");
			RadioButtonHeaderTemplate =
				NStr("ru = 'Выберите вариант добавления метки ""%1"" в набор ""%2""';
					|en = 'Select an option to add the ""%1"" label to the ""%2"" set';");
		Else
			Title = NStr("ru = 'Добавление дополнительного реквизита';
							|en = 'Add additional attribute';");
			ListHeaderTemplate =
				NStr("ru = 'Выберите дополнительный реквизит для включения в набор ""%1""';
					|en = 'Select an additional attribute to include in the ""%1"" set';");
			RadioButtonHeaderTemplate =
				NStr("ru = 'Выберите вариант добавления дополнительного реквизита ""%1"" в набор ""%2""';
					|en = 'Select an option to add the ""%1"" additional attribute to the ""%2"" set';");
		EndIf;
		
		Items.CommandBarLeft.Enabled = False;
		Items.NextCommand.Title = NStr("ru = 'Далее >';
												|en = 'Next >';");
	...
	Else
		Items.NextCommand.Title = NStr("ru = 'Готово';
												|en = 'Finish';");
		Items.CommandBarLeft.Enabled = True;
	EndIf;
...
Procedure RefreshFormItemsContent(WarningText = "")
	...
	Else
		...
		If OwnerProperties.PropertyKind = Enums.PropertiesKinds.AdditionalInfo Then
			AdjustmentTemplate = NStr("ru = 'Список значений общий со сведением ""%1""';
									|en = 'The value list is shared with the ""%1"" information record';");
		ElsIf OwnerProperties.PropertyKind = Enums.PropertiesKinds.Labels Then
			AdjustmentTemplate = NStr("ru = 'Список значений общий с меткой ""%1""';
									|en = 'The value list is shared with the ""%1"" label';");
		Else
			AdjustmentTemplate = NStr("ru = 'Список значений общий с реквизитом ""%1""';
									|en = 'The value list is shared with the ""%1"" attribute';");
		EndIf;
	...
	Else
		...
		If SetsList.Count() = 0 Then
			...
			If Object.PropertyKind = Enums.PropertiesKinds.AdditionalInfo
				...
				CommentText1 = NStr("ru = 'Сведение не входит в наборы';
										|en = 'The information record is not included in any sets';");
			ElsIf Object.PropertyKind = Enums.PropertiesKinds.Labels Then
				CommentText1 = NStr("ru = 'Метка не входит в наборы';
										|en = 'The label is not included in any sets';");
			Else
				CommentText1 = NStr("ru = 'Реквизит не входит в наборы';
										|en = 'The attribute is not included in any sets';");
			EndIf;
		ElsIf SetsList.Count() < 2 Then
			If Object.PropertyKind = Enums.PropertiesKinds.AdditionalInfo
				Or Object.IsAdditionalInfo Then
				AdjustmentTemplate = NStr("ru = 'Сведение входит в набор: %1';
										|en = 'The information record is included in the set: %1';");
			ElsIf Object.PropertyKind = Enums.PropertiesKinds.Labels Then
				AdjustmentTemplate = NStr("ru = 'Метка входит в набор: %1';
										|en = 'The label is included in the set: %1';");
			Else
				AdjustmentTemplate = NStr("ru = 'Реквизит входит в набор: %1';
										|en = 'The attribute is included in the set: %1';");
			EndIf;
			CommentText1 = StringFunctionsClientServer.SubstituteParametersToString(AdjustmentTemplate, TrimAll(SetsList[0].Presentation));
		Else
			If Object.PropertyKind = Enums.PropertiesKinds.AdditionalInfo
				Or Object.IsAdditionalInfo Then
				AdjustmentTemplate = NStr("ru = 'Сведение входит в %1 %2';
										|en = 'The information record is included in %1 %2';");
			ElsIf Object.PropertyKind = Enums.PropertiesKinds.Labels Then
				AdjustmentTemplate = NStr("ru = 'Метка входит в %1 %2';
										|en = 'The label is included in %1 %2';");
			Else
				AdjustmentTemplate = NStr("ru = 'Реквизит входит в %1 %2';
										|en = 'The attribute is included in %1 %2';");
			EndIf;
			
			StringSets = UsersInternalClientServer.IntegerSubject(SetsList.Count(),
				"", NStr("ru = 'набор,набора,наборов,,,,,,0';
						|en = 'set,sets,,,0';"));
		...
		If Items.SetsAdjustmentComment.Hyperlink Then
			Items.SetsAdjustmentComment.ToolTip = NStr("ru = 'Переход к набору';
																	|en = 'Go to set.';");
		Else
...
Procedure FillActionListOnAddAttribute()
	...
	If PassedFormParameters.PropertyKind = Enums.PropertiesKinds.AdditionalInfo Then
		AddCommon = NStr("ru = 'Добавить сведение как есть (рекомендуется)
			|
			|В этом случае будет возможно отбирать по нему данные разных типов в списках и отчетах.';
			|en = 'Add the information record ""as is"" (recommended)
			|
		...
		MakeBySample = NStr("ru = 'Сделать копию сведения по образцу (с общим списком значений)
			...
			|en = 'Copy the information record from a master record (with a shared value list)
			|
		...
		If AttributeWithAdditionalValuesList Then
			CreateByCopying = NStr("ru = 'Сделать копию сведения
				...
				|en = 'Copy the information record
				|
	...
	Else
		AddCommon = NStr("ru = 'Добавить реквизит как есть (рекомендуется)
			|
			|В этом случае будет возможно отбирать по нему данные разных типов в списках и отчетах.';
			|en = 'Add the attribute ""as is"" (recommended)
			|
		...
		MakeBySample = NStr("ru = 'Сделать копию реквизита по образцу (с общим списком значений)
			...
			|en = 'Copy the attribute from a master attribute (with a shared value list)
			|
...
Procedure SetHyperlinkTitles()
	...
	TemplateDependenceDefined = NStr("ru = 'с условием';
									|en = 'conditionally';");
	TemplateDependenceNotDefined = NStr("ru = 'всегда';
									|en = 'always';");
...
Procedure SetFormHeader()
	...
	If ValueIsFilled(Object.Ref) Then
		If Object.PropertyKind = Enums.PropertiesKinds.AdditionalInfo
			Or Object.IsAdditionalInfo Then
			Title = String(Object.Title) + " " + NStr("ru = '(Дополнительное сведение)';
																|en = '(Additional information record)';");
		ElsIf Object.PropertyKind = Enums.PropertiesKinds.Labels Then
			Title = String(Object.Title) + " " + NStr("ru = '(Метка)';
																|en = '(Label)';");
		Else
			Title = String(Object.Title) + " " + NStr("ru = '(Дополнительный реквизит)';
																|en = '(Additional attribute)';");
		EndIf;
	Else
		If Object.PropertyKind = Enums.PropertiesKinds.AdditionalInfo
			Or Object.IsAdditionalInfo Then
			Title = NStr("ru = 'Дополнительное сведение (создание)';
							|en = 'Additional information record (Create)';");
		ElsIf Object.PropertyKind = Enums.PropertiesKinds.Labels Then
			Title = NStr("ru = 'Метка (создание)';
							|en = 'Label (Create)';");
		Else
			Title = NStr("ru = 'Дополнительный реквизит (создание)';
							|en = 'Additional attribute (Create)';");
		EndIf;
	EndIf;
...
Function IDForFormulasAlreadyUsed(Val IDForFormulas, Val CurrentProperty)
	...
	Refinement = NStr("ru = 'Создать новый идентификатор для формул и продолжить запись?';
					|en = 'Do you want to create a new ID for formulas and continue saving?';");
...
Procedure SetFormatButtonTitle(Form)
	...
	If IsBlankString(Form.Object.FormatProperties) Then
		TitleText = NStr("ru = 'Формат по умолчанию';
								|en = 'Default format';");
	Else
		TitleText = NStr("ru = 'Формат установлен';
								|en = 'Custom format';");
	EndIf;
