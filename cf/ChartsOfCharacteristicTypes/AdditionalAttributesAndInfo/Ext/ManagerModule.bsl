	...
	If ValueIsFilled(IDForFormulas) Then
		...
		If IDByRules Then
			If Not IDForFormulasUnique(IDForFormulas, Ref) Then
				...
				ErrorText = NStr("ru = 'Идентификатор для формул ""%1"" не уникален';
									|en = 'ID for formulas ""%1"" is not unique';");
				ErrorText = StringFunctionsClientServer.SubstituteParametersToString(ErrorText,
					IDForFormulas);
				...
				LanguageCode = Common.DefaultLanguageCode();
				ErrorText = NStr("ru = 'Идентификатор для формул ""%1"" не уникален';
									|en = 'ID for formulas ""%1"" is not unique';", LanguageCode);
				ErrorText = StringFunctionsClientServer.SubstituteParametersToString(ErrorText,
					IDForFormulas);
				EventName = NStr("ru = 'Запись дополнительного реквизита (сведения)';
									|en = 'Save additional attribute or information record';", LanguageCode);
	...
	Try
		...
		If OwnerMetadata = Undefined Then
			Raise StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Настройки свойства %1 не были изменены.
				           |Набор свойств %2 не связан ни с одним владельцем значений свойств.';
							|en = 'The %1 property settings were not changed.
...
Procedure ProcessDataForMigrationToNewVersion(Parameters) Export
	...
	If ObjectsProcessed = 0 And ObjectsWithIssuesCount <> 0 Then
		MessageText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Не удалось обработать некоторые дополнительные реквизиты или сведения (пропущены): %1';
				|en = 'Failed to process (skipped) some additional attributes or information records: %1';"), 
			ObjectsWithIssuesCount);
		Raise MessageText;
	Else
		WriteLogEvent(InfobaseUpdate.EventLogEvent(), EventLogLevel.Information,
			Metadata.ChartsOfCharacteristicTypes.AdditionalAttributesAndInfo,,
			StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Обработана очередная порция дополнительных реквизитов (сведений): %1';
																		|en = 'Yet another batch of additional attributes or information records is processed: %1';"),
				ObjectsProcessed));
	EndIf;
