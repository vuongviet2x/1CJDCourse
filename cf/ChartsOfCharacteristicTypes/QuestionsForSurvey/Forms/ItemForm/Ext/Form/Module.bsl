...
Procedure BeforeWrite(Cancel, WriteParameters)
	...
	If Object.ReplyType = PredefinedValue("Enum.TypesOfAnswersToQuestion.Number") Then
		...
		If Object.MinValue > Object.MaxValue Then
			CommonClient.MessageToUser(
				NStr("ru = 'Минимально допустимое значение не может быть больше чем максимальное.';
					|en = 'The minimum allowed value cannot be greater than the maximum allowed value.';"),,
				"Object.MinValue");
			Cancel = True;
		EndIf;
		...
		If Object.ShouldUseMinValue And Object.ShouldUseMaxValue Then
			If Object.MinValue = Object.MaxValue Then
				CommonClient.MessageToUser(
					NStr("ru = 'Минимально допустимое значение не может быть равно максимальному.';
						|en = 'The minimum allowed value cannot be equal to the maximum allowed value.';"),,
					"Object.MinValue");
				Cancel = True;
			EndIf;
		EndIf;
		...
		If Object.ShouldShowRangeSlider Then
			...
			CommonClient.MessageToUser(
				NStr("ru = 'Шаг полосы не может быть больше разницы максимального и минимального значений.';
					|en = 'A slider increment cannot be greater than the difference between the maximum and minimum values.';"),,
	...
	ElsIf Object.ReplyType = PredefinedValue("Enum.TypesOfAnswersToQuestion.String") Then	
		...
		If StringLength = 0 Then
			CommonClient.MessageToUser(NStr("ru = 'Не заполнено значение длины строки.';
															|en = 'The string length is not specified.';"),,"StringLength");
			Cancel = True;
		EndIf;
...
Procedure PresentationStartChoice(Item, ChoiceData, StandardProcessing)
	...
	CommonClient.ShowMultilineTextEditingForm(ClosingNotification1, Item.EditText, NStr("ru = 'Формулировка';
																																	|en = 'Wording';"));
...
Procedure HintsRangeBeforeEditEnd(Item, NewRow, CancelEdit, Cancel)
	...
	Else
		...
		EndIf;
		CommonClient.MessageToUser(
			NStr("ru = 'Такое значение уже присутствует в диапазоне подсказок.';
				|en = 'This value already exists in a hint range.';"),, NameOfFormField);
	EndIf;
...
Procedure AnswersOptionsTableAvailability(Form)
	...
	If Form.Object.Ref.IsEmpty() Then
		...
		Form.AnswersOptionsInfo                       = NStr("ru = 'Для редактирования вариантов ответов запишите вопрос для анкетирования';
																	|en = 'Before you start editing the responses, save the question';");
	Else
		Form.Items.TableAnswersOptions.ReadOnly = False;
		Form.AnswersOptionsInfo                      = NStr("ru = 'Варианты ответов на вопрос:';
																	|en = 'Response options:';");
	EndIf; 
	...
	For Each CurrentRow In TableOfRanges Do
		...
		If CurrentRow.ValueUpTo = NumericalQuestionHintsRangeCapValue() Then
			If CurrentIndex = 0 Then
				CurrentRow.ValuePresentation = NStr("ru = 'любое значение';
															|en = 'any value';");
				Continue;
			EndIf;
			...
			Else
				CurrentRow.ValuePresentation = StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'более %1';
						|en = 'greater than %1';"), String(PreviousString.ValueUpTo));
			EndIf;
		Else
			If CurrentIndex = 0 Then
				CurrentRow.ValuePresentation = StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = '%1 и менее';
						|en = '%1 or less';"), String(CurrentRow.ValueUpTo));
					Continue;
			EndIf;
			PreviousString = TableOfRanges[CurrentIndex - 1];
