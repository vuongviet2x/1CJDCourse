...
Procedure ProcessDataForMigrationToNewVersion(Parameters) Export
	...
	If ObjectsProcessed = 0 And ObjectsWithIssuesCount <> 0 Then
		MessageText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Не удалось обработать некоторые вопросы для анкетирования (пропущены): %1';
				|en = 'Couldn''t process (skipped) some survey questions: %1';"), 
		...
		WriteLogEvent(InfobaseUpdate.EventLogEvent(), EventLogLevel.Information,
			Metadata.ChartsOfCharacteristicTypes.QuestionsForSurvey,,
				StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Обработана очередная порция вопросов для анкетирования: %1';
						|en = 'Another batch of survey questions is processed: %1';"),
					ObjectsProcessed));
	EndIf;
