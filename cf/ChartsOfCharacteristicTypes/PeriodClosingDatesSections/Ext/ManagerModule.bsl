...
Function ClosingDatesSectionsProperties() Export
	...
	For Each Section In Sections Do
		If Not ValueIsFilled(Section.Name) Then
			Raise ErrorTitle + NStr("ru = 'Имя раздела дат запрета не заполнено.';
													|en = 'Name is required for the period-end closing date section.';");
		EndIf;
		...
		If ClosingDatesSections.Get(Section.Name) <> Undefined Then
			Raise ErrorTitle + StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Имя раздела дат запрета ""%1"" уже определено.';
					|en = '""%1"" period-end closing date section already has a name.';"),
				Section.Name);
		EndIf;
		
		If Not ValueIsFilled(Section.Id) And Section.Name <> "SingleDate" Then
			Raise ErrorTitle + StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Идентификатор раздела дат запрета ""%1"" не заполнен.';
					|en = 'ID is required for ""%1"" period-end closing date section.';"),
				Section.Name);
		EndIf;
...
Function SectionProperties(Ref, Val Name = "", Val Presentation = "", Val ObjectsTypes = Undefined) Export
	...
	SectionProperties.Insert("Presentation", ?(IsBlankString(Presentation), NStr("ru = 'Общая дата';
																					|en = 'Common date';"), Presentation));
