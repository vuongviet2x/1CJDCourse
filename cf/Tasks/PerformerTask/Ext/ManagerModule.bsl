...
Procedure PresentationGetProcessing(Data, Presentation, StandardProcessing)
	...
	Description = ?(IsBlankString(Data.Description), NStr("ru = 'Без описания';
															|en = 'No details';"), Data.Description);
	Date = Format(Data.Date, ?(GetFunctionalOption("UseDateAndTimeInTaskDeadlines"), "DLF=DT", "DLF=D"));
	Presentation = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = '%1 от %2';
																				|en = '%1, created on %2';"), Description, Date);
	StandardProcessing = False;
