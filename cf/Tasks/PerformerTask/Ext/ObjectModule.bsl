...
Procedure FillCheckProcessing(Cancel, CheckedAttributes)
	...
	If Executed And TaskWasExecuted <> True And Not AddressingAttributesAreFilled() Then
		...
		Common.MessageToUser(
			NStr("ru = 'Укажите исполнителя задачи.';
				|en = 'Specify a task assignee.';"),,,
	...
	If TaskDueDate <> '00010101' And StartDate > TaskDueDate Then
		Common.MessageToUser(
			NStr("ru = 'Дата начала исполнения не должна превышать крайний срок.';
				|en = 'Execution start date cannot be later than the deadline.';"),,,
...
Procedure BeforeWrite(Cancel)
	...
	If Not InitialAttributes.Executed And Executed Then
		...
		If BusinessProcessState = Enums.BusinessProcessStates.Suspended Then
			Raise NStr("ru = 'Нельзя выполнять задачи остановленных бизнес-процессов.';
									|en = 'Cannot perform tasks of suspended business processes.';");
		EndIf;
	...
	ElsIf Not DeletionMark And InitialAttributes.Executed And Executed Then
			Common.MessageToUser(
				NStr("ru = 'Эта задача уже была выполнена ранее.';
					|en = 'This task is already completed.';"),,,, Cancel);
			Return;
	EndIf;
