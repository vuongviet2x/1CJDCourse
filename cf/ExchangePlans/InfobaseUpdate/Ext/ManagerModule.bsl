...
Function NodeInQueue(Queue, Temporary = False) Export
	...
	If TypeOf(Queue) <> Type("Number") Or Queue = 0 Then
		Raise StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Невозможно получить узел плана обмена %1, т.к. не передан номер очереди.';
				|en = 'Cannot get the node of exchange plan %1 because the position in queue is not provided.';"),
			"InfobaseUpdate");
	EndIf;
	...
	Else
		...
		Try
			...
			Else
				...
				ObjectNode.Description = QueueString + ?(Temporary, " " + NStr("ru = 'Новая для перезапуска';
																					|en = 'New for restart';"), "");
				ObjectNode.Write();
				Node = ObjectNode.Ref;
			EndIf;
