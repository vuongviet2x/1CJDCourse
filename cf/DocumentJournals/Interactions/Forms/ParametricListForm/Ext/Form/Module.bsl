...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If OnlyEmail Then
		TitleParticipantsMail =  NStr("ru = 'Кому, от';
										|en = 'To, from';");
	...
	If TypeOf(Parameters.Filter) = Type("Structure") Then
		...
		TitleTemplate1 = NStr("ru = 'Взаимодействия по %1';
								|en = 'Interactions on: %1';");
...
Procedure InteractionsTreeBeforeDeleteRow(Item, Cancel)
	...
	If Items.InteractionsTree.SelectedRows.Count() > 0 Then
		...
		If HasItemsMarkedForDeletion Then
			QueryText = NStr("ru = 'Снять с выделенных элементов пометку на удаление?';
								|en = 'Clear deletion mark from the selected items?';");
		Else
			QueryText = NStr("ru = 'Пометить выделенные строки на удаление?';
								|en = 'Mark the selected lines for deletion?';");
		EndIf;
...
Procedure PagesManagementServer()
	...
	If InTreeStructure Then
		...
		Commands.SwitchViewMode.ToolTip = NStr("ru = 'Установить режим просмотра в виде списка';
	...
	Else
		...
		Commands.SwitchViewMode.ToolTip = NStr("ru = 'Установить режим просмотра в виде дерева';
															|en = 'Switch to Tree view';");
...
Procedure FillInteractionsTree()
	...
	TitleTemplate1 = NStr("ru = 'Тип взаимодействий: %1';
							|en = 'Interaction category: %1';");
