...
Procedure FoldersBeforeAddRow(Item, Cancel, Copy, Parent, Var_Group)
	...
	If CurrentData.HasEditPermission = 0 Then
		Raise(NStr("ru = 'Недостаточно прав для создания папки.';
								|en = 'Insufficient rights to create a folder.';"), ErrorCategory.AccessViolation);
	EndIf;
...
Procedure FoldersBeforeDeleteRow(Item, Cancel)
	...
	QueryText = StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = 'Удалить папку ""%1"" и переместить все ее содержимое в папку ""Удаленные""?';
			|en = 'Delete the ""%1"" folder and move all its contents to the ""Trash"" folder?';"),
...
Procedure SendReceiveEmailExecute(Command)
	...
	If SendReceiveEmailInProgress Then
		MessageText = NStr("ru = 'В настоящий момент отправка и получение почты уже выполняется.';
								|en = 'Mail synchronization in progress.';");
		CommonClient.MessageToUser(MessageText);
		Return;
	EndIf;
	
	If DateOfPreviousEmailReceiptSending + 15 > CommonClient.SessionDate() Then
		MessageText = NStr("ru = 'С момента завершения предыдущей отправки и получения почты прошло менее 15 секунд. Повторите позже.';
								|en = 'It''s been less than 15 seconds since the last mail sync. Try again later.';");
		CommonClient.MessageToUser(MessageText);
		Return;
	EndIf;
	
	If DateOfPreviousExecutionOfSendReceiveEmailCommand + 15 > CommonClient.SessionDate() Then
		MessageText = NStr("ru = 'С момента предыдущего выполнения команды отправки и получения почты прошло менее 15 секунд. Повторите позже.';
								|en = 'It''s been less than 15 seconds since you run mail sync. Try again later.';");
		CommonClient.MessageToUser(MessageText);
		Return;
	EndIf;
...
Procedure Reply(Command)
	...
	If CorrectChoice(Items.List.Name, True) Then
		...
		If TypeOf(CurrentInteraction) <> Type("DocumentRef.IncomingEmail") Then
			ShowMessageBox(, NStr("ru = 'Команда ""Ответить"" может быть выполнена только для входящего письма.';
											|en = 'You can use ""Reply"" only for incoming messages.';"));
			Return;
		EndIf;
	Else
		Return;
...
Procedure ReplyToAll(Command)
	...
	If CorrectChoice(Items.List.Name, True) Then
		CurrentInteraction = Items.List.CurrentData.Ref;
		If TypeOf(CurrentInteraction) <> Type("DocumentRef.IncomingEmail") Then
			ShowMessageBox(, NStr("ru = 'Команда ""Ответить всем"" может быть выполнена только для входящего письма.';
											|en = 'You can use ""Reply all"" only for incoming messages.';"));
			Return;
		EndIf;
	Else
		Return;
...
Procedure ForwardMail(Command)
	...
	If CorrectChoice(Items.List.Name, True) Then
		...
		If TypeOf(CurrentInteraction) <> Type("DocumentRef.OutgoingEmail") 
			...
			ShowMessageBox(, NStr("ru = 'Команда ""Переслать"" может быть выполнена только для писем.';
											|en = 'You can use ""Forward"" only for mail messages.';"));
			Return;
		EndIf;
	Else
		Return;
...
Procedure AddToTabs(Command)
	...
	CurrentItemName = CurrentItem.Name;
	If StrStartsWith(CurrentItemName, "List") And Not CorrectChoice(CurrentItemName) Then
		ShowMessageBox(, NStr("ru = 'Не выбран элемент для добавления в закладки.';
										|en = 'Select an item you want to add to bookmarks.';"));
		Return;
	EndIf;
	...
	If Not Result.ItemAdded Then
		...
		Return;
	EndIf;
	ShowUserNotification(NStr("ru = 'Добавлены в закладки:';
										|en = 'Items added to bookmarks:';"),
...
Procedure DeferReviewExecute(Command)
	...
	ShowInputDate(OnCloseNotifyHandler, ProcessingDate, NStr("ru = 'Отработать после';
																		|en = 'Snooze till';"), DateFractions.DateTime);
...
Procedure MoveToFolder(Command)
	...
	If CurrentItemName = "Folders" Then
		If TypeOf(FoldersCurrentData.Value) = Type("CatalogRef.EmailAccounts") 
			...
			ShowMessageBox(, NStr("ru = 'Команда не может быть выполнена для данного объекта';
											|en = 'Cannot execute the command on this object';"));
			Return;
		ElsIf FoldersCurrentData.HasEditPermission = 0 Then
			Raise(NStr("ru = 'Недостаточно прав для изменения папок.';
									|en = 'Insufficient rights to edit folders.';"), ErrorCategory.AccessViolation);
		EndIf;
	EndIf;
...
Procedure ForwardAsAttachment(Command)
	...
	Else
		...
		MessageText = NStr("ru = 'Пересылать как вложения можно только отправленные и полученные письма.';
								|en = 'You can forward as an attachment only messages you sent or received.';");
...
Procedure ChangeFilterList(TableName, DataForProcessing)
	...
	ElsIf TableName = "Tabs" Then
		...
		NavigationPanelTitle   = StringFunctionsClientServer.SubstituteParametersToString(
			TitleTemplate1, NStr("ru = 'Закладка';
									|en = 'Bookmark';"), DataForProcessing.Value);
...
Procedure ManageVisibilityOnSwitchNavigationPanel()
	...
	If NavigationPanelHidden Then
		...
		ChangeNavigationPanelDisplayCommand.ToolTip = NStr("ru = 'Показать панель навигации';
																	|en = 'Show navigation panel';");
		ChangeNavigationPanelDisplayCommand.Title = NStr("ru = 'Показать панель навигации';
																	|en = 'Show navigation panel';");
	Else
		Items.EditNavigationPanelView.Picture = PictureLib.LeftArrow;
		ChangeNavigationPanelDisplayCommand.ToolTip = NStr("ru = 'Скрыть панель навигации';
																	|en = 'Hide navigation panel';");
		ChangeNavigationPanelDisplayCommand.Title = NStr("ru = 'Скрыть панель навигации';
																	|en = 'Hide navigation panel';");
	EndIf;
...
Procedure SetNavigationPanelViewTitle(FilterValue = Undefined)
	...
	If NavigationPanelHidden Then
		...
		Items.SelectNavigationOption.ToolTip = ?(IsBlankString(NavigationPanelTitle),
		                                              NStr("ru = 'Не указан';
															|en = 'Not specified';") + NavigationPanelTitleTooltip,
		                                              NavigationPanelTitleTooltip);
	Else
	
		If Items.NavigationPanelPages.CurrentPage = Items.PropertiesPage Then
			Items.SelectNavigationOption.Title = NStr("ru = 'По';
															|en = 'By';") + " " + CurrentPropertyPresentation;
		...
		ElsIf Items.NavigationPanelPages.CurrentPage = Items.TabsPage Then
			...
			Items.SelectNavigationOption.Title = NStr("ru = 'По закладкам';
															|en = 'By bookmark';");
			Items.SetNavigationMethodByTabs.Check = True;
			
		ElsIf Items.NavigationPanelPages.CurrentPage = Items.EmailSubjectPage Then
			
			Items.SelectNavigationOption.Title = NStr("ru = 'По предметам';
															|en = 'By topic';");
			Items.SetNavigationMethodBySubject.Check = True;
			
		ElsIf Items.NavigationPanelPages.CurrentPage = Items.ContactPage Then
			
			Items.SelectNavigationOption.Title = NStr("ru = 'По контактам';
															|en = 'By contact';");
			Items.SetNavigationMethodByContact.Check = True;
			
		ElsIf Items.NavigationPanelPages.CurrentPage = Items.FoldersPage Then
			
			Items.SelectNavigationOption.Title = NStr("ru = 'По папкам';
															|en = 'By folder';");
			Items.SetNavigationMethodByFolders.Check = True;
			
		ElsIf Items.NavigationPanelPages.CurrentPage = Items.CategoriesPage Then
			
			Items.SelectNavigationOption.Title = NStr("ru = 'По категориям';
															|en = 'By category';");
			Items["AdditionalButtonCategoryNavigationOptionSelection"].Check = True;
			
		EndIf;
		
		Items.SelectNavigationOption.ToolTip = NStr("ru = 'Выберите вариант навигации';
														|en = 'Select navigation option';");
		
	EndIf;
	
	Items.NavigationPanelGroup.Title = ?(IsBlankString(NavigationPanelTitle), 
	                                           NStr("ru = 'Отбор не установлен';
													|en = 'Filter not set';"),
	                                           NStr("ru = 'Установлен отбор';
													|en = 'Filter set';"));
...
Procedure AddRowAll(FormDataCollection, PictureNumber = 0)
	...
	NewRow.Presentation = NStr("ru = 'Все';
									|en = 'All';");
...
Procedure FillPropertiesTree(CommandName = "")
	...
	Query.SetParameter("NotSpecifiedPresentation", NStr("ru = 'Не указан';
															|en = 'Not specified';"));
	...
	For Each String In ParentString.Rows Do
		...
		If InteractionsClientServer.IsInteraction(String.Value) Then
			...
			NewRow.Presentation = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = '%1 от %2 %3';
					|en = '%1, %2 %3';"),
				?(IsBlankString(DetailsRow.Subject),NStr("ru = 'Тема не указана';
														|en = 'Subject not specified';"), DetailsRow.Subject),
...
Procedure AfterFillNavigationPanel(SetDontTestNavigationPanelActivationFlag = True)
	...
	ElsIf Items.NavigationPanelPages.CurrentPage = Items.PropertiesPage Then
		...
		If CurrentData = Undefined Then
			Items.Properties.CurrentRow = FindStringInFormDataTree(Properties,NStr("ru = 'Все';
																							|en = 'All';"),"Value",False);
			CurrentData = Properties.FindByID(Items.Properties.CurrentRow);
		EndIf;
	...
	ElsIf Items.NavigationPanelPages.CurrentPage = Items.CategoriesPage Then
		...
		If CurrentData = Undefined Then
			Items.Categories.CurrentRow = FindRowInCollectionFormData(Categories,NStr("ru = 'Все';
																								|en = 'All';"),"Value");
			CurrentData = Categories.FindByID(Items.Categories.CurrentRow);
		EndIf;
...
Procedure AddToNavigationPanel()
	...
	While Selection.Next() Do
		...
		ItemButtonSubmenu.Title = NStr("ru = 'По';
												|en = 'By';") + " " + Selection.Presentation;
	...
	If AddlAttributesPropertiesTableOfBooleanType.Count() > 0 Then
		...
		ItemButtonSubmenu.Type = FormButtonType.CommandBarButton;
		ItemButtonSubmenu.CommandName = NewCommand.Name;
		ItemButtonSubmenu.Title = NStr("ru = 'По категориям';
												|en = 'By categories';");
...
Function AddToTabsServer(Val DataForProcessing, FormItemName)
	...
	If StrStartsWith(FormItemName, "List") Then
		...
		If InteractionsList.Count() = 0 Then
			Result.ErrorMessageText1 = NStr("ru = 'Не выбран элемент для добавления в закладки.';
													|en = 'Select an item you want to add to bookmarks.';");
			Return Result;
		EndIf;
		...
		TabDescription = ?(OnlyEmail, NStr("ru = 'Избранные письма';
													|en = 'Favorite Mails';"), NStr("ru = 'Избранные взаимодействия';
																					|en = 'Favorite interactions';"));
		If InteractionsList.Count() > 1 Then
			Text = ?(OnlyEmail, NStr("ru = 'Выбранные письма (%1)';
										|en = 'Selected mails (%1)';"), NStr("ru = 'Выбранные взаимодействия (%1)';
																				|en = 'Selected interactions (%1)';"));
	...
	Else
		...
		If DataForProcessing.Value = "AllValues" Then
			Result.ErrorMessageText1 = NStr("ru = 'Закладку без отбора создать нельзя.';
													|en = 'Cannot create a bookmark without a filter.';");
			Return Result;
		EndIf;
		...
		If FormItemName = "NavigationPanelSubjects" Then
			...
			If Common.RefTypeValue(DataForProcessing.Value) Then
				TabDescription       = NStr("ru = 'Предмет';
													|en = 'Topic';") + " = " + String(DataForProcessing.Value); 
				Text = ?(OnlyEmail, NStr("ru = 'Письма по предмету %1';
											|en = 'Mails on topic %1';"), NStr("ru = 'Взаимодействия по предмету %1';
																					|en = 'Interactions on topic %1';"));
		...
		ElsIf FormItemName = "Properties" Then
			...
			If TypeOf(DataForProcessing.Value) = Type("String") 
				And DataForProcessing.Value = "NotSpecified" Then
				TabDescription       = CurrentPropertyDescription + " " + NStr("ru = 'не указан';
																						|en = 'not specified';");
				Result.ItemPresentation = ?(OnlyEmail, NStr("ru = 'Письма';
																		|en = 'Mail messages';"), NStr("ru = 'Взаимодействия';
																							|en = 'Interactions';"));
			Else
				TabDescription       = CurrentPropertyDescription + " = " + String(DataForProcessing.Value);
				Result.ItemPresentation = ?(OnlyEmail, 
				                                   NStr("ru = 'Письма с заданным свойством: %1';
														|en = 'Mail messages with the property: %1';"), 
				                                   NStr("ru = 'Взаимодействия с заданным свойством: %1';
														|en = 'Interactions with the property: %1';"));
		...
		ElsIf FormItemName = "Categories" Then
			...
			TabDescription       = NStr("ru = 'Входит в категорию';
												|en = 'Included in category';") + " " + String(DataForProcessing.Value);
			Result.ItemPresentation = ?(OnlyEmail, NStr("ru = 'Письма из категории: %1';
																	|en = 'Mail messages in category: %1';"), NStr("ru = 'Взаимодействия из категории: %1';
																											|en = 'Interactions in category: %1';"));
			Result.ItemPresentation = 
				StringFunctionsClientServer.SubstituteParametersToString(Result.ItemPresentation, DataForProcessing.Value);
		...
		ElsIf FormItemName = "NavigationPanelContacts" Then
			...
			If Common.RefTypeValue(DataForProcessing.Value) Then
				TabDescription       = NStr("ru = 'Контакт';
													|en = 'Contact';") + " = " + String(DataForProcessing.Value); 
				Text = ?(OnlyEmail, NStr("ru = 'Письма по контакту %1';
											|en = 'Email conversations with: %1';"), NStr("ru = 'Взаимодействия по контакту %1';
																					|en = 'Interactions with: %1';"));
				Result.ItemPresentation = 
		...
		ElsIf FormItemName = "Folders" Then
			...
			TabDescription       = NStr("ru = 'В папке';
												|en = 'In folder';") + " " + String(DataForProcessing.Value);
			Result.ItemPresentation = ?(OnlyEmail, NStr("ru = 'Письма в папке: %1';
																	|en = 'Mail messages in folder: %1';"), NStr("ru = 'Взаимодействия в папке: %1';
																									|en = 'Interactions in folder: %1';"));
	...
	While Selection.Next() Do
		If ValueInXML(SettingsComposer.GetSettings()) =  ValueInXML(Selection.SettingsComposer.Get()) Then
			Result.ErrorMessageText1 = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Закладка с такими настройками уже существует : %1';
																											|en = 'A bookmark with the same settings already exists: %1';"),
...
Procedure ExecuteFullTextSearch()
	...
	If Not Result.HasError  Then
		...
		NotificationText1 = StringFunctionsClientServer.SubstituteParametersToString(
			?(OnlyEmail, NStr("ru = 'Найдено писем: %1.';
								|en = 'Found emails: %1.';"), NStr("ru = 'Найдено взаимодействий: %1.';
																	|en = 'Found business interactions: %1.';")), 
			String(Result.FoundItemsCount2));
		ShowUserNotification(NStr("ru = 'Результаты поиска';
											|en = 'Search results';"),, NotificationText1);
		CurrentData = Items.List.CurrentData;
		If CurrentData <> Undefined Then
			FillInTheDescriptionFoundByFullTextSearch(Items.List.CurrentData.Ref);
		Else
...
Function InteractionSearchResultFullTextSearch()
	...
	If SearchResultsList.TooManyResults() Then
		...
		Result.ErrorText = NStr("ru = 'Слишком много результатов, уточните запрос.';
	...
	If SearchResultsList.TotalCount() = 0 Then
		...
		Result.ErrorText         = NStr("ru = 'Ничего не найдено.';
											|en = 'No result found.';");
		Return Result;
...
Procedure FillInTheDescriptionFoundByFullTextSearch(Interaction)
	...
	If InteractionsClientServer.IsAttachedInteractionsFile(TableRowWithDetails.Value) Then
		TextFound = NStr("ru = 'Найдено в присоединенном файле %1.';
							|en = 'Found in attachment %1.';");
	Else
		TextFound = NStr("ru = 'Найдено в %1.';
							|en = 'Found in %1.';");
	EndIf;
...
Procedure GenerateImportantContactsOnlyDecoration()
	...
	If ImportantContactsOnly Then
		TitleText = StringFunctions.FormattedString(NStr("ru = 'Контакты с взаимодействиями по отбору, <a href = ""%1"">изменить</a>';
																	|en = 'Show topics of filtered interactions. <a href = ""%1"">Click to change</a>.';"), 
		                                                        "ChangeImportantContactsOnly");
	Else
		TitleText = StringFunctions.FormattedString(NStr("ru = 'Отображаются все контакты, <a href = ""%1"">изменить</a>';
																	|en = 'Show all topics. <a href = ""%1"">Click to change</a>.';"),
		                                                        "ChangeImportantContactsOnly");
	EndIf;
...
Procedure GenerateImportantSubjectsOnlyDecoration()
	...
	If ImportantSubjectsOnly Then
		TitleText = StringFunctions.FormattedString(NStr("ru = 'Предметы с взаимодействиями по отбору, <a href = ""%1"">изменить</a>';
																	|en = 'Show topics of filtered interactions. <a href = ""%1"">Click to change</a>.';"), 
		                                                        "ChangeImportantSubjectsOnly");
	Else
		TitleText = StringFunctions.FormattedString(NStr("ru = 'Отображаются все предметы, <a href = ""%1"">изменить</a>';
																	|en = 'Show all topics. <a href = ""%1"">Click to change</a>.';"), 
		                                                        "ChangeImportantSubjectsOnly");
	EndIf;
